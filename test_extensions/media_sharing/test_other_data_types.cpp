                     ...
                     TSharedHandleType sharedHandle)
{
    ...
    "image2d_t imageOut, " NL
    "                            sampler_t sampler, __global int *imageRes)" NL
    "{" NL "  int w = get_global_id(0);" NL "  int h = get_global_id(1);" NL
    "  int width = get_image_width(imageIn);" NL
    "  int height = get_image_height(imageOut);" NL
    "  float4 color0 = read_imagef(imageIn, sampler, (int2)(w,h)) - 0.2f;" NL
    "  float4 color1 = read_imagef(imageIn, sampler, (float2)(w,h)) - 0.2f;" NL
    "  color0 = (color0 == color1) ? color0: (float4)(0.5, 0.5, 0.5, 0.5);" NL
    "  write_imagef(imageOut, (int2)(w,h), color0);" NL
    ...
    while (deviceWrapper->AdapterNext())
    {
        ...
        clCommandQueueWrapper cmdQueue =
        clCreateCommandQueueWithProperties(ctx, gDeviceIDdetected, 0, &error);
        if (error != CL_SUCCESS)
        {
        ...
        if (!MediaSurfaceCreate(
            ...
            surfaceSrc, (sharedHandle == SHARED_HANDLE_ENABLED) ? true : false,
            &objectSrcHandle))
        {
        ...
        if (!MediaSurfaceCreate(
            adapterType, width, height, surfaceFormat, *deviceWrapper,
            surfaceDst, (sharedHandle == SHARED_HANDLE_ENABLED) ? true : false,
            &objectDstHandle))
        {
            log_error("Media surface creation failed for %i adapter\n",
                      deviceWrapper->AdapterIdx());
        ...
        for (size_t frameIdx = 0; frameIdx < iterationNum; ++frameIdx)
        {
            ...
            cmdQueue, static_cast<cl_uint>(memObjList.size()), &memObjList[0],
            0, 0, 0);
            if (error != CL_SUCCESS)
            {
            ...
            { // read operation
                ...
                error =
                clEnqueueReadImage(cmdQueue, objectSrcShared, CL_TRUE, origin,
                                   region, 0, 0, &out[0], 0, 0, 0);
                if (error != CL_SUCCESS)
                {
                ...
                {
                    log_error(
                    "Frame idx: %i, OCL object is different then expected\n",
                    frameIdx);
                    result.ResultSub(CResult::TEST_FAIL);
                }
            }
            ...
            { // kernel operations
                ...
                error =
                clEnqueueReadBuffer(cmdQueue, imageRes, CL_TRUE, 0, bufferSize,
                                    &imageResOut[0], 0, NULL, NULL);
                if (error != CL_SUCCESS)
                {
            ...
            { // map operation
                ...
                cmdQueue, objectDstShared, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE,
                mapOrigin, mapRegion, &rowPitch, &slicePitch, 0, 0, 0, &error);
                if (error != CL_SUCCESS)
                {
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    if (other_data_types<cl_half>(
        deviceID, context, queue, num_elements, 10, 64, 64, CL_ADAPTER_D3D9_KHR,
        SURFACE_FORMAT_G16R16F, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        ...
        CL_ADAPTER_D3D9_KHR, SURFACE_FORMAT_A8B8G8R8, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        ...
        CL_ADAPTER_D3D9_KHR, SURFACE_FORMAT_X8B8G8R8, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        ...
        CL_ADAPTER_D3D9_KHR, SURFACE_FORMAT_A8R8G8B8, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 256, 256,
        CL_ADAPTER_D3D9_KHR, SURFACE_FORMAT_X8R8G8B8, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_float>(
        deviceID, context, queue, num_elements, 10, 32, 1024,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_G32R32F, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_float>(
        deviceID, context, queue, num_elements, 10, 32, 1024,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_G32R32F, SHARED_HANDLE_ENABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_half>(
        ...
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_G16R16F, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_ushort>(
        deviceID, context, queue, num_elements, 10, 256, 256,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_G16R16, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 128, 64,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_A8B8G8R8, SHARED_HANDLE_DISABLED)
        != 0)
    {
        log_error(
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 128, 64,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_A8B8G8R8, SHARED_HANDLE_ENABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 16, 512,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_X8B8G8R8, SHARED_HANDLE_DISABLED)
        != 0)
    {
        log_error(
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 16, 512,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_X8B8G8R8, SHARED_HANDLE_ENABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 512, 16,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_A8R8G8B8, SHARED_HANDLE_DISABLED)
        != 0)
    {
        log_error(
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 512, 16,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_A8R8G8B8, SHARED_HANDLE_ENABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 256, 256,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_X8R8G8B8, SHARED_HANDLE_DISABLED)
        != 0)
    {
        log_error(
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 256, 256,
        CL_ADAPTER_D3D9EX_KHR, SURFACE_FORMAT_X8R8G8B8, SHARED_HANDLE_ENABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_half>(
        deviceID, context, queue, num_elements, 10, 64, 64, CL_ADAPTER_DXVA_KHR,
        SURFACE_FORMAT_G16R16F, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_ushort>(
        deviceID, context, queue, num_elements, 10, 64, 128,
        CL_ADAPTER_DXVA_KHR, SURFACE_FORMAT_A16B16G16R16, SHARED_HANDLE_ENABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 128, 64,
        CL_ADAPTER_DXVA_KHR, SURFACE_FORMAT_A8B8G8R8, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 16, 512,
        CL_ADAPTER_DXVA_KHR, SURFACE_FORMAT_X8B8G8R8, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 512, 16,
        CL_ADAPTER_DXVA_KHR, SURFACE_FORMAT_A8R8G8B8, SHARED_HANDLE_DISABLED)
        != 0)
    {
    ...
    if (other_data_types<cl_uchar>(
        deviceID, context, queue, num_elements, 10, 256, 256,
        CL_ADAPTER_DXVA_KHR, SURFACE_FORMAT_X8R8G8B8, SHARED_HANDLE_DISABLED)
        != 0)
    {
