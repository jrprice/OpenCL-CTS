...
bool ExtensionCheck(const std::string &extension, cl_device_id deviceID)
{
    ...
    error =
    clGetDeviceInfo(deviceID, CL_DEVICE_EXTENSIONS, size, &extensions[0], 0);
    if (error != CL_SUCCESS)
    {
        print_error(error, "clGetDeviceInfo failed\n");
        return false;
    ...
    if (extensions.find(extension) != std::string::npos) return true;
                                   ...
                                   std::string &contextFunction)
{
    switch (contextCreateFunction)
    {
        ...
        default:
            ...
            log_error(
            "FunctionContextCreateToString(): Unknown create function enum!");
            break;
    }
}
        ...
        for (unsigned int j = 0; j < width; ++j)
        {
            ...
            switch (channelNum)
            {
                ...
                case 4:
                    ...
                    static_cast<cl_uchar>(valueMin + valueY + valueAdd));
                    data.push_back(
                    static_cast<cl_uchar>(valueMin + valueX / 2 + valueAdd));
                   ...
                   void *shareHandleExp)
{
    ...
    switch (adapterType)
    {
        ...
        case CL_ADAPTER_DXVA_KHR: {
            ...
            {
                log_error(
                "clGetImageInfo(CL_MEM_DX9_MEDIA_SURFACE_INFO_KHR) failed\n");
                result = false;
            }
            ...
            {
                log_error(
                "clGetImageInfo(CL_MEM_DX9_MEDIA_ADAPTER_TYPE_KHR) failed\n");
                result = false;
            }
                      ...
                      const cl_image_format imageFormatCheck)
{
    ...
    {
        log_error(
        "Invalid image format number returned by clGetSupportedImageFormats\n");
        return false;
    }
