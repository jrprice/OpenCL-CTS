    ...
    while (deviceWrapper->AdapterNext())
    {
        ...
        switch (functionCreate)
        {
            ...
            case CONTEXT_CREATE_FROM_TYPE:
                ...
                &contextProperties[0], gDeviceTypeSelected, NULL, NULL, &error);
                break;
            default:
        ...
        *(static_cast<CD3D9SurfaceWrapper *>(surface.get()));
        ...
        cmdQueue, static_cast<cl_uint>(memObjList.size()), &memObjList.at(0), 0,
        NULL, &event);
        if (error != CL_SUCCESS)
        {
             ...
             ++contextFuncIdx)
        {
            ...
            for (size_t formatIdx = 0; formatIdx < formats.size(); ++formatIdx)
            {
                     ...
                     ++sharedHandleIdx)
                {
                    ...
                    if (context_create(
                        deviceID, context, queue, num_elements, WIDTH, HEIGHT,
