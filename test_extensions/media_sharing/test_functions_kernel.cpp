                     ...
                     TSharedHandleType sharedHandle)
{
    ...
    "image2d_t planeOut, " NL
    "                            sampler_t sampler, __global int *planeRes)" NL
    ...
    "  float4 color0 = read_imagef(planeIn, sampler, (int2)(w,h)) + 0.2f;" NL
    "  float4 color1 = read_imagef(planeIn, sampler, (float2)(w,h)) + 0.2f;" NL
    "  color0 = (color0 == color1) ? color0: (float4)(0.5, 0.5, 0.5, 0.5);" NL
    "  write_imagef(planeOut, (int2)(w,h), color0);" NL
    ...
    while (deviceWrapper->AdapterNext())
    {
        ...
        *(static_cast<CD3D9SurfaceWrapper *>(surfaceDst.get()));
        ...
        {
            planeSrcList[planeIdx] =
            clCreateFromDX9MediaSurfaceKHR(ctx, CL_MEM_READ_WRITE, adapterType,
                                           &surfaceInfoSrc, planeIdx, &error);
            if (error != CL_SUCCESS)
            {
                log_error(
            ...
            planeDstList[planeIdx] =
            clCreateFromDX9MediaSurfaceKHR(ctx, CL_MEM_READ_WRITE, adapterType,
                                           &surfaceInfoDst, planeIdx, &error);
            if (error != CL_SUCCESS)
            {
                log_error(
                "clCreateFromDX9MediaSurfaceKHR failed for plane %i: %s\n",
        ...
        for (size_t frameIdx = 0; frameIdx < iterationNum; ++frameIdx)
        {
            ...
            clMemWrapper imageRes =
            clCreateBuffer(ctx, CL_MEM_READ_WRITE, bufferSize, NULL, &error);
            if (error != CL_SUCCESS)
            {
            ...
            for (size_t i = 0; i < memObjSrcList.size(); ++i)
            {
                ...
                error =
                clEnqueueReadImage(cmdQueue, memObjDstList[i], CL_TRUE, origin,
                                   regionPlane, 0, 0, &out[offset], 0, 0, 0);
                if (error != CL_SUCCESS)
                {
