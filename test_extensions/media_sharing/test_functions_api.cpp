                  ...
                  TSurfaceFormat surfaceFormat, TSharedHandleType sharedHandle)
{
    ...
    while (deviceWrapper->AdapterNext())
    {
        ...
        if (!MediaSurfaceCreate(
            adapterType, width, height, surfaceFormat, *deviceWrapper, surface,
            (sharedHandle == SHARED_HANDLE_ENABLED) ? true : false,
            &objectSharedHandle))
        {
        ...
        for (size_t frameIdx = 0; frameIdx < iterationNum; ++frameIdx)
        {
            ...
            { // write operation
                ...
                for (size_t i = 0; i < memObjList.size(); ++i)
                {
                    ...
                    cmdQueue, memObjList[i], CL_TRUE, origin, regionPlane, 0, 0,
                    &bufferRef2[frameIdx % FRAME_NUM][offset], 0, 0, 0);
                    if (error != CL_SUCCESS)
                    {
            ...
            { // copy operation (shared OCL to OCL)
                ...
                for (size_t i = 0; i < memObjList.size(); ++i)
                {
                    ...
                    error =
                    clEnqueueCopyImage(cmdQueue, memObjList[i], planeOCL,
                                       origin, origin, regionPlane, 0, 0, 0);
                    if (error != CL_SUCCESS)
                    {
            ...
            { // copy operation (OCL to shared OCL)
                ...
                for (size_t i = 0; i < memObjList.size(); ++i)
                {
                    ...
                    * sizeof(cl_uchar);
                    ...
                    error =
                    clEnqueueCopyImage(cmdQueue, planeOCL, memObjList[i],
                                       origin, origin, regionPlane, 0, 0, 0);
                    if (error != CL_SUCCESS)
                    {
                        log_error("clEnqueueCopyImage failed: %s\n",
            ...
            { // copy buffer to image
                size_t bufferSize = sizeof(cl_uchar) * frameSize;
                clMemWrapper buffer =
                clCreateBuffer(ctx, CL_MEM_COPY_HOST_PTR, bufferSize,
