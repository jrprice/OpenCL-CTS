...
// convert_float_to_half and convert_half_to_float may be found in
// test_conformance/images/image_helpers.cpp
...
void *CreateGLTexture1DArray(size_t width, size_t length, GLenum target,
                             GLenum glFormat, GLenum internalFormat,
                             GLenum glType, ExplicitType type,
                             GLuint *outTextureID, int *outError,
                             bool allocateMem, MTdata d)
{
    *outError = 0;
    ...
    if ((glType == GL_UNSIGNED_INT_2_10_10_10_REV)
    ...
    glTexParameteri(get_base_gl_target(target), GL_TEXTURE_MIN_FILTER,
                    GL_NEAREST);
    glTexParameteri(get_base_gl_target(target), GL_TEXTURE_MAG_FILTER,
                    GL_NEAREST);
    err = glGetError();
    if (err != GL_NO_ERROR)
    {
    ...
    if (err != GL_NO_ERROR)
    {
        ...
        {
            log_error(
            "ERROR: Unable to load data using glTexImage2D for "
            "TEXTURE_1D_ARRAY : %s : %s : %d : %d : %s : %s : Error %s\n",
            GetGLTargetName(target), GetGLFormatName(internalFormat),
            (int)(width), (int)(length), GetGLFormatName(glFormat),
            GetGLTypeName(glType), gluErrorString(err));
        ...
        {
            log_info(
            "WARNING: Unable to load data using glTexImage2D for "
            "TEXTURE_1D_ARRAY : %s : %s : %d : %d : %s : %s : Error %s\n",
            GetGLTargetName(target), GetGLFormatName(internalFormat),
            (int)(width), (int)(length), GetGLFormatName(glFormat),
            GetGLTypeName(glType), gluErrorString(err));
                             ...
                             int *outError, bool allocateMem, MTdata d)
{
    *outError = 0;
    ...
    {
        // Re-fill the created buffer to just have [0-1] floats, since that's
        // what it'd expect
    ...
    glGetError();
    // the default alignment in OpenGL is 4 bytes and need to be changed for
    // GL_DEPTH_COMPONENT16 which is aligned to 2 bytes
    ...
    if (err != GL_NO_ERROR)
    {
        ...
        {
            log_error(
            "ERROR: Unable to load data into GL texture (%s) format %s "
            "type %s internal format %s\n",
            gluErrorString(err), GetGLFormatName(glFormat),
            get_explicit_type_name(type), GetGLFormatName(internalFormat));
            *outError = -1;
        }
        else
        {
            log_info(
            "WARNING: Unable to load data into GL texture (%s) format %s "
            "type %s internal format %s\n",
            gluErrorString(err), GetGLFormatName(glFormat),
            get_explicit_type_name(type), GetGLFormatName(internalFormat));
            *outError = -2;
...
void *CreateGLTextureBuffer(size_t width, GLenum target, GLenum glFormat,
                            GLenum internalFormat, GLenum glType,
                            ...
                            int *outError, bool allocateMem, MTdata d)
{
    ...
    *outError = 0;
    ...
    if ((err = glGetError()))
    {
        log_error("ERROR: Unable to load data into glTexBuffer : %s : %s : %d "
                  ": %s : %s : Error %s\n",
                  GetGLTargetName(target), GetGLFormatName(internalFormat),
                  (int)(size), GetGLFormatName(glFormat), GetGLTypeName(glType),
                  gluErrorString(err));
        *outError = -1;
        delete[] buffer;
        return NULL;
...
void *CreateGLTexture1D(size_t width, GLenum target, GLenum glFormat,
                        ...
                        GLuint *outTextureID, int *outError, bool allocateMem,
                        MTdata d)
{
    *outError = 0;
    GLenum err = 0;
    ...
    if (err != GL_NO_ERROR)
    {
        if (err != GL_OUT_OF_MEMORY)
        {
            log_error("ERROR: Unable to load data into glTexImage1D : %s : %s "
                      ": %d : %s : %s : Error %s\n",
                      GetGLTargetName(target), GetGLFormatName(internalFormat),
                      (int)(width), GetGLFormatName(glFormat),
                      GetGLTypeName(glType), gluErrorString(err));
            *outError = -1;
        }
        else
        {
            log_info("WARNING: Unable to load data into glTexImage1D : %s : %s "
                     ": %d : %s : %s : Error %s\n",
                     GetGLTargetName(target), GetGLFormatName(internalFormat),
                     (int)(width), GetGLFormatName(glFormat),
                     GetGLTypeName(glType), gluErrorString(err));
...
void *CreateGLTexture2D(size_t width, size_t height, GLenum target,
                        GLenum glFormat, GLenum internalFormat, GLenum glType,
                        ...
                        bool allocateMem, MTdata d)
{
    *outError = 0;
    ...
    {
        char *temp =
        (char *)malloc(size * get_explicit_type_size(type) * sizeof(cl_char));
        if (allocateMem)
            memcpy(temp, buffer, size * get_explicit_type_size(type));
        else
            memset(temp, 0, size * get_explicit_type_size(type));

        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, internalFormat,
                     (GLsizei)width, (GLsizei)height, 0, glFormat, glType,
                     temp);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, internalFormat,
                     (GLsizei)width, (GLsizei)height, 0, glFormat, glType,
                     temp);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, internalFormat,
                     (GLsizei)width, (GLsizei)height, 0, glFormat, glType,
                     temp);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, internalFormat,
                     (GLsizei)width, (GLsizei)height, 0, glFormat, glType,
                     temp);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, internalFormat,
                     (GLsizei)width, (GLsizei)height, 0, glFormat, glType,
                     temp);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, internalFormat,
                     (GLsizei)width, (GLsizei)height, 0, glFormat, glType,
                     temp);
        free(temp);
    }
    else
    {
        ...
        log_info("- glTexImage2D : %s : %s : %d : %d : %s : %s\n",
                 GetGLTargetName(target), GetGLFormatName(internalFormat),
                 width, height, GetGLFormatName(glFormat),
        ...
        glTexImage2D(get_base_gl_target(target), 0, internalFormat,
                     (GLsizei)width, (GLsizei)height, 0, glFormat, glType,
                     buffer);
    ...
    if (err != GL_NO_ERROR)
    {
        if (err != GL_OUT_OF_MEMORY)
        {
            log_error("ERROR: Unable to load data into glTexImage2D : %s : %s "
                      ": %d : %d : %s : %s : Error %s\n",
                      GetGLTargetName(target), GetGLFormatName(internalFormat),
                      (int)(width), (int)(height), GetGLFormatName(glFormat),
                      GetGLTypeName(glType), gluErrorString(err));
            *outError = -1;
        }
        else
        {
            log_info("WARNING: Unable to load data into glTexImage2D : %s : %s "
                     ": %d : %d : %s : %s : Error %s\n",
                     GetGLTargetName(target), GetGLFormatName(internalFormat),
                     (int)(width), (int)(height), GetGLFormatName(glFormat),
                     GetGLTypeName(glType), gluErrorString(err));
...
#ifdef DEBUG
    char *test =
    (char *)malloc(width * height * 4 * get_explicit_type_size(type));
    ...
    log_info("- glGetTexImage : %s : %s : %s\n", GetGLTargetName(target),
             GetGLFormatName(glFormat), GetGLTypeName(glType));
    ...
    if (err != GL_NO_ERROR)
    {
        log_error("ERROR: Unable to read data from glGetTexImage : %s : %s : "
                  "%s : Error %s\n",
                  GetGLTargetName(target), GetGLFormatName(glFormat),
                  GetGLTypeName(glType), gluErrorString(err));
        return NULL;
    }
#endif
...
void *CreateGLTexture3D(size_t width, size_t height, size_t depth,
                        GLenum target, GLenum glFormat, GLenum internalFormat,
                        GLenum glType, ExplicitType type, GLuint *outTextureID,
                        int *outError, MTdata d, bool allocateMem)
{
    *outError = 0;
    ...
    glGetError();
    glTexImage3D(target, 0, internalFormat, (GLsizei)width, (GLsizei)height,
                 (GLsizei)depth, 0, glFormat, glType, buffer);
    GLenum err = glGetError();
    if (err != GL_NO_ERROR)
    {
        if (err != GL_OUT_OF_MEMORY)
        {
            log_error("ERROR: Unable to load data into GL texture (%s) format "
                      "%s type %s internal format %s\n",
                      gluErrorString(err), GetGLFormatName(glFormat),
        ...
        {
            log_info("WARNING: Unable to load data into GL texture (%s) format "
                     "%s type %s internal format %s\n",
                     gluErrorString(err), GetGLFormatName(glFormat),
                     get_explicit_type_name(type),
                     GetGLFormatName(internalFormat));
...
void *ReadGLTexture(GLenum glTarget, GLuint glTexture, GLuint glBuf,
                    GLint width, GLenum glFormat, GLenum glInternalFormat,
                    GLenum glType, ExplicitType typeToReadAs, size_t outWidth,
                    size_t outHeight)
{
...
#ifdef DEBUG
    log_info(
    "- Reading back from GL: %d x %d : %s : %s : %s : %s (stored as %s)\n",
    realWidth, realHeight, GetGLTargetName(glTarget),
    GetGLFormatName(glInternalFormat), GetGLFormatName(glFormat),
    GetGLTypeName(glType), GetGLFormatName(realInternalFormat));
#endif

    GLenum readBackFormat;
    switch (glFormat)
    {
        case GL_RGBA_INTEGER_EXT: readBackFormat = GL_RGBA_INTEGER_EXT; break;
        case GL_DEPTH_COMPONENT: readBackFormat = GL_DEPTH_COMPONENT; break;
        case GL_DEPTH_STENCIL: readBackFormat = GL_DEPTH_STENCIL; break;
        default: readBackFormat = GL_RGBA; break;
    }
    ...
    {
        outBytes =
        realWidth * realHeight * realDepth * 4 * GetGLTypeSize(readBackType);
    }
    else
    {
        outBytes = width * 4;
    ...
    log_info("- glGetTexImage: %s : %s : %s \n", GetGLTargetName(glTarget),
             GetGLFormatName(readBackFormat), GetGLTypeName(readBackType));
...
int CreateGLRenderbufferRaw(GLsizei width, GLsizei height, GLenum attachment,
                            GLenum glFormat, GLenum internalFormat,
                            GLenum glType, GLuint *outFramebuffer,
                            GLuint *outRenderbuffer)
{
    ...
    glGetRenderbufferParameterivEXT(GL_RENDERBUFFER_EXT,
                                    GL_RENDERBUFFER_RED_SIZE_EXT, &rsize);
    glGetRenderbufferParameterivEXT(GL_RENDERBUFFER_EXT,
                                    GL_RENDERBUFFER_GREEN_SIZE_EXT, &gsize);
    glGetRenderbufferParameterivEXT(GL_RENDERBUFFER_EXT,
                                    GL_RENDERBUFFER_BLUE_SIZE_EXT, &bsize);
    glGetRenderbufferParameterivEXT(GL_RENDERBUFFER_EXT,
                                    GL_RENDERBUFFER_ALPHA_SIZE_EXT, &asize);

    log_info("Renderbuffer internal format requested: %s actual: %s sizes: "
             "r=%d g=%d b=%d a=%d\n",
             GetGLFormatName(internalFormat),
             GetGLFormatName(realInternalFormat), rsize, gsize, bsize, asize);
#endif
    ...
    {
        log_error(
        "ERROR: Unable to attach renderbuffer to framebuffer (%s, status %x)\n",
...
void reorder_verification_buffer(GLenum glFormat, GLenum glType, char *buffer,
                                 size_t num_pixels)
{
    if (glType == GL_UNSIGNED_INT_8_8_8_8_REV && glFormat == GL_BGRA)
    {
...
const char *get_gl_vector_type(GLenum internalformat)
{
    switch (internalformat)
    {
        ...
        case GL_DEPTH32F_STENCIL8: return "vec4"; break;
        case GL_RGBA8I_EXT:
        case GL_RGBA16I_EXT:
        case GL_RGBA32I_EXT: return "ivec4"; break;
        case GL_RGBA8UI_EXT:
        case GL_RGBA16UI_EXT:
        case GL_RGBA32UI_EXT: return "uvec4"; break;
        default:
...
const char *get_gl_data_type(GLenum internalformat)
{
    switch (internalformat)
    {
        ...
        case GL_DEPTH_COMPONENT32F:
        case GL_DEPTH24_STENCIL8:
        case GL_DEPTH32F_STENCIL8: return "float"; break;
        case GL_RGBA8I_EXT:
        case GL_RGBA16I_EXT:
        case GL_RGBA32I_EXT: return "int"; break;
        case GL_RGBA8UI_EXT:
        case GL_RGBA16UI_EXT:
        case GL_RGBA32UI_EXT: return "uint"; break;
        default:
            log_error("Test error: unsupported data type\n");
            return "";
            break;
                                   ...
                                   bool fixedSampleLocations)
{
    ...
    if (max_samples < (GLint)samples) log_error(
    "GL error: requested samples (%d) exceeds renderer max samples (%d)\n",
    samples, max_samples);

    // Setup the GLSL program
    const GLchar *vertex_source = "#version 140\n"
                                  "in vec4 att0;\n"
                                  "void main (void) {\n"
    ...
    sprintf(fragmentShader, fragmentSource, get_gl_vector_type(internalFormat),
            get_gl_data_type(internalFormat),
            get_gl_vector_type(internalFormat));
    ...
    {
        case GL_DEPTH_COMPONENT16:
        case GL_DEPTH_COMPONENT32F: attachment = GL_DEPTH_ATTACHMENT; break;
        case GL_DEPTH24_STENCIL8:
        case GL_DEPTH32F_STENCIL8:
            attachment = GL_DEPTH_STENCIL_ATTACHMENT;
            break;
        default: attachment = GL_COLOR_ATTACHMENT0; break;
    }
    ...
    {
        log_info("GL status: GL_FRAMEBUFFER_UNSUPPORTED format %s multisample "
                 "is not supported\n",
                 GetGLFormatName(internalFormat));
        *outTextureID = 0;
    ...
    if (fbo_samples < (GLint)samples)
        log_error(
        "GL Error: requested samples (%d) exceeds FBO capability (%d)\n",
        samples, fbo_samples);
    ...
    for (size_t i = 0; i != samples; ++i)
    {
        ...
        switch (internalFormat)
        {
            ...
            case GL_RGBA32UI_EXT:
                ...
                break;
            default: glUniform1f(colorUniformLocation, color); break;
        }
    ...
    *outTextureID = tex;
    ...
    for (size_t s = 0; s != samples; ++s)
    {
        ...
        for (size_t i = 0; i != size; ++i)
        {
            switch (type)
            {
                case kChar: *((char *)p) = val * 0x7f; break;
                case kUChar: *((unsigned char *)p) = val * 0xff; break;
                case kFloat: *((float *)p) = val; break;
                case kShort: *((short *)p) = val * 0x7fff; break;
                case kUShort: *((unsigned short *)p) = val * 0xffff; break;
                case kInt: *((int *)p) = val * 0x7fffffff; break;
                case kUInt: *((unsigned int *)p) = val * 0xffffffff; break;
                case kHalf:
    ...
    if (allocateMem)
        reorder_verification_buffer(glFormat, glType, data,
...
void *CreateGLTexture2DArrayMultisample(size_t width, size_t height,
                                        ...
                                        GLenum internalFormat, GLenum glType,
                                        ExplicitType type, GLuint *outTextureID,
                                        int *outError, bool allocateMem,
                                        MTdata d, bool fixedSampleLocations)
{
    // This function creates a multisample texture and renders into each sample
    // using a GLSL shader
    ...
    glTexImage3DMultisample(GL_TEXTURE_2D_MULTISAMPLE_ARRAY, samples,
                            internalFormat, width, height, total_layers,
    ...
    for (size_t l = 0; l != total_layers; ++l)
    {
        ...
        if (status == GL_FRAMEBUFFER_UNSUPPORTED)
        {
            log_info("GL status: GL_FRAMEBUFFER_UNSUPPORTED format %s "
                     "multisample array is not supported\n",
                     GetGLFormatName(internalFormat));
            *outTextureID = 0;
            *outError = GL_FRAMEBUFFER_UNSUPPORTED;
            return NULL;
        ...
        for (size_t s = 0; s != samples; ++s)
        {
            ...
            switch (internalFormat)
            {
                ...
                case GL_RGBA32UI_EXT:
                    ...
                    break;
                default: glUniform1f(colorUniformLocation, val); break;
            }
    ...
    *outTextureID = tex;
    ...
    char *data = (char *)malloc(get_explicit_type_size(type) * size
                                * total_layers * samples);
    char *p = data;
    size_t stride = get_explicit_type_size(type);
    ...
    if (allocateMem)
        reorder_verification_buffer(glFormat, glType, data,
                                    width * height * samples * total_layers);
...
void *CreateGLRenderbuffer(GLsizei width, GLsizei height, GLenum attachment,
                           GLenum glFormat, GLenum internalFormat,
                           GLenum glType, ExplicitType type,
                           GLuint *outFramebuffer, GLuint *outRenderbuffer,
                           int *outError, MTdata d, bool allocateMem)
{
    *outError =
    CreateGLRenderbufferRaw(width, height, attachment, glFormat, internalFormat,
                            glType, outFramebuffer, outRenderbuffer);

    if (*outError != 0) return NULL;
    ...
    log_info("- Fillling renderbuffer: %d : %d : %s : %s \n", (int)width,
             (int)height, GetGLFormatName(glFormat), GetGLTypeName(glType));
    ...
    glTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_T,
                    GL_CLAMP_TO_EDGE);
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, internalFormat, width, height, 0,
                 glFormat, glType, buffer);
    ...
    glReadPixels(0, 0, (GLsizei)width, (GLsizei)height, glFormat, glType,
                 buffer);

    err = glGetError();
    if (err != GL_NO_ERROR)
    {
        log_error("ERROR: Unable to read data via glReadPixels : %d : %d : %s "
                  ": %s : Error %s\n",
                  (int)width, (int)height, GetGLFormatName(glFormat),
                  GetGLTypeName(glType), gluErrorString(err));
        *outError = -1;
...
void *ReadGLRenderbuffer(GLuint glFramebuffer, GLuint glRenderbuffer,
                         ...
                         ExplicitType typeToReadAs, size_t outWidth,
                         size_t outHeight)
{
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, glFramebuffer);
    glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, attachment,
                                 GL_RENDERBUFFER_EXT, glRenderbuffer);
    ...
    log_info("- Reading back from GL: %d x %d : %s : %s : %s\n", (int)outWidth,
             (int)outHeight, GetGLFormatName(glInternalFormat),
             GetGLFormatName(glFormat), GetGLTypeName(glType));
#endif

    GLenum readBackFormat =
    glFormat == GL_RGBA_INTEGER_EXT ? GL_RGBA_INTEGER_EXT : GL_RGBA;
    ...
    glReadPixels(0, 0, (GLsizei)outWidth, (GLsizei)outHeight, readBackFormat,
                 readBackType, outBuffer);

#ifdef DEBUG
    log_info("- glReadPixels: %d : %d : %s : %s \n", (int)outWidth,
             (int)outHeight, GetGLFormatName(readBackFormat),
...
GLenum GetGLFormat(GLenum internalFormat)
{
    GLenum glFormat;
    switch (internalFormat)
    {
        ...
        case GL_RGBA8:
        case GL_RGBA16:
        case GL_RGBA32F_ARB: glFormat = GL_RGBA; break;
        case GL_RGBA8I_EXT:
        case GL_RGBA16I_EXT:
        case GL_RGBA32I_EXT:
        case GL_RGBA8UI_EXT:
        case GL_RGBA16UI_EXT:
        case GL_RGBA32UI_EXT: glFormat = GL_RGBA_INTEGER_EXT; break;
...
GLenum GetGLTypeForExplicitType(ExplicitType type)
{
    switch (type)
    {
        case kFloat: return GL_FLOAT;
        case kInt: return GL_INT;
        case kUInt: return GL_UNSIGNED_INT;
        case kShort: return GL_SHORT;
        case kUShort: return GL_UNSIGNED_SHORT;
        case kChar: return GL_BYTE;
        case kUChar: return GL_UNSIGNED_BYTE;
        case kHalf:
#if defined(__APPLE__)
...
size_t GetGLTypeSize(GLenum type)
{
    switch (type)
    {
        ...
        case GL_FLOAT_32_UNSIGNED_INT_24_8_REV: return 2 * sizeof(GLfloat);
        default: log_error("Unknown type 0x%x\n", type); return 0;
    };
}
...
ExplicitType GetExplicitTypeForGLType(GLenum type)
{
    switch (type)
    {
        case GL_FLOAT: return kFloat;
        case GL_INT: return kInt;
        case GL_UNSIGNED_INT: return kUInt;
        case GL_SHORT: return kShort;
        case GL_UNSIGNED_SHORT: return kUShort;
        case GL_BYTE: return kChar;
        case GL_UNSIGNED_BYTE: return kUChar;
#if defined(__APPLE__)
        case GL_HALF_FLOAT:
#else
        case GL_HALF_FLOAT_ARB:
...
const char *GetGLTargetName(GLenum tgt)
{
    ...
    if (tgt == GL_TEXTURE_CUBE_MAP_POSITIVE_X)
        return "GL_TEXTURE_CUBE_MAP_POSITIVE_X";
    if (tgt == GL_TEXTURE_CUBE_MAP_POSITIVE_Y)
        return "GL_TEXTURE_CUBE_MAP_POSITIVE_Y";
    if (tgt == GL_TEXTURE_CUBE_MAP_POSITIVE_Z)
        return "GL_TEXTURE_CUBE_MAP_POSITIVE_Z";
    if (tgt == GL_TEXTURE_CUBE_MAP_NEGATIVE_X)
        return "GL_TEXTURE_CUBE_MAP_NEGATIVE_X";
    if (tgt == GL_TEXTURE_CUBE_MAP_NEGATIVE_Y)
        return "GL_TEXTURE_CUBE_MAP_NEGATIVE_Y";
    if (tgt == GL_TEXTURE_CUBE_MAP_NEGATIVE_Z)
        return "GL_TEXTURE_CUBE_MAP_NEGATIVE_Z";
    if (tgt == GL_TEXTURE_2D_MULTISAMPLE) return "GL_TEXTURE_2D_MULTISAMPLE";
    if (tgt == GL_TEXTURE_2D_MULTISAMPLE_ARRAY)
...
void *CreateRandomData(ExplicitType type, size_t count, MTdata d)
{
    switch (type)
    {
        ...
        default: {
            log_error(
            "Invalid explicit type specified for create random data!\n");
            return 0;
        }
    }
    return 0;
...
int init_clgl_ext()
{
    ...
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
    ...
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
                  "clCreateFromGLTexture2D) returned NULL.\n");
        return -1;
    }
    clCreateFromGLTexture3D_ptr =
    (clCreateFromGLTexture3D_fn)clGetExtensionFunctionAddressForPlatform(
    platform, "clCreateFromGLTexture3D");
    if (clCreateFromGLTexture3D_ptr == NULL)
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
                  "clCreateFromGLTexture3D\") returned NULL.\n");
        return -1;
    }
    ...
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
                  "\"clCreateFromGLTexture\") returned NULL.\n");
        return -1;
    }
    ...
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
                  "clCreateFromGLRenderbuffer) returned NULL.\n");
        return -1;
    }
    ...
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
                  "clGetGLObjectInfo) returned NULL.\n");
        return -1;
    }
    ...
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
                  "clGetGLTextureInfo) returned NULL.\n");
        return -1;
    }
    ...
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
                  "clEnqueueAcquireGLObjects) returned NULL.\n");
        return -1;
    }
    ...
    {
        log_error("clGetExtensionFunctionAddressForPlatform(platform,"
                  "clEnqueueReleaseGLObjects) returned NULL.\n");
        return -1;
    }
