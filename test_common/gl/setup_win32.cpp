...
public:
    ...
    virtual cl_context CreateCLContext(void)
    {
        ...
        {
            // As OpenCL for the platforms.  Warn if more than one platform
            // found, since this might not be the platform we want.  By default,
            // we simply use the first returned platform.
            ...
            {
                log_info(
                "clGetPlatformIDs returned multiple values.  This is not "
        ...
        {
            log_info(
            "GL context current device is not a CL device, using device %d.\n",
            ctxDevice);
        }
    ...
    virtual int SupportsCLGLInterop(cl_device_type device_type)
    {
        ...
        error = clGetDeviceIDs(m_platform, device_type, MAX_DEVICES, devices,
                               &num_of_devices);
        if (error)
        {
...
GLEnvironment *GLEnvironment::Instance(void)
{
    ...
    if (env == NULL) env = new WGLEnvironment();
    return env;
}
