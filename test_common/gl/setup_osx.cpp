...
public:
    OSXGLEnvironment() { mCGLContext = NULL; }
    ...
    virtual cl_context CreateCLContext(void)
    {
        ...
        if (mCGLContext == NULL) mCGLContext = CGLGetCurrentContext();
        ...
        for (int i = 0; i < (int)(size_out / sizeof(cl_device_id)); i++)
        {
            ...
            {
                log_error("Device %d does not support required extension "
                          "cl_APPLE_gl_sharing.\n",
                          i);
                return NULL;
            }
    ...
    virtual int SupportsCLGLInterop(cl_device_type device_type)
    {
        ...
        for (int i = 0; i < (int)num_of_devices; i++)
        {
            ...
            {
                log_info("Device %d of %d does support required extension "
                         "cl_APPLE_gl_sharing.\n",
                         i, num_of_devices);
...
GLEnvironment *GLEnvironment::Instance(void)
{
    ...
    if (env == NULL) env = new OSXGLEnvironment();
    return env;
}
