...
public:
    X11GLEnvironment() { m_device_count = 0; }
    ...
    virtual int SupportsCLGLInterop(cl_device_type device_type)
    {
        ...
        error =
        clGetDeviceIDs(platform, device_type, 64, devices, &num_of_devices);
        // If this platform doesn't have any of the requested device_type
        // (namely GPUs) then return 0
...
GLEnvironment *GLEnvironment::Instance(void)
{
    ...
    if (env == NULL) env = new X11GLEnvironment();
    return env;
}
