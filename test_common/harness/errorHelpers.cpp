...
static float Ulp_Error_Half_Float(float test, double reference)
{
    ...
    if (isinf(reference))
    {
        if ((double)test == reference) return 0.0f;
                       ...
                       cl_device_id *device_list)
{
    ...
    if (program != NULL)
    {
        ...
        if (num_devices == 0)
        {
            ...
            error =
            clGetContextInfo(context, CL_CONTEXT_DEVICES, 0, NULL, &size_ret);
...
int check_opencl_version_with_testname(const char *subtestname,
                                       cl_device_id device)
{
...
int check_opencl_version(cl_device_id device, cl_uint requestedMajorVersion,
                         cl_uint requestedMinorVersion)
{
    int error;
    ...
    error = clGetDeviceInfo(device, CL_DEVICE_VERSION, sizeof(device_version),
                            device_version, NULL);
    test_error(error, "unable to get CL_DEVICE_VERSION");

    if (strncmp(device_version, "OpenCL 1.2", 10) == 0
        && (device_version[10] == 0 || device_version[10] == ' '))
    {
        majorVersion = 1;
        minorVersion = 2;
    }
    else if (strncmp(device_version, "OpenCL 1.1", 10) == 0
             && (device_version[10] == 0 || device_version[10] == ' '))
    {
        majorVersion = 1;
        minorVersion = 1;
    }
    else if (strncmp(device_version, "OpenCL 2.0", 10) == 0
             && (device_version[10] == 0 || device_version[10] == ' '))
    {
        majorVersion = 2;
        minorVersion = 0;
    }
    else if (strncmp(device_version, "OpenCL 2.1", 10) == 0
             && (device_version[10] == 0 || device_version[10] == ' '))
    {
        majorVersion = 2;
        minorVersion = 1;
    ...
    if (majorVersion >= requestedMajorVersion) return 0;
