                                       ...
                                       const char *buildOptions)
{
    ...
    error =
    save_kernel_source_to_disk(gCompilationCachePath, kernelNamePrefix, kernel);
                                     ...
                                     std::string &clDeviceInfo)
{
    ...
    {
        /* An error message will have already been printed by
         * alloc_and_get_device_info(), so we can just return, here. */
        return -1;
    }
...
const cl_uint device_address_space_size, const CompilationMode compilationMode,
const std::string &bOptions, const std::string &sourceFilename,
const std::string &outputFilename, const std::string &clDeviceInfoFilename)
{
                                   ...
                                   const bool openclCXX)
{
    ...
    if (openclCXX)
    {
        ...
        runString =
        get_khronos_compiler_command(device_address_space_size, openclCXX,
    ...
    else
    {
        ...
        cl_int err =
        write_cl_device_info(device, device_address_space_size, compilationMode,
...
const char *buildOptions, const bool openclCXX, CompilationMode compilationMode)
{
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    else if (compilationMode == kSpir_v)
    {
        ...
        *outProgram =
        clCreateProgramWithIL(context, &modifiedKernelBuf[0], length, &error);
        if (*outProgram == NULL || error != CL_SUCCESS)
        {
...
const char *buildOptions, const bool openclCXX, CompilationMode compilationMode)
{
    ...
    compilationMode, gCompilationCacheMode, gCompilationCachePath, filePrefix);
...
{
    return create_single_kernel_helper_create_program(
    context, device, outProgram, numKernelLines, kernelProgram, buildOptions,
    openclCXX, gCompilationMode);
}

int create_single_kernel_helper_with_build_options(
cl_context context, cl_program *outProgram, cl_kernel *outKernel,
unsigned int numKernelLines, const char **kernelProgram, const char *kernelName,
const char *buildOptions, const bool openclCXX)
{
...
// -----------------------------------------------------------------------------------
    ...
    context, outProgram, outKernel, numKernelLines, kernelProgram, kernelName,
    newBuildOptions.c_str());
}
...
const char *buildOptions)
{
    ...
    for (z = 0; z < deviceCount; z++)
    {
        ...
        if (buildStatus != CL_BUILD_SUCCESS)
        {
            ...
            *outProgram, devices[z], CL_PROGRAM_BUILD_LOG, 0, NULL, &paramSize);
            if (error != CL_SUCCESS)
            {
...
int get_device_version(cl_device_id id, size_t *major, size_t *minor)
{
    ...
    cl_int error =
    clGetDeviceInfo(id, CL_DEVICE_VERSION, sizeof(buffer), buffer, &length);
                                    ...
                                    size_t *outMaxSize, size_t *outLimits)
{
    ...
    for (i = 0; i < numDevices; i++)
    {
        ...
        error =
        clGetKernelWorkGroupInfo(kernel, devices[i], CL_KERNEL_WORK_GROUP_SIZE,
                                 sizeof(size), &size, NULL);
        test_error(
        error,
        ...
        test_error(
        error, "clGetDeviceInfo failed for CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS");
        sizeLimit[0] = 1;
                              ...
                              const cl_image_format *fmt)
{
    ...
    cl_uint count = 0;
    cl_int err =
    clGetSupportedImageFormats(context, flags, image_type, 128, NULL, &count);
    if (count == 0) return 0;
    ...
    cl_int error =
    clGetSupportedImageFormats(context, flags, image_type, count, list, NULL);
    if (error)
    {
...
int checkFor3DImageSupport(cl_device_id device)
{
    ...
    char profile[128];
    error =
    clGetDeviceInfo(device, CL_DEVICE_PROFILE, sizeof(profile), profile, NULL);
