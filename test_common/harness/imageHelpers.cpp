...
float MirroredRepeatNormalizedAddressFn(float fValue, size_t maxValue)
{
    ...
    2.0f
    * rintf(
    fValue * 0.5f); // Note halfway values flip flop here due to rte, but they
                    // both end up pointing the same place at the end of the day
                             ...
                             int isLinearFilter)
{
    ...
    switch (format->image_channel_data_type)
    {
        ...
        case CL_UNORM_SHORT_555:
        case CL_UNORM_INT_101010:
            maxError = 2 * FLT_EPSILON
            * sampleCount; // Maximum sampling error for round to zero
                           // normalization based on multiplication
            // by reciprocal (using reciprocal generated in round to +inf mode,
            // so that 1.0 matches spec)
            break;
...
#if defined(__APPLE__)
    {
        if (sampler->filter_mode != CL_FILTER_NEAREST)
        {
            ...
            if (gDeviceType == CL_DEVICE_TYPE_GPU)
                maxError += MAKE_HEX_FLOAT(0x1.0p-4f, 0x1L,
                                           -4); // Some GPUs ain't so accurate
            else
            ...
            maxError +=
            4.0f * FLT_EPSILON; // normalized coordinates will introduce some
                                // error into the fractional part of the
                                // address, affecting results
    }
#else
    {
#if !defined(_WIN32)
...
{
    // We have to take care of a few special cases, but in general, we just
    // extract the same components from the half that exist in the float and
    // re-stuff them For a description of the actual half format, see
    // http://en.wikipedia.org/wiki/Half_precision Note: we store these in
    // 32-bit ints to make the bit manipulations easier later
    int sign = (halfValue >> 15) & 0x0001;
    int exponent = (halfValue >> 10) & 0x001f;
    int mantissa = (halfValue)&0x03ff;

    // Note: we use a union here to be able to access the bits of a float
    // directly
    union {
        unsigned int bits;
        float floatValue;
    ...
    if (exponent == 0)
    {
        ...
        // The first bit is implicit, so we take it off and inc the exponent
        // accordingly
        exponent++;
        mantissa &= ~(0x00000400);
    }
    ...
    {
        // If the exponent is 31, it's a special case number (+/- infinity or
        // NAN). If the mantissa is 0, it's infinity, else it's NAN, but in
        // either case, the packing method is the same
        outFloat.bits = (sign << 31) | 0x7f800000 | (mantissa << 13);
        return outFloat.floatValue;
    }

    // Plain ol' normalized number, so adjust to the ranges a 32-bit float
    // expects and repack
    exponent += (127 - 15);
    mantissa <<= 13;
...
cl_ushort convert_float_to_half(float f)
{
    ...
    {
        case kRoundToNearestEven: return float2half_rte(f);
        case kRoundTowardZero: return float2half_rtz(f);
        default:
                                 ...
                                 BufferOwningPtr<char> &P, MTdata d)
{
    ...
    if (gDeviceType == CL_DEVICE_TYPE_CPU)
    {
        ...
        void *map =
        mmap(0, mapSize, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, 0, 0);
    ...
    P.reset(NULL); // Free already allocated memory first, then try to allocate
                   // new block.
    char *data =
    (char *)align_malloc(allocSize, get_pixel_size(imageInfo->format));
    ...
    {
        log_error(
        "ERROR: Unable to malloc %lu bytes for generate_random_image_data\n",
        allocSize);
        return 0;
    }
    ...
    if (/*!gTestMipmaps*/ imageInfo->num_mip_levels < 2)
    {
        ...
        if (imageInfo->slicePitch > slice_dimension * imageInfo->rowPitch)
        {
            ...
            for (i = 0; i < depth; i++)
            {
                ...
                size_t length =
                imageInfo->slicePitch - slice_dimension * imageInfo->rowPitch;
                memset(data + offset, 0xff, length);
            }
        }
    }
...
int *containsDenorms, int lod)
{
    ...
    if (imageSampler->normalized_coords)
    {
        ...
        switch (imageInfo->type)
        {
            ...
            default:
                ...
                imageSampler->addressing_mode, verbose);
                z =
                unnormalize_coordinate("z", z, zAddressOffset, (float)depth_lod,
                                       imageSampler->addressing_mode, verbose);
        }
    }
    else if (verbose)
    ...
    else
    {
            ...
            || imageInfo->type == CL_MEM_OBJECT_IMAGE1D_ARRAY)
        {
            ...
            outData[3] =
            (float)((upLeft[3] * weights[0][0]) + (upRight[3] * weights[1][0])
                              ...
                              int *outZ, int lod)
{
    ...
    {
        log_info(
        "WARNING: Unable to search for debug pixel: invalid image format\n");
        return false;
    }
    ...
    switch (imageFormat->image_channel_data_type)
    {
        ...
        case CL_SIGNED_INT32: {
            ...
            for (unsigned int i = 0; i < channelCount; i++)
                ...
                MAKE_HEX_FLOAT(0x1.fffffep30f, 0x1fffffe, 30 - 23), CL_INT_MAX);
            break;
        }
        case CL_UNSIGNED_INT8: {
            cl_uchar *ptr = (cl_uchar *)outData;
        ...
        case CL_UNSIGNED_INT16: {
            ...
            for (unsigned int i = 0; i < channelCount; i++)
                ptr[i] =
                (cl_ushort)CONVERT_UINT(srcVector[i], 32767.f, CL_USHRT_MAX);
            break;
        }
        case CL_UNSIGNED_INT32: {
            cl_uint *ptr = (cl_uint *)outData;
                            ...
                            const void *results, float *errors)
{
    ...
    switch (imageFormat->image_channel_data_type)
    {
        ...
        case CL_SNORM_INT16: {
            ...
            for (unsigned int i = 0; i < channelCount; i++)
                errors[i] = ptr[i]
                - NORMALIZE_SIGNED_UNROUNDED(srcVector[i], -32767.f, 32767.f);
        ...
        case CL_UNORM_INT_101010: {
            ...
            errors[0] =
            ((ptr[0] >> 20) & 1023) - NORMALIZE_UNROUNDED(srcVector[0], 1023.f);
            errors[1] =
            ((ptr[0] >> 10) & 1023) - NORMALIZE_UNROUNDED(srcVector[1], 1023.f);
            errors[2] =
            ((ptr[0] >> 0) & 1023) - NORMALIZE_UNROUNDED(srcVector[2], 1023.f);
        ...
        case CL_SIGNED_INT16: {
            ...
            for (unsigned int i = 0; i < channelCount; i++)
                errors[i] =
                ptr[i] - CONVERT_INT(srcVector[i], -32767.f, 32767.f, 32767);
            break;
        }
        case CL_SIGNED_INT32: {
        ...
        case kFloat: {
            ...
            switch (imageInfo->format->image_channel_data_type)
            {
                case CL_HALF_FLOAT: {
                    ...
                    for (; i < numPixels * 4; i++)
                        inputValues[i] =
                        get_random_float(-HALF_MAX - 2.f, HALF_MAX + 2.f, d);
                }
                break;
#ifdef CL_SFIXED14_APPLE
                case CL_SFIXED14_APPLE: {
                     ...
                     const size_t destPos[], const size_t regionSize[])
{
    ...
    + destPos_lod[2] * dst_slice_pitch_lod + destPos_lod[1] * dst_row_pitch_lod
    + pixelSize * destPos_lod[0] + dst_mip_level_offset;
                       ...
                       bool err, int t)
{
    ...
    if (t)
    {
        if (err)
            ...
            sampler->filter_mode == CL_FILTER_NEAREST ? "CL_FILTER_NEAREST"
                                                      : "CL_FILTER_LINEAR",
            addressMode, normalizedNames[sampler->normalized_coords ? 1 : 0],
    ...
    */
    ...
    else
    {
        copy(fullProfReadOrWriteFormats.begin(),
             fullProfReadOrWriteFormats.end(), back_inserter(formatsToSupport));
    }
    ...
    if (!gIsEmbedded && version >= Version(2, 0) && version <= Version(2, 2))
    {
        ...
        if (image_type == CL_MEM_OBJECT_IMAGE2D_ARRAY
            || image_type == CL_MEM_OBJECT_IMAGE2D)
        {
        ...
        if (image_type != CL_MEM_OBJECT_IMAGE1D_BUFFER)
        {
            ...
            {
                copy(fullProf2XSRGBFormats.begin(), fullProf2XSRGBFormats.end(),
                     back_inserter(formatsToSupport));
            }
        }
    }
