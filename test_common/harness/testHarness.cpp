...
void version_expected_info(const char *test_name, const char *expected_version,
                           const char *device_version)
{
    log_info("%s skipped (requires at least version %s, but the device reports "
             "version %s)\n",
...
#define kHasSSE4_2 0x00000800
    ...
    if (getenv("OPENCL_1_0_DEVICE"))
    {
        ...
        if ((err = clGetDeviceInfo(device, CL_DEVICE_OPENCL_C_VERSION,
                                   sizeof(c_version), c_version, NULL)))
        {
            log_error("FAILURE: unable to get CL_DEVICE_OPENCL_C_VERSION on "
                      "1.0 device. (%d)\n",
                      err);
            return EXIT_FAILURE;
        }
        ...
        if (0 == strncmp(c_version, "OpenCL C 1.0 ", strlen("OpenCL C 1.0 ")))
        {
            ...
            log_info(
            "Device is a OpenCL 1.0 device, but supports OpenCL C 1.1\n");
    }
                       ...
                       cl_command_queue_properties queueProps)
{
    for (int i = 0; i < testNum; ++i)
    {
        if (selectedTestList[i])
        {
            ...
            testList[i], deviceToUse, forceNoContextCreation, numElementsToUse,
            queueProps);
        }
    }
}
                                   ...
                                   const cl_queue_properties queueProps)
{
    ...
    {
        version_expected_info(test.name, test.min_version.to_string().c_str(),
    ...
    {
        context =
        clCreateContext(NULL, 1, &deviceToUse, notify_callback, NULL, &error);
        if (!context)
        {
    ...
    else
    {
        ...
        numElementsToUse); // test_threaded_function( ptr_basefn_list[i], group,
...
cl_device_id GetOpposingDevice(cl_device_id device)
{
    ...
    error =
    clGetDeviceIDs(plat, CL_DEVICE_TYPE_ALL, actualCount, otherDevices, NULL);
    if (error != CL_SUCCESS)
    {
