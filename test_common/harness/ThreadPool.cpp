...
|| defined(_WIN32) // or any other POSIX system
...
typedef struct
{
    ...
    volatile cl_int
    mReleaseCount; // Number of releases to execute before reseting the event.
} _CONDITION_VARIABLE;
...
#endif // !_WIN32
volatile cl_int gRunCount =
0; // Condition variable state. How many iterations on the function left to run.
// set to CL_INT_MAX to cause worker threads to exit. Note: this value might go
// negative.
...
#endif // !_WIN32
volatile cl_int gRunning =
0; // # of threads intended to be running. Running threads will decrement this
...
#endif
{
    ...
    while (MAX_COUNT > item)
    {
        ...
        if (0 >= item)
        {
            ...
            {
                log_error(
                "Error %d from pthread_mutex_lock. Worker %d unable to block "
            ...
            { // last thread out signal the main thread to wake up
                ...
                {
                    log_error(
                    "Error %d from pthread_cond_broadcast. Unable to wake up "
                    "main thread. ThreadPool_WorkerFunc failed.\n",
                    err);
                    goto exit;
                }
        ...
        if (CL_SUCCESS
            == jobError) // but only if we haven't already encountered an error
        {
            ...
            if (err)
            {
                ...
                if (pthread_mutex_lock(&gAtomicLock))
                    log_error(
                    "Atomic operation failed. pthread_mutex_lock(&gAtomicLock) "
                    "returned an error\n");
                if (jobError == CL_SUCCESS)
                    ;
                jobError = err;
...
#warning How about this, instead of hard coding it to 2?
        ...
        {
            gThreadCount = (cl_int)sysconf(
            _SC_NPROCESSORS_CONF); // Hopefully your system returns logical cpus
                                   // here, as does MacOS X
        }
#else // !_WIN32
        gThreadCount = (cl_int)sysconf(
        _SC_NPROCESSORS_CONF); // Hopefully your system returns logical cpus
...
// process memory is not sufficient and the test fails
    ...
    // Allow the app to set thread count to <0 for debugging purposes.  This
    // will cause the test to run single threaded.
    if (gThreadCount < 2)
    {
    ...
    for (i = 0; i < gThreadCount; i++)
    {
        ...
        pthread_t tid = 0;
        err =
        pthread_create(&tid, NULL, ThreadPool_WorkerFunc, (void *)&threadID);
#endif // !_WIN32
        if (err)
        {
...
#warning This is buggy and race prone.  Find a better way.
    ...
    if ((err = pthread_mutex_lock(&gThreadPoolLock)))
    {
        switch (err)
        {
            case EDEADLK:
                log_error(
                "Error EDEADLK returned in ThreadPool_Do(). ThreadPool_Do is "
                "not designed to work recursively!\n");
                break;
            case EINVAL:
