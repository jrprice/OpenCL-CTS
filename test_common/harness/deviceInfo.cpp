...
/* Helper to allocate and return a buffer containing device information for the
 * specified device info parameter. */
static void *alloc_and_get_device_info(cl_device_id device,
                                       ...
                                       const char *param_description)
{
    ...
    {
        log_error(
        "Error: failed to determine size of device %s at %s:%d (err = %d)\n",
    ...
    if (NULL == buffer)
    {
        log_error("Error: unable to allocate %zu byte buffer for device %s at "
                  "%s:%d (err = %d)\n",
...
{
    return (char *)alloc_and_get_device_info(device, CL_DEVICE_EXTENSIONS,
                                             "extensions string");
}
...
{
    return (char *)alloc_and_get_device_info(device, CL_DEVICE_IL_VERSION,
                                             "IL version string");
}
...
{
    return (char *)alloc_and_get_device_info(device, CL_DEVICE_VERSION,
                                             "version string");
}
