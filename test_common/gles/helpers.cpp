                        ...
                        bool allocateMem, MTdata d)
{
    *outError = 0;
    ...
    if (get_base_gl_target(target) == GL_TEXTURE_CUBE_MAP)
    {
        ...
        width * height * 4 * get_explicit_type_size(type) * sizeof(cl_char));
        if (allocateMem)
            memcpy(temp, buffer,
                   width * height * 4 * get_explicit_type_size(type));
        else
    ...
    if (err != GL_NO_ERROR)
    {
         ...
         *
         *     If a GL texture object with an internal format from table 9.4 is
         * successfully created by OpenGL, then there is guaranteed to be a
         * mapping to one of the corresponding CL image format(s) in that table.
         *
         *  Notice that some of the formats in table 9.4 are not supported in
         * OpenGL ES 2.0.
         */
        log_info("Warning: Skipping %s : %s : %d : %d : %s : %s : because "
                 "glTexImage2D returned %s\n",
                 GetGLTargetName(target), GetGLFormatName(internalFormat),
                        ...
                        int *outError, MTdata d, bool allocateMem)
{
    *outError = 0;
    ...
    char *buffer =
    (char *)create_random_data(type, d, width * height * depth * 4);
    ...
    if (type == kFloat && allocateMem)
    {
        ...
        memset(buffer, 0,
               width * height * depth * 4 * get_explicit_type_size(type));
...
void *ReadGLTexture(GLenum glTarget, GLuint glTexture, GLenum glFormat,
                    ...
                    size_t outHeight)
{
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, glTarget,
                           glTexture, 0);
    err = glGetError();
    if (err != GL_NO_ERROR)
    {
                            ...
                            GLuint *outFramebuffer, GLuint *outRenderbuffer)
{
    ...
    glGetError();
    glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, rbFormat, (GLsizei)width,
                             (GLsizei)height);
    err = glGetError();
    if (err != GL_NO_ERROR)
    {
...
void *CreateGLRenderbuffer(GLsizei width, GLsizei height, GLenum attachment,
                           GLenum rbFormat, GLenum rbType, GLenum texFormat,
                           GLenum texType, ExplicitType type,
                           GLuint *outFramebuffer, GLuint *outRenderbuffer,
                           int *outError, MTdata d, bool allocateMem)
{
    *outError =
    CreateGLRenderbufferRaw(width, height, attachment, rbFormat, rbType,
                            outFramebuffer, outRenderbuffer);
    ...
    memset(buffer, 0, width * height * 4 * get_explicit_type_size(type));
    glReadPixels(0, 0, (GLsizei)width, (GLsizei)height, texFormat, texType,
                 buffer);
    ...
    if (err != GL_NO_ERROR)
    {
        log_error("ERROR: Unable to read data via glReadPixels : %d : %d : %s "
                  ": %s : Error %s\n",
                  (int)width, (int)height, GetGLFormatName(texFormat),
                  GetGLTypeName(texType), gluErrorString(err));
        *outError = -1;
    }
...
void *ReadGLRenderbuffer(GLuint glFramebuffer, GLuint glRenderbuffer,
                         GLenum attachment, GLenum rbFormat, GLenum rbType,
...
int init_clgl_ext(cl_platform_id platform_id)
{
    ...
    platform_id, "clCreateFromGLBuffer");
    ...
    platform_id, "clCreateFromGLTexture");
    ...
    platform_id, "clCreateFromGLRenderbuffer");
    ...
    platform_id, "clGetGLObjectInfo");
    if (clGetGLObjectInfo_ptr == NULL)
    {
        log_error("clGetExtensionFunctionAddressForPlatform(clGetGLObjectInfo) "
                  "returned NULL.\n");
        return -1;
    }
    ...
    platform_id, "clGetGLTextureInfo");
    if (clGetGLTextureInfo_ptr == NULL)
    {
        log_error("clGetExtensionFunctionAddressForPlatform(clGetGLTextureInfo)"
                  " returned NULL.\n");
        return -1;
    }
    ...
    platform_id, "clEnqueueAcquireGLObjects");
    ...
    platform_id, "clEnqueueReleaseGLObjects");
