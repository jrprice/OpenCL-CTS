...
struct unary_func : public detail::base_func_type<OUT1>
{
    ...
    // test.
    std::vector<IN1> in_special_cases() { return {}; }
...
std::string generate_kernel_unary(func_type func)
{
    ...
    if (func.is_in1_bool())
    {
        std::string i = vector_size<in_type>::value == 1
        ? ""
        : std::to_string(vector_size<in_type>::value);
    ...
    if (func.is_out_bool())
    {
        ...
        function_call =
        "convert_int" + i + "(" + func.str() + "(" + in1_value + "))";
    }
...
std::string generate_kernel_unary(func_type func)
{
    ...
    if (func.is_out_bool())
    {
        ...
        : std::to_string(vector_size<out_type>::value);
        function_call =
        "convert_cast<int" + i + ">(" + func.str() + "(" + in1_value + "))";
    }
    ...
    + "#include <opencl_memory>\n"
      ...
      "using namespace cl;\n"
      "__kernel void "
    + func.get_kernel_name() + "(global_ptr<" + type_name<in_type>()
    + "[]> input,"
      "global_ptr<"
    + type_name<out_type>()
    + "[]> output)\n"
...
bool verify_unary(const std::vector<INPUT> &in, const std::vector<OUTPUT> &out,
                  unary_op op)
{
...
int test_unary_func(cl_device_id device, cl_context context,
                    cl_command_queue queue, size_t count, unary_op op)
{
    cl_mem buffers[2];
    cl_program program;
    cl_kernel kernel;
