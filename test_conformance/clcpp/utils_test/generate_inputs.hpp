...
template <class type>
std::vector<type> generate_input(
...
// std::uniform_int_distribution<> does not work in VS2015 for cl_uchar and
// cl_char, because VS2015 thinks that use cl_int, because VS2015 thinks
// cl_uchar cl_char are not int types
&& !(
std::is_same<typename scalar_type<type>::type, cl_uchar>::value
|| std::is_same<typename scalar_type<type>::type, cl_char>::value)>::type* = 0)
{
...
// cl_uchar cl_char are not int types
&& (std::is_same<typename scalar_type<type>::type, cl_uchar>::value
    || std::is_same<typename scalar_type<type>::type, cl_char>::value)>::type* =
0)
{
    typedef typename scalar_type<type>::type SCALAR;
    const size_t vec_size = vector_size<type>::value;
    ...
    for (size_t i = 0; i < vec_size; i++)
    {
        ...
        static_cast<cl_int>(min.s[i]), static_cast<cl_int>(max.s[i]));
    }
    for (auto& i : input)
    {
        for (size_t j = 0; j < vec_size; j++)
...
size_t count, const type& min, const type& max,
const std::vector<type> special_cases,
typename std::enable_if<!is_vector_type<type>::value
                        && std::is_integral<type>::value
                        // std::uniform_int_distribution<> does not work in
                        // VS2015 for cl_uchar and cl_char, because VS2015
                        // thinks that use cl_int, because VS2015 thinks
                        // cl_uchar cl_char are not int types
                        && !(std::is_same<type, cl_uchar>::value
                             || std::is_same<type, cl_char>::value)>::type* = 0)
{
    std::vector<type> input(count);
    std::random_device rd;
    std::mt19937 gen(rd());
                            ...
                            || std::is_same<type, cl_char>::value)>::type* = 0)
{
    ...
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<cl_int> dis(static_cast<cl_int>(min),
                                              static_cast<cl_int>(max));
    for (auto& i : input)
    {
...
typename std::enable_if<
is_vector_type<type>::value
&& std::is_floating_point<typename scalar_type<type>::type>::value>::type* = 0)
{
    typedef typename scalar_type<type>::type SCALAR;
    const size_t vec_size = vector_size<type>::value;
    ...
    {
        // Fatal error
        if (std::fpclassify(max.s[i]) == FP_SUBNORMAL
            || std::fpclassify(min.s[i]) == FP_SUBNORMAL)
        {
            log_error("ERROR: min and max value for input generation CAN NOT "
                      "BE subnormal\n");
        }
...
const std::vector<type> special_cases,
typename std::enable_if<!is_vector_type<type>::value
                        && std::is_floating_point<type>::value>::type* = 0)
{
    // Fatal error
    if (std::fpclassify(max) == FP_SUBNORMAL
        || std::fpclassify(min) == FP_SUBNORMAL)
    {
        log_error(
        "ERROR: min and max value for input generation CAN NOT BE subnormal\n");
    }
    std::vector<type> input(count);
    std::random_device rd;
    std::mt19937 gen(rd());
...
template <class type>
std::vector<type> generate_output(
size_t count,
...
{
    type value;
    for (size_t i = 0; i < vector_size<type>::value; i++) value.s[i] = svalue;
    return std::vector<type>(count, value);
}

template <class type>
std::vector<type> generate_output(
size_t count, type svalue = type(0),
typename std::enable_if<!is_vector_type<type>::value>::type* = 0)
{
                           ...
                           std::vector<M>& in3_spec_cases)
{
    ...
    size_t new_size =
    in1_spec_cases.size() * in2_spec_cases.size() * in3_spec_cases.size();
    std::vector<T> new_in1(new_size);
    std::vector<K> new_in2(new_size);
    ...
    for (size_t i = 0; i < in1_spec_cases.size(); i++)
    {
        for (size_t j = 0; j < in2_spec_cases.size(); j++)
        {
            ...
            {
                size_t idx = (i * in2_spec_cases.size() * in3_spec_cases.size())
                + (j * in3_spec_cases.size()) + k;
