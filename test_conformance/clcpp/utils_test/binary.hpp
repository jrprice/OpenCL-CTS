...
bool verify_binary(const std::vector<INPUT1> &in1,
                   ...
                   const std::vector<OUTPUT> &out, binary_op op)
{
    for (size_t i = 0; i < in1.size(); i++)
    {
        auto expected = op(in1[i], in2[i]);
        if (!are_equal(expected, out[i], op.delta(in1[i], in2[i], expected),
                       op))
        {
...
int test_binary_func(cl_device_id device, cl_context context,
                     cl_command_queue queue, size_t count, binary_op op)
{
    cl_mem buffers[3];
    cl_program program;
    cl_kernel kernel;
    ...
    std::string code_str =
    generate_kernel_binary<binary_op, INPUT1, INPUT2, OUTPUT>(op);
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    {
        RETURN_ON_ERROR_MSG(-1, "test_%s %s(%s, %s) failed", op.str().c_str(),
