...
struct ternary_func : public detail::base_func_type<OUT1>
{
    ...
    std::string decl_str()
    {
        ...
        + type_name<IN2>() + ", " + type_name<IN3>() + ")";
    }

    bool is_in1_bool() { return false; }

    bool is_in2_bool() { return false; }

    bool is_in3_bool() { return false; }

    IN1 min1() { return detail::get_min<IN1>(); }

    IN1 max1() { return detail::get_max<IN1>(); }

    IN2 min2() { return detail::get_min<IN2>(); }

    IN2 max2() { return detail::get_max<IN2>(); }

    IN3 min3() { return detail::get_min<IN3>(); }

    IN3 max3() { return detail::get_max<IN3>(); }

    std::vector<IN1> in1_special_cases() { return {}; }

    std::vector<IN2> in2_special_cases() { return {}; }

    std::vector<IN3> in3_special_cases() { return {}; }
...
template <class func_type, class in1_type, class in2_type, class in3_type,
          class out_type>
std::string generate_kernel_ternary(func_type func)
{
    ...
    if (func.is_in1_bool())
    {
        std::string i = vector_size<in1_type>::value == 1
        ? ""
        : std::to_string(vector_size<in1_type>::value);
        in1_value = "(input1[gid] != (int" + i + ")(0))";
    }
    std::string in2_value = "input2[gid]";
    if (func.is_in2_bool())
    {
        std::string i = vector_size<in2_type>::value == 1
        ? ""
        : std::to_string(vector_size<in2_type>::value);
        in2_value = "(input2[gid] != (int" + i + ")(0))";
    }
    std::string in3_value = "input3[gid]";
    if (func.is_in3_bool())
    {
        std::string i = vector_size<in3_type>::value == 1
        ? ""
        : std::to_string(vector_size<in3_type>::value);
        in3_value = "(input3[gid] != (int" + i + ")(0))";
    }
    std::string function_call =
    func.str() + "(" + in1_value + ", " + in2_value + ", " + in3_value + ")";
    if (func.is_out_bool())
    {
        std::string i = vector_size<out_type>::value == 1
        ? ""
        : std::to_string(vector_size<out_type>::value);
        function_call = "convert_int" + i + "(" + func.str() + "(" + in1_value
        + ", " + in2_value + ", " + in3_value + "))";
    }
    return "__kernel void " + func.get_kernel_name() + "(global "
    + type_name<in1_type>()
    + " *input1,\n"
      "                                      global "
    + type_name<in2_type>()
...
std::string generate_kernel_ternary(func_type func)
{
    ...
    if (func.is_out_bool())
    {
        ...
        ? ""
        : std::to_string(vector_size<out_type>::value);
        function_call = "convert_cast<int" + i + ">(" + func.str() + "("
        + in1_value + ", " + in2_value + ", " + in3_value + "))";
    }
    ...
    + "#include <opencl_memory>\n"
      ...
      "using namespace cl;\n"
      "__kernel void "
    + func.get_kernel_name() + "(global_ptr<" + type_name<in1_type>()
    + "[]> input1,\n"
      "                                      global_ptr<"
    + type_name<in2_type>()
    + "[]> input2,\n"
...
bool verify_ternary(const std::vector<INPUT1> &in1,
                    ...
                    const std::vector<OUTPUT> &out, ternary_op op)
{
    for (size_t i = 0; i < in1.size(); i++)
    {
        auto expected = op(in1[i], in2[i], in3[i]);
        if (!are_equal(expected, out[i],
                       op.delta(in1[i], in2[i], in3[i], expected), op))
        {
...
int test_ternary_func(cl_device_id device, cl_context context,
                      cl_command_queue queue, size_t count, ternary_op op)
{
    cl_mem buffers[4];
    cl_program program;
    cl_kernel kernel;
    ...
    if (!(type_supported<INPUT1>(device) && type_supported<INPUT2>(device)
          && type_supported<INPUT3>(device) && type_supported<OUTPUT>(device)))
    {
        return CL_SUCCESS;
    }

    std::string code_str =
    generate_kernel_ternary<ternary_op, INPUT1, INPUT2, INPUT3, OUTPUT>(op);
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    std::vector<INPUT1> input1 =
    generate_input<INPUT1>(count, op.min1(), op.max1(), in1_spec_cases);
    std::vector<INPUT2> input2 =
    generate_input<INPUT2>(count, op.min2(), op.max2(), in2_spec_cases);
    std::vector<INPUT3> input3 =
    generate_input<INPUT3>(count, op.min3(), op.max3(), in3_spec_cases);
    ...
    buffers[1] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                                sizeof(INPUT2) * input2.size(), NULL, &err);
    RETURN_ON_CL_ERROR(err, "clCreateBuffer")

    buffers[2] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                                sizeof(INPUT3) * input3.size(), NULL, &err);
    RETURN_ON_CL_ERROR(err, "clCreateBuffer")

    buffers[3] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
    ...
    {
        RETURN_ON_ERROR_MSG(
        -1, "test_%s %s(%s, %s, %s) failed", op.str().c_str(),
        type_name<OUTPUT>().c_str(), type_name<INPUT1>().c_str(),
        type_name<INPUT2>().c_str(), type_name<INPUT3>().c_str());
    }
