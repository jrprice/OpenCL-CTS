...
template <class IN1, class OUT1> struct as_type : public unary_func<IN1, OUT1>
{
    static_assert(sizeof(IN1) == sizeof(OUT1),
                  "It is an error to use the as_type<T> operator to "
                  "reinterpret data to a type of a different number of bytes");

    std::string str() { return "as_type"; }

    std::string headers() { return "#include <opencl_reinterpret>\n"; }
...
std::string generate_kernel_as_type(func_type func)
{
    ...
    "as_" + type_name<out_type>() + "(" + in1_value + ");";
...
std::string generate_kernel_as_type(func_type func)
{
    ...
    std::string in1_value = "input[gid]";
    std::string function_call =
    "as_type<" + type_name<out_type>() + ">(" + in1_value + ")";
...
bool verify_as_type(const std::vector<INPUT> &in,
                    const std::vector<OUTPUT> &out, as_type_op op)
{
    // When the operand and result type contain a different number of elements,
    // the result is implementation-defined, i.e. any result is correct
...
int test_as_type_func(cl_device_id device, cl_context context,
                      cl_command_queue queue, size_t count, as_type_op op)
{
    cl_mem buffers[2];
    cl_program program;
    cl_kernel kernel;
    ...
    std::string code_str =
    generate_kernel_as_type<as_type_op, INPUT, OUTPUT>(op);
...
#define TEST_AS_TYPE_MACRO(TYPE1, TYPE2)                                       \
    last_error = test_as_type_func(device, context, queue, n_elems,            \
                                   as_type<TYPE1, TYPE2>());                   \
