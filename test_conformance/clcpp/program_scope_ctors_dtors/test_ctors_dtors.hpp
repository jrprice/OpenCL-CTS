...
struct ps_ctor_test : public ps_ctors_dtors_test_base
{
    ps_ctor_test(const cl_uint test_value)
        : ps_ctors_dtors_test_base(true, false), m_test_value(test_value)
    {}

    std::string str() { return "ps_ctor_test"; }

    std::vector<std::string> get_kernel_names()
    {
        return { this->str() + "_set", this->str() + "_read" };
    }

    // Returns value that is expected to be in output_buffer[i]
    cl_uint operator()(size_t i)
    {
        if (i % 2 == 0) return m_test_value;
        return cl_uint(0xbeefbeef);
    }

    // In 1st kernel 0th work-tem sets member m_x of program scope variable
    // global_var to m_test_value and m_y to uint(0xbeefbeef), In 2nd kernel: 1)
    // if global id is even, then work-item reads global_var.m_x and writes it
    // to output[its-global-id]; 2) otherwise, work-item reads global_var.m_y
    // and writes it to output[its-global-id].
...
// -----------------------------------------------------------------------------------
        ...
        return "__kernel void " + this->get_kernel_names()[0]
        + "(global uint *output)\n"
          "{\n"
        ...
        return "#include <opencl_memory>\n"
               ...
               // non-trivial ctor
               "   ctor_test_class(T x, T y) : ctor_test_class_base<T>(x), "
               "m_y(y) { };\n"
               "   T m_y;\n"
               "};\n"
...
struct ps_dtor_test : public ps_ctors_dtors_test_base
{
    ps_dtor_test(const cl_uint test_value)
        : ps_ctors_dtors_test_base(false, true), m_test_value(test_value)
    {}

    std::string str() { return "ps_dtor_test"; }
    ...
    // In 1st kernel 0th work-item saves pointer to output buffer and its size
    // in program scope variable global_var, it also sets counter to 1; After
    // global_var is destroyed all even elements of output buffer should equal
    // m_test_value, and all odd should equal 1. If odd elements of output
    // buffer are >1 it means dtor was executed more than once.
    std::string generate_program()
    {
// -----------------------------------------------------------------------------------
// ------------- ONLY FOR OPENCL 22 CONFORMANCE TEST 22 DEVELOPMENT
...
// -----------------------------------------------------------------------------------
        ...
        + ";\n"
          ...
          // When global_var is being destroyed, first dtor ~dtor_test_class is
          // called, and then ~dtor_test_class_base is called.
...
struct ps_ctor_dtor_test : public ps_ctors_dtors_test_base
{
    ...
    std::string str() { return "ps_ctor_dtor_test"; }

    // Returns value that is expected to be in output_buffer[i]
    cl_uint operator()(size_t i) { return m_test_value; }

    // In 1st kernel 0th work-item saves pointer to output buffer and its size
    // in program scope variable global_var. After global_var is destroyed all
    // even elements of output buffer should equal m_test_value, and all odd
    // should equal 1.
    std::string generate_program()
    {
// -----------------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------------
        ...
        return "#include <opencl_memory>\n"
               ...
               "};\n"
               // global scope program variable
               "ctor_dtor_test_class global_var("
        + std::to_string(m_test_value)
        + ");\n"
...
// This contains tests for program scope (global) constructors and destructors,
// more detailed tests are also in clcpp/api.
