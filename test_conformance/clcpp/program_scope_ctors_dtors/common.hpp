...
#define RUN_PS_CTORS_DTORS_TEST_MACRO(TEST_CLASS)                              \
    last_error =                                                               \
    run_ps_ctor_dtor_test(device, context, queue, count, TEST_CLASS);          \
...
struct ps_ctors_dtors_test_base : public detail::base_func_type<cl_uint>
{
    ...
    ps_ctors_dtors_test_base(const bool ctor, const bool dtor)
    ...
    virtual cl_int execute(const std::vector<cl_kernel>& kernels,
                           cl_mem& output_buffer, cl_command_queue& queue,
                           size_t work_size)
    {
        cl_int err;
        for (auto& k : kernels)
        {
            ...
            err = clEnqueueNDRangeKernel(queue, k, 1, NULL, &work_size, NULL, 0,
                                         NULL, NULL);
    ...
    virtual cl_int ctors_dtors_present_queries(cl_program program)
    {
        ...
        // This indicates that the program object contains non-trivial
        // destructor(s) that will be executed by runtime when program is
        // destroyed.
        ...
        program, CL_PROGRAM_SCOPE_GLOBAL_CTORS_PRESENT, sizeof(cl_bool),
        static_cast<void*>(&ctors_present), &cl_bool_size);
        RETURN_ON_CL_ERROR(error, "clGetProgramInfo")
        if (cl_bool_size != sizeof(cl_bool))
        {
            error = -1;
            CHECK_ERROR_MSG(error,
                            "Test failed, param_value_size_ret != "
                            "sizeof(cl_bool) (%lu != %lu).\n",
                            cl_bool_size, sizeof(cl_bool));
        }

        // CL_PROGRAM_SCOPE_GLOBAL_DTORS_PRESENT
        cl_bool dtors_present = 0;
        error = clGetProgramInfo(
        program, CL_PROGRAM_SCOPE_GLOBAL_DTORS_PRESENT, sizeof(cl_bool),
        static_cast<void*>(&ctors_present), &cl_bool_size);
        RETURN_ON_CL_ERROR(error, "clGetProgramInfo")
        if (cl_bool_size != sizeof(cl_bool))
        {
        ...
        if (m_ctor && ctors_present != CL_TRUE)
        {
            ...
            CHECK_ERROR_MSG(
            error,
            "Test failed, CL_PROGRAM_SCOPE_GLOBAL_CTORS_PRESENT: 0, should be: "
            "1.\n");
        }
        else if (!m_ctor && ctors_present == CL_TRUE)
        {
            ...
            CHECK_ERROR_MSG(
            error,
            "Test failed, CL_PROGRAM_SCOPE_GLOBAL_CTORS_PRESENT: 1, should be: "
            "0.\n");
        }
        ...
        if (m_dtor && dtors_present != CL_TRUE)
        {
            ...
            CHECK_ERROR_MSG(
            error,
            "Test failed, CL_PROGRAM_SCOPE_GLOBAL_DTORS_PRESENT: 0, should be: "
            "1.\n");
        }
        else if (!m_dtor && dtors_present == CL_TRUE)
        {
            ...
            CHECK_ERROR_MSG(
            error,
            "Test failed, CL_PROGRAM_SCOPE_GLOBAL_DTORS_PRESENT: 1, should be: "
            "0.\n");
        }
        return error;
#endif
...
int run_ps_ctor_dtor_test(cl_device_id device, cl_context context,
                          cl_command_queue queue, size_t count,
                          ps_ctor_dtor_test op)
{
    cl_mem buffers[1];
    cl_program program;
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    err = create_opencl_kernel(context, &program, &(kernels[0]), code_str,
                               kernel_names[0]);
    return err;
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    for (size_t i = 0; i < output.size(); i++)
    {
        ...
        {
            RETURN_ON_ERROR_MSG(-1, "test_%s(%s) failed. Expected: %s, got: %s",
                                op.str().c_str(), type_name<cl_uint>().c_str(),
                                format_value(v).c_str(),
                                format_value(output[i]).c_str());
        }
    }
