...
std::string generate_kernel_atomic_fetch(func_type func)
{
    ...
    std::string out1_value =
    "output[gid / " + std::to_string(atomic_bucket_size) + "]";
    std::string function_call =
    "atomic_" + func.str() + "(&" + out1_value + ", " + in1_value + ")";
    return "" + func.defs() + "__kernel void test_" + func.str() + "(global "
    + type_name<type>() + " *input, global atomic_" + type_name<type>()
    + " *output)\n"
      "{\n"
...
std::string generate_kernel_atomic_fetch(func_type func)
{
    ...
    + "#include <opencl_memory>\n"
      ...
      "using namespace cl;\n"
      "__kernel void test_"
    + func.str() + "(global_ptr<" + type_name<type>()
    + "[]> input,"
      "global_ptr<atomic<"
    + type_name<type>()
    + ">[]> output)\n"
...
bool verify_atomic_fetch(const std::vector<TYPE> &in,
                         const std::vector<TYPE> &out, atomic_fetch op)
{
...
int test_atomic_fetch_func(cl_device_id device, cl_context context,
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    generate_input<TYPE>(count, op.min1(), op.max1(), std::vector<TYPE>());
    std::vector<TYPE> output =
    generate_output<TYPE>((count - 1) / atomic_bucket_size + 1);
    ...
    err = clEnqueueFillBuffer(queue, buffers[1], &pattern, sizeof(pattern), 0,
                              sizeof(TYPE) * output.size(), 0, NULL, NULL);
    ...
    {
        RETURN_ON_ERROR_MSG(-1, "test_%s %s failed", op.str().c_str(),
...
template <class TYPE> struct atomic_fetch
{
    typedef TYPE in_type;

    std::string decl_str() { return type_name<TYPE>(); }
    ...
    std::string headers() { return "#include <opencl_atomic>\n"; }

    TYPE min1() { return 0; }

    TYPE max1() { return 1000; }
};


#define DEF_ATOMIC_FETCH_FUNC(CLASS_NAME, FUNC_NAME, HOST_FUNC_EXPRESSION,     \
                              INIT_OUT)                                        \
    template <class TYPE> struct CLASS_NAME : public atomic_fetch<TYPE>        \
    {                                                                          \
        std::string str() { return #FUNC_NAME; }                               \
                                                                               \
        TYPE init_out() { return INIT_OUT; }                                   \
                                                                               \
...
DEF_ATOMIC_FETCH_FUNC(atomic_fetch_sub, fetch_sub, x - y,
                      (std::numeric_limits<TYPE>::max)())

DEF_ATOMIC_FETCH_FUNC(atomic_fetch_and, fetch_and, x &y,
                      (std::numeric_limits<TYPE>::max)())
...
DEF_ATOMIC_FETCH_FUNC(atomic_fetch_min, fetch_min, (std::min)(x, y),
                      (std::numeric_limits<TYPE>::max)())
...
#define TEST_ATOMIC_MACRO(TEST_CLASS)                                          \
    last_error =                                                               \
    test_atomic_fetch_func(device, context, queue, n_elems, TEST_CLASS);       \
