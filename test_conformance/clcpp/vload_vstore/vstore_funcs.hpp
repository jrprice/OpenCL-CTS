...
std::string generate_kernel_vstore(func_type func)
{
    ...
    return "__kernel void test_" + func.str() + "(global " + input1_type_str
    + " *input, global " + output1_type_str
    + " *output)\n"
      "{\n"
      "    size_t gid = get_global_id(0);\n"
      "    "
    + func.str() + std::to_string(N)
    + "(input[gid], gid, output);\n"
      "}\n";
}
#else
template <class func_type, class in_type, class out_type, size_t N>
std::string generate_kernel_vstore(func_type func)
{
    ...
    + "#include <opencl_memory>\n"
      ...
      "using namespace cl;\n"
      "__kernel void test_"
    + func.str() + "(global_ptr<" + input1_type_str
    + "[]> input,"
      "global_ptr<"
    + output1_type_str
    + "[]> output)\n"
      ...
      "    "
    + func.str()
    + "(input[gid], gid, output.get());\n"
      "}\n";
}
#endif

template <class INPUT, class OUTPUT, class vload_op>
bool verify_vstore(const std::vector<INPUT> &in, const std::vector<OUTPUT> &out,
                   vload_op op)
{
    for (size_t i = 0; i < in.size(); i++)
    {
        ...
        for (size_t j = 0; j < vload_op::vector_size; j++)
        {
            ...
            if (!are_equal(expected.s[j], out[idx],
                           op.delta(in[i], expected).s[j], op))
            {
...
int test_vstore_func(cl_device_id device, cl_context context,
                     cl_command_queue queue, size_t count, vload_op op)
{
    cl_mem buffers[2];
    cl_program program;
    cl_kernel kernel;
    ...
    std::string code_str =
    generate_kernel_vstore<vload_op, INPUT, OUTPUT, vload_op::vector_size>(op);
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    generate_output<OUTPUT>(count * vector_size<INPUT>::value);
...
struct vstore_func : public unary_func<typename make_vector_type<T, N>::type, T>
{
    ...
    std::string str() { return "vstore"; }

    std::string headers() { return "#include <opencl_vector_load_store>\n"; }

    result_type operator()(const input1_type &in)
    {
        static_assert(!is_vector_type<T>::value, "T must be scalar type");
        return in;
    }

    bool is_out_half() { return false; }
};

template <cl_int N /* Vector size */>
struct vstore_half_func
    : public unary_func<typename make_vector_type<cl_float, N>::type, cl_half>
{
    ...
    std::string str() { return "vstore_half"; }
    ...
    input1_type min1() { return detail::make_value<input1_type>(-512.f); }

    input1_type max1() { return detail::make_value<input1_type>(512.f); }

    bool is_out_half() { return true; }
};

template <cl_int N /* Vector size */>
struct vstorea_half_func
    : public unary_func<typename make_vector_type<cl_float, N>::type, cl_half>
{
    typedef typename make_vector_type<cl_float, N>::type input1_type;
    typedef typename make_vector_type<cl_half, N>::type result_type;
    ...
    std::string str() { return "vstorea_half"; }
...
#define TEST_VSTORE_FUNC_MACRO(CLASS)                                          \
    last_error = test_vstore_func(device, context, queue, n_elems, CLASS);     \
