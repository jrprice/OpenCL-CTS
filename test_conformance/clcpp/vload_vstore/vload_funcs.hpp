...
bool verify_vload(const std::vector<INPUT> &in, const std::vector<OUTPUT> &out,
                  vload_op op)
{
    for (size_t i = 0; i < out.size(); i++)
    {
        ...
        for (size_t j = 0; j < vload_op::vector_size; j++)
        {
            ...
            if (!are_equal(expected.s[j], out[i].s[j],
                           op.delta(in[idx], expected.s[j]), op))
            {
...
int test_vload_func(cl_device_id device, cl_context context,
                    cl_command_queue queue, size_t count, vload_op op)
{
    ...
    std::string code_str =
    generate_kernel_vload<vload_op, INPUT, OUTPUT, vload_op::vector_size>(op);
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    count * vector_size<OUTPUT>::value, op.min1(), op.max1(),
    op.in_special_cases(), op.is_in1_half());
      ...
      >
{
    ...
    {
        static_assert(!is_vector_type<IN1>::value, "IN1 must be scalar type");
        static_assert(
        ...
        typedef
        typename std::iterator_traits<Iterator>::difference_type diff_type;
    ...
    bool is_in1_half() { return false; }
};

template <cl_int N /* Vector size */>
struct vload_half_func
    : public unary_func<
      cl_half,
      ...
      >
{
    ...
    std::string str() { return "vload_half"; }
    ...
    bool is_in1_half() { return true; }
};

template <cl_int N /* Vector size */>
struct vloada_half_func
    : public unary_func<
      cl_half,
      typename make_vector_type<cl_float, N>::type /* create IN1N type */
      >
{
    ...
    std::string str() { return "vloada_half"; }
...
#define TEST_VLOAD_FUNC_MACRO(CLASS)                                           \
    last_error = test_vload_func(device, context, queue, n_elems, CLASS);      \
