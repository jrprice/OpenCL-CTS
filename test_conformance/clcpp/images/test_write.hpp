...
struct image_test : image_test_base<ImageType, ChannelType>
{
    ...
    image_test(cl_channel_order channel_order): channel_order(channel_order) {}
// -----------------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------------
    ...
    std::string generate_source()
    {
        ...
        s << R"(
        typedef )"
          << type_name<typename image_test::element_type>() << R"( element_type;

        kernel void test(
            write_only )"
          << image_test::image_type_name() << R"(_t img,
            ...
            write_image)"
          << image_test::function_suffix() << "(img, coords[gid]."
          << image_test::coord_accessor() << R"(, input[gid]);
        }
        )";
    ...
    std::string generate_source()
    {
        ...
        kernel void test(
          ...
          << image_test::image_type_name()
          << R"(<element_type, image_access::write> img,
            const global_ptr<int4[]> coords,
            const global_ptr<element_type[]> input
        ) {
            const ulong gid = get_global_linear_id();

            img.write(coords[gid].)"
          << image_test::coord_accessor() << R"(, input[gid]);
        }
        )";
    ...
    int run(cl_device_id device, cl_context context, cl_command_queue queue,
            int num_elements)
    {
...
// Normal run
        ...
        get_channel_order_channel_count(image_format.image_channel_order);
        ...
        create_image_descriptor(image_desc, &image_format);

        std::vector<channel_type> random_image_values =
        generate_input(image_desc.image_width * image_desc.image_height
                       * image_desc.image_depth * channel_count,
        ...
        count, detail::make_value<coord_type>(0),
        coord_type{ static_cast<cl_int>(image_desc.image_width - 1),
                    static_cast<cl_int>(image_desc.image_height - 1),
                    static_cast<cl_int>(image_desc.image_depth - 1), 0 },
        ...
        for (size_t i = 0; i < count; i++)
        {
            ...
            // Use read_image_pixel from harness/imageHelpers to fill input
            // values (it will deal with correct channels, orders etc.)
            read_image_pixel<scalar_element_type>(
            static_cast<void *>(random_image_values.data()), &image_info,
            c.s[0], c.s[1], c.s[2], input[i].s);
        }

        // image_row_pitch and image_slice_pitch must be 0, when clCreateImage
        // is used with host_ptr = NULL
        image_desc.image_row_pitch = 0;
        image_desc.image_slice_pitch = 0;
        cl_mem img = clCreateImage(context, CL_MEM_WRITE_ONLY, &image_format,
                                   &image_desc, NULL, &error);
        RETURN_ON_CL_ERROR(error, "clCreateImage")

        cl_mem coords_buffer = clCreateBuffer(
        ...
        clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR,
                       ...
                       static_cast<void *>(input.data()), &error);
        RETURN_ON_CL_ERROR(error, "clCreateBuffer")

        error = clSetKernelArg(kernel, 0, sizeof(cl_mem), &img);
        RETURN_ON_CL_ERROR(error, "clSetKernelArg")
        error =
        clSetKernelArg(kernel, 1, sizeof(coords_buffer), &coords_buffer);
        RETURN_ON_CL_ERROR(error, "clSetKernelArg")
        ...
        std::vector<channel_type> image_values(
        image_desc.image_width * image_desc.image_height
        * image_desc.image_depth * channel_count);

        const size_t origin[3] = { 0 };
        const size_t region[3] = { image_desc.image_width,
                                   image_desc.image_height,
                                   image_desc.image_depth };
        error = clEnqueueReadImage(queue, img, CL_TRUE, origin, region, 0, 0,
                                   static_cast<void *>(image_values.data()), 0,
                                   NULL, NULL);
        ...
        for (size_t i = 0; i < count; i++)
        {
            ...
            element_type result;
            read_image_pixel<scalar_element_type>(
            static_cast<void *>(image_values.data()), &image_info, c.s[0],
            c.s[1], c.s[2], result.s);

            if (!are_equal(result, expected))
            {
                RETURN_ON_ERROR_MSG(
                -1, "Writing to coordinates %s failed. Expected: %s, got: %s",
                format_value(c).c_str(), format_value(expected).c_str(),
...
int run_test_cases(cl_device_id device, cl_context context,
                   cl_command_queue queue, int num_elements)
{
    if (!is_test_supported(device)) return CL_SUCCESS;
...
(cl_device_id device, cl_context context, cl_command_queue queue,
 int num_elements)
{
    return run_test_cases<CL_MEM_OBJECT_IMAGE1D>(device, context, queue,
                                                 num_elements);
}
...
(cl_device_id device, cl_context context, cl_command_queue queue,
 int num_elements)
{
    return run_test_cases<CL_MEM_OBJECT_IMAGE2D>(device, context, queue,
                                                 num_elements);
}
...
(cl_device_id device, cl_context context, cl_command_queue queue,
 int num_elements)
{
    return run_test_cases<CL_MEM_OBJECT_IMAGE3D>(device, context, queue,
                                                 num_elements);
}
