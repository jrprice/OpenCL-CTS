...
const sampler_source sampler_sources[] = { sampler_source::param,
                                           sampler_source::program_scope };
...
struct image_test : image_test_base<ImageType, ChannelType>
{
    ...
    image_test(cl_channel_order channel_order, sampler_source source)
        : channel_order(channel_order), source(source)
    {}
...
// -----------------------------------------------------------------------------------
    ...
    std::string generate_source()
    {
        ...
        kernel void test(
          ...
          << image_test::image_type_name() << R"(_t img,
            ...
            output[gid] = read_image)"
          << image_test::function_suffix() << "(img, " << sampler
          << ", coords[gid]." << image_test::coord_accessor() << R"();
        }
        )";
    ...
    std::string generate_source()
    {
        ...
        kernel void test(
          ...
          << R"(<element_type, image_access::sample> img,
            ...
            output[gid] = img.sample()"
          << sampler << ", coords[gid]." << image_test::coord_accessor()
          << R"();
        }
        )";
...
// Normal run
        ...
        &image_desc, static_cast<void *>(image_values.data()), &error);
        ...
        context, CL_MEM_READ_ONLY, sizeof(element_type) * count, NULL, &error);
        RETURN_ON_CL_ERROR(error, "clCreateBuffer")
        ...
        queue, output_buffer, CL_TRUE, 0, sizeof(element_type) * count,
                   ...
                   cl_command_queue queue, int num_elements)
{
    ...
    for (auto channel_order : get_channel_orders(device))
        for (auto source : sampler_sources)
        {
            ...
            RETURN_ON_ERROR(error)
            error =
            image_test<ImageType, CL_SIGNED_INT16>(channel_order, source)
            .run(device, context, queue, num_elements);
            RETURN_ON_ERROR(error)
            error =
            image_test<ImageType, CL_SIGNED_INT32>(channel_order, source)
            .run(device, context, queue, num_elements);
            RETURN_ON_ERROR(error)

            error =
            image_test<ImageType, CL_UNSIGNED_INT8>(channel_order, source)
            .run(device, context, queue, num_elements);
            RETURN_ON_ERROR(error)
            error =
            image_test<ImageType, CL_UNSIGNED_INT16>(channel_order, source)
            .run(device, context, queue, num_elements);
            RETURN_ON_ERROR(error)
            error =
            image_test<ImageType, CL_UNSIGNED_INT32>(channel_order, source)
            .run(device, context, queue, num_elements);
            RETURN_ON_ERROR(error)
