...
{
    std::string offset1s =
    options.offset1_param ? "offset1" : std::to_string(options.offset1);
    std::string offset2s =
    options.offset2_param ? "offset2" : std::to_string(options.offset2);

    std::string init = "i = 0";
    std::string cond = std::string("i < ")
    + (options.iter_count_param ? "iter_count"
                                : std::to_string(options.iter_count));
    std::string incr = "i += 2";
    ...
    if (options.loop == loop_kind::for_loop)
        s << "for (" << (options.init_in_header ? init : "") << ";"
        ...
        s << "do";
    s << "{" << std::endl;
    if (!options.cond_in_header)
        s << "if (!(" << cond << ")) break;" << std::endl;
    s << "a[i + " << offset1s << "] = b[i + " << offset1s << "] * c[i + "
      << offset1s << "];" << std::endl;
...
std::string generate_source(test_options options)
{
    ...
    // Loop #2
    if (!options.init_in_header) s << init << ";" << std::endl;
    if (options.ivdep_length > 0)
        s << "[[cl::ivdep(" << options.ivdep_length << ")]]" << std::endl;
    if (options.loop == loop_kind::for_loop)
        s << "for (" << (options.init_in_header ? init : "") << ";"
          << (options.cond_in_header ? cond : "") << ";"
          << (options.incr_in_header ? incr : "") << ")";
...
// Normal run
    ...
    error =
    clEnqueueReadBuffer(queue, a_buffer, CL_TRUE, 0, sizeof(int) * count,
                        static_cast<void *>(a_output.data()), 0, NULL, NULL);
    RETURN_ON_CL_ERROR(error, "clEnqueueReadBuffer")

    for (int i = 0; i < options.iter_count; i += 2)
    {
        a[i + options.offset1] =
        b[i + options.offset1] * c[i + options.offset1];
    }
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (value != expected)
        {
            RETURN_ON_ERROR_MSG(-1,
                                "Test failed. Element %lu: %d should be: %d", i,
                                value, expected);
        }
    }
                        ...
                        for (bool cond_in_header : { false, true })
                            for (bool init_in_header : { false, true })
                                for (bool incr_in_header : { false, true })
                                {
                                    ...
                                    error =
                                    test(device, context, queue, options);
                                    RETURN_ON_ERROR(error)
                                }
