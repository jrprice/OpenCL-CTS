...
const param_kind param_kinds[] = { param_kind::ptr_type, param_kind::ptr,
                                   param_kind::ref };
...
std::string generate_source(test_options options)
{
    ...
    if (options.spec_const)
    {
        ...
        s << "cl::spec_constant<int, 1> max_size_spec{ 1234567890 };"
          << std::endl;
    }

    s << "kernel void test(";
    s << "[[cl::max_size("
      << (options.spec_const ? "max_size_spec"
                             : std::to_string(options.max_size))
      << ")]] ";
    s << (options.space == address_space::constant ? "cl::constant"
                                                   : "cl::local");
...
// Normal run
    ...
    if (options.spec_const)
    {
        error = clSetProgramSpecializationConstant(program, 1, sizeof(cl_int),
                                                   &options.max_size);
    ...
    const int sizes[] = { 1, max_size / 2, max_size, max_size + 1,
                          max_size * 2 };
        ...
        else
        {
            ...
            if (error != CL_MAX_SIZE_RESTRICTION_EXCEEDED)
            {
                RETURN_ON_ERROR_MSG(-1,
                                    "clSetKernelArg must fail with "
                                    "CL_MAX_SIZE_RESTRICTION_EXCEEDED,"
                                    " but returned %s (%d)",
                                    get_cl_error_string(error).c_str(), error);
            }
        }
 ...
 int num_elements)
{
    ...
    cl_ulong max_size;
    error = clGetDeviceInfo(device, CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE,
 ...
 int num_elements)
{
    ...
    cl_ulong max_size;
    error = clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(max_size),
                            &max_size, NULL);
    RETURN_ON_CL_ERROR(error, "clGetDeviceInfo")
