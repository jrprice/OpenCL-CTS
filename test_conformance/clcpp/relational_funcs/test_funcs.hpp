...
#define DEF_UNARY_TEST_FUNC(CLASS_NAME, FUNC_NAME, HOST_FUNC_EXPRESSION)       \
    ...
    struct CLASS_NAME                                                          \
        : public unary_func<                                                   \
          typename make_vector_type<cl_float, N>::type, /* create cl_floatN    \
                                                           type */             \
    ...
    {                                                                          \
        ...
        std::string str() { return #FUNC_NAME; }                               \
                                                                               \
        std::string headers() { return "#include <opencl_relational>\n"; }     \
                                                                               \
        ...
        {                                                                      \
            ...
            x, [](const SCALAR& a) {                                           \
                ...
                return cl_int(0);                                              \
                                                                               ...
                                                                               \
        bool is_out_bool() { return true; }                                    \
                                                                               \
        input_type min1() { return detail::def_limit<input_type>(-10000.0f); } \
                                                                               \
        input_type max1() { return detail::def_limit<input_type>(10000.0f); }  \
                                                                               \
        std::vector<input_type> in1_special_cases()                            \
        {                                                                      \
            typedef typename scalar_type<input_type>::type SCALAR;             \
            return { detail::make_value<input_type>(                           \
                     std::numeric_limits<SCALAR>::infinity()),                 \
                     detail::make_value<input_type>(                           \
                     -std::numeric_limits<SCALAR>::infinity()),                \
                     detail::make_value<input_type>(                           \
...
#define DEF_BINARY_TEST_FUNC(CLASS_NAME, FUNC_NAME, HOST_FUNC_EXPRESSION)      \
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            x, y, [](const SCALAR& a, const SCALAR& b) {                       \
                if (HOST_FUNC_EXPRESSION)                                      \
                {                                                              \
                    return cl_int(1);                                          \
                }                                                              \
        ...
        input_type max1() { return detail::def_limit<input_type>(10000.0f); }  \
                                                                               \
        input_type min2() { return detail::def_limit<input_type>(-10000.0f); } \
                                                                               \
        input_type max2() { return detail::def_limit<input_type>(10000.0f); }  \
                                                                               \
        std::vector<input_type> in1_special_cases()                            \
        {                                                                      \
            typedef typename scalar_type<input_type>::type SCALAR;             \
...
DEF_BINARY_TEST_FUNC(test_func_isunordered, isunordered,
                     (std::isunordered)(a, b))
      ...
      >
{
    ...
    bool is_in1_bool() { return true; }

    std::vector<input_type> in1_special_cases()
    {
        return { detail::make_value<input_type>(0),
...
// Tests for all(booln x) and any(booln x) are not run in USE_OPENCLC_KERNELS
// mode, because those functions in OpenCL C require different reference
// functions on host compared to their equivalents from OpenCL C++. (In OpenCL C
// those functions returns true/false based on the most significant bits in
// any/all component/s of x)
#ifndef USE_OPENCLC_KERNELS
    TEST_UNARY_REL_FUNC_MACRO(test_func_all)
    TEST_UNARY_REL_FUNC_MACRO(test_func_any)
#else
    log_info("WARNING:\n\tTests for bool all(booln x) are not run in "
             "USE_OPENCLC_KERNELS mode\n");
    log_info("WARNING:\n\tTests for bool any(booln x) are not run in "
             "USE_OPENCLC_KERNELS mode\n");
#endif
