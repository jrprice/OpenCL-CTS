      ...
      >
{
    ...
    std::string str() { return "select"; }

    std::string headers() { return "#include <opencl_relational>\n"; }

    result_type operator()(const input1_type& x, const input2_type& y,
                           const input3_type& z)
    {
        ...
        return perform_function<input1_type, input2_type, input3_type,
                                result_type>(
        x, y, z, [](const SCALAR1& a, const SCALAR2& b, const SCALAR3& c) {
            return (c != 0) ? b : a;
        });
    }

    bool is_in3_bool() { return true; }

    std::vector<input3_type> in3_special_cases()
    {
        return { detail::make_value<input3_type>(0),
      ...
      >
{
    ...
    std::string str() { return "bitselect"; }
    ...
    result_type operator()(const input1_type& x, const input2_type& y,
                           const input3_type& z)
    {
        static_assert(std::is_integral<IN1>::value,
                      "bitselect test is implemented only for integers.");
        static_assert(std::is_unsigned<IN1>::value,
                      "IN1 type should be unsigned, bitwise operations on "
                      "signed int may cause problems.");
        typedef typename scalar_type<input1_type>::type SCALAR1;
        typedef typename scalar_type<input2_type>::type SCALAR2;
        typedef typename scalar_type<input3_type>::type SCALAR3;
...
// Tests for select(gentype a, gentype b, booln c) are not run in
// USE_OPENCLC_KERNELS mode, because this functions in OpenCL C requires
// different reference functions on host compared to their equivalent in OpenCL
// C++. (In OpenCL C the result of select(), when gentype is vector type, is
// based on the most significant bits of c components)
...
#else
    log_info("WARNING:\n\tTests for select(gentype a, gentype b, booln c) are "
             "not run in USE_OPENCLC_KERNELS mode\n");
#endif
