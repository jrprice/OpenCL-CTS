...
OUT1 perform_function(
const IN1& in1, const IN2& in2, const IN3& in3, F func,
typename std::enable_if<is_vector_type<OUT1>::value>::type* = 0)
{
    OUT1 result;
...
OUT1 perform_function(
const IN1& in1, const IN2& in2, F func,
typename std::enable_if<is_vector_type<OUT1>::value>::type* = 0)
{
    OUT1 result;
    for (size_t i = 0; i < vector_size<OUT1>::value; i++)
...
template <class IN1>
cl_int perform_all_function(
const IN1& in1, typename std::enable_if<is_vector_type<IN1>::value>::type* = 0)
{
    cl_int result = 1;
...
cl_int perform_all_function(
const cl_int& in1,
typename std::enable_if<!is_vector_type<cl_int>::value>::type* = 0)
{
    return (in1 != 0) ? cl_int(1) : cl_int(0);
}

template <class IN1>
cl_int perform_any_function(
const IN1& in1, typename std::enable_if<is_vector_type<IN1>::value>::type* = 0)
{
    cl_int result = 0;
    for (size_t i = 0; i < vector_size<IN1>::value; i++)
