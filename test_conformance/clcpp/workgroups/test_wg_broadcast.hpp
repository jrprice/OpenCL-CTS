...
{
    return "__kernel void test_wg_broadcast(global uint *input, global uint "
           "*output)\n"
           "{\n"
           "    ulong tid = get_global_id(0);\n"
           "    uint result = work_group_broadcast(input[tid], get_group_id(0) "
           "% get_local_size(0));\n"
           "    output[tid] = result;\n"
           "}\n";
}
...
std::string generate_wg_broadcast_3D_kernel_code()
{
    return "__kernel void test_wg_broadcast(global uint *input, global uint "
           ...
           "    ulong idx = (tid_z * get_global_size(1) * get_global_size(0)) "
           "+ (tid_y * get_global_size(0)) + tid_x;\n"
...
std::string generate_wg_broadcast_1D_kernel_code()
{
    return "#include <opencl_memory>\n"
           ...
           "using namespace cl;\n"
           "__kernel void test_wg_broadcast(global_ptr<uint[]> input, "
...
int verify_wg_broadcast_1D(const std::vector<cl_uint> &in,
                           const std::vector<cl_uint> &out, size_t n,
                           size_t wg_size)
{
    size_t i, j;
    size_t group_id;
    ...
    for (i = 0, group_id = 0; i < n; i += wg_size, group_id++)
    {
        ...
        for (j = 0; j < local_size; j++)
        {
            ...
            {
                log_info(
                "work_group_broadcast: Error at %lu: expected = %u, got = %u\n",
...
int verify_wg_broadcast_2D(const std::vector<cl_uint> &in,
                           const std::vector<cl_uint> &out, size_t nx,
            ...
            for (j = 0, group_id_x = 0; j < nx; j += wg_size_x, group_id_x++)
            {
                ...
                for (_j = 0; _j < local_size_x; _j++)
                {
                    ...
                    if (broadcast_result != out[indx])
                    {
                        ...
                        log_info("%lu\n", ((i + y) * nx + (j + x)));
                        log_info("%lu\n", out.size());
                        log_info("work_group_broadcast: Error at (%lu, %lu): "
                                 "expected = %u, got = %u\n",
...
int verify_wg_broadcast_3D(const std::vector<cl_uint> &in,
                           const std::vector<cl_uint> &out, size_t nx,
            ...
            for (j = 0, group_id_y = 0; j < ny; j += wg_size_y, group_id_y++)
            {
                ...
                for (_j = 0; _j < local_size_y; _j++)
                {
                         ...
                         k += wg_size_x, group_id_x++)
                    {
                        ...
                        cl_uint broadcast_result =
                        in[(i + z) * ny * nz + (j + y) * nx + (k + x)];
                        for (_k = 0; _k < local_size_x; _k++)
                        {
                            ...
                            if (broadcast_result != out[indx])
                            {
                                log_info("work_group_broadcast: Error at (%lu, "
                                         "%lu, %lu): expected = %u, got = %u\n",
                                         k + _k, j + _j, i + _i,
...
int work_group_broadcast(cl_device_id device, cl_context context,
                         cl_command_queue queue, size_t count, size_t dim)
{
    cl_mem buffers[2];
    cl_program program;
    cl_kernel kernel;
    ...
    std::string code_str;
    if (dim > 2)
        code_str = generate_wg_broadcast_3D_kernel_code();
    else if (dim > 1)
        code_str = generate_wg_broadcast_2D_kernel_code();
    else
        code_str = generate_wg_broadcast_1D_kernel_code();
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    err = create_opencl_kernel(context, &program, &kernel, code_str,
                               "test_wg_broadcast", "-cl-std=CL2.0", false);
    RETURN_ON_ERROR(err)
#else
    err = create_opencl_kernel(context, &program, &kernel, code_str,
                               "test_wg_broadcast");
    ...
    err = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_WORK_GROUP_SIZE,
                                   sizeof(size_t), &flat_wg_size, NULL);
    ...
    if (dim > 2)
    {
        ...
        std::ceil(static_cast<double>(count / 3)
    ...
    {
        size_t wg_number =
        static_cast<size_t>(std::ceil(static_cast<double>(count) / wg_size[0]));
        flat_work_size = wg_number * wg_size[0];
        work_size[0] = flat_work_size;
    }

    std::vector<cl_uint> input =
    generate_input_wg_broadcast(flat_work_size, flat_wg_size);
    std::vector<cl_uint> output =
    generate_output_wg_broadcast(flat_work_size, flat_wg_size);
    ...
    err = clEnqueueNDRangeKernel(queue, kernel, dim, NULL, work_size, wg_size,
                                 0, NULL, NULL);
    ...
    if (dim > 2)
    {
        result = verify_wg_broadcast_3D(input, output, work_size[0],
                                        work_size[1], work_size[2], wg_size[0],
                                        wg_size[1], wg_size[2]);
    }
    // 2D
    else if (dim > 1)
    {
        result = verify_wg_broadcast_2D(input, output, work_size[0],
                                        work_size[1], wg_size[0], wg_size[1]);
    }
    // 1D
    else
    {
        result =
        verify_wg_broadcast_1D(input, output, work_size[0], wg_size[0]);
    }
