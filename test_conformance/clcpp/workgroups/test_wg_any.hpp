...
{
    return "__kernel void test_wg_any(global uint *input, global uint "
           "*output)\n"
           "{\n"
...
std::string generate_wg_any_kernel_code()
{
    return "#include <opencl_memory>\n"
           ...
           "using namespace cl;\n"
           "__kernel void test_wg_any(global_ptr<uint[]> input, "
...
int verify_wg_any(const std::vector<cl_uint> &in,
    ...
    for (i = 0; i < count; i += wg_size)
    {
        ...
        for (j = 0; j < ((count - i) > wg_size ? wg_size : (count - i)); j++)
        {
            if (any_uint != out[i + j])
            {
                ...
                static_cast<size_t>(any_uint), static_cast<size_t>(out[i + j]));
                return -1;
            }
        }
    }
...
int work_group_any(cl_device_id device, cl_context context,
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    err =
    create_opencl_kernel(context, &program, &kernel, code_str, "test_wg_any");
    RETURN_ON_ERROR(err)
    return err;
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
#elif defined(DEVELOPMENT) && defined(USE_OPENCLC_KERNELS)
    err = create_opencl_kernel(context, &program, &kernel, code_str,
                               "test_wg_any", "-cl-std=CL2.0", false);
    RETURN_ON_ERROR(err)
#else
    err =
    create_opencl_kernel(context, &program, &kernel, code_str, "test_wg_any");
    ...
    generate_input_wg_any(flat_work_size + 1, wg_size);
    std::vector<cl_uint> output =
    generate_output_wg_any(flat_work_size, wg_size);
