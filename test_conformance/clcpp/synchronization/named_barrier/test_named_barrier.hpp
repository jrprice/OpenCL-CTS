...
struct local_fence_named_barrier_test
    : public work_group_named_barrier_test_base
{
    std::string str() { return "local_fence"; }
...
// -----------------------------------------------------------------------------------
        ...
        return "__kernel void " + this->get_kernel_name()
        + "(global uint *output, "
          "local uint * lmem)\n"
          "{\n"
        ...
        + "(global_ptr<uint[]> output, "
          ...
          "  {\n"
          "    size_t other_lid = (lid + get_max_sub_group_size()) % "
          "get_enqueued_local_size(0);\n"
          "    size_t other_gid = (gid - lid) + other_lid;\n"
          "    lmem[other_lid] = other_gid;\n"
          ...
          "    {\n"
          "      const size_t two_first_subgroups = 2 * "
          "get_max_sub_group_size();"
          // local and global id of some work-item outside of work-item
          // subgroup, but within subgroups 0 and 1.
          "      size_t other_lid = (lid + get_max_sub_group_size()) % "
          "two_first_subgroups;\n"
          "      size_t other_gid = (gid - lid) + other_lid;\n"
          "      lmem[other_lid] = other_gid;\n"
          "      b.wait(mem_fence::local);\n" // subgroup 0 and 1 are sync
                                              // (local)
          "      value = lmem[lid];" // lmem[lid] shoule be equal to gid
          "    }\n"
          "    else\n"
                   ...
                   const size_t work_group_size)
    {
        ...
        err |=
        clSetKernelArg(kernel, 1, work_group_size * sizeof(cl_uint), NULL);
...
struct global_fence_named_barrier_test
    : public work_group_named_barrier_test_base
{
    std::string str() { return "global_fence"; }
    ...
    // At the end every work-item writes its local id to
    // ouput[work-item-global-id].
    std::string generate_program()
    {
// -----------------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------------
        ...
        + "(global_ptr<uint[]> output, "
          ...
          "    size_t other_lid_same_subgroup = (lid + 2) % "
          "get_sub_group_size();\n"
          "    size_t other_gid_same_subgroup = (gid - lid) + "
          "other_lid_same_subgroup;\n"
          "    temp[other_gid_same_subgroup] = temp[other_gid_same_subgroup] - "
          "1;\n"
          ...
          "    size_t other_gid = (gid - lid) + other_lid;\n"
          "    temp[other_gid] = other_lid + 1;\n"
          "    b.wait(mem_fence::global);\n" // both subgroups wait, both are
                                             // sync
          "    size_t other_lid_same_subgroup = "
          "((lid + 1) % get_sub_group_size()) + (get_sub_group_id() * "
          "get_sub_group_size());\n"
          "    size_t other_gid_same_subgroup = (gid - lid) + "
          "other_lid_same_subgroup;\n"
          "    temp[other_gid_same_subgroup] = temp[other_gid_same_subgroup] - "
          ...
          "      b.wait(mem_fence::global);\n" // both subgroups wait, both are
                                               // sync
          // local and global id of some other work-item within work-item
          // subgroup
          "      size_t other_lid_same_subgroup = "
          "((lid + 1) % get_sub_group_size()) + (get_sub_group_id() * "
          "get_sub_group_size());\n"
...
struct global_local_fence_named_barrier_test
    : public work_group_named_barrier_test_base
{
    std::string str() { return "global_local_fence"; }
 ...
 int num_elements)
{
    ...
    // The exact maximum number can be queried using clGetDeviceInfo with
    // CL_DEVICE_MAX_NAMED_BARRIER_COUNT_KHR from the OpenCL 2.2 Extension
    // Specification.
    cl_uint named_barrier_count;
    error = clGetDeviceInfo(device, CL_DEVICE_MAX_NAMED_BARRIER_COUNT_KHR,
                            sizeof(cl_uint), &named_barrier_count, NULL);
    RETURN_ON_CL_ERROR(error, "clGetDeviceInfo")

    if (named_barrier_count < 8)
    {
        RETURN_ON_ERROR_MSG(
        -1, "Maximum number of named barriers must be at least 8.");
    }
#endif
