...
struct spec_example_work_group_named_barrier_test
    : public work_group_named_barrier_test_base
{
    std::string str() { return "spec_example"; }

    // Return value that is expected to be in output_buffer[i]
    cl_uint operator()(size_t i, size_t work_group_size,
                       size_t mas_sub_group_size)
    {
        return static_cast<cl_uint>(i);
    }

    // At the end every work-item writes its global id to
    // ouput[work-item-global-id].
...
// -----------------------------------------------------------------------------------
        ...
        return
        // In OpenCL C kernel we imitate subgroups by partitioning work-group
        // (based on local ids of work-items), work_group_named_barrier.wait(..)
        // calls are replaced with work_group_barriers.
        "__kernel void " + this->get_kernel_name()
        + "(global uint *output, "
          "global uint * temp, "
          "local uint * lmem)\n"
          "{\n"
          ...
          // [0 - e_wg)[ew_g; q_wg)[q_wg; 3 * ew_g)[3 * ew_g; h_wg)[h_wg;
          // get_local_size(0) - 1] to simulate 8 subgroups
...
#else
        return "#define cl_khr_subgroup_named_barrier\n"
               "#include <opencl_memory>\n"
               "#include <opencl_work_item>\n"
               "#include <opencl_synchronization>\n"
               "using namespace cl;\n"

               "void b_function(work_group_named_barrier &b, size_t value, "
               "local_ptr<uint[]> lmem)\n"
               "{\n\n"
               "size_t lid = get_local_id(0);\n"
               // Work-items from the 1st subgroup writes to local memory that
               // will be later read byt the 0th subgroup, and the other way
               // around - 0th subgroup writes what 1st subgroup will later
               // read. b.wait(mem_fence::local) should provide sync between
               // those two subgroups.
               "if(get_sub_group_id() < 1) lmem[lid + "
               "get_max_sub_group_size()] = value;\n"
               "else                       lmem[lid - "
               "get_max_sub_group_size()] = value;\n"
               "b.wait(mem_fence::local);\n\n" // sync writes to lmem for 2
                                               // subgroups (ids: 0, 1)
               "}\n"

               "__kernel void "
        + this->get_kernel_name()
        + "(global_ptr<uint[]> output, "
          ...
          "{\n"
          "    lmem[lid] = gid;\n"
          "    a.wait(mem_fence::local);\n" // sync writes to lmem for 4
                                            // subgroups (ids: 0, 1, 2, 3)
          // Now all four subgroups should see changes in lmem.
          "    size_t other_lid = (lid + (2 * get_max_sub_group_size())) % (4 "
          "* get_max_sub_group_size());\n"
          "    size_t value = 0;\n"
          "    if(lmem[other_lid] == ((gid - lid) + other_lid)){\n"
          "        value = gid;\n"
          "    }\n"
          "    a.wait(mem_fence::local);\n" // sync reads from lmem for 4
                                            // subgroups (ids: 0, 1, 2, 3)
          ...
          "    else" // ids: 2, 3
          "    {\n"
          // Work-items from the 2nd subgroup writes to local memory that will
          // be later read byt the 3rd subgroup, and the other way around - 3rd
          // subgroup writes what 2nd subgroup will later read.
          // c.wait(mem_fence::local) should provide sync between those two
          // subgroups.
          "        if(get_sub_group_id() < 3) lmem[lid + "
          "get_max_sub_group_size()] = value ;\n"
          "        else                       lmem[lid - "
          "get_max_sub_group_size()] = value;\n"
          ...
          // Now (0, 1) are in sync (local mem), and (3, 4) are in sync (local
          // mem). However, subgroups (0, 1) are not in sync with (3, 4).
          "    if(get_sub_group_id() < 4) {\n" // ids: 0, 1, 2, 3
          "        if(get_sub_group_id() < 2) output[gid + (2 * "
          "get_max_sub_group_size())] = lmem[lid];\n"
          "        else                       output[gid - (2 * "
          "get_max_sub_group_size())] = lmem[lid];\n"
          "        a.wait(mem_fence::global);\n" // sync writes to global memory
                                                 // (output)
          ...
          "    if(get_sub_group_id() < 1) temp[gid] = output[gid + (3 * "
          "get_max_sub_group_size())];\n"
          "    else                       temp[gid] = output[gid + "
          "(get_max_sub_group_size())];\n"
          "}\n"
          "else if(get_sub_group_id() < 4) {\n"
          "    if(get_sub_group_id() < 3) temp[gid] = output[gid - "
          "(get_max_sub_group_size())];\n"
          "    else                       temp[gid] = output[gid - (3 * "
          "get_max_sub_group_size())];\n"
          "}\n"

          // Synchronize the entire work-group (in terms of accesses to global
          // memory)
    ...
    size_t
    get_max_local_size(const cl_kernel kernel, const cl_device_id device,
                       const size_t work_group_size, // default work-group size
                       cl_int& error)
    {
        // Set size of the local memory, we need to to this to correctly
        // calculate max possible work-group size.
        size_t wg_size;
        for (wg_size = work_group_size; wg_size > 1; wg_size /= 2)
        {
            error = clSetKernelArg(kernel, 2,
                                   ((wg_size / 2) + 1) * sizeof(cl_uint), NULL);
...
// -----------------------------------------------------------------------------------
        ...
        sizeof(size_t), &min_num_sub_groups, sizeof(size_t) * 3, &local_size,
        NULL);
        ...
        if (local_size[0] == 0 || local_size[1] != 1 || local_size[2] != 1)
        {
            ...
            {
                RETURN_ON_ERROR_MSG(
                -1, "Can't produce local size with one subgroup")
            }
            return 0;
        }
        ...
        sizeof(size_t) * 3, local_size, sizeof(size_t),
    ...
    cl_int execute(const cl_kernel kernel, const cl_mem output_buffer,
                   const cl_command_queue queue, const size_t work_size,
                   const size_t work_group_size)
    {
        cl_int err;
        // Get context from queue
        cl_context context;
        err = clGetCommandQueueInfo(queue, CL_QUEUE_CONTEXT, sizeof(cl_context),
                                    &context, NULL);
        ...
        err |=
        clSetKernelArg(kernel, 2, work_group_size * sizeof(cl_uint), NULL);
 ...
 int num_elements)
{
      ...
      && (defined(USE_OPENCLC_KERNELS) || defined(ONLY_SPIRV_COMPILATION)))
    if (!is_extension_available(device, "cl_khr_subgroup_named_barrier"))
    {
        log_info("SKIPPED: Extension `cl_khr_subgroup_named_barrier` is not "
    ...
    RUN_WG_NAMED_BARRIER_TEST_MACRO(
    spec_example_work_group_named_barrier_test())
