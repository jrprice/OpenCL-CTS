...
template <class T> struct private_storage_test : public address_spaces_test<T>
{
    std::string str() { return "private_storage"; }
...
// -----------------------------------------------------------------------------------
        ...
        return "__kernel void " + this->get_kernel_name() + "(global "
        + type_name<T>()
        + " *output)\n"
          "{\n"
        ...
        return "#include <opencl_memory>\n"
               ...
               "using namespace cl;\n"
               "__kernel void "
        + this->get_kernel_name() + "(global_ptr<" + type_name<T>()
        + "[]> output)\n"
          "{\n"
          "    size_t gid = get_global_id(0);\n"
          "    typedef "
...
template <class T> struct local_storage_test : public address_spaces_test<T>
{
    std::string str() { return "local_storage"; }
    ...
    bool set_local_size() { return true; }

    // Every work-item writes id of its work-group to
    // output[work-item-global-id]
    std::string generate_program()
    {
// -----------------------------------------------------------------------------------
// ------------- ONLY FOR OPENCL 22 CONFORMANCE TEST 22 DEVELOPMENT
...
template <class T> struct global_storage_test : public address_spaces_test<T>
{
    ...
    std::string str() { return "global_storage"; }
    ...
    {
        return { this->get_kernel_name() + "1", this->get_kernel_name() + "2" };
    }
...
// -----------------------------------------------------------------------------------
#if defined(DEVELOPMENT) && defined(USE_OPENCLC_KERNELS)
        return "__kernel void " + this->get_kernel_names()[0] + "(global "
        + type_name<T>()
        + " *output, "
          "uint test_value)\n"
          "{\n"
        ...
        + ")(test_value);\n"
          "}\n"
          "__kernel void "
        + this->get_kernel_names()[1] + "(global " + type_name<T>()
        + " *output)\n"
          "{\n"
          "    size_t gid = get_global_id(0);\n"
        ...
        + " TYPE;\n"
          ...
          "__kernel void "
        + this->get_kernel_names()[0] + "(global_ptr<" + type_name<T>()
        + "[]> output, "
          "uint test_value)\n"
          "{\n"
          "    size_t gid = get_global_id(0);\n"
          ...
          "}\n"
          "__kernel void "
        + this->get_kernel_names()[1] + "(global_ptr<" + type_name<T>()
        + "[]> output)\n"
          "{\n"
          "    size_t gid = get_global_id(0);\n"
          "    static global<uint> func_scope_global_var { 0 };\n"
          // if (func_scope_global_var == 1) is true then
          // each work-item saves program_scope_global_array[0] to
          // output[work-item-global-id]
    ...
    cl_int execute(const std::vector<cl_kernel>& kernels, cl_mem& output_buffer,
                   cl_command_queue& queue, size_t work_size, size_t wg_size)
    {
        cl_int err;
        err =
        clSetKernelArg(kernels[0], 0, sizeof(output_buffer), &output_buffer);
        err |= clSetKernelArg(kernels[0], 1, sizeof(cl_uint), &m_test_value);
        RETURN_ON_CL_ERROR(err, "clSetKernelArg");

        // Run first kernel, once.
        // This kernel saves m_test_value to program scope global variable
        // called program_scope_global_var
        err = clEnqueueNDRangeKernel(queue, kernels[0], 1, NULL, &work_size,
                                     this->set_local_size() ? &wg_size : NULL,
                                     0, NULL, NULL);
        ...
        err =
        clSetKernelArg(kernels[1], 0, sizeof(output_buffer), &output_buffer);
        // Run 2nd kernel, twice.
        // 1st run: program_scope_global_var is saved to function scope global
        // array called func_scope_global_array 2nd run: each work-item saves
        // func_scope_global_array[0] to ouput[work-item-global-id]
        for (size_t i = 0; i < 2; i++)
        {
            err = clEnqueueNDRangeKernel(
            queue, kernels[1], 1, NULL, &work_size,
            this->set_local_size() ? &wg_size : NULL, 0, NULL, NULL);
            RETURN_ON_CL_ERROR(err, "clEnqueueNDRangeKernel");
            err = clFinish(queue);
            RETURN_ON_CL_ERROR(err, "clFinish")
        }
...
template <class T> struct constant_storage_test : public address_spaces_test<T>
{
    // m_test_value is just a random value we use in this test.
    constant_storage_test(): m_test_value(0xdeaddeadU) {}

    std::string str() { return "constant_storage"; }
...
// -----------------------------------------------------------------------------------
        ...
        + " *output)\n"
          ...
          "    size_t gid = get_global_id(0);\n"
          "    output[gid] = ("
        + type_name<T>() + ")(" + std::to_string(m_test_value)
        + ");\n"
          "}\n";
        ...
        return "#include <opencl_memory>\n"
               ...
               "#include <opencl_array>\n"
               "using namespace cl;\n"
               // Program scope constant variable, program_scope_var ==
               // (m_test_value - 1)
               "constant<uint> program_scope_const{ ("
        + std::to_string(m_test_value)
        + " - 1) };"
          "__kernel void "
        + this->get_kernel_name() + "(global_ptr<" + type_name<T>()
        + "[]> output)\n"
        ...
        + " TYPE;\n"
          ...
          // "    constant_ptr<array<uint, 1>> fs_const_ptr =
          // &func_scope_const;\n"
