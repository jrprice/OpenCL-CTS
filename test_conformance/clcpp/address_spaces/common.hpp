...
#define RUN_ADDRESS_SPACES_TEST_MACRO(TEST_CLASS)                              \
    last_error =                                                               \
    run_address_spaces_test(device, context, queue, n_elems, TEST_CLASS);      \
    CHECK_ERROR(last_error)                                                    \
    error |= last_error;

// This is a base class for address spaces tests.
template <class T> struct address_spaces_test : public detail::base_func_type<T>
{
    // output buffer type
    typedef T type;
...
int run_address_spaces_test(cl_device_id device, cl_context context,
                            cl_command_queue queue, size_t count,
                            address_spaces_test op)
{
    cl_mem buffers[1];
    cl_program program;
