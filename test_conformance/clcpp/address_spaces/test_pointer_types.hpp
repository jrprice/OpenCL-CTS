...
template <class T> struct private_pointer_test : public address_spaces_test<T>
{
    std::string str() { return "private_pointer"; }
...
// -----------------------------------------------------------------------------------
        ...
        + " TYPE;\n"
          "    TYPE v = TYPE(gid);\n"
          "    private_ptr<TYPE> "
          "v_ptr1(dynamic_asptr_cast<private_ptr<TYPE>>(&v));\n"
          "    private_ptr<TYPE> v_ptr2(v_ptr1);\n"
          "    TYPE a[] = { TYPE(0), TYPE(1) };\n"
          "    private_ptr<TYPE> a_ptr = "
          "dynamic_asptr_cast<private_ptr<TYPE>>(a);\n"
          "    a_ptr++;\n"
...
template <class T> struct local_pointer_test : public address_spaces_test<T>
{
    std::string str() { return "local_pointer"; }
                              ...
                              cl_int& error)
    {
        ...
        // calculate max possible work-group size. Additionally this already set
        // 2nd argument of the test kernel, so we don't have to modify execute()
        // method.
        ...
        size_t wg_size;
        error =
        clGetKernelWorkGroupInfo(kernels[0], device, CL_KERNEL_WORK_GROUP_SIZE,
...
// -----------------------------------------------------------------------------------
        ...
        + " TYPE;\n"
          ...
          "}\n"
          "    work_group_barrier(mem_fence::local);\n"
          // each work-item in work-group writes local_mem_ptr[0] to
          // output[work-item-global-id]
...
template <class T> struct global_pointer_test : public address_spaces_test<T>
{
    std::string str() { return "global_pointer"; }
...
template <class T> struct constant_pointer_test : public address_spaces_test<T>
{
    // m_test_value is just a random value we use in this test.
    constant_pointer_test(): m_test_value(0xdeaddeadU) {}

    std::string str() { return "constant_pointer"; }
...
// -----------------------------------------------------------------------------------
    ...
    cl_int execute(const std::vector<cl_kernel>& kernels, cl_mem& output_buffer,
                   cl_command_queue& queue, size_t work_size,
                   size_t work_group_size)
    {
        ...
        auto const_buff = clCreateBuffer(
        context, (cl_mem_flags)(CL_MEM_READ_ONLY), sizeof(cl_uint), NULL, &err);
        RETURN_ON_CL_ERROR(err, "clCreateBuffer");

        // Write m_test_value to const_buff
        err =
        clEnqueueWriteBuffer(queue, const_buff, CL_TRUE, 0, sizeof(cl_uint),
        ...
        queue, kernels[0], 1, NULL, &work_size,
        this->set_local_size() ? &work_group_size : NULL, 0, NULL, NULL);
