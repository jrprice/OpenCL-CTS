...
{
    std::string str() { return "popcount"; }
...
{
    std::string str() { return "ctz"; }

    std::string headers() { return "#include <opencl_integer>\n"; }

    OUT1 operator()(IN1 x)
    {
        if (x == 0) return sizeof(x);
...
struct int_func_rotate : public binary_func<IN1, IN2, OUT1>
{
    ...
    IN2 max2() { return sizeof(IN1) * 8; }
};

template <class IN1, class IN2, class OUT1>
struct int_func_upsample : public binary_func<IN1, IN2, OUT1>
{
    std::string str() { return "upsample"; }

    std::string headers() { return "#include <opencl_integer>\n"; }

    OUT1 operator()(IN1 hi, IN2 lo)
    {
        static_assert(sizeof(IN1) == sizeof(IN2), "sizeof(IN1) != sizeof(IN2)");
        static_assert(sizeof(OUT1) == 2 * sizeof(IN1),
                      "sizeof(OUT1) != 2 * sizeof(IN1)");
        static_assert(std::is_unsigned<IN2>::value,
