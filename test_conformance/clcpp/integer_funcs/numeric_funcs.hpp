...
{
    std::string str() { return "abs"; }

    std::string headers() { return "#include <opencl_integer>\n"; }

    OUT1 operator()(const IN1& x)
    {
        static_assert(std::is_unsigned<OUT1>::value,
                      "OUT1 type must be unsigned");
        if (x < IN1(0)) return static_cast<OUT1>(-x);
...
{
    std::string str() { return "abs_diff"; }

    std::string headers() { return "#include <opencl_integer>\n"; }

    OUT1 operator()(const IN1& x, const IN2& y)
    {
        static_assert(std::is_same<IN1, IN2>::value, "IN1 must be IN2");
        static_assert(std::is_unsigned<OUT1>::value,
                      "OUT1 type must be unsigned");
        if (x < y) return static_cast<OUT1>(y - x);
...
struct int_func_add_sat : public binary_func<IN1, IN2, OUT1>
{
    ...
    {
        static_assert(std::is_same<IN1, IN2>::value, "IN1 must be IN2");
        static_assert(std::is_same<OUT1, IN2>::value, "OUT1 must be IN2");
        // sat unsigned integers
        if (std::is_unsigned<OUT1>::value)
        {
            OUT1 z = x + y;
            if (z < x || z < y) return (std::numeric_limits<OUT1>::max)();
            return z;
        }
        // sat signed integers
        OUT1 z = x + y;
        if (y > 0)
        {
            if (z < x) return (std::numeric_limits<OUT1>::max)();
        }
        else
        {
            if (z > x) return (std::numeric_limits<OUT1>::min)();
        }
        return z;
    }
};
...
{
    std::string str() { return "clamp"; }

    std::string headers() { return "#include <opencl_integer>\n"; }

    OUT1 operator()(const IN1& x, const IN2& minval, const IN3& maxval)
    {
        static_assert(std::is_same<IN2, IN3>::value, "IN3 must be IN2");
        static_assert(std::is_same<OUT1, IN1>::value, "OUT1 must be IN1");
        return (std::min)((std::max)(x, minval), maxval);
    }

    IN2 min2() { return (std::numeric_limits<IN2>::min)(); }

    IN2 max2() { return (std::numeric_limits<IN2>::max)() / IN2(2); }

    IN3 min3() { return IN3(1) + ((std::numeric_limits<IN3>::max)() / IN3(2)); }

    IN3 max3() { return (std::numeric_limits<IN3>::max)(); }
};

// gentype clamp(gentype x, scalar minval, scalar maxval);
template <class IN1, class IN2, class IN3, class OUT1>
struct int_func_clamp<
IN1, IN2, IN3, OUT1, typename std::enable_if<is_vector_type<OUT1>::value>::type>
    : public ternary_func<IN1, IN2, IN3, OUT1>
{
    std::string str() { return "clamp"; }
    ...
    {
        static_assert(std::is_same<IN2, IN3>::value, "IN3 must be IN2");
        static_assert(!is_vector_type<IN2>::value
                      && !is_vector_type<IN3>::value,
...
{
    std::string str() { return "mul_hi"; }

    std::string headers() { return "#include <opencl_integer>\n"; }

    OUT1 operator()(const IN1& x, const IN2& y)
    {
        static_assert(std::is_same<IN1, IN2>::value
                      && std::is_same<IN2, OUT1>::value,
                      "Types must be the same");
        static_assert(!std::is_same<IN1, cl_long>::value
                      && !std::is_same<IN1, cl_ulong>::value,
...
struct int_func_mad_sat : public ternary_func<IN1, IN2, IN3, OUT1>
{
    ...
    OUT1 operator()(const IN1& x, const IN2& y, const IN3& z)
    {
        ...
        OUT1 w1 = (x * y);
        if (x != 0 && w1 / x != y) return (std::numeric_limits<OUT1>::max)();
        OUT1 w2 = w1 + z;
        if (w2 < w1) return (std::numeric_limits<OUT1>::max)();
        return w2;
    }
};
...
struct int_func_sub_sat : public binary_func<IN1, IN2, OUT1>
{
    ...
    OUT1 operator()(const IN1& x, const IN2& y)
    {
        ...
        {
            OUT1 z = x - y;
            if (x < y) return (std::numeric_limits<OUT1>::min)();
            return z;
        }
        // sat signed integers
        OUT1 z = x - y;
...
template <class IN1, class IN2, class OUT1>
struct int_func_max<IN1, IN2, OUT1,
                    typename std::enable_if<is_vector_type<OUT1>::value>::type>
    : public binary_func<IN1, IN2, OUT1>
{
    std::string str() { return "max"; }
    ...
    OUT1 operator()(const IN1& x, const IN2& y)
    {
        static_assert(std::is_same<IN1, OUT1>::value,
                      "IN1 and OUT1 must be the same types");
        static_assert(!is_vector_type<IN2>::value, "IN2 must be scalar");
        static_assert(
...
template <class IN1, class IN2, class OUT1>
struct int_func_min<IN1, IN2, OUT1,
                    typename std::enable_if<is_vector_type<OUT1>::value>::type>
    : public binary_func<IN1, IN2, OUT1>
{
    std::string str() { return "min"; }
...
(cl_device_id device, cl_context context, cl_command_queue queue, int n_elems)
{
    ...
    TEST_TERNARY_FUNC_MACRO(
    (int_func_clamp<cl_uint, cl_uint, cl_uint, cl_uint>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_clamp<cl_long, cl_long, cl_long, cl_long>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_clamp<cl_ulong, cl_ulong, cl_ulong, cl_ulong>()))

    // gentype clamp(gentype x, scalar minval, scalar maxval);
    TEST_TERNARY_FUNC_MACRO(
    (int_func_clamp<cl_int2, cl_int, cl_int, cl_int2>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_clamp<cl_uint4, cl_uint, cl_uint, cl_uint4>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_clamp<cl_long8, cl_long, cl_long, cl_long8>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_clamp<cl_ulong16, cl_ulong, cl_ulong, cl_ulong16>()))

    // gentype mad_hi(gentype a, gentype b, gentype c);
    TEST_TERNARY_FUNC_MACRO(
    (int_func_mad_hi<cl_short, cl_short, cl_short, cl_short>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_mad_hi<cl_ushort, cl_ushort, cl_ushort, cl_ushort>()))
    TEST_TERNARY_FUNC_MACRO((int_func_mad_hi<cl_int, cl_int, cl_int, cl_int>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_mad_hi<cl_uint, cl_uint, cl_uint, cl_uint>()))

    // gentype mad_sat(gentype a, gentype b, gentype c);
    TEST_TERNARY_FUNC_MACRO(
    (int_func_mad_sat<cl_ushort, cl_ushort, cl_ushort, cl_ushort>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_mad_sat<cl_uint, cl_uint, cl_uint, cl_uint>()))
    TEST_TERNARY_FUNC_MACRO(
    (int_func_mad_sat<cl_ulong, cl_ulong, cl_ulong, cl_ulong>()))
