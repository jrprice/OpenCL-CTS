...
// Verify queries clGetProgramInfo correctly return the presence of constructors
// and/or destructors in the program (using option
// CL_PROGRAM_SCOPE_GLOBAL_CTORS_PRESENT/CL_PROGRAM_SCOPE_GLOBAL_DTORS_PRESENT)
...
std::string generate_ctor_dtor_program(const bool ctor, const bool dtor)
{
    ...
    if (ctor)
    {
        program += "struct ctor_test_class {\n"
                   // non-trivial ctor
    ...
    if (dtor)
    {
        program += "struct dtor_test_class {\n"
                   // non-trivial dtor
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    // Create program
    error = create_openclcpp_program(context, &program, 1, &source_ptr,
                                     options.c_str());
...
// Normal run
    ...
    // constructor(s) that will be executed by runtime before any kernel from
...
(cl_device_id device, cl_context context, cl_command_queue queue, int count)
{
    ...
    // both present
    last_error = test_get_program_info_global_ctors_dtors_present(
    device, context, queue, true, true);
    CHECK_ERROR(last_error);
    error |= last_error;
    // dtor
    last_error = test_get_program_info_global_ctors_dtors_present(
    device, context, queue, false, true);
    CHECK_ERROR(last_error);
    error |= last_error;
    // ctor
    last_error = test_get_program_info_global_ctors_dtors_present(
    device, context, queue, true, false);
    CHECK_ERROR(last_error);
    error |= last_error;
    // none present
    last_error = test_get_program_info_global_ctors_dtors_present(
    device, context, queue, false, false);
    CHECK_ERROR(last_error);
    error |= last_error;
