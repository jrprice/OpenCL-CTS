...
// How: destructor of struct dtor_test_class has a side effect: zeroing buffer.
// If values in buffer are not zeros after releasing program, destructor was not
// executed.
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    error = create_opencl_kernel(context, &program, &kernel,
                                 program_test_dtor_is_executed,
                                 "test_dtor_is_executed");
...
// Normal run
    ...
    // host vector, size == count, output[0...count-1] == 0xbeefbeef
    // (3203383023) values in output __MUST BE__ greater than 0 for the test to
    // work correctly
...
// TEST 2
// Verify that multiple destructors, if present, are executed. Order between
// multiple destructors is undefined. Verify that each destructor is executed
// only once.

// How:
// 0) dtor_test_class struct has a global pointer to a buffer, it's set by
// test_dtors_executed_once kernel.
// 1) Destructors have a side effect: each dtor writes to its part of the
// buffer. If all dtors are executed, all values in that buffer should be
// changed. 2) The first time destructors are executed, they set their parts of
// the buffer to zero. Next time to 1, next time to 2 etc. Since dtors should be
// executed only once, all values in that buffer should be equal to zero.
...
#if defined(DEVELOPMENT) && defined(ONLY_SPIRV_COMPILATION)
    error = create_opencl_kernel(context, &program, &kernel,
                                 program_test_dtors_executed_once,
                                 "test_dtors_executed_once");
    RETURN_ON_ERROR(error)
    return error;
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
#elif defined(DEVELOPMENT) && defined(USE_OPENCLC_KERNELS)
    error = create_opencl_kernel(context, &program, &kernel,
                                 program_test_dtors_executed_once,
                                 "test_dtors_executed_once", "", false);
    RETURN_ON_ERROR(error)
// Normal run
#else
    error = create_opencl_kernel(context, &program, &kernel,
    ...
    for (auto &i : output)
    {
        ...
        {
            log_error(
            "ERROR: Test test_global_scope_dtors_executed_once failed.");
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    context, &program, &kernel, program_test_dtor_ndrange, "test_dtor_ndrange");
    RETURN_ON_ERROR(error)
    return error;
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
#elif defined(DEVELOPMENT) && defined(USE_OPENCLC_KERNELS)
    error =
    create_opencl_kernel(context, &program, &kernel, program_test_dtor_ndrange,
                         "test_dtor_ndrange", "", false);
    RETURN_ON_ERROR(error)
// Normal run
#else
    error = create_opencl_kernel(
