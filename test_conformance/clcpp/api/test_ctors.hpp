...
// Verify that when present, multiple constructors are executed. The order
// between constructors is undefined, but they should all execute.
...
int test_ctors_execution(cl_device_id device, cl_context context,
                         cl_command_queue queue, int count,
                         std::string kernel_name, const char *kernel_source)
{
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    error = create_opencl_kernel(context, &program, &kernel, kernel_source,
                                 kernel_name);
...
// Normal run
    ...
    queue, output_buffer, CL_TRUE, 0, sizeof(cl_uint) * output.size(),
...
(cl_device_id device, cl_context context, cl_command_queue queue, int count)
{
    ...
    local_error =
    test_ctors_execution(device, context, queue, count, "test_ctors_executed",
                         kernel_test_ctors_executed);
    CHECK_ERROR(local_error);
    error |= local_error;

    local_error = test_ctors_execution(
    device, context, queue, count, "test_ctors_executed_multiple_ctors",
    kernel_test_ctors_executed_multiple_ctors);
    CHECK_ERROR(local_error);
    error |= local_error;
...
// TEST 2
// Verify that constructors are only executed once when multiple kernels from a
// program are executed.

// How: The first kernel (test_ctors_executed_once_set) is run once. It changes
// values of program scope variables, then the second kernel is run multiple
// times, each time verifying that global variables have correct values (the
// second kernel should observe the values assigned by the first kernel, not by
// the constructors).
...
// Only OpenCL C++ to SPIR-V compilation
#if defined(DEVELOPMENT) && defined(ONLY_SPIRV_COMPILATION)
    error = create_opencl_kernel(context, &program, &kernel_set_global_vars,
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    error = create_opencl_kernel(context, &program, &kernel_set_global_vars,
                                 program_test_ctors_executed_once,
                                 "test_ctors_executed_once_set", "", false);
    RETURN_ON_ERROR(error)
    // Get the second kernel
    kernel_read_global_vars =
    clCreateKernel(program, "test_ctors_executed_once_read", &error);
...
// Normal run
    ...
    work_size[0] = count;
    error = clEnqueueNDRangeKernel(queue, kernel_set_global_vars, dim, NULL,
    ...
    for (size_t i = 0; i < 4; i++)
    {
        ...
        error = clSetKernelArg(kernel_read_global_vars, 0,
                               sizeof(output_buffer), &output_buffer);
        RETURN_ON_CL_ERROR(error, "clSetKernelArg")

        work_size[0] = output.size();
        error = clEnqueueNDRangeKernel(queue, kernel_read_global_vars, dim,
...
#if defined(DEVELOPMENT) && defined(ONLY_SPIRV_COMPILATION)
    error =
    create_opencl_kernel(context, &program, &kernel, program_test_ctors_ndrange,
                         "test_ctors_ndrange");
    RETURN_ON_ERROR(error)
    return error;
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
