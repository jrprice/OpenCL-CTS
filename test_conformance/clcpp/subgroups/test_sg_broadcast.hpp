...
std::string generate_sg_broadcast_kernel_code()
{
    return "#include <opencl_memory>\n"
           ...
           "using namespace cl;\n"
           "__kernel void test_sg_broadcast(global_ptr<uint[]> input, "
...
int verify_sg_broadcast(const std::vector<cl_uint> &in,
                        const std::vector<cl_uint> &out, size_t count,
...
int sub_group_broadcast(cl_device_id device, cl_context context,
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    generate_input_sg_broadcast(flat_work_size, wg_size);
    std::vector<cl_uint> output =
    generate_output_sg_broadcast(flat_work_size, wg_size);
    ...
    int result =
    verify_sg_broadcast(input, output, work_size[0], wg_size, sg_max_size);
