...
std::string generate_sg_all_kernel_code()
{
    return "#include <opencl_memory>\n"
           ...
           "using namespace cl;\n"
           "__kernel void test_sg_all(global_ptr<uint[]> input, "
...
int verify_sg_all(const std::vector<cl_uint> &in,
    ...
    for (i = 0; i < count; i += wg_size)
    {
             ...
             j += sg_size)
        {
                 ...
                 k++)
            {
                ...
                {
                    log_info("sub_group_all %s: Error at %lu: expected = %lu, "
                             "got = %lu\n",
...
std::vector<cl_uint> generate_input_sg_all(size_t count, size_t wg_size)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        // generate false, it means that for sub_group_all(input[tid] <
        // input[tid+1]) should return false for all sub-groups in that
        // work-groups
...
int sub_group_all(cl_device_id device, cl_context context,
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    err =
    create_opencl_kernel(context, &program, &kernel, code_str, "test_sg_all");
    RETURN_ON_ERROR(err)
    return err;
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    generate_input_sg_all(flat_work_size + 1, wg_size);
    std::vector<cl_uint> output =
    generate_output_sg_all(flat_work_size, wg_size);
    ...
    if (verify_sg_all(input, output, flat_work_size, wg_size, sg_max_size)
        != CL_SUCCESS)
    {
