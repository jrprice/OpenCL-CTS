...
std::string generate_sg_any_kernel_code()
{
    return "#include <opencl_memory>\n"
           ...
           "using namespace cl;\n"
           "__kernel void test_sg_any(global_ptr<uint[]> input, "
...
int verify_sg_any(const std::vector<cl_uint> &in,
                  const std::vector<cl_uint> &out, size_t count, size_t wg_size,
                  size_t sg_size)
{
    size_t i, j, k;
    for (i = 0; i < count; i += wg_size)
    {
        for (j = 0; j < ((count - i) > wg_size ? wg_size : (count - i));
             j += sg_size)
        {
            // sub-group any
            bool any = false;
                 ...
                 k++)
            {
                ...
                {
                    log_info("sub_group_any %s: Error at %lu: expected = %lu, "
                             "got = %lu\n",
...
std::vector<cl_uint> generate_input_sg_any(size_t count, size_t wg_size)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        // In one place in ~half of work-groups (input[tid] == input[tid+1])
        // will generate true, it means that for sub_group_all(input[tid] ==
        // input[tid+1]) should return false for one sub-group in that
        // work-groups
...
int sub_group_any(cl_device_id device, cl_context context,
...
// Only OpenCL C++ to SPIR-V compilation
    ...
    err =
    create_opencl_kernel(context, &program, &kernel, code_str, "test_sg_any");
    RETURN_ON_ERROR(err)
    return err;
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
    ...
    generate_input_sg_any(flat_work_size + 1, wg_size);
    std::vector<cl_uint> output =
    generate_output_sg_any(flat_work_size, wg_size);
    ...
    if (verify_sg_any(input, output, flat_work_size, wg_size, sg_max_size)
        != CL_SUCCESS)
    {
