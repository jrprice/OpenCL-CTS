...
{
    // Check if cl_float (float) and cl_double (double) fulfill the requirements
    // of IEC 559 (IEEE 754) standard. This is required for the tests to run
    // correctly.
    if (!std::numeric_limits<cl_float>::is_iec559)
    {
        RETURN_ON_ERROR_MSG(-1,
                            "cl_float (float) does not fulfill the "
    ...
    {
        RETURN_ON_ERROR_MSG(-1,
                            "cl_double (double) does not fulfill the "
                            "requirements of IEC 559 (IEEE 754) standard. "
                            "Tests won't run correctly.");
    }
