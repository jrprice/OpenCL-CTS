...
struct geometric_func_fast_distance
    : public binary_func<cl_float4, cl_float4, cl_float>
{

    std::string str() { return "fast_distance"; }
    ...
    cl_float4 min1() { return detail::def_limit<cl_float4>(-512.0f); }

    cl_float4 max1() { return detail::def_limit<cl_float4>(512.0f); }

    cl_float4 min2() { return detail::def_limit<cl_float4>(-512.0f); }

    cl_float4 max2() { return detail::def_limit<cl_float4>(512.0f); }

    cl_double delta(const cl_float4& p0, const cl_float4& p1,
                    const cl_float& expected)
    {
        (void)p0;
        (void)p1;
...
{
    std::string str() { return "fast_length"; }
...
{
    std::string str() { return "fast_normalize"; }
    ...
    float ulp()
    {
        return 8192.5f + // error in rsqrt + error in multiply
        (0.5f * 4.0f) + // cumulative error for multiplications
        (0.5f * 3.0f); // cumulative error for additions
    }
};
