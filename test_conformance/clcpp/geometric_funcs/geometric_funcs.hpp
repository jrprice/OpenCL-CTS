...
struct geometric_func_cross
    : public binary_func<cl_float4, cl_float4, cl_float4>
{
    ...
    std::string str() { return "cross"; }

    std::string headers() { return "#include <opencl_geometric>\n"; }
    ...
    cl_float4 max1() { return detail::def_limit<cl_float4>(1000.0f); }

    cl_float4 max2() { return detail::def_limit<cl_float4>(1000.0f); }

    cl_float4 min1() { return detail::def_limit<cl_float4>(-1000.0f); }

    cl_float4 min2() { return detail::def_limit<cl_float4>(-1000.0f); }

    bool use_ulp() { return false; }

    cl_double4 delta(const cl_float4& p0, const cl_float4& p1,
                     const cl_float4& expected)
    {
        (void)p0;
        (void)p1;
...
struct geometric_func_distance
    : public binary_func<cl_float4, cl_float4, cl_float>
{

    std::string str() { return "distance"; }
    ...
    cl_float operator()(const cl_float4& p0, const cl_float4& p1)
    {
        ...
        {
            t =
            static_cast<cl_double>(p0.s[i]) - static_cast<cl_double>(p1.s[i]);
            r += t * t;
        }
        return std::sqrt(r);
    }
...
struct geometric_func_length : public unary_func<cl_float4, cl_float>
{
    ...
    cl_float operator()(const cl_float4& p)
    {
        ...
        {
            r +=
            static_cast<cl_double>(p.s[i]) * static_cast<cl_double>(p.s[i]);
        }
        return std::sqrt(r);
    }
...
{
    std::string str() { return "normalize"; }
    ...
    cl_float4 operator()(const cl_float4& p)
    {
        ...
        // normalize( v ) for which any element in v is infinite shall proceed
        // as if the elements in v were replaced as follows: for( i = 0; i <
        // sizeof(v) / sizeof(v[0] ); i++ )
        ...
        for (size_t i = 0; i < 4; i++)
        {
            if ((std::isinf)(p.s[i]))
            {
                for (size_t j = 0; j < 4; j++)
                {
                    r.s[j] = (std::isinf)(p.s[j]) ? (std::copysign)(1.0, p.s[j])
                                                  : 0.0 * p.s[j];
                }
                r = (*this)(r);
                return r;
    ...
    float ulp()
    {
        return 2.5f + // error in rsqrt + error in multiply
        (0.5f * 4.0f) + // cumulative error for multiplications
        (0.5f * 3.0f); // cumulative error for additions
    }
};
