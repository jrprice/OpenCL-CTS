...
struct spec_const_required_work_group_size_test
    : public spec_constants_test<cl_uint>
{
    ...
    std::string str()
    {
        if (m_set_spec_constant)
            return "spec_const_in_required_work_group_size_"
            + std::to_string(m_work_group_size_0);
        else
    ...
    // Check if query for CL_KERNEL_COMPILE_WORK_GROUP_SIZE using
    // clGetKernelWorkGroupInfo return correct values. It should return the
    // work-group size specified by the cl::required_work_group_size(X, Y, Z)
    // qualifier.
                         ...
                         cl_command_queue queue)
    {
        ...
        if (m_set_spec_constant)
        {
            if (compile_wg_size[0] != m_work_group_size_0
                || compile_wg_size[1] != 1 || compile_wg_size[2] != 1)
            {
                error = -1;
            }
        }
        ...
        {
            if (compile_wg_size[0] != 1 || compile_wg_size[1] != 1
                || compile_wg_size[2] != 1)
            {
                error = -1;
            }
    ...
    cl_int set_spec_constants(const cl_program& program)
    {
        ...
        if (m_set_spec_constant)
        {
            ...
            program, cl_uint(1), sizeof(cl_uint),
            static_cast<void*>(&m_work_group_size_0));
...
// -----------------------------------------------------------------------------------
        ...
        if (with_attribute)
        {
            ...
            att = "\n__attribute__((reqd_work_group_size(" + work_group_size_0
            + ",1,1)))\n";
        }
        return "__kernel" + att + "void " + this->get_kernel_name()
        + "(global uint *output)\n"
          "{\n"
        ...
        + att + "__kernel void " + this->get_kernel_name()
    ...
    // Each work-item writes get_local_size(0) to output[work-item-global-id]
    std::string generate_program() { return generate_program(true); }
...
// Use OpenCL C kernels instead of OpenCL C++ kernels (test C++ host code)
#if defined(DEVELOPMENT) && defined(USE_OPENCLC_KERNELS)
    err = create_opencl_kernel(context, &program, &(kernels[0]), source,
                               kernel_names[0], "-cl-std=CL2.0", false);
    RETURN_ON_ERROR(err)
    ...
    {
        size_t max_wg_size;
        err = clGetKernelWorkGroupInfo(k, device, CL_KERNEL_WORK_GROUP_SIZE,
...
// ------------------
// -----------------------------------------------------------------------------------
// If ONLY_SPIRV_COMPILATION is defined we can't check the max work-group size
// for the kernel because OpenCL kernel object is never created in that mode.
#if defined(DEVELOPMENT) && defined(ONLY_SPIRV_COMPILATION)
    const size_t max_wg_size = 16;
#else
    // Get max work-group size that can be used in
    // [[cl::required_work_group_size(X, 1, 1)]] We do this by building kernel
    // without this attribute and checking what is the max work-group size we
    // can use with it.
    auto test = spec_const_required_work_group_size_test(true, 1);
    const size_t max_wg_size =
    get_max_wg_size(test.generate_program(false), test.get_kernel_names(),
                    1024, // max wg size we want to test
                    device, context, queue, error);
    ...
    for (size_t i = 1; i <= max_wg_size; i *= 2)
    {
        ...
        spec_const_required_work_group_size_test(true, i));
    }
    // This test does not set spec constant
    RUN_SPEC_CONSTANTS_TEST_MACRO(spec_const_required_work_group_size_test(
