...
// This function reads values of FP_ILOGB0 and FP_ILOGBNAN macros defined on the
// device. OpenCL C++ Spec: The value of FP_ILOGB0 shall be either {INT_MIN} or
// {INT_MAX}. The value of FP_ILOGBNAN shall be either {INT_MAX} or {INT_MIN}.
int get_ilogb_nan_zero(cl_device_id device, cl_context context,
                       cl_command_queue queue, cl_int& ilogb_nan,
                       cl_int& ilogb_zero)
{
    cl_mem buffers[1];
    cl_program program;
    ...
    err = clEnqueueReadBuffer(queue, buffers[0], CL_TRUE, 0,
                              sizeof(cl_int) * output.size(),
...
// max_delta, min1, max1
MATH_FUNCS_DEFINE_UNARY_FUNC(logarithmic, log, std::log, true, 3.0f, 4.0f,
                             0.001f, -10.0f, 1000.0f)
MATH_FUNCS_DEFINE_UNARY_FUNC(logarithmic, logb, std::logb, true, 0.0f, 0.0f,
                             0.001f, -10.0f, 1000.0f)
MATH_FUNCS_DEFINE_UNARY_FUNC(logarithmic, log2, std::log2, true, 3.0f, 4.0f,
                             0.001f, -10.0f, 1000.0f)
MATH_FUNCS_DEFINE_UNARY_FUNC(logarithmic, log10, std::log10, true, 3.0f, 4.0f,
                             0.001f, -10.0f, 1000.0f)
MATH_FUNCS_DEFINE_UNARY_FUNC(logarithmic, log1p, std::log1p, true, 2.0f, 4.0f,
                             0.001f, -10.0f, 1000.0f)

// gentype lgamma(gentype x);
// OpenCL C++ Spec.:
// The ULP values for built-in math functions lgamma and lgamma_r is currently
// undefined. Because of that we don't check ULP and set acceptable delta to
// 0.2f (20%).
MATH_FUNCS_DEFINE_UNARY_FUNC(logarithmic, lgamma, std::lgamma, false, 0.0f,
                             0.0f, 0.2f, -10.0f, 1000.0f)
...
MATH_FUNCS_DEFINE_UNARY_FUNC(logarithmic, lgamma_r, std::lgamma, false, 0.0f,
                             0.0f, 0.2f, -10.0f, 1000.0f)

// We need to specialize generate_kernel_unary<>() function template for
// logarithmic_func_lgamma_r because it takes two arguments, but only one of it
// is input, the 2nd one is used to return the sign of the gamma function.
// -----------------------------------------------------------------------------------
...
template <>
std::string
generate_kernel_unary<logarithmic_func_lgamma_r, cl_float, cl_float>(
logarithmic_func_lgamma_r func)
{
    return "__kernel void test_lgamma_r(global float *input, global float "
           "*output)\n"
           "{\n"
...
logarithmic_func_lgamma_r func)
{
    ...
    + "#include <opencl_memory>\n"
      ...
      "using namespace cl;\n"
      "__kernel void test_lgamma_r(global_ptr<float[]> input, "
      "global_ptr<float[]> output)\n"
      "{\n"
      "    size_t gid = get_global_id(0);\n"
      "    int sign;\n"
      "    output[gid] = lgamma_r(input[gid], &sign);\n"
...
(cl_device_id device, cl_context context, cl_command_queue queue, int n_elems)
{
    ...
    // Write values of FP_ILOGB0 and FP_ILOGBNAN, which are macros defined on
    // the device, to ilogb_zero and ilogb_nan.
    cl_int ilogb_nan = 0;
    cl_int ilogb_zero = 0;
    error =
    detail::get_ilogb_nan_zero(device, context, queue, ilogb_nan, ilogb_zero);
    ...
    //
    // Note:
    // We DO NOT test if sign of the gamma function return by lgamma_r is
    // correct
