...
cl_double2 fract(cl_double x)
{
    ...
    if (r.s[0] < 0.0)
    {
        r.s[0] = 1.0f + r.s[0];
        r.s[1] -= 1.0f;
        if (r.s[0] == 1.0f) r.s[0] = HEX_FLT(+, 1, fffffe, -, 1);
    }
    return r;
}
...
// are not already defined in STL.
cl_double divide(cl_double x, cl_double y) { return x / y; }

cl_double recip(cl_double x) { return 1.0 / x; }

// Reference functions for OpenCL other functions that
// are not already defined in STL.
cl_double mad(cl_double x, cl_double y, cl_double z) { return (x * y) + z; }
...
cl_double powr(const cl_double& x, const cl_double& y)
{
    ...
    if (x < 0.0) return std::numeric_limits<cl_double>::quiet_NaN();
    ...
    if (x == 0.0)
    {
        ...
        if (y < 0.0) return std::numeric_limits<cl_double>::infinity();
    ...
    if ((std::isinf)(fabsy))
    {
        if (y < 0.0)
        {
            if (fabsx < 1.0) return std::numeric_limits<cl_double>::infinity();
            return 0;
        }
        if (fabsx < 1.0) return 0.0;
        return std::numeric_limits<cl_double>::infinity();
    }
    return (std::pow)(x, y);
}
...
cl_double rootn(const cl_double& x, const cl_int n)
{
    ...
    if (n == 0) return std::numeric_limits<cl_double>::quiet_NaN();
...
// are not already defined in STL.
cl_double acospi(cl_double x) { return (std::acos)(x) / CL_M_PI; }

cl_double asinpi(cl_double x) { return (std::asin)(x) / CL_M_PI; }

cl_double atanpi(cl_double x) { return (std::atan)(x) / CL_M_PI; }

cl_double cospi(cl_double x) { return (std::cos)(x * CL_M_PI); }

cl_double sinpi(cl_double x) { return (std::sin)(x * CL_M_PI); }

cl_double tanpi(cl_double x) { return (std::tan)(x * CL_M_PI); }
