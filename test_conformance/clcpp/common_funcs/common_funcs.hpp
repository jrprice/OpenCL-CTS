...
struct common_func_clamp : public ternary_func<IN1, IN2, IN3, OUT1>
{
    ...
    std::string headers() { return "#include <opencl_common>\n"; }
    ...
    IN2 max2() { return (std::numeric_limits<IN2>::max)() / IN2(4000.0f); }
    ...
    IN3 max3() { return (std::numeric_limits<IN3>::max)() / IN3(2000.0f); }
...
struct common_func_mix : public ternary_func<IN1, IN2, IN3, OUT1>
{
    ...
    IN3 min3() { return IN3(0.0f + CL_FLT_EPSILON); }

    IN3 max3() { return IN3(1.0f - CL_FLT_EPSILON); }

    bool use_ulp() { return false; }
};
...
{
    std::string str() { return "smoothstep"; }
    ...
    // edge0 must be < edge1
    IN1 min1() { return (std::numeric_limits<IN1>::min)(); }

    IN1 max1() { return (std::numeric_limits<IN1>::max)() / IN1(8000.0f); }
    ...
    IN2 max2() { return (std::numeric_limits<IN2>::max)() / IN2(2000.0f); }
...
struct common_func_sign : public unary_func<IN1, OUT1>
{
    ...
    std::vector<IN1> in_special_cases() { return { -0.0f, +0.0f }; }
};
...
(cl_device_id device, cl_context context, cl_command_queue queue, int n_elems)
{
    ...
    (common_func_clamp<cl_float, cl_float, cl_float, cl_float>()))

    // floatn degrees(floatn t)
    TEST_UNARY_FUNC_MACRO(
    (common_func_degrees<cl_float, cl_float, cl_double>()))
    ...
    // floatn mix(floatn x, floatn y, floatn a);
    TEST_TERNARY_FUNC_MACRO(
    (common_func_mix<cl_float, cl_float, cl_float, cl_float>()))

    // floatn radians(floatn t)
    TEST_UNARY_FUNC_MACRO(
    (common_func_radians<cl_float, cl_float, cl_double>()))
    ...
    // floatn smoothstep(floatn edge0, floatn edge1, floatn x)
    TEST_TERNARY_FUNC_MACRO(
    (common_func_smoothstep<cl_float, cl_float, cl_float, cl_float>()))
