...
typedef int (*OpVerifyFn)(void *actualPtr, void *inputPtr, size_t vecSize,
                          size_t numVecs, cl_char *controls);

int test_unary_op(cl_command_queue queue, cl_context context,
                  OpKonstants whichOp, ExplicitType vecType, size_t vecSize,
                  MTdata d, OpVerifyFn verifyFn)
{
    ...
    char sizeNames[][4] = { "", "", "2", "3", "4", "", "", "",  "8",
                            "", "", "",  "",  "",  "", "", "16" };
    sprintf(kernelSource, testKernel,
            get_explicit_type_name(vecType), /*sizeNames[ vecSize ],*/
            get_explicit_type_name(vecType), sizeNames[vecSize], loadLine,
            storeLine);
    ...
    // Generate two streams. The first is our random data to test against, the
    // second is our control stream
    ...
    {
        size_t which = i & 7;
        if (which == 0) bits = genrand_int32(d);
    ...
    // Read the results
    error =
    clEnqueueReadBuffer(queue, streams[0], CL_TRUE, 0,
...
template <typename T>
int VerifyFn(void *actualPtr, void *inputPtr, size_t vecSize, size_t numVecs,
             cl_char *controls)
{
    ...
    for (size_t i = 0; i < numVecs; i++)
    {
        for (size_t j = 0; j < vecSize; j++, index++)
        {
            ...
            {
                log_error("ERROR: Validation failed on vector %ld:%ld "
...
int test_unary_op_set(cl_command_queue queue, cl_context context,
                      OpKonstants whichOp)
{
    ExplicitType types[] = { kChar, kUChar, kShort, kUShort,          kInt,
                             kUInt, kLong,  kULong, kNumExplicitTypes };
    OpVerifyFn verifys[] = {
        VerifyFn<cl_char>,   VerifyFn<cl_uchar>, VerifyFn<cl_short>,
        VerifyFn<cl_ushort>, VerifyFn<cl_int>,   VerifyFn<cl_uint>,
        VerifyFn<cl_long>,   VerifyFn<cl_ulong>, NULL
    };
    ...
    for (typeIndex = 0; types[typeIndex] != kNumExplicitTypes; typeIndex++)
    {
        ...
        {
            if (test_unary_op(queue, context, whichOp, types[typeIndex],
                              vecSizes[index], seed, verifys[typeIndex])
                != 0)
            {
...
int test_unary_ops_full(cl_device_id deviceID, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    return test_unary_op_set(queue, context, kBoth);
}

int test_unary_ops_increment(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    return test_unary_op_set(queue, context, kIncrement);
}

int test_unary_ops_decrement(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
