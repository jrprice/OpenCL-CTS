...
"__kernel void test_int_mul24(__global int *srcA, __global int *srcB, __global "
"int *dst)\n"
"{\n"
...
int verify_int_mul24(int *inptrA, int *inptrB, int *outptr, size_t n,
                     size_t vecSize)
{
    int r;
    size_t i;
...
int verify_uint_mul24(cl_uint *inptrA, cl_uint *inptrB, cl_uint *outptr,
                      size_t n, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            log_error(
            "failed at %ld: 0x%8.8x * 0x%8.8x = *0x%8.8x vs 0x%8.8x\n", i,
...
int test_integer_mul24(cl_device_id device, cl_context context,
                       cl_command_queue queue, int n_elems)
{
    ...
    for (i = 0; i < NUM_PROGRAMS; i++)
    {
        ...
        err = verify_int_mul24(input_ptr[0], input_ptr[1], output_ptr,
                               vector_sizes[i], vector_sizes[i]);
        if (err)
        {
