...
"__kernel void test_int_mad24(__global int *srcA, __global int *srcB, __global "
"int *srcC, __global int *dst)\n"
"{\n"
...
"{\n"
"    int  tid = get_global_id(0);\n"
"    int3 tmp = mad24(vload3(tid, srcA), vload3(tid, srcB), vload3(tid, "
"srcC));\n"
"    vstore3(tmp, tid, dst);\n"
"}\n";
...
"{\n"
"    int  tid = get_global_id(0);\n"
"    uint3 tmp = mad24(vload3(tid, srcA), vload3(tid, srcB), vload3(tid, "
"srcC));\n"
"    vstore3(tmp, tid, dst);\n"
"}\n";
...
int verify_int_mad24(int *inptrA, int *inptrB, int *inptrC, int *outptr,
                     size_t n, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_error("Failed at %ld)  0x%8.8x * 0x%8.8x + 0x%8.8x = *0x%8.8x "
                      "vs 0x%8.8x\n",
...
int verify_uint_mad24(cl_uint *inptrA, cl_uint *inptrB, cl_uint *inptrC,
                      cl_uint *outptr, size_t n, size_t vecSize)
{
    cl_uint r;
    size_t i;
...
static const char *test_str_names[] = { "int",   "int2",  "int3",  "int4",
...
static inline int random_int32(MTdata d) { return genrand_int32(d); }


int test_integer_mad24(cl_device_id device, cl_context context,
                       cl_command_queue queue, int n_elems)
{
    ...
    p = input_ptr[0];
    for (i = 0; i < num_elements; i++) p[i] = random_int24(d);
    p = input_ptr[1];
    for (i = 0; i < num_elements; i++) p[i] = random_int24(d);
    p = input_ptr[2];
    for (i = 0; i < num_elements; i++) p[i] = random_int32(d);
    free_mtdata(d);
    d = NULL;
    ...
    err = create_single_kernel_helper(context, &program[0], &kernel[0], 1,
                                      &int_mad24_kernel_code, "test_int_mad24");
    if (err) return -1;
    err =
    create_single_kernel_helper(context, &program[1], &kernel[1], 1,
                                &int2_mad24_kernel_code, "test_int2_mad24");
    if (err) return -1;
    err =
    create_single_kernel_helper(context, &program[2], &kernel[2], 1,
                                &int3_mad24_kernel_code, "test_int3_mad24");
    if (err) return -1;
    err =
    create_single_kernel_helper(context, &program[3], &kernel[3], 1,
                                &int4_mad24_kernel_code, "test_int4_mad24");
    if (err) return -1;
    err =
    create_single_kernel_helper(context, &program[4], &kernel[4], 1,
                                &int8_mad24_kernel_code, "test_int8_mad24");
    if (err) return -1;
    err =
    create_single_kernel_helper(context, &program[5], &kernel[5], 1,
                                &int16_mad24_kernel_code, "test_int16_mad24");
    if (err) return -1;

    err = create_single_kernel_helper(
    context, &program[NUM_PROGRAMS], &kernel[NUM_PROGRAMS], 1,
    &uint_mad24_kernel_code, "test_uint_mad24");
    if (err) return -1;
    err = create_single_kernel_helper(
    context, &program[NUM_PROGRAMS + 1], &kernel[NUM_PROGRAMS + 1], 1,
    &uint2_mad24_kernel_code, "test_uint2_mad24");
    if (err) return -1;
    err = create_single_kernel_helper(
    context, &program[NUM_PROGRAMS + 2], &kernel[NUM_PROGRAMS + 2], 1,
    &uint3_mad24_kernel_code, "test_uint3_mad24");
    if (err) return -1;
    err = create_single_kernel_helper(
    context, &program[NUM_PROGRAMS + 3], &kernel[NUM_PROGRAMS + 3], 1,
    &uint4_mad24_kernel_code, "test_uint4_mad24");
    if (err) return -1;
    err = create_single_kernel_helper(
    context, &program[NUM_PROGRAMS + 4], &kernel[NUM_PROGRAMS + 4], 1,
    &uint8_mad24_kernel_code, "test_uint8_mad24");
    if (err) return -1;
    err = create_single_kernel_helper(
    context, &program[NUM_PROGRAMS + 5], &kernel[NUM_PROGRAMS + 5], 1,
    &uint16_mad24_kernel_code, "test_uint16_mad24");
    if (err) return -1;
    ...
    for (i = 0; i < NUM_PROGRAMS; i++)
    {
        ...
        if (verify_int_mad24(input_ptr[0], input_ptr[1], input_ptr[2],
                             output_ptr, n_elems * vector_sizes[i],
                             vector_sizes[i]))
        {
    ...
    p = input_ptr[0];
    for (i = 0; i < num_elements; i++) p[i] &= 0xffffffU;
    p = input_ptr[1];
    for (i = 0; i < num_elements; i++) p[i] &= 0xffffffU;
    ...
    for (i = NUM_PROGRAMS; i < 2 * NUM_PROGRAMS; i++)
    {
        ...
        if (verify_uint_mad24((cl_uint *)input_ptr[0], (cl_uint *)input_ptr[1],
                              (cl_uint *)input_ptr[2], (cl_uint *)output_ptr,
                              n_elems * vector_sizes[i - NUM_PROGRAMS],
