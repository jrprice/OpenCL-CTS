...
static int verify_abs_char(const void *p, const void *q, size_t n,
                           const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            log_info(
            "%ld) Failure for abs( (char%s) 0x%2.2x) = *0x%2.2x vs 0x%2.2x\n",
...
static int verify_abs_short(const void *p, const void *q, size_t n,
                            const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            log_info(
            "%ld) Failure for abs( (short%s) 0x%4.4x) = *0x%4.4x vs 0x%4.4x\n",
            i, sizeName, inA[i], r, outptr[i]);
            return -1;
        }
    }
...
static int verify_abs_int(const void *p, const void *q, size_t n,
                          const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            log_info(
            "%ld) Failure for abs( (int%s) 0x%2.2x) = *0x%8.8x vs 0x%8.8x\n", i,
            sizeName, inA[i], r, outptr[i]);
            return -1;
        }
    }
    return 0;
...
static int verify_abs_long(const void *p, const void *q, size_t n,
                           const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_info("%ld) Failure for abs( (long%s) 0x%16.16llx) = "
...
static int verify_abs_uchar(const void *p, const void *q, size_t n,
                            const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            log_info(
            "%ld) Failure for abs( (uchar%s) 0x%2.2x) = *0x%2.2x vs 0x%2.2x\n",
            i, sizeName, inA[i], r, outptr[i]);
            return -1;
        }
    }
...
static int verify_abs_ushort(const void *p, const void *q, size_t n,
                             const char *sizeName, size_t vecSize)
{
...
static int verify_abs_uint(const void *p, const void *q, size_t n,
                           const char *sizeName, size_t vecSize)
{
...
static int verify_abs_ulong(const void *p, const void *q, size_t n,
                            const char *sizeName, size_t vecSize)
{
...
typedef int (*verifyFunc)(const void *, const void *, size_t n,
                          const char *sizeName, size_t vecSize);
static const verifyFunc verify[] = { verify_abs_char,  verify_abs_short,
                                     verify_abs_int,   verify_abs_long,
                                     verify_abs_uchar, verify_abs_ushort,
                                     verify_abs_uint,  verify_abs_ulong };
...
static const char *vector_size_names_io_types[] = {
    "", "2", "", "4", "8", "16"
};
static const size_t kSizes[9] = { 1, 2, 4, 8, 1, 2, 4, 8 };

static const char *source_loads[] = { "srcA[tid]", "vload3(tid, srcA)" };

static const char *dest_stores[] = { "    dst[tid] = tmp;\n",
                                     "    vstore3(tmp, tid, dst);\n" };

int test_integer_abs(cl_device_id device, cl_context context,
                     cl_command_queue queue, int n_elems)
{
    ...
    for (i = 0; i < n_elems * 4; i++) p[i] = genrand_int32(d);
    free_mtdata(d);
    d = NULL;
         ...
         type++)
    {
        ...
        for (vectorSizeIdx = 0; vectorSizeIdx
             ...
             vectorSizeIdx++)
        {
            ...
            const char *source[] = {
                ...
                vector_size_names[vectorSizeIdx],
                " tmp = abs(",
                source_loads[!!(vector_sizes[vectorSizeIdx] == 3)],
                ");\n",
            ...
            char kernelName[128];
            snprintf(kernelName, sizeof(kernelName), "test_abs_%s%s",
                     test_str_names[type], vector_size_names[vectorSizeIdx]);
            ...
            {
                if (f(inP, outP, (vector_sizes[vectorSizeIdx]),
                      vector_size_names[vectorSizeIdx],
                      vector_sizes[vectorSizeIdx]))
                {
                    ++fail_count;
                    break; // return -1;
                }
