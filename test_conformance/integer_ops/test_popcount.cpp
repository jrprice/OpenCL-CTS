...
#define __verify_popcount_func(__T)                                            \
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            {                                                                  \
                log_info(                                                      \
                "%ld) Failure for popcount( (%s%s) 0x%x ) = *%d vs %d\n", i,   \
                str(__T), sizeName, x, (int)ref, (int)res);                    \
                return -1;                                                     \
            }                                                                  \
        }                                                                      \
        return 0;                                                              \
...
static const verifyFunc verify[] = {
    verify_popcount_cl_char,  verify_popcount_cl_uchar,
...
int test_popcount(cl_device_id device, cl_context context,
                  cl_command_queue queue, int n_elems)
{
    ...
    p = input_ptr[0];
    for (i = 0; i < 8 * n_elems; i++) p[i] = genrand_int32(d);
    free_mtdata(d);
    d = NULL;
         ...
         type++)
    {
             ...
             vectorSize++)
        {
            ...
            const char *source[] = { "__kernel void test_popcount_",
                                     test_str_names[type],
            ...
            char kernelName[128];
            snprintf(kernelName, sizeof(kernelName), "test_popcount_%s%s",
            ...
            {
                if (f(inP, outP, (vector_sizes[vectorSize]),
