...
typedef bool (*singleParamIntegerVerifyFn)(void *source, void *destination,
                                           ExplicitType vecType);
static void patchup_divide_results(void *outData, const void *inDataA,
                                   const void *inDataB, size_t count,
                                   ExplicitType vecType);
bool verify_integer_divideAssign(void *source, void *destination,
                                 ExplicitType vecType);
bool verify_integer_moduloAssign(void *source, void *destination,
                                 ExplicitType vecType);

int test_single_param_integer_kernel(cl_command_queue queue, cl_context context,
                                     const char *fnName, ExplicitType vecType,
                                     size_t vecSize,
                                     singleParamIntegerVerifyFn verifyFn,
                                     MTdata d, bool useOpKernel = false)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    clMemWrapper streams[2];
    cl_long inDataA[TEST_SIZE * 16], outData[TEST_SIZE * 16],
    inDataB[TEST_SIZE * 16], expected;
    int error, i;
    ...
    if (vecSize == 1)
        sprintf(kernelSource, singleParamSingleSizeIntegerKernelSourcePattern,
                get_explicit_type_name(vecType),
                get_explicit_type_name(vecType), useOpKernel ? fnName : "",
                useOpKernel ? "" : fnName);
    else
    ...
    if (create_single_kernel_helper_with_build_options(
        ...
        "sample_test", isOpenCL20Function ? "-cl-std=CL2.0" : ""))
    {
    ...
    context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
    ...
    clCreateBuffer(context,
                   (cl_mem_flags)(CL_MEM_READ_WRITE
                                  | (useOpKernel ? CL_MEM_COPY_HOST_PTR : 0)),
    ...
    if (verifyFn == verify_integer_divideAssign
        || verifyFn == verify_integer_moduloAssign)
        patchup_divide_results(outData, inDataA, inDataB, TEST_SIZE * vecSize,
                               vecType);
            ...
            if (memcmp(&expected, p, get_explicit_type_size(vecType)) != 0)
            {
                switch (get_explicit_type_size(vecType))
                {
                    case 1:
                        if (useOpKernel)
                            log_error("ERROR: Data sample %d:%d does not "
                                      "validate! Expected (0x%2.2x), got "
                                      "(0x%2.2x), sources (0x%2.2x, 0x%2.2x)\n",
                                      (int)i, (int)j,
                                      ((cl_uchar *)&expected)[0],
                                      *((cl_uchar *)p), *((cl_uchar *)in),
                                      *((cl_uchar *)in2));
                        else
                            log_error("ERROR: Data sample %d:%d does not "
                                      "validate! Expected (0x%2.2x), got "
                                      "(0x%2.2x), sources (0x%2.2x)\n",
                                      (int)i, (int)j,
                                      ((cl_uchar *)&expected)[0],
                                      *((cl_uchar *)p), *((cl_uchar *)in));
                        break;
                    ...
                    case 2:
                        if (useOpKernel)
                            log_error("ERROR: Data sample %d:%d does not "
                                      "validate! Expected (0x%4.4x), got "
                                      "(0x%4.4x), sources (0x%4.4x, 0x%4.4x)\n",
                                      (int)i, (int)j,
                                      ((cl_ushort *)&expected)[0],
                                      *((cl_ushort *)p), *((cl_ushort *)in),
                                      *((cl_ushort *)in2));
                        else
                            log_error("ERROR: Data sample %d:%d does not "
                                      "validate! Expected (0x%4.4x), got "
                                      "(0x%4.4x), sources (0x%4.4x)\n",
                                      (int)i, (int)j,
                                      ((cl_ushort *)&expected)[0],
                                      *((cl_ushort *)p), *((cl_ushort *)in));
                        break;
                    ...
                    case 4:
                        if (useOpKernel)
                            log_error("ERROR: Data sample %d:%d does not "
                                      "validate! Expected (0x%8.8x), got "
                                      "(0x%8.8x), sources (0x%8.8x, 0x%8.8x)\n",
                                      (int)i, (int)j, ((cl_uint *)&expected)[0],
                                      *((cl_uint *)p), *((cl_uint *)in),
                                      *((cl_uint *)in2));
                        else
                            log_error("ERROR: Data sample %d:%d does not "
                                      "validate! Expected (0x%8.8x), got "
                                      "(0x%8.8x), sources (0x%8.8x)\n",
                                      (int)i, (int)j, ((cl_uint *)&expected)[0],
                                      *((cl_uint *)p), *((cl_uint *)in));
                        break;
                    ...
                    case 8:
                        if (useOpKernel)
                            log_error(
                            "ERROR: Data sample %d:%d does not validate! "
                            "Expected (0x%16.16llx), got (0x%16.16llx), "
                            "sources (0x%16.16llx, 0x%16.16llx)\n",
                            (int)i, (int)j, ((cl_ulong *)&expected)[0],
                            *((cl_ulong *)p), *((cl_ulong *)in),
                            *((cl_ulong *)in2));
                        else
                            log_error("ERROR: Data sample %d:%d does not "
                                      "validate! Expected (0x%16.16llx), got "
                                      "(0x%16.16llx), sources (0x%16.16llx)\n",
                                      (int)i, (int)j,
                                      ((cl_ulong *)&expected)[0],
                                      *((cl_ulong *)p), *((cl_ulong *)in));
                        break;
...
int test_single_param_integer_fn(cl_command_queue queue, cl_context context,
                                 const char *fnName,
                                 singleParamIntegerVerifyFn verifyFn,
                                 bool useOpKernel = false)
{
    ExplicitType types[] = { kChar, kUChar, kShort, kUShort,          kInt,
                             kUInt, kLong,  kULong, kNumExplicitTypes };
    ...
    {
        if ((types[typeIndex] == kLong || types[typeIndex] == kULong)
            && !gHasLong)
            continue;

        for (index = 0; vecSizes[index] != 0; index++)
        {
            if (test_single_param_integer_kernel(
                queue, context, fnName, types[typeIndex], vecSizes[index],
                verifyFn, seed, useOpKernel)
                != 0)
            {
                log_error("   Vector %s%d FAILED\n",
                          get_explicit_type_name(types[typeIndex]),
                          vecSizes[index]);
                retVal = -1;
            }
        }
...
bool verify_integer_clz(void *source, void *destination, ExplicitType vecType)
{
    ...
    switch (vecType)
    {
        ...
        case kLong:
            ...
            break;
        case kULong:
            // Hack for now: just treat it as a signed cl_long, since it won't
            // matter for bitcounting
    ...
    switch (vecType)
    {
        case kChar: *((cl_char *)destination) = count; break;
        case kUChar: *((cl_uchar *)destination) = count; break;
        case kShort: *((cl_short *)destination) = count; break;
        case kUShort: *((cl_ushort *)destination) = count; break;
        case kInt: *((cl_int *)destination) = count; break;
        case kUInt: *((cl_uint *)destination) = count; break;
        case kLong: *((cl_long *)destination) = count; break;
        case kULong: *((cl_ulong *)destination) = count; break;
        default:
            // Should never happen
            return false;
    }
...
int test_integer_clz(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    return test_single_param_integer_fn(queue, context, "clz",
                                        verify_integer_clz);
}
...
bool verify_integer_ctz(void *source, void *destination, ExplicitType vecType)
{
    ...
    {
        for (count = 0; (0 == (testValue & 0x1)); count++) testValue >>= 1;
    }
...
int test_integer_ctz(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    return test_single_param_integer_fn(queue, context, "ctz",
                                        verify_integer_ctz);
}
...
#define OP_TEST(op, opName)                                                    \
    ...
    int test_integer_##opName##Assign(                                         \
...
#define PATCH_CASE(_out, _src, _dest, _count, _cl_type)                        \
    {                                                                          \
        ...
        for (size_t i = 0; i < _count; i++)                                    \
            if (denom[i] == 0) result[i] = (_cl_type)-1;                       \
    }
                                   ...
                                   ExplicitType vecType)
{
    ...
    {
        case kChar:
            PATCH_CASE_SIGNED(outData, inDataA, inDataB, count, cl_char,
                              CL_CHAR_MIN)
            break;
        case kUChar:
            ...
            break;
        case kShort:
            PATCH_CASE_SIGNED(outData, inDataA, inDataB, count, cl_short,
                              CL_SHRT_MIN)
            break;
        case kUShort:
            ...
            break;
        case kInt:
            PATCH_CASE_SIGNED(outData, inDataA, inDataB, count, cl_int,
                              CL_INT_MIN)
            break;
        case kUInt: PATCH_CASE(outData, inDataA, inDataB, count, cl_uint) break;
        case kLong:
            PATCH_CASE_SIGNED(outData, inDataA, inDataB, count, cl_long,
                              CL_LONG_MIN)
            break;
        case kULong:
...
"__kernel void sample_test(__global %s%s *sourceA, __global %s%s *sourceB, "
...
typedef bool (*twoParamIntegerVerifyFn)(void *sourceA, void *sourceB,
                                        void *destination,
                                        ExplicitType vecType);

static char *build_load_statement(char *outString, size_t vecSize,
                                  const char *name)
{
    if (vecSize != 3)
...
static char *build_store_statement(char *outString, size_t vecSize,
                                   const char *name, const char *srcName)
{
    if (vecSize != 3)
...
int test_two_param_integer_kernel(cl_command_queue queue, cl_context context,
                                  const char *fnName, ExplicitType vecAType,
                                  ExplicitType vecBType, unsigned int vecSize,
                                  twoParamIntegerVerifyFn verifyFn, MTdata d)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    clMemWrapper streams[3];
    cl_long inDataA[TEST_SIZE * 16], inDataB[TEST_SIZE * 16],
    outData[TEST_SIZE * 16], expected;
    int error, i;
    size_t threads[1], localThreads[1];
    char kernelSource[10240];
    char *programPtr;
    ...
    sprintf(
    kernelSource, twoParamIntegerKernelSourcePattern,
    ...
    build_load_statement(sourceALoad, (size_t)vecSize, "sourceA"),
    get_explicit_type_name(vecBType), sizeName,
    build_load_statement(sourceBLoad, (size_t)vecSize, "sourceB"),
    get_explicit_type_name(vecAType), sizeName, fnName,
    ...
    memset(outData, 0xFF,
           get_explicit_type_size(vecAType) * TEST_SIZE * vecSize);

    /* Now get the results */
    error =
    clEnqueueReadBuffer(queue, streams[2], CL_TRUE, 0,
                        get_explicit_type_size(vecAType) * TEST_SIZE * vecSize,
                        outData, 0, NULL, NULL);
    test_error(error, "Unable to read output array!");
    ...
    for (i = 0; i < (int)TEST_SIZE; i++)
    {
        for (size_t j = 0; j < vecSize; j++)
        {
            ...
            if (test
                && (memcmp(&expected, out, get_explicit_type_size(vecAType))
                    != 0))
            {
                switch (get_explicit_type_size(vecAType))
                {
                    case 1:
                        log_error("ERROR: Data sample %d:%d does not validate! "
                                  "Expected (0x%2.2x), got (0x%2.2x), sources "
                                  "(0x%2.2x, 0x%2.2x), TEST_SIZE %d\n",
                                  (int)i, (int)j, ((cl_uchar *)&expected)[0],
                                  *((cl_uchar *)out), *((cl_uchar *)inA),
                                  *((cl_uchar *)inB), TEST_SIZE);
                        break;

                    case 2:
                        log_error("ERROR: Data sample %d:%d does not validate! "
                                  "Expected (0x%4.4x), got (0x%4.4x), sources "
                                  "(0x%4.4x, 0x%4.4x), TEST_SIZE %d\n",
                                  (int)i, (int)j, ((cl_ushort *)&expected)[0],
                                  *((cl_ushort *)out), *((cl_ushort *)inA),
                                  *((cl_ushort *)inB), TEST_SIZE);
                        break;

                    case 4:
                        log_error("ERROR: Data sample %d:%d does not validate! "
                                  "Expected (0x%8.8x), got (0x%8.8x), sources "
                                  "(0x%8.8x, 0x%8.8x)\n",
                                  (int)i, (int)j, ((cl_uint *)&expected)[0],
                                  *((cl_uint *)out), *((cl_uint *)inA),
                                  *((cl_uint *)inB));
                        break;
...
int test_two_param_integer_fn(cl_command_queue queue, cl_context context,
                              const char *fnName,
                              twoParamIntegerVerifyFn verifyFn)
{
    ExplicitType types[] = { kChar, kUChar, kShort, kUShort,          kInt,
                             kUInt, kLong,  kULong, kNumExplicitTypes };
    unsigned int vecSizes[] = { 1, 2, 3, 4, 8, 16, 0 }; // TODO : 3 not tested
    ...
    for (typeIndex = 0; types[typeIndex] != kNumExplicitTypes; typeIndex++)
    {
        ...
        for (index = 0; vecSizes[index] != 0; index++)
        {
            if (test_two_param_integer_kernel(
                queue, context, fnName, types[typeIndex], types[typeIndex],
                vecSizes[index], verifyFn, seed)
                != 0)
            {
                log_error("   Vector %s%d FAILED\n",
                          get_explicit_type_name(types[typeIndex]),
...
int test_two_param_unmatched_integer_fn(cl_command_queue queue,
                                        cl_context context, const char *fnName,
                                        twoParamIntegerVerifyFn verifyFn)
{
    ExplicitType types[] = { kChar, kUChar, kShort, kUShort,          kInt,
                             kUInt, kLong,  kULong, kNumExplicitTypes };
    ...
    {
        if ((types[typeAIndex] == kLong || types[typeAIndex] == kULong)
            && !gHasLong)
            continue;

        for (typeBIndex = 0; types[typeBIndex] != kNumExplicitTypes;
             typeBIndex++)
        {
            if ((types[typeBIndex] == kLong || types[typeBIndex] == kULong)
                && !gHasLong)
                continue;
            ...
            {
                if (test_two_param_integer_kernel(
                    queue, context, fnName, types[typeAIndex],
                    types[typeBIndex], vecSizes[index], verifyFn, seed)
                    != 0)
                {
                    log_error(
                    "   Vector %s%d / %s%d FAILED\n",
                    get_explicit_type_name(types[typeAIndex]), vecSizes[index],
                    get_explicit_type_name(types[typeBIndex]), vecSizes[index]);
                    retVal = -1;
                }
            }
        }
...
bool verify_integer_hadd(void *sourceA, void *sourceB, void *destination,
                         ExplicitType vecType)
{
    ...
    switch (vecType)
    {
        case kChar:
            ...
            *((cl_char *)destination) =
            (cl_char)((testValueA + testValueB) >> 1);
            break;
        case kUChar:
            testValueA = *((cl_uchar *)sourceA);
            testValueB = *((cl_uchar *)sourceB);
            *((cl_uchar *)destination) =
            (cl_uchar)((testValueA + testValueB) >> 1);
            break;
        case kShort:
            testValueA = *((cl_short *)sourceA);
            testValueB = *((cl_short *)sourceB);
            *((cl_short *)destination) =
            (cl_short)((testValueA + testValueB) >> 1);
            break;
        case kUShort:
            testValueA = *((cl_ushort *)sourceA);
            testValueB = *((cl_ushort *)sourceB);
            *((cl_ushort *)destination) =
            (cl_ushort)((testValueA + testValueB) >> 1);
            break;
        case kInt:
        ...
        case kUInt:
            ...
            *((cl_uint *)destination) =
            (cl_uint)((testValueA + testValueB) >> 1);
            break;
        case kLong:
            ...
            *((cl_long *)destination) =
            ((testValueA >> 1) + (testValueB >> 1)) + (overflow >> 1);
            break;
        case kULong:
            // The long way to avoid dropping bits
            ...
            *((cl_ulong *)destination) =
            ((uValueA >> 1) + (uValueB >> 1)) + (uOverflow >> 1);
            break;
        default:
            // Should never happen
            return false;
...
int test_integer_hadd(cl_device_id deviceID, cl_context context,
                      cl_command_queue queue, int num_elements)
{
    return test_two_param_integer_fn(queue, context, "hadd",
                                     verify_integer_hadd);
}

bool verify_integer_rhadd(void *sourceA, void *sourceB, void *destination,
                          ExplicitType vecType)
{
    cl_long testValueA, testValueB, overflow;
    cl_ulong uValueA, uValueB, uOverflow;
    ...
    switch (vecType)
    {
        case kChar:
            ...
            testValueB = *((cl_char *)sourceB);
            *((cl_char *)destination) =
            (cl_char)((testValueA + testValueB + 1) >> 1);
            break;
        case kUChar:
            testValueA = *((cl_uchar *)sourceA);
            testValueB = *((cl_uchar *)sourceB);
            *((cl_uchar *)destination) =
            (cl_uchar)((testValueA + testValueB + 1) >> 1);
            break;
        case kShort:
            testValueA = *((cl_short *)sourceA);
            testValueB = *((cl_short *)sourceB);
            *((cl_short *)destination) =
            (cl_short)((testValueA + testValueB + 1) >> 1);
            break;
        case kUShort:
            testValueA = *((cl_ushort *)sourceA);
            testValueB = *((cl_ushort *)sourceB);
            *((cl_ushort *)destination) =
            (cl_ushort)((testValueA + testValueB + 1) >> 1);
            break;
        case kInt:
            testValueA = *((cl_int *)sourceA);
            testValueB = *((cl_int *)sourceB);
            *((cl_int *)destination) =
            (cl_int)((testValueA + testValueB + 1) >> 1);
            break;
        case kUInt:
            testValueA = *((cl_uint *)sourceA);
            testValueB = *((cl_uint *)sourceB);
            *((cl_uint *)destination) =
            (cl_uint)((testValueA + testValueB + 1) >> 1);
            break;
        case kLong:
            // The long way to avoid dropping bits
            testValueA = *((cl_long *)sourceA);
            ...
            *((cl_long *)destination) =
            ((testValueA >> 1) + (testValueB >> 1)) + overflow;
            break;
        case kULong:
            // The long way to avoid dropping bits
            uValueA = *((cl_ulong *)sourceA);
...
int test_integer_rhadd(cl_device_id deviceID, cl_context context,
                       cl_command_queue queue, int num_elements)
{
    return test_two_param_integer_fn(queue, context, "rhadd",
                                     verify_integer_rhadd);
}
...
#define MIN_CASE(type, const)                                                  \
    case const: {                                                              \
        ...
        *((cl_##type *)destination) =                                          \
        (cl_##type)(valueB < valueA ? valueB : valueA);                        \
        break;                                                                 \
    }

bool verify_integer_min(void *sourceA, void *sourceB, void *destination,
                        ExplicitType vecType)
{
    switch (vecType)
    {
...
int test_integer_min(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, int num_elements)
{
...
#define MAX_CASE(type, const)                                                  \
    case const: {                                                              \
        ...
        cl_##type valueB = *((cl_##type *)sourceB);                            \
        *((cl_##type *)destination) =                                          \
        (cl_##type)(valueA < valueB ? valueB : valueA);                        \
        break;                                                                 \
    }

bool verify_integer_max(void *sourceA, void *sourceB, void *destination,
                        ExplicitType vecType)
{
    switch (vecType)
    {
...
int test_integer_max(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, int num_elements)
{
...
void multiply_unsigned_64_by_64(cl_ulong sourceA, cl_ulong sourceB,
                                cl_ulong &destLow, cl_ulong &destHi)
{
    cl_ulong lowA, lowB;
    ...
    // + ( aLO * bLO ) ) >> 64 = ( ( aHI * bHI << 64 ) + ( aHI * bLO << 32 ) + (
    // aLO * bHI << 32 ) + ( aLO * bLO ) ) >> 64 = ( aHI * bHI ) + ( aHI * bLO
    // >> 32 ) + ( aLO * bHI >> 32 ) + ( aLO * bLO >> 64 )
    ...
    //
    //      We note that we can add the maximum N-bit number to the 2N-bit
    //      product twice without overflow:
    //
    //          (2**N-1)^2 + 2*(2**N-1) = 2**2N - 2**(N+1) + 1 + 2**(N+1) - 2 =
    //          2**2N - 1
    //
    //  If we breakdown the product of two numbers a,b into high and low halves
    ...
    // (b.hi*a.lo).lo to the 2N bit term [(b.lo*a.hi).hi + (b.lo*a.hi).lo]
    // without overflow.  That takes care of all of the terms on the right half
    // that might carry.  Do that now.
    //
    ...
    // All of the overflow potential from the right half has now been
    // accumulated into the [ (b.lo*a.hi).hi + (b.lo*a.hi).lo ] 2N bit term. We
    // can safely separate into high and low parts. Per our rule above, we know
    // we can accumulate the high part of that and (b.hi*a.lo).hi into the 2N
    // bit term (b.lo*a.hi) without carry.  The low part can be pieced together
    // with (b.lo*a.lo).lo, to give the final low result

    destHi = aHibHi + (aHibLo >> 32) + (aLobHi >> 32); // Cant overflow
    destLow = (aHibLo << 32) | (aLobLo & 0xFFFFFFFFULL);
}

void multiply_signed_64_by_64(cl_long sourceA, cl_long sourceB,
                              cl_ulong &destLow, cl_long &destHi)
{
    // Find sign of result
    ...
    cl_ulong hi;
    multiply_unsigned_64_by_64((cl_ulong)sourceA, (cl_ulong)sourceB, destLow,
                               hi);
    ...
    if (resultSign)
    {
        ...
        // carry if necessary
        if (0 == destLow) hi -= resultSign;
    }

    destHi = (cl_long)hi;
}

bool verify_integer_mul_hi(void *sourceA, void *sourceB, void *destination,
                           ExplicitType vecType)
{
    ...
    switch (vecType)
    {
        case kChar:
            ...
            testValueB = *((cl_char *)sourceB);
            *((cl_char *)destination) =
            (cl_char)((testValueA * testValueB) >> 8);
            break;
        case kUChar:
            testValueA = *((cl_uchar *)sourceA);
            testValueB = *((cl_uchar *)sourceB);
            *((cl_uchar *)destination) =
            (cl_uchar)((testValueA * testValueB) >> 8);
            break;
        case kShort:
            testValueA = *((cl_short *)sourceA);
            testValueB = *((cl_short *)sourceB);
            *((cl_short *)destination) =
            (cl_short)((testValueA * testValueB) >> 16);
            break;
        case kUShort:
            testValueA = *((cl_ushort *)sourceA);
            testValueB = *((cl_ushort *)sourceB);
            *((cl_ushort *)destination) =
            (cl_ushort)((testValueA * testValueB) >> 16);
            break;
        case kInt:
            testValueA = *((cl_int *)sourceA);
            testValueB = *((cl_int *)sourceB);
            *((cl_int *)destination) =
            (cl_int)((testValueA * testValueB) >> 32);
            break;
        case kUInt:
            testValueA = *((cl_uint *)sourceA);
            testValueB = *((cl_uint *)sourceB);
            *((cl_uint *)destination) =
            (cl_uint)((testValueA * testValueB) >> 32);
            break;
        case kLong:
            testValueA = *((cl_long *)sourceA);
            testValueB = *((cl_long *)sourceB);

            multiply_signed_64_by_64(testValueA, testValueB, lowHalf,
                                     highSigned);
        ...
        case kULong:
            ...
            multiply_unsigned_64_by_64(testValueA, testValueB, lowHalf,
                                       highUnsigned);
...
int test_integer_mul_hi(cl_device_id deviceID, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    return test_two_param_integer_fn(queue, context, "mul_hi",
                                     verify_integer_mul_hi);
}

bool verify_integer_rotate(void *sourceA, void *sourceB, void *destination,
                           ExplicitType vecType)
{
    cl_ulong testValueA;
    char numBits;
    ...
    switch (vecType)
    {
        ...
        case kUChar:
            ...
            if (numBits == 0)
                ...
                *((cl_uchar *)destination) = (cl_uchar)(
                (testValueA << numBits) | (testValueA >> (8 - numBits)));
            break;
        case kShort:
        case kUShort:
            testValueA = *((cl_ushort *)sourceA);
            ...
            if (numBits == 0)
                ...
                *((cl_ushort *)destination) = (cl_ushort)(
                (testValueA << numBits) | (testValueA >> (16 - numBits)));
            break;
        case kInt:
        case kUInt:
            testValueA = *((cl_uint *)sourceA);
            ...
            if (numBits == 0)
                ...
                *((cl_uint *)destination) = (cl_uint)(
                (testValueA << numBits) | (testValueA >> (32 - numBits)));
            break;
        case kLong:
        case kULong:
            testValueA = *((cl_ulong *)sourceA);
            ...
            if (numBits == 0)
                ...
                *((cl_ulong *)destination) = (cl_ulong)(
                (testValueA << numBits) | (testValueA >> (64 - numBits)));
            break;
        default:
            // Should never happen
            log_error("Unknown type encountered in verify_integer_rotate. Test "
                      "failed. Aborting...\n");
            abort();
            return false;
    }
...
int test_integer_rotate(cl_device_id deviceID, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    return test_two_param_integer_fn(queue, context, "rotate",
                                     verify_integer_rotate);
}

const char *threeParamIntegerKernelSourcePattern =
"__kernel void sample_test(__global %s%s *sourceA, __global %s%s *sourceB, "
"__global %s%s *sourceC, __global %s%s *destValues)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"    %s%s sA = %s;\n"
"    %s%s sB = %s;\n"
...
typedef bool (*threeParamIntegerVerifyFn)(void *sourceA, void *sourceB,
                                          void *sourceC, void *destination,
                                          ExplicitType vecAType,
                                          ExplicitType vecBType,
                                          ExplicitType vecCType,
                                          ExplicitType destType);

int test_three_param_integer_kernel(cl_command_queue queue, cl_context context,
                                    const char *fnName, ExplicitType vecAType,
                                    ExplicitType vecBType,
                                    ExplicitType vecCType,
                                    ExplicitType destType, unsigned int vecSize,
                                    threeParamIntegerVerifyFn verifyFn,
                                    MTdata d)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    ...
    cl_long inDataA[TEST_SIZE * 16], inDataB[TEST_SIZE * 16],
    inDataC[TEST_SIZE * 16], outData[TEST_SIZE * 16], expected;
    int error, i;
    size_t threads[1], localThreads[1];
    char kernelSource[10240];
    char *programPtr;
    ...
    sprintf(
    kernelSource, threeParamIntegerKernelSourcePattern,
    get_explicit_type_name(vecAType), paramSizeName,
    get_explicit_type_name(vecBType), paramSizeName,
    ...
    get_explicit_type_name(vecBType), sizeName,
    build_load_statement(sourceBLoad, (size_t)vecSize, "sourceB"),
    get_explicit_type_name(vecCType), sizeName,
    build_load_statement(sourceCLoad, (size_t)vecSize, "sourceC"),
    get_explicit_type_name(destType), sizeName, fnName,
    build_store_statement(destStore, (size_t)vecSize, "destValues", "dst"));
    ...
    memset(outData, 0xFF,
           get_explicit_type_size(destType) * TEST_SIZE * vecSize);

    /* Now get the results */
    error =
    clEnqueueReadBuffer(queue, streams[3], CL_TRUE, 0,
                        get_explicit_type_size(destType) * TEST_SIZE * vecSize,
                        outData, 0, NULL, NULL);
    test_error(error, "Unable to read output array!");
    ...
    for (i = 0; i < (int)TEST_SIZE; i++)
    {
        for (size_t j = 0; j < vecSize; j++)
        {
            bool test = verifyFn(inA, inB, inC, &expected, vecAType, vecBType,
                                 vecCType, destType);
            if (test
                && (memcmp(&expected, out, get_explicit_type_size(destType))
                    != 0))
            {
                switch (get_explicit_type_size(vecAType))
                {
                    ...
                    case 4:
                        log_error("ERROR: Data sample %d:%d does not validate! "
                                  ...
                                  (int)i, (int)j, ((cl_uint *)&expected)[0],
                                  *((cl_uint *)out), *((cl_uint *)inA),
                                  *((cl_uint *)inB), *((cl_uint *)inC));
                        break;

                    case 8:
                        log_error(
                        "ERROR: Data sample %d:%d does not validate! Expected "
                        "(0x%16.16llx), got (0x%16.16llx), sources "
                        "(0x%16.16llx, 0x%16.16llx, 0x%16.16llx)\n",
                        (int)i, (int)j, ((cl_ulong *)&expected)[0],
                        *((cl_ulong *)out), *((cl_ulong *)inA),
...
int test_three_param_integer_fn(cl_command_queue queue, cl_context context,
                                const char *fnName,
                                threeParamIntegerVerifyFn verifyFn)
{
    ExplicitType types[] = { kChar, kUChar, kShort, kUShort,          kInt,
    ...
    for (typeAIndex = 0; types[typeAIndex] != kNumExplicitTypes; typeAIndex++)
    {
        ...
        for (index = 0; vecSizes[index] != 0; index++)
        {
            if (test_three_param_integer_kernel(
                queue, context, fnName, types[typeAIndex], types[typeAIndex],
                types[typeAIndex], types[typeAIndex], vecSizes[index], verifyFn,
                seed)
                != 0)
            {
                log_error(
...
bool verify_integer_clamp(void *sourceA, void *sourceB, void *sourceC,
                          void *destination, ExplicitType vecAType,
                          ExplicitType vecBType, ExplicitType vecCType,
                          ExplicitType destType)
{
    if (vecAType == kULong || vecAType == kUInt || vecAType == kUShort
        || vecAType == kUChar)
    {
        ...
        switch (vecAType)
        {
            ...
            case kUInt:
                ...
                (cl_uint)(MAX(MIN(valueA, valueC), valueB));
                break;
            case kUShort:
                ((cl_ushort *)destination)[0] =
                (cl_ushort)(MAX(MIN(valueA, valueC), valueB));
                break;
            case kUChar:
                ((cl_uchar *)destination)[0] =
                (cl_uchar)(MAX(MIN(valueA, valueC), valueB));
                break;
            default:
                // error -- should never get here
                abort();
    ...
    else
    {
        ...
        switch (vecAType)
        {
            ...
            case kInt:
                ...
                (cl_int)(MAX(MIN(valueA, valueC), valueB));
                break;
            case kShort:
                ((cl_short *)destination)[0] =
                (cl_short)(MAX(MIN(valueA, valueC), valueB));
                break;
            case kChar:
                ((cl_char *)destination)[0] =
                (cl_char)(MAX(MIN(valueA, valueC), valueB));
                break;
            default:
                // error -- should never get here
                abort();
...
int test_integer_clamp(cl_device_id deviceID, cl_context context,
                       cl_command_queue queue, int num_elements)
{
    return test_three_param_integer_fn(queue, context, "clamp",
                                       verify_integer_clamp);
}

bool verify_integer_mad_sat(void *sourceA, void *sourceB, void *sourceC,
                            void *destination, ExplicitType vecAType,
                            ExplicitType vecBType, ExplicitType vecCType,
                            ExplicitType destType)
{
        ...
        || vecAType == kUChar)
    {
        ...
        if (multHi) multLo = 0xFFFFFFFFFFFFFFFFULL;

        switch (vecAType)
        {
            case kULong: ((cl_ulong *)destination)[0] = multLo; break;
            case kUInt:
                ((cl_uint *)destination)[0] =
                (cl_uint)MIN(multLo, (cl_ulong)CL_UINT_MAX);
                break;
            case kUShort:
                ((cl_ushort *)destination)[0] =
                (cl_ushort)MIN(multLo, (cl_ulong)CL_USHRT_MAX);
                break;
            case kUChar:
                ((cl_uchar *)destination)[0] =
                (cl_uchar)MIN(multLo, (cl_ulong)CL_UCHAR_MAX);
                break;
            default:
                // error -- should never get here
                abort();
    ...
    else
    {
        ...
        switch (vecAType)
        {
            case kLong: ((cl_long *)destination)[0] = result; break;
            case kInt:
...
int test_integer_mad_sat(cl_device_id deviceID, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    return test_three_param_integer_fn(queue, context, "mad_sat",
                                       verify_integer_mad_sat);
}

bool verify_integer_mad_hi(void *sourceA, void *sourceB, void *sourceC,
                           void *destination, ExplicitType vecAType,
                           ExplicitType vecBType, ExplicitType vecCType,
                           ExplicitType destType)
{
        ...
        || vecAType == kUChar)
    {
        ...
        switch (vecAType)
        {
            case kULong: ((cl_ulong *)destination)[0] = multHi + valueC; break;
            case kUInt:
                ((cl_uint *)destination)[0] =
                (cl_uint)((multLo >> 32) + valueC);
                break;
            case kUShort:
                ((cl_ushort *)destination)[0] =
                (cl_ushort)((multLo >> 16) + valueC);
                break;
            case kUChar:
                ((cl_uchar *)destination)[0] =
                (cl_uchar)((multLo >> 8) + valueC);
                break;
            default:
                // error -- should never get here
                abort();
    ...
    else
    {
        ...
        switch (vecAType)
        {
            case kLong: ((cl_long *)destination)[0] = multHi + valueC; break;
            case kInt:
                ((cl_int *)destination)[0] = (cl_int)((multLo >> 32) + valueC);
                break;
            case kShort:
                ((cl_short *)destination)[0] =
                (cl_int)((multLo >> 16) + valueC);
                break;
            case kChar:
                ((cl_char *)destination)[0] =
                (cl_char)(cl_int)((multLo >> 8) + valueC);
                break;
            default:
                // error -- should never get here
                abort();
...
int test_integer_mad_hi(cl_device_id deviceID, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    return test_three_param_integer_fn(queue, context, "mad_hi",
                                       verify_integer_mad_hi);
}
