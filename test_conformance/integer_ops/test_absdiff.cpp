...
static int verify_absdiff_char(const void *p, const void *q, const void *r,
                               size_t n, const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (inB[i] > inA[i]) r = inB[i] - inA[i];
        if (r != outptr[i])
        {
            log_info("%ld) Failure for absdiff( (char%s) 0x%2.2x, (char%s) "
...
static int verify_absdiff_uchar(const void *p, const void *q, const void *r,
                                size_t n, const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_info("%ld) Failure for absdiff( (uchar%s) 0x%2.2x, (uchar%s) "
                     "0x%2.2x) = *0x%2.2x vs 0x%2.2x\n",
                     i, sizeName, inA[i], sizeName, inB[i], r, outptr[i]);
            return -1;
        }
...
static int verify_absdiff_short(const void *p, const void *q, const void *r,
                                size_t n, const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_info("%ld) Failure for absdiff( (short%s) 0x%4.4x, (short%s) "
...
static int verify_absdiff_ushort(const void *p, const void *q, const void *r,
                                 size_t n, const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_info("%ld) Failure for absdiff( (ushort%s) 0x%4.4x, (ushort%s) "
                     "0x%4.4x) = *0x%4.4x vs 0x%4.4x\n",
                     i, sizeName, inA[i], sizeName, inB[i], r, outptr[i]);
            return -1;
        }
...
static int verify_absdiff_int(const void *p, const void *q, const void *r,
                              size_t n, const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_info("%ld) Failure for absdiff( (int%s) 0x%8.8x, (int%s) "
...
static int verify_absdiff_uint(const void *p, const void *q, const void *r,
                               size_t n, const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_info("%ld) Failure for absdiff( (uint%s) 0x%8.8x, (uint%s) "
                     "0x%8.8x) = *0x%8.8x vs 0x%8.8x\n",
                     i, sizeName, inA[i], sizeName, inB[i], r, outptr[i]);
            return -1;
        }
...
static int verify_absdiff_long(const void *p, const void *q, const void *r,
                               size_t n, const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_info("%ld) Failure for absdiff( (long%s) 0x%16.16llx, (long%s) "
...
static int verify_absdiff_ulong(const void *p, const void *q, const void *r,
                                size_t n, const char *sizeName, size_t vecSize)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (r != outptr[i])
        {
            log_info("%ld) Failure for absdiff( (ulong%s) 0x%16.16llx, "
...
typedef int (*verifyFunc)(const void *, const void *, const void *, size_t n,
                          const char *sizeName, size_t vecSize);
static const verifyFunc verify[] = {
    verify_absdiff_char,   verify_absdiff_uchar, verify_absdiff_short,
    verify_absdiff_ushort, verify_absdiff_int,   verify_absdiff_uint,
...
int test_integer_abs_diff(cl_device_id device, cl_context context,
                          cl_command_queue queue, int n_elems)
{
         ...
         type++)
    {
             ...
             vectorSize++)
        {
            ...
            const char *source[] = {
                "__kernel void test_absdiff_",
                test_str_names[type],
                vector_size_names[vectorSize],
                "(__global ",
                test_str_names[type],
                vector_param_size_names[vectorSize],
                " *srcA, __global ",
                test_str_names[type],
                vector_param_size_names[vectorSize],
                " *srcB, __global u",
                test_str_names[type & -2],
                vector_param_size_names[vectorSize],
                " *dst)\n"
                "{\n"
                ...
                " sA, sB;\n",
                "    sA = ",
                (vector_sizes[vectorSize] == 3) ? "vload3( tid, srcA )"
                                                : "srcA[tid]",
                ";\n",
                "    sB = ",
                (vector_sizes[vectorSize] == 3) ? "vload3( tid, srcB )"
                                                : "srcB[tid]",
                ";\n",
                "    u",
                test_str_names[type & -2],
                vector_size_names[vectorSize],
                " dstVal = abs_diff(sA, sB);\n"
                "     ",
                (vector_sizes[vectorSize] == 3) ? "vstore3( dstVal, tid, dst )"
                                                : "dst[ tid ] = dstVal",
                ";\n",
                "}\n"
            };
            ...
            char kernelName[128];
            snprintf(kernelName, sizeof(kernelName), "test_absdiff_%s%s",
