                        ...
                        cl_ulong *max_physical_result)
{
    ...
    for (i = 0; i < num_devices_returned; i++)
    {
        ...
        clGetDeviceInfo(devices[i], CL_DEVICE_MAX_MEM_ALLOC_SIZE,
                        sizeof(current_allocation), &current_allocation, NULL);
        if (err != CL_SUCCESS)
        {
...
"                error = 256;\n"
"\n"
"        unsigned long t_address = (unsigned long)get_global_id(2)*(unsigned "
"long)final_y_size*(unsigned long)final_x_size + \n"
             ...
             int explict_local)
{
    ...
    * (cl_ulong)final_y_size * (cl_ulong)final_z_size * sizeof(cl_uint);
    ...
    while (end_valid_memory_address <= last_memory_address)
    {
        ...
        {
            err =
            clEnqueueNDRangeKernel(queue, kernel, dimensions, NULL, global_size,
                           ...
                           int explicit_local)
{
    ...
    if (quick_test)
        log_info(
        "WARNING: Running quick test. This will only test the base dimensions "
        "(power of two) and base-1 with all local threads fixed in one dim.\n");
    ...
    {
        array = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                               memory_size, NULL, &err);
                         ...
                         local_test++)
                    {
                        ...
                        else
                        {
                            ...
                            if (local_test % 2)
                            {
                                ...
                                {
                                    local_y_size =
                                    (int)get_random_float(1, (int)remainder, d);
                                    while (
                                    (local_y_size > 1)
                        ...
                        if (total_local_size < max_workgroup_size)
                        {
                            if (total_global_size > 16384 * 16384)
                            {
                                ...
                                {
                                    log_info(
                                    "Skipping test as local_size is small and "
                        ...
                        err = run_test(
                        context, queue, kernel, array, memory_size, dimensions,
                        final_x_size, final_y_size, final_z_size, local_x_size,
                        local_y_size, local_z_size, explicit_local);
