...
static uint32_t clz(uint64_t value)
{
    ...
    {
        if (0x8000000000000000 & (value << num_zeros)) break;
    }
    return num_zeros;
}
...
float qcom_s64_2_f32(int64_t data, bool sat, roundingMode rnd)
{
    switch (rnd)
    {
        case qcomRTZ: {
            ...
            uint32_t exponent = (127 + 64 - clz(data) - 1)
            << (FLT_MANT_DIG
                - 1); // add 1 for the implied 1.0 in normalized fp32 numbers
        ...
        case qcomRTP: {
            ...
            if (!data) return 0.0f;
            if (data == 0x8000000000000000) return as_float(f);
            if (data < 0)
            {
                data = -data;
                sign = 1;
            ...
            if (mantShift >= 0)
            {
                ...
                if ((temp << mantShift) != data) inExact = 1;
            ...
            if (sign)
                return as_float(
                result); // for negative inputs return rtz results
            else
            {
                if (inExact)
                { // for positive inputs return higher next fp
                    ...
                    return nextafterf(
                    as_float(result),
                    as_float(high_float)); // could be simplified with some inc
                                           // and carry operation
                }
                else
                    return as_float(result);
        ...
        case qcomRTN: {
            ...
            if (!sign)
                return as_float(
                result); // for positive inputs return RTZ result
            else
            {
                if (inExact)
                { // for negative inputs find the lower next fp number
                    uint32_t low_float = 0xff7fffff;
                    return nextafterf(
                    as_float(result),
                    as_float(low_float)); // could be simplified with some inc
                                          // and carry operation
                }
                else
                    return as_float(result);
    ...
    switch (rnd)
    {
        ...
        case qcomRTP: {
            ...
            { // for positive inputs return higher next fp
                ...
                return nextafterf(
                as_float(result),
                as_float(high_float)); // could be simplified with some inc and
                                       // carry operation
            }
            else
                return as_float(result);
