...
static void PrintUsage(void)
{
    ...
    vlog(
    "\t\t-w\tToggle wimpy mode. When wimpy mode is on, we run a very small "
    "subset of the tests for each fn. NOT A VALID TEST! (Off by default.)\n");
...
{
    volatile cl_event
    calcReferenceValues; // user event which signals when main thread is done
...
cl_int InitData(cl_uint job_id, cl_uint thread_id, void *p)
{
    ...
    info->round, info->outType, info->start + job_id * info->size, info->size,
    info->d[thread_id]);
    return CL_SUCCESS;
}
...
cl_int PrepareReference(cl_uint job_id, cl_uint thread_id, void *p)
{
    ...
    if (outType != inType)
    {
         ...
         * modes, we need to use alternative reference function */
        switch (round)
        {
                  ...
                  SaturationMode sat, RoundingMode round, MTdata d)
{
    ...
    gTestCount++;
    size_t blockCount =
    BUFFER_SIZE / MAX(gTypeSizes[inType], gTypeSizes[outType]);
    ...
    for (i = 0; i < (uint64_t)lastCase; i += step)
    {
        ...
        if ((error = clSetEventCallback(writeInputBuffer, CL_COMPLETE,
                                        WriteInputBufferComplete,
                                        &writeInputBufferInfo)))
        {
        ...
        // The event can't be destroyed until the callback is called, so we can
        // release it now.
        ...
        if ((error =
             clWaitForEvents(1, (cl_event *)&writeInputBufferInfo.doneBarrier)))
        {
...
void CL_CALLBACK WriteInputBufferComplete(cl_event e, cl_int status, void *data)
{
    ...
    for (vectorSize = gMinVectorSize; vectorSize < gMaxVectorSize; vectorSize++)
    {
        ...
        gQueue, gOutBuffers[vectorSize], CL_FALSE, CL_MAP_READ | CL_MAP_WRITE,
        ...
        if ((status = clSetEventCallback(mapComplete, CL_COMPLETE,
                                         MapResultValuesComplete,
                                         info->calcInfo + vectorSize)))
        {
            vlog_error(
            "ERROR: WriteInputBufferComplete calback failed with status: %d\n",
            status);
        ...
        {
            vlog_error(
            "ERROR: clReleaseEvent calback failed in WriteInputBufferComplete "
            "for vector size %d with status: %d\n",
            vectorSize, status);
            gFailCount++;
            return;
        }
                                             ...
                                             void *data)
{
    ...
    if (1 == ThreadPool_AtomicAdd(&info->parent->barrierCount, -1))
    {
        ...
        {
            vlog_error(
            "ERROR: clSetUserEventStatus failed in CalcReferenceValuesComplete "
            "(err: %d). We're probably going to deadlock.\n",
            status);
            gFailCount++;
            return;
        }
                              ...
                              cl_kernel *outKernel)
{
    ...
    if (0 == vectorSize)
    {
        char inName[32];
        char outName[32];
        const char *programSource[] = { "", // optional pragma
                                        "__kernel void ",
                                        testName,
                                        "( __global ",
                                        inName,
    ...
    else
    {
        ...
        const char *programSource[] = { "", // optional pragma
                                        ...
                                        "( src[i] );\n"
                                        "}\n" };
        const char *programSourceV3[] = { "", // optional pragma
                                          "__kernel void ",
                                          testName,
                                          "( __global ",
                                          inName,
                                          ...
                                          "   }\n"
                                          "}\n" };
        stringCount = 3 == vectorSizetmp
        ? sizeof(programSourceV3) / sizeof(programSourceV3[0])
    ...
    *outKernel = NULL;
    ...
    // build it
    error =
    create_single_kernel_helper(gContext, &program, outKernel,
                                (cl_uint)stringCount, strings, testName, flags);
    if (error)
    {
        char buffer[2048] = "";
