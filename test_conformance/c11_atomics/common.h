...
public:
                                  ...
                                  cl_command_queue queue)
    {
        if (LocalMemory() && MemoryScope() != MEMORY_SCOPE_EMPTY
            && MemoryScope()
            != MEMORY_SCOPE_WORK_GROUP) // memory scope should only be used for
                                        // global memory
            return 0;
        ...
        for (unsigned oi = 0; oi < memoryOrder.size(); oi++)
        {
            for (unsigned si = 0; si < memoryScope.size(); si++)
            {
                ...
                (CBasicTest<HostAtomicType,
                            HostDataType>::ExecuteForEachParameterSet(deviceID,
                                                                      context,
                                                                      queue)));
            }
        }
...
CBasicTest<HostAtomicType, HostDataType>::KernelCode(cl_uint maxNumDestItems)
{
    ...
    if (LocalMemory())
    {
        code += "  // initialize atomics not reachable from host (first thread "
                "is doing this, other threads are waiting on barrier)\n"
                "  if(get_local_id(0) == 0)\n"
                "    for(uint dstItemIdx = 0; dstItemIdx < numDestItems; "
                "dstItemIdx++)\n"
                "    {\n";
        if (aTypeName == "atomic_flag")
        {
            code += "      if(finalDest[dstItemIdx])\n"
        ...
        {
            code += "      atomic_store(destMemory+dstItemIdx, "
    ...
    if (LocalMemory() || DeclaredInProgram())
    {
        ...
        if (LocalMemory())
            ...
            code += "  if(atomic_fetch_add(&finishedThreads, 1) == "
                    "get_global_size(0)-1)\n"
        ...
        if (aTypeName == "atomic_flag")
        {
            code += "      finalDest[dstItemIdx] = "
                    "atomic_flag_test_and_set(destMemory+dstItemIdx);\n";
        }
        else
        {
            code += "      finalDest[dstItemIdx] = "
                    "atomic_load(destMemory+dstItemIdx);\n";
        }
    }
    code += "}\n"
            "\n";
...
cl_device_id deviceID, cl_context context, cl_command_queue queue)
{
    ...
    if (deviceThreadCount > 0)
    {
        ...
        if (LocalMemory())
        {
            ...
            kernel, deviceID, CL_KERNEL_LOCAL_MEM_SIZE, sizeof(usedLocalMemory),
            &usedLocalMemory, NULL);
            test_error(error, "clGetKernelWorkGroupInfo failed");

            error =
            clGetDeviceInfo(deviceID, CL_DEVICE_LOCAL_MEM_SIZE,
                            sizeof(totalLocalMemory), &totalLocalMemory, NULL);
            test_error(error, "clGetDeviceInfo failed");

            // We know that each work-group is going to use typeSize *
            // deviceThreadCount bytes of local memory so pick the maximum value
            // for deviceThreadCount that uses all the local memory.
            maxWorkGroupSize =
    ...
    if (UseSVM())
    {
        if (gUseHostPtr)
            ...
            | (SVMDataBufferAllSVMConsistent() ? CL_MEM_SVM_ATOMICS : 0),
    ...
    if (deviceThreadCount > 0)
    {
        ...
        error =
        clSetKernelArg(kernel, argInd++, sizeof(numDestItems), &numDestItems);
