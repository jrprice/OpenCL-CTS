                              ...
                              bool useSVM)
{
    ...
    if (AtomicTypeInfo(TYPE_ATOMIC_SIZE_T).Size(deviceID) == 4)
    {
        ...
        EXECUTE_TEST(
        error, test_intptr_t.Execute(deviceID, context, queue, num_elements));
        CBasicTestStore<HOST_ATOMIC_UINTPTR_T32, HOST_UINTPTR_T32>
        test_uintptr_t(TYPE_ATOMIC_UINTPTR_T, useSVM);
        EXECUTE_TEST(
        error, test_uintptr_t.Execute(deviceID, context, queue, num_elements));
        ...
        EXECUTE_TEST(
        error, test_ptrdiff_t.Execute(deviceID, context, queue, num_elements));
    }
    else
    {
...
public:
    ...
    virtual std::string ProgramCore()
    {
        std::string memoryOrderScope = MemoryOrderScopeStr();
        std::string postfix(memoryOrderScope.empty() ? "" : "_explicit");
        return "  atomic_store(&destMemory[tid], tid);\n"
               "  oldValues[tid] = atomic_load"
        + postfix + "(&destMemory[tid]" + memoryOrderScope + ");\n";
    }
    virtual void HostFunction(cl_uint tid, cl_uint threadCount,
                              volatile HostAtomicType *destMemory,
                              HostDataType *oldValues)
...
public:
    ...
    {
        oldValues[tid] =
        host_atomic_exchange(&destMemory[0], (HostDataType)tid, MemoryOrder());
        for (int i = 0; i < Iterations(); i++)
            oldValues[tid] =
            host_atomic_exchange(&destMemory[0], oldValues[tid], MemoryOrder());
    }
    virtual bool VerifyRefs(bool &correct, cl_uint threadCount,
                            HostDataType *refValues,
                            HostAtomicType *finalValues)
...
public:
    ...
    virtual std::string ProgramCore()
    {
        ...
        + memoryOrderScope
        + ") || expected == tid)\n"
          "    oldValues[tid] = threadCount+1; //mark unexpected success with "
          "invalid value\n"
          "  else\n"
          "  {\n"
          "    for(int i = 0; i < "
        ...
        + "))\n"
          ...
          "      else\n"
          "      {\n"
          "        if(previous == expected) // spurious failure - shouldn't "
          "occur for 'strong'\n"
          "        {\n"
...
public:
    ...
    virtual std::string ProgramCore()
    {
        ...
        + "(&destMemory[0], (" + DataType().AddSubOperandTypeName() + ")tid + 3"
        + memoryOrderScope + ");\n" + "  atomic_fetch_add" + postfix
        + "(&destMemory[0], (" + DataType().AddSubOperandTypeName() + ")tid + 3"
        + memoryOrderScope
        + ");\n"
          "  atomic_fetch_add"
        + postfix + "(&destMemory[0], (" + DataType().AddSubOperandTypeName()
        + ")tid + 3" + memoryOrderScope
        + ");\n"
          "  atomic_fetch_add"
        + postfix + "(&destMemory[0], ((" + DataType().AddSubOperandTypeName()
        + ")tid + 3) << (sizeof(" + DataType().AddSubOperandTypeName()
        + ")-1)*8" + memoryOrderScope + ");\n";
    }
    virtual void HostFunction(cl_uint tid, cl_uint threadCount,
                              volatile HostAtomicType *destMemory,
                              HostDataType *oldValues)
...
public:
    ...
    virtual std::string ProgramCore()
    {
        ...
        + "(&destMemory[0], tid + 3 +(((" + DataType().AddSubOperandTypeName()
        + ")tid + 3) << (sizeof(" + DataType().AddSubOperandTypeName()
        + ")-1)*8)" + memoryOrderScope + ");\n";
    }
    virtual void HostFunction(cl_uint tid, cl_uint threadCount,
...
public:
    ...
    virtual std::string ProgramCore()
    {
        ...
        + DataType().RegularTypeName() + ")1 << bitIndex) " + memoryOrderScope
        + ");\n";
    }
    virtual void HostFunction(cl_uint tid, cl_uint threadCount,
                              volatile HostAtomicType *destMemory,
                              HostDataType *oldValues)
    {
        ...
        oldValues[tid] = host_atomic_fetch_or(
        &destMemory[whichResult], ((HostDataType)1 << bitIndex), MemoryOrder());
    }
    virtual bool ExpectedValue(HostDataType &expected, cl_uint threadCount,
                               HostDataType *startRefValues,
                               cl_uint whichDestValue)
...
public:
                              ...
                              HostDataType *oldValues)
    {
        ...
        oldValues[tid] =
        host_atomic_fetch_and(&destMemory[whichResult],
...
public:
                              ...
                              HostDataType *oldValues)
    {
        ...
        {
            value =
            host_atomic_fetch_or(destMemory + valueInd, bitMask, MemoryOrder());
            if (value & bitMask) // bit should be set to 0
                oldValues[tid]++;
...
public:
    ...
    {
        oldValues[tid] =
        host_atomic_fetch_min(&destMemory[0], oldValues[tid], MemoryOrder());
    }
...
public:
    ...
    {
        oldValues[tid] =
        host_atomic_fetch_max(&destMemory[0], oldValues[tid], MemoryOrder());
    }
    virtual bool GenerateRefs(cl_uint threadCount, HostDataType *startRefValues,
                              MTdata d)
    {
...
public:
    ...
    virtual std::string ProgramCore()
    {
        ...
        std::string program =
        "  uint cnt, stop = 0;\n"
        "  for(cnt = 0; !stop && cnt < threadCount; cnt++) // each thread must "
        "find critical section where it is the first visitor\n"
        "  {\n"
        "    bool set = atomic_flag_test_and_set"
        + postfix + "(&destMemory[cnt]" + memoryOrderScope + ");\n";
        if (MemoryOrder() == MEMORY_ORDER_RELAXED
            || MemoryOrder() == MEMORY_ORDER_RELEASE)
            program += "    atomic_work_item_fence("
            + std::string(LocalMemory() ? "CLK_LOCAL_MEM_FENCE, "
...
public:
    ...
    virtual cl_uint NumNonAtomicVariablesPerThread()
    {
        ...
        if (LocalMemory())
        {
            if (gIsEmbedded)
            {
                if (CurrentGroupSize() > 1024) CurrentGroupSize(1024);
                return 1; // 1KB of local memory required by spec. Clamp group
                          // size to 1k and allow 1 variable per thread
            }
            else
    ...
    virtual std::string ProgramHeader(cl_uint maxNumDestItems)
    {
        ...
        return header
        + CBasicTestMemOrderScope<HostAtomicType, HostDataType>::ProgramHeader(
               maxNumDestItems);
    }
    virtual std::string ProgramCore()
    {
             ...
             workOffset += workSize)
        {
                 ...
                 i++)
            {
                        ...
                        if (myValue > hisRealValue) // I'm slower
                        {
                            if (hisRealValue != hisValue)
                            {
                                log_error("ERROR: Invalid reference value #%u "
                                          ...
                                          hisRealValue);
                                log_error(
                                "ERROR: Slower thread #%u should know value "
                                "written by faster thread #%u\n",
                ...
                else
                {
                    ...
                    {
                        log_error(
                        "ERROR: Invalid atomic value #%u (%d instead of %d)\n",
                        workOffset + i, myValue,
