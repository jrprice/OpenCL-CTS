...
public:
    CStressTest(const std::vector<std::string>& kernel)
...
int test_max_number_of_params(cl_device_id deviceID, cl_context context,
                              cl_command_queue queue, int num_elements)
{
    cl_int error;
    ...
    error =
    clGetDeviceInfo(deviceID, CL_DEVICE_ADDRESS_BITS, sizeof(deviceAddressBits),
    ...
    const std::string KERNEL_FUNCTION_TEMPLATE[] = {
        ...
        + NL NL "bool helperFunction(int *ptr0 ",
        // the rest of arguments goes here
        ") {" NL "    // check first pointer only" NL
        "    if (!isFenceValid(get_fence(ptr0)))" NL
        "        return false;" NL NL "    return true;" NL "}" NL NL
        "__kernel void testKernel(__global uint *results) {" NL
        "    uint tid = get_global_id(0);" NL NL "    __global int * gptr;" NL
        "    __local int * lptr;" NL "    __private int * pptr;" NL NL
        "    size_t failures = 0;" NL NL,
    ...
    const std::string KERNEL_FUNCTION = KERNEL_FUNCTION_TEMPLATE[0]
    + type_params.str() + KERNEL_FUNCTION_TEMPLATE[1] + function_calls.str()
    + KERNEL_FUNCTION_TEMPLATE[2];
