...
"__kernel void test_bcast(const __global Type *in, __global int2 *xy, __global "
"Type *out)\n"
"{\n"
"    int gid = get_global_id(0);\n"
"    XY(xy,gid);\n"
    ...
    static void gen(Ty *x, Ty *t, cl_int *m, int ns, int nw, int ng)
    {
        ...
        for (k = 0; k < ng; ++k)
        {
            for (j = 0; j < nj; ++j)
            {
                ...
                for (i = 0; i < n; ++i)
                    t[ii + i] =
                    (Ty)((int)(genrand_int32(gMTdata) & 0x7fffffff) % ns + 1);
            }
        ...
        for (k = 0; k < ng; ++k)
        {
            ...
            for (j = 0; j < nj; ++j)
            {
                ...
                for (i = 0; i < n; ++i)
                {
                    ...
                    {
                        tr =
                        i == 0 ? mx[ii] : (tr > mx[ii + i] ? tr : mx[ii + i]);
                    }
                    else
                    {
                        tr =
                        i == 0 ? mx[ii] : (tr > mx[ii + i] ? mx[ii + i] : tr);
                    }
                    ...
                    {
                        log_error(
                        "ERROR: sub_group_scan_inclusive_%s(%s) mismatch for "
                        "local id %d in sub group %d in group %d\n",
                        Which == 0 ? "add" : (Which == 1 ? "max" : "min"),
                        TypeName<Ty>::val(), i, j, k);
                        return -1;
        ...
        for (k = 0; k < ng; ++k)
        {
            ...
            for (j = 0; j < nj; ++j)
            {
                ...
                for (i = 0; i < n; ++i)
                {
                    ...
                    {
                        log_error(
                        "ERROR: sub_group_scan_exclusive_%s(%s) mismatch for "
                        "local id %d in sub group %d in group %d\n",
                        Which == 0 ? "add" : (Which == 1 ? "max" : "min"),
                        TypeName<Ty>::val(), i, j, k);
                        return -1;
    ...
    static void gen(Ty *x, Ty *t, cl_int *m, int ns, int nw, int ng)
    {
        ...
        for (k = 0; k < ng; ++k)
        {
            for (j = 0; j < nj; ++j)
            {
                ...
                n = ii + ns > nw ? nw - ii : ns;
                l =
                (int)(genrand_int32(gMTdata) & 0x7fffffff) % (d > n ? n : d);

                for (i = 0; i < n; ++i)
                    t[ii + i] = (Ty)(
                    (int)(genrand_int32(gMTdata) & 0x7fffffff) % 100 * 100 + l);
            }
