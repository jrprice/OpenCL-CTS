                        ...
                        cl_command_queue queue, int num_elements)
{
    ...
    {
        error =
        get_max_common_work_group_size(context, kernel, global[0], &local[0]);
    ...
    kernel, device, CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE_KHR, sizeof(local),
    ...
    log_info(
    "The CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE_KHR for the kernel is %d.\n",
    ...
    error = clGetKernelSubGroupInfoKHR_ptr(
    kernel, device, CL_KERNEL_SUB_GROUP_COUNT_FOR_NDRANGE_KHR, sizeof(local),
    ...
    out =
    clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(result), NULL, &error);
