                           ...
                           void **outResultBuffer)
{
    ...
    *outType = get_read_kernel_type(outFormat);
    ...
    channelSize * 4 * imageWidth * imageHeight * imageDepth, NULL, &error);
    ...
    error =
    get_max_common_3D_work_group_size(context, kernel, threads, localThreads);
    ...
    *outResultBuffer =
    (void *)(new char[channelSize * 4 * imageWidth * imageHeight * imageDepth]);
    error =
    clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0,
                        channelSize * 4 * imageWidth * imageHeight * imageDepth,
                           ...
                           GLenum glType, ExplicitType type, MTdata d)
{
    ...
    void *tmp =
    CreateGLTexture3D(width, height, depth, target, format, internalFormat,
    ...
    error = test_image_read(context, queue, target, glTexture, width, height,
                            depth, &clFormat, &actualType, (void **)&outBuffer);
    if (error != 0) return error;
    BufferOwningPtr<char> actualResults(outBuffer);

    log_info("- Read [%4d x %4d x %4d] : GL Texture : %s : %s : %s => CL Image "
             ": %s : %s \n",
             (int)width, (int)height, (int)depth, GetGLFormatName(format),
    ...
    if (actualType == kFloat)
        return validate_float_results(convertedInputs, actualResults, width,
                                      height, depth);
    else
        return validate_integer_results(convertedInputs, actualResults, width,
                                        height, depth,
             ...
             tgtIdx++)
        {
            ...
            for (i = 0; i < iter; i++)
            {
                if (test_image_format_read(
                    context, queue, sizes[i], sizes[i], sizes[i],
                    targets[tgtIdx], formats[fmtIdx].format,
