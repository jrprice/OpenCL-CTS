                           ...
                           ExplicitType *outType, void **outResultBuffer)
{
    ...
    error = clGetImageInfo(image, CL_IMAGE_FORMAT, sizeof(cl_image_format),
                           outFormat, NULL);
    test_error(error, "Unable to get CL image format");

    return CheckGLObjectInfo(image, CL_GL_OBJECT_TEXTURE2D, glTexture, glTarget,
                             0);
}
                           ...
                           cl_command_queue queue, int numElements)
{
    ...
    for (fmtIdx = 0; fmtIdx < sizeof(formats) / sizeof(formats[0]); fmtIdx++)
    {
             ...
             tgtIdx++)
        {
            size_t i;
            log_info(
            "Testing image texture object info test for %s : %s : %s : %s\n",
            ...
            for (i = 0; i < iter; i++)
            {
                ...
                {
                    log_error("ERROR: Image texture object info test failed "
                              "for %s : %s : %s : %s\n\n",
                              GetGLTargetName(targets[tgtIdx]),
                              GetGLFormatName(formats[fmtIdx].internal),
                              GetGLBaseFormatName(formats[fmtIdx].format),
            ...
            if (i == iter)
            {
                log_info("passed: Image texture object info test passed for %s "
                         ": %s : %s : %s\n\n",
                         GetGLTargetName(targets[tgtIdx]),
                         GetGLFormatName(formats[fmtIdx].internal),
                         GetGLBaseFormatName(formats[fmtIdx].format),
                             ...
                             cl_command_queue queue, int numElements)
{
    ...
    for (fmtIdx = 0; fmtIdx < sizeof(formats) / sizeof(formats[0]); fmtIdx++)
    {
             ...
             tgtIdx++)
        {
            size_t i;
            log_info(
            "Testing cube map object info test for %s : %s : %s : %s\n",
            GetGLTargetName(targets[tgtIdx]),
            GetGLFormatName(formats[fmtIdx].internal),
            GetGLBaseFormatName(formats[fmtIdx].format),
            GetGLTypeName(formats[fmtIdx].datatype));
            ...
            for (i = 0; i < iter; i++)
            {
                if (test_image_object_info(
                    ...
                    formats[fmtIdx].datatype, formats[fmtIdx].type, seed))
                {
                    log_error("ERROR: Cube map object info test failed for %s "
                              ": %s : %s : %s\n\n",
                              GetGLTargetName(targets[tgtIdx]),
                              GetGLFormatName(formats[fmtIdx].internal),
                              GetGLBaseFormatName(formats[fmtIdx].format),
            ...
            if (i == iter)
            {
                log_info("passed: Cube map object info test passed for %s : %s "
                         ": %s : %s\n\n",
                         GetGLTargetName(targets[tgtIdx]),
                         GetGLFormatName(formats[fmtIdx].internal),
                         GetGLBaseFormatName(formats[fmtIdx].format),
