...
static const char vertexshader[] = "uniform mat4 projMatrix;\n"
...
static const char fragmentshader[] = "varying   vec4 outColor;\n"
                                     "void main (void) {\n"
                           ...
                           cl_device_id rend_device)
{
    ...
    error = clGetDeviceInfo(device, CL_DEVICE_PLATFORM, sizeof(platform_id),
                            &platform_id, NULL);
    if (error)
    {
        return error;
    ...
    platform_id, "clCreateEventFromGLsyncKHR");
    if (clCreateEventFromGLsyncKHR_ptr == NULL)
    {
        ...
        clCreateEventFromGLsyncKHR_ptr =
        (clCreateEventFromGLsyncKHR_fn)clGetExtensionFunctionAddressForPlatform(
        platform_id, "clCreateEventFromGLsyncAPPLE");
        return -1;
    }
...
#else
    // Need platform specific way to query if current GL context was created
    // with 3.x profile
    ...
    framebufferSize, 128, GL_COLOR_ATTACHMENT0_EXT, GL_RGBA, GL_UNSIGNED_BYTE,
    ...
    streams[0] =
    (*clCreateFromGLBuffer_ptr)(context, CL_MEM_READ_WRITE, vtxBuffer, &error);
...
#else
    // Need platform specific code to set the current rendering device (OpenGL
    // target) to device with id read_vs so that next glReadPixels get submitted
    // to that device
    log_error("ERROR: not implemented\n\n");
    return -1;
#endif
    ...
    glFramebuffer, glRenderbuffer, GL_COLOR_ATTACHMENT0_EXT, GL_RGBA8_OES,
    GL_UNSIGNED_BYTE, GL_RGBA, GL_UNSIGNED_BYTE, kUChar, framebufferSize, 128));
