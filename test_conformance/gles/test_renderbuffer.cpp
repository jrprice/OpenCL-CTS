...
extern int test_cl_image_write(cl_context context, cl_command_queue queue,
                               cl_mem clImage, size_t imageWidth,
...
int test_renderbuffer_read_image(cl_context context, cl_command_queue queue,
                                 ...
                                 GLenum attachment, GLenum rbFormat,
                                 GLenum rbType, GLenum texFormat,
                                 GLenum texType, ExplicitType type, MTdata d)
{
    int error;
    ...
    width, height, attachment, rbFormat, rbType, texFormat, texType, type,
    ...
    log_info("- Read [%4d x %4d] : GL renderbuffer : %s : %s : %s => CL Image "
             ": %s : %s \n",
             width, height, GetGLFormatName(rbFormat),
             GetGLFormatName(rbFormat), GetGLTypeName(rbType),
    ...
    // validate.
    BufferOwningPtr<char> convertedInput(
    ...
    if (convertedInput)
    {
        if (actualType == kFloat)
            valid = validate_float_results(convertedInput, actualResults, width,
                                           height);
        else
            valid = validate_integer_results(
            convertedInput, actualResults, width, height,
                           ...
                           cl_command_queue queue, int numElements)
{
    ...
    struct
    {
        ...
        //{ GL_RGBA16F_QCOM, GL_HALF_FLOAT_OES,  GL_RGBA, GL_HALF_FLOAT_OES,
        //kHalf  },  // Half-float not supported by ReadPixels
             ...
             attIdx++)
        {
            ...
            for (i = 0; i < iter; i++)
            {
                ...
                if (test_renderbuffer_read_image(
                    ...
                    formats[fmtIdx].rbFormat, formats[fmtIdx].rbType,
                    formats[fmtIdx].texFormat, formats[fmtIdx].texType,
                                  ...
                                  GLenum texType, ExplicitType type, MTdata d)
{
    ...
    glFramebufferWrapper glFramebuffer;
    glRenderbufferWrapper glRenderbuffer;
    CreateGLRenderbuffer(width, height, attachment, rbFormat, rbType, texFormat,
                         texType, type, &glFramebuffer, &glRenderbuffer, &error,
                         d, false);
    if (error != 0)
    {
        // GL_RGBA_INTEGER_EXT doesn't exist in GLES2. No need to check for it.
    ...
    void *tmp =
    ReadGLRenderbuffer(glFramebuffer, glRenderbuffer, attachment, rbFormat,
                       rbType, texFormat, texType, type, width, height);
    ...
    // We have to convert our input buffer to the returned type, so we can
    // validate.
    BufferOwningPtr<char> convertedData(
    convert_to_expected(resultData, width * height, type, sourceType));

#ifdef GLES_DEBUG
    log_info("- start input data -- \n");
    DumpGLBuffer(GetGLTypeForExplicitType(sourceType), width, height,
                 sourceData);
    log_info("- end input data -- \n");
#endif
    ...
    if (convertedData)
    {
        if (sourceType == kFloat)
            valid =
            validate_float_results(sourceData, convertedData, width, height);
        else
            valid =
            validate_integer_results(sourceData, convertedData, width, height,
                            ...
                            cl_command_queue queue, int numElements)
{
    ...
    struct
    {
        ...
        { GL_RGBA8_OES, GL_UNSIGNED_BYTE, GL_RGBA, GL_UNSIGNED_BYTE, kUChar },
        //{ GL_RGBA16F_QCOM, GL_UNSIGNED_SHORT,  GL_RGBA, GL_UNSIGNED_SHORT,
        //kHalf  },  // Half float not supported by ReadPixels
        { GL_RGBA32F, GL_FLOAT, GL_RGBA, GL_FLOAT, kFloat },
        // XXX add others
    };
