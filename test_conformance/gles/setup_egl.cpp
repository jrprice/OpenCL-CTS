...
#define EGLERR() assert(eglGetError() == EGL_SUCCESS);
...
public:
    EGLGLEnvironment()
        : _platform(NULL), _display(EGL_NO_DISPLAY), _context(NULL),
    ...
    virtual int Init(int *argc, char **argv, int use_opengl_32)
    {
        EGLint ConfigAttribs[] = {
            ...
            EGL_ALPHA_SIZE, 8, EGL_DEPTH_SIZE, 16, EGL_SURFACE_TYPE,
            EGL_PBUFFER_BIT,
    ...
    virtual cl_context CreateCLContext(void)
    {
        cl_context_properties properties[] = { CL_CONTEXT_PLATFORM,
                                               ...
                                               CL_GL_CONTEXT_KHR,
                                               (cl_context_properties)_context,
                                               CL_EGL_DISPLAY_KHR,
                                               (cl_context_properties)_display,
                                               0 };
    ...
    virtual int SupportsCLGLInterop(cl_device_type device_type)
    {
        ...
        error = clGetDeviceIDs(_platform, device_type, MAX_DEVICES, devices,
                               &num_of_devices);
        if (error)
        {
        ...
        {
            error = clGetDeviceInfo(devices[i], CL_DEVICE_EXTENSIONS,
...
GLEnvironment *GLEnvironment::Instance(void) { return new EGLGLEnvironment(); }
