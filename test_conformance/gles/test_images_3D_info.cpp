                           ...
                           void **outResultBuffer)
{
    ...
    return CheckGLObjectInfo(streams[0], CL_GL_OBJECT_TEXTURE3D, glTexture,
                             glTarget, 0);
}
                                  ...
                                  ExplicitType type, MTdata d)
{
    ...
    CreateGLTexture3D(width, height, depth, target, format, internalFormat,
                      glType, type, &glTexture, &error, d, true);
    ...
    char *outBuffer;
    return test_image_read(context, queue, target, glTexture, width, height,
    ...
    for (fmtIdx = 0; fmtIdx < sizeof(formats) / sizeof(formats[0]); fmtIdx++)
    {
             ...
             tgtIdx++)
        {
            ...
            for (i = 0; i < iter; i++)
            {
                ...
                {
                    log_error(
                    "ERROR: Image info test failed for %s : %s : %s : %s\n\n",
                    GetGLTargetName(targets[tgtIdx]),
                    GetGLFormatName(formats[fmtIdx].internal),
                    GetGLBaseFormatName(formats[fmtIdx].format),
                    GetGLTypeName(formats[fmtIdx].datatype));
