...
#ifndef GL_ES_VERSION_2_0
test_definition test_list32[] = { TEST_FN_REDIRECT(fence_sync) };
#endif
...
int main(int argc, const char *argv[])
{
    ...
    { // for
        if (strcmp(argv[z], "-list") == 0)
        {
            ...
            log_info("Note: Any 3.2 test names must follow 2.1 test names on "
                     "the command line.");
        ...
        else if (!strcmp(argv[z], "CL_DEVICE_TYPE_CPU"))
        {
            printf("Requested device type is CL_DEVICE_TYPE_CPU\n");
            log_info(
            "Invalid CL device type. GL tests can only run on a GPU device.\n");
            return 0;
        }
    } // for
    ...
    if ((argc == 1) || (first_32_testname != 1))
    {
        ...
        for (size_t i = 0; i < numDevices; i++)
        {
            ...
            error = clGetDeviceInfo(deviceIDs[i], CL_DEVICE_PLATFORM,
            ...
            /* parseAndCallCommandLineTests considers every command line
           argument as a test name. This results in the test failing because of
           considering args such as 'CL_DEVICE_TYPE_GPU' as test names unless
        ...
        */
            ...
            error =
            parseAndCallCommandLineTests(argc_tmp, argv_tmp, deviceIDs[i],
                                         test_num, test_list, true, 0, 1024);
            if (error != 0) break;
        }

        // Clean-up.
        // We move this to a common cleanup step to make sure that things will
        // be released properly before the test exit
        goto cleanup;
    ...
    if ((argc == 1) || first_32_testname)
    {
        ...
        for (size_t i = 0; i < numDevices; i++)
        {
            ...
            log_info(
            "Cannot test OpenGL 3.2! This test was built for OpenGL ES 2.0\n");
            error = -1;
            goto cleanup;
#else
            // Note: don't use the entire harness, because we have a different
            ...
            argc_, argv_, deviceIDs[i], test_num32, test_list32, true, 0, 1024);
            if (error != 0) break;
#endif
        }

        // Converge on a common cleanup to make sure that things will be
        // released properly before the test exit
        goto cleanup;
    }
...
cleanup:
    ...
    // Always make sure that OpenCL context is released properly when the test
    // exit
    if (sCurrentContext)
    {
        clReleaseContext(sCurrentContext);
