...
const char *get_kernel_suffix(cl_image_format *format)
{
    switch (format->image_channel_data_type)
    {
        ...
        case CL_SNORM_INT8:
        case CL_SNORM_INT16:
        case CL_FLOAT: return "f";
        case CL_HALF_FLOAT: return "h";
        case CL_SIGNED_INT8:
...
int validate_integer_results(void *expectedResults, void *actualResults,
                             size_t width, size_t height, size_t typeSize)
{
    ...
    for (size_t z = 0; z < ((depth == 0) ? 1 : depth); z++)
    {
        for (size_t y = 0; y < height; y++)
        {
            for (size_t x = 0; x < width; x++)
            {
                ...
                if (err > 1.f
                    / 127.f) // Max expected range of error if we converted from
                             // an 8-bit integer to a normalized float
                {
                    if (depth == 0)
                        log_error(
                        "ERROR: Data sample %d,%d did not validate!\n", (int)x,
                      ...
                      GLint expected_cl_gl_mipmap_level)
{
        ...
        || object_type == CL_GL_OBJECT_TEXTURE3D)
    {
        ...
        {
            log_error("clGetGLTextureInfo did not return expected mipmap "
                      "level: expected %d, got %d.\n",
