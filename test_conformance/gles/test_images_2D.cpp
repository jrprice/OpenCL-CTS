...
"#pragma OPENCL EXTENSION cl_khr_fp16 : enable\n" /* added support for half
                                                     floats */
"__kernel void sample_test( read_only image2d_t source, sampler_t sampler, "
"__global %s4 *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
"    results[ tidY * get_image_width( source ) + tidX ] = read_image%s( "
"source, sampler, (int2)( tidX, tidY ) );\n"
"}\n";
...
int test_cl_image_read(cl_context context, cl_command_queue queue,
                       cl_mem clImage, size_t imageWidth, size_t imageHeight,
                       ...
                       void **outResultBuffer)
{
    ...
    *outType = get_read_kernel_type(outFormat);
    ...
    sprintf(kernelSource, imageReadKernelPattern,
            get_explicit_type_name(*outType), get_kernel_suffix(outFormat));
    ...
    clCreateBuffer(context, CL_MEM_READ_WRITE,
                   channelSize * 4 * imageWidth * imageHeight, NULL, &error);
    ...
    error =
    get_max_common_2D_work_group_size(context, kernel, threads, localThreads);
    ...
    *outResultBuffer = malloc(channelSize * 4 * imageWidth * imageHeight);
    error = clEnqueueReadBuffer(queue, outStream, CL_TRUE, 0,
                                channelSize * 4 * imageWidth * imageHeight,
...
static int test_image_read(cl_context context, cl_command_queue queue,
                           ...
                           size_t imageHeight, cl_image_format *outFormat,
                           ExplicitType *outType, void **outResultBuffer)
{
    ...
    context, CL_MEM_READ_ONLY, glTarget, 0, glTexture, &error);
    if (error != CL_SUCCESS)
    {
        ...
        log_error(
        "    Supplied GL texture was baseformat %s and internalformat %s\n",
        GetGLBaseFormatName(fmt), GetGLFormatName(fmt));
#endif
        return error;
    }

    return test_cl_image_read(context, queue, image, imageWidth, imageHeight,
                              outFormat, outType, outResultBuffer);
}

int test_image_format_read(cl_context context, cl_command_queue queue,
                           size_t width, size_t height, GLenum target,
                           GLenum format, GLenum internalFormat, GLenum glType,
                           ExplicitType type, MTdata d)
{
    ...
    void *tmp = CreateGLTexture2D(width, height, target, format, internalFormat,
                                  glType, type, &glTexture, &error, true, d);
    ...
    char *outBuffer;
    error = test_image_read(context, queue, target, glTexture, width, height,
                            &clFormat, &actualType, (void **)&outBuffer);
    if (error != 0) return error;
    BufferOwningPtr<char> actualResults(outBuffer);

    log_info(
    "- Read [%4d x %4d] : GL Texture : %s : %s : %s => CL Image : %s : %s \n",
    (int)width, (int)height, GetGLFormatName(format),
    ...
    convert_to_expected(inputBuffer, width * height, type, actualType));
    ...
    if (convertedInputs)
    {
        if (actualType == kFloat)
            valid = validate_float_results(convertedInputs, actualResults,
                                           width, height);
        else
            valid = validate_integer_results(
            convertedInputs, actualResults, width, height,
...
int test_images_read(cl_device_id device, cl_context context,
                     cl_command_queue queue, int numElements)
{
    ...
    {
        for (tgtIdx = 0; tgtIdx < sizeof(targets) / sizeof(targets[0]);
             tgtIdx++)
        {
            size_t i;
            ...
            for (i = 0; i < iter; i++)
            {
                ...
                if (test_image_format_read(
                    context, queue, width, height, targets[tgtIdx],
                    formats[fmtIdx].format, formats[fmtIdx].internal,
                    formats[fmtIdx].datatype, formats[fmtIdx].type, seed))
                {
                    log_error(
                    "ERROR: Image read test failed for %s : %s : %s : %s\n\n",
                    GetGLTargetName(targets[tgtIdx]),
                    GetGLFormatName(formats[fmtIdx].internal),
                    GetGLBaseFormatName(formats[fmtIdx].format),
                    GetGLTypeName(formats[fmtIdx].datatype));
            ...
            {
                log_info(
                "passed: Image read for GL format %s : %s : %s : %s\n\n",
                GetGLTargetName(targets[tgtIdx]),
                GetGLFormatName(formats[fmtIdx].internal),
                GetGLBaseFormatName(formats[fmtIdx].format),
                GetGLTypeName(formats[fmtIdx].datatype));
                          ...
                          cl_command_queue queue, int numElements)
{
    ...
    for (fmtIdx = 0; fmtIdx < sizeof(formats) / sizeof(formats[0]); fmtIdx++)
    {
             ...
             tgtIdx++)
        {
            ...
            log_info(
            "Testing image read cubemap for GL format  %s : %s : %s : %s\n\n",
            GetGLTargetName(targets[tgtIdx]),
            GetGLFormatName(formats[fmtIdx].internal),
            GetGLBaseFormatName(formats[fmtIdx].format),
            GetGLTypeName(formats[fmtIdx].datatype));
            ...
            {
                if (test_image_format_read(
                    context, queue, sizes[i], sizes[i], targets[tgtIdx],
                    formats[fmtIdx].format, formats[fmtIdx].internal,
                    formats[fmtIdx].datatype, formats[fmtIdx].type, seed))
                {
                    log_error("ERROR: Image read cubemap test failed for %s : "
                              "%s : %s : %s\n\n",
                              GetGLTargetName(targets[tgtIdx]),
                              GetGLFormatName(formats[fmtIdx].internal),
                              GetGLBaseFormatName(formats[fmtIdx].format),
            ...
            if (i == iter)
            {
                log_info("passed: Image read cubemap for GL format  %s : %s : "
                         "%s : %s\n\n",
                         GetGLTargetName(targets[tgtIdx]),
                         GetGLFormatName(formats[fmtIdx].internal),
                         GetGLBaseFormatName(formats[fmtIdx].format),
                ...
                break; // Skip other cube map targets; they're unlikely to pass
                       // either
        }
    }
                        ...
                        void **outSourceBuffer, MTdata d)
{
    ...
    *outType = get_write_kernel_type(outFormat);
    ...
    sprintf(kernelSource, imageWriteKernelPattern,
            get_explicit_type_name(*outType), get_explicit_type_name(*outType),
            suffix, convert);
    ...
    *outSourceBuffer =
    CreateRandomData(*outType, imageWidth * imageHeight * 4, d);
    ...
    clEnqueueReadImage(queue, clImage, 1, origin, region, 0, 0, cldata, 0, 0,
                       0);
    log_info("- start CL Image Data -- \n");
    DumpGLBuffer(GetGLTypeForExplicitType(*outType), imageWidth, imageHeight,
                 cldata);
...
int test_image_write(cl_context context, cl_command_queue queue,
                     GLenum glTarget, GLuint glTexture, size_t imageWidth,
                     size_t imageHeight, cl_image_format *outFormat,
                     ExplicitType *outType, void **outSourceBuffer, MTdata d)
{
    ...
    return test_cl_image_write(context, queue, image, imageWidth, imageHeight,
                               outFormat, outType, outSourceBuffer, d);
}
                            ...
                            ExplicitType type, MTdata d)
{
    ...
    error =
    test_image_write(context, queue, target, glTexture, width, height,
    ...
    log_info(
    "- Write [%4d x %4d] : GL Texture : %s : %s : %s => CL Image : %s : %s \n",
    (int)width, (int)height, GetGLFormatName(format),
    GetGLFormatName(internalFormat), GetGLTypeName(glType),
    GetChannelOrderName(clFormat.image_channel_order),
    GetChannelTypeName(clFormat.image_channel_data_type));
    ...
    BufferOwningPtr<char> glResults(ReadGLTexture(target, glTexture, format,
                                                  internalFormat, glType,
                                                  readType, width, height));

    // We have to convert our input buffer to the returned type, so we can
    // validate.
    BufferOwningPtr<char> convertedGLResults(
    convert_to_expected(glResults, width * height, readType, sourceType));
    ...
    if (convertedGLResults)
    {
        if (sourceType == kFloat)
            valid = validate_float_results(actualSource, convertedGLResults,
                                           width, height);
        else
            valid =
            validate_integer_results(actualSource, convertedGLResults, width,
                                     height, get_explicit_type_size(readType));
    }
    ...
    for (fmtIdx = 0; fmtIdx < sizeof(formats) / sizeof(formats[0]); fmtIdx++)
    {
             ...
             tgtIdx++)
        {
            ...
            for (i = 0; i < iter; i++)
            {
                ...
                if (targets[tgtIdx] == GL_TEXTURE_2D) width = height;
             ...
             tgtIdx++)
        {
            ...
            for (i = 0; i < iter; i++)
            {
                if (test_image_format_write(
                    ...
                    formats[fmtIdx].datatype, formats[fmtIdx].type, seed))
                {
                    log_error("ERROR: Image write cubemap test failed for %s : "
                              "%s : %s : %s\n\n",
                              GetGLTargetName(targets[tgtIdx]),
                              GetGLFormatName(formats[fmtIdx].internal),
                              GetGLBaseFormatName(formats[fmtIdx].format),
            ...
            if (i == iter)
            {
                log_info("passed: Image write cubemap for GL format  %s : %s : "
                         "%s : %s\n\n",
                         GetGLTargetName(targets[tgtIdx]),
                         GetGLFormatName(formats[fmtIdx].internal),
                         GetGLBaseFormatName(formats[fmtIdx].format),
