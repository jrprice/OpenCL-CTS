...
extern "C" float verify_smoothstep(float *edge0, float *edge1, float *x,
                                   float *outptr, int n, int veclen)
{
                     ...
                     cl_command_queue queue, int n_elems)
{
    ...
    create_single_kernel_helper(context, &program[1], &kernel[1], 1,
                                &smoothstep2_kernel_code, "test_smoothstep2f");
    if (err) return -1;
    err =
    create_single_kernel_helper(context, &program[2], &kernel[2], 1,
                                &smoothstep4_kernel_code, "test_smoothstep4f");
    if (err) return -1;
