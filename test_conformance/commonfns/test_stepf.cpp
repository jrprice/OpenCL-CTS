...
"__kernel void test_step(__global float *srcA, __global float *srcB, __global "
"float *dst)\n"
"{\n"
...
"__kernel void test_step3(__global float *srcA, __global float *srcB, __global "
"float *dst)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
               ...
               int n_elems)
{
    ...
    for (i = 0; i < kTotalVecCount; i++)
    {
        ...
        switch (i)
        {
            case 0:
                err =
                verify_step(input_ptr[0], input_ptr[1], output_ptr, n_elems, 1);
                if (err)
                      ...
                      cl_command_queue queue, int n_elems)
{
    ...
    create_single_kernel_helper(context, &program[5], &kernel[5], 1,
                                &step3_kernel_code_double, "test_step3_double");
    if (err) return -1;
