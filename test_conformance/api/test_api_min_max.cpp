                                 ...
                                 cl_command_queue queue, int num_elements)
{
    ...
    clGetDeviceInfo(deviceID, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                    sizeof(deviceMaxThreadSize), &deviceMaxThreadSize, NULL);
                                 ...
                                 cl_command_queue queue, int num_elements)
{
    ...
    error =
    clGetDeviceInfo(deviceID, CL_DEVICE_ADDRESS_BITS, sizeof(deviceAddressSize),
                    &deviceAddressSize, NULL);
    ...
    programSrc =
    (char *)malloc(strlen(sample_read_image_kernel_pattern[0])
    ...
    context, &program, &kernel, 1, (const char **)&programSrc, "sample_test");
    test_error(error, "Failed to create the program and kernel.");
    free(programSrc);

    result = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                                  ...
                                  cl_command_queue queue, int num_elements)
{
    ...
    if (maxWriteImages == 0)
    {
        ...
        "image arguments unsupported). Skipping test (implicitly passes). This "
        "is only valid if the number of image formats is also 0.\n");
        return 0;
    }
    ...
    programSrc =
    (char *)malloc(strlen(sample_write_image_kernel_pattern[0])
    ...
    {
        streams[i] =
        create_image_2d(context, CL_MEM_READ_WRITE, &image_format_desc, 16, 16,
                        0, NULL, &error);
        test_error(error, "Unable to allocate test image");
    }
                                ...
                                cl_command_queue queue, int num_elements)
{
    ...
    requiredAllocSize =
    ((memSize / 4) > (1024 * 1024 * 1024)) ? 1024 * 1024 * 1024 : memSize / 4;
    ...
    {
        log_error(
        "ERROR: Reported max allocation size is less than required of total "
        "memory! (%llu or %lluMB, from a total mem size of %lluMB)\n",
    ...
    {
        log_info(
        "Global memory size is less than max allocation size, using that.\n");
                                ...
                                cl_command_queue queue, int num_elements)
{
    ...
    error = clGetDeviceInfo(deviceID, CL_DEVICE_VERSION, sizeof(buffer), buffer,
                            &length);
    ...
    {
        log_error("ERROR: device version string does not match required "
                  "format! (returned: %s)\n",
                  (char *)buffer);
        return -1;
    }
                                  ...
                                  cl_command_queue queue, int num_elements)
{
    ...
    {
        log_error(
        "ERROR: Reported max image array size is less than required! (%d)\n",
        (int)maxDimension);
        return -1;
    }
    ...
    /* Try to allocate a very big image */
    streams[0] =
    create_image_2d_array(context, CL_MEM_READ_ONLY, &image_format_desc, 1, 1,
                                   ...
                                   cl_command_queue queue, int num_elements)
{
    ...
    {
        log_error(
        "ERROR: Reported max image buffer size is less than required! (%d)\n",
    ...
    if ((streams[0] == NULL) || (error != CL_SUCCESS))
    {
        print_error(
        error,
        "1D Image from buffer creation failed for maximum image buffer size");
        return -1;
    }
                                ...
                                cl_command_queue queue, int num_elements)
{
    ...
    while (numberOfIntParametersToTry > 0)
    {
        ...
        {
            log_info(
            "Trying a kernel with %ld int arguments (%ld bytes) and one cl_mem "
            "(%ld bytes) for %ld bytes total.\n",
        ...
        if (gHasLong)
        {
            ...
            {
                log_error(
                "Expected result (%lld) does not equal actual result (%lld).\n",
        ...
        else
        {
            ...
            {
                log_error(
                "Expected result (%lld) does not equal actual result (%d).\n",
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    clMemWrapper stream = clCreateBuffer(
    context, (cl_mem_flags)(CL_MEM_READ_WRITE), 16, NULL, &error);
    ...
    error = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, globalDim, localDim,
                                   0, NULL, &event);
    test_error(
    error, "clEnqueueNDRangeKernel failed with maximum number of samplers.");
                                      ...
                                      cl_command_queue queue, int num_elements)
{
    ...
    while (!allocPassed && currentSize >= maxSize / PASSING_FRACTION)
    {
        ...
        {
            log_info(
            "Kernel enqueue failed at size %lld, trying at a reduced size.\n",
            currentSize);
    ...
    if (allocPassed)
    {
        ...
        {
            log_error(
            "Failed to allocate at least 1/8 of the reported constant size.\n");
            return -1;
        }
                               ...
                               cl_command_queue queue, int num_elements)
{
    ...
    log_info(
    "Reported max constant arg count of %d and max constant buffer size of %d. "
    "Test will attempt to allocate half of that, or %d buffers of size %d.\n",
                              ...
                              cl_command_queue queue, int num_elements)
{
    ...
    error =
    clGetDeviceInfo(deviceID, CL_DEVICE_ADDRESS_BITS, sizeof(value), &value, 0);
                                ...
                                cl_command_queue queue, int num_elements)
{
    ...
    clGetKernelWorkGroupInfo(kernel, deviceID, CL_KERNEL_LOCAL_MEM_SIZE,
                             sizeof(kernelLocalUsage), &kernelLocalUsage, NULL);
    test_error(error,
    ...
    /* Create some I/O streams */
    size_t sizeToAllocate =
    ((size_t)(maxSize - kernelLocalUsage) / sizeof(cl_int)) * sizeof(cl_int);
    size_t numberOfInts = sizeToAllocate / sizeof(cl_int);
    ...
    streams[0] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
                                sizeToAllocate, localData, &error);
    test_error(error, "Creating test array failed");
    streams[1] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                                sizeToAllocate, NULL, &error);
    test_error(error, "Creating test array failed");
                                ...
                                cl_command_queue queue, int num_elements)
{
    ...
    {
        log_error("ERROR: Initial part of device version string does not match "
                  "required format! (returned: %s)\n",
                  (char *)buffer);
        return -1;
    }
    ...
    {
        log_error(
        "ERROR: Version number must contain a decimal point! (returned: %s)\n",
        (char *)buffer);
        return -1;
    }
    char *p3 = p2 + 1;
    ...
    {
        log_error(
        "ERROR: A space must appear after the minor version! (returned: %s)\n",
        (char *)buffer);
        return -1;
    }
    *p2 = ' '; // Put in a space for atoi below.
    ...
    if (getenv("OPENCL_1_0_DEVICE"))
    {
        minor_revision = 0;
        log_info(
        "WARNING: This test was run with OPENCL_1_0_DEVICE defined!  This is "
        "not a OpenCL 1.1 or OpenCL 1.2 compatible device!!!\n");
    }
    else if (getenv("OPENCL_1_1_DEVICE"))
    {
        minor_revision = 1;
        log_info("WARNING: This test was run with OPENCL_1_1_DEVICE defined!  "
                 "This is not a OpenCL 1.2 compatible device!!!\n");
    }
    if (major * 10 + minor < 10 + minor_revision)
    {
        log_error("ERROR: OpenCL device version returned is less than 1.%d! "
                  "(Returned: %s)\n",
    ...
    const char *requiredExtensions[] = { "cl_khr_global_int32_base_atomics",
    ...
    if (major * 10 + minor >= 11)
    {
        ...
        if (major >= 2)
        {
            log_info("Checking for required extensions for OpenCL 2.0 and "
                     "later devices...\n");

            // Check if double precision is supported, if it is, then check the
            // extension "cl_khr_fp64"
            cl_device_fp_config value;
            int numRequiredExtension20 =
            sizeof(requiredExtensions20) / sizeof(char *);

            error = clGetDeviceInfo(deviceID, CL_DEVICE_DOUBLE_FP_CONFIG,
                                    sizeof(value), &value, 0);
            test_error(error, "Unable to get device double fp config");

            // if double precision is not supported, then don't check extension
            // "cl_khr_fp64"
            ...
            for (i = 0; i < numRequiredExtension20; i++)
            {
                ...
                {
                    log_error("ERROR: Required extension for 2.0 and greater "
                              "devices is not in extension string: %s\n",
        ...
        log_info(
        "WARNING: skipping required extension test -- OpenCL 1.0 device.\n");
                                  ...
                                  cl_command_queue queue, int num_elements)
{
    ...
    *p2 = ' '; // Put in a space for atoi below.
    ...
    if (major * 10 + minor < 10 + minor_revision)
    {
        ...
        {
            log_info(
            "WARNING: This test was run with OPENCL_1_0_DEVICE defined!  This "
            "is not a OpenCL 1.1 or OpenCL 1.2 compatible device!!!\n");
        }
        else if (getenv("OPENCL_1_1_DEVICE"))
        {
            log_info(
            "WARNING: This test was run with OPENCL_1_1_DEVICE defined!  This "
            "is not a OpenCL 1.2 compatible device!!!\n");
        }
        else
        {
