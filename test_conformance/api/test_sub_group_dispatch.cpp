...
size_t flatten_ndrange(size_t *ndrange, size_t dim)
{
    switch (dim)
    {
        case 1: return *ndrange;
        case 2: return ndrange[0] * ndrange[1];
        case 3: return ndrange[0] * ndrange[1] * ndrange[2];
        default: log_error("ERROR: bad ndrange value"); return 0;
    }
}

cl_int get_sub_group_num(cl_command_queue queue, cl_kernel kernel,
                         clMemWrapper &out, size_t &size, size_t local_size,
                         size_t dim)
{
    ...
    error += clEnqueueNDRangeKernel(queue, kernel, dim, NULL, ndrange, ndrange,
                                    0, NULL, NULL);
    error +=
    clEnqueueReadBuffer(queue, out, CL_TRUE, 0, 4, &size, 0, NULL, NULL);
    return error;
}

int test_sub_group_dispatch(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    ...
    "subgroup_dispatch_kernel", "-cl-std=CL2.0");
    if (error != 0) return error;

    out = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
    ...
    kernel, deviceID, CL_KERNEL_MAX_SUB_GROUP_SIZE_FOR_NDRANGE,
    ...
    error = clGetKernelSubGroupInfo(
    kernel, deviceID, CL_KERNEL_SUB_GROUP_COUNT_FOR_NDRANGE, sizeof(max_local),
    &max_local, sizeof(kernel_subgroup_count), (void *)&kernel_subgroup_count,
    &realSize);
    test_error(
    error,
    "clGetKernelSubGroupInfo failed for CL_KERNEL_SUB_GROUP_COUNT_FOR_NDRANGE");
    log_info(
    ...
    for (size_t i = kernel_subgroup_count; i > 0; --i)
    {
        ...
        error = clGetKernelSubGroupInfo(
        kernel, deviceID, CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT, sizeof(i),
        ...
        {
            log_error(
            "ERROR: Incorrect value returned for "
            "CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT! (Expected %d, got %d)\n",
            (int)expect_size, (int)ret_ndrange1d);
            return -1;
        }

        error = get_sub_group_num(queue, kernel, out, kernel_ret_size,
                                  ret_ndrange1d, 1);
        test_error(error, "Failed to query number of subgroups from kernel");
        if (i != kernel_ret_size)
        {
            log_error(
            "ERROR: Mismatch between requested number of subgroups and what "
            "get_num_sub_groups() in kernel returned! (Expected %d, got %d)\n",
        ...
        if (ret_ndrange2d_flattened != expect_size || ret_ndrange2d[1] != 1)
        {
            log_error(
            "ERROR: Incorrect value returned for "
            "CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT! (Expected %d, got %d)\n",
        ...
        if (ret_ndrange3d_flattened != expect_size || ret_ndrange3d[1] != 1
            || ret_ndrange3d[2] != 1)
        {
            log_error(
            "ERROR: Incorrect value returned for "
    ...
    error = clGetKernelSubGroupInfo(
    kernel, deviceID, CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT, sizeof(size_t),
    &large_sg_size, sizeof(ret_ndrange1d), &ret_ndrange1d, &realSize);
    test_error(error,
               "clGetKernelSubGroupInfo failed for "
               "CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT");
    if (ret_ndrange1d != 0)
    ...
    error = clGetKernelSubGroupInfo(
    kernel, deviceID, CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT, sizeof(size_t),
    &large_sg_size, sizeof(ret_ndrange2d), ret_ndrange2d, &realSize);
    test_error(error,
               "clGetKernelSubGroupInfo failed for "
               "CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT");
    if (ret_ndrange2d[0] != 0 || ret_ndrange2d[1] != 0)
    {
        log_error("ERROR: Incorrect value returned for "
                  "CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT!");
        return -1;
    }
    ...
    error = clGetKernelSubGroupInfo(
    kernel, deviceID, CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT, sizeof(size_t),
    &large_sg_size, sizeof(ret_ndrange3d), ret_ndrange3d, &realSize);
    test_error(error,
               "clGetKernelSubGroupInfo failed for "
               "CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT");
    if (ret_ndrange3d[0] != 0 || ret_ndrange3d[1] != 0 || ret_ndrange3d[2] != 0)
    {
        log_error("ERROR: Incorrect value returned for "
                  "CL_KERNEL_LOCAL_SIZE_FOR_SUB_GROUP_COUNT!");
        return -1;
