...
int test_get_kernel_info(cl_device_id deviceID, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    ...
    if (create_single_kernel_helper(context, &program, &kernel, 1,
                                    sample_single_test_kernel, "sample_test")
        != 0)
    {
        return -1;
    }
    ...
    error =
    clGetKernelInfo(kernel, CL_KERNEL_FUNCTION_NAME, 0, NULL, &paramSize);
    test_error(error, "Unable to get kernel function name param size");
    if (paramSize != strlen("sample_test") + 1)
    {
        log_error("ERROR: Kernel function name param returns invalid size "
                  "(expected %d, got %d)\n",
    ...
    {
        log_error("ERROR: Kernel function name returned invalid value "
                  "(expected sample_test, got %s)\n",
                  (char *)name);
        return -1;
    }
    ...
    if (numArgs != 2)
    {
        log_error("ERROR: Kernel arg count returned invalid value (expected "
                  "%d, got %d)\n",
                  2, numArgs);
        return -1;
    }
    ...
    error =
    clGetKernelInfo(kernel, CL_KERNEL_REFERENCE_COUNT, 0, NULL, &paramSize);
    test_error(error, "Unable to get kernel reference count param size");
    if (paramSize != sizeof(numInstances))
    {
        log_error("ERROR: Kernel reference count param returns invalid size "
                  "(expected %d, got %d)\n",
                  (int)sizeof(numInstances), (int)paramSize);
        return -1;
    }

    error = clGetKernelInfo(kernel, CL_KERNEL_REFERENCE_COUNT,
                            sizeof(numInstances), &numInstances, NULL);
    ...
    {
        log_error("ERROR: Kernel program param returns invalid size (expected "
                  "%d, got %d)\n",
                  (int)sizeof(testProgram), (int)paramSize);
        return -1;
    }

    error = clGetKernelInfo(kernel, CL_KERNEL_PROGRAM, sizeof(testProgram),
                            &testProgram, NULL);
    test_error(error, "Unable to get kernel program");
    if (testProgram != program)
    {
        log_error(
        "ERROR: Kernel program returned invalid value (expected %p, got %p)\n",
        program, testProgram);
        return -1;
    }

    error = clGetKernelInfo(kernel, CL_KERNEL_CONTEXT, sizeof(testContext),
                            &testContext, NULL);
    test_error(error, "Unable to get kernel context");
    if (testContext != context)
    {
        log_error(
        "ERROR: Kernel context returned invalid value (expected %p, got %p)\n",
        context, testContext);
        return -1;
    }
...
int test_execute_kernel_local_sizes(cl_device_id deviceID, cl_context context,
                                    cl_command_queue queue, int num_elements)
{
    int error;
    ...
    streams[0] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                                sizeof(cl_float) * 100, NULL, &error);
    ...
    for (i = 0; i < 100; i++)
        inputData[i] =
        get_random_float(-(float)0x7fffffff, (float)0x7fffffff, seed);

    error =
    clEnqueueWriteBuffer(queue, streams[0], CL_TRUE, 0, sizeof(cl_float) * 100,
                         (void *)inputData, 0, NULL, NULL);
    ...
    error =
    clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0, sizeof(cl_int) * 100,
                        (void *)outputData, 0, NULL, NULL);
    ...
    /* Try again */
    if (localThreads[0] > 1) localThreads[0] /= 2;
...
int test_set_kernel_arg_by_index(cl_device_id deviceID, cl_context context,
                                 cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
    ...
    error =
    clEnqueueWriteBuffer(queue, streams[0], CL_TRUE, 0, sizeof(cl_float) * 10,
                         (void *)inputData, 0, NULL, NULL);
    test_error(error, "Unable to set testing kernel data");
    ...
    error =
    clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0, sizeof(cl_int) * 10,
                        (void *)outputData, 0, NULL, NULL);
    test_error(error, "Unable to get result data");
...
int test_set_kernel_arg_struct(cl_device_id deviceID, cl_context context,
                               cl_command_queue queue, int num_elements)
{
    int error;
    cl_program program;
    ...
    /* Create a kernel to test with */
    if (create_single_kernel_helper(context, &program, &kernel, 1,
                                    sample_struct_test_kernel, "sample_test")
        != 0)
    {
        return -1;
    }
    ...
    clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
                   sizeof(cl_int) * 10, randomTestDataA, &error);
    test_error(error, "Creating test array failed");
    image_pair.streamB =
    clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
                   sizeof(cl_int) * 10, randomTestDataB, &error);
    test_error(error, "Creating test array failed");
    ...
    error =
    clEnqueueReadBuffer(queue, outStream, CL_TRUE, 0, sizeof(cl_int) * 10,
                        (void *)outputData, 0, NULL, NULL);
    test_error(error, "Unable to get result data");
...
int test_set_kernel_arg_constant(cl_device_id deviceID, cl_context context,
                                 cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
    ...
    {
        log_error("ERROR: Unable to test constant argument to kernel: max size "
                  "of constant buffer is reported as %d!\n",
                  (int)maxSize);
        return -1;
    }
    ...
    /* Create a kernel to test with */
    if (create_single_kernel_helper(context, &program, &kernel, 1,
                                    sample_const_test_kernel, "sample_test")
        != 0)
    {
        return -1;
    }
    ...
    {
        randomTestDataA[i] =
        (cl_int)genrand_int32(d) & 0xffffff; /* Make sure values are positive,
                                                just so we don't have to */
        randomTestDataB[i] = (cl_int)genrand_int32(d)
        & 0xffffff; /* deal with overflow on the verification */
    }
    free_mtdata(d);
    d = NULL;
    ...
    error =
    clEnqueueReadBuffer(queue, streams[2], CL_TRUE, 0, sizeof(cl_int) * 10,
                        (void *)outputData, 0, NULL, NULL);
    test_error(error, "Unable to get result data");
    ...
    for (i = 0; i < 10; i++)
    {
        if (outputData[i] != randomTestDataA[i] + randomTestDataB[i])
        {
            log_error("ERROR: Data sample %d did not verify! %d does not match "
                      "%d + %d (%d)\n",
                      i, outputData[i], randomTestDataA[i], randomTestDataB[i],
                      (randomTestDataA[i] + randomTestDataB[i]));
            return -1;
        }
    }
...
int test_set_kernel_arg_struct_array(cl_device_id deviceID, cl_context context,
                                     cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
    ...
    /* Create a kernel to test with */
    if (create_single_kernel_helper(context, &program, &kernel, 1,
                                    sample_struct_array_test_kernel,
                                    "sample_test")
        != 0)
    {
        return -1;
    ...
    streams[0] =
    clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
                   sizeof(image_pair_t) * 10, (void *)image_pair, &error);
    test_error(error, "Creating test array failed");
    streams[1] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                                sizeof(cl_int) * 10, NULL, &error);
    test_error(error, "Creating test array failed");
...
int test_create_kernels_in_program(cl_device_id deviceID, cl_context context,
                                   cl_command_queue queue, int num_elements)
{
    int error;
    cl_program program;
    ...
    error = create_single_kernel_helper(context, &program, NULL, 2,
                                        sample_two_kernel_program, NULL);
    ...
    if (kernelCount != 2)
    {
        log_error("ERROR: Returned kernel count from clCreateKernelsInProgram "
                  "is incorrect! (got %d, expected 2)\n",
                  kernelCount);
        return -1;
    }
...
int test_kernel_global_constant(cl_device_id deviceID, cl_context context,
                                cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
    ...
    /* Create a kernel to test with */
    if (create_single_kernel_helper(context, &program, &kernel, 1,
                                    sample_const_global_test_kernel,
                                    "sample_test")
        != 0)
    {
        return -1;
    ...
    {
        randomTestDataA[i] =
        (cl_int)genrand_int32(d) & 0xffff; /* Make sure values are positive and
                                              small, just so we don't have to */
    }
    free_mtdata(d);
    d = NULL;
    ...
    for (i = 0; i < 10; i++)
    {
        if (outputData[i] != randomTestDataA[i] + 1024)
        {
            log_error("ERROR: Data sample %d did not verify! %d does not match "
                      "%d + 1024 (%d)\n",
                      i, outputData[i], randomTestDataA[i],
                      (randomTestDataA[i] + 1024));
            return -1;
        }
    }
