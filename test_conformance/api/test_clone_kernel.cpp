...
int test_image_arg_shallow_clone(cl_device_id deviceID, cl_context context,
                                 ...
                                 void* pbufRes, clMemWrapper& bufOut)
{
    ...
    if (create_single_kernel_helper(context, &program, &kernel_read, 1,
    ...
    if (create_single_kernel_helper(context, &program, &kernel_write, 1,
    ...
    // read result back
    error =
    clEnqueueReadBuffer(queue, bufOut, CL_TRUE, 0, 128, pbufRes, 0, NULL, NULL);
...
int test_double_arg_clone(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements,
                          void* pbufRes, clMemWrapper& bufOut)
{
    int error = 0;
                      ...
                      cl_command_queue queue, int num_elements)
{
    ...
    // test image support
    error = clGetDeviceInfo(deviceID, CL_DEVICE_IMAGE_SUPPORT, sizeof(cl_bool),
                            &bimg, NULL);
    ...
    if (create_single_kernel_helper(context, &program, &kernel_buf_read, 1,
    ...
    if (create_single_kernel_helper(context, &program, &kernel_buf_write, 1,
                                    clone_kernel_test_kernel,
                                    "buf_write_kernel")
        != 0)
    {
    ...
    clMemWrapper pipe =
    clCreatePipe(context, CL_MEM_HOST_NO_ACCESS, sizeof(int), 16, NULL, &error);
    ...
    if (((int*)pbufRes)[0] != intarg)
    {
        test_error(
        error,
        "clCloneKernel test failed. Failed to clone integer type argument.");
        return -1;
    }
    ...
    if (((int*)pbufRes)[1] != sa.i)
    {
        test_error(
        error,
        "clCloneKernel test failed. Failed to clone structure type argument.");
        return -1;
    }
    ...
    {
        test_error(
        error, "clCloneKernel test failed.  Failed to clone cl_mem argument.");
        return -1;
    }
