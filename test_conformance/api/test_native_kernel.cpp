...
static void CL_CALLBACK test_native_kernel_fn(void *userData)
{
    ...
    for (cl_int i = 0; i < args->count; i++) args->dest[i] = args->source[i];
}

int test_native_kernel(cl_device_id device, cl_context context,
                       cl_command_queue queue, int n_elems)
{
    ...
    streams[0] = clCreateBuffer(context, CL_MEM_COPY_HOST_PTR,
                                n_elems * sizeof(cl_int), inBuffer, &error);
    test_error(error, "Unable to create I/O stream");
    streams[1] =
    clCreateBuffer(context, 0, n_elems * sizeof(cl_int), NULL, &error);
    test_error(error, "Unable to create I/O stream");
    ...
    queue, test_native_kernel_fn, &args, sizeof(args), 2, &streams[0],
    (const void **)memLocs, 0, NULL, &finishEvent);
    ...
    error =
    clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0, n_elems * sizeof(cl_int),
