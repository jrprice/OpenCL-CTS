...
int test_get_platform_info(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    ...
    error = clGetPlatformInfo(platform, CL_PLATFORM_PROFILE, sizeof(buffer),
                              buffer, &length);
    test_error(error, "Unable to get platform profile string");

    log_info("Returned CL_PLATFORM_PROFILE %s.\n", buffer);

    if (strcmp(buffer, "FULL_PROFILE") != 0
        && strcmp(buffer, "EMBEDDED_PROFILE") != 0)
    {
        log_error("ERROR: Returned platform profile string is not a valid "
                  "string by OpenCL 1.2! (Returned: %s)\n",
                  buffer);
        return -1;
    }
    if (strlen(buffer) + 1 != length)
    {
        log_error("ERROR: Returned length of profile string is incorrect "
                  "(actual length: %d, returned length: %d)\n",
    ...
    error = clGetPlatformInfo(platform, CL_PLATFORM_VERSION, sizeof(buffer),
                              buffer, &length);
    ...
    {
        log_error("ERROR: Initial part of platform version string does not "
                  "match required format! (returned: %s)\n",
                  (char *)buffer);
        return -1;
    }
    ...
    if (*p2 != '.')
    {
        log_error("ERROR: Numeric part of platform version string does not "
                  "match required format! (returned: %s)\n",
                  (char *)buffer);
        return -1;
    }
    ...
    {
        log_error(
        "ERROR: space expected after minor version number! (returned: %s)\n",
        (char *)buffer);
        return -1;
    }
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    clCreateSamplerWithProperties(context, properties, &error);
    ...
    {
        log_error("ERROR: Returned size of sampler refcount does not validate! "
                  "(expected %d, got %d)\n",
    ...
    cl_context otherCtx;
    error = clGetSamplerInfo(sampler, CL_SAMPLER_CONTEXT, sizeof(otherCtx),
                             &otherCtx, &size);
    test_error(error, "Unable to get sampler context");
    if (otherCtx != context)
    {
        log_error(
        "ERROR: Sampler context does not validate! (expected %p, got %p)\n",
        context, otherCtx);
        return -1;
    }
    if (size != sizeof(otherCtx))
    {
        log_error("ERROR: Returned size of sampler context does not validate! "
                  "(expected %d, got %d)\n",
    ...
    error = clGetSamplerInfo(sampler, CL_SAMPLER_ADDRESSING_MODE, sizeof(mode),
                             &mode, &size);
    test_error(error, "Unable to get sampler addressing mode");
    if (mode != CL_ADDRESS_CLAMP)
    {
        log_error("ERROR: Sampler addressing mode does not validate! (expected "
                  "%d, got %d)\n",
    ...
    if (size != sizeof(mode))
    {
        log_error("ERROR: Returned size of sampler addressing mode does not "
    ...
    error = clGetSamplerInfo(sampler, CL_SAMPLER_FILTER_MODE, sizeof(fmode),
                             &fmode, &size);
    test_error(error, "Unable to get sampler filter mode");
    if (fmode != CL_FILTER_LINEAR)
    {
        log_error(
        "ERROR: Sampler filter mode does not validate! (expected %d, got %d)\n",
        (int)CL_FILTER_LINEAR, (int)fmode);
        return -1;
    }
    if (size != sizeof(fmode))
    {
        log_error("ERROR: Returned size of sampler filter mode does not "
                  "validate! (expected %d, got %d)\n",
    ...
    cl_int norm;
    error = clGetSamplerInfo(sampler, CL_SAMPLER_NORMALIZED_COORDS,
                             sizeof(norm), &norm, &size);
    test_error(error, "Unable to get sampler normalized flag");
    if (norm != CL_TRUE)
    {
        log_error("ERROR: Sampler normalized flag does not validate! (expected "
                  "%d, got %d)\n",
    ...
    if (size != sizeof(norm))
    {
        log_error("ERROR: Returned size of sampler normalized flag does not "
                  "validate! (expected %d, got %d)\n",
...
#define TEST_COMMAND_QUEUE_PARAM(queue, paramName, val, expected, name, type,  \
    ...
    if (size != sizeof(val))                                                   \
    {                                                                          \
        log_error("ERROR: Returned size of command queue " name                \
                                ...
                                cl_command_queue ignoreQueue, int num_elements)
{
    ...
    clCommandQueueWrapper queue = clCreateCommandQueueWithProperties(
    context, deviceID, &queue_props[0], &error);
    ...
    if (size != sizeof(refCount))
    {
        log_error("ERROR: Returned size of command queue reference count does "
    ...
    cl_context otherCtx;
    TEST_COMMAND_QUEUE_PARAM(queue, CL_QUEUE_CONTEXT, otherCtx, context,
                             "context", "%p", cl_context)

    cl_device_id otherDevice;
    error = clGetCommandQueueInfo(queue, CL_QUEUE_DEVICE, sizeof(otherDevice),
                                  &otherDevice, &size);
    test_error(error, "clGetCommandQueue failed.");

    if (size != sizeof(cl_device_id))
    {
        log_error(" ERROR: Returned size of command queue CL_QUEUE_DEVICE does "
                  "not validate! (expected %d, got %d)\n",
    ...
    error = clGetDeviceInfo(otherDevice, CL_DEVICE_VENDOR_ID,
                            sizeof(otherDevice_vid), &otherDevice_vid, NULL);
    ...
    {
        log_error("ERROR: Incorrect device returned for queue! (Expected "
    ...
    TEST_COMMAND_QUEUE_PARAM(queue, CL_QUEUE_PROPERTIES, props,
                             (unsigned int)(device_props), "properties", "%d",
                             unsigned int)

    return 0;
}

int test_get_context_info(cl_device_id deviceID, cl_context context,
                          cl_command_queue ignoreQueue, int num_elements)
{
    int error;
    size_t size;
...
void CL_CALLBACK mem_obj_destructor_callback(cl_mem, void *data) { free(data); }
...
#define TEST_DEVICE_PARAM(device, paramName, val, name, type, cast)            \
    ...
    if (size != sizeof(val))                                                   \
    {                                                                          \
        log_error("ERROR: Returned size of device " name                       \
                  " does not validate! (expected %d, got %d)\n",               \
                  (int)sizeof(val), (int)size);                                \
        return -1;                                                             \
    }                                                                          \
...
int test_get_device_info(cl_device_id deviceID, cl_context context,
                         cl_command_queue ignoreQueue, int num_elements)
{
    int error;
    size_t size;
    ...
    cl_uint vendorID;
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_VENDOR_ID, vendorID, "vendor ID",
                      "0x%08x", int)

    char extensions[10240];
    error = clGetDeviceInfo(deviceID, CL_DEVICE_EXTENSIONS, sizeof(extensions),
                            &extensions, &size);
    test_error(error, "Unable to get device extensions");
    if (size != strlen(extensions) + 1)
    {
        log_error("ERROR: Returned size of device extensions does not "
                  "validate! (expected %d, got %d)\n",
    ...
    cl_uint preferred;
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_PREFERRED_VECTOR_WIDTH_CHAR,
                      preferred, "preferred vector char width", "%d", int)
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_PREFERRED_VECTOR_WIDTH_SHORT,
                      preferred, "preferred vector short width", "%d", int)
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_PREFERRED_VECTOR_WIDTH_INT, preferred,
                      "preferred vector int width", "%d", int)
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_PREFERRED_VECTOR_WIDTH_LONG,
                      preferred, "preferred vector long width", "%d", int)
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT,
                      preferred, "preferred vector float width", "%d", int)
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_PREFERRED_VECTOR_WIDTH_DOUBLE,
                      preferred, "preferred vector double width", "%d", int)

    // Note that even if cl_khr_fp64, the preferred width for double can be
    // non-zero.  For example, vendors extensions can support double but may not
    // support cl_khr_fp64, which implies math library support.
    ...
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE,
                      maxDataAlign, "min data type alignment", "%d bytes", int)

    cl_device_mem_cache_type cacheType;
    error = clGetDeviceInfo(deviceID, CL_DEVICE_GLOBAL_MEM_CACHE_TYPE,
                            sizeof(cacheType), &cacheType, &size);
    test_error(error, "Unable to get device global mem cache type");
    if (size != sizeof(cacheType))
    {
        log_error("ERROR: Returned size of device global mem cache type does "
                  "not validate! (expected %d, got %d)\n",
                  (int)sizeof(cacheType), (int)size);
        return -1;
    }
    const char *cacheTypeName = (cacheType == CL_NONE)
    ? "CL_NONE"
    : (cacheType == CL_READ_ONLY_CACHE)
    ? "CL_READ_ONLY_CACHE"
    : (cacheType == CL_READ_WRITE_CACHE) ? "CL_READ_WRITE_CACHE" : "<unknown>";
    log_info("\tReported device global mem cache type: %s \n", cacheTypeName);

    cl_uint cachelineSize;
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE,
                      cachelineSize, "global mem cacheline size", "%d bytes",
                      int)

    cl_ulong cacheSize;
    TEST_DEVICE_PARAM_MEM(deviceID, CL_DEVICE_GLOBAL_MEM_CACHE_SIZE, cacheSize,
                          "global mem cache size", "%d KB", 1024)

    cl_ulong memSize;
    TEST_DEVICE_PARAM_MEM(deviceID, CL_DEVICE_GLOBAL_MEM_SIZE, memSize,
                          "global mem size", "%d MB", (1024 * 1024))

    cl_device_local_mem_type localMemType;
    error = clGetDeviceInfo(deviceID, CL_DEVICE_LOCAL_MEM_TYPE,
                            sizeof(localMemType), &localMemType, &size);
    test_error(error, "Unable to get device local mem type");
    if (size != sizeof(cacheType))
    {
        log_error("ERROR: Returned size of device local mem type does not "
                  "validate! (expected %d, got %d)\n",
                  (int)sizeof(localMemType), (int)size);
        return -1;
    }
    const char *localMemTypeName = (localMemType == CL_LOCAL)
    ? "CL_LOCAL"
    : (cacheType == CL_GLOBAL) ? "CL_GLOBAL" : "<unknown>";
    log_info("\tReported device local mem type: %s \n", localMemTypeName);


    cl_bool errSupport;
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_ERROR_CORRECTION_SUPPORT, errSupport,
                      "error correction support", "%d", int)

    size_t timerResolution;
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_PROFILING_TIMER_RESOLUTION,
                      timerResolution, "profiling timer resolution",
                      "%ld nanoseconds", long)

    cl_bool endian;
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_ENDIAN_LITTLE, endian,
                      "little endian flag", "%d", int)

    cl_bool avail;
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_AVAILABLE, avail, "available flag",
                      "%d", int)

    cl_bool compilerAvail;
    TEST_DEVICE_PARAM(deviceID, CL_DEVICE_COMPILER_AVAILABLE, compilerAvail,
    ...
    {
        log_error("ERROR: Returned size of device profile does not validate! "
                  "(expected %d, got %d)\n",
                  (int)(strlen(profile) + 1), (int)size);
        return -1;
    }
    if (strcmp(profile, "FULL_PROFILE") != 0
        && strcmp(profile, "EMBEDDED_PROFILE") != 0)
    {
        log_error("ERROR: Returned profile of device not FULL or EMBEDDED as "
                  "required by OpenCL 1.2! (Returned %s)\n",
                  profile);
        return -1;
    }
...
static const char *sample_compile_size[2] = {
    ...
    "\n"
    "}\n",
    "__kernel __attribute__((reqd_work_group_size(%d,%d,%d))) void "
...
int test_kernel_required_group_size(cl_device_id deviceID, cl_context context,
                                    cl_command_queue queue, int num_elements)
{
    int error;
    size_t realSize;
...

    {
        ...
        context, &program, &kernel, 1, &sample_compile_size[0], "sample_test");
        if (error != 0) return error;

        error = clGetKernelWorkGroupInfo(
        kernel, deviceID, CL_KERNEL_WORK_GROUP_SIZE,
        sizeof(kernel_max_workgroup_size), &kernel_max_workgroup_size, NULL);
        test_error(
        error, "clGetKernelWorkGroupInfo failed for CL_KERNEL_WORK_GROUP_SIZE");
        ...
        {
            log_error("ERROR: Nonzero compile work group size returned for "
                      "nonspecified size! (returned %d,%d,%d)\n",
        ...
        {
            log_error("ERROR: Returned size of compile work group size not "
        ...
        if (max_dimensions == 1)
        {
            ...
            log_info("For global dimension %d, kernel will require local "
                     "dimension %d.\n",
        ...
        else if (max_dimensions == 2)
        {
            ...
            log_info("For global dimension %d x %d, kernel will require local "
                     "dimension %d x %d.\n",
                     (int)global[0], (int)global[1], (int)local[0],
                     (int)local[1]);
        }
        else
        {
            ...
            log_info("For global dimension %d x %d x %d, kernel will require "
                     "local dimension %d x %d x %d.\n",
                     (int)global[0], (int)global[1], (int)global[2],
                     (int)local[0], (int)local[1], (int)local[2]);
        }
    }
...

    {
        ...
        error = create_single_kernel_helper(
        context, &program, &kernel, 1, (const char **)&source, "sample_test");
        if (error != 0) return error;
        ...
        {
            log_error(
            "ERROR: Incorrect compile work group size returned for specified "
            "size! (returned %d,%d,%d, expected %d,%d,%d)\n",
            (int)size[0], (int)size[1], (int)size[2], (int)local[0],
            (int)local[1], (int)local[2]);
            return -1;
        }

        // Verify that the kernel will only execute with that size.
        in = clCreateBuffer(context, CL_MEM_READ_ONLY,
                            sizeof(cl_int) * global[0], NULL, &error);
        ...
        error = clEnqueueNDRangeKernel(queue, kernel, 3, NULL, global, local, 0,
                                       NULL, NULL);
        ...
        log_info("kernel_required_group_size may report spurious ERRORS in the "
                 "conformance log.\n");
        ...
        {
            log_error("Incorrect error returned for executing a kernel with "
                      "the wrong required local work group size. (used "
                      "%d,%d,%d, required %d,%d,%d)\n",
                      (int)local[0], (int)local[1], (int)local[2],
                      (int)local[0] - 1, (int)local[1], (int)local[2]);
