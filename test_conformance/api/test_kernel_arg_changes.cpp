...
// This test is designed to stress changing kernel arguments between execute
// calls (that are asynchronous and thus potentially overlapping) to make sure
// each kernel gets the right arguments

// Note: put a delay loop in the kernel to make sure we have time to queue the
// next kernel before this one finishes
const char *inspect_image_kernel_source[] = {
    "__kernel void sample_test(read_only image2d_t src, __global int "
    "*outDimensions )\n"
    "{\n"
...
int test_kernel_arg_changes(cl_device_id device, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    ...
    error = get_8_bit_image_format(context, CL_MEM_OBJECT_IMAGE2D,
                                   CL_MEM_READ_WRITE, 0, &imageFormat);
    ...
    context, &program, &kernel, 1, inspect_image_kernel_source, "sample_test");
    ...
    for (i = 0; i < NUM_TRIES; i++)
    {
        ...
        images[i] =
        create_image_2d(context, (cl_mem_flags)(CL_MEM_READ_ONLY), &imageFormat,
    ...
    for (i = 0; i < NUM_TRIES; i++)
    {
        error = clEnqueueReadBuffer(queue, results[i], CL_TRUE, 0,
                                    sizeof(cl_int) * threads[0] * 2,
                                    resultArray, 0, NULL, NULL);
        ...
        for (int j = 0; j < NUM_THREADS; j++)
        {
            ...
            {
                log_error("ERROR: Verficiation for kernel try %d, sample %d "
                          "FAILED, expected a width of %d, got %d\n",
            ...
            {
                log_error("ERROR: Verficiation for kernel try %d, sample %d "
                          "FAILED, expected a height of %d, got %d\n",
