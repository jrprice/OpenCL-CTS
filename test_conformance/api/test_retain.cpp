...
// Note: According to spec, the various functions to get instance counts should
// return an error when passed in an object that has already been released.
// However, the spec is out of date. If it gets re-updated to allow such action,
// re-enable this define.
//#define VERIFY_AFTER_RELEASE    1

#define GET_QUEUE_INSTANCE_COUNT(p)                                            \
    numInstances =                                                             \
    ((err = clGetCommandQueueInfo(p, CL_QUEUE_REFERENCE_COUNT,                 \
                                  sizeof(numInstances), &numInstances, NULL))  \
     == CL_SUCCESS                                                             \
     ? numInstances                                                            \
     : 0)
#define GET_MEM_INSTANCE_COUNT(p)                                              \
    numInstances =                                                             \
    ((err = clGetMemObjectInfo(p, CL_MEM_REFERENCE_COUNT,                      \
                               sizeof(numInstances), &numInstances, NULL))     \
     == CL_SUCCESS                                                             \
     ? numInstances                                                            \
     : 0)

#define VERIFY_INSTANCE_COUNT(c, rightValue)                                   \
    if (c != rightValue)                                                       \
    {                                                                          \
        log_error("ERROR: Instance count for test object is not valid! "       \
                  "(should be %d, really is %d)\n",                            \
                  rightValue, c);                                              \
        return -1;                                                             \
    }

int test_retain_queue_single(cl_device_id deviceID, cl_context context,
                             cl_command_queue queueNotUsed, int num_elements)
{
    ...
    /* We're not allowed to get the instance count after the object has been
     completely released. But that's exactly how we can tell the release
     worked--by making sure getting the instance count fails! */
    GET_QUEUE_INSTANCE_COUNT(queue);
...
int test_retain_queue_multiple(cl_device_id deviceID, cl_context context,
                               cl_command_queue queueNotUsed, int num_elements)
{
    cl_command_queue queue;
...
int test_retain_mem_object_single(cl_device_id deviceID, cl_context context,
...
int test_retain_mem_object_multiple(cl_device_id deviceID, cl_context context,
                                    cl_command_queue queue, int num_elements)
{
    cl_mem object;
    unsigned int numInstances, i;
...
int test_retain_mem_object_set_kernel_arg(cl_device_id deviceID,
                                          cl_context context,
                                          cl_command_queue queue,
                                          int num_elements)
{
    ...
    sValue = 0;
    auto callback = [](cl_mem, void *) { ++sValue; };
    ...
    err = create_single_kernel_helper(context, &program, nullptr, 1,
                                      testProgram, nullptr);
