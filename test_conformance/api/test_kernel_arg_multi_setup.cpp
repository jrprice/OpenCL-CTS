...
// This test is designed to stress passing multiple vector parameters to kernels
// and verifying access between them all

const char *multi_arg_kernel_source_pattern =
"__kernel void sample_test(__global %s *src1, __global %s *src2, __global %s "
"*src3, __global %s *dst1, __global %s *dst2, __global %s *dst3 )\n"
"{\n"
...
int test_multi_arg_set(cl_device_id device, cl_context context,
                       cl_command_queue queue, ExplicitType vec1Type,
                       int vec1Size, ExplicitType vec2Type, int vec2Size,
                       ExplicitType vec3Type, int vec3Size, MTdata d)
{
    ...
    sprintf(vec1Name, "%s%s", get_explicit_type_name(vec1Type),
            sizeNames[vec1Size - 1]);
    sprintf(vec2Name, "%s%s", get_explicit_type_name(vec2Type),
            sizeNames[vec2Size - 1]);
    sprintf(vec3Name, "%s%s", get_explicit_type_name(vec3Type),
            sizeNames[vec3Size - 1]);

    sprintf(programSrc, multi_arg_kernel_source_pattern, vec1Name, vec2Name,
            vec3Name, vec1Name, vec2Name, vec3Name, vec1Size, vec1Size,
            vec2Size, vec2Size, vec3Size, vec3Size);
    ptr = programSrc;
    ...
    initData[0] =
    create_random_data(vec1Type, d, (unsigned int)threads[0] * vec1Size);
    streams[0] =
    ...
    initData[1] =
    create_random_data(vec2Type, d, (unsigned int)threads[0] * vec2Size);
    streams[1] =
    clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
    ...
    initData[2] =
    create_random_data(vec3Type, d, (unsigned int)threads[0] * vec3Size);
    streams[2] =
    clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
    ...
    // Read results
    resultData[0] =
    malloc(get_explicit_type_size(vec1Type) * vec1Size * threads[0]);
    resultData[1] =
    malloc(get_explicit_type_size(vec2Type) * vec2Size * threads[0]);
    resultData[2] =
    malloc(get_explicit_type_size(vec3Type) * vec3Size * threads[0]);
    error = clEnqueueReadBuffer(queue, streams[3], CL_TRUE, 0,
                                get_explicit_type_size(vec1Type) * vec1Size
                                * threads[0],
                                resultData[0], 0, NULL, NULL);
    error |= clEnqueueReadBuffer(queue, streams[4], CL_TRUE, 0,
                                 get_explicit_type_size(vec2Type) * vec2Size
                                 * threads[0],
                                 resultData[1], 0, NULL, NULL);
    error |= clEnqueueReadBuffer(queue, streams[5], CL_TRUE, 0,
                                 get_explicit_type_size(vec3Type) * vec3Size
                                 * threads[0],
                                 resultData[2], 0, NULL, NULL);
    ...
    for (i = 0; i < (int)threads[0]; i++)
    {
        for (j = 0; j < vec1Size; j++)
        {
            ...
            {
                log_error("ERROR: Value did not validate for component %d of "
                          "item %d of stream 0!\n",
                          j, i);
                free(initData[0]);
                free(initData[1]);
...
int test_kernel_arg_multi_setup_exhaustive(cl_device_id device,
                                           cl_context context,
                                           ...
                                           int num_elements)
{
    ...
    for (type1 = 0; types[type1] != kNumExplicitTypes; type1++)
    {
        for (type2 = 0; types[type2] != kNumExplicitTypes; type2++)
        {
            ...
            {
                log_info("\n\ttesting %s, %s, %s...",
                         get_explicit_type_name(types[type1]),
                         get_explicit_type_name(types[type2]),
                ...
                for (size1 = 2; size1 <= 8; size1 <<= 1)
                {
                    for (size2 = 2; size2 <= 8; size2 <<= 1)
                    {
                        for (size3 = 2; size3 <= 8; size3 <<= 1)
                        {
                            log_info(".");
                            fflush(stdout);
                            if (test_multi_arg_set(
                                device, context, queue, types[type1], size1,
                                types[type2], size2, types[type3], size3, seed))
                                return -1;
                        }
                    }
                }
...
int test_kernel_arg_multi_setup_random(cl_device_id device, cl_context context,
        ...
        for (size2 = 2; size2 <= 8; size2 <<= 1)
        {
            for (size3 = 2; size3 <= 8; size3 <<= 1)
            {
                ...
                for (n = 0; n < 4; n++)
                {
                    ...
                    if (test_multi_arg_set(device, context, queue, types[type1],
                                           size1, types[type2], size2,
