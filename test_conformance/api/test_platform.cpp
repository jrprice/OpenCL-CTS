                             ...
                             cl_command_queue queue, int num_elements)
{
    ...
    {
        vlog_error(
        "test_platform_extensions : could not get platformID from device\n");
        return -1;
    }
    ...
    err =
    clGetDeviceInfo(deviceID, CL_DEVICE_EXTENSIONS, sizeof(device_extensions),
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    for (int p = 0; p < (int)num_platforms; p++)
    {
        ...
        {
            log_error(
            "Returned string length %ld does not equal reported one %ld.\n",
        ...
        for (int d = 0; d < (int)num_devices; d++)
        {
            ...
            {
                log_error("Reported return size (%ld) does not match expected "
                          "size (%ld).\n",
