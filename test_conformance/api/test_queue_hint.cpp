...
int test_enqueue(cl_context context, clCommandQueueWrapper& queue,
                 clKernelWrapper& kernel, size_t num_elements)
{
    ...
    clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
                   num_elements * sizeof(int), buf, &error);
    ...
    error = clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0,
                                num_elements * sizeof(int), buf, 0, NULL, NULL);
...
int test_queue_hint(cl_device_id deviceID, cl_context context,
                    cl_command_queue queue, int num_elements)
{
    ...
    context, &program, &kernel, 1, queue_hint_test_kernel, "vec_cpy", NULL);
    if (err != 0)
    {
        return err;
    }
    ...
    if (is_extension_available(deviceID, "cl_khr_priority_hints"))
    {
        ...
        cl_queue_properties queue_prop[][3] = {
            ...
            { CL_QUEUE_PRIORITY_KHR, CL_QUEUE_PRIORITY_MED_KHR, 0 },
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            context, deviceID, queue_prop[i], &err);
    ...
    if (is_extension_available(deviceID, "cl_khr_throttle_hints"))
    {
        ...
        cl_queue_properties queue_prop[][3] = {
            ...
            { CL_QUEUE_THROTTLE_KHR, CL_QUEUE_THROTTLE_MED_KHR, 0 },
            { CL_QUEUE_THROTTLE_KHR, CL_QUEUE_THROTTLE_LOW_KHR, 0 }
        };
