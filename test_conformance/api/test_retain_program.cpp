...
int test_release_kernel_order(cl_device_id deviceID, cl_context context,
                              cl_command_queue queue, int num_elements)
{
    ...
    error =
    create_single_kernel_helper(context, &program, NULL, 1, testProgram, NULL);
    ...
    /* Now try freeing the program first, then the kernel. If refcounts are
     * right, this should work just fine */
    clReleaseProgram(program);
    clReleaseKernel(kernel);

    /* If we got here fine, we succeeded. If not, well, we won't be able to
     * return an error :) */
    return 0;
}
...
int test_release_during_execute(cl_device_id deviceID, cl_context context,
                                cl_command_queue queue, int num_elements)
{
    int error;
    cl_program program;
    ...
    if (create_single_kernel_helper(context, &program, &kernel, 1,
                                    sample_delay_kernel, "sample_test"))
    {
        return -1;
    }
    ...
    error = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, threads,
                                   &localThreadSize, 0, NULL, NULL);
    test_error(error, "Unable to execute test kernel");

    /* The kernel should still be executing, but we should still be able to
       release it. It's not terribly
       useful, but we should be able to do it, if the internal refcounting is
       indeed correct. */
