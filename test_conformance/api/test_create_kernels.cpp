...
int test_load_single_kernel(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    ...
    error = create_single_kernel_helper(context, &program, NULL, 1,
                                        sample_single_kernel, NULL);
    ...
    error = clGetKernelInfo(kernel, CL_KERNEL_PROGRAM, sizeof(cl_program),
                            &testProgram, &realSize);
    test_error(error, "Unable to get kernel's program");
    if ((cl_program)testProgram != (cl_program)program)
    {
        log_error("ERROR: Returned kernel's program does not match program "
                  "used to create it! (Got %p, expected %p)\n",
                  (cl_program)testProgram, (cl_program)program);
        return -1;
    }
    if (realSize != sizeof(cl_program))
    {
        log_error("ERROR: Returned size of kernel's program does not match "
                  "expected size (expected %d, got %d)\n",
                  (int)sizeof(cl_program), (int)realSize);
        return -1;
    }

    error = clGetKernelInfo(kernel, CL_KERNEL_CONTEXT, sizeof(cl_context),
                            &testContext, &realSize);
    test_error(error, "Unable to get kernel's context");
    if ((cl_context)testContext != (cl_context)context)
    {
        log_error("ERROR: Returned kernel's context does not match program "
                  "used to create it! (Got %p, expected %p)\n",
    ...
    {
        log_error("ERROR: Returned size of kernel's context does not match "
                  "expected size (expected %d, got %d)\n",
    ...
    error = clGetKernelInfo(kernel, CL_KERNEL_NUM_ARGS, sizeof(testArgCount),
                            &testArgCount, NULL);
    ...
    /* Test function name */
    error = clGetKernelInfo(kernel, CL_KERNEL_FUNCTION_NAME, sizeof(testName),
                            testName, &realSize);
    ...
    {
        log_error("ERROR: Length of kernel name returned does not validate "
                  "(expected %d, got %d)\n",
...
int test_load_two_kernels(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
    ...
    /* Check first kernel */
    error = clGetKernelInfo(kernel[0], CL_KERNEL_FUNCTION_NAME,
                            sizeof(testName), testName, NULL);
    ...
    {
        log_error("ERROR: Invalid kernel name returned: \"%s\" expected \"%s\" "
                  "or \"%s\".\n",
                  testName, "sample_test", "sample_test2");
        return -1;
    }

    error = clGetKernelInfo(kernel[1], CL_KERNEL_FUNCTION_NAME,
                            sizeof(testName), testName, NULL);
    ...
    error = clGetKernelInfo(kernel[0], CL_KERNEL_NUM_ARGS, sizeof(testArgCount),
                            &testArgCount, NULL);
    test_error(error, "Unable to get arg count from kernel");
...
int test_load_two_kernels_in_one(cl_device_id deviceID, cl_context context,
                                 cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
...
int test_load_two_kernels_manually(cl_device_id deviceID, cl_context context,
                                   cl_command_queue queue, int num_elements)
{
    clProgramWrapper program;
                                       ...
                                       cl_command_queue queue, int num_elements)
{
    ...
    const char *actual_names[] = { "sample_test;sample_test2",
    ...
    for (i = 0; i < sizeof(actual_names) / sizeof(actual_names[0]); i++)
        if (0 == strcmp(actual_names[i], kernel_names)) break;
...
int test_enqueue_task(cl_device_id deviceID, cl_context context,
                      cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
    ...
    if (create_single_kernel_helper(context, &program, &kernel, 1,
                                    single_task_kernel, "sample_test"))
        return -1;

    // Create args
    count = 100;
    output = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                            sizeof(cl_int) * count, NULL, &error);
    ...
    error = clEnqueueReadBuffer(queue, output, CL_TRUE, 0,
                                sizeof(cl_int) * count, results, 0, NULL, NULL);
    ...
    for (cl_int i = 0; i < count; i++)
    {
        if (results[i] != i)
        {
            log_error("ERROR: Task result value %d did not validate! Expected "
                      "%d, got %d\n",
...
int test_repeated_setup_cleanup(cl_device_id deviceID, cl_context context,
                                cl_command_queue queue, int num_elements)
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        local_context =
        clCreateContext(NULL, 1, &deviceID, notify_callback, NULL, &error);
        test_error(error, "clCreateContext failed");

        local_queue = clCreateCommandQueue(local_context, deviceID, 0, &error);
        test_error(error, "clCreateCommandQueue failed");

        error = create_single_kernel_helper(local_context, &local_program, NULL,
                                            1, &repeate_test_kernel, NULL);
        test_error(error, "Unable to build test program");

        local_kernel = clCreateKernel(local_program, "test_kernel", &error);
        test_error(error, "clCreateKernel failed");

        local_mem_in = clCreateBuffer(local_context, CL_MEM_READ_ONLY,
                                      TEST_SIZE * sizeof(cl_int), NULL, &error);
        test_error(error, "clCreateBuffer failed");

        local_mem_out =
        clCreateBuffer(local_context, CL_MEM_WRITE_ONLY,
                       TEST_SIZE * sizeof(cl_int), NULL, &error);
        test_error(error, "clCreateBuffer failed");

        error =
        clEnqueueWriteBuffer(local_queue, local_mem_in, CL_TRUE, 0,
                             TEST_SIZE * sizeof(cl_int), inData, 0, NULL, NULL);
        test_error(error, "clEnqueueWriteBuffer failed");

        error = clEnqueueWriteBuffer(local_queue, local_mem_out, CL_TRUE, 0,
                                     TEST_SIZE * sizeof(cl_int), outData, 0,
                                     NULL, NULL);
        test_error(error, "clEnqueueWriteBuffer failed");

        error =
        clSetKernelArg(local_kernel, 0, sizeof(local_mem_in), &local_mem_in);
        test_error(error, "clSetKernelArg failed");

        error =
        clSetKernelArg(local_kernel, 1, sizeof(local_mem_out), &local_mem_out);
        test_error(error, "clSetKernelArg failed");

        error = clEnqueueNDRangeKernel(local_queue, local_kernel, 1, NULL,
                                       global_dim, NULL, 0, NULL, &local_event);
        test_error(error, "clEnqueueNDRangeKernel failed");

        error = clWaitForEvents(1, &local_event);
        test_error(error, "clWaitForEvents failed");

        error = clGetEventInfo(local_event, CL_EVENT_COMMAND_EXECUTION_STATUS,
        ...
        error =
        clEnqueueReadBuffer(local_queue, local_mem_out, CL_TRUE, 0,
                            TEST_SIZE * sizeof(cl_int), outData, 0, NULL, NULL);
        ...
        for (j = 0; j < TEST_SIZE; j++)
        {
            ...
            {
                log_error(
                "Results failed to validate at iteration %d. %d != %d.\n", i,
                outData[j], inData[j] + 1);
                free(inData);
                free(outData);
                return -1;
            }
