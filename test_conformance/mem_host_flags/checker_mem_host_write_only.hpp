...
template <class T> cl_int cBuffer_check_mem_host_write_only<T>::SetupBuffer()
{
    ...
    this->m_buffer =
    clCreateBuffer(this->m_context, this->buffer_mem_flag, block_size_in_byte,
...
cl_int cBuffer_check_mem_host_write_only<T>::Setup_Test_Environment()
{
    ...
    this->m_buffer2 =
    clCreateBuffer(this->m_context, buffer_mem_flag2,
...
cl_int cBuffer_check_mem_host_write_only<T>::verify_RW_Buffer_rect()
{
    ...
    this->m_queue, this->m_buffer, this->m_blocking, this->buffer_origin_bytes,
    ...
    this->host_slice_pitch_bytes, tmp_host_m.pData, 1, &event_1, &event);
    ...
    this->host_origin_bytes, this->region_bytes, this->buffer_row_pitch_bytes,
    this->buffer_slice_pitch_bytes, this->host_row_pitch_bytes,
    this->host_slice_pitch_bytes, this->host_m_2.pData, 0, NULL, &event);

    if (err == CL_SUCCESS)
    {
        log_error(
        "Calling clEnqueueReadBufferRect on a memory object created with the "
        "CL_MEM_HOST_WRITE_ONLY flag should not return CL_SUCCESS\n");
        err = FAILURE;
        return FAILURE;
    }
...
cl_int cBuffer_check_mem_host_write_only<T>::update_host_mem_2()
{
    ...
    cl_int err =
    clEnqueueCopyBuffer(this->m_queue, this->m_buffer, this->m_buffer2, 0, 0,
...
cl_int cBuffer_check_mem_host_write_only<T>::verify_RW_Buffer_mapping()
{
    ...
    {
        log_error(
        "Calling clEnqueueMapBuffer (CL_MAP_READ) on a memory object created "
        "with the MEM_HOST_WRITE_ONLY flag should not return CL_SUCCESS\n");
        err = FAILURE;
    }
    else
    {
