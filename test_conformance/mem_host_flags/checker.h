...
cl_int cBuffer_checker<T>::SetupASSubBuffer(cl_mem_flags parent_buffer_flag)
{
    ...
    err =
    clGetDeviceInfo(m_deviceID, CL_DEVICE_MEM_BASE_ADDR_ALIGN,
                    sizeof base_addr_align_bits, &base_addr_align_bits, NULL);
    ...
    {
        this->pHost_ptr =
        (this->host_m_0.pData + this->m_sub_buffer_region.origin / sizeof(T));
    }
                                        ...
                                        void *pdata)
{
    ...
    m_buffer =
    clCreateBuffer(m_context, buffer_mem_flag, block_size_in_byte, pdata, &err);
