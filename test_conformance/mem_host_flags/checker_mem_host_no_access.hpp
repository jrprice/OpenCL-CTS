...
cl_int cBuffer_check_mem_host_no_access<T>::verify_RW_Buffer_rect()
{
    ...
    {
        log_error(
        "Calling clEnqueueWriteBufferRect on a memory object created with the "
        "CL_MEM_HOST_NO_ACCESS flag should not return CL_SUCCESS\n");
        err = FAILURE;
        return FAILURE;
    }
...
cl_int cBuffer_check_mem_host_no_access<T>::verify_RW_Buffer_mapping()
{
    ...
    if (err == CL_SUCCESS)
    {
        ...
        "with the CL_MEM_HOST_NO_ACCESS flag should not return CL_SUCCESS\n");
        err = FAILURE;
        return FAILURE;
    }
    else
    ...
    {
        log_error(
        "Calling clEnqueueMapBuffer (CL_MAP_WRITE) on a memory object created "
        "with the CL_MEM_HOST_NO_ACCESS flag should not return CL_SUCCESS\n");
        err = FAILURE;
        return FAILURE;
    }
