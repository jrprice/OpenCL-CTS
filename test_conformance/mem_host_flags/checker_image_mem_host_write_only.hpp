...
cl_int cImage_check_mem_host_write_only<T>::Setup_Test_Environment()
{
    ...
    &(this->m_cl_image_format), &(this->m_cl_Image_desc), this->host_m_2.pData,
    &err);
...
cl_int cImage_check_mem_host_write_only<T>::update_host_mem_2()
{
    ...
    this->region, this->buffer_row_pitch_bytes, this->buffer_slice_pitch_bytes,
...
template <class T> cl_int cImage_check_mem_host_write_only<T>::verify_data()
{
    ...
    if (!this->host_m_1.Equal_rect_from_orig(
        this->host_m_2, this->buffer_origin, this->region, this->host_row_pitch,
...
template <class T> cl_int cImage_check_mem_host_write_only<T>::verify_RW_Image()
{
   ...
   * Further, we don't need to do any additional flushes or finishes here since
  we were in sync
...
cl_int cImage_check_mem_host_write_only<T>::verify_RW_Image_Mapping()
{
    ...
    {
        log_error(
        "Calling clEnqueueMapImage (CL_MAP_READ) on a memory object created "
        "with the CL_MEM_HOST_WRITE_ONLY flag should not return CL_SUCCESS\n");
        err = FAILURE;
        return FAILURE;
    }
    else
