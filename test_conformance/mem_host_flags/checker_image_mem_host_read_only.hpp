...
{
    int offset_pixel =
    (int)(this->buffer_origin[0]
          + this->buffer_origin[1] * this->buffer_row_pitch_bytes / sizeof(T)
          + this->buffer_origin[2] * this->buffer_slice_pitch_bytes
          / sizeof(T));
...
cl_int cImage_check_mem_host_read_only<T>::verify_RW_Image_Mapping()
{
    ...
    {
        log_error(
        "Calling clEnqueueMapImage (CL_MAP_WRITE) on a memory object created "
