                    ...
                    cl_device_id *devices, int num_elements)
{
    ...
    {
        log_error("Number of devices in set (%ld) is greater than the number "
    ...
    for (i = 0; i < deviceCount; i++)
    {
        ...
        error = clGetDeviceInfo(devices[i], CL_DEVICE_NAME, sizeof(deviceName),
                                deviceName, NULL);
...
int test_two_devices(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    ...
    if (numDevices < 2)
    {
        log_info("WARNING: two device test unable to get two devices via "
                 "CL_DEVICE_TYPE_ALL (got %d devices). Skipping test...\n",
                 (int)numDevices);
        return 0;
    }
...
int test_max_devices(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    cl_platform_id platform;
    ...
    /* Get some devices */
    err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, MAX_DEVICES, devices,
                         &deviceCount);
...
int test_hundred_queues(cl_device_id device, cl_context contextIgnore,
                        cl_command_queue queueIgnore, int num_elements)
{
