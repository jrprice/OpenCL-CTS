...
sampleActionFn sampleActions[4] = { sampleAction1, sampleAction2, sampleAction3,
                                    sampleAction4 };
...
TestItem *CreateTestItem(cl_device_id deviceID, cl_int *err)
{
    ...
    if (NULL == item)
    {
        ...
        {
            log_error(
            "FAILURE: Failed to allocate TestItem -- out of host memory!\n");
    ...
    if (NULL == item->d)
    {
        if (err)
        {
            log_error("FAILURE: Failed to allocate mtdata om CreateTestItem -- "
                      "out of host memory!\n");
            *err = CL_OUT_OF_HOST_MEMORY;
        }
    ...
    if (item->q == NULL || error != CL_SUCCESS)
    {
        ...
        {
            log_error(
            "FAILURE: clCreateCommandQueue failed in CreateTestItem: %d\n",
            error);
            *err = error;
        }
        DestroyTestItem(item);
    ...
    // Create a program
    error = create_single_kernel_helper_create_program(item->c, &item->p, 1,
                                                       context_test_kernels);
    ...
    {
        static const char *kernelNames[] = { "sample_test_1", "sample_test_2",
                                             "sample_test_3", "sample_test_4" };
        item->k[i] = clCreateKernel(item->p, kernelNames[i], &error);
        if (NULL == item->k[i] || CL_SUCCESS != error)
        {
            if (err)
            {
                log_error("FAILURE: clCreateKernel( \"%s\" ) failed in "
                          "CreateTestItem: %d\n",
                          kernelNames[i], error);
                *err = error;
            }
...
static void DestroyTestItem(TestItem *item)
{
    ...
    if (item->d) free_mtdata(item->d);
    if (item->c) clReleaseContext(item->c);
    if (item->q) clReleaseCommandQueue(item->q);
    if (item->p) clReleaseProgram(item->p);
    for (i = 0; i < sizeof(item->k) / sizeof(item->k[0]); i++)
    {
        if (item->k[i]) clReleaseKernel(item->k[i]);
    }
    for (i = 0; i < BUFFER_COUNT; i++)
    {
        if (item->m[i]) clReleaseMemObject(item->m[i]);
    }
    free(item);
}
...
cl_int UseTestItem(const TestItem *item, cl_int *err)
{
    ...
    cl_int *mapped = (cl_int *)clEnqueueMapBuffer(
    item->q, item->m[0], CL_TRUE, CL_MAP_WRITE, 0, TEST_SIZE * sizeof(cl_int),
    0, NULL, NULL, &error);
    ...
    for (j = 0; j < TEST_SIZE; j++) mapped[j] = genrand_int32(item->d);
    ...
    for (j = 0; j < sizeof(item->k) / sizeof(item->k[0]); j++)
    {
        ...
        mapped = (cl_int *)clEnqueueMapBuffer(
        item->q, item->m[1], CL_TRUE, CL_MAP_WRITE, 0,
        TEST_SIZE * sizeof(cl_int), 0, NULL, NULL, &error);
        if (NULL == mapped || CL_SUCCESS != error)
        {
            if (err)
            {
        ...
        for (i = 0; i < TEST_SIZE; i++) mapped[i] = 0xdeaddead;

        error =
        clEnqueueUnmapMemObject(item->q, item->m[1], mapped, 0, NULL, NULL);
        if (CL_SUCCESS != error)
        {
            if (err)
            {
        ...
        if (error)
        {
            ...
            {
                log_error(
                "FAILURE: Unable to set arg 1 for kernel # %ld :  %d\n", j,
                error);
                *err = error;
            }
            return error;
        ...
        error = clEnqueueNDRangeKernel(item->q, item->k[j], 1, &global_offset,
        ...
        cl_int *inputData = (cl_int *)clEnqueueMapBuffer(
        ...
        for (i = 0; i < TEST_SIZE; i++)
        {
            ...
            if (expected != result)
            {
                log_error("FAILURE:  Sample data at position %ld does not "
        ...
        // Clean up
        error =
        clEnqueueUnmapMemObject(item->q, item->m[0], inputData, 0, NULL, NULL);
        if (CL_SUCCESS != error)
        {
            if (err)
            {
...
int test_context_multiple_contexts_same_device(cl_device_id deviceID,
                                               size_t maxCount, size_t minCount)
{
    size_t i, j;
    ...
    for (i = 0; i < maxCount; i++)
    {
        ...
        TestItem *current =
        CreateTestItem(deviceID, NULL /*no error reporting*/);
...
//  This test tests to make sure that your implementation isn't super leaky.  We
//  make a bunch of contexts (up to some sane limit, currently 200), attempting
//  to use each along the way. We keep track of how many we could make before a
//  failure occurred.   We then free everything and attempt to go do it again a
//  few times.  If you are able to make that many contexts 5 times over, then
//  you pass.
int test_context_multiple_contexts_same_device(cl_device_id deviceID,
                                               cl_context context,
...
int test_context_two_contexts_same_device(cl_device_id deviceID,
                                          cl_context context,
                                          cl_command_queue queue,
                                          int num_elements)
{
...
int test_context_three_contexts_same_device(cl_device_id deviceID,
                                            cl_context context,
                                            cl_command_queue queue,
                                            int num_elements)
{
...
int test_context_four_contexts_same_device(cl_device_id deviceID,
                                           cl_context context,
                                           cl_command_queue queue,
                                           int num_elements)
{
