...
static const char *int_kernel_name[] = {
    "test_buffer_read_int", "test_buffer_read_int2", "test_buffer_read_int4",
    "test_buffer_read_int8", "test_buffer_read_int16"
};
...
static const char *uint_kernel_name[] = {
    "test_buffer_read_uint", "test_buffer_read_uint2", "test_buffer_read_uint4",
    "test_buffer_read_uint8", "test_buffer_read_uint16"
};
...
static const char *long_kernel_name[] = {
    "test_buffer_read_long", "test_buffer_read_long2", "test_buffer_read_long4",
    "test_buffer_read_long8", "test_buffer_read_long16"
};
...
static const char *ulong_kernel_name[] = { "test_buffer_read_ulong",
                                           "test_buffer_read_ulong2",
                                           "test_buffer_read_ulong4",
                                           "test_buffer_read_ulong8",
                                           "test_buffer_read_ulong16" };
...
static const char *short_kernel_name[] = { "test_buffer_read_short",
                                           "test_buffer_read_short2",
                                           "test_buffer_read_short4",
                                           "test_buffer_read_short8",
                                           "test_buffer_read_short16" };
...
static const char *ushort_kernel_name[] = { "test_buffer_read_ushort",
                                            "test_buffer_read_ushort2",
                                            "test_buffer_read_ushort4",
                                            "test_buffer_read_ushort8",
                                            "test_buffer_read_ushort16" };
...
static const char *float_kernel_name[] = { "test_buffer_read_float",
                                           "test_buffer_read_float2",
                                           "test_buffer_read_float4",
                                           "test_buffer_read_float8",
                                           "test_buffer_read_float16" };
...
static const char *half_kernel_name[] = {
    "test_buffer_read_half", "test_buffer_read_half2", "test_buffer_read_half4",
    "test_buffer_read_half8", "test_buffer_read_half16"
};
...
static const char *char_kernel_name[] = {
    "test_buffer_read_char", "test_buffer_read_char2", "test_buffer_read_char4",
    "test_buffer_read_char8", "test_buffer_read_char16"
};
...
static const char *uchar_kernel_name[] = { "test_buffer_read_uchar",
                                           "test_buffer_read_uchar2",
                                           "test_buffer_read_uchar4",
                                           "test_buffer_read_uchar8",
                                           "test_buffer_read_uchar16" };
...
static int verify_read_short(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != (cl_short)((1 << 8) + 1)) return -1;
    }
...
//----- the test functions
int test_buffer_read(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, int num_elements, size_t size,
                     char *type, int loops, const char *kernelCode[],
                     const char *kernelName[], int (*fn)(void *, int))
{
    ...
    for (src_flag_id = 0; src_flag_id < NUM_FLAGS; src_flag_id++)
    {
        ...
        for (i = 0; i < loops; i++)
        {
            ...
            err = create_single_kernel_helper(context, &program[i], &kernel[i],
                                              1, &kernelCode[i], kernelName[i]);
            if (err)
            {
            ...
            err =
            clSetKernelArg(kernel[i], 0, sizeof(cl_mem), (void *)&buffers[i]);
            if (err != CL_SUCCESS)
            {
            ...
            err =
            clEnqueueNDRangeKernel(queue, kernel[i], 1, NULL, global_work_size,
...
int test_buffer_read_async(cl_device_id deviceID, cl_context context,
                           ...
                           int (*fn)(void *, int))
{
    ...
    for (src_flag_id = 0; src_flag_id < NUM_FLAGS; src_flag_id++)
    {
        ...
        for (i = 0; i < loops; i++)
        {
            ...
            memset(outptr[i], 0,
                   ptrSizes[i]
                   * num_elements); // initialize to zero to tell difference
            inptr[i] = align_malloc(ptrSizes[i] * num_elements, min_alignment);
            if (!inptr[i])
            {
                log_error(" unable to allocate %d bytes for inptr\n",
            ...
            err = clEnqueueReadBuffer(queue, buffers[i], false, 0,
                                      ptrSizes[i] * num_elements, outptr[i], 0,
                                      NULL, &event);
            ...
            {
                log_error(
                "    clEnqueueReadBuffer() possibly returned only after "
                "inappropriately waiting for execution to be finished\n");
                log_error(
                "    Function was run asynchornously, but last value in array "
                "was set in code line following clEnqueueReadBuffer()\n");
            }
#endif
            if (err != CL_SUCCESS)
            {
...
int test_buffer_read_array_barrier(cl_device_id deviceID, cl_context context,
                                   cl_command_queue queue, int num_elements,
                                   size_t size, char *type, int loops,
...
#define DECLARE_READ_TEST(type, realType)                                      \
    int test_buffer_read_##type(cl_device_id deviceID, cl_context context,     \
                                cl_command_queue queue, int num_elements)      \
    {                                                                          \
        return test_buffer_read(deviceID, context, queue, num_elements,        \
...
int test_buffer_half_read(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    return test_buffer_read(deviceID, context, queue, num_elements,
...
#define DECLARE_ASYNC_TEST(type, realType)                                     \
    int test_buffer_read_async_##type(                                         \
    cl_device_id deviceID, cl_context context, cl_command_queue queue,         \
    int num_elements)                                                          \
    {                                                                          \
        return test_buffer_read_async(deviceID, context, queue, num_elements,  \
                                      sizeof(realType), (char *)#type, 5,      \
                                      buffer_read_##type##_kernel_code,        \
                                      type##_kernel_name, verify_read_##type); \
    }
...
#define DECLARE_BARRIER_TEST(type, realType)                                   \
    int test_buffer_read_array_barrier_##type(                                 \
    cl_device_id deviceID, cl_context context, cl_command_queue queue,         \
    int num_elements)                                                          \
    {                                                                          \
        return test_buffer_read_array_barrier(                                 \
        deviceID, context, queue, num_elements, sizeof(realType),              \
...
/*
 int test_buffer_half_read(cl_device_group device, cl_device id, cl_context
 ...
 buffers[0] = clCreateBuffer(device, (cl_mem_flags)(CL_MEM_READ_WRITE),  ptrSize
 * num_elements, NULL); if( !buffers[0] ){ log_error("clCreateBuffer failed\n");
 return -1;
 }

 err = create_program_and_kernel(device, buffer_read_half_kernel_code,
 "test_buffer_read_half", &program[0], &kernel[0]); if( err ){ log_error( "
 Error creating program for half\n" ); clReleaseMemObject(buffers[0]); free(
 (void *)outptr ); return -1;
 }
 ...
 global_work_size[0] = (cl_uint)num_elements;
 err = clEnqueueNDRangeKernel(queue, kernel[0], 1, NULL, threads, NULL, 0, NULL,
 NULL ); if( err != CL_SUCCESS ){ log_error("clEnqueueNDRangeKernel failed\n");
 return -1;
 }

 err = clEnqueueReadBuffer( queue, buffers[0], true, 0, ptrSize*num_elements,
 (void *)outptr, 0, NULL, NULL ); if( err != CL_SUCCESS ){
...
int test_buffer_read_struct(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    cl_mem buffers[1];
    ...
    output_ptr =
    (TestStruct *)align_malloc(objSize * num_elements, min_alignment);
    if (!output_ptr)
    {
    ...
    buffers[0] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
    ...
    err =
    clEnqueueReadBuffer(queue, buffers[0], true, 0, objSize * num_elements,
...
static int testRandomReadSize(cl_device_id deviceID, cl_context context,
                              cl_command_queue queue, int num_elements,
                              cl_uint startOfRead, size_t sizeOfRead)
{
    cl_mem buffers[3];
    int *outptr[3];
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        buffers[i] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                                    ptrSizes[i] * num_elements, NULL, &err);
        if (err != CL_SUCCESS)
        {
            print_error(err, " clCreateBuffer failed\n");
    ...
    err = create_single_kernel_helper(context, &program[1], &kernel[1], 1,
                                      &buffer_read_int_kernel_code[1],
                                      "test_buffer_read_int2");
    if (err)
    {
    ...
    err = create_single_kernel_helper(context, &program[2], &kernel[2], 1,
                                      &buffer_read_int_kernel_code[2],
                                      "test_buffer_read_int4");
    if (err)
    {
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        queue, buffers[i], true, startOfRead * ptrSizes[i],
        ptrSizes[i] * sizeOfRead, (void *)(outptr[i]), 0, NULL, NULL);
        if (err != CL_SUCCESS)
        {
            print_error(err, "clEnqueueReadBuffer failed");
            clReleaseMemObject(buffers[i]);
...
int test_buffer_read_random_size(cl_device_id deviceID, cl_context context,
                                 cl_command_queue queue, int num_elements)
{
    int err = 0;
    int i;
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if (testRandomReadSize(deviceID, context, queue, num_elements, start,
                               size))
            err++;
    }
