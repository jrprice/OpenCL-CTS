...
// Design:
// To test sub buffers, we first create one main buffer. We then create several
// sub-buffers and queue Actions on each one. Each Action is encapsulated in a
// class so it can keep track of what results it expects, and so we can test
// scaling degrees of Actions on scaling numbers of sub-buffers.
...
public:
    ...
    cl_int Allocate(cl_mem parent, cl_mem_flags flags, size_t origin,
                    size_t size)
    {
        ...
        cl_int error;
        mMem = clCreateSubBuffer(mParentBuffer, flags,
...
public:
    virtual ~Action() {}
    virtual cl_int Execute(cl_context context, cl_command_queue queue,
                           cl_char tag, SubBufferWrapper &buffer1,
                           SubBufferWrapper &buffer2,
                           cl_char *parentBufferState) = 0;
    virtual const char *GetName(void) const = 0;

    static MTdata d;
    static MTdata GetRandSeed(void)
    {
        if (d == 0) d = init_genrand(gRandomSeed);
        return d;
    }
...
public:
                           ...
                           cl_char *parentBufferState)
    {
        ...
        queue, buffer1, CL_TRUE, 0, buffer1.mSize, tempBuffer, 0, NULL, NULL);
        ...
        error = clEnqueueWriteBuffer(queue, buffer1, CL_TRUE, 0, buffer1.mSize,
                                     tempBuffer, 0, NULL, NULL);
...
public:
                           ...
                           cl_char *parentBufferState)
    {
        ...
        size_t size =
        get_random_size_t(0, MIN(buffer1.mSize, buffer2.mSize), GetRandSeed());

        size_t startOffset =
        get_random_size_t(0, buffer1.mSize - size, GetRandSeed());
        size_t endOffset =
        get_random_size_t(0, buffer2.mSize - size, GetRandSeed());

        cl_int error = clEnqueueCopyBuffer(queue, buffer1, buffer2, startOffset,
                                           endOffset, size, 0, NULL, NULL);
        test_error(error, "Unable to enqueue buffer copy");

        memcpy(parentBufferState + buffer2.mOrigin + endOffset,
               parentBufferState + buffer1.mOrigin + startOffset, size);
...
public:
                           ...
                           cl_char *parentBufferState)
    {
        ...
        queue, buffer1, CL_TRUE, (cl_map_flags)(CL_MAP_READ | CL_MAP_WRITE),
        ...
        error =
        clEnqueueUnmapMemObject(queue, buffer1, mappedPtr, 0, NULL, NULL);
...
cl_int get_reasonable_buffer_size(cl_device_id device, size_t &outSize)
{
    ...
    if (outSize > 32 << 20) outSize = 32 << 20;

    return CL_SUCCESS;
}

size_t find_subbuffer_by_index(SubBufferWrapper *subBuffers,
                               size_t numSubBuffers, size_t index)
{
    for (size_t i = 0; i < numSubBuffers; i++)
    {
        if (subBuffers[i].mOrigin > index) return numSubBuffers;
        if ((subBuffers[i].mOrigin <= index)
            && ((subBuffers[i].mOrigin + subBuffers[i].mSize) > index))
            return i;
    }
    return numSubBuffers;
}

// This tests the read/write capabilities of sub buffers (if we are read/write,
// the sub buffers can't overlap)
int test_sub_buffers_read_write_core(cl_context context,
                                     cl_command_queue queueA,
                                     cl_command_queue queueB, size_t mainSize,
                                     size_t addressAlign)
{
    ...
    mainBuffer =
    clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, mainSize,
                   mainBufferContents, &error);
    ...
    for (numSubBuffers = 0; numSubBuffers < 8; numSubBuffers++)
    {
        ...
        if (endRange > mainSize) endRange = mainSize;

        size_t offset =
        get_random_size_t(toStartFrom / addressAlign, endRange / addressAlign,
                          Action::GetRandSeed())
        * addressAlign;
        size_t size = get_random_size_t(
                      1, (MIN(mainSize / 8, mainSize - offset)) / addressAlign,
                      Action::GetRandSeed())
        * addressAlign;
        error = subBuffers[numSubBuffers].Allocate(
        mainBuffer, CL_MEM_READ_WRITE, offset, size);
        test_error(error, "Unable to allocate sub buffer");

        toStartFrom = offset + size;
        if (toStartFrom > (mainSize - (addressAlign * 256))) break;
    }
    ...
    // Do the following steps twice, to make sure the parent gets updated *and*
    // we can still work on the sub-buffers
    ...
    for (int time = 0; time < 2; time++)
    {
        ...
        for (i = 0; i < 64; i++)
        {
            ...
            int whichBufferA =
            random_in_range(0, (int)numSubBuffers - 1, Action::GetRandSeed());
            int whichBufferB;
            do
            {
                whichBufferB = random_in_range(0, (int)numSubBuffers - 1,
                                               Action::GetRandSeed());
            ...
            error = actions[which]->Execute(
            context, queue, (cl_int)i, subBuffers[whichBufferA],
            subBuffers[whichBufferB], mainBufferContents);
        ...
        error = clEnqueueReadBuffer(queueA, mainBuffer, CL_TRUE, 0, mainSize,
                                    actualResults, 0, NULL, NULL);
        test_error(error, "Unable to enqueue buffer read");

        for (i = 0; i < mainSize; i += 65536)
        {
            size_t left = 65536;
            if ((i + left) > mainSize) left = mainSize - i;

            if (memcmp(actualResults + i, mainBufferContents + i, left) == 0)
                continue;

            // The fast compare failed, so we need to determine where exactly
            // the failure is
            ...
            for (size_t j = 0; j < left; j++)
            {
                ...
                {
                    // Hit a failure; report the subbuffer at this address as
                    // having failed
                    size_t sbThatFailed =
                    find_subbuffer_by_index(subBuffers, numSubBuffers, i + j);
                    if (sbThatFailed == numSubBuffers)
                    {
                        log_error("ERROR: Validation failure outside of a "
                                  "sub-buffer! (Shouldn't be possible, but it "
                                  "happened at index %ld out of %ld...)\n",
                                  i + j, mainSize);
                        // Since this is a nonsensical, don't bother continuing
                        // to check (we will, however, print our map of
                        // sub-buffers for comparison)
                    ...
                    log_error("ERROR: Validation failure on sub-buffer %ld "
                              "(start: %ld, length: %ld)\n",
                              sbThatFailed, subBuffers[sbThatFailed].mOrigin,
                              subBuffers[sbThatFailed].mSize);
                    size_t newPos = subBuffers[sbThatFailed].mOrigin
                    + subBuffers[sbThatFailed].mSize - 1;
                    i = newPos & ~65535;
                    j = newPos - i;
                    numErrors++;
                }
                                ...
                                cl_command_queue queue, int num_elements)
{
    ...
    // Determine the alignment of the device so we can make sure sub buffers are
    // valid
    ...
    return test_sub_buffers_read_write_core(context, queue, queue, mainSize,
                                            addressAlign);
}

// This test performs the same basic operations as sub_buffers_read_write, but
// instead of a single device, it creates a context and buffer shared between
// two devices, then executes commands on queues for each device to ensure that
// everything still operates as expected.
int test_sub_buffers_read_write_dual_devices(cl_device_id deviceID,
                                             cl_context context,
                                             ...
                                             int num_elements)
{
    ...
    if (otherDevice == NULL)
    {
        log_error("ERROR: Unable to obtain a second device for sub-buffer "
                  "dual-device test.\n");
        return -1;
    }
    if (otherDevice == deviceID)
    {
        log_info("Note: Unable to run dual-device sub-buffer test (only one "
                 "device available). Skipping test (implicitly passing).\n");
        return 0;
    }
...
#endif
    error = clGetDeviceInfo(otherDevice, CL_DEVICE_NAME, param_size,
                            &device_name[0], NULL);
    test_error(error, "Error obtaining device name");
    ...
    clContextWrapper testingContext =
    clCreateContext(NULL, 2, devices, NULL, NULL, &error);
    test_error(error, "Unable to create shared context");

    // Create two queues (can't use the existing one, because it's on the wrong
    // context)
    clCommandQueueWrapper queue1 =
    clCreateCommandQueue(testingContext, deviceID, 0, &error);
    test_error(error, "Unable to create command queue on main device");

    clCommandQueueWrapper queue2 =
    clCreateCommandQueue(testingContext, otherDevice, 0, &error);
    test_error(error, "Unable to create command queue on secondary device");

    // Determine the reasonable buffer size and address alignment that applies
    // to BOTH devices
    ...
    error =
    clGetDeviceInfo(deviceID, CL_DEVICE_MEM_BASE_ADDR_ALIGN,
                    sizeof(addressAlign1Bits), &addressAlign1Bits, NULL);
    test_error(error, "Unable to get main device's address alignment");

    error =
    clGetDeviceInfo(otherDevice, CL_DEVICE_MEM_BASE_ADDR_ALIGN,
                    sizeof(addressAlign2Bits), &addressAlign2Bits, NULL);
    ...
    // Finally time to run!
    return test_sub_buffers_read_write_core(testingContext, queue1, queue2,
                                            maxBuffer1, addressAlign1);
}

cl_int read_buffer_via_kernel(cl_context context, cl_command_queue queue,
                              cl_mem buffer, size_t length, cl_char *outResults)
{
    const char *kernelCode[] = { "__kernel void readTest( __global char "
                                 "*inBuffer, __global char *outBuffer )\n"
                                 "{\n"
                                 "    int tid = get_global_id(0);\n"
    ...
    if (create_single_kernel_helper(context, &program, &kernel, 1, kernelCode,
                                    "readTest"))
    {
        return -1;
    }
    ...
    clCreateBuffer(context, CL_MEM_READ_WRITE, length, NULL, &error);
    ...
    error = clEnqueueReadBuffer(queue, outStream, CL_TRUE, 0, length,
                                outResults, 0, NULL, NULL);
                                 ...
                                 cl_command_queue queue, int num_elements)
{
    ...
    mainBuffer =
    clCreateBuffer(context, CL_MEM_READ_WRITE, mainSize, NULL, &error);
    test_error(error, "Unable to create test main buffer");
    ...
    // Create some sub-buffers to use. Note: they don't have to not overlap (we
    // actually *want* them to overlap)
    for (size_t i = 0; i < 16; i++)
    {
        size_t offset =
        get_random_size_t(0, mainSize / addressAlign, Action::GetRandSeed())
        * addressAlign;
        size_t size = get_random_size_t(1, (mainSize - offset) / addressAlign,
                                        Action::GetRandSeed())
        * addressAlign;

        error =
        subBuffers[i].Allocate(mainBuffer, CL_MEM_READ_ONLY, offset, size);
        test_error(error, "Unable to allocate sub buffer");
    }
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        {
            size_t pt = (sbMap[j] < 0)
            ? (subBuffers[-sbMap[j]].mOrigin + subBuffers[-sbMap[j]].mSize)
            : subBuffers[sbMap[j]].mOrigin;
            if (subBuffers[i].mOrigin < pt)
            {
                // Origin is before this part of the map, so move map forward so
                // we can insert
    ...
    for (int i = 0; i < 32; i++)
    {
        ...
        {
            overlap++;
            if (overlap > 1) delta -= (long long)(subBuffers[sbMap[i]].mOrigin);
            if (overlap > maxOverlap) maxOverlap = overlap;
        }
        else
        {
            if (overlap > 1)
                delta += (long long)(subBuffers[-sbMap[i] - 1].mOrigin
                                     + subBuffers[-sbMap[i] - 1].mSize);
            overlap--;
        }
    }

    log_info("\tTesting %d sub-buffers with %lld overlapping Kbytes (%d%%; as "
             "many as %ld buffers overlapping at once)\n",
    ...
    error = clEnqueueWriteBuffer(queue, mainBuffer, CL_TRUE, 0, mainSize,
                                 contents, 0, NULL, NULL);
    test_error(error, "Unable to write to main buffer");

    // Now read from each sub-buffer and check to make sure that they make sense
    // w.r.t. the main contents
    ...
    for (size_t i = 0; i < 16; i++)
    {
        ...
        if (which)
            error =
            clEnqueueReadBuffer(queue, subBuffers[i], CL_TRUE, 0,
                                subBuffers[i].mSize, tempBuffer, 0, NULL, NULL);
        else
            error = read_buffer_via_kernel(context, queue, subBuffers[i],
                                           subBuffers[i].mSize, tempBuffer);
        test_error(error, "Unable to read sub buffer contents");

        if (memcmp(tempBuffer, contents + subBuffers[i].mOrigin,
                   subBuffers[i].mSize)
            != 0)
        {
