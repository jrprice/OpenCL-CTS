...
int test_array_info_size(cl_device_id deviceID, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    ...
    memobj = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                            elementSize * w * h * d, NULL, &err);
    test_error(err, "clCreateBuffer failed.");

    err = clGetMemObjectInfo(memobj, CL_MEM_SIZE, sizeof(size_t),
                             (void *)&retSize, NULL);
    if (err)
    {
