...
//----- the test functions
static int test_buffer_map_read(cl_device_id deviceID, cl_context context,
                                ...
                                int (*fn)(void *, int))
{
    ...
    if (!gHasLong && strstr(type, "long")) return 0;
    ...
    for (src_flag_id = 0; src_flag_id < NUM_FLAGS; src_flag_id++)
    {
        ...
        for (i = 0; i < loops; i++)
        {
            ...
            err = get_max_common_work_group_size(context, kernel[i], threads[0],
                                                 &localThreads[0]);
            ...
            mappedPtr =
            clEnqueueMapBuffer(queue, buffers[i], CL_TRUE, CL_MAP_READ, 0,
            ...
            err = clEnqueueUnmapMemObject(queue, buffers[i], mappedPtr, 0, NULL,
                                          NULL);
...
#define DECLARE_LOCK_TEST(type, realType)                                      \
    int test_buffer_map_read_##type(cl_device_id deviceID, cl_context context, \
                                    cl_command_queue queue, int num_elements)  \
    {                                                                          \
        return test_buffer_map_read(deviceID, context, queue, num_elements,    \
...
int test_buffer_map_read_struct(cl_device_id deviceID, cl_context context,
                                cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
