...
static int test_copy(cl_command_queue queue, cl_context context,
                     int num_elements, MTdata d)
{
    cl_mem buffers[2];
    ...
    for (src_flag_id = 0; src_flag_id < NUM_FLAGS; src_flag_id++)
    {
        for (dst_flag_id = 0; dst_flag_id < NUM_FLAGS; dst_flag_id++)
        {
                ...
                && !(flag_set[src_flag_id] & CL_MEM_COPY_HOST_PTR))
            {
                ...
                queue, buffers[0], CL_TRUE, 0, sizeof(cl_int) * num_elements,
            ...
            err =
            clEnqueueCopyBuffer(queue, buffers[0], buffers[1], 0, 0,
...
static int testPartialCopy(cl_command_queue queue, cl_context context,
                           int num_elements, cl_uint srcStart, cl_uint dstStart,
                           int size, MTdata d)
{
    cl_mem buffers[2];
    int *inptr, *outptr;
    ...
    for (src_flag_id = 0; src_flag_id < NUM_FLAGS; src_flag_id++)
    {
        for (dst_flag_id = 0; dst_flag_id < NUM_FLAGS; dst_flag_id++)
        {
            ...
            if ((flag_set[src_flag_id] & CL_MEM_USE_HOST_PTR)
                ...
                buffers[0] =
                clCreateBuffer(context, flag_set[src_flag_id],
                               sizeof(cl_int) * num_elements, inptr, &err);
            else
                buffers[0] =
                clCreateBuffer(context, flag_set[src_flag_id],
                               sizeof(cl_int) * num_elements, NULL, &err);
            ...
            if ((flag_set[dst_flag_id] & CL_MEM_USE_HOST_PTR)
                ...
                buffers[1] =
                clCreateBuffer(context, flag_set[dst_flag_id],
                               sizeof(cl_int) * num_elements, outptr, &err);
            else
                buffers[1] =
                clCreateBuffer(context, flag_set[dst_flag_id],
                               sizeof(cl_int) * num_elements, NULL, &err);
            ...
            dstStart * sizeof(cl_int), sizeof(cl_int) * size, 0, NULL, NULL);
            if (err != CL_SUCCESS)
            {
                     ...
                     cl_command_queue queue, int num_elements)
{
    ...
    if (test_copy(queue, context, num_elements, d)) err++;
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if (test_copy(queue, context, size, d)) err++;
    }
...
int test_buffer_partial_copy(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    int i, err = 0;
    int size;
