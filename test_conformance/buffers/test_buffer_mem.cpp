                              ...
                              cl_command_queue queue, int num_elements)
{
    ...
    buffers[0] = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
                                sizeof(cl_int) * num_elements, NULL, &err);
    if (err != CL_SUCCESS)
    {
        print_error(err, "clCreateBuffer failed\n");
...
int test_mem_read_only_flags(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    cl_mem buffers[2];
    int *inptr, *outptr;
    ...
    {
        print_error(
        err, " clCreateBuffer failed to create MEM_ALLOC_GLOBAL_POOL array\n");
        clReleaseMemObject(buffers[0]);
        align_free((void *)inptr);
        align_free((void *)outptr);
        return -1;
...
int test_mem_copy_host_flags(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    cl_mem buffers[1];
    int *ptr;
    ...
    clCreateBuffer(context, CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE,
                   sizeof(cl_int) * num_elements, (void *)ptr, &err);
    if (err != CL_SUCCESS)
    {
    ...
    {
        log_error(
        "CL_MEM_COPY_HOST_PTR | CL_MEM_ALLOC_CONSTANT_POOL failed.\n");
        clReleaseMemObject(buffers[0]);
        clReleaseKernel(kernel[0]);
        clReleaseProgram(program[0]);
        align_free((void *)ptr);
