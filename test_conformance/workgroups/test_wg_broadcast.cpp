...
"__kernel void test_wg_broadcast_1D(global float *input, global float "
"*output)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
"    float result = work_group_broadcast(input[tid], get_group_id(0) % "
"get_local_size(0));\n"
"    output[tid] = result;\n"
"}\n";

const char *wg_broadcast_2D_kernel_code =
"__kernel void test_wg_broadcast_2D(global float *input, global float "
"*output)\n"
"{\n"
...
"__kernel void test_wg_broadcast_3D(global float *input, global float "
"*output)\n"
"{\n"
"    size_t tid_x = get_global_id(0);\n"
"    size_t tid_y = get_global_id(1);\n"
...
"\n"
"    size_t indx = (tid_z * get_global_size(1) * get_global_size(0)) + (tid_y "
"* get_global_size(0)) + tid_x;\n"
"    float result = work_group_broadcast(input[indx], x, y, z);\n"
"    output[indx] = result;\n"
"}\n";

static int verify_wg_broadcast_1D(float *inptr, float *outptr, size_t n,
                                  size_t wg_size)
{
    size_t i, j;
    size_t group_id;
    ...
    for (i = 0, group_id = 0; i < n; i += wg_size, group_id++)
    {
        ...
        for (j = 0; j < local_size; j++)
        {
            ...
            {
                log_info(
                "work_group_broadcast: Error at %u: expected = %f, got = %f\n",
...
static int verify_wg_broadcast_2D(float *inptr, float *outptr, size_t nx,
                                  size_t ny, size_t wg_size_x, size_t wg_size_y)
{
    size_t i, j, _i, _j;
        ...
        for (_i = 0; _i < local_size_y; _i++)
        {
            for (j = 0, group_id_x = 0; j < nx; j += wg_size_x, group_id_x++)
            {
                ...
                for (_j = 0; _j < local_size_x; _j++)
                {
                    ...
                    {
                        log_info("work_group_broadcast: Error at (%u, %u): "
                                 "expected = %f, got = %f\n",
...
static int verify_wg_broadcast_3D(float *inptr, float *outptr, size_t nx,
                                  size_t ny, size_t nz, size_t wg_size_x,
    ...
    for (i = 0, group_id_z = 0; i < nz; i += wg_size_z, group_id_z++)
    {
        ...
        for (_i = 0; _i < local_size_z; _i++)
        {
            for (j = 0, group_id_y = 0; j < ny; j += wg_size_y, group_id_y++)
            {
                size_t y = group_id_y % wg_size_y;
                size_t local_size_y =
                (ny - j) > wg_size_y ? wg_size_y : (ny - j);
                for (_j = 0; _j < local_size_y; _j++)
                {
                    for (k = 0, group_id_x = 0; k < nx;
                         k += wg_size_x, group_id_x++)
                    {
                        size_t x = group_id_x % wg_size_x;
                        size_t local_size_x =
                        (nx - k) > wg_size_x ? wg_size_x : (nx - k);
                        float broadcast_result =
                        inptr[(i + z) * ny * nz + (j + y) * nx + (k + x)];
                        for (_k = 0; _k < local_size_x; _k++)
                        {
                            size_t indx =
                            (i + _i) * ny * nx + (j + _j) * nx + (k + _k);
                            if (broadcast_result != outptr[indx])
                            {
                                log_info("work_group_broadcast: Error at (%u, "
                                         "%u, %u): expected = %f, got = %f\n",
                                         k + _k, j + _j, i + _i,
...
int test_work_group_broadcast_1D(cl_device_id device, cl_context context,
                                 cl_command_queue queue, int n_elems)
{
    ...
    context, &program, &kernel, 1, &wg_broadcast_1D_kernel_code,
    "test_wg_broadcast_1D", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    err = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, globalsize, wg_size, 0,
                                 NULL, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    if (verify_wg_broadcast_1D(input_ptr[0], output_ptr, num_elements,
                               wg_size[0]))
    {
...
int test_work_group_broadcast_2D(cl_device_id device, cl_context context,
                                 cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    cl_float *input_ptr[1], *p;
    ...
    err = create_single_kernel_helper_with_build_options(
    context, &program, &kernel, 1, &wg_broadcast_2D_kernel_code,
    "test_wg_broadcast_2D", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_broadcast_2D(input_ptr[0], output_ptr, globalsize[0],
                               globalsize[1], localsize[0], localsize[1]))
    {
...
int test_work_group_broadcast_3D(cl_device_id device, cl_context context,
                                 cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    cl_float *input_ptr[1], *p;
    ...
    err = create_single_kernel_helper_with_build_options(
    context, &program, &kernel, 1, &wg_broadcast_3D_kernel_code,
    "test_wg_broadcast_3D", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    err = clEnqueueNDRangeKernel(queue, kernel, 3, NULL, globalsize, localsize,
                                 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
        log_error("clEnqueueNDRangeKernel failed\n");
    ...
    if (verify_wg_broadcast_3D(input_ptr[0], output_ptr, globalsize[0],
                               globalsize[1], globalsize[2], localsize[0],
...
int test_work_group_broadcast(cl_device_id device, cl_context context,
                              cl_command_queue queue, int n_elems)
{
    int err;
