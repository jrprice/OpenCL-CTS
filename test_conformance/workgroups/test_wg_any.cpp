...
static int verify_wg_any(float *inptr, int *outptr, size_t n, size_t wg_size)
{
    size_t i, j;
    ...
    for (i = 0; i < n; i += wg_size)
    {
        ...
        for (j = 0; j < ((n - i) > wg_size ? wg_size : (n - i)); j++)
        {
            ...
            {
                log_info(
                "work_group_any: Error at %lu: expected = %d, got = %d\n",
...
int test_work_group_any(cl_device_id device, cl_context context,
                        cl_command_queue queue, int n_elems)
{
    ...
    context, &program, &kernel, 1, &wg_any_kernel_code, "test_wg_any",
    "-cl-std=CL2.0");
    if (err) return -1;
    ...
    err = clEnqueueWriteBuffer(queue, streams[0], true, 0,
                               sizeof(cl_float) * (num_elements + 1),
