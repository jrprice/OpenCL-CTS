...
"__kernel void test_wg_reduce_add_uint(global uint *input, global uint "
"*output)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
...
"__kernel void test_wg_reduce_add_long(global long *input, global long "
"*output)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
...
"__kernel void test_wg_reduce_add_ulong(global ulong *input, global ulong "
"*output)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
...
static int verify_wg_reduce_add_int(int *inptr, int *outptr, size_t n,
                                    size_t wg_size)
{
    size_t i, j;
    ...
    for (i = 0; i < n; i += wg_size)
    {
        ...
        for (j = 0; j < ((n - i) > wg_size ? wg_size : (n - i)); j++)
        {
            if (sum != outptr[i + j])
            {
                log_info("work_group_reduce_add int: Error at %u: expected = "
                         "%d, got = %d\n",
                         i + j, sum, outptr[i + j]);
                return -1;
            }
...
static int verify_wg_reduce_add_uint(unsigned int *inptr, unsigned int *outptr,
                                     size_t n, size_t wg_size)
{
    ...
    for (i = 0; i < n; i += wg_size)
    {
        ...
        for (j = 0; j < ((n - i) > wg_size ? wg_size : (n - i)); j++)
        {
            if (sum != outptr[i + j])
            {
                log_info("work_group_reduce_add uint: Error at %u: expected = "
                         "%d, got = %d\n",
                         i + j, sum, outptr[i + j]);
                return -1;
            }
...
static int verify_wg_reduce_add_long(cl_long *inptr, cl_long *outptr, size_t n,
                                     size_t wg_size)
{
    size_t i, j;
    ...
    for (i = 0; i < n; i += wg_size)
    {
        ...
        for (j = 0; j < ((n - i) > wg_size ? wg_size : (n - i)); j++)
        {
            if (sum != outptr[i + j])
            {
                log_info("work_group_reduce_add long: Error at %u: expected = "
                         "%lld, got = %lld\n",
                         i + j, sum, outptr[i + j]);
                return -1;
            }
...
static int verify_wg_reduce_add_ulong(cl_ulong *inptr, cl_ulong *outptr,
                                      size_t n, size_t wg_size)
{
    size_t i, j;
    ...
    for (i = 0; i < n; i += wg_size)
    {
        ...
        for (j = 0; j < ((n - i) > wg_size ? wg_size : (n - i)); j++)
        {
            if (sum != outptr[i + j])
            {
                log_info("work_group_reduce_add ulong: Error at %u: expected = "
                         "%llu, got = %llu\n",
                         i + j, sum, outptr[i + j]);
                return -1;
            }
...
int test_work_group_reduce_add_int(cl_device_id device, cl_context context,
                                   cl_command_queue queue, int n_elems)
{
    ...
    context, &program, &kernel, 1, &wg_reduce_add_kernel_code_int,
    "test_wg_reduce_add_int", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_reduce_add_int(input_ptr[0], output_ptr, num_elements,
                                 wg_size[0]))
    {
...
int test_work_group_reduce_add_uint(cl_device_id device, cl_context context,
                                    cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    ...
    err = create_single_kernel_helper_with_build_options(
    context, &program, &kernel, 1, &wg_reduce_add_kernel_code_uint,
    "test_wg_reduce_add_uint", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_reduce_add_uint(input_ptr[0], output_ptr, num_elements,
                                  wg_size[0]))
    {
...
int test_work_group_reduce_add_long(cl_device_id device, cl_context context,
                                    cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    ...
    err = create_single_kernel_helper_with_build_options(
    context, &program, &kernel, 1, &wg_reduce_add_kernel_code_long,
    "test_wg_reduce_add_long", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_reduce_add_long(input_ptr[0], output_ptr, num_elements,
                                  wg_size[0]))
    {
...
int test_work_group_reduce_add_ulong(cl_device_id device, cl_context context,
                                     cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    ...
    err = create_single_kernel_helper_with_build_options(
    context, &program, &kernel, 1, &wg_reduce_add_kernel_code_ulong,
    "test_wg_reduce_add_ulong", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_reduce_add_ulong(input_ptr[0], output_ptr, num_elements,
                                   wg_size[0]))
    {
