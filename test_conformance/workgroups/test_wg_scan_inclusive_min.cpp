...
"__kernel void test_wg_scan_inclusive_min_int(global int *input, global int "
"*output)\n"
"{\n"
...
"__kernel void test_wg_scan_inclusive_min_uint(global uint *input, global uint "
"*output)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
...
"__kernel void test_wg_scan_inclusive_min_long(global long *input, global long "
"*output)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
...
"__kernel void test_wg_scan_inclusive_min_ulong(global ulong *input, global "
"ulong *output)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
...
static int verify_wg_scan_inclusive_min_int(int *inptr, int *outptr, size_t n,
                                            size_t wg_size)
{
    ...
    for (j = 0; j < n; j += wg_size)
    {
        ...
        for (i = 0; i < m; ++i)
        {
            ...
            if (outptr[j + i] != min_)
            {
                log_info("work_group_scan_inclusive_min int: Error at %u: "
                         "expected = %d, got = %d\n",
...
static int verify_wg_scan_inclusive_min_uint(unsigned int *inptr,
...
static int verify_wg_scan_inclusive_min_long(cl_long *inptr, cl_long *outptr,
                                             size_t n, size_t wg_size)
{
    ...
    for (j = 0; j < n; j += wg_size)
    {
        ...
        for (i = 0; i < m; ++i)
        {
            ...
            if (outptr[j + i] != min_)
            {
                log_info("work_group_scan_inclusive_min long: Error at %u: "
...
static int verify_wg_scan_inclusive_min_ulong(cl_ulong *inptr, cl_ulong *outptr,
                                              size_t n, size_t wg_size)
{
    ...
    for (j = 0; j < n; j += wg_size)
    {
        ...
        for (i = 0; i < m; ++i)
        {
            ...
            if (outptr[j + i] != min_)
            {
                log_info("work_group_scan_inclusive_min ulong: Error at %u: "
...
int test_work_group_scan_inclusive_min_int(cl_device_id device,
                                           cl_context context,
                                           cl_command_queue queue, int n_elems)
{
    ...
    context, &program, &kernel, 1, &wg_scan_inclusive_min_kernel_code_int,
    "test_wg_scan_inclusive_min_int", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_scan_inclusive_min_int(input_ptr[0], output_ptr, num_elements,
                                         wg_size[0]))
    {
...
int test_work_group_scan_inclusive_min_uint(cl_device_id device,
                                            cl_context context,
                                            cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    ...
    err = create_single_kernel_helper_with_build_options(
    context, &program, &kernel, 1, &wg_scan_inclusive_min_kernel_code_uint,
    "test_wg_scan_inclusive_min_uint", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_scan_inclusive_min_uint(input_ptr[0], output_ptr,
...
int test_work_group_scan_inclusive_min_long(cl_device_id device,
                                            cl_context context,
                                            cl_command_queue queue, int n_elems)
{
    cl_mem streams[2];
    ...
    err = create_single_kernel_helper_with_build_options(
    context, &program, &kernel, 1, &wg_scan_inclusive_min_kernel_code_long,
    "test_wg_scan_inclusive_min_long", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_scan_inclusive_min_long(input_ptr[0], output_ptr,
                                          num_elements, wg_size[0]))
    {
...
int test_work_group_scan_inclusive_min_ulong(cl_device_id device,
                                             cl_context context,
                                             ...
                                             int n_elems)
{
    ...
    err = create_single_kernel_helper_with_build_options(
    context, &program, &kernel, 1, &wg_scan_inclusive_min_kernel_code_ulong,
    "test_wg_scan_inclusive_min_ulong", "-cl-std=CL2.0");
    if (err) return -1;
    ...
    if (verify_wg_scan_inclusive_min_ulong(input_ptr[0], output_ptr,
                                           num_elements, wg_size[0]))
    {
                                       ...
                                       cl_command_queue queue, int n_elems)
{
    ...
    if (err) return err;
    err =
    test_work_group_scan_inclusive_min_long(device, context, queue, n_elems);
    if (err) return err;
    err =
    test_work_group_scan_inclusive_min_ulong(device, context, queue, n_elems);
    return err;
}
