...
static int verify_wg_all(float *inptr, int *outptr, size_t n, size_t wg_size)
{
    size_t i, j;
    ...
    for (i = 0; i < n; i += wg_size)
    {
        ...
        for (j = 0; j < ((n - i) > wg_size ? wg_size : (n - i)); j++)
        {
            ...
            {
                log_info(
                "work_group_all: Error at %lu: expected = %d, got = %d\n",
...
int test_work_group_all(cl_device_id device, cl_context context,
                        cl_command_queue queue, int n_elems)
{
    ...
    context, &program, &kernel, 1, &wg_all_kernel_code, "test_wg_all",
    "-cl-std=CL2.0");
    if (err) return -1;
