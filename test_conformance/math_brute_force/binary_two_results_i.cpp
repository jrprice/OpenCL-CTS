...
extern "C"
#endif
const vtbl _binary_two_results_i = { "binary_two_results_i",
            ...
            for (k = gMinVectorSizeIndex; k < gMaxVectorSizeIndex; k++)
            {
                ...
                // if( t[j] != q[j] || t2[j] != q2[j] )
                {
                    ...
                    if (fail)
                    {
                        ...
                        "({0x%8.8x, 0x%8.8x}): *{%a, %d} ({0x%8.8x, 0x%8.8x}) "
                        "vs. {%a, %d} ({0x%8.8x, 0x%8.8x})\n",
            ...
            for (k = gMinVectorSizeIndex; k < gMaxVectorSizeIndex; k++)
            {
                ...
                // if( t[j] != q[j] || t2[j] != q2[j] )
                {
                    ...
                    if (fail)
                    {
                        ...
                        "%.13la} ({ 0x%16.16llx, 0x%16.16llx}): *{%.13la, %d} "
                        "({ 0x%16.16llx, 0x%8.8x}) vs. {%.13la, %d} ({ "
                        "0x%16.16llx, 0x%8.8x})\n",
