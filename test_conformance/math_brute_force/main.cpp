...
char *gBasicDoubleFuncs[] = { "add",      "assignment",  "divide",
                              "isequal",  "isgreater",   "isgreaterequal",
                              "isless",   "islessequal", "isnotequal",
                              "multiply", "sqrt",        "subtract" };
...
#if defined(__APPLE__)
        {
            if (gHasBasicDouble && NULL != func_data->vtbl_ptr->DoubleTestFunc
                && NULL != func_data->dfunc.p)
            {
                // Disable fast-relaxed-math for double precision floating-point
                int testFastRelaxedTmp = gTestFastRelaxed;
...
test_status InitCL(cl_device_id device)
{
    ...
    size_t extSize = 0;
    if ((error =
         clGetDeviceInfo(gDevice, CL_DEVICE_EXTENSIONS, 0, NULL, &extSize)))
    {
    ...
    else
    {
        ...
        {
            if ((error = clGetDeviceInfo(gDevice, CL_DEVICE_EXTENSIONS, extSize,
                                         ext, NULL)))
            {
            ...
            else
            {
                if (strstr(ext, "cl_khr_fp64"))
                {
                    ...
                    if ((error =
                         clGetDeviceInfo(gDevice, CL_DEVICE_DOUBLE_FP_CONFIG,
                        ...
                        != (gDoubleCapabilities & DOUBLE_REQUIRED_FEATURES))
                    {
                        ...
                        if (0 == (gDoubleCapabilities & CL_FP_ROUND_TO_NEAREST))
                            strncat(list, "CL_FP_ROUND_TO_NEAREST, ",
                                    sizeof(list) - 1);
                        if (0 == (gDoubleCapabilities & CL_FP_ROUND_TO_ZERO))
                            strncat(list, "CL_FP_ROUND_TO_ZERO, ",
                                    sizeof(list) - 1);
                        if (0 == (gDoubleCapabilities & CL_FP_ROUND_TO_INF))
                            strncat(list, "CL_FP_ROUND_TO_INF, ",
                                    sizeof(list) - 1);
                        ...
                        vlog_error(
                        "ERROR: required double features are missing: %s\n",
                        list);
...
#else
                    vlog_error(
                    "FAIL: device says it supports cl_khr_fp64 but "
    ...
    {
        gOutBuffer[i] =
        clCreateBuffer(gContext, device_flags, BUFFER_SIZE, gOut[i], &error);
        ...
        gOutBuffer2[i] =
        clCreateBuffer(gContext, device_flags, BUFFER_SIZE, gOut2[i], &error);
    ...
    vlog(
    "\tCorrectly rounded divide and sqrt supported for floats? %s\n",
    ...
    vlog(
    "\tTesting with correctly rounded float divide and sqrt? %s\n",
    no_yes[0 != (CL_FP_CORRECTLY_ROUNDED_DIVIDE_SQRT & gFloatCapabilities)]);
...
int InitILogbConstants(void)
{
    ...
    cl_program query;
    error =
    create_single_kernel_helper(gContext, &query, NULL, 1, &kernel, NULL);
    if (NULL == query || error)
    {
        vlog_error("Error: Unable to create program to get FP_ILOGB0 and "
    ...
    if ((error = clSetKernelArg(k, 0, sizeof(gOutBuffer[gMinVectorSizeIndex]),
    ...
    size_t dim = 1;
    if ((error =
         clEnqueueNDRangeKernel(gQueue, k, 1, NULL, &dim, NULL, 0, NULL, NULL)))
    {
    ...
    if ((error =
         clEnqueueReadBuffer(gQueue, gOutBuffer[gMinVectorSizeIndex], CL_TRUE,
                             0, sizeof(data), &data, 0, NULL, NULL)))
    {
...
int IsTininessDetectedBeforeRounding(void)
{
    ...
    if (error != CL_SUCCESS)
    {
        vlog_error("Error: Unable to create program to detect how tininess is "
    ...
    cl_kernel k =
    clCreateKernel(query, "IsTininessDetectedBeforeRounding", &error);
    if (NULL == k || error)
    {
               ...
               cl_program *p)
{
    ...
    *k = clCreateKernel(*p, name, &error);
    if (NULL == *k || error)
    {
        ...
        clGetProgramBuildInfo(*p, gDevice, CL_PROGRAM_BUILD_LOG, sizeof(buffer),
                              buffer, NULL);
        vlog_error("Log: %s\n", buffer);
        clReleaseProgram(*p);
        return error;
...
static int IsInRTZMode(void)
{
    ...
    if (error != CL_SUCCESS)
    {
        vlog_error("Error: Unable to create program to detect RTZ mode for the "
                   "device. (%d)",
                   error);
        return error;
    }
    ...
    if (NULL == k || error)
    {
        vlog_error("Error: Unable to create kernel to gdetect RTZ mode for the "
                   "device. Err = %d",
                   error);
        return error;
    }
    ...
    {
        vlog_error(
        "Error: unable to read RTZ mode data from the device. Err = %d", error);
        return error;
    }
...
result is correctly,
  ...
  // would ensure that cases like fabs(reference) > FLT_MAX are weeded out
before we get here.
  // Otherwise, we'll return inf ulp error here, for what are otherwise
correctly rounded
  // results.
    ...
    // reference is a normal power of two or a zero
    int ulp_exp =  HALF_MANT_DIG - 1 - MAX( ilogb( reference) - 1,
HALF_MIN_EXP-1 );
