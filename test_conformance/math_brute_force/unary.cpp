...
typedef struct TestInfo
{
    ...
    *tinfo; // An array of thread specific information for each worker thread
    ...
    int
    isRangeLimited; // 1 if the function is only to be evaluated over a range
...
int TestFunc_Float_Float(const Func *f, MTdata d)
{
    ...
    if (0 == strcmp(f->name, "half_sin") || 0 == strcmp(f->name, "half_cos"))
    {
        ...
        + test_info.ulps
        * (FLT_EPSILON
           / 2.0f); // out of range results from finite inputs must be in [-1,1]
    }
...
static cl_int TestFloat(cl_uint job_id, cl_uint thread_id, void *data)
{
    ...
    for (j = 0; j < buffer_elements; j++)
    {
        ...
        if (gTestFastRelaxed)
        {
            ...
            {
                if (fabs(p_j) > M_PI) p[j] = NAN;
            }

            if (strcmp(fname, "reciprocal") == 0)
            {
                if (fabs(p_j) > 0x7E800000) // the domain of the function is
                                            // [2^-126,2^126]
                    p[j] = NAN;
            }
        }
