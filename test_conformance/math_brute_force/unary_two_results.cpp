...
extern "C"
#endif
const vtbl _unary_two_results = { "unary_two_results", TestFunc_Float2_Float,
...
int TestFunc_Float2_Float(const Func *f, MTdata d)
{
    ...
    if (gTestFastRelaxed) float_ulps = f->relaxed_error;
...
*/
    ...
    for (i = 0; i < (1ULL << 32); i += step)
    {
        ...
        for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
        {
            ...
            if ((error =
                 clSetKernelArg(kernels[j], 2, sizeof(gInBuffer), &gInBuffer)))
            {
                LogBuildError(programs[j]);
                goto exit;
            }
        ...
        for (j = 0; j < bufferSize / sizeof(float); j++)
        {
            for (k = gMinVectorSizeIndex; k < gMaxVectorSizeIndex; k++)
            {
                ...
                if (t[j] != q[j] || t2[j] != q2[j])
                {
                    ...
                    double, float); // If we are in fast relaxed math, we have a
                                    // different calculation for the subnormal
                                    // threshold.
                    ...
                    int fail =
                    !(fabsf(err) <= float_ulps && fabsf(err2) <= float_ulps);
                    ...
                    if (ftz)
                    {
                        ...
                        if ((*isFloatResultSubnormalPtr)(correct, float_ulps))
                        {
                            ...
                            {
                                fail = fail
                                && !(test == 0.0f && fabsf(err2) <= float_ulps);
                                if (!fail) err = 0.0f;
                            }
                        }
            ...
            for (k = gMinVectorSizeIndex; k < gMaxVectorSizeIndex; k++)
            {
                ...
                if (t[j] != q[j] || t2[j] != q2[j])
                {
                    ...
                    if (ftz)
                    {
                        ...
                        {
                            fail = fail
                            && !(test2 == 0.0f && fabsf(err) <= f->double_ulps);
                            if (!fail) err2 = 0.0f;
                        }
                    ...
                    if (fail)
                    {
                        ...
                        f->name, sizeNames[k], err, err2, ((double *)gIn)[j],
                        ((double *)gOut_Ref)[j], ((double *)gOut_Ref2)[j], test,
                        test2);
                        error = -1;
                        goto exit;
                    }
