...
#define ENTRY_EXT(_name, _ulp, _embedded_ulp, _relaxed_ulp, _rmode, _type)     \
    {                                                                          \
        STRINGIFY(_name), STRINGIFY(_name), { NULL }, { NULL }, { NULL },      \
        _ulp, _ulp, _embedded_ulp, _relaxed_ulp, _rmode, RELAXED_ON, _type     \
    }
...
#define OPERATOR_ENTRY(_name, _operator, _ulp, _embedded_ulp, _rmode, _type)   \
    {                                                                          \
        ...
        _embedded_ulp, INFINITY, _rmode, RELAXED_OFF, _type                    \
    }
#define unaryF NULL
...
#define ENTRY(_name, _ulp, _embedded_ulp, _rmode, _type)                       \
    {                                                                          \
        ...
        { (void*)reference_##_name##l }, { (void*)reference_##_name }, _ulp,   \
        _ulp, _embedded_ulp, INFINITY, _rmode, RELAXED_OFF, _type              \
    }
#define ENTRY_EXT(_name, _ulp, _embedded_ulp, _relaxed_ulp, _rmode, _type)     \
    {                                                                          \
        STRINGIFY(_name), STRINGIFY(_name), { (void*)reference_##_name },      \
        ...
        _relaxed_ulp, _rmode, RELAXED_ON, _type                                \
    }
#define HALF_ENTRY(_name, _ulp, _embedded_ulp, _rmode, _type)                  \
    {                                                                          \
...
#define OPERATOR_ENTRY(_name, _operator, _ulp, _embedded_ulp, _rmode, _type)   \
    {                                                                          \
        ...
        { (void*)reference_##_name##l }, { NULL }, _ulp, _ulp, _embedded_ulp,  \
...
extern const vtbl
_binary_nextafter; // float foo( float, float ), special handling for nextafter
...
const Func functionList[] = {
    ...
    ENTRY_EXT(cos, 4.0f, 4.0f, 0.00048828125f, FTZ_OFF,
              unaryF), // relaxed ulp 2^-11
    ...
    ENTRY_EXT(exp, 3.0f, 4.0f, 3.0f, FTZ_OFF,
              unaryF), // relaxed error is actually overwritten in unary.c as it
                       // is 3+floor(fabs(2*x))
    ENTRY_EXT(exp2, 3.0f, 4.0f, 3.0f, FTZ_OFF,
              unaryF), // relaxed error is actually overwritten in unary.c as it
                       // is 3+floor(fabs(2*x))
    ENTRY_EXT(exp10, 3.0f, 4.0f, 8192.0f, FTZ_OFF,
              unaryF), // relaxed error is actually overwritten in unary.c as it
                       // is 3+floor(fabs(2*x)) in derived mode,
    ...
    ENTRY_EXT(log, 3.0f, 4.0f, 4.76837158203125e-7f, FTZ_OFF,
              unaryF), // relaxed ulp 2^-21
    ENTRY_EXT(log2, 3.0f, 4.0f, 4.76837158203125e-7f, FTZ_OFF,
              unaryF), // relaxed ulp 2^-21
    ENTRY(log10, 3.0f, 4.0f, FTZ_OFF, unaryF),
    ENTRY(log1p, 2.0f, 4.0f, FTZ_OFF, unaryF),
    ENTRY(logb, 0.0f, 0.0f, FTZ_OFF, unaryF),
    ENTRY_EXT(mad, INFINITY, INFINITY, INFINITY, FTZ_OFF,
              mad_function), // in fast-relaxed-math mode it has to be either
    ...
    ENTRY_EXT(
    pow, 16.0f, 16.0f, 8192.0f, FTZ_OFF,
    binaryF), // in derived mode the ulp error is calculated as exp2(y*log2(x))
              // and in non-derived it is the same as half_pow
    ...
    ENTRY_EXT(sin, 4.0f, 4.0f, 0.00048828125f, FTZ_OFF,
              unaryF), // relaxed ulp 2^-11
    ENTRY_EXT(sincos, 4.0f, 4.0f, 0.00048828125f, FTZ_OFF,
              unaryF_two_results), // relaxed ulp 2^-11
    ...
    { "sqrt_cr",
      ...
      unaryF },
    ENTRY_EXT(
    tan, 5.0f, 5.0f, 8192.0f, FTZ_OFF,
    unaryF), // in derived mode it the ulp error is calculated as sin/cos and in
             // non-derived mode it is the same as half_tan.
