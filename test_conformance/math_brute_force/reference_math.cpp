...
#define isfinite(x)                                                            \
    (sizeof(x) == sizeof(float)                                                \
     ? fabsf(x) < INFINITY                                                     \
     : sizeof(x) == sizeof(double) ? fabs(x) < INFINITY : fabsl(x) < INFINITY)
#endif
...
#ifndef isnormal
#define isnormal(x)                                                            \
    (sizeof(x) == sizeof(float) ? (fabsf(x) < INFINITY && fabsf(x) >= FLT_MIN) \
                                : sizeof(x) == sizeof(double)                  \
     ? (fabs(x) < INFINITY && fabs(x) >= DBL_MIN)                              \
...
static inline void add64(cl_ulong *p, cl_ulong c, int *exponent)
{
    ...
    if (carry)
    {
        ...
        c |=
        carry | 0x8000000000000000ULL; // or in carry bit, and sticky bit. The
...
float reference_fma(float a, float b, float c, int shouldFlush)
{
        ...
        0 == (uc.u & ~kMSB)) // c == 0, defeat host FTZ behavior
    {
        ...
        a, b); // some risk that the compiler will insert a non-compliant fma
               // here on some platforms.
        a = (float)reference_add(
        a, c); // We use STDC FP_CONTRACT OFF above to attempt to defeat that.
...
double reference_tanpi(double x)
{
    ...
    double nearest = reference_rint(
    z); // round to nearest even places n + 0.5 values in the right place for us
             ...
             + (0.001333355902958566035
                + (0.000154034191902497930
                   + (0.000015252317761038105
                      + (0.000001326283129417092 + 0.000000102593187638680 * f)
                      * f)
                   * f)
                * f)
             * f)
       ...
       + (0.166666666672817459505
          + (0.041666666612283048687
             + (0.008333330214567431435
                + (0.001389005319303770070 + 0.000198833381525156667 * f) * f)
             * f)
          * f)
       * f)
    * f;
    ...
    else if (ix < 0x40000000)
    {
        ...
        switch (i)
        {
            case 0:
                ...
                p2 =
                z * (a1 + z * (a3 + z * (a5 + z * (a7 + z * (a9 + z * a11)))));
                p = y * p1 + p2;
                r += (p - 0.5 * y);
                break;
            case 1:
...
static inline void mul128(cl_ulong a, cl_ulong b, cl_ulong *hi, cl_ulong *lo)
{
    ...
    alobhi += (aloblo >> 32)
    + (ahiblo & 0xffffffffULL); // cannot overflow: (2^32-1)^2 + 2 * (2^32-1) =
                                // (2^64 - 2^33 + 1) + (2^33 - 2) = 2^64 - 1
    *hi = ahibhi + (alobhi >> 32)
    + (ahiblo >> 32); // cannot overflow: (2^32-1)^2 + 2 * (2^32-1)   = (2^64 -
                      // 2^33 + 1) + (2^33 - 2) = 2^64 - 1
...
long double reference_tanpil(long double x)
{
    ...
    // reduce to the range [ -0.5, 0.5 ]
    long double nearest = reference_rintl(
    z); // round to nearest even places n + 0.5 values in the right place for us
    int64_t i =
...
{
    uint32_t
    X[3]; // three 32 bit integers are sufficient to represnt double in base_30
...
long double reference_acosl(long double x)
{
    ...
    tail = (long double)((cl_long)pi_bits[0]
                         - (cl_long)
