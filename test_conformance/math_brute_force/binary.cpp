...
int TestFunc_Double_Double_Double_common(const Func *f, MTdata,
                                         int isNextafter);
...
extern "C"
#endif
const vtbl _binary = { "binary", TestFunc_Float_Float_Float,
                       TestFunc_Double_Double_Double };

#if defined(__cplusplus)
extern "C"
#endif
const vtbl _binary_nextafter = { "binary_nextafter",
...
typedef struct ThreadInfo
{
    ...
    double
    maxErrorValue2; // position of the max error value (param 2).  Init to 0.
    MTdata d;
...
typedef struct TestInfo
{
    ...
    cl_kernel
    *k[VECTOR_SIZE_COUNT]; // arrays of thread-specific kernels for each worker
                           // thread:  k[vector_size][thread_id]
    ThreadInfo
...
int TestFunc_Float_Float_Float_common(const Func *f, MTdata d, int isNextafter)
{
    ...
    for (i = 0; i < test_info.threadCount; i++)
    {
        ...
        {
            test_info.tinfo[i].outBuf[j] =
            clCreateSubBuffer(gOutBuffer[j], CL_MEM_WRITE_ONLY,
                              CL_BUFFER_CREATE_TYPE_REGION, &region, &error);
    ...
    if (gMeasureTimes)
    {
        ...
        for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
        {
            ...
            for (i = 0; i < PERF_LOOP_COUNT; i++)
            {
                ...
                if ((error =
                     clEnqueueNDRangeKernel(gQueue, test_info.k[j][0], 1, NULL,
            ...
            * gComputeDevices * gSimdSize * 1e6 / (BUFFER_SIZE / sizeof(float));
...
static cl_int TestFloat(cl_uint job_id, cl_uint thread_id, void *data)
{
    ...
    for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
    {
        ...
        tinfo->tQueue, tinfo->outBuf[j], CL_FALSE, CL_MAP_WRITE, 0, buffer_size,
        0, NULL, e + j, &error);
        if (error || NULL == out[j])
        {
    ...
    for (j = gMinVectorSizeIndex; j < gMaxVectorSizeIndex; j++)
    {
        ...
        if ((error =
             clSetKernelArg(kernel, 1, sizeof(tinfo->inBuf), &tinfo->inBuf)))
        {
            LogBuildError(program);
            return error;
        }
...
#define ref_func(s, s2) (copysign_test ? func.f_ff_f(s, s2) : func.f_ff(s, s2))
    ...
    {
        out[j] = (cl_uint *)clEnqueueMapBuffer(
        tinfo->tQueue, tinfo->outBuf[j], CL_FALSE, CL_MAP_READ, 0, buffer_size,
        0, NULL, NULL, &error);
        if (error || NULL == out[j])
        {
            vlog_error("Error: clEnqueueMapBuffer %d failed! err: %d\n", j,
            ...
            for (k = gMinVectorSizeIndex; k < gMaxVectorSizeIndex; k++)
            {
                ...
                if (t[j] != q[j])
                {
                    ...
                    if (fail)
                    {
                        ...
                        name, sizeNames[k], err, s[j], ((cl_uint *)s)[j], s2[j],
                        ((cl_uint *)s2)[j], r[j], test, ((cl_uint *)&test)[0],
                        j);
                        error = -1;
                        goto exit;
                    }
                                         ...
                                         int isNextafter)
{
    ...
    if (gWimpyMode)
    {
        ...
        / (sizeof(cl_double) * RoundUpToNextPowerOfTwo(test_info.threadCount));
        test_info.scale =
        (cl_uint)sizeof(cl_double) * 2 * gWimpyReductionFactor;
    }
    test_info.step = (cl_uint)test_info.subBufferSize * test_info.scale;
    if (test_info.step / test_info.subBufferSize != test_info.scale)
    {
