...
const char *test_kernels[] = { "__kernel void kernelA(__global int *dst)\n"
                               "{\n"
                               "\n"
                                ...
                                const cl_device_id *parentDevice)
{
    ...
    context, outProgram, numKernelLines, kernelProgram);
    ...
    int printedSource = 0;
    error = clBuildProgram(*outProgram, ((parentDevice == NULL) ? 0 : 1),
                           parentDevice, NULL, NULL, NULL);
    if (error != CL_SUCCESS)
    {
        unsigned int i;
    ...
    for (z = 0; z < deviceCount; z++)
    {
        ...
        error =
        clGetProgramBuildInfo(*outProgram, devices[z], CL_PROGRAM_BUILD_STATUS,
        ...
        if (buildStatus != CL_BUILD_SUCCESS || buildProgramFailed)
        {
            char log[10240] = "";
            if (buildStatus == CL_BUILD_SUCCESS && buildProgramFailed)
                log_error("clBuildProgram returned an error, but buildStatus "
                          "is marked as CL_BUILD_SUCCESS.\n");
            ...
            error =
            clGetProgramBuildInfo(*outProgram, devices[z], CL_PROGRAM_BUILD_LOG,
                                  sizeof(log), log, NULL);
...
template <class T> class AutoDestructArray {
public:
...
int test_device_set(size_t deviceCount, size_t queueCount,
                    ...
                    cl_device_id *parentDevice = NULL)
{
    ...
    int **expectedResultsOneDevice =
    (int **)alloca(sizeof(int **) * deviceCount);
    size_t i;
    AutoDestructArray<int> autoDestruct(expectedResultsOneDeviceArray);

    for (i = 0; i < deviceCount; i++)
    {
        expectedResultsOneDevice[i] =
        expectedResultsOneDeviceArray + (i * TEST_SIZE);
    }
    ...
    {
        log_error("Number of queues (%ld) is greater than the number for which "
    ...
    for (i = 0; i < deviceCount; i++)
    {
        ...
        error =
        clGetDeviceInfo(devices[i], CL_DEVICE_NAME, 0, NULL, &deviceNameSize);
        test_error(error, "clGetDeviceInfo CL_DEVICE_NAME failed");
        char *deviceName = (char *)alloca(deviceNameSize * (sizeof(char)));
        error = clGetDeviceInfo(devices[i], CL_DEVICE_NAME, deviceNameSize,
                                deviceName, NULL);
        test_error(error, "clGetDeviceInfo CL_DEVICE_NAME failed");
    ...
    /* Create a context */
    context = clCreateContext(NULL, (cl_uint)deviceCount, devices,
                              notify_callback, NULL, &error);
    test_error(error, "Unable to create testing context");

    /* Create our kernels (they all have the same arguments so we don't need
     * multiple ones for each device) */
    if (create_single_kernel_helper(context, &program, &kernels[0], 1,
                                    test_kernels, "kernelA", parentDevice)
        != 0)
    {
        return -1;
    }
    ...
    for (i = 0; i < TEST_SIZE; i++) data[i] = genrand_int32(seed);
    ...
    for (i = 0; i < queueCount; i++)
    {
        ...
        context, devices[i % deviceCount], 0, &error);
    ...
    for (int z = 0; z < num_elements / TEST_SIZE; z++)
    {
        for (i = 0; i < queueCount; i++)
        {
            ...
            error =
            clEnqueueNDRangeKernel(queues[i], kernels[kernel_selection], 1,
                                   NULL, threads, localThreads, 0, NULL, NULL);
            test_error(error, "Kernel execution failed");

            // Update the expected results
            for (int j = 0; j < TEST_SIZE; j++)
            {
                expectedResults[j] = (kernel_selection)
                ? expectedResults[j] + 1
                : expectedResults[j] * 3;
                expectedResultsOneDevice[i % deviceCount][j] =
                (kernel_selection)
    ...
    {
        error = clEnqueueReadBuffer(queues[q], stream, CL_TRUE, 0,
                                    sizeof(cl_int) * TEST_SIZE,
                                    (char *)outputData, 0, NULL, NULL);
...
{
    int err = clGetDeviceInfo(parentDevice, CL_DEVICE_MAX_COMPUTE_UNITS,
                              sizeof(maxComputeUnits), &maxComputeUnits, NULL);
    test_error(err, "Unable to get maximal number of compute units");
    err = clGetDeviceInfo(parentDevice, CL_DEVICE_PARTITION_MAX_SUB_DEVICES,
...
int test_device_partition_type_support(
cl_device_id parentDevice, const cl_device_partition_property partitionType,
const cl_device_affinity_domain affinityDomain)
{
    typedef std::vector<cl_device_partition_property> properties_t;
    properties_t supportedProps(
    3); // only 3 types defined in the spec (but implementation can define more)
    size_t const propSize =
    sizeof(cl_device_partition_property); // Size of one property in bytes.
    size_t size; // size of all properties in bytes.
    cl_int err;
    size = 0;
    err = clGetDeviceInfo(parentDevice, CL_DEVICE_PARTITION_PROPERTIES, 0, NULL,
                          &size);
    if (err == CL_SUCCESS)
    {
        if (size % propSize != 0)
        {
            log_error(
            "ERROR: clGetDeviceInfo: Bad size of returned partition properties "
            "(%llu), it must me a multiply of partition property size (%llu)\n",
            llu(size), llu(propSize));
            return -1;
        }
        supportedProps.resize(size / propSize);
        size = 0;
        err = clGetDeviceInfo(parentDevice, CL_DEVICE_PARTITION_PROPERTIES,
                              supportedProps.size() * propSize,
                              &supportedProps.front(), &size);
        test_error_ret(err, "Unable to get device partition properties (2)",
                       -1);
    }
    else if (err == CL_INVALID_VALUE)
    {
        log_error("ERROR: clGetDeviceInfo: CL_DEVICE_PARTITION_PROPERTIES is "
                  "not supported.\n");
        return -1;
    }
    else
    {
        test_error_ret(err, "Unable to get device partition properties (1)",
                       -1);
    };
    for (int i = 0; i < supportedProps.size(); i++)
    {
        if (supportedProps[i] == partitionType)
        {
            if (partitionType == CL_DEVICE_PARTITION_BY_AFFINITY_DOMAIN)
            {
                ...
                err = clGetDeviceInfo(parentDevice,
                                      CL_DEVICE_PARTITION_AFFINITY_DOMAIN,
                                      sizeof(supportedAffinityDomain),
                                      &supportedAffinityDomain, NULL);
                test_error(err, "Unable to get supported affinity domains");
                if (supportedAffinityDomain & affinityDomain) return 0;
            }
            else
                return 0;
        }
...
int test_partition_of_device(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements,
                             cl_device_partition_property *partition_type,
                             cl_uint starting_property, cl_uint ending_property)
{
    cl_uint maxComputeUnits;
    cl_uint maxSubDevices; // maximal number of sub-devices that can be created
                           // in one call to clCreateSubDevices
    int err = 0;

    if (init_device_partition_test(deviceID, maxComputeUnits, maxSubDevices)
        != 0)
        return -1;

    if (maxComputeUnits <= 1) return 0;
    // confirm that this devices reports how it was partitioned
    if (partition_type != NULL)
    { // if we're not the root device
        size_t psize;
        err =
        clGetDeviceInfo(deviceID, CL_DEVICE_PARTITION_TYPE, 0, NULL, &psize);
        test_error(err, "Unable to get CL_DEVICE_PARTITION_TYPE");
        cl_device_partition_property *properties_returned =
        (cl_device_partition_property *)alloca(psize);
        err = clGetDeviceInfo(deviceID, CL_DEVICE_PARTITION_TYPE, psize,
                              (void *)properties_returned, NULL);
        test_error(err, "Unable to get CL_DEVICE_PARTITION_TYPE");

        // test returned type
        for (cl_uint i = 0; i < psize / sizeof(cl_device_partition_property);
             i++)
        {
            if (properties_returned[i] != partition_type[i])
            {
                if (!(
                    partition_type[0] == CL_DEVICE_PARTITION_BY_AFFINITY_DOMAIN
                    && i == 1
                    && partition_type[1]
                    == CL_DEVICE_AFFINITY_DOMAIN_NEXT_PARTITIONABLE
                    && (properties_returned[1] == CL_DEVICE_AFFINITY_DOMAIN_NUMA
                        || properties_returned[1]
                ...
                {
                    log_error(
                    "properties_returned[%d] 0x%x != 0x%x partition_type[%d].",
                    i, properties_returned[i], partition_type[i], i);
                    return -1;
                }
            }
        } // for
...
#define PROPERTY_TYPES 8
    cl_device_partition_property partitionProp[PROPERTY_TYPES][5] = {
        ...
        { CL_DEVICE_PARTITION_BY_COUNTS, 1, maxComputeUnits - 1,
    ...
    for (cl_uint i = starting_property; i < ending_property; i++)
    {
        ...
        if (test_device_partition_type_support(deviceID, partitionProp[i][0],
                                               partitionProp[i][1])
            != 0)
        {
        ...
        err =
        clCreateSubDevices(deviceID, partitionProp[i], 0, NULL, &deviceCount);
        ...
        //  create room for 1 more device_id, so that we can put the parent
        //  device in there.
        cl_device_id *subDevices =
        (cl_device_id *)alloca(sizeof(cl_device_id) * (deviceCount + 1));
        err = clCreateSubDevices(deviceID, partitionProp[i], deviceCount,
                                 subDevices, &deviceCount);
        test_error(err, "Actual creation of sub-devices failed");

        log_info("Testing on all devices in context\n");
        err =
        test_device_set(deviceCount, deviceCount, subDevices, num_elements);
        if (err == 0)
        {
        ...
        // now, recurse and test the FIRST of these sub-devices, to make sure it
        // can be further partitioned
        err = test_partition_of_device(subDevices[0], context, queue,
                                       num_elements, partitionProp[i],
                                       starting_property, ending_property);
        if (err != 0)
        {
            printf("error! returning %d\n", err);
            return err;
...
int test_partition_equally(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    return test_partition_of_device(deviceID, context, queue, num_elements,
                                    NULL, 0, 1);
}

int test_partition_by_counts(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    return test_partition_of_device(deviceID, context, queue, num_elements,
                                    NULL, 1, 2);
...
int test_partition_by_affinity_domain_numa(cl_device_id deviceID,
                                           cl_context context,
...
int test_partition_by_affinity_domain_l4_cache(cl_device_id deviceID,
                                               cl_context context,
                                               cl_command_queue queue,
                                               int num_elements)
{
...
int test_partition_by_affinity_domain_l3_cache(cl_device_id deviceID,
                                               cl_context context,
                                               cl_command_queue queue,
                                               int num_elements)
{
...
int test_partition_by_affinity_domain_l2_cache(cl_device_id deviceID,
                                               cl_context context,
                                               cl_command_queue queue,
                                               int num_elements)
{
...
int test_partition_by_affinity_domain_l1_cache(cl_device_id deviceID,
                                               cl_context context,
                                               cl_command_queue queue,
                                               int num_elements)
{
...
int test_partition_by_affinity_domain_next_partitionable(cl_device_id deviceID,
                                                         cl_context context,
                                                         cl_command_queue queue,
                                                         int num_elements)
{
...
int test_partition_all(cl_device_id deviceID, cl_context context,
                       cl_command_queue queue, int num_elements)
{
    return test_partition_of_device(deviceID, context, queue, num_elements,
                                    NULL, 0, 8);
