...
cl_int create_linked_lists_on_device_no_map(int ci, cl_command_queue cmdq,
                                            size_t *pAllocator,
...
cl_int verify_linked_lists_on_device_no_map(int vi, cl_command_queue cmdq,
                                            cl_int *pNumCorrect,
...
// only the fine-grain with buffers mode. This is done by creating a linked list
// on a device and then verifying the correctness of the list on another device
// or the host.  This basic test is performed for all combinations of devices
// and the host that exist within the platform.  The test passes only if every
// combination passes.
int test_svm_shared_address_space_fine_grain_buffers(cl_device_id deviceID,
                                                     cl_context context2,
                                                     ...
                                                     int num_elements)
{
    ...
    &queues[0], &num_devices, CL_DEVICE_SVM_FINE_GRAIN_BUFFER);
    if (error == 1)
    ...
    context, CL_MEM_READ_WRITE | CL_MEM_SVM_FINE_GRAIN_BUFFER, sizeof(size_t),
    0);

    // this buffer holds the count of correct nodes, which is computed by the
    // verify kernel.
    cl_int *pNumCorrect = (cl_int *)clSVMAlloc(
    context, CL_MEM_READ_WRITE | CL_MEM_SVM_FINE_GRAIN_BUFFER, sizeof(cl_int),
    0);
    ...
    error |=
    clSetKernelArg(kernel_create_lists, 2, sizeof(cl_int), (void *)&ListLength);
    ...
    error |=
    clSetKernelArg(kernel_verify_lists, 2, sizeof(cl_int), (void *)&ListLength);
               ...
               // on
    {
                   ...
                   // linked list on
        {
            ...
            else
            {
                ...
                vi, queues[vi], pNumCorrect, kernel_verify_lists, ListLength,
                numLists);
                if (error) return -1;
            }
        }
