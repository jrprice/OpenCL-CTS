...
const char *shared_sub_buffers_test_kernel[] = {
    ...
    "__global Node* allocate_node(__global Node* pNodes1, __global Node* "
    "pNodes2, volatile __global int* allocation_index, size_t i)\n"
    "{\n"
    ...
    // This tests passing 4 different sub-buffers that come from two parent
    // buffers. Note that we have arguments that appear to be unused, but they
    // are required so that system knows to get all the sub-buffers on to the
    // device
    "__kernel void create_linked_lists(__global Node* pNodes_sub1, __global "
    "Node* pNodes2_sub1, __global Node* pNodes_sub2, __global Node* "
    ...
    "        pNew = allocate_node(pNodes_sub1, pNodes2_sub1, allocation_index, "
    "i);\n"
    "        pNew->global_id = i;\n"
    ...
    "    }\n"
    "}\n"
    // Note that we have arguments that appear to be unused, but they are
    // required so that system knows to get all the sub-buffers on to the device
    "__kernel void verify_linked_lists(__global Node* pNodes_sub1, __global "
    "Node* pNodes2_sub1, __global Node* pNodes_sub2, __global Node* "
    "pNodes2_sub2, volatile __global uint* num_correct, int list_length)\n"
    "{\n"
    "    size_t i = get_global_id(0);\n"
    "    __global Node *pNode = &pNodes_sub1[i];\n"
...
cl_int create_linked_lists_on_host_sb(cl_command_queue cmdq, cl_mem nodes,
                                      cl_mem nodes2, cl_int ListLength,
                                      size_t numLists)
{
    ...
    Node *pNodes = (Node *)clEnqueueMapBuffer(
    ...
    Node *pNodes2 = (Node *)clEnqueueMapBuffer(
    cmdq, nodes2, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0,
    sizeof(Node) * ListLength * numLists, 0, NULL, NULL, &error);
...
cl_int verify_linked_lists_on_host_sb(int ci, cl_command_queue cmdq,
                                      cl_mem nodes, cl_mem nodes2,
...
// This tests that shared sub-buffers can be created and that they inherit the
// flags from the parent buffer when no flags are specified. This tests that
// passing only the sub-buffers to a kernel works. The test is derived from the
// cross-buffer pointers test which tests that shared buffers are able to
// contain pointers that point to other shared buffers. This tests that all
// devices and the host share a common address space; using only the
// coarse-grain features. This is done by creating a linked list on a device and
// then verifying the correctness of the list on another device or the host. The
// linked list nodes are allocated from two different buffers this is done to
// ensure that cross buffer pointers work correctly. This basic test is
// performed for all combinations of devices and the host.
int test_svm_shared_sub_buffers(cl_device_id deviceID, cl_context context2,
                                cl_command_queue queue, int num_elements)
{
    ...
    error = create_cl_objects(deviceID, &shared_sub_buffers_test_kernel[0],
                              &context, &program, &queues[0], &num_devices,
...

    {
        ...
        clCreateBuffer(context, CL_MEM_USE_HOST_PTR,
                       sizeof(Node) * ListLength * numLists, pNodes2, &error);
        test_error(error, "clCreateBuffer failed.");
        r.origin = 0;
        r.size = nodes_bufsize / 2;
        // this should inherit the flag settings from nodes buffer
