                                   ...
                                   void *svm_pointers[], void *user_data)
{
    ...
    cl_context context;
    if (clGetCommandQueueInfo(queue, CL_QUEUE_CONTEXT, sizeof(cl_context),
                              &context, 0)
        != CL_SUCCESS)
    {
...
int test_svm_enqueue_api(cl_device_id deviceID, cl_context c,
                         cl_command_queue queue, int num_elements)
{
    ...
    error = create_cl_objects(deviceID, NULL, &context, NULL, &queues[0],
    ...
    for (const auto test_case : testTypes)
    {
        ...
        for (size_t i = 0; i < ARRAY_SIZE(typeSizes); ++i)
        {
            ...
            cl_uchar *srcBuffer =
            (cl_uchar *)clSVMAlloc(context, CL_MEM_READ_WRITE, data_size, 0);
            cl_uchar *dstBuffer =
            (cl_uchar *)clSVMAlloc(context, CL_MEM_READ_WRITE, data_size, 0);
            ...
            error =
            clEnqueueSVMMemFill(queue, srcBuffer, &fillData0[0], typeSizes[i],
                                data_size, 1, &userEvent, &eventMemFillList[0]);
            test_error(error, "clEnqueueSVMMemFill failed");
            error =
            clEnqueueSVMMemFill(queue, dstBuffer, &fillData1[0], typeSizes[i],
                                data_size, 1, &userEvent, &eventMemFillList[1]);
            test_error(error, "clEnqueueSVMMemFill failed");
            ...
            error =
            clEnqueueSVMMemcpy(queue, CL_FALSE, dst_ptr, src_ptr, data_size, 2,
                               &eventMemFillList[0], &eventMemcpy);
            ...
            for (size_t j = 0; j < data_size; ++j)
            {
                ...
                {
                    log_error(
                    "Invalid data at index %ld, dst_ptr %d, src_ptr %d\n", j,
                    dst_ptr[j], src_ptr[j]);
                    return TEST_FAIL;
                }
            }
            ...
            error =
            clEnqueueSVMMemFill(queue, dstBuffer + data_size / 2, &fillData1[0],
                                typeSizes[i], data_size / 2, 0, 0, 0);
            test_error(error, "clEnqueueSVMMemFill failed");
    ...
    for (size_t i = 0; i < buffers.size(); ++i)
    {
        ...
        {
            log_error(
            "Invalid SVM pointer returned in the callback, idx: %ld\n", i);
            return TEST_FAIL;
        }
    }
