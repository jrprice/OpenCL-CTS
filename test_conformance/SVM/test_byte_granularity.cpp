...
const char *byte_manipulation_kernels[] = {
    // Each device will write it's id into the bytes that it "owns", ownership
    // is based on round robin (global_id % num_id)
    // num_id is equal to number of SVM devices in the system plus one (for the
    // host code).
    // id is the index (id) of the device that this kernel is executing on.
    // For example, if there are 2 SVM devices and the host; the buffer should
    // look like this after each device and the host write their id's:
    // 0, 1, 2, 0, 1, 2, 0, 1, 2...
    "__kernel void write_owned_locations(__global char* a, uint num_id, uint "
    "id)\n"
    "{\n"
    "    size_t i = get_global_id(0);\n"
    "   int owner = i % num_id;\n"
    "    if(id == owner) \n"
    "       a[i] = id;\n" // modify location if it belongs to this device, write
                          // id
    "}\n"

    // Verify that a device can see the byte sized updates from the other
    // devices, sum up the device id's and see if they match expected value.
    // Note: this must be called with a reduced NDRange so that neighbor acesses
    // don't go past end of buffer.
    // For example if there are two SVM devices and the host (3 total devices)
    // the buffer should look like this:
    // 0,1,2,0,1,2...
    // and the expected sum at each point is 0+1+2 = 3.
    "__kernel void sum_neighbor_locations(__global char* a, uint num_devices, "
    "volatile __global uint* error_count)\n"
    "{\n"
    "    size_t i = get_global_id(0);\n"
...
int test_svm_byte_granularity(cl_device_id deviceID, cl_context c,
                              cl_command_queue queue, int num_elements)
{
    ...
    err = create_cl_objects(deviceID, &byte_manipulation_kernels[0], &context,
    ...
    for (cl_uint id = 0; id < num_devices; id++)
    {
        ...
        queues[id], k2, 1, NULL, &adjusted_num_elements, NULL, 0, NULL, NULL);
        test_error(err, "clEnqueueNDRangeKernel failed");
    }
