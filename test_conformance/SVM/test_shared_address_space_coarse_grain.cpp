...
cl_int create_linked_lists_on_host(cl_command_queue cmdq, cl_mem nodes,
                                   Node *pNodes2, cl_int ListLength,
                                   size_t numLists, cl_bool useNewAPI)
{
    ...
    {
        pNodes = (Node *)clEnqueueMapBuffer(
        cmdq, nodes, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0,
        sizeof(Node) * ListLength * numLists, 0, NULL, NULL, &error);
    ...
    else
    {
        pNodes = pNodes2;
        error =
        clEnqueueSVMMap(cmdq, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, pNodes2,
                        sizeof(Node) * ListLength * numLists, 0, NULL, NULL);
...
cl_int verify_linked_lists_on_host(int ci, cl_command_queue cmdq, cl_mem nodes,
                                   Node *pNodes2, cl_int ListLength,
                                   size_t numLists, cl_bool useNewAPI)
{
    cl_int error = CL_SUCCESS;
...
cl_int create_linked_lists_on_device(int ci, cl_command_queue cmdq,
                                     cl_mem allocator,
                                     cl_kernel kernel_create_lists,
                                     size_t numLists)
{
    cl_int error = CL_SUCCESS;
    ...
    cmdq, allocator, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, sizeof(cl_int), 0,
    NULL, NULL, &error);
    test_error2(error, pAllocator, "clEnqueueMapBuffer failed");
    // reset allocator index
    *pAllocator =
    numLists; // the first numLists elements of the nodes array are already
              // allocated (they hold the head of each list).
    error = clEnqueueUnmapMemObject(cmdq, allocator, pAllocator, 0, NULL, NULL);
    test_error(error, " clEnqueueUnmapMemObject failed.");

    error = clEnqueueNDRangeKernel(cmdq, kernel_create_lists, 1, NULL,
                                   &numLists, NULL, 0, NULL, NULL);
...
cl_int verify_linked_lists_on_device(int vi, cl_command_queue cmdq,
                                     cl_mem num_correct,
                                     cl_kernel kernel_verify_lists,
                                     cl_int ListLength, size_t numLists)
{
    cl_int error = CL_SUCCESS;

    log_info(" and verifying on device: %d ", vi);

    cl_int *pNumCorrect = (cl_int *)clEnqueueMapBuffer(
    cmdq, num_correct, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, sizeof(cl_int),
    0, NULL, NULL, &error);
    ...
    *pNumCorrect = 0; // reset numCorrect to zero
    ...
    error =
    clEnqueueUnmapMemObject(cmdq, num_correct, pNumCorrect, 0, NULL, NULL);
    test_error(error, "clEnqueueUnmapMemObject failed.");

    error = clEnqueueNDRangeKernel(cmdq, kernel_verify_lists, 1, NULL,
                                   &numLists, NULL, 0, NULL, NULL);
...
// This tests that all devices and the host share a common address space; using
// only the coarse-grain features. This is done by creating a linked list on a
// device and then verifying the correctness of the list on another device or
// the host.  This basic test is performed for all combinations of devices and
// the host that exist within the platform.  The test passes only if every
// combination passes.
int shared_address_space_coarse_grain(cl_device_id deviceID,
                                      cl_context context2,
                                      ...
                                      cl_bool useNewAPI)
{
    ...
    deviceID, &linked_list_create_and_verify_kernels[0], &context, &program,
    &queues[0], &num_devices, CL_DEVICE_SVM_COARSE_GRAIN_BUFFER);
    if (error) return -1;
...

    {
        ...
        if (useNewAPI == CL_FALSE)
        {
            nodes = clCreateBuffer(context, CL_MEM_USE_HOST_PTR,
                                   sizeof(Node) * ListLength * numLists, pNodes,
                                   &error);
            test_error(error, "clCreateBuffer failed.");

            // verify if buffer uses SVM pointer
            size_t paramSize = 0;
            error = clGetMemObjectInfo(nodes, CL_MEM_USES_SVM_POINTER, 0, 0,
                                       &paramSize);
            test_error(error, "clGetMemObjectInfo failed.");

            if (paramSize != sizeof(cl_bool))
            {
                log_error("clGetMemObjectInfo(CL_MEM_USES_SVM_POINTER) "
                          "returned wrong size.");
                return -1;
            }

            error = clGetMemObjectInfo(nodes, CL_MEM_USES_SVM_POINTER,
                                       sizeof(cl_bool), &usesSVMpointer, 0);
            test_error(error, "clGetMemObjectInfo failed.");

            if (usesSVMpointer != CL_TRUE)
            {
                log_error(
                "clGetMemObjectInfo(CL_MEM_USES_SVM_POINTER) returned CL_FALSE "
                "for buffer created from SVM pointer.");
                return -1;
            }
        }
        ...
        error = clGetMemObjectInfo(allocator, CL_MEM_USES_SVM_POINTER,
                                   sizeof(cl_bool), &usesSVMpointer, 0);
        test_error(error, "clGetMemObjectInfo failed.");

        if (usesSVMpointer != CL_FALSE)
        {
            log_error("clGetMemObjectInfo(CL_MEM_USES_SVM_POINTER) returned "
                      "CL_TRUE for non-SVM buffer.");
            return -1;
        }

        // this buffer holds the count of correct nodes, which is computed by
        // the verify kernel.
                   ...
                   // list on
        {
                       ...
                       // linked list on
            {
                                       ...
                                       // note the num_device+1 above.
                {
                    ...
                    queues[0], nodes, pNodes, ListLength, numLists, useNewAPI);
                    if (error) return -1;
                }
                else
                {
                    error = create_linked_lists_on_device(
                    ci, queues[ci], allocator, kernel_create_lists, numLists);
                    if (error) return -1;
                }

                if (vi == num_devices)
                {
                    error = verify_linked_lists_on_host(vi, queues[0], nodes,
                                                        pNodes, ListLength,
                                                        numLists, useNewAPI);
                    if (error) return -1;
                }
...
int test_svm_shared_address_space_coarse_grain_old_api(cl_device_id deviceID,
                                                       cl_context context2,
                                                       cl_command_queue queue,
                                                       int num_elements)
{
    return shared_address_space_coarse_grain(deviceID, context2, queue,
                                             num_elements, CL_FALSE);
}

int test_svm_shared_address_space_coarse_grain_new_api(cl_device_id deviceID,
                                                       cl_context context2,
                                                       cl_command_queue queue,
                                                       int num_elements)
{
