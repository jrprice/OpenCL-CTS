...
const char *SVMPointerPassing_test_kernel[] = {
    "__kernel void verify_char(__global uchar* pChar, volatile __global uint* "
    "num_correct, uchar expected)\n"
    "{\n"
...
// Test that arbitrarily aligned char pointers into shared buffers can be passed
// directly to a kernel. This iterates through a buffer passing a pointer to
// each location to the kernel. The buffer is initialized to known values at
// each location. The kernel checks that it finds the expected value at each
// location.
// TODO: possibly make this work across all base types (including typeN?), also
// check ptr arithmetic ++,--.
int test_svm_pointer_passing(cl_device_id deviceID, cl_context context2,
                             cl_command_queue queue, int num_elements)
{
    ...
    {
        clMemWrapper buf =
        clCreateBuffer(context, CL_MEM_USE_HOST_PTR, sizeof(cl_uchar) * bufSize,
                       pbuf_svm_alloc, &error);
             ...
             ++ii) // iterate over all devices in the platform.
        {
            ...
            for (int i = 0; i < (int)bufSize; i++)
            {
                ...
                kernel_verify_char, 2, sizeof(cl_uchar),
                (void *)&i); // pass the expected value at the above location.
                test_error(error, "clSetKernelArg failed");
                error =
