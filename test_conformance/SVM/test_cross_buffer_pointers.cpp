...
const char *SVMCrossBufferPointers_test_kernel[] = {
    ...
    "__kernel void create_linked_lists(__global Node* pNodes, __global Node* "
    "pNodes2, volatile __global int* allocation_index, int list_length)\n"
    "{\n"
    ...
    "}\n"
    "\n"
    "__kernel void verify_linked_lists(__global Node* pNodes, __global Node* "
    "pNodes2, volatile __global uint* num_correct, int list_length)\n"
    "{\n"
    "    size_t i = get_global_id(0);\n"
    "    __global Node *pNode = &pNodes[i];\n"
    "\n"
...
// This tests that shared buffers are able to contain pointers that point to
// other shared buffers. This tests that all devices and the host share a common
// address space; using only the coarse-grain features. This is done by creating
// a linked list on a device and then verifying the correctness of the list on
// another device or the host. The linked list nodes are allocated from two
// different buffers this is done to ensure that cross buffer pointers work
// correctly. This basic test is performed for all combinations of devices and
// the host.
int test_svm_cross_buffer_pointers_coarse_grain(cl_device_id deviceID,
                                                cl_context context2,
                                                ...
                                                int num_elements)
{
...

    {
        ...
        clCreateBuffer(context, CL_MEM_USE_HOST_PTR,
                       sizeof(Node) * ListLength * numLists, pNodes, &error);
        ...
        error |=
        clSetKernelArg(kernel_create_lists, 0, sizeof(void *), (void *)&nodes);
        // error |= clSetKernelArgSVMPointer(kernel_create_lists, 0, (void *)
        // pNodes);
        error |=
        clSetKernelArg(kernel_create_lists, 1, sizeof(void *), (void *)&nodes2);
        ...
        error |=
        clSetKernelArg(kernel_verify_lists, 0, sizeof(void *), (void *)&nodes);
        error |=
        clSetKernelArg(kernel_verify_lists, 1, sizeof(void *), (void *)&nodes2);
