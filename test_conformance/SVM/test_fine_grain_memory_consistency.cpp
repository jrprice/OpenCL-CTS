...
"{\n"
" __global BinNode *pNew = &pNodes[ atomic_fetch_add_explicit(pNumNodes, 1, "
...
" } while(!atomic_compare_exchange_strong_explicit(&(pNodes[b].pNext), &next, "
"(uintptr_t)pNew, memory_order_seq_cst, memory_order_relaxed, "
"memory_scope_all_svm_devices));\n"
"}\n";
...
void build_hash_table_on_host(cl_context c, cl_uint *input, size_t inputSize,
                              BinNode *pNodes, cl_int volatile *pNumNodes,
                              cl_uint numBins)
{
    for (cl_uint i = 0; i < inputSize; i++)
    {
        BinNode *pNew =
        &pNodes[AtomicFetchAddExplicit(pNumNodes, 1, memory_order_relaxed)];
        ...
        } while (!AtomicCompareExchangeStrongExplicit(
        &(pNodes[b].pNext), &next, pNew, memory_order_relaxed,
        memory_order_seq_cst));
    }
}


int launch_kernels_and_verify(clContextWrapper &context,
                              clCommandQueueWrapper *queues,
                              clKernelWrapper &kernel, cl_uint num_devices,
                              cl_uint numBins, size_t num_pixels)
{
    int err = CL_SUCCESS;
    ...
    sizeof(BinNode) * (num_pixels * (num_devices + 1) + numBins), 0);
    cl_int *pNumNodes = (cl_int *)clSVMAlloc(
    context,
    CL_MEM_READ_WRITE | CL_MEM_SVM_FINE_GRAIN_BUFFER | CL_MEM_SVM_ATOMICS,
    sizeof(cl_int), 0);

    *pNumNodes =
    numBins; // using the first numBins nodes to hold the list heads.
    ...
    cl_event done;
    // get all the devices going simultaneously, each device (and the host) will
    // insert all the pixels.
    for (cl_uint d = 0; d < num_devices; d++)
    {
        err = clEnqueueNDRangeKernel(queues[d], kernel, 1, NULL, &num_pixels, 0,
                                     0, NULL, &done);
    ...
    build_hash_table_on_host(context, pInputImage, num_pixels, pNodes,
                             pNumNodes, numBins);
    ...
    for (cl_uint i = 0; i < numBins; i++)
    {
        ...
        while (pNode)
        {
            ...
            {
                log_error(
                "Something went wrong, item is in wrong hash bucket\n");
                break;
            }
            num_items++;
            pNode = pNode->pNext;
    ...
    // each device and the host inserted all of the pixels, check that none are
    // missing.
    if (num_items != num_pixels * (num_devices + 1))
    {
        log_error(
        "The hash table is not correct, num items %d, expected num items: %d\n",
...
// Each device and the host simultaneously insert values into a single hash
// table. Each bin in the hash table is a linked list.  Each bin is protected
// against simultaneous update using a lock free technique.  The correctness of
// the list is verfied on the host. This test requires the new OpenCL 2.0 atomic
// operations that implement the new seq_cst memory ordering.
int test_svm_fine_grain_memory_consistency(cl_device_id deviceID, cl_context c,
                                           ...
                                           int num_elements)
{
    ...
    deviceID, (const char **)source, &context, &program, &queues[0],
    ...
    result = launch_kernels_and_verify(context, queues, kernel, num_devices,
                                       numBins, num_pixels);
