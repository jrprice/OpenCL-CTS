...
// SVM Atomic wrappers.
// Platforms that support SVM atomics (atomics that work across the host and
// devices) need to implement these host side functions correctly. Platforms
// that do not support SVM atomics can simpy implement these functions as empty
// stubs since the functions will not be called. For now only Windows x86 is
// implemented, add support for other platforms as needed.
cl_int AtomicLoadExplicit(volatile cl_int *pValue, cl_memory_order order)
{
#if (defined(_MSC_VER) && (defined(_M_IX86) || defined(_M_X64)))               \
|| (defined(__GNUC__) && (defined(__i386__) || defined(__x86_64__)))
    return *pValue; // provided the value is aligned x86 doesn't need anything
                    // more than this for seq_cst.
...
// all the x86 atomics are seq_cst, so don't need to do anything with the memory
// order parameter.
cl_int AtomicFetchAddExplicit(volatile cl_int *object, cl_int operand,
                              cl_memory_order o)
{
...
cl_int AtomicExchangeExplicit(volatile cl_int *object, cl_int desired,
                              cl_memory_order mo)
{
#if (defined(_WIN32) || defined(_WIN64)) && defined(_MSC_VER)
...
const char *linked_list_create_and_verify_kernels[] = {
    ...
    "} Node;\n"
    "\n"
    // The allocation_index parameter must be initialized on the host to N
    // work-items The first N nodes in pNodes will be the heads of the lists.
    "__kernel void create_linked_lists(__global Node* pNodes, volatile "
    "__attribute__((nosvm)) __global int* allocation_index, int list_length)\n"
    "{\n"
    ...
    "    {\n"
    "        pNew = &pNodes[ atomic_inc(allocation_index) ];// allocate a new "
    "node\n"
    "        pNew->global_id = i;\n"
    ...
    "__kernel void verify_linked_lists(__global Node* pNodes, volatile "
    "__global uint* num_correct, int list_length)\n"
    "{\n"
    "    size_t i = get_global_id(0);\n"
    "    __global Node *pNode = &pNodes[i];\n"
    "\n"
...
void create_linked_lists(Node *pNodes, size_t num_lists, int list_length)
{
    ...
    num_lists; // heads of lists are in first num_lists nodes.
...
// Note that we don't use the context provided by the test harness since it
// doesn't support multiple devices, so we create are own context here that has
// all devices, we use the same platform that the harness used.
cl_int create_cl_objects(cl_device_id device_from_harness,
                         const char **ppCodeString, cl_context *context,
                         cl_program *program, cl_command_queue *queues,
                         cl_uint *num_devices,
                         cl_device_svm_capabilities required_svm_caps,
                         std::vector<std::string> extensions_list)
{
    ...
    error = clGetDeviceInfo(device_from_harness, CL_DEVICE_PLATFORM,
    ...
    std::vector<cl_device_id> devicesTmp(*num_devices), devices,
    capable_devices;

    error = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_ALL, *num_devices,
                           &devicesTmp[0], NULL);
    test_error(error, "clGetDeviceIDs failed");
    ...
    // Note that if requested SVM capabilities are not supported by any device
    // then the test still passes (even though it does not execute).
    ...
    for (cl_uint i = 0; i < *num_devices; i++)
    {
        ...
        error =
        clGetDeviceInfo(devices[i], CL_DEVICE_SVM_CAPABILITIES,
                        sizeof(cl_device_svm_capabilities), &caps, NULL);
        test_error(error,
                   "clGetDeviceInfo failed for CL_DEVICE_SVM_CAPABILITIES");
        if (caps
            & (~(CL_DEVICE_SVM_COARSE_GRAIN_BUFFER
                 | CL_DEVICE_SVM_FINE_GRAIN_BUFFER
                 | CL_DEVICE_SVM_FINE_GRAIN_SYSTEM | CL_DEVICE_SVM_ATOMICS)))
        {
            log_error("clGetDeviceInfo returned an invalid "
    ...
    devices =
    capable_devices; // the only devices we care about from here on are the ones
                     // capable of supporting the requested SVM level.
    *num_devices = num_capable_devices;
    if (num_capable_devices == 0)
    //    if(svm_level > CL_DEVICE_COARSE_SVM && 0 == num_capable_devices)
    {
        log_info("Requested SVM level or required extensions not supported by "
                 "any device on this platform, test not executed.\n");
    ...
    *context = clCreateContext(context_properties, *num_devices, &devices[0],
                               NULL, NULL, &error);
    test_error(error, "Unable to create context");

    //    *queues = (cl_command_queue *) malloc( *num_devices * sizeof(
    //    cl_command_queue ) );

    for (cl_uint i = 0; i < *num_devices; i++)
    {
        queues[i] =
        clCreateCommandQueueWithProperties(*context, devices[i], 0, &error);
        test_error(error, "clCreateCommandQueue failed");
    }

    if (ppCodeString)
    {
        error = create_single_kernel_helper(*context, program, 0, 1,
                                            ppCodeString, 0, "-cl-std=CL2.0");
...
{
    return runTestHarnessWithCheck(argc, argv, test_num, test_list, true, 0,
                                   InitCL);
}
