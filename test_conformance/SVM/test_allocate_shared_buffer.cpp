...
int test_svm_allocate_shared_buffer(cl_device_id deviceID, cl_context context2,
                                    cl_command_queue queue, int num_elements)
{
    ...
    for (int i = 0; i < num_flags; i++)
    {
        ...
        {
            clMemWrapper buf =
            clCreateBuffer(context, CL_MEM_USE_HOST_PTR, size, pBufData1, &err);
            ...
            if (!(flag_set[i] & CL_MEM_HOST_NO_ACCESS))
            {
                ...
                queues[0], buf, CL_TRUE, flags, 0, size, 0, NULL, NULL, &err);
