...
const char *find_targets_kernel[] = {
    ...
    " uint index;\n"
    " if(image[i] == target) {\n"
    "   index = atomic_fetch_add_explicit(numTargetsFound, 1, "
...
// Goals: demonstrate use of SVM's atomics to do fine grain synchronization
// between the device and host. Concept: a device kernel is used to search an
// input image for regions that match a target pattern. The device immediately
// notifies the host when it finds a target (via an atomic operation that works
// across host and devices). The host is then able to spawn a task that further
// analyzes the target while the device continues searching for more targets.
int test_svm_fine_grain_sync_buffers(cl_device_id deviceID, cl_context c,
                                     cl_command_queue queue, int num_elements)
{
    ...
    deviceID, &find_targets_kernel[0], &context, &program, &queues[0],
