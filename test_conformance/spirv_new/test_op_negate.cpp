...
int test_negation(cl_device_id deviceID, cl_context context,
                  cl_command_queue queue, const char *Tname,
                  const char *funcName, const std::vector<Tv> &h_in,
...
#define TEST_NEG_VEC(TYPE, N)                                                  \
    TEST_NEGATION(TYPE##N, cl_##TYPE##N, op_neg, (negOpVec<cl_##TYPE##N, N>))
#define TEST_NOT_VEC(TYPE, N)                                                  \
    TEST_NEGATION(TYPE##N, cl_##TYPE##N, op_not, (notOpVec<cl_##TYPE##N, N>))
...
#ifdef __GNUC__
// std::vector<cl_short> is causing compilation errors on GCC 5.3 (works on
// gcc 4.8) Needs further investigation
