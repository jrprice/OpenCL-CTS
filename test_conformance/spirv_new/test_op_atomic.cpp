...
int test_atomic(cl_device_id deviceID, cl_context context,
                cl_command_queue queue, const char *name, const int num,
                bool is_inc)
{
    ...
    clMemWrapper ctr_mem =
    clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(T), NULL, &err);
    ...
    err = clEnqueueWriteBuffer(queue, ctr_mem, CL_TRUE, 0, sizeof(T), &initial,
                               0, NULL, NULL);
    SPIRV_CHECK_ERROR(err, "Failed to copy to lhs buffer");

    clMemWrapper val_mem =
    ...
    T count = 0;
    err = clEnqueueReadBuffer(queue, ctr_mem, CL_TRUE, 0, sizeof(T), &count, 0,
                              NULL, NULL);
    ...
    err = clEnqueueReadBuffer(queue, val_mem, CL_TRUE, 0, bytes, &host[0], 0,
                              NULL, NULL);
...
{
    int num = 1 << 16;
    return test_atomic<cl_int>(deviceID, context, queue, "atomic_inc_global",
                               num, true);
}
...
{
    int num = 1 << 16;
    return test_atomic<cl_int>(deviceID, context, queue, "atomic_dec_global",
                               num, false);
}
