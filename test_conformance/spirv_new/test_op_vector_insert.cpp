                ...
                const std::vector<Ts> &h_in, const int n)
{
    ...
    clMemWrapper in =
    clCreateBuffer(context, CL_MEM_READ_WRITE, in_bytes, NULL, &err);
    ...
    for (int k = 0; k < n; k++)
    {
        ...
        err = clEnqueueWriteBuffer(queue, out, CL_TRUE, 0, out_bytes, &h_ref[0],
                                   0, NULL, NULL);
        ...
        err = clEnqueueReadBuffer(queue, out, CL_TRUE, 0, out_bytes, &h_out[0],
                                  0, NULL, NULL);
        SPIRV_CHECK_ERROR(err, "Failed to copy from cl_buffer");

        for (int i = 0; i < num; i++)
        {
            // Kernel should have replaced k'th value of the vector in h_out
            // with h_in The remaining values should be unchanged.
            Tv refVec = h_ref[i];
            refVec.s[k] = h_in[i];
            for (int j = 0; j < n; j++)
            {
                ...
                {
                    log_error("Values do not match at location %d for vector "
                              "position %d\n",
                              i, k);
                    return -1;
                }
