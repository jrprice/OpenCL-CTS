...
template <typename T> bool isNotEqual(const T &lhs, const T &rhs)
{
    return lhs != rhs;
}

// Can replace the following with tuples if c++11 can be used
template <typename T> struct AbstractStruct1
{
    T val;
};

template <typename T>
inline bool operator!=(const AbstractStruct1<T> &lhs,
                       const AbstractStruct1<T> &rhs)
{
    return lhs.val != rhs.val;
}

template <typename T0, typename T1> struct AbstractStruct2
{
    T0 val0;
    T1 val1;
};
...
template <> struct is_double<cl_double>
{
    static const bool value = true;
};
template <> struct is_double<cl_double2>
{
    static const bool value = true;
};

template <typename T> T genrandReal(RandomSeed &seed)
{
    return genrand_real1(seed);
}

template <typename T, int N> T genrandRealVec(RandomSeed &seed)
{
    T res;
    for (int i = 0; i < N; i++)
    {
...
template <typename T> T genrand(RandomSeed &seed)
{
...
template <typename Tv> Tv negOp(Tv in) { return -in; }

template <typename Tv> Tv notOp(Tv in) { return ~in; }

template <typename Tv, int N> Tv negOpVec(Tv in)
{
    Tv out;
    for (int i = 0; i < N; i++)
    {
...
template <typename Tv, int N> Tv notOpVec(Tv in)
{
    Tv out;
    for (int i = 0; i < N; i++)
    {
