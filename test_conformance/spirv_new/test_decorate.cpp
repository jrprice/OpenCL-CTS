...
be disclosed h_in whole or h_in part to third parties, and may not be
reproduced, republished, distributed, transmitted, displayed, broadcast or
otherwise exploited h_in any manner without the express prior written permission
of Khronos Group. The receipt or possession of this code does not convey any
rights to reproduce, disclose, or distribute its contents, or to manufacture,
use, or sell anything that it may describe, h_in whole or h_in part other than
under the terms of the Khronos Adopters Agreement or Khronos Conformance Test
Source License Agreement as executed between Khronos and the recipient.
******************************************************************/
...
static int verify_results(cl_device_id deviceID, cl_context context,
...
int test_decorate_full(cl_device_id deviceID, cl_context context,
...
TEST_SPIRV_FUNC(decorate_cpacked)
{
    ...
    clProgramWrapper prog;
    cl_int err =
    get_program_with_il(prog, deviceID, context, "decorate_cpacked");
                             ...
                             const clProgramWrapper &prog)
{
    ...
    clMemWrapper lhs =
    clCreateBuffer(context, CL_MEM_READ_ONLY, in_bytes, NULL, &err);
    SPIRV_CHECK_ERROR(err, "Failed to create  in buffer");

    err = clEnqueueWriteBuffer(queue, lhs, CL_TRUE, 0, in_bytes, &h_lhs[0], 0,
                               NULL, NULL);
    SPIRV_CHECK_ERROR(err, "Failed to copy to in buffer");
    ...
    err = clEnqueueWriteBuffer(queue, rhs, CL_TRUE, 0, in_bytes, &h_rhs[0], 0,
                               NULL, NULL);
    SPIRV_CHECK_ERROR(err, "Failed to copy to in buffer");

    clMemWrapper res =
    clCreateBuffer(context, CL_MEM_READ_WRITE, out_bytes, NULL, &err);
    SPIRV_CHECK_ERROR(err, "Failed to create  in buffer");
    ...
    err = clEnqueueReadBuffer(queue, res, CL_TRUE, 0, out_bytes, &h_res[0], 0,
                              NULL, NULL);
    SPIRV_CHECK_ERROR(err, "Failed to read to output");
...
template <typename Ti, typename Tl, typename To>
int test_saturate_full(cl_device_id deviceID, cl_context context,
                       cl_command_queue queue, const char *name,
                       const char *types)
{
...
int test_image_decorate(cl_device_id deviceID, cl_context context,
                        cl_command_queue queue, const char *name)
{
    ...
    err = clEnqueueReadBuffer(queue, dstBuffer, CL_TRUE, 0, bytes, &dst[0], 0,
                              NULL, NULL);
...
TEST_SPIRV_FUNC(decorate_nonreadable)
{
    ...
    size_t bytes = src.size() * sizeof(cl_uint4);
    clMemWrapper srcBuffer =
    clCreateBuffer(context, CL_MEM_READ_WRITE, bytes, NULL, &err);
    SPIRV_CHECK_ERROR(err, "Failed to create image object");

    err = clEnqueueWriteBuffer(queue, srcBuffer, CL_TRUE, 0, bytes, &src[0], 0,
                               NULL, NULL);
    SPIRV_CHECK_ERROR(err, "Failed to copy data back to host");
    ...
    clMemWrapper dstImage =
    clCreateImage(context, CL_MEM_WRITE_ONLY, &imageFormat, &desc, NULL, &err);
    SPIRV_CHECK_ERROR(err, "Failed to create image object");
    ...
    err = clEnqueueReadImage(queue, dstImage, CL_TRUE, origin, region, 0, 0,
                             &dst[0], 0, NULL, NULL);
...
int test_fp_rounding(cl_device_id deviceID, cl_context context,
                     cl_command_queue queue, const char *name,
                     std::vector<Ti> &h_in, std::vector<To> &h_out)
{
    ...
    clMemWrapper out =
    clCreateBuffer(context, CL_MEM_READ_ONLY, out_bytes, NULL, &err);
    SPIRV_CHECK_ERROR(err, "Failed to create output buffer");

    err = clEnqueueWriteBuffer(queue, in, CL_TRUE, 0, in_bytes, &h_in[0], 0,
                               NULL, NULL);
    ...
    std::vector<To> h_res(num);
    err = clEnqueueReadBuffer(queue, out, CL_TRUE, 0, out_bytes, &h_res[0], 0,
                              NULL, NULL);
...
template <typename Ti, typename To> inline To round_to_zero(Ti in)
{
    To out = (To)(in);
    return out;
}
...
template <typename Ti, typename To> inline To round_to_even(Ti in)
{
...
template <typename Ti, typename To> inline To round_to_posinf(Ti in)
{
    To out = std::ceil(in);
    return out;
}

template <typename Ti, typename To> inline To round_to_neginf(Ti in)
{
    To out = std::floor(in);
    return out;
}
