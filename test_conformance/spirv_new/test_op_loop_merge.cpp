...
template <typename T>
int test_selection_merge(cl_device_id deviceID, cl_context context,
                         cl_command_queue queue, const char *name,
                         const std::vector<T> &h_in,
                         const std::vector<T> &h_ref, const int rep)
{
    ...
    err = clEnqueueReadBuffer(queue, out, CL_TRUE, 0, bytes, &h_out[0], 0, NULL,
                              NULL);
