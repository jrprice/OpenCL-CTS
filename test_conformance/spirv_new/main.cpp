...
std::vector<unsigned char> readSPIRV(const char *file_name)
{
    ...
    spvBinariesPath + slash + file_name + spvExt + gAddrWidth;
...
size_t spirvTestsRegistry::getNumTests() { return testDefinitions.size(); }
                                       ...
                                       const char *prog_name)
{
    ...
    std::string("..") + slash + std::string("spv_to_binary.py");
    std::string outputFilename =
    spvBinariesPath + slash + std::string(prog_name);
    std::string sourceFilename = outputFilename + spvExt;

    std::string scriptArgs = sourceFilename + " " + outputFilename + " "
    + gAddrWidth + " " + outputTypeStr + " " + "-cl-std=CL2.0";
    ...
    if (file_bytes == 0)
    {
        log_error("OfflinerCompiler: Failed to open binary file: %s",
    ...
    cl_int status = 0;
    prog = clCreateProgramWithBinary(context, 1, &deviceID, &file_bytes,
                                     &buffer, &status, &err);
    SPIRV_CHECK_ERROR(
    (err || status), "Failed to create program with clCreateProgramWithBinary");
    return err;
}
