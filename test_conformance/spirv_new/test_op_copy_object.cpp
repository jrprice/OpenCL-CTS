...
int test_copy(cl_device_id deviceID, cl_context context, cl_command_queue queue,
              const char *name, std::vector<T> &results,
              bool (*notEqual)(const T &, const T &) = isNotEqual<T>)
{
    ...
    err =
    clEnqueueReadBuffer(queue, mem, CL_TRUE, 0, bytes, &host[0], 0, NULL, NULL);
...
TEST_SPIRV_FUNC(op_copy_int3_simple)
{
    ...
    return test_copy(deviceID, context, queue, "copy_int3_simple", results,
...
TEST_SPIRV_FUNC(op_copy_struct_int_float_simple)
{
    ...
    return test_copy(deviceID, context, queue, "copy_struct_int_float_simple",
                     results);
}
...
TEST_SPIRV_FUNC(op_copy_struct_int_char_simple)
{
    ...
    return test_copy(deviceID, context, queue, "copy_struct_int_char_simple",
                     results);
}
...
TEST_SPIRV_FUNC(op_copy_half_simple)
{
    ...
    return test_copy(deviceID, context, queue, "copy_half_simple", results);
}
