...
int test_vector_times_scalar(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, const char *Tname,
                             std::vector<Tv> &h_lhs, std::vector<Ts> &h_rhs)
{
    ...
    clMemWrapper lhs =
    clCreateBuffer(context, CL_MEM_READ_ONLY, lhs_bytes, NULL, &err);
    SPIRV_CHECK_ERROR(err, "Failed to create lhs buffer");

    err = clEnqueueWriteBuffer(queue, lhs, CL_TRUE, 0, lhs_bytes, &h_lhs[0], 0,
                               NULL, NULL);
    SPIRV_CHECK_ERROR(err, "Failed to copy to lhs buffer");

    clMemWrapper rhs =
    clCreateBuffer(context, CL_MEM_READ_ONLY, rhs_bytes, NULL, &err);
    SPIRV_CHECK_ERROR(err, "Failed to create rhs buffer");

    err = clEnqueueWriteBuffer(queue, rhs, CL_TRUE, 0, rhs_bytes, &h_rhs[0], 0,
                               NULL, NULL);
    ...
    std::vector<Tv> h_ref(num);
    {
        ...
        clCreateKernel(prog, "vector_times_scalar", &err);
        SPIRV_CHECK_ERROR(err, "Failed to create cl kernel");

        clMemWrapper ref =
        clCreateBuffer(context, CL_MEM_READ_WRITE, res_bytes, NULL, &err);
        ...
        err = clEnqueueReadBuffer(queue, ref, CL_TRUE, 0, res_bytes, &h_ref[0],
                                  0, NULL, NULL);
    ...
    err = clEnqueueReadBuffer(queue, res, CL_TRUE, 0, res_bytes, &h_res[0], 0,
                              NULL, NULL);
    SPIRV_CHECK_ERROR(err, "Failed to read from ref");
