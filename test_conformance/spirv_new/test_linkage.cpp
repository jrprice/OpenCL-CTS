...
static int test_linkage_compile(cl_device_id deviceID, cl_context context,
                                cl_command_queue queue, const char *fname,
                                clProgramWrapper &prog)
{
    ...
    SPIRV_CHECK_ERROR(err,
                      "Failed to create program with clCreateProgramWithIL");
...
{
    clProgramWrapper prog;
    return test_linkage_compile(deviceID, context, queue, "linkage_export",
                                prog);
}
...
{
    clProgramWrapper prog;
    return test_linkage_compile(deviceID, context, queue, "linkage_import",
                                prog);
}
...
TEST_SPIRV_FUNC(linkage_import_function_link)
{
    ...
    err = test_linkage_compile(deviceID, context, queue, "linkage_export",
                               prog_export);
    SPIRV_CHECK_ERROR(err, "Failed to compile export program");

    clProgramWrapper prog_import;
    err = test_linkage_compile(deviceID, context, queue, "linkage_import",
                               prog_import);
    ...
    err =
    clEnqueueWriteBuffer(queue, in, CL_TRUE, 0, bytes, &h_in[0], 0, NULL, NULL);
    ...
    err =
    clEnqueueReadBuffer(queue, in, CL_TRUE, 0, bytes, &h_out[0], 0, NULL, NULL);
