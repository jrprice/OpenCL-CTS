...
const char* strType[] = { "int",      "float",       "octal",
                          "unsigned", "hexadecimal", "char",
                          "string",   "vector",      "address space" };
...
//[string] flag | [string] specifier | [string] type | [string] vector-data
//representation | [string] vector size   |
...
// [string] argument type qualifier |[string] variable type qualifier +
// initialization | [string] format | [string] parameter |[string]%p
// indicator/additional code |
...
testCase* allTestCase[] = { &testCaseInt,         &testCaseFloat,
                            &testCaseOctal,       &testCaseUnsigned,
                            &testCaseHexadecimal, &testCaseChar,
                            &testCaseString,      &testCaseVector,
                            &testCaseAddrSpace };
                          ...
                          size_t testId, cl_ulong pAddr)
{
    ...
    if (pTestCase->_type == TYPE_ADDRESS_SPACE
        && strcmp(pTestCase->_genParameters[testId].addrSpacePAdd, ""))
    ...
    if ((exp = strstr(analysisBuffer, "E+")) != NULL
        || (exp = strstr(analysisBuffer, "e+")) != NULL
        || (exp = strstr(analysisBuffer, "E-")) != NULL
        || (exp = strstr(analysisBuffer, "e-")) != NULL)
    {
        char correctExp[3] = { 0 };
        ...
        char* eCorrectBuffer =
        strstr((char*)pTestCase->_correctBuffer[testId], correctExp);
        if (eCorrectBuffer == NULL) return 1;

        eCorrectBuffer += 2;
        exp += 2;

        // Exponent always contains at least two digits
        if (strlen(exp) < 2) return 1;
    ...
    if (!strcmp(pTestCase->_correctBuffer[testId], "inf"))
        ...
        && strcmp(analysisBuffer, "1.#INF00") && strcmp(analysisBuffer, "Inf");
    if (!strcmp(pTestCase->_correctBuffer[testId], "nan")
        || !strcmp(pTestCase->_correctBuffer[testId], "-nan"))
    {
        return strcmp(analysisBuffer, "nan") && strcmp(analysisBuffer, "-nan")
        && strcmp(analysisBuffer, "1.#IND00")
        && strcmp(analysisBuffer, "-1.#IND00") && strcmp(analysisBuffer, "NaN")
        && strcmp(analysisBuffer, "nan(ind)")
        && strcmp(analysisBuffer, "nan(snan)")
        && strcmp(analysisBuffer, "-nan(ind)");
    }
