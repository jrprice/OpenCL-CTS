...
int getImageInfo(cl_device_id device)
{
    ...
    for (ii = 0; ii < ni; ++ii)
    {
        ...
        for (fi = 0; fi < nf; ++fi)
        {
            ...
            formats =
            (cl_image_format*)malloc(num_supported * sizeof(cl_image_format));
            if (formats == NULL)
            {
...
int getConfigInfo(cl_device_id device, config_info* info)
{
    ...
    switch (info->config_type)
    {
        ...
        case type_size_t:
            err =
            clGetDeviceInfo(device, info->opcode, sizeof(info->config.sizet),
        ...
        case type_string:
            err =
            clGetDeviceInfo(device, info->opcode, 0, NULL, &config_size_set);
...
void dumpConfigInfo(cl_device_id device, config_info* info)
{
    ...
    switch (info->config_type)
    {
        ...
        case type_cl_command_queue_properties:
                     ...
                     : "");
            {
                ...
                if (info->config.queue_properties & ~all_queue_properties)
                    ...
                    info->opcode_name,
                    (info->config.exec_capabilities & ~all_queue_properties));
            }
            break;
        case type_cl_device_id:
            log_info("\t%s == %ld\n", info->opcode_name,
        ...
        case type_cl_device_affinity_domain:
            ...
            : "",
            (info->config.affinity_domain & CL_DEVICE_AFFINITY_DOMAIN_L4_CACHE)
            ? "CL_DEVICE_AFFINITY_DOMAIN_L4_CACHE"
            : "",
            (info->config.affinity_domain & CL_DEVICE_AFFINITY_DOMAIN_L3_CACHE)
            ? "CL_DEVICE_AFFINITY_DOMAIN_L3_CACHE"
            : "",
            (info->config.affinity_domain & CL_DEVICE_AFFINITY_DOMAIN_L2_CACHE)
            ? "CL_DEVICE_AFFINITY_DOMAIN_L2_CACHE"
            : "",
            (info->config.affinity_domain & CL_DEVICE_AFFINITY_DOMAIN_L1_CACHE)
            ...
            : "");
            {
                ...
                if (info->config.affinity_domain & ~all_affinity_domain)
                    log_error(
                    "ERROR: %s unknown bits found 0x%08llX", info->opcode_name,
        ...
        case type_cl_device_svm_capabilities:
            log_info(
            "\t%s == %s|%s|%s|%s\n", info->opcode_name,
            (info->config.svmCapabilities & CL_DEVICE_SVM_COARSE_GRAIN_BUFFER)
