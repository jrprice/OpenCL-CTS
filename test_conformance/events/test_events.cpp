                              ...
                              cl_event *outEvent)
{
    ...
    streams[0] = clCreateBuffer(inContext, (cl_mem_flags)(CL_MEM_READ_WRITE),
                                sizeof(cl_float) * 1000, NULL, &error);
    test_error(error, "Creating test array failed");
    streams[1] = clCreateBuffer(inContext, (cl_mem_flags)(CL_MEM_READ_WRITE),
                        ...
                        cl_command_queue queue, int num_elements)
{
    ...
    error =
    clGetEventInfo(event, CL_EVENT_COMMAND_TYPE, sizeof(type), &type, &size);
    test_error(error, "Unable to get event info!");
    ...
    cl_context testCtx;
    error =
    clGetEventInfo(event, CL_EVENT_CONTEXT, sizeof(testCtx), &testCtx, &size);
                                      ...
                                      cl_command_queue queue, int num_elements)
{
    ...
    stream = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
...
associated with the command-queue), CL_RUNNING (device is currently executing
this command), CL_COMPLETE (the command has completed), or Error code given by a
negative integer value. (command was abnormally terminated – this may be caused
by a bad memory access etc.).
*/
                                              ...
                                              int num_elements)
{
    ...
    error =
    clEnqueueMarkerWithWaitList(queue, event_count, event_list, &event_list[2]);
