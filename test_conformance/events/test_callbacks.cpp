                               ...
                               cl_command_queue queue, Action *actionToTest)
{
    ...
    for (int i = 0; i < EVENT_CALLBACK_TYPE_TOTAL; i++)
    {
        ...
        user_data[i].index = i;
        error =
        clSetEventCallback(actualEvent, event_callback_types[i],
                           single_event_callback_function_flags, user_data + i);
    }
...
#define TEST_ACTION(name)                                                      \
    {                                                                          \
        ...
        if ((error =                                                           \
             test_callback_event_single(deviceID, context, queue, &action))    \
            != CL_SUCCESS)                                                     \
            retVal++;                                                          \
        clFinish(queue);                                                       \
    }
