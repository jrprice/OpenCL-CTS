...
int use_marker)
{
    ...
    if (test_enqueue_wait_for_events)
        log_info(
        "\tTesting with clEnqueueBarrierWithWaitList as barrier function.\n");
    if (test_barrier)
        log_info(
        "\tTesting with clEnqueueBarrierWithWaitList as barrier function.\n");
    if (use_waitlist)
    ...
    if (two_devices)
    {
        ...
        context_to_use =
        clCreateContext(NULL, 2, two_device_ids, notify_callback, NULL, &error);
    ...
    if (two_queues)
    {
        ...
        if (two_devices)
        {
            ...
            {
                log_info(
                "WARNING: One or more device for multi-device test does not "
                "support out-of-order exec mode; skipping test.\n");
                return -1942;
            }
            ...
            test_error(
            error,
            "clCreateCommandQueue for second queue on second device failed.");
        }
        else
        {
    ...
    for (loop_count = 0; loop_count < TEST_COUNT; loop_count++)
    {
        ...
        if (use_waitlist | use_marker)
        {
            ...
            queues[queue_to_use], kernel1[loop_count], 1, NULL, threads, NULL,
            1, &event[event_count - 1], &event[event_count]);
        }
        else
        {
            if (DEBUG_OUT)
        ...
        if (use_waitlist | use_marker)
        {
            if (DEBUG_OUT)
                log_info("clEnqueueNDRangeKernel(queues[%d], kernel2[%d], 1, "
                         ...
                         event_count);
            error = clEnqueueNDRangeKernel(
            queues[queue_to_use], kernel2[loop_count], 1, NULL, threads, NULL,
            1, &event[event_count - 1], &event[event_count]);
        }
        else
        {
    ...
    error =
    clEnqueueReadBuffer(queues[0], data, CL_TRUE, 0, TEST_SIZE * sizeof(cl_int),
                        values, 1, &event[event_count], NULL);
         ...
         int use_marker)
{
    ...
    deviceID, context, queue, num_elements, two_queues, two_devices, 0, 0, 0,
    0);
    if (baseline_results == 0)
    {
        if (test_enqueue_wait_for_events)
            ...
            "using clEnqueueBarrierWithWaitList, so this test is not a valid "
