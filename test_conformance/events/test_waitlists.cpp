                  ...
                  cl_command_queue queue, Action *actionToTest, bool multiple)
{
    ...
    error =
    actionToTest->Execute(queue, (multiple) ? 2 : 1, &events[0], &events[2]);
    ...
    if (multiple)
    {
        ...
        {
            log_error(
            "ERROR: Test failed because the second wait event is complete and "
            "the first is not.(status: 0: %s and 1: %s)\n",
    ...
    {
        log_error(
        "ERROR: Waited for first event but it's not complete (status: 0: %s)\n",
        IGetStatusString(status[0]));
        clFinish(queue);
        return -1;
    }
    ...
    if (multiple && status[1] != CL_COMPLETE)
    {
        ...
        log_info(
        "\t\tEvent status after waiting for reference event 1: reference event "
