 ...
 */
void get_cl_file_path(const char* folder, const char* test_name,
 ...
 */
void get_bc_file_path(const char* folder, const char* test_name,
                      std::string& bc_file_path, cl_uint size_t_width)
{
    assert(folder && "folder is empty");
    assert(test_name && "test_name is empty");
 ...
 */
void get_h_file_path(const char* folder, const char* file_name,
                     std::string& h_file_path)
{
    assert(folder && "folder is empty");
...
void get_kernel_name(const char* test_name, std::string& kernel_name)
{
    ...
    // the convention is that the test's kernel name is embedded in the test
...
extern "C" void CL_CALLBACK notify_callback(const char* errInfo,
                                            const void* privateInfo, size_t cb,
                                            void* userData);

void create_context_and_queue(cl_device_id device, cl_context* out_context,
 ...
 */
cl_program create_program_from_cl(cl_context context,
                                  const std::string& file_name)
{
    ...
    cl_program program =
    clCreateProgramWithSource(context, 1, &text_str, NULL, &error);
 ...
 */
cl_program create_program_from_bc(cl_context context,
                                  const std::string& file_name)
{
    cl_int load_error = CL_SUCCESS;
    cl_int error;
    size_t binary_size;
    BufferOwningPtr<const unsigned char> binary(
    load_file_bc(file_name, &binary_size));
    const unsigned char* ptr = binary;

    cl_device_id device = get_context_device(context);
    cl_program program = clCreateProgramWithBinary(
    context, 1, &device, &binary_size, &ptr, &load_error, &error);


    if (program == NULL || error != CL_SUCCESS)
    {
        throw Exceptions::TestError("clCreateProgramWithBinary failed: Unable "
                                    "to load valid program binary\n",
                                    error);
    }
    ...
    if (load_error != CL_SUCCESS)
    {
        ...
        "clCreateProgramWithBinary failed: Unable to load valid device binary "
        "into program\n",
        load_error);
    }
 ...
 */
cl_kernel create_kernel_helper(cl_program program,
...
cl_device_id get_program_device(cl_program program)
{
    ...
    int error =
    clGetProgramInfo(program, CL_PROGRAM_DEVICES, sizeof(device), device, NULL);
    if (error != CL_SUCCESS)
    {
...
void generate_kernel_ws(cl_device_id device, cl_kernel kernel, WorkSizeInfo& ws)
{
    ...
    ws.work_dim = 1;
    ws.global_work_size[0] =
    (GLOBAL_WORK_SIZE <= 32) ? GLOBAL_WORK_SIZE : 32; // kernels limitations
    ws.local_work_size[0] = ((GLOBAL_WORK_SIZE % 4) == 0)
    ? (GLOBAL_WORK_SIZE / 4)
    : (GLOBAL_WORK_SIZE / 2);

    // Check if the kernel was compiled with specific work group size
    int error = clGetKernelWorkGroupInfo(
    ...
    {
        // the kernel compiled with __attribute__((reqd_work_group_size(X, Y,
        // Z)))
        memcpy(ws.global_work_size, compile_work_group_size,
               sizeof(ws.global_work_size));

        // Now, check the correctness of the local work size and fix it if
        // necessary
...
TestResult* TestResult::clone(cl_context ctx, const WorkSizeInfo& ws,
                              ...
                              const cl_device_id device) const
{
    ...
    for (size_t i = 0; i < m_kernelArgs.getArgCount(); ++i)
        cpy->m_kernelArgs.addArg(
        m_kernelArgs.getArg(i)->clone(ctx, ws, kernel, device));
...
std::string& DataRow::operator[](int column)
{
    ...
    if ((size_t)column < m_row.size()) return m_row[column];
...
size_t DataTable::getNumRows() const { return m_rows.size(); }

void DataTable::addTableRow(DataRow* dr) { m_rows.push_back(dr); }

const DataRow& DataTable::operator[](int index) const
{
    assert((index > -1 && (size_t)index < m_rows.size())
           && "Index out of bound");
    return *m_rows[index];
}
...
OclExtensions OclExtensions::getDeviceCapabilities(cl_device_id devId)
{
    ...
    size_t set_size;
    cl_int errcode =
    clGetDeviceInfo(devId, CL_DEVICE_EXTENSIONS, 0, NULL, &set_size);
    if (errcode) throw Exceptions::TestError("Device query failed");
    // Querying the device for its supported extensions
    std::vector<char> extensions(set_size);
    errcode = clGetDeviceInfo(devId, CL_DEVICE_EXTENSIONS, extensions.size(),
                              extensions.data(), &size);

    if (errcode) throw Exceptions::TestError("Device query failed");

    char device_profile[1024] = { 0 };
    errcode = clGetDeviceInfo(devId, CL_DEVICE_PROFILE, sizeof(device_profile),
                              device_profile, NULL);
    if (errcode) throw Exceptions::TestError("Device query failed");
...
OclExtensions OclExtensions::empty() { return OclExtensions(0); }
...
#define APPEND_STR_IF_SUPPORTS(STR, E)                                         \
    ...
    APPEND_STR_IF_SUPPORTS(s,
                           OclExtensions::cl_khr_global_int32_extended_atomics);
    APPEND_STR_IF_SUPPORTS(s, OclExtensions::cl_khr_local_int32_base_atomics);
    APPEND_STR_IF_SUPPORTS(s,
                           OclExtensions::cl_khr_local_int32_extended_atomics);
...
DataRow* KhrSupport::parseLine(const std::string& line)
{
    ...
    if (!token.empty()) (*dr)[tIndex] = token;

    assert(tIndex && "empty data row??");
    return dr;
}

OclExtensions KhrSupport::getRequiredExtensions(const char* suite,
                                                const char* test) const
{
    ...
    for (size_t rowIndex = 0; rowIndex < m_dt.getNumRows(); rowIndex++)
    {
        ...
        bool sameSuite = (csvSuite == strSuite),
             sameTest = (csvTest == strTest) || (csvTest == "*");
...
cl_bool KhrSupport::isImages3DRequired(const char* suite,
                                       const char* test) const
{
    cl_bool ret = CL_FALSE;
    const std::string strSuite(suite), strTest(test);
...
static void generate_kernel_args(cl_context context, cl_kernel kernel,
                                 const WorkSizeInfo& ws, KernelArgs& cl_args,
                                 const cl_device_id device)
{
    ...
    error = clGetKernelInfo(kernel, CL_KERNEL_NUM_ARGS, sizeof(num_args),
                            &num_args, NULL);
    if (error != CL_SUCCESS)
    {
    ...
    for (cl_uint j = 0; j < num_args; ++j)
    {
        ...
        error =
        clGetKernelArgInfo(kernel, j, CL_KERNEL_ARG_ADDRESS_QUALIFIER,
        ...
        error =
        clGetKernelArgInfo(kernel, j, CL_KERNEL_ARG_ACCESS_QUALIFIER,
                           sizeof(cl_kernel_arg_access_qualifier),
                           kernel_arg_info.getAccessQualifierRef(), &size);
        if (error != CL_SUCCESS)
        {
            throw Exceptions::TestError(
            "Unable to get argument access qualifier\n", error);
        }

        // Try to get the type qualifier of each argument.
        error =
        clGetKernelArgInfo(kernel, j, CL_KERNEL_ARG_TYPE_QUALIFIER,
                           sizeof(cl_kernel_arg_type_qualifier),
                           kernel_arg_info.getTypeQualifierRef(), &size);
        if (error != CL_SUCCESS)
        {
            throw Exceptions::TestError(
            "Unable to get argument type qualifier\n", error);
        }

        // Try to get the type of each argument.
        memset(name, 0, max_name_len);
        error = clGetKernelArgInfo(kernel, j, CL_KERNEL_ARG_TYPE_NAME,
                                   max_name_len, name, NULL);
        if (error != CL_SUCCESS)
        {
        ...
        memset(name, 0, max_name_len);
        error = clGetKernelArgInfo(kernel, j, CL_KERNEL_ARG_NAME, max_name_len,
                                   name, NULL);
        if (error != CL_SUCCESS)
        {
        ...
        cl_arg = dg->generateKernelArg(context, kernel_arg_info, ws, NULL,
                                       kernel, device);
...
void set_kernel_args(cl_kernel kernel, KernelArgs& args)
{
    ...
    {
        error = clSetKernelArg(kernel, i, args.getArg(i)->getArgSize(),
                               args.getArg(i)->getArgValue());
        if (error != CL_SUCCESS)
        {
...
void generate_kernel_data(cl_context context, cl_kernel kernel,
                          WorkSizeInfo& ws, TestResult& results)
{
    cl_device_id device = get_context_device(context);
...
/**
 Run the single kernel
 */
void run_kernel(cl_kernel kernel, cl_command_queue queue, WorkSizeInfo& ws,
                TestResult& result)
{
    ...
    queue, kernel, ws.work_dim, ws.global_work_offset, ws.global_work_size,
    ws.local_work_size, 0, NULL, &execute_event);
    if (error != CL_SUCCESS)
    {
...
bool compare_results(const TestResult& lhs, const TestResult& rhs, float ulps)
{
    ...
    {
        log_error("number of kernel parameters differ between SPIR and CL "
    ...
    for (size_t i = 0; i < lhs.kernelArgs().getArgCount(); ++i)
    {
        if (!lhs.kernelArgs().getArg(i)->compare(*rhs.kernelArgs().getArg(i),
                                                 ulps))
        {
            log_error("the kernel parameter (%d) is different between SPIR and "
                      "CL version of the kernel\n",
                      i);
            return false;
        }
    }
