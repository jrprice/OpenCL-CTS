...
void get_cl_file_path(const char* folder, const char* str,
                      std::string& cl_file_path);
void get_bc_file_path(const char* folder, const char* str,
                      std::string& bc_file_path, cl_uint size_t_width);
void get_h_file_path(const char* folder, const char* str,
...
void generate_kernel_ws(cl_device_id device, cl_kernel kernel,
                        WorkSizeInfo& ws);
...
public:
     ...
     */
    TestResult* clone(cl_context ctx, const WorkSizeInfo& ws,
...
template <int i> struct KhrValue
{
    enum
    {
        Mask = (1 << i)
...
#define RETURN_IF_ENUM(S, E)                                                   \
    if (S == STRINIGFY(E)) return E
...
public:
    ...
    OclExtensions getRequiredExtensions(const char* suite,
