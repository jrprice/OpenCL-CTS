...
void Task::setErrorLog(cl_program prog)
{
    ...
    cl_int err_code =
    clGetProgramBuildInfo(prog, m_devid, CL_PROGRAM_BUILD_LOG, 0, NULL, &len);
    if (err_code != CL_SUCCESS)
    {
        m_log =
        "Error: clGetProgramBuildInfo(CL_PROGRAM_BUILD_LOG, &len) failed.\n";
        return;
    }
    ...
    {
        m_log =
        "Error: clGetProgramBuildInfo(CL_PROGRAM_BUILD_LOG, &log) failed.\n";
        return;
    }
    m_log.append(&log[0]);
}
...
LinkTask::LinkTask(cl_program* programs, int num_programs, cl_context ctxt,
    ...
    : m_programs(programs), m_numPrograms(num_programs), m_context(ctxt),
      m_executable(NULL), Task(dev, options)
{}
