...
#define GLOBAL_WORK_SIZE                                                       \
    (((CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE / sizeof(cl_double) / 16) / 2)       \
     * 2) // max buffer size = CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE /
          // sizeof(double16)
...
public:
    RandomGenerator(): m_d(NULL) { init(0); }

    ~RandomGenerator()
    {
        if (NULL != m_d) free_mtdata(m_d);
    }

    void init(cl_uint seed) { m_d = init_genrand(seed); }
...
template <>
ESINNS_PREF()
cl_char ESINNS_POST() getNext(cl_char low, cl_char high)
{
...
template <>
ESINNS_PREF()
cl_double ESINNS_POST() getNext(cl_double low, cl_double high)
{
...
template <>
ESINNS_PREF()
cl_float ESINNS_POST() getNext(cl_float low, cl_float high)
{
...
template <>
ESINNS_PREF()
cl_long ESINNS_POST() getNext(cl_long low, cl_long high)
{
...
template <>
ESINNS_PREF()
cl_short ESINNS_POST() getNext(cl_short low, cl_short high)
{
...
template <>
ESINNS_PREF()
cl_uchar ESINNS_POST() getNext(cl_uchar low, cl_uchar high)
{
...
template <>
ESINNS_PREF()
cl_uint ESINNS_POST() getNext(cl_uint low, cl_uint high)
{
...
template <>
ESINNS_PREF()
cl_ulong ESINNS_POST() getNext(cl_ulong low, cl_ulong high)
{
...
template <>
ESINNS_PREF()
cl_ushort ESINNS_POST() getNext(cl_ushort low, cl_ushort high)
{
...
public:
    virtual KernelArg* generate(cl_context context, const WorkSizeInfo& ws,
                                const KernelArgInfo& argInfo,
                                const KernelArg* refArg, const cl_kernel kernel,
                                const cl_device_id device) = 0;
    virtual ~KernelArgGenerator() {}
};

/**
 Mock: 'Not implemented' version of the kernel argument generator - used for the
 still unsupported types
 */
class KernelArgGeneratorNI : public KernelArgGenerator {
public:
    KernelArgGeneratorNI(bool isBuffer, size_t vectorSize, int minValue,
                         int maxValue)
    {}

    KernelArg* generate(cl_context context, const WorkSizeInfo& ws,
                        const KernelArgInfo& argInfo, const KernelArg* refArg,
                        const cl_kernel kernel, const cl_device_id device)
    {
        // assert(false && "Not implemented");
...
public:
    KernelArgGeneratorImage(bool isBuffer, size_t vectorSize, char minValue,
                            char maxValue)
        : m_isBuffer(isBuffer), m_vectorSize(vectorSize), m_minValue(minValue),
          m_maxValue(maxValue)
    {
    ...
    bool isValidChannelOrder(cl_context context, cl_channel_order order) const
    {
        ...
        context, flags, m_desc.image_type, 0, NULL, &actualNumFormats);
        if (CL_SUCCESS != error)
            throw Exceptions::TestError("clGetSupportedImageFormats failed\n",
                                        error);

        std::auto_ptr<cl_image_format> supportedFormats(
        new cl_image_format[actualNumFormats]);
        error = clGetSupportedImageFormats(context, flags, m_desc.image_type,
                                           actualNumFormats,
                                           supportedFormats.get(), NULL);
        if (CL_SUCCESS != error)
            throw Exceptions::TestError("clGetSupportedImageFormats failed\n",
                                        error);
        ...
        for (size_t i = 0; i < actualNumFormats; ++i)
        {
            ...
            if (imgFormat.image_channel_order == curFormat.image_channel_order
                ...
                == curFormat.image_channel_data_type)
                return true;
        }
    ...
    {
        void* pBuffer = NULL;
        size_t numPixels = m_desc.image_width * m_desc.image_height
        * m_desc.image_depth * m_desc.image_array_size;
        const int alignment =
        sizeof(cl_int) * 4; // RGBA channel size * sizeof (cl_int)
        ...
        return new KernelArgImage(context, argInfo, pBuffer, allocSize,
                                  mem_flags, m_format, m_desc);
    }
...
class KernelArgGeneratorImage1dArray : public KernelArgGeneratorImage {
public:
    KernelArgGeneratorImage1dArray(bool isBuffer, size_t vectorSize,
        ...
        : KernelArgGeneratorImage(isBuffer, vectorSize, minValue, maxValue)
    {
        ...
        m_desc.image_row_pitch =
        m_desc.image_width * 4 * 4; // RGBA channel size * sizeof (cl_int)
 ...
 */
template <cl_channel_type channel_type>
class KernelArgGeneratorImage1dBuffer : public KernelArgGeneratorImage {
public:
    KernelArgGeneratorImage1dBuffer(bool isBuffer, size_t vectorSize,
                                    char minValue, char maxValue)
        : KernelArgGeneratorImage(isBuffer, vectorSize, minValue, maxValue)
    {
 ...
 */
template <cl_channel_type channel_type>
class KernelArgGeneratorImage1d : public KernelArgGeneratorImage {
public:
    KernelArgGeneratorImage1d(bool isBuffer, size_t vectorSize, char minValue,
                              char maxValue)
        : KernelArgGeneratorImage(isBuffer, vectorSize, minValue, maxValue)
    {
 ...
 */
template <cl_channel_type channel_type>
class KernelArgGeneratorImage2dArray : public KernelArgGeneratorImage {
public:
    KernelArgGeneratorImage2dArray(bool isBuffer, size_t vectorSize,
                                   char minValue, char maxValue)
        : KernelArgGeneratorImage(isBuffer, vectorSize, minValue, maxValue)
    {
 ...
 */
template <cl_channel_type channel_type>
class KernelArgGeneratorImage2d : public KernelArgGeneratorImage {
public:
    KernelArgGeneratorImage2d(bool isBuffer, size_t vectorSize, char minValue,
                              char maxValue)
        : KernelArgGeneratorImage(isBuffer, vectorSize, minValue, maxValue)
    {
 ...
 */
template <cl_channel_type channel_type>
class KernelArgGeneratorImage3d : public KernelArgGeneratorImage {
public:
    KernelArgGeneratorImage3d(bool isBuffer, size_t vectorSize, char minValue,
                              char maxValue)
        : KernelArgGeneratorImage(isBuffer, vectorSize, minValue, maxValue)
    {
...
public:
    KernelArgGeneratorSampler(bool isBuffer, size_t vectorSize, int minValue,
                              int maxValue);
    ...
    KernelArg* generate(cl_context context, const WorkSizeInfo& ws,
                        ...
                        const cl_kernel kernel, const cl_device_id device)
    {
        return new KernelArgSampler(context, m_normalized, m_addressingMode,
                                    m_filterMode);
    }
...
template <typename T>
class KernelStructTypeArgGenerator : public KernelArgGenerator {

public:
    KernelStructTypeArgGenerator(bool isBuffer, size_t vectorSize,
                                 cl_int minValue, cl_int maxValue)
        : m_isBuffer(isBuffer), m_vectorSize(vectorSize), m_alignment(0),
          m_size(0)
    {}
                        ...
                        const cl_kernel kernel, const cl_device_id device)
    {
        ...
        if (m_isBuffer)
        {
            cl_kernel_arg_address_qualifier addrQ =
            argInfo.getAddressQualifier();

            if (CL_KERNEL_ARG_ADDRESS_CONSTANT == addrQ)
            {
                if ((CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE / m_size) * m_size
                    < m_size)
                    size =
                    (CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE / m_size) * m_size;
            }
...
private:
    ...
    std::string getTypeString(testStruct* pStruct) { return "testStruct"; }
    ...
    void fillBuffer(typedef_struct_type* pStruct, size_t no_e)
    {
        for (size_t e = 0; e < no_e; ++e)
        {
            ...
            {
                pStruct[e].float4d[i] =
                gRG.getNext<cl_float>(-0x01000000, 0x01000000);
            }
    ...
    void fillBuffer(image_kernel_data* pStruct, size_t no_e)
    {
        for (size_t e = 0; e < no_e; ++e)
        {
            ...
            pStruct[e].expectedChannelOrder =
            gRG.getNext<cl_int>(0, 0x7fffffff);
        }
    }
    ...
    // size of the structure is the size of the largest field multiple by the
    // number of fields
...
public:
    KernelArgGeneratorT(bool isBuffer, size_t vectorSize, T minValue,
                        T maxValue)
        : m_isBuffer(isBuffer), m_vectorSize(vectorSize), m_minValue(minValue),
          m_maxValue(maxValue)
    {}
                        ...
                        const cl_kernel kernel, const cl_device_id device)
    {
        ...
        clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE,
                        sizeof(totalDeviceLocalMem), &totalDeviceLocalMem,
                        NULL);
        clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_LOCAL_MEM_SIZE,
                                 sizeof(localMemUsedByKernel),
                                 &localMemUsedByKernel, NULL);
        clGetKernelInfo(kernel, CL_KERNEL_NUM_ARGS, sizeof(numArgs), &numArgs,
                        NULL);
        ...
        {
            error =
            clGetKernelArgInfo(kernel, i, CL_KERNEL_ARG_ADDRESS_QUALIFIER,
                               sizeof(cl_kernel_arg_address_qualifier),
                               kernel_arg_info.getAddressQualifierRef(), &size);
            if (error != CL_SUCCESS)
            {
                throw Exceptions::TestError(
                "Unable to get argument address qualifier\n", error);
            }

            if (kernel_arg_info.getAddressQualifier()
        ...
        // reduce the amount of local memory by the amount the kernel +
        // implementation uses
        ...
        if (m_isBuffer)
        {
            ...
            // decide about the buffer size - take into account the alignment
            // and padding
            ...
            if (numLocalArgs
                && size
                > floor(static_cast<double>(totalDeviceLocalMem / numLocalArgs))
                ...
                || CL_KERNEL_ARG_ADDRESS_CONSTANT == addrQ)
            {
                ...
                {
                    throw Exceptions::TestError(
                    "align_malloc failed for array buffer\n", 1);
                }
                assert((size_t)pBuffer % alignment == 0);
                if (NULL == refArg)
                {
...
public:
    ...
    KernelArg*
    generateKernelArg(cl_context context, const KernelArgInfo& argInfo,
                      const WorkSizeInfo& ws, const KernelArg* refArg,
                      const cl_kernel kernel, const cl_device_id device)
    {
        KernelArgGenerator* pArgGenerator = getArgGenerator(argInfo);
        return pArgGenerator->generate(context, ws, argInfo, refArg, kernel,
                                       device);
    }
...
public:
    ...
    public:
        ...
        /*
         * Returns the data type of the image object this iterator creates.
         * (e.g., cl_float, cl_int).
         */
        ...
        /*
         * Returns the data type of the image object this iterator creates.
         * (e.g., float, int), in string format.
         */
    ...
    private:
         ...
         * In case the new value reaches the limit, the index is reset to hold
         * zero. Returns: true if the value of the index was incremented, false
         * if it was reset to zero.
         */
