                ...
                const char *extension)
{
    ...
    for (unsigned int i = 0; i < number_of_tests; ++i)
    {
            ...
            && (!is_extension_available(device, extension)))
        {
            ...
            std::cout
            << test_name[i]
            << "... Skipped. (Cannot run on device due to missing extension: "
            << extension << " )." << std::endl;
            continue;
        }
...
static bool test_kernel_attributes(cl_device_id device, cl_uint width,
                                   const char *folder)
{
    try_extract(folder);
    std::cout << "Running tests:" << std::endl;
    ...
    try
    {
        ...
        if (!build_task.execute())
        {
            std::cerr << "Cannot run kernel_attributes suite due to the "
                         "following build error: "
                      << build_task.getErrorLog() << std::endl;
            throw std::exception();
        }
        ...
        cl_int err_code =
        clGetKernelInfo(kernel, CL_KERNEL_ATTRIBUTES, sizeof(attributes),
                        attributes, &res_size);
        if (err_code != CL_SUCCESS)
        {
            std::cerr
            << "clGetKernelInfo unable retrieve kernel attributes (error code: "
            << err_code << " )\n";
            throw std::exception();
        }
        ...
        for (i = 0; i < expected.size(); ++i)
        {
            ...
            {
                // Attribute not found
                std::cout << "Extracted from kernel: " << attributes
                          << std::endl;
                std::cerr << "expected " << expected[i]
                          << " attribute not found" << std::endl;
                throw std::exception();
            }
        }
        (SuccE)(test_name, "");
                            ...
                            cl_uint *platform_index, cl_uint *size_t_width)
{
            ...
            || !strcmp(argv[1], "--help")))
    {
        ...
        log_info(
        "\tno-unzip\t\tDo not extract test files from Zip; use existing.\n");

        for (unsigned int i = 0; i < (sizeof(spir_suites) / sizeof(sub_suite));
             i++)
        {
            log_info("\t\t%s\n", spir_suites[i].name);
        }
...
int main(int argc, const char *argv[])
{
    ...
    try
    {
        ...
        if (!is_extension_available(device, "cl_khr_spir")
            || std::find(versions.begin(), versions.end(), 1.2f)
            == versions.end())
        {
            log_info(
