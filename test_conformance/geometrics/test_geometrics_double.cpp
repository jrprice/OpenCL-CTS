...
"{\n"
"    int  tid = get_global_id(0);\n"
"    destValues[tid] = %s( vload3( tid, (__global double*) sourceA), vload3( "
"tid, (__global double*) sourceB ) );\n"
"\n"
"}\n";
...
"{\n"
"    int  tid = get_global_id(0);\n"
"    vstore3( %s( vload3( tid, (__global double*) sourceA) ), tid, (__global "
"double*) destValues );\n"
"\n"
"}\n";
                          ...
                          double ulpTolerance)
{
    ...
    * (ulpTolerance * FLT_EPSILON); // This gives us max squared times ulp
                                    // tolerance, i.e. the worst-case expected
                           ...
                           cl_command_queue queue, int num_elements, MTdata d)
{
    ...
    for (vecsize = 3; vecsize <= 4; ++vecsize)
    {
        ...
        context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR), bufSize, inDataA, NULL);
        if (streams[0] == NULL)
        {
        ...
        context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR), bufSize, inDataB, NULL);
        if (streams[1] == NULL)
        {
        ...
        for (i = 0; i < size; i++)
        {
                ...
                || errs[2] > errorTolerances[2])
            {
                ...
                log_error(
                "    Errors: (%a out of %a), (%a out of %a), (%a out of %a)\n",
                errs[0], errorTolerances[0], errs[1], errorTolerances[1],
                errs[2], errorTolerances[2]);
                log_error(
                "     ulp %g\n",
                                  ...
                                  double ulpLimit, MTdata d)
{
    ...
    /* Now get the results */
    error =
    clEnqueueReadBuffer(queue, streams[2], true, 0,
                        sizeof(cl_double) * TEST_SIZE, outData, 0, NULL, NULL);
    test_error(error, "Unable to read output array!");
        ...
        if ((double)expected != outData[i])
        {
            ...
            else
            {
                ...
                if (fabs(error) > ulpLimit)
                {
                    ...
                    "Expected (%a), got (%a), sources (%a and %a) ulp of %f\n",
                    (int)i, (int)vecSize, expected, outData[i],
                    inDataA[i * vecSize], inDataB[i * vecSize], error);
                                   ...
                                   MTdata d)
{
    ...
    for (size = 0; sizes[size] != 0; size++)
    {
        ...
        (1.5f * (double)sizes[size]
         + // cumulative error for multiplications  (a-b+0.5ulp)**2 = (a-b)**2 +
           // a*0.5ulp + b*0.5 ulp + 0.5 ulp for multiplication
                                  ...
                                  double ulpLimit, MTdata d)
{
    ...
    /* Now get the results */
    error =
    clEnqueueReadBuffer(queue, streams[1], true, 0,
                        sizeof(cl_double) * TEST_SIZE, outData, 0, NULL, NULL);
    test_error(error, "Unable to read output array!");
        ...
        for (j = 0; j < vecSize; j++)
        {
            ...
            if (expected[j] != outData[i * vecSize + j])
            {
                ...
                if (fabs(error) > ulpLimit)
                {
                    ...
                    outData[i * vecSize + j], outData[i * vecSize + j], error);
                    log_error("       Source: ");
