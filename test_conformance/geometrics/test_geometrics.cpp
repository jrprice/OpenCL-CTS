...
"{\n"
"    int  tid = get_global_id(0);\n"
"    destValues[tid] = %s( vload3( tid, (__global float*) sourceA), vload3( "
"tid, (__global float*) sourceB) );\n"
"\n"
"}\n";
...
"{\n"
"    int  tid = get_global_id(0);\n"
"    vstore3( %s( vload3( tid, (__global float*) sourceA) ), tid, (__global "
"float*) destValues );\n"
"\n"
"}\n";
                           ...
                           MTdata d)
{
    ...
    clEnqueueReadBuffer(queue, streams[2], true, 0,
                        sizeof(cl_float) * TEST_SIZE, outData, 0, NULL, NULL);
    test_error(error, "Unable to read output array!");
                         ...
                         int softball, MTdata d)
{
    ...
    for (i = 0; i < TEST_SIZE; i++)
    {
        ...
        {
            log_error(
            "ERROR: Data sample {%d,%d} at size %d does not validate! Expected "
            "%12.24f (%a), got %12.24f (%a), ulp %f\n",
