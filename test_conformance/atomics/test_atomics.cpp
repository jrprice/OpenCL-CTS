...
const char *atomic_global_pattern[] = {
    "__kernel void test_atomic_fn(volatile __global %s *destMemory, __global "
    "%s *oldValues)\n"
    "{\n"
...
const char *atomic_local_pattern[] = {
    "__kernel void test_atomic_fn(__global %s *finalDest, __global %s "
    "*oldValues, volatile __local %s *destMemory, int numDestItems )\n"
    "{\n"
    "    int  tid = get_global_id(0);\n"
    "     int  dstItemIdx;\n"
    "\n"
    "    // Everybody does the following line(s), but it all has the same "
    "result. We still need to ensure we sync before the atomic op, though\n"
    "     for( dstItemIdx = 0; dstItemIdx < numDestItems; dstItemIdx++ )\n"
    "        destMemory[ dstItemIdx ] = finalDest[ dstItemIdx ];\n"
    "    barrier( CLK_LOCAL_MEM_FENCE );\n"
    "\n",
    "    barrier( CLK_LOCAL_MEM_FENCE );\n"
    "    // Finally, write out the last value. Again, we're synced, so "
    "everyone will be writing the same value\n"
    "     for( dstItemIdx = 0; dstItemIdx < numDestItems; dstItemIdx++ )\n"
                          ...
                          ExplicitType dataType)
{
    const char *extensionNames[8] = {
        ...
        "cl_khr_int64_base_atomics",
        "cl_khr_int64_extended_atomics" // this line intended to be the same as
                                        // the last one
    };
                         ...
                         bool matchGroupSize)
{
    ...
    clCreateBuffer(context,
                   (cl_mem_flags)((startRefValues != NULL ? CL_MEM_COPY_HOST_PTR
                                                          : CL_MEM_READ_WRITE)),
    ...
    {
        log_error(
        "ERROR: Test doesn't check total or refs; no values are verified!\n");
        return -1;
    }
    ...
    for (size_t i = 0; i < numDestItems; i++)
        memcpy(destItems + i * typeSize, startValue, typeSize);
    error =
    clEnqueueWriteBuffer(queue, streams[0], true, 0, typeSize * numDestItems,
                         destItems, 0, NULL, NULL);
                             ...
                             bool usingAtomicPrefix)
{
    ...
    int errors = 0;
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements, programCore,
                         testFns, extended, false, kInt, matchGroupSize);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements, programCore,
                         testFns, extended, false, kUInt, matchGroupSize);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements, programCore,
                         testFns, extended, true, kInt, matchGroupSize);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements, programCore,
                         testFns, extended, true, kUInt, matchGroupSize);
                     ...
                     cl_command_queue queue, int num_elements)
{
    ...
    deviceID, context, queue, num_elements, atom_xchg_core, set, false, true,
...
{
    for (size_t i = 0; i < size; i++)
        startRefValues[i] = (cl_long)(
        genrand_int32(d) | (((cl_long)genrand_int32(d) & 0x7fffffffL) << 16));
}
                        ...
                        cl_command_queue queue, int num_elements)
{
    ...
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements,
                         atom_cmpxchg64_core, set, false, false, kLong, true);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements,
                         atom_cmpxchg64_core, set, false, false, kULong, true);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements,
                         atom_cmpxchg64_core, set, false, true, kLong, true);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements,
                         atom_cmpxchg64_core, set, false, true, kULong, true);

    log_info("    Testing atomic_ functions...\n");
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements,
                         atomic_cmpxchg_core, set, false, false, kInt, true);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements,
                         atomic_cmpxchg_core, set, false, false, kUInt, true);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements,
                         atomic_cmpxchg_core, set, false, true, kInt, true);
    errors |=
    test_atomic_function(deviceID, context, queue, num_elements,
                         atomic_cmpxchg_core, set, false, true, kUInt, true);
