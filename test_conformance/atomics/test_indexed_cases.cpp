...
"thread ID.\n"
"// At the end of this we should have all thread IDs in some random location "
"in counts\n"
"// exactly once. If atom_add failed then we will write over various thread "
"IDs and we\n"
...
const char *add_index_bin_kernel[] = {
    "#pragma OPENCL EXTENSION cl_khr_global_int32_base_atomics : enable\n"
    "// This test assigns a bunch of values to bins and then tries to put them "
    "in the bins in parallel\n"
    "// using an atomic add to keep track of the current location to write "
    "into in each bin.\n"
    "// This is the same as the memory update for the particles demo.\n"
    "\n"
    "__kernel void add_index_bin_test(__global int *bin_counters, __global int "
    "*bins, __global int *bin_assignments, int max_counts_per_bin) {\n"
    "    int tid = get_global_id(0);\n"
    "\n"
                       ...
                       cl_context context, MTdata d)
{
    ...
    context, &program, &kernel, 1, add_index_bin_kernel, "add_index_bin_test");
    ...
    clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_ONLY),
    ...
    queue, bins, true, 0, sizeof(cl_int) * number_of_bins * max_counts_per_bin,
    values, 0, NULL, NULL);
    if (err)
    {
        log_error(
    ...
    for (i = 0; i < number_of_items; i++)
    {
        ...
        if (l_bin_counts[bin] + 1 > max_counts_per_bin)
            log_error(
            "add_index_bin_test internal error generating bin assignments: bin "
            "%d has more entries (%d) than max_counts_per_bin (%d).\n",
    ...
    err |=
    clSetKernelArg(kernel, 3, sizeof(max_counts_per_bin), &max_counts_per_bin);
    if (err)
    {
