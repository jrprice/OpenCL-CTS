...
int test_load_program_source(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    ...
    program =
    clCreateProgramWithSource(context, 1, sample_kernel_code_single_line,
                              sample_single_line_lengths, &error);
    ...
    // Note: according to spec section 5.4.5, the length returned should include
    // the null terminator
...
int test_load_multistring_source(cl_device_id deviceID, cl_context context,
                                 cl_command_queue queue, int num_elements)
{
    int error;
    clProgramWrapper program;
    ...
    program =
    clCreateProgramWithSource(context, 7, sample_kernel_code_multi_line,
                              sample_multi_line_lengths, &error);
    if (program == NULL)
    {
...
int test_load_two_kernel_source(cl_device_id deviceID, cl_context context,
                                cl_command_queue queue, int num_elements)
{
    int error;
    cl_program program;
    ...
    program = clCreateProgramWithSource(context, 2, sample_kernel_code_two_line,
                                        sample_two_line_lengths, &error);
    if (program == NULL)
    {
...
int test_load_null_terminated_source(cl_device_id deviceID, cl_context context,
                                     cl_command_queue queue, int num_elements)
{
    int error;
    cl_program program;
    ...
    context, 1, sample_kernel_code_single_line, NULL, &error);
    if (program == NULL)
    {
...
int test_load_null_terminated_multi_line_source(cl_device_id deviceID,
                                                cl_context context,
                                                ...
                                                int num_elements)
{
    ...
    /* Now create a program using the macro function */
    program = clCreateProgramWithSource(
    context, 7, sample_kernel_code_multi_line, NULL, &error);
    if (program == NULL)
    {
        log_error("ERROR: Unable to create null-terminated program!");
        return -1;
...
int test_load_discreet_length_source(cl_device_id deviceID, cl_context context,
                                     cl_command_queue queue, int num_elements)
{
    int error;
    cl_program program;
    ...
    for (i = 0; i < 7; i++)
    {
        sample_bad_multi_line_lengths[i] =
        strlen(sample_kernel_code_bad_multi_line[i]);
    }
    ...
    /* Now create a program using the macro function */
    program =
    clCreateProgramWithSource(context, 7, sample_kernel_code_bad_multi_line,
                              sample_bad_multi_line_lengths, &error);
    if (program == NULL)
    {
        log_error("ERROR: Unable to create null-terminated program!");
        return -1;
...
int test_load_null_terminated_partial_multi_line_source(cl_device_id deviceID,
                                                        cl_context context,
                                                        cl_command_queue queue,
                                                        int num_elements)
{
    ...
    {
        if (i & 0x01)
            sample_multi_line_lengths[i] =
            0; /* Should force for null-termination on this line only */
        else
            sample_multi_line_lengths[i] =
            strlen(sample_kernel_code_multi_line[i]);
    }
...
int test_get_program_info(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    int error;
    cl_program program;
    ...
    context, &program, 1, sample_kernel_code_single_line);
    if (program == NULL)
    {
        log_error("ERROR: Unable to create reference program!\n");
        return -1;
    ...
    error = clGetProgramInfo(program, CL_PROGRAM_DEVICES, sizeof(device1),
                             &device1, NULL);
    ...
    cl_uint devCount;
    error = clGetProgramInfo(program, CL_PROGRAM_NUM_DEVICES, sizeof(devCount),
                             &devCount, NULL);
    test_error(error, "Unable to get device count of program");

    if (devCount != 1)
    {
        log_error("ERROR: Invalid device count returned for program! (Expected "
                  "1, got %d)\n",
                  (int)devCount);
        return -1;
    }
    ...
    error = clGetProgramInfo(program, CL_PROGRAM_CONTEXT, sizeof(context1),
                             &context1, NULL);
    test_error(error, "Unable to get device of program");
    ...
    {
        log_error(
        "ERROR: Invalid context returned for program! (Expected %p, got %p)\n",
        context, context1);
        return -1;
    }

    error = clGetProgramInfo(program, CL_PROGRAM_REFERENCE_COUNT,
    ...
    error =
    clGetProgramInfo(program, CL_PROGRAM_REFERENCE_COUNT, 0, NULL, &paramSize);
    ...
    error =
    clGetProgramInfo(program, CL_PROGRAM_NUM_DEVICES, 0, NULL, &paramSize);
...
int test_get_program_source(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    cl_program program;
    int error;
    ...
    /* Try normal source */
    error =
    clGetProgramInfo(program, CL_PROGRAM_SOURCE, sizeof(buffer), buffer, NULL);
    ...
    /* Try both at once */
    error = clGetProgramInfo(program, CL_PROGRAM_SOURCE, sizeof(buffer), buffer,
                             &length);
    test_error(error, "Unable to get program source");
    if (strlen(buffer) != strlen(sample_kernel_code_single_line[0])
        && gCompilationMode == kOnline)
...
int test_get_program_build_info(cl_device_id deviceID, cl_context context,
                                cl_command_queue queue, int num_elements)
{
    cl_program program;
    int error;
    ...
    error = clGetProgramBuildInfo(program, deviceID, CL_PROGRAM_BUILD_STATUS, 0,
                                  NULL, &length);
    test_error(error, "Unable to get program build status length");
    if (length != sizeof(status))
    {
        log_error("ERROR: Returned length of program build status is invalid! "
                  "(Expected %d, got %d)\n",
    ...
    {
        log_error("ERROR: Getting built program build status did not return "
                  "CL_BUILD_SUCCESS! (%d)\n",
                  (int)status);
        return -1;
    }
    ...
    error =
    clBuildProgram(program, 1, &deviceID, "-cl-opt-disable", NULL, NULL);
    if (error != CL_SUCCESS)
    {
    ...
    {
        log_error(
        "ERROR: Getting program build options for program with -cl-opt-disable "
        "build options did not return expected value (got %s)\n",
        buffer);
        return -1;
    }
