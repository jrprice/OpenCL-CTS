...
const char *compile_static_struct =
"static constant struct bar {float x, y, z, r; int color; } foo = {3.14159};\n";
                                  ...
                                  cl_command_queue queue, unsigned int numLines)
{
    ...
    {
        log_error(
        "ERROR: Unable to allocate kernels array with %d kernels! (in %s:%d)\n",
        numLines, __FILE__, __LINE__);
        return -1;
    }
    /* First, allocate the array for our line pointers */
    ...
    /* Compile it */
    error =
    clCompileProgram(program, 1, &deviceID, NULL, 0, NULL, NULL, NULL, NULL);
    ...
    clLinkProgram(context, 1, &deviceID, NULL, 2, programs, NULL, NULL, &error);
                                         ...
                                         unsigned int numLines)
{
    ...
    {
        log_error(
        "ERROR: Unable to allocate headers array with %d lines! (in %s:%d)\n",
        numLines, __FILE__, __LINE__);
        return -1;
    }
    /* First, allocate the array for our line pointers */
                                  ...
                                  cl_command_queue queue, unsigned int numLines)
{
    ...
    {
        my_program_and_libraries[i + 1] =
        clLinkProgram(context, 1, &deviceID, "-create-library", numFilesInLib,
                              ...
                              cl_command_queue queue, unsigned int numLines)
{
    ...
    context, 1, &deviceID, NULL, 1 + numLines, my_programs, NULL, NULL, &error);
    test_error(error, "Unable to link a program with a library");
                                      ...
                                      cl_command_queue queue, int num_elements)
{
    ...
    test_error(
    error,
    "clWaitForEvents failed when waiting on compile_program_completion_event");
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    cl_program my_newly_linked_program =
    clLinkProgram(context, 1, &deviceID, NULL, 1, &program, NULL, NULL, &error);
                                          ...
                                          int num_elements)
{
    ...
    error =
    create_single_kernel_helper_create_program(context, &program, 2, sources);
    if (program == NULL || error != CL_SUCCESS)
    {
                                        ...
                                        int num_elements)
{
    ...
    error = clCompileProgram(second_program, 1, &deviceID, NULL, 0, NULL, NULL,
                             NULL, NULL);
    test_error(
    error, "Unable to compile a program that tries to access a regular struct");
                            ...
                            cl_kernel kernel)
{
    ...
    if (srcBuffer && dstBuffer)
    {
        ...
        context, CL_MEM_WRITE_ONLY, cnDimension * sizeof(cl_float), 0, &error);
...
int num_elements)
{
    ...
    error = create_single_kernel_helper_create_program(
    context, &program, 1, &simple_kernel_with_defines, "-DFIRST=5 -DSECOND=37");
    if (program == NULL || error != CL_SUCCESS)
    {
        log_error(
        "ERROR: Unable to create a simple test program! (%s in %s:%d)\n",
                                               ...
                                               int num_elements)
{
    ...
    log_info(
    "Testing execution after serialization and reloading of the object...\n");
    error = create_single_kernel_helper_create_program(context, &program, 1,
                                                       &simple_kernel);
    if (program == NULL || error != CL_SUCCESS)
    {
    ...
    cl_program my_newly_linked_program = clLinkProgram(
    context, 1, &deviceID, NULL, 1, &program_with_binary, NULL, NULL, &error);
    test_error(error, "Unable to link a simple program");
                                                ...
                                                int num_elements)
{
    ...
    context, 1, &deviceID, "-create-library", 1, &program, NULL, NULL, &error);
...
int num_elements)
{
    ...
    log_info(
    "Testing execution after a simple compile and link with callbacks...\n");
    error = create_single_kernel_helper_create_program(context, &program, 1,
                                                       &simple_kernel);
    if (program == NULL || error != CL_SUCCESS)
    {
    ...
    cl_program my_newly_linked_program = clLinkProgram(
    context, 1, &deviceID, NULL, 1, &program, program_link_completion_callback,
                                            ...
                                            int num_elements)
{
    ...
    {
        log_error(
        "ERROR: Unable to change to original working directory! (in %s:%d)\n",
        __FILE__, __LINE__);
        return -1;
    }
#if (defined(__linux__) || defined(__APPLE__)) && (!defined(__ANDROID__))
                             ...
                             cl_command_queue queue, int num_elements)
{
    ...
    error =
    clGetProgramBuildInfo(program, deviceID, CL_PROGRAM_BINARY_TYPE,
    ...
    // Create a buffer and get the actual binary
    {
        ...
        if (program_type != CL_PROGRAM_BINARY_TYPE_COMPILED_OBJECT)
        {
            ...
            "object to be CL_PROGRAM_BINARY_TYPE_COMPILED_OBJECT (in %s:%d)\n",
            __FILE__, __LINE__);
            return -1;
        }
        program_type = -1;
    ...
    test_error(error, "Unable to create program with binary");
    error =
    clGetProgramBuildInfo(library_with_binary, deviceID, CL_PROGRAM_BINARY_TYPE,
                          sizeof(cl_program_binary_type), &program_type, NULL);
    test_error(error, "Unable to get program binary type");
    if (program_type != CL_PROGRAM_BINARY_TYPE_LIBRARY)
    {
                                                   ...
                                                   unsigned int numLines)
{
    ...
    error =
    clGetProgramBuildInfo(my_newly_minted_library, deviceID,
                          CL_PROGRAM_BUILD_LOG, size_ret, library_log, NULL);
    if (error != CL_SUCCESS)
    {
    ...
    test_error(
    error,
    "Device failed to return library creation options.\nclGetProgramBuildInfo "
    "CL_PROGRAM_BUILD_OPTIONS failed");
    ...
    error =
    clGetProgramBuildInfo(my_newly_linked_program, deviceID,
                          CL_PROGRAM_BUILD_LOG, size_ret, linking_log, NULL);
    if (error != CL_SUCCESS)
    {
                                             ...
                                             int num_elements)
{
    ...
    for (i = 0; toTest[i] != 0; i++)
    {
        ...
        {
            log_error(
            "ERROR: large program compilation, linking, status, options and "
            "logging test failed for %d lines! (in %s:%d)\n",
            toTest[i], __FILE__, __LINE__);
            return -1;
        }
