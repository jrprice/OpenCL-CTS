...
void CL_CALLBACK test_notify_build_complete(cl_program program, void *userData)
{
    ...
    {
        log_error("ERROR: User data passed in to build notify function was not "
                  "correct!\n");
                     ...
                     cl_command_queue queue, int num_elements)
{
    ...
    error = clBuildProgram(program, 1, &deviceID, NULL,
                           test_notify_build_complete, (void *)"userData");
    ...
    {
        log_error(
        "ERROR: Async build completed, but build notification was not sent!\n");
        return -1;
    }
