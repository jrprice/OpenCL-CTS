...
const char *define_kernel_code[] = {
    " #define VALUE\n"
    "__kernel void define_test(__global int *src, __global int *dstA, __global "
    "int *dstB)\n"
    "{\n"
...
int test_preprocessor_define_udef(cl_device_id deviceID, cl_context context,
                                  cl_command_queue queue, int num_elements)
{
    ...
    error =
    clEnqueueWriteBuffer(queue, buffer[0], CL_TRUE, 0,
                         num_elements * sizeof(cl_int), srcData, 0, NULL, NULL);
...
int test_preprocessor_include(cl_device_id deviceID, cl_context context,
                              cl_command_queue queue, int num_elements)
{
...
int test_preprocessor_line_error(cl_device_id deviceID, cl_context context,
                                 cl_command_queue queue, int num_elements)
{
    ...
    program =
    clCreateProgramWithSource(context, 1, line_error_kernel_code, NULL, &error);
    ...
    {
        error2 = clGetProgramBuildInfo(program, deviceID, CL_PROGRAM_BUILD_LOG,
...
const char *pragma_kernel_code[] = {
    ...
    "{\n"
    "#pragma A fool thinks himself to be wise, but a wise man knows himself to "
    "be a fool.\n"
    " int tid = get_global_id(0);\n"
    "#pragma\n"
    " dstA[tid] = tid;\n"
    "#pragma  mark Though I am not naturally honest, I am so sometimes by "
    "chance.\n"
    "\n"
    "}\n"
};
...
int test_preprocessor_pragma(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
