...
const char *pipe_readwrite_errors_kernel_code = {
    "__kernel void test_pipe_write_error(__global int *src, __write_only pipe "
    "int out_pipe, __global int *status)\n"
    "{\n"
    ...
    "}\n"
    "\n"
    "__kernel void test_pipe_read_error(__read_only pipe int in_pipe, __global "
    "int *dst, __global int *status)\n"
    "{\n"
    "    int gid = get_global_id(0);\n"
    "    int reserve_idx;\n"
    "    reserve_id_t res_id;\n"
...
int test_pipe_readwrite_errors(cl_device_id deviceID, cl_context context,
                               cl_command_queue queue, int num_elements)
{
    ...
    pipe = clCreatePipe(context, CL_MEM_HOST_NO_ACCESS, sizeof(int),
                        num_elements - (num_elements / 2), NULL, &err);
    if (err)
    {
        clReleaseMemObject(buffers[0]);
        clReleaseMemObject(buffers[1]);
    ...
    err = clEnqueueNDRangeKernel(queue, kernel[1], 1, NULL, global_work_size,
                                 NULL, 0, NULL, &consumer_sync_event);
    if (err != CL_SUCCESS)
    {
    ...
    err = clEnqueueReadBuffer(queue, buffers[2], true, 0, sizeof(status),
                              &status, 1, &consumer_sync_event, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    err = clEnqueueReadBuffer(queue, buffers[2], true, 0, sizeof(status),
                              &status, 1, &producer_sync_event, NULL);
    if (err != CL_SUCCESS)
    {
        print_error(err, " clEnqueueReadBuffer failed");
        clReleaseMemObject(buffers[0]);
