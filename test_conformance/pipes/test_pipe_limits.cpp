...
void createKernelSourceCode(char *source, int num_pipes)
{
    ...
    strcpy(source,
           "__kernel void test_multiple_pipe_write(__global int *src, ");
    ...
    {
        sprintf(str,
                "  else if(gid < (%d*get_global_size(0))/num_pipes)\n  {\n    "
                "res_id = reserve_write_pipe(pipe%d, 1);\n    "
                "if(is_valid_reserve_id(res_id))\n    {\n",
                i + 1, i);
        strcat(source, str);
        sprintf(str,
                "      write_pipe(pipe%d, res_id, 0, &src[gid]);\n      "
    ...
    for (i = 1; i < num_pipes; i++)
    {
        ...
        strcat(source, str);
        sprintf(str,
                "      read_pipe(pipe%d, res_id, 0, &dst[gid]);\n      "
...
int test_pipe_max_args(cl_device_id deviceID, cl_context context,
                       cl_command_queue queue, int num_elements)
{
    ...
    if (max_pipe_args < 16)
    {
        log_error("The device should support minimum 16 pipe objects that "
                  "could be passed as arguments to the kernel");
        return -1;
    }
    ...
    buffers[1] =
    clCreateBuffer(context, CL_MEM_USE_HOST_PTR, size, outptr, &err);
    if (err)
    {
        clReleaseMemObject(buffers[0]);
    ...
    {
        pipes[i] = clCreatePipe(context, CL_MEM_HOST_NO_ACCESS, sizeof(int),
                                num_pipe_elements, NULL, &err);
        if (err)
        {
            clReleaseMemObject(buffers[0]);
            clReleaseMemObject(buffers[1]);
    ...
    context, &program, &kernel[0], 1, (const char **)&source, kernelName[0],
    "-cl-std=CL2.0");
    if (err)
    {
        clReleaseMemObject(buffers[0]);
    ...
    {
        err |=
        clSetKernelArg(kernel[0], i + 1, sizeof(cl_mem), (void *)&pipes[i]);
    }
    err |= clSetKernelArg(kernel[0], max_pipe_args + 1, sizeof(int),
                          (void *)&max_pipe_args);
    err |= clSetKernelArg(kernel[1], 0, sizeof(cl_mem), (void *)&buffers[1]);
    for (i = 0; i < max_pipe_args; i++)
    {
        err |=
        clSetKernelArg(kernel[1], i + 1, sizeof(cl_mem), (void *)&pipes[i]);
    }
    err |= clSetKernelArg(kernel[1], max_pipe_args + 1, sizeof(int),
                          (void *)&max_pipe_args);
    if (err != CL_SUCCESS)
    {
        clReleaseMemObject(buffers[0]);
    ...
    err = clEnqueueReadBuffer(queue, buffers[1], true, 0, size, outptr, 1,
...
int test_pipe_max_packet_size(cl_device_id deviceID, cl_context context,
                              cl_command_queue queue, int num_elements)
{
    cl_mem pipe;
    cl_mem buffers[2];
    ...
    pipe = clCreatePipe(context, CL_MEM_HOST_NO_ACCESS, max_pipe_packet_size,
                        num_pipe_elements, NULL, &err);
    if (err)
    {
        clReleaseMemObject(buffers[0]);
    ...
    sprintf(str,
            "typedef struct{\n  char a[%d];\n}TestStruct;\n\n__kernel void "
            "test_pipe_max_packet_size_write(__global TestStruct *src, "
            "__write_only pipe TestStruct out_pipe)\n{\n",
    ...
    strcat(source, str);
    sprintf(str,
            "__kernel void test_pipe_max_packet_size_read(__read_only pipe "
            "TestStruct in_pipe, __global TestStruct *dst)\n{\n");
    strcat(source, str);
    strcat(source, "  int gid = get_global_id(0);\n  reserve_id_t res_id;\n\n");
    sprintf(str,
...
int test_pipe_max_active_reservations(cl_device_id deviceID, cl_context context,
                                      cl_command_queue queue, int num_elements)
{
    cl_mem pipe;
    cl_mem buffers[2];
    ...
    clGetDeviceInfo(deviceID, CL_DEVICE_GLOBAL_MEM_SIZE,
                    sizeof(max_global_size), (void *)&max_global_size, NULL);
    if (err)
    {
        print_error(err, " clGetDeviceInfo failed\n");
        return -1;
    ...
    buf_reserve_id_t_size = clCreateBuffer(
    context, CL_MEM_HOST_READ_ONLY, sizeof(reserve_id_t_size), NULL, &err);
    if (err)
    {
    ...
    sprintf(
    str,
    "__kernel void test_pipe_max_active_reservations_write(__global int *src, "
    "__write_only pipe int out_pipe, __global char *reserve_id, __global int "
    "*reserve_id_t_size_aligned, __global int *status)\n{\n");
    strcpy(source, str);
    sprintf(str,
    ...
    strcat(source, str);
    sprintf(str,
            "  for(reserve_idx = 0; reserve_idx < %d; reserve_idx++)\n  {\n",
            max_active_reservations);
    strcat(source, str);
    sprintf(str,
            "    res_id_ptr = (__global reserve_id_t*)(reserve_id + "
            "reserve_idx*reserve_id_t_size_aligned[0]);\n");
    strcat(source, str);
    ...
    sprintf(str,
            "    res_id_ptr = (__global reserve_id_t*)(reserve_id + "
            "commit_idx*reserve_id_t_size_aligned[0]);\n");
    strcat(source, str);
    sprintf(str, "    commit_write_pipe(out_pipe, res_id_ptr[0]);\n  }\n}\n\n");
    strcat(source, str);
    sprintf(
    str,
    "__kernel void test_pipe_max_active_reservations_read(__read_only pipe int "
    "in_pipe, __global int *dst, __global char *reserve_id, __global int "
    "*reserve_id_t_size_aligned, __global int *status)\n{\n");
    strcat(source, str);
    sprintf(str,
            "  __global reserve_id_t *res_id_ptr;\n  int reserve_idx;\n  int "
    ...
    strcat(source, str);
    sprintf(str,
            "__kernel void pipe_get_reserve_id_t_size(__global int "
            "*reserve_id_t_size) \n");
    strcat(source, str);
    ...
    err = clSetKernelArg(kernel[2], 0, sizeof(cl_mem),
                         (void *)&buf_reserve_id_t_size);
    if (err)
    {
        clReleaseMemObject(buf_reserve_id_t_size);
        print_error(err, "Error creating program\n");
    ...
    err = clEnqueueNDRangeKernel(queue, kernel[2], 1, NULL, global_work_size,
                                 NULL, 0, NULL, &sync_event);
    if (err != CL_SUCCESS)
    {
        print_error(err, " clEnqueueNDRangeKernel failed");
        clReleaseMemObject(buf_reserve_id_t_size);
    ...
    err = clEnqueueReadBuffer(queue, buf_reserve_id_t_size, true, 0,
                              sizeof(reserve_id_t_size), &reserve_id_t_size, 1,
                              &sync_event, &read_event);
    if (err != CL_SUCCESS)
    {
        print_error(err, " clEnqueueReadBuffer failed");
        clReleaseMemObject(buf_reserve_id_t_size);
    ...
    temp = 1;
    while (temp < reserve_id_t_size) temp *= 2;
    ...
    buf_reserve_id_t_size_aligned =
    clCreateBuffer(context, CL_MEM_COPY_HOST_PTR, sizeof(reserve_id_t_size),
                   &reserve_id_t_size, &err);
    if (err)
    {
        clReleaseMemObject(buf_reserve_id_t_size);
    ...
    // For error status
    buf_status =
    clCreateBuffer(context, CL_MEM_COPY_HOST_PTR, sizeof(int), &status, &err);
    if (err)
    {
    ...
    pipe = clCreatePipe(context, CL_MEM_HOST_NO_ACCESS, sizeof(int),
                        max_active_reservations, NULL, &err);
    if (err)
    {
        clReleaseMemObject(buf_status);
        clReleaseMemObject(buf_reserve_id_t_size);
    ...
    clCreateBuffer(context, CL_MEM_HOST_NO_ACCESS,
                   reserve_id_t_size * max_active_reservations, NULL, &err);
    if (err != CL_SUCCESS)
    {
        print_error(err, " clCreateBuffer failed");
        clReleaseMemObject(buf_status);
    ...
    err |= clSetKernelArg(kernel[0], 1, sizeof(cl_mem), (void *)&pipe);
    err |=
    clSetKernelArg(kernel[0], 2, sizeof(cl_mem), (void *)&buf_reservations);
    err |= clSetKernelArg(kernel[0], 3, sizeof(cl_mem),
                          (void *)&buf_reserve_id_t_size_aligned);
    ...
    err |= clSetKernelArg(kernel[1], 1, sizeof(cl_mem), (void *)&buffers[1]);
    err |=
    clSetKernelArg(kernel[1], 2, sizeof(cl_mem), (void *)&buf_reservations);
    err |= clSetKernelArg(kernel[1], 3, sizeof(cl_mem),
                          (void *)&buf_reserve_id_t_size_aligned);
    ...
    err = clEnqueueReadBuffer(queue, buf_status, true, 0, sizeof(int), &status,
                              1, &sync_event, NULL);
    if (err != CL_SUCCESS)
    {
        print_error(err, " clEnqueueReadBuffer failed");
    ...
    // Launch Consumer kernel
    err = clEnqueueNDRangeKernel(queue, kernel[1], 1, NULL, global_work_size,
                                 NULL, 0, NULL, &sync_event);
    if (err != CL_SUCCESS)
    {
        print_error(err, " clEnqueueNDRangeKernel failed");
