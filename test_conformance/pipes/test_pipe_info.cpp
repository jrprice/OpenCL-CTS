...
int test_pipe_info(cl_device_id deviceID, cl_context context,
                   cl_command_queue queue, int num_elements)
{
    ...
    err = clGetPipeInfo(pipe, CL_PIPE_MAX_PACKETS, sizeof(pipe_depth),
                        (void *)&returnVal, NULL);
    if (err)
    {
        log_error("Error calling clGetPipeInfo(): %d\n", err);
    ...
    context, &program, &kernel, 1, (const char **)&pipe_kernel_code,
    "pipe_kernel", "-cl-std=CL2.0 -cl-kernel-arg-info");
    if (err)
    {
        clReleaseMemObject(pipe);
    ...
    clGetKernelArgInfo(kernel, 0, CL_KERNEL_ARG_TYPE_QUALIFIER,
                       sizeof(arg_type_qualifier), &arg_type_qualifier, NULL);
    if (err)
    {
        clReleaseMemObject(pipe);
