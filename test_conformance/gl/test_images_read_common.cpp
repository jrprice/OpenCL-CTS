...
"__kernel void sample_test( read_only image1d_t source, sampler_t sampler, "
"__global %s4 *results )\n"
"{\n"
...
"__kernel void sample_test( read_only image1d_buffer_t source, sampler_t "
...
"__kernel void sample_test( read_only image1d_array_t source, sampler_t "
"sampler, __global %s4 *results )\n"
"{\n"
...
"__kernel void sample_test( read_only image2d_array_t source, sampler_t "
"sampler, __global %s4 *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"\n"
"     results[ offset ] = read_image%s( source, sampler, (int4)( tidX, tidY, "
"tidZ, 0 ) );\n"
"}\n";

static const char *kernelpattern_image_read_3d =
"__kernel void sample_test( read_only image3d_t source, sampler_t sampler, "
"__global %s4 *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"__kernel void sample_test( read_only image2d_depth_t source, sampler_t "
"sampler, __global %s *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
"    results[ tidY * get_image_width( source ) + tidX ] = read_image%s( "
...
"__kernel void sample_test( read_only image2d_array_depth_t source, sampler_t "
"sampler, __global %s *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"__kernel void sample_test( read_only image2d_msaa_t source, sampler_t "
"sampler, __global %s4 *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"#pragma OPENCL EXTENSION cl_khr_gl_msaa_sharing : enable\n"
"__kernel void sample_test( read_only image2d_msaa_depth_t source, sampler_t "
"sampler, __global %s *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"#pragma OPENCL EXTENSION cl_khr_gl_msaa_sharing : enable\n"
"__kernel void sample_test( read_only image2d_array_msaa_t source, sampler_t "
"sampler, __global %s4 *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"    for(size_t sample = 0; sample< num_samples; ++sample) {\n"
"      int offset = (array_size * width * height) * sample + (width * height) "
"* tidZ + tidY * width + tidX;\n"
"         results[ offset ] = read_image%s( source, (int4)( tidX, tidY, tidZ, "
"1 ), sample );\n"
"    }\n"
"}\n";
...
"#pragma OPENCL EXTENSION cl_khr_gl_msaa_sharing : enable\n"
"__kernel void sample_test( read_only image2d_array_msaa_depth_t source, "
"sampler_t sampler, __global %s *results )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
"    int  tidZ = get_global_id(2);\n"
                       ...
                       void **outResultBuffer)
{
    ...
    * get_channel_order_channel_count(outFormat->image_channel_order) * width
    * height * depth * sampleNum;
...
static int test_image_format_read(cl_context context, cl_command_queue queue,
                                  ...
                                  GLenum target, struct format *fmt,
                                  MTdata data)
{
    int error = 0;
    ...
    switch (get_base_gl_target(target))
    {
        ...
        case GL_TEXTURE_2D_MULTISAMPLE:
            ...
            width, height, samples, target, gl_fmt, gl_internal_fmt, gl_type,
            type, &texture, &error, true, data, true);
            break;
        case GL_TEXTURE_2D_MULTISAMPLE_ARRAY:
...
int test_images_read_common(cl_device_id device, cl_context context,
                            cl_command_queue queue, struct format *formats,
                            ...
                            sizevec_t *sizes, size_t nsizes)
{
    ...
    for (fidx = 0; fidx < nformats; fidx++)
    {
        for (tidx = 0; tidx < ntargets; tidx++)
        {
            ...
            {
                log_info(
                "passed: Image read test for GL format  %s : %s : %s : %s\n\n",
                GetGLTargetName(targets[tidx]),
                GetGLFormatName(formats[fidx].internal),
                GetGLBaseFormatName(formats[fidx].formattype),
                GetGLTypeName(formats[fidx].datatype));
