        ...
        for (size_t z = 0; z < ((depth == 0) ? 1 : depth); z++)
        {
            for (size_t y = 0; y < height; y++)
            {
                for (size_t x = 0; x < width; x++)
                {
                    ...
                    if (err > 1.f
                        / 127.f) // Max expected range of error if we converted
                                 // from an 8-bit integer to a normalized float
                    {
                        if (depth == 0)
                            log_error(
                            "ERROR: Data sample %d,%d,%d did not validate!\n",
                      ...
                      GLint expected_cl_gl_mipmap_level)
{
    ...
    mem, CL_GL_TEXTURE_TARGET, sizeof(texture_target), &texture_target, NULL);
...
int is_rgb_101010_supported(cl_context context, GLenum gl_target)
{
    ...
    if ((error =
         clGetSupportedImageFormats(context, CL_MEM_READ_WRITE, image_type, 128,
