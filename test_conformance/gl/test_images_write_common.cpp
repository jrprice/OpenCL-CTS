...
"__kernel void sample_test( __global %s4 *source, write_only image1d_buffer_t "
"dest )\n"
"{\n"
"    uint index = get_global_id(0);\n"
"    %s4 value = source[index];\n"
...
"__kernel void sample_test( __global half4 *source, write_only "
"image1d_buffer_t dest )\n"
"{\n"
"    uint index = get_global_id(0);\n"
"    write_imagef( dest, index, vload_half4(index, (__global half *)source));\n"
"}\n";
...
"    int  tidY = get_global_id(1);\n"
"    uint index = tidY * get_image_width( dest ) + tidX;\n"
"    write_imagef( dest, (int2)( tidX, tidY ), vload_half4(index, (__global "
"half *)source));\n"
"}\n";

static const char *kernelpattern_image_write_1Darray =
"__kernel void sample_test( __global %s4 *source, write_only image1d_array_t "
"dest )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"__kernel void sample_test( __global half4 *source, write_only image1d_array_t "
"dest )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"    int  height = get_image_height( dest );\n"
"    int  index = tidZ * width * height + tidY * width + tidX;\n"
"    write_imagef( dest, (int4)( tidX, tidY, tidZ, 0 ), vload_half4(index, "
"(__global half *)source));\n"
"}\n";

static const char *kernelpattern_image_write_2Darray =
"__kernel void sample_test( __global %s4 *source, write_only image2d_array_t "
"dest )\n"
"{\n"
"    int  tidX   = get_global_id(0);\n"
"    int  tidY   = get_global_id(1);\n"
...
"__kernel void sample_test( __global half4 *source, write_only image2d_array_t "
"dest )\n"
"{\n"
"    int  tidX   = get_global_id(0);\n"
"    int  tidY   = get_global_id(1);\n"
...
"__kernel void sample_test( __global %s *source, write_only image2d_depth_t "
"dest )\n"
"{\n"
"    int  tidX = get_global_id(0);\n"
"    int  tidY = get_global_id(1);\n"
...
"__kernel void sample_test( __global %s *source, write_only "
"image2d_array_depth_t dest )\n"
"{\n"
"    int  tidX   = get_global_id(0);\n"
"    int  tidY   = get_global_id(1);\n"
"    int  tidZ   = get_global_id(2);\n"
...
int supportsHalf(cl_context context, bool *supports_half)
{
    ...
    error = clGetDeviceInfo(devices[0], CL_DEVICE_EXTENSIONS,
                            sizeof(char) * size, extensions, NULL);
                                   ...
                                   ExplicitType type, MTdata d)
{
    ...
    if (clFormat.image_channel_data_type != CL_UNORM_INT_101010)
    {
        ...
        get_channel_order_channel_count(clFormat.image_channel_order), glType));
    }
                             ...
                             sizevec_t *sizes, size_t nsizes)
{
    ...
    {
        log_error(
        "ERROR: clGetDeviceInfo failed for CL_DEVICE_MAX_MEM_ALLOC_SIZE.\n");
        error++;
        return error;
    }
        ...
        for (tidx = 0; tidx < ntargets; tidx++)
        {
            ...
            for (sidx = 0; sidx < nsizes; sidx++)
            {
                ...
                * get_explicit_type_size(formats[fidx].type);
                ...
                {
                    log_info(
                    "The requested allocation size (%gMB) is larger than the "
                ...
                if (test_image_format_write(
                    ...
                    sizes[sidx].depth, targets[tidx], formats[fidx].formattype,
                    formats[fidx].internal, formats[fidx].datatype,
                    formats[fidx].type, seed))
                {
                    ...
                    GetGLFormatName(formats[fidx].internal),
                    GetGLBaseFormatName(formats[fidx].formattype),
                    GetGLTypeName(formats[fidx].datatype), sizes[sidx].width,
                    sizes[sidx].height, sizes[sidx].depth);
