                              ...
                              GLenum target, format format, MTdata d)
{
    ...
    switch (get_base_gl_target(target))
    {
        ...
        case GL_TEXTURE_2D_MULTISAMPLE:
            CreateGLTexture2DMultisample(
            width, height, samples, target, format.formattype, format.internal,
            format.datatype, format.type, &glTexture, &error, false, d, false);
            break;
        case GL_TEXTURE_2D_MULTISAMPLE_ARRAY:
            CreateGLTexture2DArrayMultisample(
            width, height, arraySize, samples, target, format.formattype,
            format.internal, format.datatype, format.type, &glTexture, &error,
            false, d, false);
            break;
    ...
    if (doImageDim && doImageHeight)
        result |=
        verify(height, outKernelData.heightDim, "height from get_image_dim");
    if (doImageChannelDataType)
    ...
    for (size_t i = 0; i < nsizes; i++)
    {
        for (size_t itarget = 0; itarget < ntargets; ++itarget)
        {
            for (size_t iformat = 0; iformat < nformats; ++iformat)
                ...
                (depth_targets[itarget] == GL_TEXTURE_2D_ARRAY) ? sizes[i].depth
                                                                : 1,
