...
const char* texture2DPatterns[2][2] = {
    { "aAbBcCdDeEfFgGhHiIjJ", "AaBbCcDdEeFfGgHhIiJj" },
    { "zZyYxXwWvVuUtTsSrRqQ", "ZzYyXxWwVvUuTtSsRrQq" },
};

void SubTestTexture2D(cl_context context, cl_command_queue command_queue,
                      ...
                      const Texture2DSize* size)
{
    ...
    HarnessD3D11_TestBegin(
    "2D Texture: Format=%s, Width=%d, Height=%d, MipLevels=%d, ArraySize=%d",
    ...
    // create the D3D11 resources
    {
        ...
        TestRequire(SUCCEEDED(hr),
                    "ID3D11Device::CreateTexture2D failed (non-OpenCL D3D "
                    "error, but test is invalid).");
    }
    ...
    for (UINT i = 0; i < size->SubResourceCount; ++i)
    {
        ...
        D3D11CalcSubresource(size->subResources[i].MipLevel,
                             size->subResources[i].ArraySlice, size->MipLevels);
            ...
            for (UINT y = 0; y < 2; ++y)
            {
                ...
                ID3D11Texture2D* pStagingBuffer = NULL;
                {
                    ...
                    TestRequire(SUCCEEDED(hr),
                                ...
                                "(non-OpenCL D3D error, but test is invalid).");
                }

                // write the data to the staging buffer
                {
                    D3D11_MAPPED_SUBRESOURCE mappedTexture;
                    hr = pDC->Map(pStagingBuffer, 0, D3D11_MAP_READ_WRITE, 0,
                                  &mappedTexture);
                    memcpy(mappedTexture.pData, texture2DPatterns[x][y],
                ...
                // copy the data to to the texture
                {
                    ...
                    pTexture, subResourceInfo[i].subResource,
                    x ? subResourceInfo[i].width - 1 : 0,
                    y ? subResourceInfo[i].height - 1 : 0, 0, pStagingBuffer, 0,
                    &box);
                }
    ...
    for (UINT i = 0; i < size->SubResourceCount; ++i)
    {
        ...
        context, 0, pTexture, subResourceInfo[i].subResource, &result);
        ...
        result =
        clGetMemObjectInfo(subResourceInfo[i].mem, CL_MEM_D3D11_RESOURCE_KHR,
                           sizeof(clResource), &clResource, NULL);
        TestRequire(result == CL_SUCCESS,
                    "clGetMemObjectInfo for CL_MEM_D3D11_RESOURCE_KHR failed.");
        TestRequire(clResource == pTexture,
                    "clGetMemObjectInfo for CL_MEM_D3D11_RESOURCE_KHR returned "
                    "incorrect value.");

        // query subresource and verify
        UINT clSubResource;
        result =
        clGetImageInfo(subResourceInfo[i].mem, CL_IMAGE_D3D11_SUBRESOURCE_KHR,
                       sizeof(clSubResource), &clSubResource, NULL);
        TestRequire(result == CL_SUCCESS,
                    "clGetImageInfo for CL_IMAGE_D3D11_SUBRESOURCE_KHR failed");
        TestRequire(clSubResource == subResourceInfo[i].subResource,
                    "clGetImageInfo for CL_IMAGE_D3D11_SUBRESOURCE_KHR "
                    "returned incorrect value.");

        // query format and verify
        result = clGetImageInfo(subResourceInfo[i].mem, CL_IMAGE_FORMAT,
                                sizeof(clFormat), &clFormat, NULL);
        TestRequire(result == CL_SUCCESS,
                    "clGetImageInfo for CL_IMAGE_FORMAT failed");
        TestRequire(
        clFormat.image_channel_order == format->channel_order,
        "clGetImageInfo for CL_IMAGE_FORMAT returned incorrect channel order.");
        TestRequire(clFormat.image_channel_data_type == format->channel_type,
                    "clGetImageInfo for CL_IMAGE_FORMAT returned incorrect "
                    "channel data type.");

        // query width
        size_t width;
        result = clGetImageInfo(subResourceInfo[i].mem, CL_IMAGE_WIDTH,
                                sizeof(width), &width, NULL);
        TestRequire(result == CL_SUCCESS,
                    "clGetImageInfo for CL_IMAGE_WIDTH failed");
        TestRequire(
        width == subResourceInfo[i].width,
        "clGetImageInfo for CL_IMAGE_HEIGHT returned incorrect value.");

        // query height
        size_t height;
        result = clGetImageInfo(subResourceInfo[i].mem, CL_IMAGE_HEIGHT,
                                sizeof(height), &height, NULL);
        TestRequire(result == CL_SUCCESS,
                    "clGetImageInfo for CL_IMAGE_HEIGHT failed");
        TestRequire(
        height == subResourceInfo[i].height,
        "clGetImageInfo for CL_IMAGE_HEIGHT returned incorrect value.");
    }
    ...
    for (UINT i = 0; i < 2; ++i)
    {
        ...
        // cut the registered sub-resources into two sets and send the acquire
        // calls for them separately
        if (i == 0)
        {
        ...
        {
            for (UINT j = size->SubResourceCount / 2;
                 j < size->SubResourceCount; ++j)
            {
                memToAcquire[memCount++] = subResourceInfo[j].mem;
            }
        }
        ...
        command_queue, memCount, memToAcquire, 0, NULL, &events[0 + i]);
        TestRequire(result == CL_SUCCESS,
                    "clEnqueueAcquireD3D11ObjectsKHR failed.");
        TestRequire(events[0 + i],
                    "clEnqueueAcquireD3D11ObjectsKHR did not return an event.");

        // make sure the event type is correct
        cl_uint eventType = 0;
        result = clGetEventInfo(events[0 + i], CL_EVENT_COMMAND_TYPE,
                                sizeof(eventType), &eventType, NULL);
        TestRequire(result == CL_SUCCESS,
                    "clGetEventInfo for event created by "
                    "clEnqueueAcquireD3D11ObjectsKHR failed.");
        TestRequire(eventType == CL_COMMAND_ACQUIRE_D3D11_OBJECTS_KHR,
                    "clGetEventInfo for CL_EVENT_COMMAND_TYPE was not "
    ...
    for (UINT i = 0; i < size->SubResourceCount; ++i)
    {
        ...
        size_t region[3] = { subResourceInfo[i].width,
                             subResourceInfo[i].height, 1 };
        cl_mem tempImage;
        ...
        tempImage =
        clCreateImage(context, 0, &clFormat, &image_desc, NULL, &result);
        TestRequire(result == CL_SUCCESS, "clCreateImage failed");

        result =
        clEnqueueCopyImage(command_queue, subResourceInfo[i].mem, tempImage,
        ...
        {
            result =
            clSetKernelArg(kernel, 0, sizeof(cl_mem), (void*)&tempImage);
            result = clSetKernelArg(kernel, 1, sizeof(cl_mem),
        ...
        for (UINT x = 0; x < 2; ++x)
            for (UINT y = 0; y < 2; ++y)
            {
                ...
                command_queue, subResourceInfo[i].mem, subResourceInfo[i].mem,
                src, dst, region, 0, NULL, NULL);
                TestRequire(result == CL_SUCCESS, "clEnqueueCopyImage failed.");
            }
    ...
    for (UINT i = 0; i < 2; ++i)
    {
        ...
        // cut the registered sub-resources into two sets and send the release
        // calls for them separately
        if (i == 0)
        {
            for (UINT j = size->SubResourceCount / 4;
                 j < size->SubResourceCount; ++j)
            {
                memToAcquire[memCount++] = subResourceInfo[j].mem;
            }
        ...
        command_queue, memCount, memToAcquire, 0, NULL, &events[2 + i]);
        TestRequire(result == CL_SUCCESS,
                    "clEnqueueReleaseD3D11ObjectsKHR failed.");
        TestRequire(events[2 + i],
                    "clEnqueueReleaseD3D11ObjectsKHR did not return an event.");

        // make sure the event type is correct
        cl_uint eventType = 0;
        result = clGetEventInfo(events[2 + i], CL_EVENT_COMMAND_TYPE,
                                sizeof(eventType), &eventType, NULL);
        TestRequire(result == CL_SUCCESS,
                    "clGetEventInfo for event created by "
                    "clEnqueueReleaseD3D11ObjectsKHR failed.");
        TestRequire(eventType == CL_COMMAND_RELEASE_D3D11_OBJECTS_KHR,
                    "clGetEventInfo for CL_EVENT_COMMAND_TYPE was not "
    ...
    for (UINT i = 0; i < size->SubResourceCount; ++i)
        for (UINT x = 0; x < 2; ++x)
            for (UINT y = 0; y < 2; ++y)
            {
                ...
                // wipe out the staging buffer to make sure we don't get stale
                // values
                {
                    D3D11_MAPPED_SUBRESOURCE mappedTexture;
                    hr = pDC->Map(pStagingBuffer, 0, D3D11_MAP_READ_WRITE, 0,
                ...
                {
                    D3D11_BOX box = { 0 };
                    box.left = x ? subResourceInfo[i].width - 2 : 1;
                    box.right = box.left + 1;
                    box.top = y ? subResourceInfo[i].height - 2 : 1;
                    box.bottom = box.top + 1;
                    box.front = 0;
                    box.back = 1;
                    pDC->CopySubresourceRegion(
                    pStagingBuffer, 0, 0, 0, 0, pTexture,
                    subResourceInfo[i].subResource, &box);
                }
                ...
                // make sure we read back what was written next door
                {
                    ...
                    hr = pDC->Map(pStagingBuffer, 0, D3D11_MAP_READ_WRITE, 0,
                                  &mappedTexture);
                    TestRequire(SUCCEEDED(hr), "Failed to map staging buffer");
                    TestRequire(!memcmp(mappedTexture.pData,
                                        texture2DPatterns[x][y],
                                        format->bytesPerPixel),
...
Cleanup:
    ...
    for (UINT i = 0; i < 4; ++i)
    {
        if (events[i])
        {
            ...
            TestRequire(result == CL_SUCCESS,
                        "clReleaseEvent for event failed.");
        }
    }
...
{
    for (std::vector<cl_image_format>::const_iterator it =
         supported_image_formats.begin();
         it != supported_image_formats.end(); ++it)
        if (it->image_channel_data_type == channel_type
            && it->image_channel_order == channel_order)
            return true;

    return false;
}

void TestDeviceTexture2D(cl_device_id device, cl_context context,
                         ...
                         ID3D11DeviceContext* pDC)
{
    ...
    result =
    clGetSupportedImageFormats(context, CL_MEM_READ_ONLY, CL_MEM_OBJECT_IMAGE2D,
                               0, NULL, &supported_formats_count);
    TestRequire(CL_SUCCESS == result, "clGetSupportedImageFormats failed.");

    supported_image_formats.resize(supported_formats_count);
    result = clGetSupportedImageFormats(
    context, CL_MEM_READ_ONLY, CL_MEM_OBJECT_IMAGE2D, supported_formats_count,
    &supported_image_formats[0], NULL);
    TestRequire(CL_SUCCESS == result, "clGetSupportedImageFormats failed.");

    char source[2048];
    sprintf(source, sourceRaw, "float4", "f", "f", "f", "f");
    result = HarnessD3D11_CreateKernelFromSource(&kernels[0], device, context,
                                                 source, "texture2D");
    TestRequire(CL_SUCCESS == result,
                "HarnessD3D11_CreateKernelFromSource failed.");

    sprintf(source, sourceRaw, "uint4", "ui", "ui", "ui", "ui");
    result = HarnessD3D11_CreateKernelFromSource(&kernels[1], device, context,
                                                 source, "texture2D");
    TestRequire(CL_SUCCESS == result,
                "HarnessD3D11_CreateKernelFromSource failed.");

    sprintf(source, sourceRaw, "int4", "i", "i", "i", "i");
    result = HarnessD3D11_CreateKernelFromSource(&kernels[2], device, context,
                                                 source, "texture2D");
    TestRequire(CL_SUCCESS == result,
                "HarnessD3D11_CreateKernelFromSource failed.");
    ...
    {
        if (!is_format_supported(formats[format].channel_order,
                                 formats[format].channel_type,
                                 supported_image_formats))
        {
            HarnessD3D11_TestBegin(
            "2D_texture: Format=%s, Width=%d, Height=%d, MipLevels=%d, "
            "ArraySize=%d\n",
        ...
        SubTestTexture2D(
        context, command_queue, kernels[formats[format].generic], pDevice, pDC,
        &formats[format], &texture2DSizes[size % texture2DSizeCount]);
    }
