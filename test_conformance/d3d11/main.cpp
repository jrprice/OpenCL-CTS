...
int main(int argc, const char* argv[])
{
    ...
    // NonTestRequire(num_devices_tested, "No D3D11 compatible cl_device_ids
    // were found.");
                            ...
                            ID3D11Device* pDevice, cl_uint* num_devices)
{
    ...
    // get the cl_device_ids for the adapter
    {
        ...
        else
        {
            ...
            result = clGetDeviceIDsFromD3D11KHR(
            platform, CL_D3D11_DXGI_ADAPTER_KHR, pAdapter,
            CL_ALL_DEVICES_FOR_D3D11_KHR, num_adapter_devices, adapter_devices,
            NULL);
    ...
    {
        result = clGetDeviceIDsFromD3D11KHR(
        platform, CL_D3D11_DEVICE_KHR, pDevice, CL_ALL_DEVICES_FOR_D3D11_KHR, 0,
        ...
        else
        {
            ...
            CL_ALL_DEVICES_FOR_D3D11_KHR, num_device_devices, device_devices,
            NULL);
            TestRequire((result == CL_SUCCESS),
                        "clGetDeviceIDsFromD3D11KHR failed.");
        }
                        ...
                        cl_uint num_devices_expected)
{
    ...
    result = clGetDeviceIDsFromD3D11KHR(
    platform, CL_D3D11_DEVICE_KHR, pDevice, CL_ALL_DEVICES_FOR_D3D11_KHR,
...
void TestDevice(cl_device_id device, ID3D11Device* pDevice,
                ID3D11DeviceContext* pDC)
{
    ...
    context, CL_CONTEXT_D3D11_PREFER_SHARED_RESOURCES_KHR,
    ...
    // run buffer tests
    TestDeviceBuffer(context, command_queue, pDevice, pDC);

    // run 2D texture tests
    TestDeviceTexture2D(device, context, command_queue, pDevice, pDC);

    // run 3D texture tests
    TestDeviceTexture3D(device, context, command_queue, pDevice, pDC);
                             ...
                             cl_command_queue* out_command_queue)
{
    ...
    // check CL_CONTEXT_D3D11_DEVICE_KHR
    {
        ...
        result =
        clGetContextInfo(context, CL_CONTEXT_PROPERTIES, param_value_size_ret,
                         &contextProperties[0], NULL);
        TestRequire((result == CL_SUCCESS),
                    "clGetContextInfo with CL_CONTEXT_PROPERTIES failed");
        ...
        TestRequire(
        (iter != contextProperties.end()),
        "CL_CONTEXT_PROPERTIES doesn't include CL_CONTEXT_D3D11_DEVICE_KHR");
    }
