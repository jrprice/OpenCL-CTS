...
const char *work_offset_test[] = {
    ...
    "                        __global int * outputID_B, __global int * "
    "outputID_C )\n"
    "{\n"
    "    size_t id0 = get_local_id( 0 ) + get_group_id( 0 ) * get_local_size( "
    "0 );\n"
    "    size_t id1 = get_local_id( 1 ) + get_group_id( 1 ) * get_local_size( "
    "1 );\n"
    "    size_t id2 = get_local_id( 2 ) + get_group_id( 2 ) * get_local_size( "
    "2 );\n"
...
int check_results(size_t threads[], size_t offsets[], cl_int outputA[],
                  cl_int outputB[], cl_int outputC[])
{
    size_t offsettedSizes[3] = { threads[0] + offsets[0],
                                 threads[1] + offsets[1],
                                 threads[2] + offsets[2] };
    ...
    for (size_t x = 0; x < offsettedSizes[0]; x++)
    {
        for (size_t y = 0; y < offsettedSizes[1]; y++)
        {
            ...
            {
                const char *limitMsg =
                " (further errors of this type suppressed)";
                if ((x >= offsets[0]) && (y >= offsets[1]) && (z >= offsets[2]))
                {
                    if (counts[x][y][z] < 1)
                    {
                        if (missed < 3)
                            log_error(
                            "ERROR: Map value (%ld,%ld,%ld) was missed%s\n", x,
                            y, z, (missed == 2) ? limitMsg : "");
                        missed++;
                    }
                    else if (counts[x][y][z] > 1)
                    {
                        if (multiple < 3)
                            log_error("ERROR: Map value (%ld,%ld,%ld) was "
    ...
    {
        size_t diffs[3] = {
            (offsets[0] > threads[0] ? 0 : threads[0] - offsets[0]),
            (offsets[1] > threads[1] ? 0 : threads[1] - offsets[1]),
            (offsets[2] > threads[2] ? 0 : threads[2] - offsets[2])
        };
        int diff = (int)((threads[0] - diffs[0]) * (threads[1] - diffs[1])
                         * (threads[2] - diffs[2]));

        if ((multiple == 0) && (missed == diff) && (errored == diff))
            log_error("ERROR: Global work offset values are not being "
                      "respected by get_global_id()\n");
        else
            log_error(
            "ERROR: Global work offset values did not function as expected (%d "
            "missed, %d reported multiple times, %d erroneously hit)\n",
            missed, multiple, errored);
    }
    return (missed | multiple | errored | corrected);
}

int test_global_work_offsets(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    ...
    int error;
    size_t threads[] = { 1, 1, 1 }, localThreads[] = { 1, 1, 1 },
           offsets[] = { 0, 0, 0 };
    cl_int outputA[MAX_TEST_ITEMS], outputB[MAX_TEST_ITEMS],
    outputC[MAX_TEST_ITEMS];


    // Create the kernel
    if (create_single_kernel_helper(context, &program, &kernel, 1,
                                    work_offset_test, "test")
        != 0)
    {
        return -1;
    }
    ...
    // Use just one output array to init them all (no need to init every single
    // stack storage here)
    memset(outputA, 0xff, sizeof(outputA));
    for (int i = 0; i < 3; i++)
    {
        streams[i] = clCreateBuffer(
    ...
    {
        // Choose a random combination of thread size, but in total less than
        // MAX_TEST_ITEMS
        threads[0] = random_in_range(1, 32, seed);
        threads[1] = random_in_range(1, 16, seed);
        threads[2] = random_in_range(
        1, MAX_TEST_ITEMS / (int)(threads[0] * threads[1]), seed);

        // Make sure we get the local thread count right
        error = get_max_common_3D_work_group_size(context, kernel, threads,
                                                  localThreads);
        ...
        log_info(
        "\tTesting %ld,%ld,%ld (%ld,%ld,%ld) with offsets (%ld,%ld,%ld)...\n",
        threads[0], threads[1], threads[2], localThreads[0], localThreads[1],
        localThreads[2], offsets[0], offsets[1], offsets[2]);
        ...
        error = clEnqueueNDRangeKernel(queue, kernel, 3, offsets, threads,
        ...
        {
            error =
            clEnqueueReadBuffer(queue, streams[i], CL_TRUE, 0, sizeof(outputA),
                                resultBuffers[i], 0, NULL, NULL);
            test_error(error, "Unable to get result data");
        }

        // Now we need to check the results. The outputs should have one entry
        // for each possible ID, but they won't be in order, so we need to
        // construct a count map to determine what we got
        if (check_results(threads, offsets, outputA, outputB, outputC))
        {
            log_error("\t(Test failed for global dim %ld,%ld,%ld, local dim "
                      "%ld,%ld,%ld, offsets %ld,%ld,%ld)\n",
...
const char *get_offset_test[] = {
    ...
    "   // Also note that these should be the same for ALL threads, so we "
    "won't worry about contention\n"
...
int test_get_global_offset(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    clProgramWrapper program;
    clKernelWrapper kernel;
    ...
    for (int test = 0; test < NUM_TESTS; test++)
    {
        ...
        // Read our results back now
        error =
        clEnqueueReadBuffer(queue, streams[0], CL_TRUE, 0, sizeof(outOffsets),
                            outOffsets, 0, NULL, NULL);
        test_error(error, "Unable to get result data");
        ...
        for (int j = 0; j < 3; j++)
        {
            if (outOffsets[j] != (cl_int)offsets[j])
            {
                log_error("ERROR: get_global_offset( %d ) did not return "
                          "expected value (expected %ld, got %d)\n",
