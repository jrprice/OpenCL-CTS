...
"\n"
"__kernel void test_bgra8888_write(__global unsigned char *src, write_only "
"image2d_t dstimg)\n"
"{\n"
...
"\n"
"__kernel void test_rgba8888_write(__global unsigned char *src, write_only "
"image2d_t dstimg)\n"
"{\n"
"    int            tid_x = get_global_id(0);\n"
"    int            tid_y = get_global_id(1);\n"
...
static unsigned char *generate_8888_image(int w, int h, MTdata d)
{
    ...
    for (i = 0; i < w * h * 4; i++) ptr[i] = (cl_uchar)genrand_int32(d);

    return ptr;
}

static int verify_bgra8888_image(unsigned char *image, unsigned char *outptr,
                                 int w, int h)
{
    int i;
...
static int verify_rgba8888_image(unsigned char *image, unsigned char *outptr,
                                 int w, int h)
{
    int i;
                    ...
                    cl_command_queue queue, int num_elements)
{
    ...
    if (gIsEmbedded)
    {
        ...
        clGetSupportedImageFormats(context, CL_MEM_READ_WRITE,
                                   CL_MEM_OBJECT_IMAGE2D, 0, NULL, &numFormats);
        supported_formats =
        (cl_image_format *)malloc(sizeof(cl_image_format) * numFormats);
        clGetSupportedImageFormats(context, CL_MEM_READ_WRITE,
    ...
    if (supportsBGRA)
    {
        ...
        streams[0] = clCreateImage2D(context, CL_MEM_READ_WRITE, &img_format,
    ...
    if (supportsBGRA)
    {
        img_format.image_channel_order = CL_BGRA;
        img_format.image_channel_data_type = CL_UNORM_INT8;
        streams[2] = clCreateImage2D(context, CL_MEM_WRITE_ONLY, &img_format,
                                     img_width, img_height, 0, NULL, NULL);
        if (!streams[2])
        {
            log_error("clCreateImage2D failed\n");
    ...
    img_format.image_channel_order = CL_RGBA;
    img_format.image_channel_data_type = CL_UNORM_INT8;
    streams[3] = create_image_2d(context, CL_MEM_WRITE_ONLY, &img_format,
                                 img_width, img_height, 0, NULL, NULL);
    if (!streams[3])
    {
        log_error("create_image_2d failed\n");
    ...
    err = clEnqueueWriteBuffer(queue, streams[4], CL_TRUE, 0, length,
    ...
    err = clEnqueueWriteBuffer(queue, streams[5], CL_TRUE, 0, length,
    ...
    for (i = 0; i < 4; i++)
    {
        if (!supportsBGRA && (i == 0 || i == 2)) continue;

        err = clEnqueueNDRangeKernel(queue, kernel[i], 2, NULL, threads, NULL,
                                     0, NULL, NULL);
        if (err != CL_SUCCESS)
        {
        ...
        err = clEnqueueReadImage(queue, streams[i], CL_TRUE, origin, region, 0,
                                 0, output_ptr, 0, NULL, NULL);
        if (err != CL_SUCCESS)
        {
        ...
        switch (i)
        {
            case 0:
            case 2:
                err = verify_bgra8888_image(input_ptr[i & 0x01], output_ptr,
                                            img_width, img_height);
                break;
            case 1:
            case 3:
                err = verify_rgba8888_image(input_ptr[i & 0x01], output_ptr,
                                            img_width, img_height);
                break;
        }
    ...
    // cleanup
    if (supportsBGRA) clReleaseMemObject(streams[0]);

    clReleaseMemObject(streams[1]);

    if (supportsBGRA) clReleaseMemObject(streams[2]);
