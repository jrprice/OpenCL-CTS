...
"%s\n" // optional pragma
"kernel void test(global ulong *results, %s %s *mem0, %s %s2 *mem2, %s %s3 "
"*mem3, %s %s4 *mem4, %s %s8 *mem8, %s %s16 *mem16)\n"
"{\n"
...
const char *local_kernel_long = "%s\n" // optional pragma
...
// For constant
const char *constant_kernel_long = "%s\n" // optional pragma
                                   "  constant %s mem0[3]    = {0};\n"
...
"%s\n" // optional pragma
"kernel void test(global uint *results, %s %s *mem0, %s %s2 *mem2, %s %s3 "
"*mem3, %s %s4 *mem4, %s %s8 *mem8, %s %s16 *mem16)\n"
"{\n"
...
// For private and local
const char *local_kernel_no_long = "%s\n" // optional pragma
...
const char *get_explicit_address_name(AddressSpaces address)
{
     ...
     * the Enum order */
    static const char *sExplicitAddressNames[] = { "global", "local",
                                                   "constant", "private" };
...
int test_kernel_memory_alignment(cl_device_id device, cl_context context,
                                 ...
                                 AddressSpaces address)
{
    ...
    clGetDeviceInfo(device, CL_DEVICE_MAX_CONSTANT_ARGS,
                    sizeof(numConstantArgs), &numConstantArgs, NULL);
    ...
    for (typeIndex = 0; typeIndex < 10; typeIndex++)
    {
        ...
        // E.g., if it is supposed to be 4 byte aligned, we should get 4-1=3 =
        // ... 000011 We can then and the returned address with that and we
        // should have 0.
        ...
        if (address == kGlobal || address == kLocal || address == kConstant)
        {
            ...
            if ((gIsEmbedded) && (address == kConstant)
                && (numConstantArgs < 6))
            {
                sprintf(kernel_code, parameter_kernel,
                        vecType[typeIndex] == kDouble
                        ? "#pragma OPENCL EXTENSION cl_khr_fp64 : enable"
                        : "",
                        get_explicit_address_name(address),
                        get_explicit_type_name(vecType[typeIndex]),
                        get_explicit_address_name(address),
                        get_explicit_type_name(vecType[typeIndex]),
                        get_explicit_address_name(address),
            ...
            context, &program, &kernel, 1, (const char **)&kernel_code, "test");
            test_error(error, "create_single_kernel_helper failed");

            // Initialize the results
            memset(results_data, 0, sizeof(cl_long) * 5);
            error = clEnqueueWriteBuffer(queue, results, CL_TRUE, 0,
                                         sizeof(cl_long) * 6, results_data, 0,
                                         NULL, NULL);
            ...
            else
            {
                ...
                kernel, 1, get_explicit_type_size(vecType[typeIndex]), NULL);
                test_error(error, "clSetKernelArg failed");
                error = clSetKernelArg(
                kernel, 2, get_explicit_type_size(vecType[typeIndex]) * 2,
                NULL);
                test_error(error, "clSetKernelArg failed");
                error = clSetKernelArg(
                kernel, 3, get_explicit_type_size(vecType[typeIndex]) * 4,
                NULL);
                test_error(error, "clSetKernelArg failed");
                error = clSetKernelArg(
                kernel, 4, get_explicit_type_size(vecType[typeIndex]) * 4,
                NULL);
                test_error(error, "clSetKernelArg failed");
                error = clSetKernelArg(
                kernel, 5, get_explicit_type_size(vecType[typeIndex]) * 8,
                NULL);
                test_error(error, "clSetKernelArg failed");
                error = clSetKernelArg(
                kernel, 6, get_explicit_type_size(vecType[typeIndex]) * 16,
                NULL);
                test_error(error, "clSetKernelArg failed");
            }
            ...
            error = clEnqueueReadBuffer(queue, results, CL_TRUE, 0,
                                        sizeof(cl_ulong) * 6, results_data, 0,
                                        NULL, NULL);
            ...
            if (gHasLong)
            {
                for (int i = 0; i < 6; i++)
                {
                    ...
                    {
                        total_errors++;
                        log_error("\tVector size %d failed: 0x%llx is not "
                                  "properly aligned.\n",
                                  1 << i, results_data[i]);
                    }
                    else
                    {
                        if (DEBUG)
                            log_info("\tVector size %d passed: 0x%llx is "
                                     "properly aligned.\n",
                                     1 << i, results_data[i]);
                    }
                }
        ...
        if (address == kLocal || address == kPrivate)
        {
            ...
            memset(results_data, 0, sizeof(cl_long) * 5);
            error = clEnqueueWriteBuffer(queue, results, CL_TRUE, 0,
                                         sizeof(cl_long) * 5, results_data, 0,
                                         NULL, NULL);
            test_error(error, "clEnqueueWriteBuffer failed");
            ...
            // Read back the results
            error = clEnqueueReadBuffer(queue, results, CL_TRUE, 0,
                                        sizeof(cl_ulong) * 5, results_data, 0,
                                        NULL, NULL);
            test_error(error, "clEnqueueReadBuffer failed");
...
int test_kernel_memory_alignment_local(cl_device_id device, cl_context context,
                                       cl_command_queue queue, int n_elems)
{
...
int test_kernel_memory_alignment_global(cl_device_id device, cl_context context,
                                        cl_command_queue queue, int n_elems)
{
    return test_kernel_memory_alignment(device, context, queue, n_elems,
                                        kGlobal);
...
int test_kernel_memory_alignment_constant(cl_device_id device,
                                          cl_context context,
                                          cl_command_queue queue, int n_elems)
{
    // There is a class of approved OpenCL 1.0 conformant devices out there that
    // in some circumstances are unable to meaningfully take (or more precisely
    // use) the address of constant data by virtue of limitations in their ISA
    // design. This feature was not tested in 1.0, so they were declared
    // conformant by Khronos. The failure is however caught here.
    //
    // Unfortunately, determining whether or not these devices are 1.0
    // conformant is not the jurisdiction of the 1.1 tests -- We can't fail them
    // from 1.1 conformance here because they are not 1.1 devices. They are
    // merely 1.0 conformant devices that interop with 1.1 devices in a 1.1
    // platform. To add new binding tests now to conformant 1.0 devices would
    // violate the workingroup requirement of no new tests for 1.0 devices.  So
    // certain allowances have to be made in intractable cases such as this one.
    //
    // There is some precedent. Similar allowances are made for other 1.0
    // hardware features such as local memory size.  The minimum required local
    // memory size grew from 16 kB to 32 kB in OpenCL 1.1.
    ...
    int err;
    if ((err =
         clGetDeviceInfo(device, CL_DEVICE_VERSION, 0, NULL, &string_size)))
    {
    ...
    {
        log_error(
        "FAILURE: Unable to allocate memory to hold CL_DEVICE_VERSION string!");
        return -1;
    }

    // Get CL_DEVICE_VERSION string
    if ((err = clGetDeviceInfo(device, CL_DEVICE_VERSION, string_size,
                               version_string, NULL)))
    {
    ...
    {
        log_info("WARNING: Allowing device to escape testing of difficult "
                 "constant memory alignment case.\n\tDevice is not a OpenCL "
                 "1.1 device. CL_DEVICE_VERSION: \"%s\"\n",
                 version_string);
        free(version_string);
        return 0;
    }
...
int test_kernel_memory_alignment_private(cl_device_id device,
                                         cl_context context,
                                         cl_command_queue queue, int n_elems)
{
    return test_kernel_memory_alignment(device, context, queue, n_elems,
