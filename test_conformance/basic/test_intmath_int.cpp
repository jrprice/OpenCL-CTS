...
"__kernel void test_int_add(__global int *srcA, __global int *srcB, __global "
"int *dst)\n"
"{\n"
...
"__kernel void test_int_sub(__global int *srcA, __global int *srcB, __global "
"int *dst)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
...
"__kernel void test_int_mul(__global int *srcA, __global int *srcB, __global "
"int *dst)\n"
"{\n"
"    int  tid = get_global_id(0);\n"
"\n"
...
"__kernel void test_int_mad(__global int *srcA, __global int *srcB, __global "
...
int verify_int_add(int *inptrA, int *inptrB, int *outptr, int n)
{
    int r;
    int i;
...
int verify_int_sub(int *inptrA, int *inptrB, int *outptr, int n)
{
    int r;
    int i;
...
int verify_int_mul(int *inptrA, int *inptrB, int *outptr, int n)
{
    int r;
    int i;
...
int verify_int_mad(int *inptrA, int *inptrB, int *inptrC, int *outptr, int n)
{
    int r;
    int i;
                     ...
                     cl_command_queue queue, int num_elements)
{
    ...
    p = input_ptr[0];
    for (i = 0; i < num_elements; i++) p[i] = (int)genrand_int32(d);
    p = input_ptr[1];
    for (i = 0; i < num_elements; i++) p[i] = (int)genrand_int32(d);
    p = input_ptr[2];
    for (i = 0; i < num_elements; i++) p[i] = (int)genrand_int32(d);
    ...
    err = create_single_kernel_helper(context, &program[0], &kernel[0], 1,
                                      &int_add_kernel_code, "test_int_add");
    if (err != CL_SUCCESS)
    {
    ...
    err = create_single_kernel_helper(context, &program[1], &kernel[1], 1,
                                      &int_sub_kernel_code, "test_int_sub");
    if (err != CL_SUCCESS)
    {
        log_error("create_single_kernel_helper failed\n");
        return -1;
    ...
    err = create_single_kernel_helper(context, &program[2], &kernel[2], 1,
                                      &int_mul_kernel_code, "test_int_mul");
    if (err != CL_SUCCESS)
    {
        log_error("create_single_kernel_helper failed\n");
        return -1;
    ...
    err = create_single_kernel_helper(context, &program[3], &kernel[3], 1,
                                      &int_mad_kernel_code, "test_int_mad");
    if (err != CL_SUCCESS)
    {
        log_error("create_single_kernel_helper failed\n");
        return -1;
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        {
            case 0:
                err = verify_int_add(input_ptr[0], input_ptr[1], output_ptr,
                                     num_elements);
                break;
            case 1:
                err = verify_int_sub(input_ptr[0], input_ptr[1], output_ptr,
                                     num_elements);
                break;
            case 2:
                err = verify_int_mul(input_ptr[0], input_ptr[1], output_ptr,
                                     num_elements);
                break;
            case 3:
                err = verify_int_mad(input_ptr[0], input_ptr[1], input_ptr[2],
