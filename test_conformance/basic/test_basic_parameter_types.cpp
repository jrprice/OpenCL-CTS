                              ...
                              cl_command_queue queue, int num_elements)
{
    ...
    for (size_to_test = 0; size_to_test < 5; size_to_test++)
    {
        ...
        context, &program, &kernel, 1, (const char **)&ptr, "test_kernel_long");
        ...
        for (count = 0; count < 2; count++)
        {
            switch (count)
            {
                case 0:
                    ...
                    kernel, count, sizeof(cl_long) * sizes[size_to_test], &l);
                    break;
                case 1:
                    error = clSetKernelArg(
                    kernel, count, sizeof(cl_ulong) * sizes[size_to_test], &ul);
                    break;
                         ...
                         cl_command_queue queue, int num_elements)
{
    ...
    + sizeof(cl_short) + sizeof(cl_ushort) + sizeof(cl_int) + sizeof(cl_uint)
    + sizeof(cl_float);
    ...
    for (size_to_test = 0; size_to_test < 5; size_to_test++)
    {
        ...
        for (count = 0; count < 7; count++)
        {
            switch (count)
            {
                case 0:
                    error = clSetKernelArg(
                    kernel, count, sizeof(cl_char) * sizes[size_to_test], &c);
                    break;
                case 1:
                    error = clSetKernelArg(
                    kernel, count, sizeof(cl_uchar) * sizes[size_to_test], &uc);
                    break;
                case 2:
                    error = clSetKernelArg(
                    kernel, count, sizeof(cl_short) * sizes[size_to_test], &s);
                    break;
                case 3:
                    error = clSetKernelArg(
                ...
                case 4:
                    error = clSetKernelArg(
                    kernel, count, sizeof(cl_int) * sizes[size_to_test], &i);
                    break;
                case 5:
                    error = clSetKernelArg(
                    kernel, count, sizeof(cl_uint) * sizes[size_to_test], &ui);
                    break;
                case 6:
                    error = clSetKernelArg(
                    kernel, count, sizeof(cl_float) * sizes[size_to_test], &f);
                    break;
                default: log_error("Test error"); break;
            }
            if (error)
