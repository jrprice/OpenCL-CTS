                        ...
                        cl_command_queue queue, int num_elements)
{
    ...
    for (i = 0; i < num_tries; i++)
    {
        ...
        if (cb > (num_elements - offset)) cb = num_elements - offset;

        err = clEnqueueWriteBuffer(
        queue, streams[0], CL_TRUE, offset * sizeof(cl_uint),
        sizeof(cl_uint) * cb, &inptr[offset], 0, NULL, NULL);
        test_error(err, "clEnqueueWriteBuffer failed");

        err = clEnqueueReadBuffer(
        queue, streams[0], CL_TRUE, offset * sizeof(cl_uint),
        cb * sizeof(cl_uint), &outptr[offset], 0, NULL, NULL);
    ...
    if (!err) log_info("ARRAY read, write test passed\n");
