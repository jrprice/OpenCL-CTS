...
"__kernel void test_r_uint8(read_only image2d_t srcimg, __global unsigned char "
...
static unsigned char *generate_8bit_image(int w, int h, MTdata d)
{
    unsigned char *ptr = (unsigned char *)malloc(w * h * sizeof(unsigned char));
    int i;

    for (i = 0; i < w * h; i++) ptr[i] = (unsigned char)genrand_int32(d);

    return ptr;
}

static int verify_8bit_image(unsigned char *image, unsigned char *outptr, int w,
                             int h)
{
    int i;
                  ...
                  cl_command_queue queue, int num_elements)
{
    ...
    if (!is_image_format_supported(context, (cl_mem_flags)(CL_MEM_READ_ONLY),
    ...
    streams[0] =
    create_image_2d(context, (cl_mem_flags)(CL_MEM_READ_ONLY), &img_format,
    ...
    err = clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0,
                              sizeof(cl_uchar) * img_width * img_height,
