...
int test_simple_read_image_pitch(cl_device_id device, cl_context cl_context_,
                                 cl_command_queue q, int num_elements)
{
    ...
    cl_mem image =
    clCreateImage(cl_context_, CL_MEM_COPY_HOST_PTR | CL_MEM_READ_WRITE, &fmt,
                  &desc, host_image, &err);
    ...
    err = clEnqueueReadImage(q, image, CL_TRUE, origin, region, bufferW, 0,
    ...
    for (size_t j = 0; j < bufferH; ++j)
    {
        for (size_t i = 0; i < bufferW; ++i)
        {
            ...
            {
                log_error(
                "Bad value %x outside image at (byte: %lu, row: %lu)\n", val, i,
                j);
                ++errors;
            }
        }
...
int test_simple_write_image_pitch(cl_device_id device, cl_context cl_context_,
                                  cl_command_queue q, int num_elements)
{
    cl_int err = CL_SUCCESS;
    ...
    err = clEnqueueWriteImage(q, image, CL_TRUE, origin, region, bufferW, 0,
                              host_buffer, 0, NULL, NULL);
    test_error(err, "clEnqueueWriteImage");

    size_t mapped_pitch = 0;
    char* mapped_image =
    (char*)clEnqueueMapImage(q, image, CL_TRUE, CL_MAP_READ, origin, region,
                             &mapped_pitch, NULL, 0, NULL, NULL, &err);
