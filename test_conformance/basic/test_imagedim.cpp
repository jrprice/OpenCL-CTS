...
"\n"
"__kernel void test_image_dim(read_only image2d_t srcimg, write_only image2d_t "
"dstimg, sampler_t sampler)\n"
"{\n"
...
static unsigned char *generate_8888_image(int w, int h, MTdata d)
{
...
static int verify_8888_image(unsigned char *image, unsigned char *outptr, int w,
                             int h)
{
    int i;
                       ...
                       cl_command_queue queue, int n_elems)
{
    ...
    err = clGetDeviceInfo(device, CL_DEVICE_IMAGE2D_MAX_WIDTH,
                          sizeof(max_image2d_width), &max_image2d_width, NULL);
    if (err)
    {
        log_error(
        "clGetDeviceInfo for CL_DEVICE_IMAGE2D_MAX_WIDTH failed (%d)\n", err);
        return -1;
    }
    err = clGetDeviceInfo(device, CL_DEVICE_IMAGE2D_MAX_HEIGHT,
                          sizeof(max_image2d_width), &max_image2d_height, NULL);
    if (err)
    {
        log_error(
        "clGetDeviceInfo for CL_DEVICE_IMAGE2D_MAX_HEIGHT failed (%d)\n", err);
        return -1;
    }
    log_info(
    "Device reported max image sizes of %lu x %lu, and max mem size of %gMB.\n",
    ...
    // pixel,
    //  and we want to consume 1/4 of global memory (this is the minimum
    ...
    if (max_img_width > max_img_dim) max_img_width = max_img_dim;
    if (max_img_height > max_img_dim) max_img_height = max_img_dim;
    ...
    output_ptr = (unsigned char *)malloc(sizeof(unsigned char) * 4
                                         * max_img_width * max_img_height);
    ...
    for (i = 1, i2 = 0; i <= max_img_height; i <<= 1, i2++)
    {
        ...
        for (j = 1, j2 = 0; j <= max_img_width; j <<= 1, j2++)
        {
            ...
            streams[0] =
            create_image_2d(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
                            &img_format, img_width, img_height, 0, NULL, NULL);
            if (!streams[0])
            {
            ...
            err = clEnqueueWriteImage(queue, streams[0], CL_FALSE, origin,
                                      region, 0, 0, input_ptr, 0, NULL, NULL);
            if (err != CL_SUCCESS)
            {
            ...
            log_info(
            "Testing image dimensions %d x %d with local threads NULL.\n",
            img_width, img_height);
            err = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, threads, NULL,
                                         0, NULL, NULL);
            if (err != CL_SUCCESS)
            {
                log_error("clEnqueueNDRangeKernel failed\n");
                log_error("Image Dimension test failed.  image width = %d, "
                          "image height = %d, local NULL\n",
                          img_width, img_height);
                clReleaseMemObject(streams[0]);
                clReleaseMemObject(streams[1]);
                free(input_ptr);
            ...
            err = clEnqueueReadImage(queue, streams[1], CL_TRUE, origin, region,
                                     0, 0, output_ptr, 0, NULL, NULL);
            if (err != CL_SUCCESS)
            {
            ...
            err =
            verify_8888_image(input_ptr, output_ptr, img_width, img_height);
            if (err)
            {
                total_errors++;
                log_error("Image Dimension test failed.  image width = %d, "
                           ...
                           cl_command_queue queue, int n_elems)
{
    ...
    test_error(err,
               "clGetKerenlWorkgroupInfo failed for CL_KERNEL_WORK_GROUP_SIZE");
            ...
            for (j = 2, j2 = 1; j <= max_img_width; j <<= 1, j2++)
            {
                ...
                switch (plus_minus)
                {
                    case 0:
                        ...
                        local_threads[0] =
                        work_group_size > max_local_workgroup_size[0]
                        ? max_local_workgroup_size[0]
                        : work_group_size;
                    ...
                    case 1:
                        effective_img_width--;
                        local_threads[1] =
                        work_group_size > max_local_workgroup_size[1]
                        ? max_local_workgroup_size[1]
                        : work_group_size;
                ...
                context, (cl_mem_flags)(CL_MEM_READ_WRITE), &img_format,
                ...
                size_t origin[3] = { 0, 0, 0 };
                size_t region[3] = { effective_img_width, effective_img_height,
                                     1 };
                err =
                clEnqueueWriteImage(queue, streams[0], CL_FALSE, origin, region,
                                    0, 0, input_ptr, 0, NULL, NULL);
                if (err != CL_SUCCESS)
                {
                    log_error("clWriteImage failed\n");
                    clReleaseMemObject(streams[0]);
                ...
                err = clSetKernelArg(kernel, 0, sizeof streams[0], &streams[0]);
                err |=
                clSetKernelArg(kernel, 1, sizeof streams[1], &streams[1]);
                err |= clSetKernelArg(kernel, 2, sizeof sampler, &sampler);
                if (err != CL_SUCCESS)
                {
                    log_error("clSetKernelArgs failed\n");
                ...
                log_info("Testing image dimensions %d x %d with local threads "
                         "%d x %d.\n",
                         effective_img_width, effective_img_height,
                         (int)local_threads[0], (int)local_threads[1]);
                err = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, threads,
                                             local_threads, 0, NULL, NULL);
                if (err != CL_SUCCESS)
                {
                    log_error("clEnqueueNDRangeKernel failed\n");
                    log_error("Image Dimension test failed.  image width = %d, "
                              "image height = %d, local %d x %d\n",
                              effective_img_width, effective_img_height,
                              (int)local_threads[0], (int)local_threads[1]);
                    clReleaseMemObject(streams[0]);
                    clReleaseMemObject(streams[1]);
                ...
                err =
                clEnqueueReadImage(queue, streams[1], CL_TRUE, origin, region,
                                   0, 0, output_ptr, 0, NULL, NULL);
                if (err != CL_SUCCESS)
                {
                    log_error("clReadImage failed\n");
                ...
                err =
                verify_8888_image(input_ptr, output_ptr, effective_img_width,
                                  effective_img_height);
                if (err)
                {
                    total_errors++;
