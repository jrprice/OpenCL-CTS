...
static const char *linear_ids_source[1] = {
    ...
    "        lid = get_local_id(0);\n"
    "    } else if (d == 2U) {\n"
    "        gid = (get_global_id(1) - get_global_offset(1)) * "
    "get_global_size(0) +\n"
    "              (get_global_id(0) - get_global_offset(0));\n"
    "        lid = get_local_id(1) * get_local_size(0) + get_local_id(0);\n"
    "    } else {\n"
    "        gid = ((get_global_id(2) - get_global_offset(2)) * "
    "get_global_size(1) +\n"
...
int test_get_linear_ids(cl_device_id device, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    ...
    context, &program, &kernel, 1, linear_ids_source, "test_linear_ids",
    "-cl-std=CL2.0");
    if (error) return error;

    // Create the out buffer
    outbuf =
    clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(outmem), NULL, &error);
    test_error(error, "failed to create result buffer\n");

    // This will leak if there is an error, but this is what is done everywhere
    // else
    ...
    for (iter = 0; iter < NUM_ITER; ++iter)
    {
        ...
        switch (dims)
        {
            case 1:
                ...
                error =
                get_max_common_work_group_size(context, kernel, gws[0], lws);
                break;
            case 2:
                gwo[0] = random_in_range(0, MAX_OFFSET, seed);
                ...
                error =
                get_max_common_2D_work_group_size(context, kernel, gws, lws);
                break;
            case 3:
                gwo[0] = random_in_range(0, MAX_OFFSET, seed);
                gwo[1] = random_in_range(0, MAX_OFFSET, seed);
                ...
                error =
                get_max_common_3D_work_group_size(context, kernel, gws, lws);
                break;
        }
        ...
        switch (dims)
        {
            ...
            case 2:
                log_info(
                "  testing offset=(%u,%u) global=(%u,%u) local=(%u,%u)...\n",
                gwo[0], gwo[1], gws[0], gws[1], lws[0], lws[1]);
                break;
            case 3:
                log_info("  testing offset=(%u,%u,%u) global=(%u,%u,%u) "
                         "local=(%u,%u,%u)...\n",
        ...
        error = clEnqueueWriteBuffer(queue, outbuf, CL_FALSE, 0, sizeof(outmem),
                                     (void *)outmem, 0, NULL, NULL);
        test_error(error, "clEnqueueWriteBuffer failed\n");

        error = clEnqueueNDRangeKernel(queue, kernel, dims, gwo, gws, lws, 0,
                                       NULL, NULL);
        test_error(error, "clEnqueueNDRangeKernel failed\n");

        error = clEnqueueReadBuffer(queue, outbuf, CL_FALSE, 0, sizeof(outmem),
                                    (void *)outmem, 0, NULL, NULL);
            ...
            case 2:
                for (j = 0, om = outmem; j < gws[1]; ++j)
                {
                    for (i = 0; i < gws[0]; ++i, om += 2)
                    {
                        ...
                        {
                            log_error(
                            "get_global_linear_id() failed at (%d,%d)\n", i, j);
                            return -1;
                        }
                        if (om[1] != 1)
                        {
                            log_error("get_local_linear_id() failed at (%d, "
                                      "%d), (%d, %d)\n",
                ...
                for (k = 0, om = outmem; k < gws[2]; ++k)
                {
                    for (j = 0; j < gws[1]; ++j)
                    {
                        for (i = 0; i < gws[0]; ++i, om += 2)
                        {
                            if (om[0] != 1)
                            {
                                log_error("get_global_linear_id() failed at "
                                          "(%d,%d, %d)\n",
                                          i, j, k);
                                return -1;
                            }
                            if (om[1] != 1)
                            {
                                log_error("get_local_linear_id() failed at "
                                          "(%d, %d), (%d, %d), (%d, %d)\n",
                                          i % (int)lws[0], j % (int)lws[1],
                                          k % (int)lws[2], i / (int)lws[0],
                                          j / (int)lws[1], k / (int)lws[2]);
                                return -1;
                            }
                        }
                    }
