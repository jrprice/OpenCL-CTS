...
const char *kernel_call_kernel_code[] = {
    "void test_function_to_call(__global int *output, __global int *input, int "
    "where);\n"
    "\n"
    "__kernel void test_kernel_to_call(__global int *output, __global int "
    "*input, int where) \n"
    "{\n"
    "  int b;\n"
    "  if (where == 0) {\n"
    ...
    "}\n"
    "\n"
    "__kernel void test_call_kernel(__global int *src, __global int *dst, int "
    "times) \n"
    "{\n"
...
int test_kernel_call_kernel_function(cl_device_id deviceID, cl_context context,
                                     cl_command_queue queue, int num_elements)
{
    ...
    // Create the kernel
    if (create_single_kernel_helper(context, &program, &kernel1, 1,
                                    kernel_call_kernel_code, "test_call_kernel")
        != 0)
    {
        return -1;
    }
    ...
    error = clEnqueueNDRangeKernel(queue, kernel1, 1, NULL, threads, NULL, 0,
                                   NULL, NULL);
    test_error(error, "clEnqueueNDRangeKernel failed");

    error =
    clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0,
                        sizeof(cl_int) * num_elements, output, 0, NULL, NULL);
    ...
    error =
    clEnqueueWriteBuffer(queue, streams[0], CL_TRUE, 0,
                         sizeof(cl_int) * num_elements, input, 0, NULL, NULL);
    test_error(error, "clEnqueueWriteBuffer failed");
    error =
    clEnqueueWriteBuffer(queue, streams[1], CL_TRUE, 0,
                         sizeof(cl_int) * num_elements, output, 0, NULL, NULL);
    test_error(error, "clEnqueueWriteBuffer failed");
    ...
    error = clEnqueueNDRangeKernel(queue, kernel2, 1, NULL, threads, NULL, 0,
                                   NULL, NULL);
    test_error(error, "clEnqueueNDRangeKernel failed");
    ...
    log_info(
    "Testing calling the kernel we called from another kernel before...\n");
    // Reset the inputs
    for (int i = 0; i < num_elements; i++)
    {
        input[i] = i;
    ...
    error = clEnqueueNDRangeKernel(queue, kernel_to_call, 1, NULL, threads,
                                   NULL, 0, NULL, NULL);
    test_error(error, "clEnqueueNDRangeKernel failed");
    ...
    if (pass)
        log_info(
        "Passed calling the kernel we called from another kernel before...\n");
