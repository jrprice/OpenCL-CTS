...
const char *barrier_with_localmem_kernel_code[] = {
    "__kernel void compute_sum_with_localmem(__global int *a, int n, __local "
    ...
    "       *sum = tmp_sum[0];\n"
    "}\n",
    "__kernel void compute_sum_with_localmem(__global int *a, int n, __global "
    "int *sum)\n"
    "{\n"
...
static int verify_sum(int *inptr, int *outptr, int n)
{
    int r = 0;
    int i;
...
int test_local_arg_def(cl_device_id device, cl_context context,
                       cl_command_queue queue, int num_elements)
{
    ...
    err = clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof wgsize,
                          &wgsize, NULL);
    if (err)
    {
    ...
    streams[0] =
    clCreateBuffer(context, CL_MEM_READ_WRITE, in_length, NULL, NULL);
    if (!streams[0])
    {
    ...
    streams[1] =
    clCreateBuffer(context, CL_MEM_READ_WRITE, out_length, NULL, NULL);
    if (!streams[1])
    {
        log_error("clCreateBuffer failed\n");
        return -1;
    ...
    for (i = 0; i < num_elements; i++) input_ptr[i] = (int)genrand_int32(d);

    free_mtdata(d);
    d = NULL;

    err = clEnqueueWriteBuffer(queue, streams[0], CL_TRUE, 0, in_length,
    ...
    err = create_single_kernel_helper(context, &program, &kernel, 1,
                                      &barrier_with_localmem_kernel_code[0],
                                      "compute_sum_with_localmem");
    if (err) return -1;

    err = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_WORK_GROUP_SIZE,
                                   sizeof kwgsize, &kwgsize, NULL);
    test_error(err,
    ...
    while (global_threads[0] % local_threads[0] != 0) local_threads[0]--;
    ...
    err = clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0, out_length,
...
int test_local_kernel_def(cl_device_id device, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    cl_mem streams[2];
    cl_program program;
    ...
    for (i = 0; i < num_elements; i++) input_ptr[i] = (cl_int)genrand_int32(d);
    ...
    // Validate that created kernel doesn't violate local memory size allowed by
    // the device
    ...
    sprintf(program_source, barrier_with_localmem_kernel_code[1],
            (int)(wgsize * sizeof(cl_int)));
