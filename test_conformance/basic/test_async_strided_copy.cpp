...
"%s__kernel void test_fn( const __global %s *src, __global %s *dst, __local %s "
"*localBuffer, int copiesPerWorkgroup, int copiesPerWorkItem, int stride )\n"
"{\n"
" int i;\n"
// Zero the local storage first
" for(i=0; i<copiesPerWorkItem; i++)\n"
"   localBuffer[ get_local_id( 0 )*copiesPerWorkItem+i ] = (%s)(%s)0;\n"
// Do this to verify all kernels are done zeroing the local buffer before we try
// the copy
" barrier( CLK_LOCAL_MEM_FENCE );\n"
" event_t event;\n"
" event = async_work_group_strided_copy( (__local %s*)localBuffer, (__global "
"const %s*)(src+copiesPerWorkgroup*stride*get_group_id(0)), "
"(size_t)copiesPerWorkgroup, (size_t)stride, 0 );\n"
// Wait for the copy to complete, then verify by manually copying to the dest
" wait_group_events( 1, &event );\n"
" for(i=0; i<copiesPerWorkItem; i++)\n"
"   dst[ get_global_id( 0 )*copiesPerWorkItem*stride+i*stride ] = localBuffer[ "
"get_local_id( 0 )*copiesPerWorkItem+i ];\n"
"}\n";
...
" barrier( CLK_LOCAL_MEM_FENCE );\n"
" for(i=0; i<copiesPerWorkItem; i++)\n"
"   localBuffer[ get_local_id( 0 )*copiesPerWorkItem+i ] = src[ get_global_id( "
"0 )*copiesPerWorkItem*stride+i*stride ];\n"
// Do this to verify all kernels are done copying to the local buffer before we
// try the copy
" barrier( CLK_LOCAL_MEM_FENCE );\n"
" event_t event;\n"
" event = async_work_group_strided_copy((__global "
"%s*)(dst+copiesPerWorkgroup*stride*get_group_id(0)), (__local const "
"%s*)localBuffer, (size_t)copiesPerWorkgroup, (size_t)stride, 0 );\n"
" wait_group_events( 1, &event );\n"
"}\n";


int test_strided_copy(cl_device_id deviceID, cl_context context,
                      ...
                      ExplicitType vecType, int vecSize, int stride)
{
    ...
    cl_bool unified_mem;
    error = clGetDeviceInfo(deviceID, CL_DEVICE_HOST_UNIFIED_MEMORY,
                            sizeof(unified_mem), &unified_mem, NULL);
    test_error(error,
    ...
    numberOfCopiesPerWorkitem * elementSize;
    ...
    localWorkgroupSize * elementSize * numberOfCopiesPerWorkitem;
    size_t numberOfLocalWorkgroups = 579; // 1111;

    // Reduce the numberOfLocalWorkgroups so that no more than 1/2 of
    // CL_DEVICE_GLOBAL_MEM_SIZE is consumed by the allocated buffer. This is
    // done to avoid resource  errors resulting from address space
    // fragmentation.
    size_t numberOfLocalWorkgroupsLimit = max_global_mem_size
    / (2 * number_of_global_mem_buffers * localBufferSize * stride);
    if (numberOfLocalWorkgroups > numberOfLocalWorkgroupsLimit)
        numberOfLocalWorkgroups = numberOfLocalWorkgroupsLimit;

    size_t globalBufferSize =
    numberOfLocalWorkgroups * localBufferSize * stride;
    ...
    log_info("Global: %d, local %d, local buffer %db, global buffer %db, copy "
             "stride %d, each work group will copy %d elements and each work "
             "item item will copy %d elements.\n",
             (int)globalWorkgroupSize, (int)localWorkgroupSize,
             (int)localBufferSize, (int)globalBufferSize, (int)stride,
             copiesPerWorkgroup, copiesPerWorkItemInt);
    ...
    vecType, globalBufferSize / get_explicit_type_size(vecType), d, inBuffer);
    free_mtdata(d);
    d = NULL;
    ...
    test_error(error, "Unable to set kernel argument");
    error =
    clSetKernelArg(kernel, 3, sizeof(copiesPerWorkgroup), &copiesPerWorkgroup);
    test_error(error, "Unable to set kernel argument");
    error = clSetKernelArg(kernel, 4, sizeof(copiesPerWorkItemInt),
                           &copiesPerWorkItemInt);
    test_error(error, "Unable to set kernel argument");
    ...
    for (int i = 0; i < (int)globalBufferSize;
         i += (int)elementSize * (int)stride)
    {
        if (memcmp(((char *)inBuffer) + i, ((char *)outBuffer) + i, typeSize)
            != 0)
        {
...
int test_strided_copy_all_types(cl_device_id deviceID, cl_context context,
                                cl_command_queue queue, const char *kernelCode)
{
    ...
    for (typeIndex = 0; vecType[typeIndex] != kNumExplicitTypes; typeIndex++)
    {
        ...
        for (size = 0; vecSizes[size] != 0; size++)
        {
            ...
            {
                if (test_strided_copy(deviceID, context, queue, kernelCode,
...
int test_async_strided_copy_global_to_local(cl_device_id deviceID,
                                            cl_context context,
                                            cl_command_queue queue,
                                            int num_elements)
{
    return test_strided_copy_all_types(deviceID, context, queue,
                                       async_strided_global_to_local_kernel);
}

int test_async_strided_copy_local_to_global(cl_device_id deviceID,
                                            cl_context context,
                                            cl_command_queue queue,
                                            int num_elements)
{
