...
const char *kernelSource[] = {
    "__kernel void test( __global unsigned int * input, __global unsigned int "
    "*outMaxes )\n"
    "{\n"
    ...
    "\n"
    "   // If we just write linearly, there's no verification that the items "
    "in a group share local data\n"
    "   // So we write reverse-linearly, which requires items to read the "
    "local data written by at least one\n"
    "   // different item\n"
    "   localStorage[ get_local_size( 0 ) - get_local_id( 0 ) - 1 ] = "
    "theValue;\n"
    "\n"
    "   // The barrier ensures that all local items have written to the local "
    "storage\n"
    "   barrier( CLK_LOCAL_MEM_FENCE );\n"
    "\n"
    "   // Now we loop back through the local storage and look for the max "
    "value. We only do this if\n"
...
int test_local_kernel_scope(cl_device_id device, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    ...
    // Make sure the work group size doesn't overrun our local storage size in
    // the kernel
    while (workGroupSize > MAX_LOCAL_STORAGE_SIZE) workGroupSize >>= 1;

    size_t testSize = workGroupSize;
    while (testSize < 1024) testSize += workGroupSize;
    ...
    clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                   testSize * sizeof(cl_uint), inputData, &error);
    test_error(error, "Unable to create input buffer");

    cl_uint *outputData = (cl_uint *)malloc(numGroups * sizeof(cl_uint));
    streams[1] = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
                                numGroups * sizeof(cl_uint), NULL, &error);
    ...
    error = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &testSize,
    ...
    clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0,
                        numGroups * sizeof(cl_uint), outputData, 0, NULL, NULL);
    test_error(error, "Unable to read output data");

    // MingW compiler seems to have a bug that otimizes the code below
    // incorrectly. adding the volatile keyword to size_t decleration to avoid
    // aggressive optimization by the compiler.
