...
const cl_mem_flags flag_set[] = { CL_MEM_ALLOC_HOST_PTR,
                                  ...
                                  0 };
const char *flag_set_names[] = { "CL_MEM_ALLOC_HOST_PTR",
                                 "CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR",
                                 "CL_MEM_USE_HOST_PTR", "CL_MEM_COPY_HOST_PTR",
                                 "0" };
                            ...
                            cl_command_queue queue, int num_elements)
{
    ...
    for (int src_flag_id = 0; src_flag_id < ARRAY_SIZE(flag_set); src_flag_id++)
    {
        ...
        if (hasHostPtr) hostPtr = initialData;
                           ...
                           cl_command_queue queue, int num_elements)
{
    ...
    for (int src_flag_id = 0; src_flag_id < ARRAY_SIZE(flag_set); src_flag_id++)
    {
        ...
        if (!hasHostPtr)
        {
            ...
            error =
            clEnqueueWriteImage(queue, memObject, CL_TRUE, write_origin,
                                write_region, 0, 0, initialData, 0, NULL, NULL);
        ...
        for (int i = 0; i < 128; i++)
        {
            ...
            for (size_t y = 0; y < region[1]; y++)
            {
                for (size_t x = 0; x < region[0] * 4; x++)
                {
                    ...
                    // Also update the initial data array
                    value =
                    initialData[((offset[1] + y) * imageSize + offset[0]) * 4
                                + x];
                    value = spin - value;
                    initialData[((offset[1] + y) * imageSize + offset[0]) * 4
                                + x] = value;
        ...
        for (size_t q = 0; q < imageSize * imageSize * 4; q++)
        {
            if (initialData[q] != finalData[q])
            {
                log_error("ERROR: Sample %d (coord %d,%d) did not validate! "
                          "Got %d, expected %d\n",
