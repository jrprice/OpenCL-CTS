...
int test_queue_priority(cl_device_id device, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    ...
    cl_queue_properties queue_properties[] = {
        CL_QUEUE_PROPERTIES, 0, 0, 0, 0, 0, 0
    };
    int idx = 2;

    // Check to see if queue priority is supported
    if (((command_queue_priority =
          is_extension_available(device, "cl_APPLE_command_queue_priority")))
        == 0)
    {
        log_info("cl_APPLE_command_queue_priority extension is not supported - "
                 "skipping test\n");
    }

    // Check to see if selecting the number of compute units is supported
    if (((command_queue_select_compute_units = is_extension_available(
          device, "cl_APPLE_command_queue_select_compute_units")))
        == 0)
    {
        log_info("cl_APPLE_command_queue_select_compute_units extension is not "
    ...
    if (err)
    {
        log_error("clGetDeviceInfo for CL_DEVICE_MAX_COMPUTE_UNITS failed: %d",
                  err);
        return -1;
    }
    ...
    cl_command_queue background_queue =
    clCreateCommandQueueWithProperties(context, device, queue_properties, &err);
    if (err)
    {
