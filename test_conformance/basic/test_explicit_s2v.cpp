...
#define DECLARE_S2V_IDENT_KERNEL(srctype, dsttype, size)                       \
    "__kernel void test_conversion(__global " srctype                          \
    " *sourceValues, __global " dsttype #size " *destValues )\n"               \
    "{\n"                                                                      \
...
#define DECLARE_S2V_IDENT_KERNELS_SET(srctype)                                 \
    {                                                                          \
        ...
        DECLARE_S2V_IDENT_KERNELS(#srctype, float), DECLARE_EMPTY              \
    }
...
int test_explicit_s2v_function(cl_device_id deviceID, cl_context context,
                               cl_command_queue queue, const char *programSrc,
                               ...
                               void *inputData)
{
    ...
    const char *finalProgramSrc[2] = { "", // optional pragma
                                       programSrc };
    ...
    error = clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0,
                                destStride * count, outData, 0, NULL, NULL);
    ...
    for (i = 0; i < count; i++)
    {
        ...
        convert_explicit_value((void *)inPtr, (void *)convertedData, srcType,
                               false, kDefaultRoundingType, destType);
        ...
        for (s = 0; s < vecSize; s++)
        {
                ...
                != 0)
            {
                ...
                log_error(
                "ERROR: Output value %d:%d does not validate for size %d:%d!\n",
...
int test_explicit_s2v_function_set(cl_device_id deviceID, cl_context context,
                                   cl_command_queue queue, ExplicitType srcType,
                                   unsigned int count, void *inputData)
{
    ...
    for (dstType = kBool; dstType < kNumExplicitTypes; dstType++)
    {
        ...
        if ((dstType == kLong || dstType == kULong) && !gHasLong) continue;

        for (i = 0; sizes[i] != 0; i++)
        {
            if (dstType != srcType) continue;
            if (strchr(get_explicit_type_name((ExplicitType)srcType), ' ')
                != NULL
                || strchr(get_explicit_type_name((ExplicitType)dstType), ' ')
                != NULL)
                continue;

            if (test_explicit_s2v_function(
                deviceID, context, queue,
                kernel_explicit_s2v_set[srcType][dstType][i], srcType, count,
                (ExplicitType)dstType, sizes[i], inputData)
                != 0)
            {
                log_error("ERROR: Explicit cast of scalar %s to vector %s%d "
                          "FAILED; skipping other %s vector tests\n",
                          get_explicit_type_name(srcType),
                          get_explicit_type_name((ExplicitType)dstType),
                          sizes[i],
                          get_explicit_type_name((ExplicitType)dstType));
                failed = -1;
                break;
...
int test_explicit_s2v_char(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    ...
    return test_explicit_s2v_function_set(deviceID, context, queue, kChar, 128,
                                          data);
}

int test_explicit_s2v_uchar(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kUChar, 128,
                                       data)
        != 0)
        return -1;
    if (test_explicit_s2v_function_set(deviceID, context, queue, kUnsignedChar,
                                       128, data)
        != 0)
        return -1;
    return 0;
...
int test_explicit_s2v_short(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    short data[128];
    RandomSeed seed(gRandomSeed);
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kShort, 128,
                                       data)
        != 0)
        return -1;
    return 0;
...
int test_explicit_s2v_ushort(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kUShort, 128,
                                       data)
        != 0)
        return -1;
    if (test_explicit_s2v_function_set(deviceID, context, queue, kUnsignedShort,
                                       128, data)
        != 0)
        return -1;
    return 0;
...
int test_explicit_s2v_int(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    int data[128];
    RandomSeed seed(gRandomSeed);
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kInt, 128,
                                       data)
        != 0)
        return -1;
    return 0;
...
int test_explicit_s2v_uint(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kUInt, 128,
                                       data)
        != 0)
        return -1;
    if (test_explicit_s2v_function_set(deviceID, context, queue, kUnsignedInt,
                                       128, data)
        != 0)
        return -1;
    return 0;
...
int test_explicit_s2v_long(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    cl_long data[128];
    RandomSeed seed(gRandomSeed);
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kLong, 128,
                                       data)
        != 0)
        return -1;
    return 0;
...
int test_explicit_s2v_ulong(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    cl_ulong data[128];
    RandomSeed seed(gRandomSeed);
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kULong, 128,
                                       data)
        != 0)
        return -1;
    if (test_explicit_s2v_function_set(deviceID, context, queue, kUnsignedLong,
                                       128, data)
        != 0)
        return -1;
    return 0;
...
int test_explicit_s2v_float(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    float data[128];
    RandomSeed seed(gRandomSeed);
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kFloat, 128,
                                       data)
        != 0)
        return -1;
    return 0;
...
int test_explicit_s2v_double(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    double data[128];
    RandomSeed seed(gRandomSeed);
    ...
    if (test_explicit_s2v_function_set(deviceID, context, queue, kDouble, 128,
                                       data)
        != 0)
        return -1;
    return 0;
