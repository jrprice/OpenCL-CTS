                             ...
                             cl_command_queue queue, int num_elements)
{
    ...
    error = create_single_kernel_helper(context, &program, &kernel, 1,
                                        &workItemKernelCode, "sample_kernel");
    test_error(error, "Unable to create testing kernel");

    outData =
    clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(testData), NULL, &error);
    ...
    for (size_t dim = 1; dim <= 3; dim++)
    {
        for (int i = 0; i < NUM_TESTS; i++)
        {
            ...
            {
                // All of our thread sizes should be within the max local sizes,
                // since they're all <= 20
                threads[j] = (size_t)random_in_range(1, 20, d);
                localThreads[j] =
                threads[j] / (size_t)random_in_range(1, (int)threads[j], d);
            ...
            error =
            clEnqueueNDRangeKernel(queue, kernel, (cl_uint)dim, NULL, threads,
                                   localThreads, 0, NULL, NULL);
            test_error(error, "Unable to run kernel");

            error =
            clEnqueueReadBuffer(queue, outData, CL_TRUE, 0, sizeof(testData),
            ...
            {
                // We can't really validate the actual value of each one, but we
                // can validate that they're within a sane range
                if (testData[q].workDim != (cl_uint)dim)
                {
                    log_error("ERROR: get_work_dim() did not return proper "
                              "value for %d dimensions (expected %d, got %d)\n",
                ...
                for (size_t j = 0; j < dim; j++)
                {
                    ...
                    {
                        log_error(
                        "ERROR: get_global_size(%d) did not return proper "
                        "value for %d dimensions (expected %d, got %d)\n",
                    ...
                    if (testData[q].globalID[j] < 0
                        || testData[q].globalID[j] >= (cl_uint)threads[j])
                    {
                        log_error(
                        "ERROR: get_global_id(%d) did not return proper value "
                        "for %d dimensions (max %d, got %d)\n",
                        (int)j, (int)dim, (int)threads[j],
                    ...
                    {
                        log_error(
                        "ERROR: get_local_size(%d) did not return proper value "
                    ...
                    if (testData[q].localID[j] < 0
                        && testData[q].localID[j] >= (cl_uint)localThreads[j])
                    {
                        log_error(
                        "ERROR: get_local_id(%d) did not return proper value "
                        "for %d dimensions (max %d, got %d)\n",
                        (int)j, (int)dim, (int)localThreads[j],
                    ...
                    size_t groupCount =
                    (threads[j] + localThreads[j] - 1) / localThreads[j];
                    if (testData[q].numGroups[j] != (cl_uint)groupCount)
                    {
                        log_error(
                        "ERROR: get_num_groups(%d) did not return proper value "
                        "for %d dimensions (expected %d with global dim %d and "
                        "local dim %d, got %d)\n",
                        (int)j, (int)dim, (int)groupCount, (int)threads[j],
                        (int)localThreads[j], (int)testData[q].numGroups[j]);
                        free_mtdata(d);
                        return -1;
                    }
                    if (testData[q].groupID[j] < 0
                        || testData[q].groupID[j] >= (cl_uint)groupCount)
                    {
                        log_error(
                        "ERROR: get_group_id(%d) did not return proper value "
                        "for %d dimensions (max %d, got %d)\n",
