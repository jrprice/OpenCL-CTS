...
int verify_float2int(cl_float *inptr, cl_int *outptr, int n)
{
    int i;
                   ...
                   cl_command_queue queue, int num_elements)
{
    ...
    for (i = 0; i < num_elements; i++)
        input_ptr[i] = get_random_float(-MAKE_HEX_FLOAT(0x1.0p31f, 0x1, 31),
    ...
    err = create_single_kernel_helper(context, &program, &kernel, 1,
                                      &float2int_kernel_code, "test_float2int");
    if (err != CL_SUCCESS)
    {
