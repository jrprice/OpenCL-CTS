...
static int verify_global_linear_id(int *result, int n)
{
    int i;
    for (i = 0; i < n; i++)
    {
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    context, &program[0], &kernel[0], 1, &global_linear_id_1d_code,
    "test_global_linear_id_1d", "-cl-std=CL2.0");
    test_error(err, "create_single_kernel_helper failed");
    err = create_single_kernel_helper_with_build_options(
    context, &program[1], &kernel[1], 1, &global_linear_id_2d_code,
    "test_global_linear_id_2d", "-cl-std=CL2.0");
    test_error(err, "create_single_kernel_helper failed");
    ...
    threads[0] = (size_t)num_elements;
    err = clEnqueueNDRangeKernel(queue, kernel[1], 1, NULL, threads, NULL, 0,
                                 NULL, NULL);
    test_error(err, "clEnqueueNDRangeKernel failed.");
