                  ...
                  int *number_of_results, int skip_to_result)
{
    ...
    sprintf(program,
            ...
            type == kDouble ? "#pragma OPENCL EXTENSION cl_khr_fp64 : enable"
                            : "",
            get_explicit_type_name(type), get_explicit_type_name(type),
    ...
    if (DEBUG)
        log_info(
        "\t\t(Program for vector type %s%s contains %d vector creations, of "
        "total program length %gkB, with a total of %d vloads.)\n",
                         ...
                         cl_command_queue queue, int num_elements)
{
    ...
    cl_int input_data_int[16] = { 0, 1, 2,  3,  4,  5,  6,  7,
    ...
    for (int type_index = 0; type_index < 10; type_index++)
    {
        ...
        {
            memcpy(input_data_converted, &input_data_double,
                   sizeof(cl_double) * 16);
        }
        ...
        clCreateBuffer(context, CL_MEM_COPY_HOST_PTR,
                       ...
                       (vecType[type_index] != kDouble) ? input_data_converted
                                                        : input_data_double,
                       &error);
        if (error)
        {
        ...
        for (int size_index = 1; size_index < 5; size_index++)
        {
            ...
            while (number_generated != 0)
            {
                ...
                number_generated = create_kernel(
                vecType[type_index], vecSizes[size_index], program_source,
                &number_of_results, number_generated);
                ...
                context, &program, &kernel, 1, (const char **)&program_source,
                ...
                number_of_results * get_explicit_type_size(vecType[type_index])
                * vecSizes[size_index],
                NULL, &error);
                if (error)
                {
                ...
                for (int i = 0; i < number_of_results; i++)
                {
                               ...
                               * vecSizes[size_index]))
                    {
                        ...
                        for (int j = 0; j < (int)vecSizes[size_index]; j++)
                        {
                            ...
                            expected_value);
                            log_error(
                            "index [%d, component %d]: got: %s expected: %s\n",
