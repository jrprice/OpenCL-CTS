...
int test_astype_set(cl_device_id device, cl_context context,
                    ...
                    unsigned int outVecSize, int numElements)
{
    ...
    if (outVecSize == 3 && vecSize == 3)
    {
        ...
        sprintf(programSrc, astype_kernel_pattern_V3srcV3dst,
                (outVecType == kDouble || inVecType == kDouble)
    ...
    size_t inBufferSize =
    sizeof(char) * numElements * get_explicit_type_size(inVecType) * vecSize;
    char *inBuffer = (char *)malloc(inBufferSize);
    size_t outBufferSize = sizeof(char) * numElements
    * get_explicit_type_size(outVecType) * outVecSize;
    char *outBuffer = (char *)malloc(outBufferSize);

    d = init_genrand(gRandomSeed);
    generate_random_data(inVecType, numElements * vecSize, d, inBuffer);
    free_mtdata(d);
    d = NULL;
    ...
    streams[1] =
    clCreateBuffer(context, CL_MEM_READ_WRITE, outBufferSize, NULL, &error);
    test_error(error, "Unable to create I/O stream");
    ...
    // The beauty is that astype is supposed to return the bit pattern as a
    // different type, which means the output should have the exact same bit
    // pattern as the input. No interpretation necessary!
    ...
    for (int i = 0; i < numElements; i++)
    {
        if (memcmp(expected, actual, compSize) != 0)
        {
            ...
            log_error(
            "ERROR: Data sample %d of %d for as_%s%d( %s%d ) did not validate "
            "(expected {%s}, got {%s})\n",
            (int)i, (int)numElements, get_explicit_type_name(outVecType),
            vecSize, get_explicit_type_name(inVecType), vecSize,
...
int test_astype(cl_device_id device, cl_context context, cl_command_queue queue,
                int n_elems)
{
    // Note: although casting to different vector element sizes that match the
    // same size (i.e. short2 -> char4) is legal in OpenCL 1.0, the result is
    // dependent on the device it runs on, which means there's no actual way for
    // us to verify what is "valid". So the only thing we can test are types
    // that match in size independent of the element count (char -> uchar, etc)
    ExplicitType vecTypes[] = {
    ...
    for (inTypeIdx = 0; vecTypes[inTypeIdx] != kNumExplicitTypes; inTypeIdx++)
    {
        ...
        if (vecTypes[inTypeIdx] == kDouble
            && !is_extension_available(device, "cl_khr_fp64"))
            continue;

        if ((vecTypes[inTypeIdx] == kLong || vecTypes[inTypeIdx] == kULong)
            && !gHasLong)
            continue;

        for (outTypeIdx = 0; vecTypes[outTypeIdx] != kNumExplicitTypes;
             outTypeIdx++)
        {
            outTypeSize = get_explicit_type_size(vecTypes[outTypeIdx]);
            if (vecTypes[outTypeIdx] == kDouble
                && !is_extension_available(device, "cl_khr_fp64"))
            {
                continue;
            }

            if ((vecTypes[outTypeIdx] == kLong
                 || vecTypes[outTypeIdx] == kULong)
                && !gHasLong)
                continue;
            ...
            log_info(" (%s->%s)\n", get_explicit_type_name(vecTypes[inTypeIdx]),
                     get_explicit_type_name(vecTypes[outTypeIdx]));
            fflush(stdout);
                ...
                == get_explicit_type_size(vecTypes[outTypeIdx]))
            {
                ...
                error +=
                test_astype_set(device, context, queue, vecTypes[inTypeIdx],
                                vecTypes[outTypeIdx], 3, 4, n_elems);
                error +=
                test_astype_set(device, context, queue, vecTypes[inTypeIdx],
                                vecTypes[outTypeIdx], 4, 3, n_elems);
            }
        }
    }
    return error;
