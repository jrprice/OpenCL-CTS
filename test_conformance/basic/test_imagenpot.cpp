...
"\n"
"__kernel void test_rgba8888(read_only image2d_t srcimg, write_only image2d_t "
...
"    float4 color;\n"
"\n"
"    if ( (tid_x >= get_image_width(dstimg)) || (tid_y >= "
"get_image_height(dstimg)) )\n"
"        return;\n"
...
static int verify_rgba8888_image(unsigned char *src, unsigned char *dst, int w,
                                 int h)
{
    int i;
    ...
    for (i = 0; i < w * h * 4; i++)
    {
        if (dst[i] != src[i])
        {
            log_error("NPOT_IMAGE_RGBA_UNORM_INT8 test for width = %d, height "
                      "= %d failed\n",
                      w, h);
            return -1;
        }
    ...
    log_info(
    "NPOT_IMAGE_RGBA_UNORM_INT8 test for width = %d, height = %d passed\n", w,
    h);
    return 0;
}
...
int test_imagenpot(cl_device_id device_id, cl_context context,
                   cl_command_queue queue, int num_elements)
{
    ...
    for (m = 0; m < sizeof(img_width_selection) / sizeof(int); m++)
    {
        ...
        err = clEnqueueWriteImage(queue, streams[0], CL_TRUE, origin, region, 0,
                                  0, input_ptr, 0, NULL, NULL);
        if (err != CL_SUCCESS)
        {
        ...
        context, &program, &kernel, 1, &rgba8888_kernel_code, "test_rgba8888");
        if (err)
        {
        ...
        kernel, device_id, CL_KERNEL_WORK_GROUP_SIZE,
        sizeof(local_workgroup_size), &local_workgroup_size, NULL);
        test_error(
        err, "clGetKernelWorkGroupInfo for CL_KERNEL_WORK_GROUP_SIZE failed");
        ...
        global_threads[0] =
        ((img_width + local_workgroup_size - 1) / local_workgroup_size)
        * local_workgroup_size;
        global_threads[1] = img_height;
        local_threads[0] = local_workgroup_size;
        local_threads[1] = 1;
        err = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, global_threads,
        ...
        err = clEnqueueReadImage(queue, streams[1], CL_TRUE, origin, region, 0,
        ...
        err =
        verify_rgba8888_image(input_ptr, output_ptr, img_width, img_height);
