...
static float *generate_float_image(int w, int h, int d, MTdata data)
{
...
static int verify_float_image(float *image, float *outptr, int w, int h, int d)
{
    int i;
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    err = create_single_kernel_helper(context, &program, &kernel, 1,
                                      &rgbaFFFF_kernel_code, "test_rgbaFFFF");
    if (err) return -1;
    ...
    err =
    verify_float_image(input_ptr, output_ptr, img_width, img_height, img_depth);
