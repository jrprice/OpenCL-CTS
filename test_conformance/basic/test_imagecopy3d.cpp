...
static unsigned char *generate_uint8_image(unsigned num_elements, MTdata d)
{
    unsigned char *ptr = (unsigned char *)malloc(num_elements);
    unsigned i;

    for (i = 0; i < num_elements; i++) ptr[i] = (unsigned char)genrand_int32(d);

    return ptr;
}

static int verify_uint8_image(unsigned char *image, unsigned char *outptr,
...
static unsigned short *generate_uint16_image(unsigned num_elements, MTdata d)
{
    unsigned short *ptr =
    (unsigned short *)malloc(num_elements * sizeof(unsigned short));
    unsigned i;
...
static int verify_uint16_image(unsigned short *image, unsigned short *outptr,
                               unsigned num_elements)
{
    unsigned i;
...
static float *generate_float_image(unsigned num_elements, MTdata d)
{
...
static int verify_float_image(float *image, float *outptr,
                              unsigned num_elements)
{
    unsigned i;
...
int test_imagecopy3d(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements_ignored)
{
    ...
    rgba8_outptr =
    (unsigned char *)malloc(sizeof(unsigned char) * num_elements);
    rgba16_outptr =
    (unsigned short *)malloc(sizeof(unsigned short) * num_elements);
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        switch (i)
        {
            case 0:
                err =
                verify_uint8_image(rgba8_inptr, rgba8_outptr, num_elements);
                if (err) log_error("Failed uint8\n");
                break;
            case 1:
                err =
                verify_uint16_image(rgba16_inptr, rgba16_outptr, num_elements);
                if (err) log_error("Failed uint16\n");
                break;
            case 2:
                err =
                verify_float_image(rgbafp_inptr, rgbafp_outptr, num_elements);
