...
int verify_long_add(cl_long *inptrA, cl_long *inptrB, cl_long *outptr, int n)
{
    cl_long r;
    int i;
...
int verify_long_sub(cl_long *inptrA, cl_long *inptrB, cl_long *outptr, int n)
{
    cl_long r;
    int i;
...
int verify_long_mul(cl_long *inptrA, cl_long *inptrB, cl_long *outptr, int n)
{
    cl_long r;
    int i;
...
int verify_long_mad(cl_long *inptrA, cl_long *inptrB, cl_long *inptrC,
                      ...
                      cl_command_queue queue, int num_elements)
{
    ...
    err = create_single_kernel_helper(context, &program[0], &kernel[0], 1,
                                      &long_add_kernel_code, "test_long_add");
    if (err != CL_SUCCESS)
    {
    ...
    err = create_single_kernel_helper(context, &program[1], &kernel[1], 1,
                                      &long_sub_kernel_code, "test_long_sub");
    if (err != CL_SUCCESS)
    {
        log_error("create_single_kernel_helper failed\n");
        return -1;
    ...
    err = create_single_kernel_helper(context, &program[2], &kernel[2], 1,
                                      &long_mul_kernel_code, "test_long_mul");
    if (err != CL_SUCCESS)
    {
        log_error("create_single_kernel_helper failed\n");
        return -1;
    ...
    err = create_single_kernel_helper(context, &program[3], &kernel[3], 1,
                                      &long_mad_kernel_code, "test_long_mad");
    if (err != CL_SUCCESS)
    {
        log_error("create_single_kernel_helper failed\n");
        return -1;
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        {
            case 0:
                err = verify_long_add(input_ptr[0], input_ptr[1], output_ptr,
                                      num_elements);
                break;
            case 1:
                err = verify_long_sub(input_ptr[0], input_ptr[1], output_ptr,
                                      num_elements);
                break;
            case 2:
                err = verify_long_mul(input_ptr[0], input_ptr[1], output_ptr,
                                      num_elements);
                break;
            case 3:
                err = verify_long_mad(input_ptr[0], input_ptr[1], input_ptr[2],
