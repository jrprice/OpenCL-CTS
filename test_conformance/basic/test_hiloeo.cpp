...
static const OffsetFunc offsetFuncs[4] = { hi_offset, lo_offset, even_offset,
                                           odd_offset };
...
int test_hiloeo(cl_device_id device, cl_context context, cl_command_queue queue,
                int n_elems)
{
         ...
         type++)
    {
        ...
        streams[0] =
        clCreateBuffer(context, CL_MEM_READ_WRITE, length, NULL, NULL);
        if (!streams[0])
        {
        ...
        for (operatorToUse = 0; operatorToUse
             < sizeof(operatorToUse_names) / sizeof(operatorToUse_names[0]);
             operatorToUse++)
        {
            log_info(" %s", operatorToUse_names[operatorToUse]);
            fflush(stdout);
            for (vectorSize = 1; vectorSize
                 < sizeof(vector_size_names) / sizeof(vector_size_names[0]);
                 vectorSize++)
            {
                for (expressionMode = 0; expressionMode < numExpressionModes;
                     ++expressionMode)
                {
                    ...
                    const char *source[] = {
                        ...
                        " *srcA, __global ",
                        test_str_names[type],
                        vector_size_names[outVectorSize],
                        " *dst)\n"
                        "{\n"
                    ...
                    else if (expressionMode == 1)
                    {
                        switch (vector_sizes[vectorSize])
                        {
                            case 16:
                                sprintf(
                                expression,
                                "((%s16)(srcA[tid].s0, srcA[tid].s1, "
                                "srcA[tid].s2, srcA[tid].s3, srcA[tid].s4, "
                                "srcA[tid].s5, srcA[tid].s6, srcA[tid].s7, "
                                "srcA[tid].s8, srcA[tid].s9, srcA[tid].sA, "
                                "srcA[tid].sB, srcA[tid].sC, srcA[tid].sD, "
                                "srcA[tid].sE, srcA[tid].sf))",
                                test_str_names[type]);
                                break;
                            case 8:
                                sprintf(
                                expression,
                                "((%s8)(srcA[tid].s0, srcA[tid].s1, "
                                "srcA[tid].s2, srcA[tid].s3, srcA[tid].s4, "
                                "srcA[tid].s5, srcA[tid].s6, srcA[tid].s7))",
                                test_str_names[type]);
                                break;
                            case 4:
                                sprintf(expression,
                                        "((%s4)(srcA[tid].s0, srcA[tid].s1, "
                                        "srcA[tid].s2, srcA[tid].s3))",
                                        test_str_names[type]);
                                break;
                            case 3:
                                sprintf(expression,
                                        "((%s3)(srcA[tid].s0, srcA[tid].s1, "
                                        "srcA[tid].s2))",
                                        test_str_names[type]);
                                break;
                            case 2:
                    ...
                    char kernelName[128];
                    snprintf(kernelName, sizeof(kernelName), "test_%s_%s%s",
                             operatorToUse_names[operatorToUse],
                             test_str_names[type],
                             vector_size_names[vectorSize]);
                    err = create_single_kernel_helper(
                    context, &program, &kernel,
                    sizeof(source) / sizeof(source[0]), source, kernelName);
                    if (err) return -1;

                    err =
                    clSetKernelArg(kernel, 0, sizeof streams[0], &streams[0]);
                    err |=
                    ...
                    err =
                    clEnqueueWriteBuffer(queue, streams[1], CL_TRUE, 0, length,
                    ...
                    err =
                    clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0, length,
                                        output_ptr, 0, NULL, NULL);
                    if (err != CL_SUCCESS)
                    {
                    ...
                    outP += kSizes[type]
                    * ((vector_sizes[outVectorSize])
                       - (vector_sizes[out_vector_idx[vectorSize]]));
                    // was                outP += kSizes[type] * ( ( 1 <<
                    // outVectorSize ) - ( 1 << ( vectorSize - 1 ) ) );
                    for (size_t e = 0; e < size; e++)
                    {
                        if (CheckResults(inP, outP, 1, type, vectorSize,
                                         operatorToUse))
                        {
    ...
    switch (elementSize)
    {
        case 1: {
            ...
            for (k = 0; k < elementCount; k++)
            {
                ...
                {
                    log_info("\n%d) Failure for %s%s.%s { %d", k,
                             test_str_names[type],
                             vector_size_names[vectorSize],
                             operatorToUse_names[operatorToUse], i[0]);
                    for (j = 1; j < halfVectorSize * 2; j++)
                        log_info(", %d", i[j]);
                    log_info(" } --> { %d", o[0]);
                    for (j = 1; j < halfVectorSize; j++) log_info(", %d", o[j]);
                    log_info(" }\n");
                    return -1;
                }
                i += 2 * halfVectorSize;
        ...
        case 4: {
            ...
            for (k = 0; k < elementCount; k++)
            {
                ...
                for (j = 0; j < cmpVectorSize; j++)
                {
                    ...
                    if (memcmp(&o[j], &o2[j], elementSize)
                        && !((strcmp(test_str_names[type], "float") == 0)
                             && isnan(((float *)o)[j])
                             && isnan(((float *)o2)[j])))
                    {
                        log_info("\n%d) Failure for %s%s.%s { 0x%8.8x", k,
                                 test_str_names[type],
                                 vector_size_names[vectorSize],
                                 operatorToUse_names[operatorToUse], i[0]);
                        for (j = 1; j < halfVectorSize * 2; j++)
        ...
        case 8: {
            ...
            for (k = 0; k < elementCount; k++)
            {
                ...
                if (memcmp(o, o2, elementSize * cmpVectorSize))
                {
                    log_info("\n%d) Failure for %s%s.%s { 0x%16.16llx", k,
                             test_str_names[type],
                             vector_size_names[vectorSize],
                             operatorToUse_names[operatorToUse], i[0]);
                    for (j = 1; j < halfVectorSize * 2; j++)
