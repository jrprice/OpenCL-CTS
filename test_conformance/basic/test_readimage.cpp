                   ...
                   cl_command_queue queue, int num_elements)
{
    ...
    img_format.image_channel_data_type = CL_UNORM_INT8;
    streams[1] = clCreateImage2D(context, CL_MEM_READ_WRITE, &img_format,
                                 img_width, img_height, 0, NULL, NULL);
    if (!streams[1])
    {
        log_error("clCreateImage2D failed\n");
    ...
    err = clEnqueueWriteImage(queue, streams[1], CL_TRUE, origin, region, 0, 0,
    ...
    if (supportsBGRA)
    {
        ...
        create_single_kernel_helper(context, &program[0], &kernel[0], 1,
                                    &bgra8888_kernel_code, "test_bgra8888");
        if (err) return -1;
    }

    err = create_single_kernel_helper(context, &program[1], &kernel[1], 1,
                                      &rgba8888_kernel_code, "test_rgba8888");
    if (err) return -1;
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {
            case 0:
                err = verify_bgra8888_image(input_ptr[i], output_ptr, img_width,
                                            img_height);
                break;
            case 1:
                err = verify_rgba8888_image(input_ptr[i], output_ptr, img_width,
                                            img_height);
                break;
        }
