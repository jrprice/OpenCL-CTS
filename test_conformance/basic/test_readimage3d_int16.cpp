...
static unsigned short *generate_16bit_image(int w, int h, int d, MTdata data)
{
    unsigned short *ptr =
    (cl_ushort *)malloc(w * h * d * 4 * sizeof(cl_ushort));
    int i;

    for (i = 0; i < w * h * d * 4; i++) ptr[i] = (cl_ushort)genrand_int32(data);

    return ptr;
}

static int verify_16bit_image(cl_ushort *image, cl_ushort *outptr, int w, int h,
                              int d)
{
    int i;
                           ...
                           cl_command_queue queue, int num_elements)
{
    ...
    err = create_single_kernel_helper(context, &program, &kernel, 1,
                                      &rgba16_kernel_code, "test_rgba16");
    if (err) return -1;
    ...
    err = clEnqueueNDRangeKernel(queue, kernel, 3, NULL, threads, NULL, 0, NULL,
                                 NULL);
    ...
    err =
    verify_16bit_image(input_ptr, output_ptr, img_width, img_height, img_depth);
