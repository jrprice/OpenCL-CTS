...
"__kernel void test_fn( const __global %s *src, __global %s *dst, __local %s "
...
"    event_t event;\n"
"    event = async_work_group_copy( (__local %s*)localBuffer, (__global const "
"%s*)(src+copiesPerWorkgroup*get_group_id(0)), (size_t)copiesPerWorkgroup, 0 "
");\n"
// Wait for the copy to complete, then verify by manually copying to the dest
"    wait_group_events( 1, &event );\n"
" for(i=0; i<copiesPerWorkItem; i++)\n"
"  dst[ get_global_id( 0 )*copiesPerWorkItem+i ] = localBuffer[ get_local_id( "
"0 )*copiesPerWorkItem+i ];\n"
"}\n";
...
"%s*)(dst+copiesPerWorkgroup*get_group_id(0)), (__local const %s*)localBuffer, "
...
" // Ignore this: %s%s%s\n"
" int i;\n"
" prefetch( (const __global %s*)(src+copiesPerWorkItem*get_global_id(0)), "
"copiesPerWorkItem);\n"
" for(i=0; i<copiesPerWorkItem; i++)\n"
"  dst[ get_global_id( 0 )*copiesPerWorkItem+i ] = src[ get_global_id( 0 "
")*copiesPerWorkItem+i ];\n"
"}\n";


int test_copy(cl_device_id deviceID, cl_context context, cl_command_queue queue,
              const char *kernelCode, ExplicitType vecType, int vecSize)
{
    int error;
    ...
    log_info("Global: %d, local %d, local buffer %db, global buffer %db, each "
             "work group will copy %d elements and each work item item will "
             "copy %d elements.\n",
             (int)globalWorkgroupSize, (int)localWorkgroupSize,
             (int)localBufferSize, (int)globalBufferSize, copiesPerWorkgroup,
...
int test_copy_all_types(cl_device_id deviceID, cl_context context,
                        cl_command_queue queue, const char *kernelCode)
{
    ...
    for (typeIndex = 0; vecType[typeIndex] != kNumExplicitTypes; typeIndex++)
    {
        ...
        {
            if (test_copy(deviceID, context, queue, kernelCode,
...
int test_async_copy_global_to_local(cl_device_id deviceID, cl_context context,
                                    cl_command_queue queue, int num_elements)
{
    return test_copy_all_types(deviceID, context, queue,
                               async_global_to_local_kernel);
}

int test_async_copy_local_to_global(cl_device_id deviceID, cl_context context,
                                    cl_command_queue queue, int num_elements)
{
    return test_copy_all_types(deviceID, context, queue,
...
int test_prefetch(cl_device_id deviceID, cl_context context,
                  cl_command_queue queue, int num_elements)
{
