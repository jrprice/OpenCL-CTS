...
"__kernel void test_multireadimage(read_only image2d_t img0, read_only "
"image2d_t img1, \n"
"                                  read_only image2d_t img2, __global float4 "
...
static unsigned short *generate_16bit_image(int w, int h, MTdata d)
{
    unsigned short *ptr =
...
static float *generate_float_image(int w, int h, MTdata d)
{
...
static int verify_multireadimage(void *image[], float *outptr, int w, int h)
{
    int i;
    float sum;
    ...
    {
        log_error(
        "READ_MULTIREADIMAGE_MULTIFORMAT test failed.  Max ulp error = %g\n",
        max_ulp);
        return -1;
    }

    log_info(
    "READ_MULTIREADIMAGE_MULTIFORMAT test passed.  Max ulp error = %g\n",
    max_ulp);
    return 0;
}


int test_mri_multiple(cl_device_id device, cl_context context,
                      cl_command_queue queue, int num_elements)
{
    ...
    err = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, threads, NULL, 0, NULL,
                                 NULL);
    if (err != CL_SUCCESS)
    {
    ...
    err = clEnqueueReadBuffer(queue, streams[3], CL_TRUE, 0,
                              sizeof(float) * 4 * img_width * img_height,
    ...
    err = verify_multireadimage(input_ptr, (float *)output_ptr, img_width,
                                img_height);

    // cleanup
    clReleaseSampler(sampler);
    for (i = 0; i < 4; i++) clReleaseMemObject(streams[i]);
    clReleaseKernel(kernel);
    clReleaseProgram(program);
    for (i = 0; i < 3; i++) free(input_ptr[i]);
    free(output_ptr);
