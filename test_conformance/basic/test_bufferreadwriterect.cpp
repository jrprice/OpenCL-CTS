...
// Initialize a buffer in host memory containing random values of the specified
// size.
static void initialize_image(BufferType* ptr, size_t w, size_t h, size_t d,
                             MTdata mt)
{
    enum
    {
        ElementSize = sizeof(BufferType) / sizeof(unsigned char)
    };
                        ...
                        size_t region[3], size_t row_pitch, size_t slice_pitch)
{
    ...
    size_t dst_end =
    dst_start + (region[2] * slice_pitch + region[1] * row_pitch + region[0]);
    size_t src_start =
    src_offset[2] * slice_pitch + src_offset[1] * row_pitch + src_offset[0];
    size_t src_end =
    src_start + (region[2] * slice_pitch + region[1] * row_pitch + region[0]);
    if (!overlap)
    {
...
// This function invokes the CopyBufferRect CL command and then mirrors the
// operation on the host side verify buffers.
                ...
                size_t doffset[3], size_t dregion[3])
{
    ...
    {
        if ((err =
             clEnqueueCopyBufferRect(queue, buffer[src], buffer[dst], soffset,
                                     doffset, sregion, /*dregion,*/
                                     width[src], src_slice_pitch, width[dst],
                                     dst_slice_pitch, 0, NULL, NULL))
            != CL_SUCCESS)
        {
            CL_EXIT_ERROR(err,
    ...
    for (size_t i = 0; i != total; ++i)
    {
        ...
        // Compute the coordinates of the element within the source and
        // destination regions.
        ...
        size_t s_idx = (soffset[2] + sz) * sslice + (soffset[1] + sy) * spitch
        + soffset[0] + sx;
        size_t d_idx = (doffset[2] + dz) * dslice + (doffset[1] + dy) * dpitch
        + doffset[0] + dx;
                  ...
                  size_t sregion[3], size_t dst, size_t doffset[3])
{
    ...
    for (size_t i = 0; i != total; ++i)
    {
        ...
        size_t s_idx = (soffset[2] + sz) * sslice + (soffset[1] + sy) * spitch
        + soffset[0] + sx;
        size_t d_idx = (doffset[2] + sz) * dslice + (doffset[1] + sy) * dpitch
        + doffset[0] + sx;

        if (device[d_idx] != verify[src][s_idx])
        {
            log_error("Verify failed on comparsion %lu: coordinate (%lu, %lu, "
                      "%lu) of region\n",
                      i, sx, sy, sz);
                       ...
                       size_t dst, size_t doffset[3], size_t dregion[3])
{
    ...
    // temporary buffer.
    CL_EXIT_ERROR(
    clEnqueueReadBufferRect(queue, buffer[src], CL_TRUE, soffset, doffset,
                            sregion, width[src], src_slice_pitch, width[dst],
                            dst_slice_pitch, tmp_buffer, 0, NULL, NULL),
    "clEnqueueCopyBufferRect failed between %u and %u", (unsigned)src,
    (unsigned)dst);
...
int map_verify_region(size_t src)
{
    ...
    size_t size_bytes =
    width[src] * height[src] * depth[src] * sizeof(BufferType);

    // Copy the source region of the cl buffer, to the destination region of the
    // temporary buffer.
    cl_int err;
    BufferType* mapped =
    (BufferType*)clEnqueueMapBuffer(queue, buffer[src], CL_TRUE, CL_MAP_READ, 0,
                                    size_bytes, 0, NULL, NULL, &err);
    CL_EXIT_ERROR(err, "clEnqueueMapBuffer failed for buffer %u",
                  (unsigned)src);
    ...
    CL_EXIT_ERROR(
    clEnqueueUnmapMemObject(queue, buffer[src], mapped, 0, NULL, NULL),
                 ...
                 size_t doffset[3], size_t dregion[3])
{
    ...
    CL_EXIT_ERROR(clEnqueueWriteBufferRect(
                  queue, buffer[dst], CL_TRUE, doffset, soffset,
                  /*sregion,*/ dregion, width[dst], dst_slice_pitch, width[src],
                  src_slice_pitch, tmp_buffer, 0, NULL, NULL),
                             ...
                             cl_command_queue queue_, int num_elements)
{
    ...
    // Compute a maximum buffer size based on the number of test images and the
    // device maximum.
    cl_ulong max_mem_alloc_size = 0;
    CL_EXIT_ERROR(clGetDeviceInfo(device, CL_DEVICE_MAX_MEM_ALLOC_SIZE,
    ...
    size_t max_mem_alloc_dim =
    (size_t)cbrt((double)(max_mem_alloc_size / sizeof(BufferType)))
    / alloc_scale;
    ...
    // Create pairs of cl buffers and host buffers on which operations will be
    // mirrored.
    ...
    for (unsigned i = 0; i != TotalImages; ++i)
    {
        ...
        (tries < max_tries)
        && (size_bytes = width[i] * height[i] * depth[i] * sizeof(BufferType))
        > max_mem_alloc_size);
    ...
    for (unsigned i = 0; i != TotalImages; ++i)
    {
        ...
        clCreateBuffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                       size_bytes, backing[i], &err);
        CL_EXIT_ERROR(err, "clCreateBuffer failed for buffer %u", i);

        // Make sure buffer is cleaned up appropriately if we encounter an error
        // in the rest of the calls.
        err = clSetMemObjectDestructorCallback(
        buffer[i], mem_obj_destructor_callback, backing[i]);
    ...
    for (size_t iter = 0; iter < num_tries; ++iter)
    {
        ...
        size_t min_height =
        height[src] < height[dst] ? height[src] : height[dst];
        ...
        // - Copy: Copies between src and dst within each set of host, buffer,
        // and images.
        // - Read & verify: Reads src region from buffer and image, and compares
        // to host.
        // - Write: Generates new buffer with src dimensions, and writes to cl
        // buffer and image.
        ...
        {
            case 0:
                log_info("%lu Copy %lu offset (%lu,%lu,%lu) -> %lu offset "
                         "(%lu,%lu,%lu) region (%lux%lux%lu = %lu)\n",
                         iter, src, soffset[0], soffset[1], soffset[2], dst,
                         doffset[0], doffset[1], doffset[2], sregion[0],
                         sregion[1], sregion[2],
                         sregion[0] * sregion[1] * sregion[2]);
                if ((err =
                     copy_region(src, soffset, sregion, dst, doffset, dregion)))
                    return err;
                break;
            case 1:
                log_info("%lu Read %lu offset (%lu,%lu,%lu) -> %lu offset "
                         "(%lu,%lu,%lu) region (%lux%lux%lu = %lu)\n",
                         iter, src, soffset[0], soffset[1], soffset[2], dst,
                         doffset[0], doffset[1], doffset[2], sregion[0],
                         sregion[1], sregion[2],
                         sregion[0] * sregion[1] * sregion[2]);
                if ((err = read_verify_region(src, soffset, sregion, dst,
                                              doffset, dregion)))
                    return err;
                break;
            case 2:
                log_info("%lu Write %lu offset (%lu,%lu,%lu) -> %lu offset "
                         "(%lu,%lu,%lu) region (%lux%lux%lu = %lu)\n",
                         iter, src, soffset[0], soffset[1], soffset[2], dst,
                         doffset[0], doffset[1], doffset[2], sregion[0],
                         sregion[1], sregion[2],
                         sregion[0] * sregion[1] * sregion[2]);
                if ((err = write_region(src, soffset, sregion, dst, doffset,
                                        dregion)))
                    return err;
                break;
        }
    ...
    for (unsigned i = 0; i < TotalImages; ++i)
    {
        ...
        if ((err = map_verify_region(i))) return err;
    }
