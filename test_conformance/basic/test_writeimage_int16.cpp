...
"__kernel void test_rgba16_write(__global unsigned short *src, write_only "
"image2d_t dstimg)\n"
"{\n"
...
static unsigned short *generate_16bit_image(int w, int h, MTdata d)
{
    ...
    for (i = 0; i < w * h * 4; i++) ptr[i] = (cl_ushort)genrand_int32(d);
...
static int verify_16bit_image(const char *string, cl_ushort *image,
                              cl_ushort *outptr, int w, int h)
{
    int i;
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    img_format.image_channel_order = CL_RGBA;
    img_format.image_channel_data_type = CL_UNORM_INT16;
    streams[1] = create_image_2d(context, CL_MEM_WRITE_ONLY, &img_format,
                                 img_width, img_height, 0, NULL, NULL);
    if (!streams[1])
    {
        log_error("create_image_2d failed\n");
    ...
    err = clEnqueueWriteBuffer(queue, streams[2], CL_TRUE, 0, length, input_ptr,
                               0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    err =
    create_single_kernel_helper(context, &program, &kernel[0], 1,
                                &rgba16_write_kernel_code, "test_rgba16_write");
    if (err) return -1;
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        err = verify_16bit_image(
        (i == 0)
        ? "WRITE_IMAGE_RGBA_UNORM_INT16 test with memflags = CL_MEM_READ_WRITE"
