...
int test_imagearraycopy3d_single_format(cl_device_id device, cl_context context,
                                        ...
                                        cl_image_format *format)
{
    ...
    image = create_image_3d(context, (cl_mem_flags)(CL_MEM_READ_WRITE), format,
    ...
    err = clEnqueueReadBuffer(queue, buffer, CL_TRUE, 0, buffer_size, bufptr, 0,
                              NULL, NULL);
    ...
    if (memcmp(imgptr, bufptr, buffer_size) != 0)
    {
        ...
        for (i = 0; i < (int)buffer_size; i += (int)elem_size)
        {
            ...
            for (j = 0; j < (int)elem_size; j++)
                if (inchar[i + j] != outchar[i + j]) failed = 1;
            char values[4096];
            values[0] = 0;
            if (failed)
            {
    ...
    if (err)
        log_error(
        "IMAGE3D to ARRAY copy test failed for image_channel_order=0x%lx and "
                          ...
                          cl_command_queue queue, int num_elements)
{
    ...
    context, CL_MEM_READ_WRITE, CL_MEM_OBJECT_IMAGE3D, 0, NULL, &num_formats);
    ...
    err = clGetSupportedImageFormats(context, CL_MEM_READ_WRITE,
                                     CL_MEM_OBJECT_IMAGE3D, num_formats,
                                     formats, NULL);
    test_error(err, "clGetSupportedImageFormats failed");

    for (i = 0; i < num_formats; i++)
    {
        err |= test_imagearraycopy3d_single_format(device, context, queue,
                                                   &formats[i]);
    }
