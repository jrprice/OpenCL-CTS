...
"                                  read_only image2d_t img0, read_only "
"image2d_t img1, \n"
"                                  read_only image2d_t img2, read_only "
"image2d_t img3, \n"
"                                  read_only image2d_t img4, read_only "
"image2d_t img5, \n"
"                                  read_only image2d_t img6, __global float4 "
"*dst)\n"
"{\n"
...
static int verify_multireadimage(void *image[], int num_images, float *outptr,
                                 int w, int h)
{
    int i, j;
    float sum;
    ...
    {
        log_error(
        "READ_MULTIREADIMAGE_RGBA8888 test failed.  Max ULP err = %g\n",
        max_ulp);
        return -1;
    }
...
int test_mri_one(cl_device_id device, cl_context context,
                 cl_command_queue queue, int num_elements)
{
    ...
    for (i = 0; i < 7; i++)
    {
        ...
        streams[i] = create_image_2d(context, CL_MEM_READ_WRITE, &img_format,
    ...
    err = clEnqueueReadBuffer(queue, streams[7], CL_TRUE, 0, length, output_ptr,
                              0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    err = verify_multireadimage(input_ptr, 7, (float *)output_ptr, img_width,
                                img_height);
