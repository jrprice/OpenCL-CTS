...
static void update_rgba8_image(unsigned char *p, int x, int y, int w, int h,
                               int img_width, MTdata d)
{
    int i, j, indx;
...
static void update_image_from_image(void *out, void *in, int x, int y, int w,
                                    int h, int img_width, int elem_size)
{
...
static unsigned short *generate_rgba16_image(int w, int h, MTdata d)
{
    unsigned short *ptr =
    (unsigned short *)malloc(w * h * 4 * sizeof(unsigned short));
    int i;

    for (i = 0; i < w * h * 4; i++) ptr[i] = (unsigned short)genrand_int32(d);

    return ptr;
}

static void update_rgba16_image(unsigned short *p, int x, int y, int w, int h,
                                int img_width, MTdata d)
{
    int i, j, indx;
...
static float *generate_rgbafp_image(int w, int h, MTdata d)
{
...
static void update_rgbafp_image(float *p, int x, int y, int w, int h,
                                int img_width, MTdata d)
{
    int i, j, indx;
...
static int verify_rgbafp_image(float *image, float *outptr, int w, int h)
{
    int i;
...
int test_imagereadwrite(cl_device_id device, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    ...
    rgba8_inptr =
    (unsigned char *)generate_rgba8_image(img_width, img_height, d);
    rgba16_inptr =
    (unsigned short *)generate_rgba16_image(img_width, img_height, d);
    ...
    rgba16_outptr = (unsigned short *)malloc(sizeof(unsigned short) * 4
    ...
    streams[0] =
    create_image_2d(context, (cl_mem_flags)(CL_MEM_READ_WRITE), &img_format,
    ...
    for (i = 0, j = 0; i < num_tries * 3; i++, j++)
    {
        ...
        switch (j)
        {
            case 0:
                ...
                if (packed_update)
                {
                    ...
                    update_image_from_image(rgba8_inptr, p, x, y, w, h,
                                            img_width, elem_size);
                }
                else
                {
            ...
            case 1:
                ...
                if (packed_update)
                {
                    ...
                    update_image_from_image(rgba16_inptr, p, x, y, w, h,
                                            img_width, elem_size);
                }
                else
                {
            ...
            case 2:
                ...
                if (packed_update)
                {
                    ...
                    update_image_from_image(rgbafp_inptr, p, x, y, w, h,
                                            img_width, elem_size);
                }
                else
                {
        ...
        err = clEnqueueWriteImage(queue, streams[j], CL_TRUE, origin, region,
                                  input_pitch, 0, p, 0, NULL, NULL);
        if (err != CL_SUCCESS)
        {
