...
"__kernel void test_rgbaFFFF_write(__global float *src, write_only image2d_t "
"dstimg)\n"
"{\n"
...
static int verify_float_image(const char *string, float *image, float *outptr,
                              int w, int h)
{
    int i;
                         ...
                         cl_command_queue queue, int num_elements)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        err = verify_float_image(
        (i == 0)
        ? "WRITE_IMAGE_RGBA_FLOAT test with memflags = CL_MEM_READ_WRITE"
