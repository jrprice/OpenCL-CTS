...
static unsigned char *generate_3d_image8(int w, int h, int d, MTdata data)
{
...
static int verify_3d_image8(double *image, float *outptr, int w, int h, int d)
{
    int i;
...
static double *prepare_reference(unsigned char *input_ptr, int w, int h, int d)
{
    double *ptr = (double *)malloc(w * h * d * 4 * sizeof(double));
    int i;
    for (i = 0; i < w * h * d * 4; i++) ptr[i] = ((double)input_ptr[i] / 255);
                     ...
                     cl_command_queue queue, int num_elements)
{
    ...
    {
        if (!is_image_format_required(formatsToTest[i].img_format,
        ...
        ref_ptr =
        prepare_reference(input_ptr, img_width, img_height, img_depth);
        output_ptr = (float *)malloc(length);

        streams[0] =
        create_image_3d(context, CL_MEM_READ_ONLY, &formatsToTest[i].img_format,
                        img_width, img_height, img_depth, 0, 0, NULL, &err);
        ...
        err = create_single_kernel_helper(context, &program, &kernel, 1,
                                          &formatsToTest[i].kernelSourceString,
                                          formatsToTest[i].kernelName);
        ...
        err = clEnqueueNDRangeKernel(queue, kernel, 3, NULL, threads, NULL, 0,
                                     NULL, NULL);
        ...
        err =
        verify_3d_image8(ref_ptr, output_ptr, img_width, img_height, img_depth);
        if (err == 0)
        {
            log_info("READ_IMAGE3D_%s_%s test passed\n",
                     GetChannelTypeName(
                     formatsToTest[i].img_format.image_channel_data_type),
                     GetChannelOrderName(
