...
// Test __FILE__, __LINE__, __OPENCL_VERSION__, __OPENCL_C_VERSION__,
// __ENDIAN_LITTLE__, __ROUNDING_MODE__, __IMAGE_SUPPORT__,
...
const char *preprocessor_test = {
    ...
    "#ifdef __kernel_exec\n"
    "    results[6] = 1;\n" // By spec, we can only really evaluate that it is
                            // defined, not what it expands to
    "#else\n"
    "    results[6] = 0xf00baa;\n"
    "#endif\n"

    // String preprocessor macros. Technically, there are strings in OpenCL, but
    // not really.
    "#ifdef __FILE__\n"
    "    int i;\n"
...
int test_kernel_preprocessor_macros(cl_device_id deviceID, cl_context context,
                                    cl_command_queue queue, int num_elements)
{
    ...
    error = clEnqueueReadBuffer(queue, streams[0], CL_TRUE, 0, sizeof(results),
                                results, 0, NULL, NULL);
    test_error(error, "Unable to get result data");
    error = clEnqueueReadBuffer(queue, streams[1], CL_TRUE, 0,
                                sizeof(fileString), fileString, 0, NULL, NULL);
    test_error(error, "Unable to get result data");
    error =
    clEnqueueReadBuffer(queue, streams[2], CL_TRUE, 0, sizeof(roundingString),
                        roundingString, 0, NULL, NULL);
    test_error(error, "Unable to get result data");
    ...
    if (checkForImageSupport(deviceID) == 0)
    {
        ...
        {
            log_error("ERROR: __IMAGE_SUPPORT__ undefined even though images "
                      "are supported\n");
            return -1;
        }
        else if (results[0] != 1)
        {
            log_error("ERROR: __IMAGE_SUPPORT__ defined, but to the wrong "
    ...
    else
    {
        ...
        if (results[0] != 0xf00baa)
        {
            log_error("ERROR: __IMAGE_SUPPORT__ defined to value %d even "
    ...
    // __ENDIAN_LITTLE__ is similar to __IMAGE_SUPPORT__: 1 if it's true,
    // undefined if it isn't
    cl_bool deviceIsLittleEndian;
    error =
    clGetDeviceInfo(deviceID, CL_DEVICE_ENDIAN_LITTLE,
                    sizeof(deviceIsLittleEndian), &deviceIsLittleEndian, NULL);
    test_error(error,
               "Unable to get endian property of device to validate against");

    if (deviceIsLittleEndian)
    {
        if (results[1] == 0xf00baa)
        {
            log_error("ERROR: __ENDIAN_LITTLE__ undefined even though the "
                      "device is little endian\n");
            return -1;
        }
        else if (results[1] != 1)
        {
            log_error("ERROR: __ENDIAN_LITTLE__ defined, but to the wrong "
                      "value (defined as %d, spec states it should be 1)\n",
                      (int)results[1]);
            return -1;
        }
    ...
    else
    {
        ...
        {
            log_error(
            "ERROR: __ENDIAN_LITTLE__ defined to value %d even though the "
            "device is not little endian (should be undefined per spec)",
            (int)results[1]);
            return -1;
        }
    }
    ...
    // The OpenCL version reported by the macro reports the feature level
    // supported by the compiler. Since this doesn't directly match any property
    // we can query, we just check to see if it's a sane value
    char versionBuffer[128];
    error = clGetDeviceInfo(deviceID, CL_DEVICE_VERSION, sizeof(versionBuffer),
                            versionBuffer, NULL);
    ...
    if (p2 == p3)
    {
        log_error("ERROR: Unable to verify OpenCL version string (platform "
    ...
    *p3 = 0;
    ...
    {
        log_error(
        "ERROR: Kernel preprocessor __OPENCL_C_VERSION__ undefined!\n");
        return -1;
    }

    // The OpenCL C version reported by the macro reports the OpenCL C supported
    // by the compiler for this OpenCL device.
    char cVersionBuffer[128];
    error = clGetDeviceInfo(deviceID, CL_DEVICE_OPENCL_C_VERSION,
                            sizeof(cVersionBuffer), cVersionBuffer, NULL);
    test_error(error,
               "Unable to get device's OpenCL C version to validate against");
    ...
    if (p2 == p3)
    {
        log_error("ERROR: Unable to verify OpenCL C version string (platform "
                  "string is incorrect format)\n");
        return -1;
    }
    *p2 = 0;
    *p3 = 0;
    ...
    {
        log_error("ERROR: Kernel preprocessor __OPENCL_C_VERSION__ does not "
                  "make sense w.r.t. device's version string! "
                  "(preprocessor states %d, real version is %d (%d.%d))\n",
                  results[2], realVersion, major, minor);
        return -1;
    }
    ...
    // This is fun--we get to search for where __LINE__ actually is so we know
    // what line it should define to! Note: it shows up twice, once for the
    // #ifdef, and the other for the actual result output
    ...
    int retCount = 1;
    for (; (retPtr < linePtr) && (retPtr != NULL);
         retPtr = strchr(retPtr + 1, '\n'))
        retCount++;

    if (retCount != results[4])
    {
        log_error("ERROR: Kernel preprocessor __LINE__ does not expand to the "
                  "actual line number! (expanded to %d, but was on line %d)\n",
                  results[4], retCount);
        return -1;
    }
    ...
    // Since we provided the program directly, __FILE__ should compile to
    // "<program source>".
    if (fileString[0] == 0)
    {
