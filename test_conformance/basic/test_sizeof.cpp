...
cl_int get_type_size(cl_context context, cl_command_queue queue,
                     const char *type, cl_ulong *size, cl_device_id device)
{
    ...
    context, &p, &k, 4, sizeof_kernel_code, "test_sizeof",
    deviceLt20 ? "" : "-cl-std=CL2.0");
    if (err) return err;

    m = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR,
                       sizeof(cl_ulong), size, &err);
    if (NULL == m)
    {
        clReleaseProgram(p);
        clReleaseKernel(k);
    ...
    err = clEnqueueReadBuffer(queue, m, CL_TRUE, 0, sizeof(cl_uint), &temp, 0,
                              NULL, NULL);
    clReleaseMemObject(m);
...
const size_table vector_table[] = {
    ...
    { "float", 4, sizeof(cl_float) }, { "long", 8, sizeof(cl_long) },
    { "ulong", 8, sizeof(cl_ulong) }
};

const char *ptr_table[] = { "global void*", "size_t",
                            "sizeof(int)", // check return type of sizeof
                            "ptrdiff_t" };

const char *other_types[] = { "event_t", "image2d_t", "image3d_t",
                              "sampler_t" };

static int IsPowerOfTwo(cl_ulong x) { return 0 == (x & (x - 1)); }

int test_sizeof(cl_device_id device, cl_context context, cl_command_queue queue,
                int num_elements)
{
    size_t i, j;
    cl_ulong test;
    ...
    err = clGetDeviceInfo(device, CL_DEVICE_ADDRESS_BITS, sizeof(ptr_size),
                          &ptr_size, NULL);
    if (err || ptr_size > 64)
    {
        log_error(
        "FAILED:  Unable to get CL_DEVICE_ADDRESS_BITS for device %p\n",
        device);
        return -1;
    }
    ...
    for (i = 0; i < sizeof(scalar_table) / sizeof(scalar_table[0]); i++)
    {
        ...
        test = CL_ULONG_MAX;
        err =
        get_type_size(context, queue, scalar_table[i].name, &test, device);
        if (err) return err;
    ...
    for (i = 0; i < sizeof(other_types) / sizeof(other_types[0]); i++)
    {
        ...
        if (gIsEmbedded && 0 == strcmp(other_types[i], "image3d_t")
        ...
        if (!IsPowerOfTwo(test))
        {
            log_error("\nFAILED: Type %s has size %lld, which is not a power "
                      "of two (section 6.1.5)!\n",
                      other_types[i], test);
            return -1;
        }
