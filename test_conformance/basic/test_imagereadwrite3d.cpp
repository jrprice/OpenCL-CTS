...
static unsigned char *generate_rgba8_image(int w, int h, int d, MTdata mtData)
{
...
static void update_rgba8_image(unsigned char *p, int x, int y, int z, int w,
                               int h, int d, int img_width, int img_height,
                               int img_depth, MTdata mtData)
{
    int i, j, k, indx;
...
static void update_image_from_image(void *out, void *in, int x, int y, int z,
                                    int w, int h, int d, int img_width,
                                    int img_height, int img_depth,
                                    int elem_size)
{
...
static int verify_rgba8_image(unsigned char *image, unsigned char *outptr,
                              int w, int h, int d)
{
    int i;
    ...
    for (i = 0; i < w * h * d * 4; i++)
    {
        if (outptr[i] != image[i])
        {
            log_error("i = %d. Expected (%d %d %d %d), got (%d %d %d %d)\n", i,
                      ...
                      outptr[i], outptr[i + 1], outptr[i + 2], outptr[i + 3]);
            return -1;
        }
    }
...
static unsigned short *generate_rgba16_image(int w, int h, int d, MTdata mtData)
{
    unsigned short *ptr =
    (unsigned short *)malloc(w * h * d * 4 * sizeof(unsigned short));
    int i;
...
static void update_rgba16_image(unsigned short *p, int x, int y, int z, int w,
                                int h, int d, int img_width, int img_height,
                                int img_depth, MTdata mtData)
{
    int i, j, k, indx;
...
static int verify_rgba16_image(unsigned short *image, unsigned short *outptr,
                               int w, int h, int d)
{
    int i;
...
static float *generate_rgbafp_image(int w, int h, int d, MTdata mtData)
{
...
static void update_rgbafp_image(float *p, int x, int y, int z, int w, int h,
                                int d, int img_width, int img_height,
                                int img_depth, MTdata mtData)
{
    int i, j, k, indx;
    int img_slice = img_width * img_height;
...
static int verify_rgbafp_image(float *image, float *outptr, int w, int h, int d)
{
    int i;
...
int test_imagereadwrite3d(cl_device_id device, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    ...
    rgba8_inptr = (unsigned char *)generate_rgba8_image(img_width, img_height,
                                                        img_depth, mtData);
    rgba16_inptr = (unsigned short *)generate_rgba16_image(
    img_width, img_height, img_depth, mtData);
    rgbafp_inptr =
    (float *)generate_rgbafp_image(img_width, img_height, img_depth, mtData);

    rgba8_outptr = (unsigned char *)malloc(sizeof(unsigned char) * 4 * img_width
                                           * img_height * img_depth);
    rgba16_outptr = (unsigned short *)malloc(
    sizeof(unsigned short) * 4 * img_width * img_height * img_depth);
    rgbafp_outptr =
    (float *)malloc(sizeof(float) * 4 * img_width * img_height * img_depth);

    img_format.image_channel_order = CL_RGBA;
    img_format.image_channel_data_type = CL_UNORM_INT8;
    streams[0] =
    create_image_3d(context, CL_MEM_READ_ONLY, &img_format, img_width,
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        err = clEnqueueWriteImage(queue, streams[i], CL_TRUE, origin, region, 0,
                                  0, p, 0, NULL, NULL);
    ...
    for (i = 0, j = 0; i < num_tries * 3; i++, j++)
    {
        ...
        // packed: the source image for the write is a whole image . unpacked:
        // the source image for the write is a subset within a larger image
        switch (j)
        {
            case 0:
                elem_size = 4;
                if (packed_update)
                {
                    ...
                    update_image_from_image(rgba8_inptr, p, x, y, z, w, h, d,
                                            img_width, img_height, img_depth,
                                            elem_size);
                }
                else
                {
                    update_rgba8_image(rgba8_inptr, x, y, z, w, h, d, img_width,
                                       img_height, img_depth, mtData);
                    p = (void *)(rgba8_inptr
            ...
            case 1:
                ...
                if (packed_update)
                {
                    ...
                    update_image_from_image(rgba16_inptr, p, x, y, z, w, h, d,
                                            img_width, img_height, img_depth,
                                            elem_size);
                }
                else
                {
                    update_rgba16_image(rgba16_inptr, x, y, z, w, h, d,
                                        img_width, img_height, img_depth,
                                        mtData);
                    p = (void *)(rgba16_inptr
                                 + ((z * img_slice + y * img_width + x) * 4));
            ...
            case 2:
                ...
                if (packed_update)
                {
                    ...
                    update_image_from_image(rgbafp_inptr, p, x, y, z, w, h, d,
                                            img_width, img_height, img_depth,
                                            elem_size);
                }
                else
                {
                    update_rgbafp_image(rgbafp_inptr, x, y, z, w, h, d,
                                        img_width, img_height, img_depth,
                                        mtData);
                    p = (void *)(rgbafp_inptr
                                 + ((z * img_slice + y * img_width + x) * 4));
        ...
        if (packed_update)
        {
            if (set_input_pitch)
            {
                // for packed updates the pitch does not need to be calculated
                // here (but can be)
        ...
        err =
        clEnqueueWriteImage(queue, streams[j], CL_TRUE, origin, region,
                            input_pitch, input_slice_pitch, p, 0, NULL, NULL);
        test_error(err, "clEnqueueWriteImage failed");
        ...
        region[1] = img_height;
        region[2] = img_depth;
        err = clEnqueueReadImage(queue, streams[j], CL_TRUE, origin, region, 0,
                                 0, outp, 0, NULL, NULL);
        ...
        {
            case 0:
                err = verify_rgba8_image(rgba8_inptr, rgba8_outptr, img_width,
                                         img_height, img_depth);
                if (err)
                {
                    log_error("x=%d y=%d z=%d w=%d h=%d d=%d pitch=%d, "
                              "slice_pitch=%d, try=%d\n",
                ...
                break;
            case 1:
                err = verify_rgba16_image(rgba16_inptr, rgba16_outptr,
                                          img_width, img_height, img_depth);
                if (err)
                {
                    log_error("x=%d y=%d z=%d w=%d h=%d d=%d pitch=%d, "
                ...
                break;
            case 2:
                err = verify_rgbafp_image(rgbafp_inptr, rgbafp_outptr,
                                          img_width, img_height, img_depth);
                if (err)
                {
                    log_error("x=%d y=%d z=%d w=%d h=%d d=%d pitch=%d, "
    ...
    if (!err) log_info("IMAGE read, write test passed\n");
