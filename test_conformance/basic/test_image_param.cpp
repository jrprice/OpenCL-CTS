...
int validate_results(size_t width, size_t height, cl_image_format &format,
                     char *inputData, cl_float *actualResults)
{
    for (size_t i = 0; i < width * height; i++)
    {
        ...
        if (error1 > tolerance || error2 > tolerance || error3 > tolerance
            || error4 > tolerance)
        {
            log_error("ERROR: Sample %d did not validate against expected "
                      "results for %d x %d %s:%s image\n",
            ...
            log_error("      Actual: %f %f %f %f\n", (float)actualResults[0],
                      (float)actualResults[1], (float)actualResults[2],
                      (float)actualResults[3]);
            ...
            if (error1 <= tolerance && error2 <= tolerance
                && error3 <= tolerance && error4 <= tolerance)
            {
                log_error(
                "\t(Kernel did not respect change in channel order)\n");
            }
            return -1;
        }
...
int test_image_param(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    size_t sizes[] = { 64, 100, 128, 250, 512 };
    cl_image_format formats[] = { { CL_RGBA, CL_UNORM_INT8 },
    ...
    if (gIsEmbedded)
    {
        ...
        clGetSupportedImageFormats(context, CL_MEM_READ_WRITE,
                                   ...
                                   &numSupportedFormats);
        supported_formats = (cl_image_format *)malloc(sizeof(cl_image_format)
                                                      * numSupportedFormats);
        clGetSupportedImageFormats(context, CL_MEM_READ_WRITE,
    ...
    for (i = 0, idx = 0; i < numSizes; i++)
    {
        for (j = 0; j < numFormats; j++, idx++)
        {
            ...
            // For each attempt, we create a pair: an input image, whose
            // parameters keep changing, and an output buffer that we can read
            // values from. The output buffer will remain consistent to ensure
            // that any changes we witness are due to the image changes
            inputs[idx].reset(
            create_random_data(types[j], d, sizes[i] * sizes[i] * 4));

            streams[idx][0] =
            create_image_2d(context, CL_MEM_COPY_HOST_PTR, &formats[j],
                            sizes[i], sizes[i], 0, inputs[idx], &error);
            {
                char err_str[256];
                sprintf(err_str,
                        "Unable to create input image for format %s order %s",
            ...
            streams[idx][1] = clCreateBuffer(
            context, CL_MEM_READ_WRITE,
            sizes[i] * sizes[i] * 4 * sizeof(cl_float), NULL, &error);
    ...
    for (i = 0, idx = 0; i < numSizes; i++)
    {
        for (j = 0; j < numFormats; j++, idx++)
        {
            ...
            error = clSetKernelArg(kernel, 0, sizeof(streams[idx][0]),
                                   &streams[idx][0]);
            error |= clSetKernelArg(kernel, 1, sizeof(sampler), &sampler);
            error |= clSetKernelArg(kernel, 2, sizeof(streams[idx][1]),
                                    &streams[idx][1]);
    ...
    for (i = 0, idx = 0; i < numSizes; i++)
    {
        for (j = 0; j < numFormats; j++, idx++)
        {
            ...
            BufferOwningPtr<cl_float> output(
            malloc(sizeof(cl_float) * sizes[i] * sizes[i] * 4));

            error = clEnqueueReadBuffer(
            queue, streams[idx][1], CL_TRUE, 0,
            sizes[i] * sizes[i] * 4 * sizeof(cl_float), output, 0, NULL, NULL);
            test_error(error, "Unable to read results");

            error = validate_results(sizes[i], sizes[i], formats[j],
                                     inputs[idx], output);
            if (error) return -1;
        }
    }
