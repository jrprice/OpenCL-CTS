...
int test_constant_source(cl_device_id device, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    ...
    cl_int outValues[17];
    cl_int expectedValues[17] = { 42,  1985, 01, 11,   12, 1955, 11,   5, 1985,
                                  113, 1,    24, 1984, 7,  23,   1979, 97 };
    ...
    create_single_kernel_helper(context, &program, &kernel, 1,
                                constant_source_kernel_code, "constant_kernel");
    test_error(error, "Unable to create testing kernel");

    // Create our output buffer
    outStream =
    clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(outValues), NULL, &error);
    ...
    // Read results
    error = clEnqueueReadBuffer(queue, outStream, CL_TRUE, 0, sizeof(outValues),
                                outValues, 0, NULL, NULL);
    ...
    for (int i = 0; i < 17; i++)
    {
        ...
        {
            if (i == 0)
                log_error("ERROR: Output value %d from constant source global "
                          "did not validate! (Expected %d, got %d)\n",
                          i, expectedValues[i], outValues[i]);
            else if (i == 1)
                log_error(
                "ERROR: Output value %d from constant-indexed constant array "
                "did not validate! (Expected %d, got %d)\n",
                i, expectedValues[i], outValues[i]);
            else
                log_error(
                "ERROR: Output value %d from variable-indexed constant array "
                "did not validate! (Expected %d, got %d)\n",
                i, expectedValues[i], outValues[i]);
            return -1;
        }
