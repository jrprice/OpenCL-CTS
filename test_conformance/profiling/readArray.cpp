...
static const char *int_kernel_name[] = {
    "test_stream_read_int", "test_stream_read_int2", "test_stream_read_int4",
    "test_stream_read_int8", "test_stream_read_int16"
};
...
const char *uint_kernel_name[] = {
    "test_stream_read_uint", "test_stream_read_uint2", "test_stream_read_uint4",
    "test_stream_read_uint8", "test_stream_read_uint16"
};
...
const char *long_kernel_name[] = {
    "test_stream_read_long", "test_stream_read_long2", "test_stream_read_long4",
    "test_stream_read_long8", "test_stream_read_long16"
};
...
const char *ulong_kernel_name[] = { "test_stream_read_ulong",
                                    "test_stream_read_ulong2",
                                    "test_stream_read_ulong4",
                                    "test_stream_read_ulong8",
                                    "test_stream_read_ulong16" };
...
const char *short_kernel_name[] = { "test_stream_read_short",
                                    "test_stream_read_short2",
                                    "test_stream_read_short4",
                                    "test_stream_read_short8",
                                    "test_stream_read_short16" };
...
static const char *ushort_kernel_name[] = { "test_stream_read_ushort",
                                            "test_stream_read_ushort2",
                                            "test_stream_read_ushort4",
                                            "test_stream_read_ushort8",
                                            "test_stream_read_ushort16" };
...
const char *float_kernel_name[] = { "test_stream_read_float",
                                    "test_stream_read_float2",
                                    "test_stream_read_float4",
                                    "test_stream_read_float8",
                                    "test_stream_read_float16" };
...
const char *half_kernel_name[] = {
    "test_stream_read_half", "test_stream_read_half2", "test_stream_read_half4",
    "test_stream_read_half8", "test_stream_read_half16"
};
...
const char *char_kernel_name[] = {
    "test_stream_read_char", "test_stream_read_char2", "test_stream_read_char4",
    "test_stream_read_char8", "test_stream_read_char16"
};
...
const char *uchar_kernel_name[] = { "test_stream_read_uchar",
                                    "test_stream_read_uchar2",
                                    "test_stream_read_uchar4",
                                    "test_stream_read_uchar8",
                                    "test_stream_read_uchar16" };
...
static int verify_read_int(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != TEST_PRIME_INT) return -1;
    }
...
static int verify_read_uint(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != TEST_PRIME_UINT) return -1;
    }
...
static int verify_read_long(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != TEST_PRIME_LONG) return -1;
    }
...
static int verify_read_ulong(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != TEST_PRIME_ULONG) return -1;
    }
...
static int verify_read_short(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != (short)((1 << 8) + 1)) return -1;
    }
...
static int verify_read_ushort(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != (cl_ushort)((1 << 8) + 1)) return -1;
    }
...
static int verify_read_float(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != TEST_PRIME_FLOAT) return -1;
    }
...
static int verify_read_half(void *ptr, int n)
{
    ...
    for (i = 0; i < n / 2; i++)
    {
        if (outptr[i] != TEST_PRIME_HALF) return -1;
    }
...
static int verify_read_char(void *ptr, int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (outptr[i] != TEST_PRIME_CHAR) return -1;
    }
...
//----- the test functions
int test_stream_read(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements, size_t size,
                     const char *type, int loops, const char *kernelCode[],
                     const char *kernelName[], int (*fn)(void *, int))
{
    cl_mem streams[5];
    void *outptr[5];
    ...
    for (i = 0; i < loops; i++)
    {
        ...
        streams[i] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
        ...
        err = clEnqueueReadBuffer(queue, streams[i], false, 0,
        ...
        err = clGetEventProfilingInfo(readEvent, CL_PROFILING_COMMAND_QUEUED,
        ...
        err = clGetEventProfilingInfo(readEvent, CL_PROFILING_COMMAND_SUBMIT,
        ...
        err = clGetEventProfilingInfo(readEvent, CL_PROFILING_COMMAND_START,
                                      sizeof(cl_ulong), &readStart, NULL);
        if (err != CL_SUCCESS)
        {
            print_error(err, "clGetEventProfilingInfo failed");
            clReleaseKernel(kernel[i]);
        ...
        err = clGetEventProfilingInfo(readEvent, CL_PROFILING_COMMAND_END,
                                      sizeof(cl_ulong), &readEnd, NULL);
        if (err != CL_SUCCESS)
        {
            print_error(err, "clGetEventProfilingInfo failed");
            clReleaseKernel(kernel[i]);
...
int test_read_array_int(cl_device_id device, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
    foo = verify_read_int;

    return test_stream_read(device, context, queue, num_elements,
                            sizeof(cl_int), "int", 5,
                            stream_read_int_kernel_code, int_kernel_name, foo);
}


int test_read_array_uint(cl_device_id device, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
    foo = verify_read_uint;

    return test_stream_read(
    device, context, queue, num_elements, sizeof(cl_uint), "uint", 5,
    stream_read_uint_kernel_code, uint_kernel_name, foo);
}


int test_read_array_long(cl_device_id device, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
    ...
    return test_stream_read(
    device, context, queue, num_elements, sizeof(cl_long), "long", 5,
    stream_read_long_kernel_code, long_kernel_name, foo);
}


int test_read_array_ulong(cl_device_id device, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
    ...
    return test_stream_read(
    device, context, queue, num_elements, sizeof(cl_ulong), "ulong", 5,
    stream_read_ulong_kernel_code, ulong_kernel_name, foo);
}


int test_read_array_short(cl_device_id device, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
...
int test_read_array_ushort(cl_device_id device, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
    ...
    return test_stream_read(
    device, context, queue, num_elements, sizeof(cl_ushort), "ushort", 5,
    stream_read_ushort_kernel_code, ushort_kernel_name, foo);
}


int test_read_array_float(cl_device_id device, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
...
int test_read_array_half(cl_device_id device, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
...
int test_read_array_char(cl_device_id device, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
...
int test_read_array_uchar(cl_device_id device, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
...
int test_read_array_struct(cl_device_id device, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    int (*foo)(void *, int);
    ...
    return test_stream_read(
    device, context, queue, num_elements, sizeof(TestStruct), "struct", 1,
    stream_read_struct_kernel_code, struct_kernel_name, foo);
}

/*
int read_struct_array(cl_device_group device, cl_device id, cl_context context,
int num_elements)
{
    cl_mem            streams[1];
...
#ifdef USE_LOCAL_THREADS
    err = clGetDeviceConfigInfo( id, CL_DEVICE_MAX_THREAD_GROUP_SIZE,
localThreads, sizeof( cl_uint ), NULL ); if( err != CL_SUCCESS ){ log_error(
"Unable to get thread group max size: %d", err ); return -1;
    }
...
(int)(objSize * num_elements) ); return -1;
    }
    streams[0] = clCreateBuffer( device, (cl_mem_flags)(CL_MEM_READ_WRITE),
objSize * num_elements, NULL ); if( !streams[0] ){ log_error( " clCreateBuffer
failed\n" ); free( output_ptr ); return -1;
    }

    err = create_program_and_kernel( device, stream_read_struct_kernel_code,
"test_stream_read_struct", &program[0], &kernel[0]); if( err ){
...
#ifdef USE_LOCAL_THREADS
    err = clEnqueueNDRangeKernel( queue, kernel[0], 1, NULL, threads,
localThreads, 0, NULL, NULL ); #else err = clEnqueueNDRangeKernel( queue,
kernel[0], 1, NULL, threads, NULL, 0, NULL, NULL ); #endif if( err != CL_SUCCESS
){ print_error( err, "clEnqueueNDRangeKernel failed" ); clReleaseProgram(
program[0] ); clReleaseKernel( kernel[0] ); clReleaseMemObject( streams[0] );
        free( output_ptr );
        return -1;
    }

    err = clEnqueueReadBuffer( queue, streams[0], true, 0, objSize*num_elements,
(void *)output_ptr, 0, NULL, NULL ); if( err != CL_SUCCESS){ print_error( err,
"clEnqueueReadBuffer failed" ); clReleaseProgram( program[0] ); clReleaseKernel(
kernel[0] ); clReleaseMemObject( streams[0] ); free( output_ptr ); return -1;
    }
