...
static const char *readKernelCode[] = {
    "__kernel void testReadf(read_only image2d_t srcimg, __global float4 "
    "*dst)\n"
    "{\n"
    ...
    "__kernel void testReadi(read_only image2d_t srcimg, __global uchar4 "
    "*dst)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testReadui(read_only image2d_t srcimg, __global uchar4 "
    "*dst)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testWritef(__global uchar *src, write_only image2d_t "
    "dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testWritei(__global char *src, write_only image2d_t "
    "dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testWriteui(__global uchar *src, write_only image2d_t "
    "dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testReadWriteff(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    "    float4 color;\n"
    "\n"
    "    color = read_imagef(srcimg, CLK_DEFAULT_SAMPLER, (int2)(tid_x, "
    "tid_y));\n"
    "    write_imagef(dstimg, (int2)(tid_x, tid_y), color);\n"
    "\n"
    "}\n",

    "__kernel void testReadWriteii(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    "    int4    color;\n"
    "\n"
    "    color = read_imagei(srcimg, CLK_DEFAULT_SAMPLER, (int2)(tid_x, "
    "tid_y));\n"
    "    write_imagei(dstimg, (int2)(tid_x, tid_y), color);\n"
    "\n"
    "}\n",

    "__kernel void testReadWriteuiui(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    "    uint4    color;\n"
    "\n"
    "    color = read_imageui(srcimg, CLK_DEFAULT_SAMPLER, (int2)(tid_x, "
    "tid_y));\n"
    "    write_imageui(dstimg, (int2)(tid_x, tid_y), color);\n"
    "\n"
    "}\n",

    "__kernel void testReadWritefi(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testReadWritefui(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testReadWriteif(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "    float4    colorf;\n"
    "\n"
    // since we are going from signed to unsigned, we need to adjust the rgba
    // values from from the signed image to add 256 to the signed image values
    // less than 0.
    "    const sampler_t sampler = CLK_ADDRESS_CLAMP_TO_EDGE | "
    "CLK_FILTER_NEAREST | CLK_NORMALIZED_COORDS_FALSE;\n"
    ...
    "__kernel void testReadWriteiui(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testReadWriteuif(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
    ...
    "__kernel void testReadWriteuii(read_only image2d_t srcimg, write_only "
    "image2d_t dstimg)\n"
    "{\n"
    "    int    tid_x = get_global_id(0);\n"
    "    int    tid_y = get_global_id(1);\n"
...
static const char *readKernelName[] = {
    "testReadf",        "testReadi",        "testReadui",
    "testWritef",       "testWritei",       "testWriteui",
    "testReadWriteff",  "testReadWriteii",  "testReadWriteuiui",
    "testReadWritefi",  "testReadWritefui", "testReadWriteif",
    "testReadWriteiui", "testReadWriteuif", "testReadWriteuii"
};
...
static int verifyImage(cl_uchar *image, cl_uchar *outptr, int w, int h)
{
    ...
    for (i = 0; i < w * h * 4; i++)
    {
        if (outptr[i] != image[i])
        {
            log_error("Image verification failed at offset %d. Actual "
...
//----- the test functions
int write_image(cl_device_id device, cl_context context, cl_command_queue queue,
                int numElements, const char *code, const char *name,
                cl_image_format image_format_desc, int readFloat)
{
    cl_mem memobjs[2];
    ...
    if (readFloat) channel_nbytes = sizeof(cl_float);

    element_nbytes =
    channel_nbytes * get_format_channel_count(&image_format_desc);
    ...
    err = clGetDeviceConfigInfo(id, CL_DEVICE_MAX_THREAD_GROUP_SIZE,
                                localThreads, sizeof(unsigned int), NULL);
    ...
    if (localThreads[1] > threads[1]) localThreads[1] = threads[1];
#endif
    ...
    memobjs[0] =
    create_image_2d(context, flags, &image_format_desc, w, h, 0, NULL, &err);
    ...
    memobjs[1] = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_READ_WRITE),
    ...
    err = clEnqueueWriteImage(queue, memobjs[0], false, origin, region, 0, 0,
                              inptr, 0, NULL, &writeEvent);
    if (err != CL_SUCCESS)
    {
        clReleaseMemObject(memobjs[0]);
...
#ifdef USE_LOCAL_THREADS
    err = clEnqueueNDRangeKernel(queue, kernel[0], 2, NULL, threads,
                                 localThreads, 0, NULL, NULL);
#else
    err = clEnqueueNDRangeKernel(queue, kernel[0], 2, NULL, threads, NULL, 0,
                                 NULL, NULL);
#endif
    if (err != CL_SUCCESS)
    {
    ...
    err = clEnqueueReadBuffer(queue, memobjs[1], true, 0, num_bytes, dst, 0,
                              NULL, NULL);
    if (err != CL_SUCCESS)
    {
...
int test_write_image_float(cl_device_id device, cl_context context,
                           cl_command_queue queue, int numElements)
{
    cl_image_format image_format_desc = { CL_RGBA, CL_UNORM_INT8 };
    PASSIVE_REQUIRE_IMAGE_SUPPORT(device)
    // 0 to 255 for unsigned image data
    return write_image(device, context, queue, numElements, readKernelCode[0],
                       readKernelName[0], image_format_desc, 1);
}


int test_write_image_char(cl_device_id device, cl_context context,
                          cl_command_queue queue, int numElements)
{
    cl_image_format image_format_desc = { CL_RGBA, CL_SIGNED_INT8 };
    PASSIVE_REQUIRE_IMAGE_SUPPORT(device)
    // -128 to 127 for signed iamge data
    return write_image(device, context, queue, numElements, readKernelCode[1],
                       readKernelName[1], image_format_desc, 0);
}


int test_write_image_uchar(cl_device_id device, cl_context context,
                           cl_command_queue queue, int numElements)
{
    cl_image_format image_format_desc = { CL_RGBA, CL_UNSIGNED_INT8 };
    PASSIVE_REQUIRE_IMAGE_SUPPORT(device)
    // 0 to 255 for unsigned image data
    return write_image(device, context, queue, numElements, readKernelCode[2],
