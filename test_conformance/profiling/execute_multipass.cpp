...
"\n"
"__kernel void read3d(read_only image3d_t srcimg, __global unsigned char *dst, "
"sampler_t sampler)\n"
"{\n"
                      ...
                      int nChannels, cl_uchar *inptr, cl_uchar *outptr)
{
    ...
    err = clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_ITEM_SIZES,
                          sizeof(cl_uint), (size_t *)localThreads, NULL);
    if (err)
    {
        localThreads[0] = 256;
        localThreads[1] = 1;
    ...
    err = clEnqueueNDRangeKernel(queue, kernel[0], 3, NULL, threads,
    ...
    err = clEnqueueReadBuffer(queue, memobjs[1], CL_TRUE, 0,
                              w * h * d * nChannels * 4, outptr, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
...
int execute_multipass(cl_device_id device, cl_context context,
