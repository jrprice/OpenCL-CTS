...
static void read_imagef(int x, int y, int w, int h, int nChannels, uchar *src,
                        float *srcRgb)
{
    // clamp the coords
...
static void write_imagef(uchar *dst, int x, int y, int w, int h, int nChannels,
                         float *dstRgb)
{
    // get tine index
...
static void basicFilterPixel(int x, int y, int n, int m, int xsize, int ysize,
                             int nChannels, const float *filter_weights,
                             uchar *src, uchar *dst)
{
    int i, j, k;
    int indx = 0;
...
static int verifyImages(uchar *ptr0, uchar *ptr1, uchar tolerance, int xsize,
                        int ysize, int nChannels)
{
    int x, y, z;
    uchar *p0 = ptr0;
    uchar *p1 = ptr1;
    ...
    for (y = 0; y < ysize; y++)
    {
        for (x = 0; x < xsize; x++)
        {
            for (z = 0; z < nChannels; z++)
            {
                ...
                {
                    log_error(
                    "  images differ at x,y = %d,%d, channel = %d, %d to %d\n",
                    x, y, z, (int)p0[-1], (int)p1[-1]);
                    return -1;
                }
            }
        }
...
static int kernelFilter(cl_device_id device, cl_context context,
                        cl_command_queue queue, int w, int h, int nChannels,
                        uchar *inptr, uchar *outptr)
{
    ...
    memobjs[1] = create_image_2d(context, CL_MEM_WRITE_ONLY, &image_format_desc,
                                 w, h, 0, NULL, &err);
    ...
    err = clEnqueueNDRangeKernel(queue, kernel[0], 2, NULL, threads,
                                 localThreads, 0, NULL, &executeEvent);
#else
    ...
    // test profiling
    while (
    (err = clGetEventProfilingInfo(executeEvent, CL_PROFILING_COMMAND_QUEUED,
    ...
    while (
    (err = clGetEventProfilingInfo(executeEvent, CL_PROFILING_COMMAND_SUBMIT,
    ...
    err = clEnqueueReadImage(queue, memobjs[1], true, origin, region, 0, 0,
                             outptr, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
...
static int basicFilter(int w, int h, int nChannels, uchar *inptr, uchar *outptr)
{
    ...
    for (y = 0; y < h; y++)
    {
        for (x = 0; x < w; x++)
        {
            basicFilterPixel(x, y, filter_w, filter_h, w, h, nChannels,
                             filter_weights, inptr, outptr);
        }
    }
...
int test_execute(cl_device_id device, cl_context context,
                 cl_command_queue queue, int num_elements)
{
    ...
    {
        log_error(
        " unable to allocate %d bytes of memory for output image #2\n",
        nElements);
        free((void *)outptr[0]);
        free((void *)inptr);
        return -1;
    ...
    err =
    kernelFilter(device, context, queue, w, h, nChannels, inptr, outptr[0]);
