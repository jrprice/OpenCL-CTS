...
static const char *readKernelName[] = { "testWritef", "testWritei",
                                        "testWriteui" };
...
//----- the test functions
int read_image(cl_device_id device, cl_context context, cl_command_queue queue,
               ...
               cl_image_format image_format_desc)
{
    ...
    err = clEnqueueReadImage(queue, memobjs[0], false, origin, region, 0, 0,
                             dst, 0, NULL, &readEvent);
    if (err != CL_SUCCESS)
    {
...
int test_read_image_float(cl_device_id device, cl_context context,
                          cl_command_queue queue, int numElements)
{
    cl_image_format image_format_desc = { CL_RGBA, CL_UNORM_INT8 };
    PASSIVE_REQUIRE_IMAGE_SUPPORT(device)
    // 0 to 255 for unsigned image data
    return read_image(device, context, queue, numElements, readKernelCode[0],
                      readKernelName[0], image_format_desc);
}


int test_read_image_char(cl_device_id device, cl_context context,
                         cl_command_queue queue, int numElements)
{
    cl_image_format image_format_desc = { CL_RGBA, CL_SIGNED_INT8 };
    PASSIVE_REQUIRE_IMAGE_SUPPORT(device)
    // -128 to 127 for signed iamge data
    return read_image(device, context, queue, numElements, readKernelCode[1],
                      readKernelName[1], image_format_desc);
}


int test_read_image_uchar(cl_device_id device, cl_context context,
                          cl_command_queue queue, int numElements)
{
    cl_image_format image_format_desc = { CL_RGBA, CL_UNSIGNED_INT8 };
    PASSIVE_REQUIRE_IMAGE_SUPPORT(device)
    // 0 to 255 for unsigned image data
    return read_image(device, context, queue, numElements, readKernelCode[2],
