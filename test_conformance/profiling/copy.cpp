...
"\n"
"__kernel void test_write(__global unsigned char *src, write_only image2d_t "
"dstimg)\n"
"{\n"
...
static int verify_subimage(unsigned char *src, unsigned char *dst, size_t srcx,
                           size_t srcy, size_t dstx, size_t dsty, size_t subw,
                           size_t subh, size_t pitch, size_t element_pitch)
{
    size_t i, j, k;
    size_t srcj, dstj;
...
static cl_uchar *generate_image(int n, MTdata d)
{
    ...
    for (i = 0; i < n; i++) ptr[i] = (cl_uchar)genrand_int32(d);

    return ptr;
}


static int copy_size(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements, MTdata d)
{
    ...
    for (i = 0; i < num_elements; i++)
    {
        ...
        int_output_ptr[i] =
        (int)genrand_int32(d) >> 30; // seed with incorrect data
    }

    err = clEnqueueWriteBuffer(queue, streams[0], true, 0,
                               sizeof(cl_int) * num_elements,
                               (void *)int_input_ptr, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    err =
    clEnqueueCopyBuffer(queue, streams[0], streams[1], 0, 0,
                        sizeof(cl_int) * num_elements, 0, NULL, &copyEvent);
    if (err != CL_SUCCESS)
    {
    ...
    // test profiling
    while (
    (err = clGetEventProfilingInfo(copyEvent, CL_PROFILING_COMMAND_QUEUED,
    ...
    while (
    (err = clGetEventProfilingInfo(copyEvent, CL_PROFILING_COMMAND_SUBMIT,
    ...
    err = clGetEventProfilingInfo(copyEvent, CL_PROFILING_COMMAND_START,
    ...
    err = clGetEventProfilingInfo(copyEvent, CL_PROFILING_COMMAND_END,
    ...
    err = clEnqueueReadBuffer(queue, streams[1], true, 0,
                              sizeof(cl_int) * num_elements,
                              (void *)int_output_ptr, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
...
static int copy_partial_size(cl_device_id device, cl_context context,
                             ...
                             MTdata d)
{
    ...
    for (i = 0; i < num_elements; i++)
    {
        ...
        outptr[i] =
        (int)get_random_float(-1.f, 1.f, d); // seed with incorrect data
    }
    ...
    queue, streams[0], streams[1], srcStart * sizeof(cl_int),
    dstStart * sizeof(cl_int), sizeof(cl_int) * size, 0, NULL, &copyEvent);
    if (err != CL_SUCCESS)
    {
        print_error(err, "clCopyArray failed");
        clReleaseMemObject(streams[0]);
...
int test_copy_array(cl_device_id device, cl_context context,
                    cl_command_queue queue, int num_elements)
{
    int i, err = 0;
    int size;
...
int test_copy_partial_array(cl_device_id device, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    int i, err = 0;
    int size;
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        dstStart =
        (cl_uint)get_random_float(0.f, (float)(num_elements - size), d);
        log_info("random partial copy from %d to %d, size: %d: ", (int)srcStart,
                 (int)dstStart, size);
        err |= copy_partial_size(device, context, queue, num_elements, srcStart,
                                 dstStart, size, d);
    }
                           ...
                           MTdata d)
{
    ...
    err = clEnqueueWriteBuffer(queue, memobjs[1], true, 0, num_bytes, inptr, 0,
                               NULL, NULL);
    if (err != CL_SUCCESS)
    {
        log_error("clWriteArray failed\n");
    ...
    err = clEnqueueCopyImage(queue, memobjs[0], memobjs[2], srcPt, destPt,
                             region, 0, NULL, &copyEvent);
    if (err != CL_SUCCESS)
    {
    ...
    err = clEnqueueReadImage(queue, memobjs[2], true, origin, region2, 0, 0,
                             dst, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    err = verify_subimage((unsigned char *)inptr, (unsigned char *)dst, srcx,
                          srcy, dstx, dsty, subw, subh, w, 4);
    // err = verify_image( (unsigned char *)inptr, (unsigned char *)dst, w * h *
    // 4 );
    if (err)
    {
                    ...
                    cl_command_queue queue, int num_elements)
{
    ...
    err = copy_image_size(device, context, queue, srcx, srcy, dstx, dsty, subw,
                          subh, d);
    if (err)
    {
    ...
    if (err)
    {
        log_error("test copy of subimage size %d,%d  %d,%d  %d x %d\n",
                  (int)srcx, (int)srcy, (int)dstx, (int)dsty, (int)subw,
                  (int)subh);
    }
    else
    {
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        err = copy_image_size(device, context, queue, srcx, srcy, dstx, dsty,
                              subw, subh, d);
        if (err)
        {
            log_error("test copy of subimage size %d,%d  %d,%d  %d x %d\n",
...
int test_copy_array_to_image(cl_device_id device, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    cl_mem memobjs[3];
    cl_image_format image_format_desc = { CL_RGBA, CL_UNORM_INT8 };
    ...
    (unsigned int)get_format_channel_count(&image_format_desc);
    size_t w = 256, h = 256;
    size_t element_nbytes;
    size_t num_bytes;
    size_t channel_nbytes = sizeof(cl_char);
    ...
    err = clEnqueueWriteBuffer(queue, memobjs[1], true, 0, num_bytes,
                               (const void *)inptr, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
        print_error(err, "clWriteArray failed");
        clReleaseMemObject(memobjs[1]);
    ...
    err = clEnqueueCopyBufferToImage(queue, memobjs[1], memobjs[0], 0, origin,
                                     region, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    err = clEnqueueReadImage(queue, memobjs[0], true, origin, region, 0, 0, dst,
                             0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
        print_error(err, "clReadImage failed");
    ...
    err = verify_subimage((cl_uchar *)inptr, (cl_uchar *)dst, 0, 0, 0, 0, w, h,
                          w, num_channels);
    if (err)
    {
        log_error(" test failed: ");
