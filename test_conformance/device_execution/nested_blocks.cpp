...
static const char* enqueue_nested_blocks_single[] = {
    NL,
    "void block_fn(__global int* res, int level)" NL,
    "{" NL,
    "  size_t tid = get_global_id(0);" NL,
    "  queue_t def_q = get_default_queue();" NL,
    "  ndrange_t ndrange = ndrange_1D(3);" NL,
    "  if(--level < 0) return;" NL,
    "" NL,
    "  void (^kernelBlock)(void) = ^{ block_fn(res, level); };" NL,
    "" NL,
    ...
    "}" NL,
    "" NL,
    "kernel void enqueue_nested_blocks_single(__global int* res, int level)" NL,
    "{" NL,
    "  block_fn(res, level);" NL,
    "}" NL
};

static const char* enqueue_nested_blocks_some_eq[] = {
    NL,
    "void block_fn(int level, __global int* res)" NL,
    "{" NL,
    "  size_t tid = get_global_id(0);" NL,
    "  queue_t def_q = get_default_queue();" NL,
    "  ndrange_t ndrange = ndrange_1D(10);" NL,
    "  if(--level < 0) return;" NL,
    "" NL,
    "  void (^kernelBlock)(void) = ^{ block_fn(level, res); };" NL,
    "" NL,
    "  // Some work-items enqueues nested blocks with the same level" NL,
    "  if(tid < (get_global_size(0) >> 1))" NL,
    "  {" NL,
    "    atomic_inc(&res[tid]);" NL,
    "    int enq_res = enqueue_kernel(def_q, CLK_ENQUEUE_FLAGS_WAIT_KERNEL, "
    "ndrange, kernelBlock);" NL,
    "    if(enq_res != CLK_SUCCESS) { res[tid] = -1; return; }" NL,
    "  }" NL,
    "}" NL,
    "" NL,
    "kernel void enqueue_nested_blocks_some_eq(__global int* res, int "
    "level)" NL,
    "{" NL,
    "  block_fn(level, res);" NL,
    "}" NL
};
...
static const char* enqueue_nested_blocks_some_diff[] = {
    ...
    "}" NL,
    "" NL,
    "kernel void enqueue_nested_blocks_some_diff(__global int* res, int "
    "level)" NL,
    "{" NL,
    "  block_fn(level, res);" NL,
    "}" NL
...
static const char* enqueue_nested_blocks_all_eq[] = {
    ...
    "  size_t tid = get_global_id(0);" NL,
    "  queue_t def_q = get_default_queue();" NL,
    "  ndrange_t ndrange = ndrange_1D(4);" NL,
    "  if(--level < 0) return;" NL,
    "" NL,
    "  void (^kernelBlock)(void) = ^{ block_fn(level, res); };" NL,
    "" NL,
    ...
    "}" NL,
    "" NL,
    "kernel void enqueue_nested_blocks_all_eq(__global int* res, int level)" NL,
    "{" NL,
    "  block_fn(level, res);" NL,
    "}" NL
};
...
static const char* enqueue_nested_blocks_all_diff[] = {
    ...
    "}" NL,
    "" NL,
    "kernel void enqueue_nested_blocks_all_diff(__global int* res, int "
    "level)" NL,
    "{" NL,
    "  block_fn(level, res);" NL,
    "}" NL
...
static int check_single(cl_int* results, cl_int len, cl_int nesting_level)
{
    ...
    for (i = 0; i < len; ++i)
    {
        if (i != tid && results[i] != 0)
        {
            fail = i;
            break;
        }
        if (i == tid && results[i] != nesting_level)
        {
            fail = i;
            break;
        }
...
void generate_reference_some_eq(std::vector<cl_int>& referenceResults,
                                cl_int len, cl_int nesting_level)
{
...
void generate_reference_some_diff(std::vector<cl_int>& referenceResults,
                                  cl_int len, cl_int nesting_level)
{
    size_t globalWorkSize = (nesting_level == gNestingLevel) ? len : 10;
    if (--nesting_level < 0) return;
    ...
    for (size_t tid = 0; tid < globalWorkSize; ++tid)
    {
        if (tid % 2)
        {
            ...
            {
                generate_reference_some_diff(referenceResults, len,
                                             nesting_level);
            }
        }
    }
...
void generate_reference_all_eq(std::vector<cl_int>& referenceResults,
                               cl_int len, cl_int nesting_level)
{
...
void generate_reference_all_diff(std::vector<cl_int>& referenceResults,
                                 cl_int len, cl_int nesting_level)
{
    size_t globalWorkSize = (nesting_level == gNestingLevel) ? len : 10;
    if (--nesting_level < 0) return;
...
int test_enqueue_nested_blocks(cl_device_id device, cl_context context,
                               cl_command_queue queue, int num_elements)
{
    ...
    kernel_arg args[] = { { sizeof(cl_int), &gNestingLevel } };

    size_t failCnt = 0;
    for (k = 0; k < arr_size(sources_nested_blocks); ++k)
    {
        if (!gKernelName.empty()
            && gKernelName != sources_nested_blocks[k].src.kernel_name)
            continue;
        ...
        // check results
        int fail = sources_nested_blocks[k].check(
        kernel_results, MAX_GLOBAL_WORK_SIZE, gNestingLevel);
                        ...
                        sources_nested_blocks[k].src.kernel_name))
        {
            ...
            continue;
                 ...
                 && check_error(-1,
                                "'%s' kernel results validation failed: [%d] "
                                "returned %d expected 0",
                                sources_nested_blocks[k].src.kernel_name, fail,
                                kernel_results[fail]))
        {
            ++failCnt;
            res = -1;
