...
static const char* enqueue_block_first_kernel[] = {
    NL,
    "void block_fn(uint num, __global int* res)" NL,
    "{" NL,
    "    size_t tid = get_global_id(0);" NL,
    "" NL,
    "    for(int i = 1 ; i < tid ; i++)" NL,
    "    {" NL,
    "      for(int j = 0 ; j < num ; j++)" NL,
    "        atomic_add(res+tid, 1);" NL,
    "    }" NL,
    "}" NL,
    "" NL,
    "kernel void enqueue_block_first_kernel(uint num, __global int* res)" NL,
    "{" NL,
    "  void (^kernelBlock)(void) = ^{ block_fn(num, res); };" NL,
    "" NL,
    "  ndrange_t ndrange = ndrange_1D(num, 1);" NL,
    "" NL,
    "  int enq_res = enqueue_kernel(get_default_queue(), "
    "CLK_ENQUEUE_FLAGS_NO_WAIT, ndrange, kernelBlock);" NL,
    "  if(enq_res != CLK_SUCCESS) { res[0] = -1; return; }" NL,
    "" NL,
    "}" NL
};
...
static const char* enqueue_block_second_kernel[] = {
    ...
    "void block_fn(uint num, __global int* res)" NL,
    "{" NL,
    "    for(int i = 2 ; i < num ; i++)" NL,
    "    {" NL,
    "      res[i] = res[i]/num - (i-1);" NL,
    "    }" NL,
    "}" NL,
    "" NL,
    "kernel void enqueue_block_second_kernel(uint num, __global int* res)" NL,
    "{" NL,
    "  void (^kernelBlock)(void) = ^{ block_fn(num, res); };" NL,
    "" NL,
...
/*
    Test checks kernel block execution order in case of two different kernels
   with enqueue block submitted to one ordered host queue.
*/
int test_host_queue_order(cl_device_id device, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    ...
    enqueue_block_first_kernel, "enqueue_block_first_kernel", "-cl-std=CL2.0");
    ...
    err_ret = clEnqueueNDRangeKernel(queue, kernel1, 1, NULL, &size, &size, 1,
                                     &event1, NULL);
    test_error(err_ret,
               "clEnqueueNDRangeKernel('enqueue_block_first_kernel') failed");
    ...
    err_ret = clEnqueueNDRangeKernel(queue, kernel2, 1, NULL, &size, &size, 0,
                                     NULL, &kernel_event);
    test_error(err_ret,
               "clEnqueueNDRangeKernel('enqueue_block_second_kernel') failed");
    ...
    // Collect resulsts
    err_ret = clEnqueueReadBuffer(queue, res_mem, CL_TRUE, 0, sizeof(result),
                                  result, 0, NULL, NULL);
    test_error(err_ret, "clEnqueueReadBuffer() failed");

    err_ret = clGetEventInfo(kernel_event, CL_EVENT_COMMAND_EXECUTION_STATUS,
    ...
    if ((k = check_kernel_results(result, num)) >= 0
        && check_error(
        -1, "'%s' results validation failed: [%d] returned %d expected 0",
        "test_host_queue_order", k, result[k]))
        res = -1;
