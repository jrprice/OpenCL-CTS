...
static const char *helper_ndrange_1d_glo[] = {
    ...
    "{" NL,
    "  atomic_fetch_add_explicit(&val[get_global_linear_id() % len], 1, "
...
static const char *helper_ndrange_1d_ofs[] = {
    ...
    "get_global_linear_id()) % len], 1, memory_order_relaxed, "
...
static const char *helper_ndrange_2d_ofs[] = {
    ...
    "len], 1, memory_order_relaxed, memory_scope_device);" NL,
    "}" NL,
    "" NL,
...
void generate_reference_1D(std::vector<cl_int> &reference_results,
                           std::vector<cl_uint> &glob_size_arr)
{
...
void generate_reference_1D_local(std::vector<cl_int> &reference_results,
                                 std::vector<cl_uint> &glob_size_arr,
                                 std::vector<cl_uint> &loc_size_arr)
{
    for (size_t g = 0; g < glob_size_arr.size(); ++g)
    {
...
void generate_reference_1D_offset(std::vector<cl_int> &reference_results,
                                  std::vector<cl_uint> &glob_size_arr,
                                  std::vector<cl_uint> &loc_size_arr,
                                  std::vector<cl_uint> &offset, cl_uint len)
{
    for (size_t g = 0; g < glob_size_arr.size(); ++g)
    {
        for (size_t l = 0; l < loc_size_arr.size(); ++l)
...
void generate_reference_2D(std::vector<cl_int> &reference_results,
                           std::vector<cl_uint> &glob_size_arr, cl_uint len)
{
    for (size_t g = 0; g < glob_size_arr.size(); ++g)
    {
        for (size_t h = 0; h < glob_size_arr[(g + 1) % glob_size_arr.size()];
             ++h)
        {
            for (size_t w = 0; w < glob_size_arr[g]; ++w)
            {
...
void generate_reference_2D_local(std::vector<cl_int> &reference_results,
                                 std::vector<cl_uint> &glob_size_arr,
                                 std::vector<cl_uint> &loc_size_arr,
                                 cl_uint len)
{
...
void generate_reference_2D_offset(std::vector<cl_int> &reference_results,
                                  std::vector<cl_uint> &glob_size_arr,
                                  std::vector<cl_uint> &loc_size_arr,
                                  std::vector<cl_uint> &offset, cl_uint len)
{
            ...
            if (glob_size_arr[(g + 1) % n] >= loc_size_arr[l])
            {
                for (size_t o = 0; o < offset.size(); ++o)
                {
                    for (size_t h = 0; h < glob_size_arr[(g + 1) % n]; ++h)
                    {
                        for (size_t w = 0; w < glob_size_arr[g]; ++w)
                        {
                            ++reference_results[(glob_size_arr[g]
                                                 * offset[(o + 1) % n]
                                                 + offset[o]
                                                 + h * glob_size_arr[g] + w)
                                                % len];
...
void generate_reference_3D(std::vector<cl_int> &reference_results,
                           std::vector<cl_uint> &glob_size_arr, cl_uint len)
{
    size_t n = glob_size_arr.size();
    for (size_t g = 0; g < glob_size_arr.size(); ++g)
    {
        size_t global_work_size = glob_size_arr[(g + 2) % n]
        * glob_size_arr[(g + 1) % n] * glob_size_arr[g];
        if (global_work_size <= (len * len))
        {
            for (size_t d = 0; d < glob_size_arr[(g + 2) % n]; ++d)
            {
                for (size_t h = 0; h < glob_size_arr[(g + 1) % n]; ++h)
                {
                    for (size_t w = 0; w < glob_size_arr[g]; ++w)
                    {
                        ++reference_results[(d * glob_size_arr[(g + 1) % n]
                                             * glob_size_arr[g]
                                             + h * glob_size_arr[g] + w)
                                            % len];
                    }
...
void generate_reference_3D_local(std::vector<cl_int> &reference_results,
                                 std::vector<cl_uint> &glob_size_arr,
                                 std::vector<cl_uint> &loc_size_arr,
                                 cl_uint len)
{
    ...
    for (size_t g = 0; g < glob_size_arr.size(); ++g)
    {
        for (size_t l = 0; l < loc_size_arr.size(); ++l)
        {
            size_t global_work_size = glob_size_arr[(g + 2) % n]
            * glob_size_arr[(g + 1) % n] * glob_size_arr[g];
            if (glob_size_arr[(g + 2) % n] >= loc_size_arr[l]
                && global_work_size <= (len * len))
            {
                for (size_t d = 0; d < glob_size_arr[(g + 2) % n]; ++d)
                {
                    for (size_t h = 0; h < glob_size_arr[(g + 1) % n]; ++h)
...
void generate_reference_3D_offset(std::vector<cl_int> &reference_results,
                                  std::vector<cl_uint> &glob_size_arr,
                                  std::vector<cl_uint> &loc_size_arr,
                                  std::vector<cl_uint> &offset, cl_uint len)
{
                ...
                for (size_t o = 0; o < offset.size(); ++o)
                {
                    for (size_t d = 0; d < glob_size_arr[(g + 2) % n]; ++d)
                    {
                        for (size_t h = 0; h < glob_size_arr[(g + 1) % n]; ++h)
                        {
                            ...
                            {
                                ++reference_results
                                [(glob_size_arr[g] * glob_size_arr[(g + 1) % n]
                                  * offset[(o + 2) % n]
                                  + glob_size_arr[g] * offset[(o + 1) % n]
                                  + offset[o]
                                  + d * glob_size_arr[(g + 1) % n]
                                  * glob_size_arr[g]
                                  + h * glob_size_arr[g] + w)
                                 % len];
                            }
...
static int check_kernel_results(cl_int *results, cl_int len,
                                std::vector<cl_uint> &glob_size_arr,
                                std::vector<cl_uint> &loc_size_arr,
                                std::vector<cl_uint> &offset, cl_int dim,
                                cl_bool use_local, cl_bool use_offset)
{
    ...
    switch (dim)
    {
        case 1:
            ...
            {
                generate_reference_1D_local(reference_results, glob_size_arr,
                                            loc_size_arr);
            }
            else
            {
                generate_reference_1D_offset(reference_results, glob_size_arr,
                                             loc_size_arr, offset, len);
            }
            break;
        case 2:
            if (use_local == CL_FALSE)
            ...
            {
                generate_reference_2D_local(reference_results, glob_size_arr,
                                            loc_size_arr, len);
            }
            else
            {
                generate_reference_2D_offset(reference_results, glob_size_arr,
                                             loc_size_arr, offset, len);
            }
            break;
        case 3:
            ...
            {
                generate_reference_3D_local(reference_results, glob_size_arr,
                                            loc_size_arr, len);
            }
            else
            {
                generate_reference_3D_offset(reference_results, glob_size_arr,
                                             loc_size_arr, offset, len);
            }
            break;
...
int test_enqueue_ndrange(cl_device_id device, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    ...
    for (i = 0; i < num_kernels_ndrange_Xd; ++i)
    {
        if (!gKernelName.empty()
            && gKernelName != sources_ndrange_Xd[i].src.kernel_name)
            continue;

        clMemWrapper mem1 = clCreateBuffer(
        context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
        glob_size_arr.size() * sizeof(cl_uint), &glob_size_arr[0], &err_ret);
        test_error(err_ret, "clCreateBuffer() failed");
        clMemWrapper mem2 = clCreateBuffer(
        context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
        loc_size_arr.size() * sizeof(cl_uint), &loc_size_arr[0], &err_ret);
        test_error(err_ret, "clCreateBuffer() failed");
        clMemWrapper mem3 = clCreateBuffer(
        context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
        glob_results.size() * sizeof(cl_int), &glob_results[0], &err_ret);
        test_error(err_ret, "clCreateBuffer() failed");
        clMemWrapper mem4 =
        clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                       ofs_arr.size() * sizeof(cl_uint), &ofs_arr[0], &err_ret);
        test_error(err_ret, "clCreateBuffer() failed");

        kernel_arg args[] = {
            { sizeof(cl_uint), &num }, { sizeof(cl_uint), &global_work_size },
            { sizeof(cl_mem), &mem1 }, { sizeof(cl_mem), &mem2 },
            { sizeof(cl_mem), &mem3 }, { sizeof(cl_mem), &mem4 },
        };

        log_info("Running '%s' kernel (%d of %d) ...\n",
                 sources_ndrange_Xd[i].src.kernel_name, i + 1,
                 num_kernels_ndrange_Xd);
        err_ret = run_single_kernel_args(
        context, queue, sources_ndrange_Xd[i].src.lines,
        sources_ndrange_Xd[i].src.num_lines,
        sources_ndrange_Xd[i].src.kernel_name, kernel_results,
        sizeof(kernel_results), arr_size(args), args);

        cl_int *ptr = (cl_int *)clEnqueueMapBuffer(
        queue, mem3, CL_TRUE, CL_MAP_READ, 0,
        glob_results.size() * sizeof(cl_int), 0, 0, 0, &err_ret);
        ...
        (k = check_kernel_results(kernel_results, arr_size(kernel_results)))
        >= 0
        && check_error(
        -1,
        "'%s' kernel results validation failed: [%d] returned %d expected 0",
        sources_ndrange_Xd[i].src.kernel_name, k, kernel_results[k]))
            res = -1;
        else if ((k = check_kernel_results(
                  ptr, global_work_size, glob_size_arr, loc_size_arr, ofs_arr,
                  sources_ndrange_Xd[i].dim, sources_ndrange_Xd[i].localSize,
                  sources_ndrange_Xd[i].offset))
                 >= 0
                 && check_error(-1,
                                "'%s' global kernel results validation failed: "
                                "[%d] returned %d expected 0",
                                sources_ndrange_Xd[i].src.kernel_name, k,
                                glob_results[k]))
            res = -1;
        else
