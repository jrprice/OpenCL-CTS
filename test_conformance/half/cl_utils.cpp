...
const char
*vector_size_name_extensions[kVectorSizeCount + kStrangeVectorSizeCount] = {
    "", "2", "4", "8", "16", "3"
};
const char *vector_size_strings[kVectorSizeCount + kStrangeVectorSizeCount] = {
    "1", "2", "4", "8", "16", "3"
};
const char *align_divisors[kVectorSizeCount + kStrangeVectorSizeCount] = {
    "1", "2", "4", "8", "16", "4"
};
const char *align_types[kVectorSizeCount + kStrangeVectorSizeCount] = {
    "half", "int", "int2", "int4", "int8", "int2"
};
...
test_status InitCL(cl_device_id device)
{
    ...
    if ((error = clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                                 configSize, &gMaxThreadGroupSize, NULL)))
    ...
    {
        vlog_error("Unable to get device extension string to see if double "
                   "present. (%d) \n",
                   error);
    }
    else
    {
        ...
        {
            if ((error = clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, extSize,
                                         ext, NULL)))
            {
                vlog_error("Unable to get device extension string to see if "
                           "double present. (%d) \n",
                           error);
            }
            else
            {
                if (strstr(ext, "cl_khr_fp64")) hasDouble = 1;
            }
            free(ext);
        }
    }
    ...
    if (NULL == gIn_half || NULL == gOut_half || NULL == gOut_half_reference
        || NULL == gOut_half_reference_double || NULL == gIn_single
        || NULL == gOut_single || NULL == gOut_single_reference
        || NULL
        == gIn_double // || NULL == gOut_double || NULL == gOut_double_reference
    )
        return TEST_FAIL;

    gInBuffer_half = clCreateBuffer(gContext, CL_MEM_READ_ONLY,
                                    getBufferSize(device) / 2, NULL, &error);
    ...
    gInBuffer_single =
    clCreateBuffer(gContext, CL_MEM_READ_ONLY, BUFFER_SIZE, NULL, &error);
    ...
    gInBuffer_double =
    clCreateBuffer(gContext, CL_MEM_READ_ONLY, BUFFER_SIZE * 2, NULL, &error);
    ...
    gOutBuffer_half =
    clCreateBuffer(gContext, CL_MEM_WRITE_ONLY, BUFFER_SIZE / 2, NULL, &error);
    ...
    gOutBuffer_single = clCreateBuffer(gContext, CL_MEM_WRITE_ONLY,
                                       getBufferSize(device), NULL, &error);
    ...
    vlog("\nCompute Device info:\n");
    error =
    clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(string), string, NULL);
    vlog("\tDevice Name: %s\n", string);
    error =
    clGetDeviceInfo(device, CL_DEVICE_VENDOR, sizeof(string), string, NULL);
    vlog("\tVendor: %s\n", string);
    error =
    clGetDeviceInfo(device, CL_DEVICE_VERSION, sizeof(string), string, NULL);
    vlog("\tDevice Version: %s\n", string);
    error = clGetDeviceInfo(device, CL_DEVICE_OPENCL_C_VERSION, sizeof(string),
                            string, NULL);
    vlog("\tOpenCL C Version: %s\n", string);
    error =
    clGetDeviceInfo(device, CL_DRIVER_VERSION, sizeof(string), string, NULL);
...
cl_program MakeProgram(cl_device_id device, const char *source[], int count)
{
    ...
    // create the program
    cl_program program;
    error = create_single_kernel_helper_create_program(gContext, &program,
                                                       (cl_uint)count, source);
    if (NULL == program)
    {
...
int RunKernel(cl_device_id device, cl_kernel kernel, void *inBuf, void *outBuf,
              uint32_t blockCount, int extraArg)
{
    ...
    while (localCount % wg_size) wg_size--;

    if ((error = clEnqueueNDRangeKernel(gQueue, kernel, 1, NULL, &localCount,
                                        &wg_size, 0, NULL, NULL)))
    {
...
{
    return mach_absolute_time(); // returns time since boot.  Ticks have better
                                 // than microsecond precsion.
}
...
uint64_t ReadTime(void) { return 0ULL; }

// return the difference between two times obtained from ReadTime in seconds
double SubtractTime(uint64_t endTime, uint64_t startTime) { return INFINITY; }
...
size_t getBufferSize(cl_device_id device_id)
{
    ...
    if (s_initialized == 0 || s_device_id != device_id)
    {
        ...
        cl_int err =
        clGetDeviceInfo(device_id, CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE,
                        sizeof(result), (void *)&result, NULL);
        if (err)
        {
            vlog_error(
        ...
        result = result / 2;
        if (maxGlobalSize < result * 10) result = result / 10;
        s_initialized = 1;
...
exit:
    ...
    {
        vlog_error("ERROR: clGetDeviceInfo is reporting a "
                   "CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE larger than addressable "
                   "memory on the host.\n It seems highly unlikely that this "
                   "is usable, due to the API design.\n");
        fflush(stdout);
        abort();
    }
