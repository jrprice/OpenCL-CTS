...
static cl_ushort float2half_rtp(float f)
{
    ...
    // overflow
    if (f > MAKE_HEX_FLOAT(0x1.ffcp15f, 0x1ffcL, 3)) return 0x7c00;
...
static cl_ushort float2half_rtn(float f)
{
    ...
    if (f < MAKE_HEX_FLOAT(-0x1.ffcp15f, -0x1ffcL, 3)) return 0xfc00;
...
static cl_ushort double2half_rte(double f)
{
    ...
    // overflow
    if (x >= MAKE_HEX_DOUBLE(0x1.ffep15, 0x1ffeLL, 3)) return 0x7c00 | sign;
    ...
    // very small
    if (x < MAKE_HEX_DOUBLE(0x1.8p-24, 0x18LL, -28)) return sign | 1;
...
static cl_ushort double2half_rtz(double f)
{
    ...
    // overflow
    if (x >= MAKE_HEX_DOUBLE(0x1.0p16, 0x1LL, 16)) return 0x7bff | sign;
...
static cl_ushort double2half_rtp(double f)
{
    ...
    // overflow
    if (f > MAKE_HEX_DOUBLE(0x1.ffcp15, 0x1ffcLL, 3)) return 0x7c00;
    ...
    double g = u.f;
    u.u &= 0xFFFFFC0000000000ULL;
    if (g != u.f && 0 == sign) u.u += 0x0000040000000000ULL;
    u.u -= 0x3F00000000000000ULL;
...
static cl_ushort double2half_rtn(double f)
{
    ...
    if (f < MAKE_HEX_DOUBLE(-0x1.ffcp15, -0x1ffcLL, 3)) return 0xfc00;
    ...
    double g = u.f;
    u.u &= 0xFFFFFC0000000000ULL;
    if (g < u.f && sign) u.u += 0x0000040000000000ULL;
    u.u -= 0x3F00000000000000ULL;
                            ...
                            d2h doubleReferenceFunc, const char *roundName)
{
         ...
         vectorSize++)
    {
        ...
        if (g_arrVecSizes[vectorSize] == 3)
        {
            ...
            device, source_local_store_v3,
            sizeof(source_local_store_v3) / sizeof(source_local_store_v3[0]));
        ...
        if (gTestDouble)
        {
            ...
            {
                doublePrograms[vectorSize][0] =
                MakeProgram(device, double_source,
            ...
            if (g_arrVecSizes[vectorSize] == 3)
                ...
                MakeProgram(device, double_source_private_store_v3,
            ...
            if (NULL == kernels[vectorSize][1])
            {
                gFailCount++;
                vlog_error(
                "\t\tFAILED -- Failed to create double private kernel. (%d)\n",
                error);
                return error;
            }
            ...
            if (NULL == kernels[vectorSize][2])
            {
                gFailCount++;
                vlog_error(
                "\t\tFAILED -- Failed to create double local kernel. (%d)\n",
                error);
                return error;
            }
        }
    ...
    for (i = 0; i < lastCase; i += stride)
    {
        ...
        error =
        clEnqueueWriteBuffer(gQueue, gInBuffer_single, CL_FALSE, 0,
                             count * sizeof(float), gIn_single, 0, NULL, NULL);
        if (error)
        {
             ...
             vectorSize++)
        {
            ...
            for (addressSpace = 0; addressSpace < 3; addressSpace++)
            {
                ...
                if (gTestDouble)
                {
                    ...
                    count * sizeof(cl_ushort), gOut_half, 0, NULL, NULL);
                    if (error)
                    {
                        vlog_error("Failure in clWriteArray\n");
                        gFailCount++;
                    ...
                    error =
                    RunKernel(device, doubleKernels[vectorSize][addressSpace],
             ...
             vectorSize++)
        {
            ...
            if (gTestDouble)
            {
                ...
                for (j = 0; j < loopCount; j++)
                {
                    uint64_t startTime = ReadTime();
                    if ((error = RunKernel(
                         device, doubleKernels[vectorSize][0], gInBuffer_double,
                         gOutBuffer_half, numVecs(count, vectorSize, aligned),
                         runsOverBy(count, vectorSize, aligned))))
                    {
                        gFailCount++;
                        goto exit;
                             ...
                             d2h doubleReferenceFunc, const char *roundName)
{
         ...
         vectorSize++)
    {
        ...
        if (g_arrVecSizes[vectorSize] == 3)
        {
            ...
            MakeProgram(device, source_local_v3,
        ...
        if (gTestDouble)
        {
            ...
            {
                doublePrograms[vectorSize][2] = MakeProgram(
                device, double_source_local,
                sizeof(double_source_local) / sizeof(double_source_local[0]));
                if (NULL == doublePrograms[vectorSize][2])
                {
                    gFailCount++;
                    return -1;
    ...
    if (gReportTimes)
    {
        ...
        if (gTestDouble)
        {
            for (vectorSize = minVectorSize; vectorSize < kLastVectorSizeToTest;
                 vectorSize++)
                vlog_perf(
                SubtractTime(doubleTime[vectorSize], 0) * 1e6 * gDeviceFrequency
                * gComputeDevices / (double)(count * loopCount),
                0, "average us/elem (double)",
                "vStoreaHalf%s avg. d (%s vector size: %d)", roundName,
                addressSpaceNames[0], (g_arrVecSizes[vectorSize]));
            for (vectorSize = minVectorSize; vectorSize < kLastVectorSizeToTest;
                 vectorSize++)
                vlog_perf(SubtractTime(min_double_time[vectorSize], 0) * 1e6
                          * gDeviceFrequency * gComputeDevices / (double)count,
                          0, "best us/elem (double)",
                          "vStoreaHalf%s best d (%s vector size: %d)",
