...
int test_roundTrip(cl_device_id device, cl_context context,
                   cl_command_queue queue, int num_elements)
{
    ...
    cl_program doublePrograms[kVectorSizeCount + kStrangeVectorSizeCount] = {
        0
    };
    ...
    for (vectorSize = kMinVectorSize; vectorSize < kLastVectorSizeToTest;
         vectorSize++)
    {
        const char *source[] = { "__kernel void test( const __global half *in, "
                                 "__global half *out )\n"
                                 "{\n"
...
*/
        ...
        {
            programs[vectorSize] = MakeProgram(
        ...
        kernels[vectorSize] =
        clCreateKernel(programs[vectorSize], "test", &error);
        ...
        if (gTestDouble)
        {
            if (g_arrVecSizes[vectorSize] == 3)
            {
                doublePrograms[vectorSize] =
                MakeProgram(device, doubleSourceV3,
                            sizeof(doubleSourceV3) / sizeof(doubleSourceV3[0]));
            ...
            {
                doublePrograms[vectorSize] =
                MakeProgram(device, doubleSource,
                            sizeof(doubleSource) / sizeof(doubleSource[0]));
                if (NULL == doublePrograms[vectorSize])
                {
                    gFailCount++;
                    return -1;
            ...
            doubleKernels[vectorSize] =
            clCreateKernel(doublePrograms[vectorSize], "test", &error);
    ...
    size_t blockCount = (size_t)getBufferSize(device)
    / elementSize; // elementSize is a power of two
    ...
    for (i = 0; i < (uint64_t)lastCase; i += stride)
    {
        ...
        { // here we loop through vector sizes -- 3 is last.
            ...
            memset_pattern4(gOut_half, &pattern,
                            (size_t)getBufferSize(device) / 2);

            if ((error = clEnqueueWriteBuffer(gQueue, gOutBuffer_half, CL_TRUE,
            ...
            error =
            RunKernel(device, kernels[vectorSize], gInBuffer_half,
                      gOutBuffer_half, numVecs(count, vectorSize, false),
            ...
            if ((error = clEnqueueReadBuffer(gQueue, gOutBuffer_half, CL_TRUE,
                                             0, count * sizeof(cl_half),
                                             gOut_half, 0, NULL, NULL)))
            {
            ...
            if ((memcmp(gOut_half, gIn_half, count * sizeof(cl_half))))
            {
                ...
                for (j = 0; j < count; j++)
                {
                    if (u1[j] != u2[j])
                    {
                        ...
                        // if reference result is sub normal, test if the output
                        // is flushed to zero
                        if (IsHalfSubnormal(u2[j])
                            && ((u1[j] == 0) || (u1[j] == 0x8000)))
                            continue;

                        vlog_error("%lld) (of %lld)  Failure at 0x%4.4x:  "
                                   "0x%4.4x   vector_size = %d \n",
                                   j, (uint64_t)count, u2[j], u1[j],
            ...
            if (gTestDouble)
            {
                ...
                if ((error = RunKernel(device, doubleKernels[vectorSize],
                                       gInBuffer_half, gOutBuffer_half,
                                       numVecs(count, vectorSize, false),
                                       runsOverBy(count, vectorSize, false))))
                {
                    gFailCount++;
                    goto exit;
                ...
                if ((memcmp(gOut_half, gIn_half, count * sizeof(cl_half))))
                {
                    ...
                    for (j = 0; j < count; j++)
                    {
                        if (u1[j] != u2[j])
                        {
                            ...
                            // if reference result is sub normal, test if the
                            // output is flushed to zero
                            if (IsHalfSubnormal(u2[j])
                                && ((u1[j] == 0) || (u1[j] == 0x8000)))
                                continue;

                            vlog_error("%lld) Failure at 0x%4.4x:  0x%4.4x   "
                                       "vector_size = %d (double precsion)\n",
                                       j, u2[j], u1[j],
                                       (g_arrVecSizes[vectorSize]));
                            gFailCount++;
                            error = -1;
    ...
    if (gReportTimes)
    {
             ...
             vectorSize++)
        {
            ...
            for (j = 0; j < loopCount; j++)
            {
                ...
                if ((error = RunKernel(device, kernels[vectorSize],
                                       gInBuffer_half, gOutBuffer_half,
                                       numVecs(count, vectorSize, false),
                                       runsOverBy(count, vectorSize, false))))
                {
            ...
            if (gTestDouble)
            {
                ...
                for (j = 0; j < loopCount; j++)
                {
                    uint64_t startTime = ReadTime();
                    if ((error = RunKernel(
                         device, doubleKernels[vectorSize], gInBuffer_half,
                         gOutBuffer_half, numVecs(count, vectorSize, false),
                         runsOverBy(count, vectorSize, false))))
                    {
                        gFailCount++;
            ...
            for (vectorSize = kMinVectorSize;
                ...
                vlog_perf(SubtractTime(min_double_time[vectorSize], 0) * 1e6
                          * gDeviceFrequency * gComputeDevices / (double)count,
                          ...
                          "roundTrip best d (vector size: %d)",
                          (g_arrVecSizes[vectorSize]));
        }
    }
