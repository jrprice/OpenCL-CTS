...
int Test_vLoadHalf_private(cl_device_id device, bool aligned)
{
    cl_int error;
    int vectorSize;
    cl_program programs[kVectorSizeCount + kStrangeVectorSizeCount]
                       [AS_NumAddressSpaces] = { { 0 } };
    cl_kernel kernels[kVectorSizeCount + kStrangeVectorSizeCount]
                     [AS_NumAddressSpaces] = { { 0 } };
         ...
         vectorSize++)
    {
        ...
        const char *source_privateV3aligned[] = {
            "__kernel void test( const __global half *p, __global float3 *f )\n"
            "{\n"
            "   ushort4 data[4];\n" // declare as vector for alignment. Make
                                    // four to check to see vloada_half3 index
                                    // is working.
            ...
            //  Fill in the 4th value so we don't have to special case this code
        ...
        const char *source_constant[] = {
            "__kernel void test( __constant half *p, __global float",
            vector_size_name_extensions[vectorSize],
            " *f )\n"
            "{\n"
            "   size_t i = get_global_id(0);\n"
        ...
        {
            programs[vectorSize][AS_Global] =
            MakeProgram(device, sourceV3aligned,
                        sizeof(sourceV3aligned) / sizeof(sourceV3aligned[0]));
            if (NULL == programs[vectorSize][AS_Global])
            {
                gFailCount++;
                vlog_error("\t\tFAILED -- Failed to create program.\n");
                for (q = 0;
                     q < sizeof(sourceV3aligned) / sizeof(sourceV3aligned[0]);
                     q++)
        ...
        {
            programs[vectorSize][AS_Global] = MakeProgram(
            device, sourceV3, sizeof(sourceV3) / sizeof(sourceV3[0]));
            if (NULL == programs[vectorSize][AS_Global])
            {
                gFailCount++;
                vlog_error("\t\tFAILED -- Failed to create program.\n");
        ...
        kernels[vectorSize][AS_Global] =
        clCreateKernel(programs[vectorSize][AS_Global], "test", &error);
        ...
        else if (g_arrVecSizes[vectorSize] == 3)
        {
            if (aligned)
            {
                ...
                source_size = sizeof(source_privateV3aligned)
                / sizeof(source_privateV3aligned[0]);
            }
            else
            {
                source_ptr = source_privateV3;
                source_size =
                sizeof(source_privateV3) / sizeof(source_privateV3[0]);
            }
        }
        else
        {
        ...
        programs[vectorSize][AS_Private] =
        MakeProgram(device, source_ptr, source_size);
        if (NULL == programs[vectorSize][AS_Private])
        {
            gFailCount++;
            vlog_error("\t\tFAILED -- Failed to create private program.\n");
            for (q = 0; q < source_size; q++) vlog_error("%s", source_ptr[q]);
            return -1;
        }

        kernels[vectorSize][AS_Private] =
        clCreateKernel(programs[vectorSize][AS_Private], "test", &error);
        ...
        else if (g_arrVecSizes[vectorSize] == 3)
        {
            if (aligned)
            {
                ...
                source_size = sizeof(source_localV3aligned)
                / sizeof(source_localV3aligned[0]);
            }
            else
            {
                source_ptr = source_localV3;
                source_size =
                sizeof(source_localV3) / sizeof(source_localV3[0]);
            }
        }
        else
        {
        ...
        kernels[vectorSize][AS_Local] =
        clCreateKernel(programs[vectorSize][AS_Local], "test", &error);
        ...
        if (g_arrVecSizes[vectorSize] == 3)
        {
            if (aligned)
            {
                programs[vectorSize][AS_Constant] =
                MakeProgram(device, source_constantV3aligned,
                            sizeof(source_constantV3aligned)
                            / sizeof(source_constantV3aligned[0]));
                if (NULL == programs[vectorSize][AS_Constant])
                {
                    gFailCount++;
                    vlog_error(
                    "\t\tFAILED -- Failed to create constant program.\n");
                    for (q = 0; q < sizeof(source_constantV3aligned)
                         / sizeof(source_constantV3aligned[0]);
                         q++)
            ...
            {
                programs[vectorSize][AS_Constant] = MakeProgram(
                device, source_constantV3,
                sizeof(source_constantV3) / sizeof(source_constantV3[0]));
                if (NULL == programs[vectorSize][AS_Constant])
                {
                    gFailCount++;
                    vlog_error(
                    "\t\tFAILED -- Failed to create constant program.\n");
                    for (q = 0; q < sizeof(source_constantV3)
                         / sizeof(source_constantV3[0]);
                         q++)
        ...
        {
            programs[vectorSize][AS_Constant] =
            MakeProgram(device, source_constant,
                        sizeof(source_constant) / sizeof(source_constant[0]));
            if (NULL == programs[vectorSize][AS_Constant])
            {
                gFailCount++;
                vlog_error(
                "\t\tFAILED -- Failed to create constant program.\n");
                for (q = 0;
                     q < sizeof(source_constant) / sizeof(source_constant[0]);
                     q++)
        ...
        kernels[vectorSize][AS_Constant] =
        clCreateKernel(programs[vectorSize][AS_Constant], "test", &error);
    ...
    size_t blockCount =
    getBufferSize(device) / elementSize; // elementSize is power of 2
    uint64_t lastCase = 1ULL
    << (8 * sizeof(cl_half)); // number of things of size cl_half
    ...
    for (i = 0; i < (uint64_t)lastCase; i += blockCount)
    {
        ...
        for (j = 0; j < count; j++) p[j] = j + i;

        if ((error = clEnqueueWriteBuffer(gQueue, gInBuffer_half, CL_TRUE, 0,
                                          count * sizeof(cl_half), gIn_half, 0,
                                          NULL, NULL)))
        {
        ...
        for (j = 0; j < count; j++) d[j] = half2float(s[j]);
        ...
        { // here we loop through vector sizes, 3 is last
            ...
            for (addressSpace = 0; addressSpace < AS_NumAddressSpaces;
                 addressSpace++)
            {
                ...
                /*
                 if (addressSpace == 3) {
                 vlog("Note: skipping address space %s due to small buffer
                 size.\n", addressSpaceNames[addressSpace]); continue;
                 }
                 */
                memset_pattern4(gOut_single, &pattern, getBufferSize(device));
                if ((error = clEnqueueWriteBuffer(
                     gQueue, gOutBuffer_single, CL_TRUE, 0,
                     count * sizeof(float), gOut_single, 0, NULL, NULL)))
                {
                    vlog_error("Failure in clWriteArray\n");
                    gFailCount++;
                    goto exit;
                ...
                if ((error = RunKernel(
                     device, kernels[vectorSize][addressSpace], gInBuffer_half,
                     gOutBuffer_single, numVecs(count, vectorSize, aligned),
                ...
                if (memcmp(gOut_single, gOut_single_reference,
                           count * sizeof(float)))
                {
                    ...
                    {
                        if (isnan(f1[j])
                            && isnan(f2[j])) // both are nan dont compare them
                            continue;
                        if (u1[j] != u2[j])
                        {
                            vlog_error(
                            " %lld)  (of %lld) Failure at 0x%4.4x:  %a vs *%a  "
                            "(0x%8.8x vs *0x%8.8x)  vector_size = %d (%s) "
                            "address space = %s, load is %s\n",
                            j, (uint64_t)count, ((unsigned short *)gIn_half)[j],
                            f1[j], f2[j], u1[j], u2[j],
                ...
                if (gReportTimes && addressSpace == 0)
                {
                    ...
                    for (j = 0; j < 100; j++)
                    {
                        ...
                        error =
                        RunKernel(device, kernels[vectorSize][addressSpace],
                                  gInBuffer_half, gOutBuffer_single,
    ...
    if (gReportTimes)
    {
        for (vectorSize = minVectorSize; vectorSize < kLastVectorSizeToTest;
            ...
            vlog_perf(SubtractTime(time[vectorSize], 0) * 1e6 * gDeviceFrequency
                      * gComputeDevices / (double)(count * 100),
                      0, "average us/elem",
                      "vLoad%sHalf avg. (%s, vector size: %d)",
                      ((aligned) ? "a" : ""), addressSpaceNames[0],
                      (g_arrVecSizes[vectorSize]));
        for (vectorSize = minVectorSize; vectorSize < kLastVectorSizeToTest;
             vectorSize++)
            vlog_perf(SubtractTime(min_time[vectorSize], 0) * 1e6
                      * gDeviceFrequency * gComputeDevices / (double)count,
                      0, "best us/elem",
                      "vLoad%sHalf best (%s vector size: %d)",
                      ((aligned) ? "a" : ""), addressSpaceNames[0],
                      (g_arrVecSizes[vectorSize]));
    }
...
int test_vload_half(cl_device_id device, cl_context context,
                    cl_command_queue queue, int num_elements)
{
    return Test_vLoadHalf_private(device, false);
}

int test_vloada_half(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements)
{
