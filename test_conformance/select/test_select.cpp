...
static cl_program makeSelectProgram(cl_kernel *kernel_ptr,
                                    const cl_context context, Type stype,
                                    Type ctype, size_t veclen);
...
static int doTest(cl_command_queue queue, cl_context context, Type stype,
                  Type cmptype, cl_device_id device);
...
static void initSrcBuffer(void *src1, Type stype, MTdata d)
{
    ...
    for (i = 0; i < BUFFER_SIZE / sizeof(cl_int); i++) s1[i] = genrand_int32(d);
}
...
static void initCmpBuffer(void *cmp, Type cmptype, uint64_t start, size_t count)
{
    ...
    switch (type_size[cmptype])
    {
        case 1: {
            ...
            for (i = 0; i < count; ++i) ub[i] = (uint8_t)start++;
            break;
        }
        case 2: {
            uint16_t *us = (uint16_t *)cmp;
            for (i = 0; i < count; ++i) us[i] = (uint16_t)start++;
            break;
        }
        case 4: {
            if (!s_wimpy_mode)
            {
                ...
                for (i = 0; i < count; ++i) ui[i] = (uint32_t)start++;
            }
            else
            {
        ...
        default: log_error("invalid cmptype %s\n", type_name[cmptype]);
    } // end switch
}
                                    ...
                                    Type cmptype, size_t vec_len)
{
    ...
    const char *sourceV3[] = {
        ...
        "       vstore3( select( vload3(tid, src1), vload3(tid, src2), "
        "vload3(tid, cmp)), tid, dest );\n"
        "   else if(tid + 1 == size)\n"
        "   {\n"
        // If the size is odd, then we have odd * 3 elements, which is an odd
        // number of scalars in the array If the size is even, then we have even
        // * 3 elements, which is an even number of scalars in the array 3 will
        // never divide evenly into a power of two sized buffer, so the last
        // vec3 will overhang by 1 or 2.
        //  The only even number x in power_of_two < x <= power_of_two+2 is
        //  power_of_two+2. The only odd number x in power_of_two < x <=
        //  power_of_two+2 is power_of_two+1.
        // Therefore, odd sizes overhang the end of the array by 1, and even
        // sizes overhang by 2.
    ...
    switch (vec_len)
    {
        case 1:
            ...
            snprintf(testname, sizeof(testname), "select_%s_%s", stypename,
                     ctypename);
        ...
        case 3:
            strncpy(stypename, type_name[srctype], sizeof(stypename));
            strncpy(ctypename, type_name[cmptype], sizeof(ctypename));
            snprintf(testname, sizeof(testname), "select_%s3_%s3", stypename,
                     ctypename);
        ...
        case 8:
        case 16:
            snprintf(stypename, sizeof(stypename), "%s%d", type_name[srctype],
                     (int)vec_len);
            snprintf(ctypename, sizeof(ctypename), "%s%d", type_name[cmptype],
                     (int)vec_len);
            snprintf(testname, sizeof(testname), "select_%s_%s", stypename,
                     ctypename);
            log_info("Building %s(%s, %s, %s)\n", testname, stypename,
    ...
    if (create_single_kernel_helper(
        ...
        (cl_uint)(vec_len == 3 ? sizeof(sourceV3) / sizeof(sourceV3[0])
                               : sizeof(source) / sizeof(source[0])),
        vec_len == 3 ? sourceV3 : source, testname))
    {
                  ...
                  Type cmptype, cl_device_id device)
{
    ...
    {
        log_info(
        "Skipping double because cl_khr_fp64 extension is not supported.\n");
        return 0;
    }

    if (gIsEmbedded)
    {
        if ((stype == klong || stype == kulong)
            && !is_extension_available(device, "cles_khr_int64"))
        {
            log_info("Long types unsupported, skipping.");
            return 0;
        }

        if ((cmptype == klong || cmptype == kulong)
            && !is_extension_available(device, "cles_khr_int64"))
        {
            log_info("Long types unsupported, skipping.");
            return 0;
    ...
    if (NULL == ref)
    {
        log_error("Error: could not allocate ref buffer\n");
        goto exit;
    }
    ...
    if (err)
    {
        log_error("Error: could not allocate src1 buffer\n");
        ++s_test_fail;
        goto exit;
    }
    ...
    if (err)
    {
        log_error("Error: could not allocate src2 buffer\n");
        ++s_test_fail;
        goto exit;
    }
    ...
    if (err)
    {
        log_error("Error: could not allocate cmp buffer\n");
        ++s_test_fail;
        goto exit;
    }
    ...
    if (err)
    {
        log_error("Error: could not allocate dest buffer\n");
        ++s_test_fail;
        goto exit;
    }
    ...
    if (type_size[stype] == 8)
        cmp_stride =
        block_elements * step * (0xffffffffffffffffULL / 0x100000000ULL + 1);
    ...
    for (i = 0; i < blocks; i += step)
    {
        ...
        if (err)
        {
            log_error("Error: Could not map src1");
            goto exit;
        }
        ...
        if (err)
        {
            log_error("Error: Could not map src2");
            goto exit;
        }
        // Setup the input data to change for each block
        ...
        if (err)
        {
            log_error("Error: Could not map cmp");
            goto exit;
        }
        // Setup the input data to change for each block
        ...
        for (vecsize = 0; vecsize < VECTOR_SIZE_COUNT; ++vecsize)
        {
            ...
            if (err)
            {
                log_error("Error: Could not map dest");
                ++s_test_fail;
                goto exit;
            }
            ...
            err = clEnqueueNDRangeKernel(queue, kernels[vecsize], 1, NULL,
            ...
            if (err)
            {
                log_error("Error: Could not map dest # 2");
                ++s_test_fail;
                goto exit;
            }

            if ((*checkResults[stype])(d, vecsize == 0 ? sref : ref,
...
int test_select_uchar_uchar(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kuchar, kuchar, deviceID);
}
int test_select_uchar_char(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kuchar, kchar, deviceID);
}
int test_select_char_uchar(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kchar, kuchar, deviceID);
}
int test_select_char_char(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kchar, kchar, deviceID);
}
int test_select_ushort_ushort(cl_device_id deviceID, cl_context context,
                              cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kushort, kushort, deviceID);
}
int test_select_ushort_short(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kushort, kshort, deviceID);
}
int test_select_short_ushort(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kshort, kushort, deviceID);
}
int test_select_short_short(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kshort, kshort, deviceID);
}
int test_select_uint_uint(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kuint, kuint, deviceID);
}
int test_select_uint_int(cl_device_id deviceID, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kuint, kint, deviceID);
}
int test_select_int_uint(cl_device_id deviceID, cl_context context,
                         cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kint, kuint, deviceID);
}
int test_select_int_int(cl_device_id deviceID, cl_context context,
                        cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kint, kint, deviceID);
}
int test_select_float_uint(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kfloat, kuint, deviceID);
}
int test_select_float_int(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kfloat, kint, deviceID);
}
int test_select_ulong_ulong(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kulong, kulong, deviceID);
}
int test_select_ulong_long(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kulong, klong, deviceID);
}
int test_select_long_ulong(cl_device_id deviceID, cl_context context,
                           cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, klong, kulong, deviceID);
}
int test_select_long_long(cl_device_id deviceID, cl_context context,
                          cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, klong, klong, deviceID);
}
int test_select_double_ulong(cl_device_id deviceID, cl_context context,
                             cl_command_queue queue, int num_elements)
{
    return doTest(queue, context, kdouble, kulong, deviceID);
}
int test_select_double_long(cl_device_id deviceID, cl_context context,
                            cl_command_queue queue, int num_elements)
{
...
int main(int argc, const char *argv[])
{
        ...
        if (arg[0] == '-')
        {
            ...
            while (*arg != '\0')
            {
                switch (*arg)
                {
                    ...
                    case 'w': s_wimpy_mode = true; break;
                    case '[':
                        parseWimpyReductionFactor(arg,
    ...
    if (s_wimpy_mode)
    {
        log_info("\n");
        log_info(
        "*** WARNING: Testing in Wimpy mode!                     ***\n");
        log_info(
        "*** Wimpy mode is not sufficient to verify correctness. ***\n");
        log_info(
        "*** It gives warm fuzzy feelings and then nevers calls. ***\n\n");
        log_info("*** Wimpy Reduction Factor: %-27u ***\n\n",
                 s_wimpy_reduction_factor);
    }

    int err =
    runTestHarness(argCount, argList, test_num, test_list, false, false, 0);
...
static void printUsage(void)
{
    ...
    log_info("\t-[2^n] Set wimpy reduction factor, recommended range of n is "
