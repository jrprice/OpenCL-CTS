...
"   int height_lod = ( get_image_height(output) >> lod ) ? ( "
"get_image_height(output) >> lod ) : 1;\n"
                              ...
                              ExplicitType inputType, MTdata d)
{
         ...
         mem_flag_index++)
    {
                ...
                if (is_sRGBA_order(imageInfo->format->image_channel_order))
                {
                    ...
                    for (size_t z = 0; z < imageInfo->arraySize; z++)
                    {
                        for (size_t y = 0; y < imageInfo->height; y++)
                        {
                            ...
                            + imageInfo->width * y * 4
                            + imageInfo->height * imageInfo->width * z * 4;
        ...
        if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
        {
            ...
            imageInfo->arraySize, 0, 0, maxImageUseHostPtrBackingStore, &error);

            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to create 2D image array of size %ld "
                          "x %ld x %ld pitch %ld (%s)\n",
        ...
        else // Either CL_MEM_ALLOC_HOST_PTR, CL_MEM_COPY_HOST_PTR or none
        {
            ...
            // flag is specified, so we just do the same thing either way Note:
            // if the flags is really CL_MEM_COPY_HOST_PTR, we want to remove
            // it, because we don't want to copy any incoming data
            if (gTestMipmaps)
            {
        ...
        for (int lod = 0; lod < num_lod_loops; lod++)
        {
            ...
            if (gTestMipmaps)
                resultSize =
                width_lod * height_lod * imageInfo->arraySize * pixelSize;
            else
            ...
            for (size_t z = 0, i = 0; z < imageInfo->arraySize; z++)
            {
                for (size_t y = 0; y < height_lod; y++)
                {
                    char *resultPtr;
                    if (gTestMipmaps)
                        resultPtr = (char *)resultValues
                        + y * width_lod * pixelSize
                        + z * width_lod * height_lod * pixelSize;
                    else
                        resultPtr = (char *)resultValues
                        + y * imageInfo->rowPitch + z * imageInfo->slicePitch;
                    for (size_t x = 0; x < width_lod; x++, i++)
                    {
                        char
                        resultBuffer[16]; // Largest format would be 4 channels
                                          // * 4 bytes (32 bits) each

                        // Convert this pixel
                        if (inputType == kFloat)
                            pack_image_pixel((float *)imagePtr,
                            ...
                            imageInfo->format->image_channel_order))
                        {
                            ...
                            if ((err[0] > max_err) || (err[1] > max_err)
                                || (err[2] > max_err)
                                || (err[3]
                                    > 0)) // there is no conversion for alpha so
                                 ...
                                 == CL_FLOAT)
                        {
                                                              ...
                                                              imageInfo))
                            {
                                ...
                                log_error("ERROR: Sample %ld (%ld,%ld,%ld) did "
                                          "not validate! (%s)\n",
                                          i, x, y, z,
                                          mem_flag_names[mem_flag_index]);
                                 ...
                                 == CL_HALF_FLOAT)
                        {
                                ...
                                != 0)
                            {
                                ...
                                for (size_t j = 0; j < get_format_channel_count(
                                                   imageInfo->format);
                                     j++)
                                {
                                ...
                                {
                                    totalErrors++;
                                    log_error("ERROR: Sample %ld (%ld,%ld) did "
                                              "not validate! (%s)\n",
                                              i, x, y,
                                              mem_flag_names[mem_flag_index]);
                                    unsigned short *e =
                                    (unsigned short *)resultBuffer;
                                    unsigned short *a =
                                    (unsigned short *)resultPtr;
                                    log_error("    Expected: 0x%04x 0x%04x "
                                              "0x%04x 0x%04x\n",
                                              e[0], e[1], e[2], e[3]);
                                    log_error("    Actual:   0x%04x 0x%04x "
                                              "0x%04x 0x%04x\n",
                                              a[0], a[1], a[2], a[3]);
                                    if (inputType == kFloat)
                                    {
                        ...
                        else
                        {
                                ...
                                != 0)
                            {
                                ...
                                // We are allowed 0.6 absolute error vs.
                                // infinitely precise for some normalized
                                // formats
                                ...
                                if (failure)
                                {
                                    ...
                                    {
                                        // Try to guess what the rounding mode
                                        // of the device really is based on what
                                        // it returned
                                        ...
                                        log_error("       Rounding mode of "
                                                  "device appears to be %s\n",
                                                  deviceRounding);
                                        return 1;
                                    }
                                    log_error("ERROR: Sample %d (%d,%d) did "
                                              "not validate!\n",
                                              (int)i, (int)x, (int)y);
                                    switch (
                        ...
                        imagePtr += get_explicit_type_size(inputType)
                                  ...
                                  ExplicitType inputType, MTdata d)
{
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                ...
                for (imageInfo.arraySize = 2; imageInfo.arraySize < 7;
                     imageInfo.arraySize++)
                {
                    if (gTestMipmaps)
                    ...
                    device, context, queue, kernel, &imageInfo, inputType, d);
    ...
    else if (gTestMaxImages)
    {
        ...
        get_max_sizes(&numbeOfSizes, 100, sizes, maxWidth, maxHeight, 1,
                      ...
                      CL_MEM_OBJECT_IMAGE2D_ARRAY, imageInfo.format, CL_TRUE);
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            {
                imageInfo.width =
                (size_t)random_log_in_range(16, maxWidthRange, d);
                imageInfo.height =
                (size_t)random_log_in_range(16, maxHeighthRange, d);
                imageInfo.arraySize =
                (size_t)random_log_in_range(8, maxArraySizeRange, d);
                ...
                if (gTestMipmaps)
                {
                    ...
                    - 1),
                    d);
                    // Need to take into account the input buffer size,
                    // otherwise we will end up with input buffer that is
                    // exceeding MaxAlloc
                ...
                else
                {
                    ...
                    // Image size and buffer size may differ due to different
                    // pixel size. See creation of buffer at line ~153.
                    size = (cl_ulong)imageInfo.slicePitch
                    * (cl_ulong)imageInfo.arraySize * 4;
                    buffSize = (cl_ulong)imageInfo.width
                    * (cl_ulong)imageInfo.height * imageInfo.arraySize
                    * get_explicit_type_size(inputType) * 4;
                }
            } while (size > maxAllocSize || buffSize > maxAllocSize
                     || (size * 3) > memSize);

            if (gDebugTrace)
                log_info("   at size %ld,%ld,%ld (pitch %ld, slice %ld) out of "
                         "%ld,%ld,%ld\n",
