...
"__kernel void sample_kernel( __global %s4 *input, read_write image1d_t output "
"%s)\n"
"{\n"
         ...
         mem_flag_index++)
    {
        ...
        if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
        {
            ...
            if (gTestMaxImages)
            {
                ...
                imageInfo->format, imageInfo->width, 0,
                maxImageUseHostPtrBackingStore, NULL, &error);
            }
            else
            {
                error =
                protImage.Create(context, mem_flag_types[mem_flag_index],
                                 imageInfo->format, imageInfo->width);
            }
            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to create 1D image of size %ld pitch "
                          "%ld (%s, %s)\n",
        ...
        else // Either CL_MEM_ALLOC_HOST_PTR, CL_MEM_COPY_HOST_PTR or none
        {
            ...
            else
            {
                ...
                if (error != CL_SUCCESS)
                {
                    log_error("ERROR: Unable to create 1D image of size %ld "
                              "pitch %ld (%s, %s)\n",
                              imageInfo->width, imageInfo->rowPitch,
                              IGetErrorString(error),
                              mem_flag_names[mem_flag_index]);
                ...
                for (size_t x = 0, i = 0; i < width_lod; x++, i++)
                {
                             ...
                             == CL_FLOAT)
                    {
                                                          ...
                                                          imageInfo))
                        {
                            ...
                            log_error(
                            "ERROR: Sample %ld did not validate! (%s)\n", i,
                            mem_flag_names[mem_flag_index]);
                             ...
                             == CL_HALF_FLOAT)
                    {
                            ...
                            != 0)
                        {
                            ...
                            {
                                totalErrors++;
                                log_error("ERROR: Sample %ld (%ld) did not "
                                          "validate! (%s)\n",
                    ...
                    else
                    {
                            ...
                            != 0)
                        {
                            ...
                            if (failure)
                            {
                                ...
                                log_error(
                                "ERROR: Sample %d (%d) did not validate!\n",
                                (int)i, (int)x);
                                switch (
                                imageInfo->format->image_channel_data_type)
                                {
