...
template <class T>
int determine_validation_error_1D_arr(
void *imagePtr, image_descriptor *imageInfo, image_sampler_data *imageSampler,
T *resultPtr, T *expected, float error, float x, float y, float xAddressOffset,
float yAddressOffset, size_t j, int &numTries, int &numClamped,
bool printAsFloat, int lod)
{
    ...
    x, y, 0.0f, xAddressOffset, yAddressOffset, 0.0f, imageInfo->width,
    imageInfo->arraySize, 0, imageSampler, imageInfo, clampedX, clampedY,
    ignoreMe);
    ...
    if (found)
    {
        ...
        if (clamped && clampedX == actualX && clampedY == actualY)
        {
            if ((--numClamped) == 0)
            {
                ...
                if (printAsFloat)
                {
                    log_error("Sample %d: coord {%f(%a), %f(%a)} did not "
                              "validate!\n\tExpected (%g,%g,%g,%g),\n\tgot     "
                              " (%g,%g,%g,%g),\n\terror of %g\n",
                ...
                {
                    log_error(
                    "Sample %d: coord {%f(%a), %f(%a)} did not "
                    "validate!\n\tExpected (%x,%x,%x,%x),\n\tgot      "
                    "(%x,%x,%x,%x)\n",
                    (int)j, x, x, y, y, (int)expected[0], (int)expected[1],
    ...
    if (!clampingErr)
    {
        ...
        {
            log_error(
            "Sample %d: coord {%f(%a), %f(%a)} did not validate!\n\tExpected "
            "(%g,%g,%g,%g),\n\tgot      (%g,%g,%g,%g), error of %g\n",
            (int)j, x, x, y, y, (float)expected[0], (float)expected[1],
            (float)expected[2], (float)expected[3], (float)resultPtr[0],
            (float)resultPtr[1], (float)resultPtr[2], (float)resultPtr[3],
            error);
        }
        else
        {
        ...
        if (printAsFloat && gExtraValidateInfo)
        {
            ...
            for (int yOff = -2; yOff <= 1; yOff++)
            {
                ...
                log_error("%d\t(%g,%g,%g,%g)", clampedY + yOff, top[0], top[1],
                          top[2], top[3]);
            ...
            if (clampedY < 1)
            {
                ...
                log_error("\t%d\t%d\t%d\t%d\n", clampedX - 2, clampedX - 1,
                          clampedX, clampedX + 1);
                for (int yOff = (int)imageInfo->arraySize - 2;
                     yOff <= (int)imageInfo->arraySize + 1; yOff++)
                {
                    float top[4], real[4], bot[4], bot2[4];
                    read_image_pixel_float(imagePtr, imageInfo, clampedX - 2,
                                           clampedY + yOff, 0, top);
...
static void InitFloatCoords(image_descriptor *imageInfo,
                            ...
                            int normalized_coords, MTdata d, int lod)
{
    size_t i = 0;
        ...
        for (size_t y = 0; y < imageInfo->arraySize; y++)
        {
            for (size_t x = 0; x < width_lod; x++, i++)
            {
                ...
                yOffsets[i] = (float)CLAMP((double)yOffsets[i], 0.0,
                                           (double)imageInfo->arraySize - 1.0);
            }
        }
    }
                             ...
                             MTdata d)
{
    ...
    malloc(sizeof(cl_float) * imageInfo->width * imageInfo->arraySize));
    ...
    if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
    {
        ...
        {
            error = protImage.Create(
            context, CL_MEM_OBJECT_IMAGE1D_ARRAY, image_read_write_flags,
            imageInfo->format, imageInfo->width, 1, 1, imageInfo->arraySize);
        }
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create 1D image array of size %d x %d "
                      "pitch %d (%s)\n",
                      (int)imageInfo->width, (int)imageInfo->arraySize,
        ...
        if (gTestMipmaps)
        {
            ...
            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to create %d level mipmapped 1D image "
                          "array of size %d x %d (pitch %d, %d ) (%s)",
    ...
    if (gMemFlagsToUse != CL_MEM_COPY_HOST_PTR)
    {
        ...
        else
        {
            ...
            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to write to %d level 1D image array "
                          "of size %d x %d\n",
                          (int)imageInfo->num_mip_levels, (int)imageInfo->width,
    ...
    clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
                   sizeof(cl_float) * imageInfo->width * imageInfo->arraySize,
                   xOffsetValues, &error);
    ...
    for (int lod = 0; (gTestMipmaps && lod < imageInfo->num_mip_levels)
         ...
         lod++)
    {
        size_t resultValuesSize = width_lod * imageInfo->arraySize
        * get_explicit_type_size(outputType) * 4;
        ...
        for (int q = 0; q < loopCount; q++)
        {
            ...
            error = clEnqueueWriteBuffer(queue, xOffsets, CL_TRUE, 0,
                                         sizeof(cl_float) * imageInfo->arraySize
                                         * imageInfo->width,
            ...
            if (gDebugTrace)
                log_info("    reading results, %ld kbytes\n",
                         (unsigned long)(width_lod * imageInfo->arraySize
                                         * get_explicit_type_size(outputType)
                                         * 4 / 1024));

            error =
            clEnqueueReadBuffer(queue, results, CL_TRUE, 0,
                    ...
                    for (size_t x = 0; x < width_lod; x++, j++)
                    {
                             ...
                             norm_offset_x += NORM_OFFSET)
                        {
                                 ...
                                 norm_offset_y += NORM_OFFSET)
                            {
                                ...
                                // If the final result DOES match, then we've
                                // found a valid result and we're done with this
                                // pixel.
                                found_pixel = (err1 <= maxErr)
                                && (err2 <= maxErr) && (err3 <= maxErr)
                                && (err4 <= maxErr);
                            } // norm_offset_x
                        } // norm_offset_y
                        ...
                        if (!found_pixel)
                        {
                                 ...
                                 norm_offset_x += NORM_OFFSET)
                            {
                                     ...
                                     norm_offset_y += NORM_OFFSET)
                                {
                                    ...
                                    // normalized, then only test with offsets
                                    // (0.0, 0.0) E.g., test one pixel.
                                    if (!imageSampler->normalized_coords
                                        || gDeviceType != CL_DEVICE_TYPE_GPU
                                        || NORM_OFFSET == 0)
                                    {
                                    ...
                                    if (!(err1 <= maxErr) || !(err2 <= maxErr)
                                        ...
                                        || !(err4 <= maxErr))
                                    {
                                        // try flushing the denormals, if there
                                        // is a failure.
                                        if (containsDenormals)
                                        {
                                            // If implementation decide to flush
                                            // subnormals to zero, max error
                                            ...
                                            err1 =
                                            fabsf(sRGBmap(resultPtr[0])
                                                  - sRGBmap(expected[0]));
                                            err2 =
                                            fabsf(sRGBmap(resultPtr[1])
                                                  - sRGBmap(expected[1]));
                                            err3 =
                                            fabsf(sRGBmap(resultPtr[2])
                                                  - sRGBmap(expected[2]));
                                            err4 =
                                            fabsf(resultPtr[3] - expected[3]);
                                        }
                                    ...
                                    {
                                        log_error(
                                        "FAILED norm_offsets: %g , %g:\n",
                                        norm_offset_x, norm_offset_y);

                                        float tempOut[4];
                                        shouldReturn |=
                                        determine_validation_error_1D_arr<
                                        float>(
                                    ...
                                    {
                                        log_error(
                                        "Test error: we should have detected "
                                        "this passing above.\n");
                                    }
                    ...
                    for (size_t x = 0; x < width_lod; x++, j++)
                    {
                             ...
                             norm_offset_x += NORM_OFFSET)
                        {
                                 ...
                                 norm_offset_y += NORM_OFFSET)
                            {
                                ...
                                error = errMax(
                                errMax(
                                abs_diff_uint(expected[0], resultPtr[0]),
                                abs_diff_uint(expected[1], resultPtr[1])),
                                errMax(
                                abs_diff_uint(expected[2], resultPtr[2]),
                    ...
                    for (size_t x = 0; x < width_lod; x++, j++)
                    {
                             ...
                             norm_offset_x += NORM_OFFSET)
                        {
                                 ...
                                 norm_offset_y += NORM_OFFSET)
                            {
                                ...
                                errMax(
                                abs_diff_int(expected[2], resultPtr[2]),
                        ...
                        if (!found_pixel)
                        {
                                 ...
                                 norm_offset_x += NORM_OFFSET)
                            {
                                     ...
                                     norm_offset_y += NORM_OFFSET)
                                {
                                    ...
                                    error = errMax(
                                    errMax(
                                    abs_diff_int(expected[0], resultPtr[0]),
                                    abs_diff_int(expected[1], resultPtr[1])),
                                    errMax(
                                    abs_diff_int(expected[2], resultPtr[2]),
                                    abs_diff_int(expected[3], resultPtr[3])));
                                    ...
                                    if (error > MAX_ERR)
                                    {
                                        ...
                                        shouldReturn |=
                                        determine_validation_error_1D_arr<int>(
                                        imagePtr, imageInfo, imageSampler,
                                        resultPtr, expected, error,
                                        xOffsetValues[j], yOffsetValues[j],
                                        norm_offset_x, norm_offset_y, j,
        ...
        {
            nextLevelOffset += width_lod * imageInfo->arraySize
                                 ...
                                 bool floatCoords, ExplicitType outputType)
{
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
                 ...
                 imageInfo.arraySize++)
            {
                ...
                int retCode = test_read_image_1D_array(
                context, queue, kernel, &imageInfo, imageSampler, floatCoords,
                outputType, seed);
    ...
    else if (gTestMaxImages)
    {
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            imageInfo.arraySize =
            sizes[idx][2]; // 3rd dimension in get_max_sizes
            imageInfo.rowPitch = imageInfo.slicePitch =
            imageInfo.width * pixelSize;
    ...
    else if (gTestRounding)
    {
        ...
        do
        {
            ...
            gRoundingStartValue += imageInfo.width * imageInfo.arraySize
        ...
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    size = (size_t)imageInfo.rowPitch
                    * (size_t)imageInfo.arraySize * 4;
                }
