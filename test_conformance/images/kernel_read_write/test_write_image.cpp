...
extern bool gDebugTrace, gDisableOffsets, gTestSmallImages, gEnablePitch,
gTestMaxImages, gTestImage2DFromBuffer, gTestMipmaps;
...
extern int test_write_image_2D_array_set(cl_device_id device,
                                         ...
                                         cl_image_format *format,
                                         ExplicitType inputType, MTdata d);
extern bool validate_float_write_results(float *expected, float *actual,
...
"   int width_lod = ( get_image_width(output) >> lod ) ? ( "
"get_image_width(output) >> lod ) : 1;\n"
                     ...
                     MTdata d)
{
    ...
    const cl_mem_flags write_only_mem_flag_types[2] = { CL_MEM_WRITE_ONLY,
                                                        CL_MEM_READ_WRITE };
    const char *write_only_mem_flag_names[2] = { "CL_MEM_WRITE_ONLY",
                                                 "CL_MEM_READ_WRITE" };
    ...
    if (gtestTypesToRun & kWriteTests)
    {
        ...
        num_flags = sizeof(write_only_mem_flag_types)
        / sizeof(write_only_mem_flag_types[0]);
    }
    else
    {
        mem_flag_types = read_write_mem_flag_types;
        mem_flag_names = read_write_mem_flag_names;
        num_flags = sizeof(read_write_mem_flag_types)
        / sizeof(read_write_mem_flag_types[0]);
    }

    size_t pixelSize = get_pixel_size(imageInfo->format);
    int channel_scale =
    (imageInfo->format->image_channel_order == CL_DEPTH) ? 1 : 4;

    for (size_t mem_flag_index = 0; mem_flag_index < num_flags;
         mem_flag_index++)
    {
        int error;
        size_t threads[2];
        ...
        // Require Apple's CPU implementation to be correctly rounded, not just
        // within 0.6
        ...
        create_random_image_data(inputType, imageInfo, imageValues, d,
                                 gTestImage2DFromBuffer);

        if (!gTestMipmaps)
        {
            if (inputType == kFloat
                && imageInfo->format->image_channel_data_type != CL_FLOAT
                && imageInfo->format->image_channel_data_type != CL_HALF_FLOAT)
            {
                ...
                {
                    // We want to generate ints (mostly) in range of the target
                    // format which should be [0,255] However the range chosen
                    // here is [-test_range_ext, 255 + test_range_ext] so that
                    ...
                    {
                        float *inputValues =
                        (float *)(char *)imageValues + imageInfo->width * y * 4;
                        for (size_t i = 0; i < imageInfo->width * 4; i++)
                        {
                            pixel_value =
                            random_in_range(formatMin, (int)formatMax, d);
                    ...
                    // Also fill in the first few vectors with some deliberate
                    // tests to determine the rounding mode is correct
                    if (imageInfo->width > 12)
                    {
                        float formatMax =
                        (float)get_format_max_int(imageInfo->format);
                ...
                else
                {
                    ...
                    {
                        float *inputValues = (float *)(char *)imageValues
                        + imageInfo->width * y * channel_scale;
                        for (size_t i = 0; i < imageInfo->width * channel_scale;
                             i++)
        ...
        if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
        {
            if (gTestImage2DFromBuffer)
            {
                ...
                context, mem_flag_types[mem_flag_index] | CL_MEM_USE_HOST_PTR,
                ...
                context, mem_flag_types[mem_flag_index], imageInfo->format,
                imageInfo->width, imageInfo->height, imageInfo->rowPitch,
                imageBuffer, &error);
            }
            else
            {
                // clProtectedImage uses USE_HOST_PTR, so just rely on that for
                // the testing (via Ian) Do not use protected images for max
                // image size test since it rounds the row size to a page size
                if (gTestMaxImages)
                {
                    create_random_image_data(inputType, imageInfo,
                                             maxImageUseHostPtrBackingStore, d);

                    unprotImage = create_image_2d(
                    context,
                    mem_flag_types[mem_flag_index] | CL_MEM_USE_HOST_PTR,
            ...
            if (error != CL_SUCCESS)
            {
                ...
                log_error("ERROR: Unable to create 2D image of size %ld x %ld "
                          "pitch %ld (%s, %s)\n",
                          imageInfo->width, imageInfo->height,
        ...
        else // Either CL_MEM_ALLOC_HOST_PTR, CL_MEM_COPY_HOST_PTR or none
        {
            ...
            {
                // Note: if ALLOC_HOST_PTR is used, the driver allocates memory
                // that can be accessed by the host, but otherwise it works just
                // as if no flag is specified, so we just do the same thing
                // either way Note: if the flags is really CL_MEM_COPY_HOST_PTR,
                // we want to remove it, because we don't want to copy any
                // incoming data
                unprotImage =
                create_image_2d(context,
                                mem_flag_types[mem_flag_index]
                                | (gMemFlagsToUse & ~(CL_MEM_COPY_HOST_PTR)),
                                imageInfo->format, imageInfo->width,
                                imageInfo->height, 0, imageValues, &error);
            }
            if (error != CL_SUCCESS)
            {
                if (gTestImage2DFromBuffer)
        ...
        for (int lod = 0; lod < num_lod_loops; lod++)
        {
            ...
            for (size_t y = 0, i = 0; y < height_lod; y++)
            {
                ...
                if (gTestMipmaps)
                    resultPtr =
                    (char *)resultValues + y * width_lod * pixelSize;
                else
                    resultPtr = (char *)resultValues + y * imageInfo->rowPitch;
                for (size_t x = 0; x < width_lod; x++, i++)
                {
                    char resultBuffer[16]; // Largest format would be 4 channels
                                           // * 4 bytes (32 bits) each

                    // Convert this pixel
                    if (inputType == kFloat)
                        pack_image_pixel((float *)imagePtr, imageInfo->format,
                                         resultBuffer);
                    else if (inputType == kInt)
                        pack_image_pixel((int *)imagePtr, imageInfo->format,
                                         resultBuffer);
                    else // if( inputType == kUInt )
                        pack_image_pixel((unsigned int *)imagePtr,
                                         imageInfo->format, resultBuffer);
                    ...
                    if (is_sRGBA_order(imageInfo->format->image_channel_order))
                    {
                        ...
                        for (unsigned int j = 0;
                             j < get_format_channel_count(imageInfo->format);
                             j++)
                        {
                            if (j < 3)
                            {
                            ...
                            else // there is no sRGB conversion for alpha
                                 // component if it exists
                            {
                                expected[j] =
                                NORMALIZE(input_values[j], 255.0f);
                            }

                            err[j] = fabsf(expected[j] - actual[j]);
                        }

                        if ((err[0] > max_err) || (err[1] > max_err)
                            || (err[2] > max_err)
                            || (err[3] > 0)) // there is no conversion for alpha
                                             // so the error should be zero
                        {
                            ...
                            log_error(
                            "       Input:     %g %g %g %g\n",
                            *((float *)imagePtr), *((float *)imagePtr + 1),
                            *((float *)imagePtr + 2), *((float *)imagePtr + 3));
                             ...
                             == CL_FLOAT)
                    {
                        ...
                        if (!validate_float_write_results(expected, actual,
                                                          imageInfo))
                        {
                            unsigned int *e = (unsigned int *)resultBuffer;
                            unsigned int *a = (unsigned int *)resultPtr;
                            log_error("ERROR: Sample %ld (%ld,%ld) did not "
                                      "validate! (%s)\n",
                             ...
                             == CL_HALF_FLOAT)
                    {
                        ...
                        // Compare half floats
                        if (memcmp(
                            resultBuffer, resultPtr,
                            2 * get_format_channel_count(imageInfo->format))
                            != 0)
                        {
                            ...
                            if (err_cnt)
                            {
                                ...
                                log_error("ERROR: Sample %ld (%ld,%ld) did not "
                                          ...
                                          mem_flag_names[mem_flag_index]);
                                log_error(
                                "    Expected: 0x%04x 0x%04x 0x%04x 0x%04x\n",
                                e[0], e[1], e[2], e[3]);
                                log_error(
                                "    Actual:   0x%04x 0x%04x 0x%04x 0x%04x\n",
                                a[0], a[1], a[2], a[3]);
                                if (inputType == kFloat)
                                {
                    ...
                    else
                    {
                        ...
                        if (memcmp(resultBuffer, resultPtr,
                            ...
                            != 0)
                        {
                            ...
                            pack_image_pixel_error((float *)imagePtr,
                                                   imageInfo->format,
                                                   resultBuffer, errors);

                            // We are allowed 0.6 absolute error vs. infinitely
                            // precise for some normalized formats
                            ...
                            if (failure)
                            {
                                ...
                                {
                                    // Try to guess what the rounding mode of
                                    // the device really is based on what it
                                    // returned
                                    const char *deviceRounding = "unknown";
                                    unsigned int deviceResults[8];
                                    read_image_pixel<unsigned int>(
                                    resultPtr, imageInfo, 0, 0, 0,
                                    deviceResults, lod);
                                    read_image_pixel<unsigned int>(
                                    resultPtr, imageInfo, 1, 0, 0,
                                    &deviceResults[4], lod);

                                    if (deviceResults[0] == 4
                                        && deviceResults[1] == 4
                                        && deviceResults[2] == 4
                                        && deviceResults[3] == 4
                                        && deviceResults[4] == 5
                                    ...
                                    log_error(
                                    "ERROR: Rounding mode sample (%ld) did not "
                                    "validate, probably due to the device's "
                                    "rounding mode being wrong (%s)\n",
                                    i, mem_flag_names[mem_flag_index]);
                                    log_error(
                                    "       Actual values rounded by device: "
                                    "%x %x %x %x %x %x %x %x\n",
                                    deviceResults[0], deviceResults[1],
                                    deviceResults[2], deviceResults[3],
                                    deviceResults[4], deviceResults[5],
                                    deviceResults[6], deviceResults[7]);
                                    log_error("       Rounding mode of device "
                                              "appears to be %s\n",
                                              deviceRounding);
                                    return 1;
                                }
                                log_error(
                                "ERROR: Sample %d (%d,%d) did not validate!\n",
                                (int)i, (int)x, (int)y);
                                switch (
                                imageInfo->format->image_channel_data_type)
                                {
                                    ...
                                    case CL_UNORM_INT_101010:
                                        log_error(
                                        "    Expected: 0x%2.2x 0x%2.2x 0x%2.2x "
                                        "0x%2.2x\n",
                                        ((cl_uchar *)resultBuffer)[0],
                                        ((cl_uchar *)resultBuffer)[1],
                                        ((cl_uchar *)resultBuffer)[2],
                                        ((cl_uchar *)resultBuffer)[3]);
                                        log_error("    Actual:   0x%2.2x "
                                                  "0x%2.2x 0x%2.2x 0x%2.2x\n",
                                                  ((cl_uchar *)resultPtr)[0],
                                                  ((cl_uchar *)resultPtr)[1],
                                                  ((cl_uchar *)resultPtr)[2],
                                                  ((cl_uchar *)resultPtr)[3]);
...
#endif
                                        log_error(
                                        "    Expected: 0x%4.4x 0x%4.4x 0x%4.4x "
                                        "0x%4.4x\n",
                                        ((cl_ushort *)resultBuffer)[0],
                                        ((cl_ushort *)resultBuffer)[1],
                                        ((cl_ushort *)resultBuffer)[2],
                                        ((cl_ushort *)resultBuffer)[3]);
                                        log_error("    Actual:   0x%4.4x "
                                                  "0x%4.4x 0x%4.4x 0x%4.4x\n",
                                                  ((cl_ushort *)resultPtr)[0],
                                                  ((cl_ushort *)resultPtr)[1],
                                                  ((cl_ushort *)resultPtr)[2],
                                                  ((cl_ushort *)resultPtr)[3]);
                                        log_error("    Error:    %f %f %f %f\n",
                                                  errors[0], errors[1],
                                                  errors[2], errors[3]);
                                        break;
                                    ...
                                    case CL_SIGNED_INT32:
                                        log_error("    Expected: 0x%8.8x "
                                                  "0x%8.8x 0x%8.8x 0x%8.8x\n",
                                                  ((cl_uint *)resultBuffer)[0],
                                                  ((cl_uint *)resultBuffer)[1],
                                                  ((cl_uint *)resultBuffer)[2],
                                                  ((cl_uint *)resultBuffer)[3]);
                                        log_error("    Actual:   0x%8.8x "
                                                  "0x%8.8x 0x%8.8x 0x%8.8x\n",
                                                  ((cl_uint *)resultPtr)[0],
                                                  ((cl_uint *)resultPtr)[1],
                                                  ((cl_uint *)resultPtr)[2],
                                    ...
                                    case CL_FLOAT:
                                        log_error(
                                        "    Expected: %a %a %a %a\n",
                                        ((cl_float *)resultBuffer)[0],
                                        ((cl_float *)resultBuffer)[1],
                                        ((cl_float *)resultBuffer)[2],
                                        ((cl_float *)resultBuffer)[3]);
                                        log_error("    Actual:   %a %a %a %a\n",
                                                  ((cl_float *)resultPtr)[0],
                                                  ((cl_float *)resultPtr)[1],
                                                  ((cl_float *)resultPtr)[2],
                                                  ((cl_float *)resultPtr)[3]);
                                        log_error("    Ulps:     %f %f %f %f\n",
                                                  errors[0], errors[1],
                                                  errors[2], errors[3]);
                                        break;
                                ...
                                log_error("      : %a %a %a %a\n", v[0], v[1],
                                          v[2], v[3]);
                                log_error(
                                "   src: %12.24f %12.24f %12.24f %12.24f\n",
                                v[0], v[1], v[2], v[3]);
                         ...
                         ExplicitType inputType, MTdata d)
{
    ...
    else
    {
        ...
        if (gTestImage2DFromBuffer)
        {
            ...
            if (!imagePitchAlign) imagePitchAlign = 1;
#endif
            test_error(
            error, "Unable to get CL_DEVICE_IMAGE_PITCH_ALIGNMENT from device");
        }
        ...
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            {
                imageInfo.width =
                (size_t)random_log_in_range(16, (int)maxWidth / 32, d);
                imageInfo.height =
                (size_t)random_log_in_range(16, (int)maxHeight / 32, d);
                ...
                else
                {
                    ...
                    if (gEnablePitch)
                    {
                        ...
                        imageInfo.rowPitch +=
                        extraWidth * get_pixel_size(imageInfo.format);
                    }
                    ...
                    {
                        size_t pitch =
                        imagePitchAlign * get_pixel_size(imageInfo.format);
                        imageInfo.rowPitch =
...
int test_write_image_formats(cl_device_id device, cl_context context,
                             cl_command_queue queue,
                             cl_image_format *formatList, bool *filterFlags,
    ...
    for (unsigned int i = 0; i < numFormats; i++)
    {
        ...
        switch (imageType)
        {
            case CL_MEM_OBJECT_IMAGE1D:
                ...
                device, context, queue, &imageFormat, inputType, seed);
                break;
            case CL_MEM_OBJECT_IMAGE2D:
                retCode = test_write_image_set(device, context, queue,
                                               &imageFormat, inputType, seed);
                break;
