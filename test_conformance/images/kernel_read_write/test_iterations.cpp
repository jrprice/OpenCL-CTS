...
extern bool gDebugTrace, gExtraValidateInfo, gDisableOffsets, gTestSmallImages,
gEnablePitch, gTestMaxImages, gTestImage2DFromBuffer, gTestMipmaps;
...
"__global float *yOffsets, __global %s%s *results %s)\n"
"{\n"
"%s"
...
"__kernel void sample_kernel( read_write %s input,%s __global float *xOffsets, "
"__global float *yOffsets, __global %s%s *results %s)\n"
"{\n"
"%s"
"   int tidX = get_global_id(0), tidY = get_global_id(1);\n"
...
"   float2 coords = (float2)( (float)( xOffsets[offset] ), (float)( "
"yOffsets[offset] ) );\n";
...
"   int width_lod = (get_image_width(input) >> lod_int) "
"?(get_image_width(input) >> lod_int):1 ;\n"
                               ...
                               bool printAsFloat, int lod = 0)
{
    ...
    x, y, 0.f, xAddressOffset, yAddressOffset, 0.0f, imageInfo->width,
    imageInfo->height, 0, imageSampler, imageInfo, clampedX, clampedY,
    ignoreMe);
        ...
        if (clamped && clampedX == actualX && clampedY == actualY)
        {
            if ((--numClamped) == 0)
            {
                log_error("ERROR: TEST FAILED: Read is erroneously clamping "
                          "coordinates for image size %ld x %ld!\n",
                          imageInfo->width, imageInfo->height);
                if (imageInfo->format->image_channel_order == CL_DEPTH)
                {
                    if (printAsFloat)
                    {
                        log_error("Sample %d: coord {%f(%.6a), %f(%.6a)} did "
                                  "not validate!\n\tExpected (%g),\n\tgot      "
                                  "(%g),\n\terror of %g\n",
                                  (int)j, x, x, y, y, (float)expected[0],
                                  (float)resultPtr[0], error);
                    }
                    else
                    {
                        log_error(
                        "Sample %d: coord {%f(%.6a), %f(%.6a)} did not "
                        "validate!\n\tExpected (%x),\n\tgot      (%x)\n",
                        (int)j, x, x, y, y, (int)expected[0],
                        (int)resultPtr[0]);
                    }
                }
                else
                {
                    ...
                    else
                    {
                        log_error("Sample %d: coord {%f(%.6a), %f(%.6a)} did "
                                  ...
                                  (int)expected[3], (int)resultPtr[0],
            ...
            && resultPtr[3] == 0)
        {
            if ((--numClamped) == 0)
            {
                log_error("ERROR: TEST FAILED: Clamping is erroneously "
                          "returning border color for image size %ld x %ld!\n",
                          imageInfo->width, imageInfo->height);
                if (imageInfo->format->image_channel_order == CL_DEPTH)
                {
                    if (printAsFloat)
    ...
    if (!clampingErr)
    {
        if (imageInfo->format->image_channel_order == CL_DEPTH)
        {
            if (printAsFloat)
            {
                log_error("Sample %d: coord {%f(%.6a), %f(%.6a)} did not "
                          "validate!\n\tExpected (%g),\n\tgot      "
                          "(%g),\n\terror of %g\n",
                          (int)j, x, x, y, y, (float)expected[0],
                          (float)resultPtr[0], error);
        ...
        log_error("img size %ld,%ld (pitch %ld)", imageInfo->width,
        ...
        if (printAsFloat && gExtraValidateInfo)
        {
            ...
            for (int yOff = -2; yOff <= 1; yOff++)
            {
                ...
                {
                    log_error("%d\t(%g,%g,%g,%g)", clampedY + yOff, top[0],
                              top[1], top[2], top[3]);
            ...
            if (clampedY < 1)
            {
                ...
                log_error("\t%d\t%d\t%d\t%d\n", clampedX - 2, clampedX - 1,
                          clampedX, clampedX + 1);
                for (int yOff = (int)imageInfo->height - 2;
                     yOff <= (int)imageInfo->height + 1; yOff++)
                {
                    float top[4], real[4], bot[4], bot2[4];
                    read_image_pixel_float(imagePtr, imageInfo, clampedX - 2,
                                           clampedY + yOff, 0, top);
                    ...
                    else
                    {
                        ...
                        log_error(" (%g,%g,%g,%g)", real[0], real[1], real[2],
                                  real[3]);
                        log_error(" (%g,%g,%g,%g)", bot[0], bot[1], bot[2],
                                  bot[3]);
                        log_error(" (%g,%g,%g,%g)\n", bot2[0], bot2[1], bot2[2],
                                  bot2[3]);
                    }
...
static void InitFloatCoords(image_descriptor *imageInfo,
                            ...
                            int normalized_coords, MTdata d)
{
    size_t i = 0;
    if (gDisableOffsets)
    {
    ...
    else
    {
        for (size_t y = 0; y < imageInfo->height; y++)
        {
            for (size_t x = 0; x < imageInfo->width; x++, i++)
            {
                xOffsets[i] =
                (float)(xfract
                        + (double)((int)x + random_in_range(-10, 10, d)));
                yOffsets[i] =
                (float)(yfract
                        + (double)((int)y + random_in_range(-10, 10, d)));
            }
        }
    }
    ...
    if (normalized_coords)
    {
        ...
        for (size_t y = 0; y < imageInfo->height; y++)
        {
            ...
            {
                xOffsets[i] =
                (float)((double)xOffsets[i] / (double)imageInfo->width);
                yOffsets[i] =
                (float)((double)yOffsets[i] / (double)imageInfo->height);
            }
        }
    }
}
                            ...
                            int normalized_coords, MTdata d, size_t lod)
{
    ...
    if (imageSampler->addressing_mode == CL_ADDRESS_NONE)
    {
        ...
        for (size_t y = 0; y < height_lod; y++)
        {
            ...
            {
                xOffsets[i] =
                (float)CLAMP((double)xOffsets[i], 0.0, (double)width_lod - 1.0);
                yOffsets[i] = (float)CLAMP((double)yOffsets[i], 0.0,
                                           (double)height_lod - 1.0);
            }
        }
    }
...
image_descriptor *imageInfo, size_t lod, char *imagePtr)
{
    ...
    if (outputType == kFloat)
    {
        ...
        float maxErr =
        get_max_relative_error(imageInfo->format, imageSampler, 0 /*not 3D*/,
                               CL_FILTER_LINEAR == imageSampler->filter_mode);
        for (size_t y = 0, j = 0; y < height_lod; y++)
        {
            for (size_t x = 0; x < width_lod; x++, j++)
            {
                // Step 1: go through and see if the results verify for the
                // pixel For the normalized case on a GPU we put in offsets to
                // the X and Y to see if we land on the right pixel. This
                // addresses the significant inaccuracy in GPU normalization in
                // OpenCL 1.0.
                     ...
                     norm_offset_x += NORM_OFFSET)
                {
                         ...
                         norm_offset_y += NORM_OFFSET)
                    {
                        ...
                        FloatPixel maxPixel;
                        maxPixel = sample_image_pixel_float_offset(
                        imageValues, imageInfo, xOffsetValues[j],
                        yOffsetValues[j], 0.0f, norm_offset_x, norm_offset_y,
                        0.0f, imageSampler, expected, 0, &containsDenormals);
                        ...
                        if (!(err1 <= maxErr1))
                        {
                            // try flushing the denormals, if there is a
                            // failure.
                            if (containsDenormals)
                            {
                                // If implementation decide to flush subnormals
                                // to zero, max error needs to be adjusted
                                maxErr1 += 4 * FLT_MIN;

                                maxPixel = sample_image_pixel_float_offset(
                                imageValues, imageInfo, xOffsetValues[j],
                                yOffsetValues[j], 0.f, norm_offset_x,
                                norm_offset_y, 0.0f, imageSampler, expected, 0,
                                NULL);
                        ...
                        // If the final result DOES match, then we've found a
                        // valid result and we're done with this pixel.
                        found_pixel = (err1 <= maxErr1);
                    } // norm_offset_x
                } // norm_offset_y


                // Step 2: If we did not find a match, then print out debugging
                // info.
                if (!found_pixel)
                {
                    // For the normalized case on a GPU we put in offsets to the
                    // X and Y to see if we land on the right pixel. This
                    // addresses the significant inaccuracy in GPU normalization
                    // in OpenCL 1.0.
                    checkOnlyOnePixel = 0;
                    int shouldReturn = 0;
                    for (float norm_offset_x = -offset;
                         norm_offset_x <= offset && !checkOnlyOnePixel;
                         norm_offset_x += NORM_OFFSET)
                    {
                        for (float norm_offset_y = -offset;
                             norm_offset_y <= offset && !checkOnlyOnePixel;
                             norm_offset_y += NORM_OFFSET)
                        {

                            // If we are not on a GPU, or we are not normalized,
                            // then only test with offsets (0.0, 0.0) E.g., test
                            // one pixel.
                            if (!imageSampler->normalized_coords
                                || gDeviceType != CL_DEVICE_TYPE_GPU
                                || NORM_OFFSET == 0)
                            ...
                            maxPixel = sample_image_pixel_float_offset(
                            imageValues, imageInfo, xOffsetValues[j],
                            yOffsetValues[j], 0.f, norm_offset_x, norm_offset_y,
                            0.0f, imageSampler, expected, 0,
                            &containsDenormals);

                            float err1 = fabsf(resultPtr[0] - expected[0]);
                            float maxErr1 =
                            MAX(maxErr * maxPixel.p[0], FLT_MIN);
                            ...
                            if (!(err1 <= maxErr1))
                            {
                                ...
                                if (containsDenormals)
                                {
                                    ...
                                    imageValues, imageInfo, xOffsetValues[j],
                                    yOffsetValues[j], 0.f, norm_offset_x,
                                    norm_offset_y, 0.0f, imageSampler, expected,
                                    0, NULL);
                            ...
                            if (!(err1 <= maxErr1))
                            {
                                ...
                                expected, error, xOffsetValues[j],
                                yOffsetValues[j], norm_offset_x, norm_offset_y,
                                j, numTries, numClamped, true, lod);
                                ...
                                tempOut, 1 /* verbose */, &containsDenormals /*dont flush while error reporting*/);
                                log_error(
                                "\tulps: %2.2f  (max allowed: %2.2f)\n\n",
                                Ulp_Error(resultPtr[0], expected[0]),
                                Ulp_Error(
                                MAKE_HEX_FLOAT(0x1.000002p0f, 0x1000002L, -24)
                                + maxErr,
                                MAKE_HEX_FLOAT(0x1.000002p0f, 0x1000002L,
                                               -24)));
                            }
                            else
                            {
                                log_error("Test error: we should have detected "
                                          "this passing above.\n");
                            }
                     ...
                     norm_offset_x += NORM_OFFSET)
                {
                         ...
                         norm_offset_y += NORM_OFFSET)
                    {
                        ...
                        if (gTestMipmaps)
                            ...
                            yOffsetValues[j], 0.0f, norm_offset_x,
                            norm_offset_y, 0.0f, imageSampler, expected, 0,
                            &containsDenormals, lod);
                        else
                            maxPixel = sample_image_pixel_float_offset(
                            imageValues, imageInfo, xOffsetValues[j],
                            yOffsetValues[j], 0.0f, norm_offset_x,
                        ...
                        if (err2 > 0 && err2 < formatAbsoluteError)
                        {
                            err2 = 0.0f;
                        }
                        if (err3 > 0 && err3 < formatAbsoluteError)
                        {
                            err3 = 0.0f;
                        }
                        if (err4 > 0 && err4 < formatAbsoluteError)
                        {
                            err4 = 0.0f;
                        }
                        float maxErr1 = MAX(maxErr * maxPixel.p[0], FLT_MIN);
                        ...
                        // If the final result DOES match, then we've found a
                        // valid result and we're done with this pixel.
                        found_pixel = (err1 <= maxErr1) && (err2 <= maxErr2)
                        && (err3 <= maxErr3) && (err4 <= maxErr4);
                    } // norm_offset_x
                } // norm_offset_y
                ...
                if (!found_pixel)
                {
                         ...
                         norm_offset_x += NORM_OFFSET)
                    {
                             ...
                             norm_offset_y += NORM_OFFSET)
                        {
                            ...
                            MAX(maxErr * maxPixel.p[0], FLT_MIN);
                            float maxErr2 =
                            MAX(maxErr * maxPixel.p[1], FLT_MIN);
                            float maxErr3 =
                            MAX(maxErr * maxPixel.p[2], FLT_MIN);
                            float maxErr4 =
                            MAX(maxErr * maxPixel.p[3], FLT_MIN);
                                ...
                                || !(err3 <= maxErr3) || !(err4 <= maxErr4))
                            {
                                ...
                                if (containsDenormals)
                                {
                                    ...
                                    if (gTestMipmaps)
                                        ...
                                        xOffsetValues[j], yOffsetValues[j], 0.f,
                                        norm_offset_x, norm_offset_y, 0.0f,
                                ...
                                || !(err3 <= maxErr3) || !(err4 <= maxErr4))
                            {
                                ...
                                if (gTestMipmaps)
                                    ...
                                    imagePtr, imageInfo, xOffsetValues[j],
                                    yOffsetValues[j], 0.f, norm_offset_x,
                                    norm_offset_y, 0.0f, imageSampler, tempOut,
                                    1 /* verbose */,
                                    &containsDenormals /*dont flush while error
                                                          reporting*/
                                    ,
                                    lod);
                                else
            ...
            for (size_t x = 0; x < width_lod; x++, j++)
            {
                     ...
                     norm_offset_x += NORM_OFFSET)
                {
                    for (float norm_offset_y = -NORM_OFFSET;
                         norm_offset_y <= NORM_OFFSET && !found_pixel
                         && !checkOnlyOnePixel;
                         norm_offset_y += NORM_OFFSET)
                    {

                        // If we are not on a GPU, or we are not normalized,
                        // then only test with offsets (0.0, 0.0) E.g., test one
                        // pixel.
                        if (!imageSampler->normalized_coords
                            || gDeviceType != CL_DEVICE_TYPE_GPU
                            || NORM_OFFSET == 0)
                        ...
                        if (gTestMipmaps)
                            ...
                            (char *)imagePtr, imageInfo, xOffsetValues[j],
                            yOffsetValues[j], 0.f, norm_offset_x, norm_offset_y,
                            0.0f, imageSampler, expected, lod);
                        else
                            sample_image_pixel_offset<unsigned int>(
                            imagePtr, imageInfo, xOffsetValues[j],
                            yOffsetValues[j], 0.f, norm_offset_x, norm_offset_y,
                            0.0f, imageSampler, expected);


                        error = errMax(
                        errMax(abs_diff_uint(expected[0], resultPtr[0]),
                               abs_diff_uint(expected[1], resultPtr[1])),
                        errMax(abs_diff_uint(expected[2], resultPtr[2]),
                               abs_diff_uint(expected[3], resultPtr[3])));
                ...
                if (!found_pixel)
                {
                    ...
                    int shouldReturn = 0;
                    for (float norm_offset_x = -NORM_OFFSET;
                         norm_offset_x <= NORM_OFFSET && !checkOnlyOnePixel;
                         norm_offset_x += NORM_OFFSET)
                    {
                        for (float norm_offset_y = -NORM_OFFSET;
                             norm_offset_y <= NORM_OFFSET && !checkOnlyOnePixel;
                             norm_offset_y += NORM_OFFSET)
                        {
            ...
            for (size_t x = 0; x < width_lod; x++, j++)
            {
                     ...
                     norm_offset_x += NORM_OFFSET)
                {
                         ...
                         norm_offset_y += NORM_OFFSET)
                    {
                        ...
                        error =
                        errMax(errMax(abs_diff_int(expected[0], resultPtr[0]),
                                      abs_diff_int(expected[1], resultPtr[1])),
                               errMax(abs_diff_int(expected[2], resultPtr[2]),
                                      abs_diff_int(expected[3], resultPtr[3])));
                ...
                if (!found_pixel)
                {
                         ...
                         norm_offset_x += NORM_OFFSET)
                    {
                             ...
                             norm_offset_y += NORM_OFFSET)
                        {
                            ...
                            error = errMax(
                            errMax(abs_diff_int(expected[0], resultPtr[0]),
                                   abs_diff_int(expected[1], resultPtr[1])),
                            errMax(abs_diff_int(expected[2], resultPtr[2]),
                                   abs_diff_int(expected[3], resultPtr[3])));
                     ...
                     norm_offset_x += NORM_OFFSET)
                {
                         ...
                         norm_offset_y += NORM_OFFSET)
                    {
                        ...
                        if (gTestMipmaps)
                            ...
                            &containsDenormals);
                        float err1 =
                        fabsf(sRGBmap(resultPtr[0]) - sRGBmap(expected[0]));
                        float err2 =
                        fabsf(sRGBmap(resultPtr[1]) - sRGBmap(expected[1]));
                        float err3 =
                        fabsf(sRGBmap(resultPtr[2]) - sRGBmap(expected[2]));
                        float err4 = fabsf(resultPtr[3] - expected[3]);
                        float maxErr = 0.5;
                            ...
                            || !(err3 <= maxErr) || !(err4 <= maxErr))
                        {
                            ...
                            if (containsDenormals)
                            {
                                ...
                                err1 = fabsf(sRGBmap(resultPtr[0])
                                             - sRGBmap(expected[0]));
                                err2 = fabsf(sRGBmap(resultPtr[1])
                                             - sRGBmap(expected[1]));
                                err3 = fabsf(sRGBmap(resultPtr[2])
                                             - sRGBmap(expected[2]));
                                err4 = fabsf(resultPtr[3] - expected[3]);
                            }
                        }
                        ...
                        // If the final result DOES match, then we've found a
                        // valid result and we're done with this pixel.
                        found_pixel = (err1 <= maxErr) && (err2 <= maxErr)
                        && (err3 <= maxErr) && (err4 <= maxErr);
                    } // norm_offset_x
                } // norm_offset_y
                         ...
                         norm_offset_x += NORM_OFFSET)
                    {
                             ...
                             norm_offset_y += NORM_OFFSET)
                        {
                                ...
                                || !(err3 <= maxErr) || !(err4 <= maxErr))
                            {
                                ...
                                if (containsDenormals)
                                {
                                    // If implementation decide to flush
                                    // subnormals to zero, max error needs to be
                                    // adjusted
                                    maxErr += 4 * FLT_MIN;
                                    if (gTestMipmaps)
                                        maxPixel =
...
bool validate_float_write_results(float *expected, float *actual,
                                  image_descriptor *imageInfo)
{
    bool pass = true;
    // Compare floats
    if (memcmp(expected, actual,
               4 * get_format_channel_count(imageInfo->format))
        != 0)
    {
        // 8.3.3 Fix up cases where we have NaNs or flushed denorms; "all other
        // values must be preserved"
        for (size_t j = 0; j < get_format_channel_count(imageInfo->format); j++)
        {
            if (isnan(expected[j]) && isnan(actual[j])) continue;
            if (IsFloatSubnormal(expected[j]) && actual[j] == 0.0f) continue;
            pass = false;
            break;
        }
    }
                       ...
                       ExplicitType outputType, MTdata d)
{
    ...
    // generate_random_image_data allocates with malloc, so we use a
    // MallocDataBuffer here
    ...
    if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
    {
        if (gTestImage2DFromBuffer)
        {
            ...
            clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
                           imageInfo->rowPitch * imageInfo->height,
                           maxImageUseHostPtrBackingStore, &error);
            test_error(error, "Unable to create buffer");
            unprotImage = create_image_2d_buffer(
            context, image_read_write_flags, imageInfo->format,
        ...
        else
        {
            ...
            // testing (via Ian) Do not use protected images for max image size
            // test since it rounds the row size to a page size
            if (gTestMaxImages)
            {
                generate_random_image_data(imageInfo,
                                           maxImageUseHostPtrBackingStore, d);
        ...
        if (error != CL_SUCCESS)
        {
            if (gTestImage2DFromBuffer)
            {
                ...
                {
                    log_info("Format not supported for "
                             "cl_khr_image2d_from_buffer skipping...\n");
                    return 0;
                }
            }

            log_error(
            "ERROR: Unable to create 2D image of size %d x %d pitch %d (%s)\n",
    ...
    else if (gMemFlagsToUse == CL_MEM_COPY_HOST_PTR)
    {
        ...
        {
            // Don't use clEnqueueWriteImage; just use copy host ptr to get the
            // data in
            unprotImage = create_image_2d(
            context, image_read_write_flags | CL_MEM_COPY_HOST_PTR,
            imageInfo->format, imageInfo->width, imageInfo->height,
            (gEnablePitch ? imageInfo->rowPitch : 0), imageValues, &error);
        }
        if (error != CL_SUCCESS)
        {
            if (gTestImage2DFromBuffer)
    ...
    else // Either CL_MEM_ALLOC_HOST_PTR or none
    {
        if (gTestMipmaps)
        {
            ...
            unprotImage =
            clCreateImage(context, CL_MEM_READ_ONLY, imageInfo->format,
                          &image_desc, NULL, &error);
        }
        else if (gTestImage2DFromBuffer)
        {
            imageBuffer = clCreateBuffer(
            context, CL_MEM_READ_WRITE | gMemFlagsToUse,
            imageInfo->rowPitch * imageInfo->height, imageValues, &error);
            test_error(error, "Unable to create buffer");
            unprotImage = create_image_2d_buffer(
            context, image_read_write_flags, imageInfo->format,
        ...
        {
            // Note: if ALLOC_HOST_PTR is used, the driver allocates memory that
            // can be accessed by the host, but otherwise it works just as if no
            // flag is specified, so we just do the same thing either way
            unprotImage = create_image_2d(
            context, image_read_write_flags | gMemFlagsToUse, imageInfo->format,
            imageInfo->width, imageInfo->height,
            (gEnablePitch ? imageInfo->rowPitch : 0), imageValues, &error);
        }
        if (error != CL_SUCCESS)
    ...
    if (gMemFlagsToUse != CL_MEM_COPY_HOST_PTR)
    {
        ...
        if (!gTestMipmaps)
        {
            ...
            {
                log_error(
                "ERROR: Unable to write to 2D image of size %d x %d\n",
                (int)imageInfo->width, (int)imageInfo->height);
                return error;
            }
        }
        else
        {
            ...
            for (size_t level = 0; level < imageInfo->num_mip_levels; level++)
            {
                ...
                ((gEnablePitch || gTestImage2DFromBuffer) ? imageInfo->rowPitch
                                                          : 0),
                0, (char *)imageValues + tmpNextLevelOffset, 0, NULL, NULL);
                tmpNextLevelOffset +=
                region[0] * region[1] * get_pixel_size(imageInfo->format);
    ...
    if (gDebugTrace) log_info(" - Creating kernel arguments...\n");
         ...
         lod++)
    {
        ...
        width_lod * height_lod * get_explicit_type_size(outputType) * 4;
        ...
        for (int q = 0; q < loopCount; q++)
        {
            ...
            error = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, threads,
                                           NULL, 0, NULL, NULL);
            test_error(error, "Unable to run kernel");

            if (gDebugTrace)
                log_info("    reading results, %ld kbytes\n",
                         (unsigned long)(width_lod * height_lod
            ...
            width_lod * height_lod * get_explicit_type_size(outputType) * 4,
            resultValues, 0, NULL, NULL); // XXX check
        ...
        {
            nextLevelOffset +=
            width_lod * height_lod * get_pixel_size(imageInfo->format);
                           ...
                           ExplicitType outputType)
{
        ...
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                if (gTestMipmaps)
                    ...
                    2,
                    compute_max_mip_levels(imageInfo.width, imageInfo.height, 0)
                    - 1,
                    seed);
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            if (gDebugTrace)
                log_info("   at max size %d,%d\n", (int)sizes[idx][0],
                         (int)sizes[idx][1]);
            int retCode =
            test_read_image_2D(context, queue, kernel, &imageInfo, imageSampler,
    ...
    {
        uint64_t typeRange = 1LL
        << (get_format_type_size(imageInfo.format) * 8);
        ...
        while (imageInfo.width >= maxWidth / 2) imageInfo.width >>= 1;
        imageInfo.rowPitch = imageInfo.width * pixelSize;
        ...
        {
            if (gDebugTrace)
                log_info("   at size %d,%d, starting round ramp at %llu for "
                         "range %llu\n",
                         (int)imageInfo.width, (int)imageInfo.height,
            ...
            gRoundingStartValue += imageInfo.width * imageInfo.height
    ...
    else
    {
        ...
        if (gTestImage2DFromBuffer)
        {
            ...
            error = clGetDeviceInfo(device, CL_DEVICE_IMAGE_PITCH_ALIGNMENT,
                                    sizeof(cl_uint), &imagePitchAlign, NULL);
#endif
            if (!imagePitchAlign || error)
            {
                test_error(
                error,
                "Unable to get CL_DEVICE_IMAGE_PITCH_ALIGNMENT from device");
                imagePitchAlign = 1;
            }
        }
        ...
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            {
                imageInfo.width =
                (size_t)random_log_in_range(16, maxWidthRange, seed);
                imageInfo.height =
                (size_t)random_log_in_range(16, maxHeightRange, seed);
                ...
                else
                {
                    ...
                    // if we are creating a 2D image from a buffer, make sure
                    // that the rowpitch is aligned to
                    // CL_DEVICE_IMAGE_PITCH_ALIGNMENT_APPLE
                    if (gTestImage2DFromBuffer)
                    {
                        size_t pitch = imagePitchAlign * pixelSize;
                        imageInfo.rowPitch =
                        ((imageInfo.rowPitch + pitch - 1) / pitch) * pitch;
                    }

                    size =
                    (size_t)imageInfo.rowPitch * (size_t)imageInfo.height * 4;
                }
