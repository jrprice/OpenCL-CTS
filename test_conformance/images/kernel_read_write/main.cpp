...
/** read_write images only support sampler-less read buildt-ins which require
 * special settings for some global parameters. This pair of functions
 * temporarily overwrite those global parameters and then recover them after
 * completing a read_write test.
 */
static void overwrite_global_params_for_read_write_test(
bool *tTestMipmaps, bool *tDisableOffsets, bool *tNormalizedModeToUse,
cl_filter_mode *tFilterModeToUse)
{
    log_info("Overwrite global settings for read_write image tests. The "
             "overwritten values:\n");
    log_info(
    "gTestMipmaps = false, gDisableOffsets = true, gNormalizedModeToUse = "
    "false, gFilterModeToUse = CL_FILTER_NEAREST\n");
    // mipmap images only support sampler read built-in while read_write images
    // only support sampler-less read built-in. Hence we cannot test mipmap for
    // read_write image.
    *tTestMipmaps = gTestMipmaps;
    gTestMipmaps = false;

    // Read_write images are read by sampler-less read which does not handle
    // out-of-bound read It's application responsibility to make sure that the
    // read happens in-bound Therefore we should not enable offset in testing
    // read_write images because it will cause out-of-bound
...
/** Recover the global settings overwritten for read_write tests. This is
 * necessary because there may be other tests (i.e. read or write) are called
 * together with read_write test.
 */
static void recover_global_params_from_read_write_test(
bool tTestMipmaps, bool tDisableOffsets, bool tNormalizedModeToUse,
cl_filter_mode tFilterModeToUse)
{
                  ...
                  cl_command_queue queue, cl_mem_object_type imageType)
{
    ...
    if (testTypesToRun & kReadTests)
    {
            ...
            && is_extension_available(device, "cl_khr_image2d_from_buffer"))
        {
            ...
            // gTestMaxImages, gTestRounding and gTestMipmaps must be false
            if (gTestSmallImages == false && gTestMaxImages == false
                && gTestRounding == false && gTestMipmaps == false)
            {
    ...
    if ((testTypesToRun & kReadWriteTests) && !gTestMipmaps)
    {
        ...
        overwrite_global_params_for_read_write_test(
        &tTestMipMaps, &tDisableOffsets, &tNormalizedModeToUse,
        &tFilterModeToUse);
        ret += test_image_set(device, context, queue, test_read_image_formats,
                              imageType);
        ...
        tTestMipMaps, tDisableOffsets, tNormalizedModeToUse, tFilterModeToUse);
    }

    return ret;
}

int test_1D(cl_device_id device, cl_context context, cl_command_queue queue,
            int num_elements)
{
    return doTest(device, context, queue, CL_MEM_OBJECT_IMAGE1D);
}
int test_2D(cl_device_id device, cl_context context, cl_command_queue queue,
            int num_elements)
{
    return doTest(device, context, queue, CL_MEM_OBJECT_IMAGE2D);
}
int test_3D(cl_device_id device, cl_context context, cl_command_queue queue,
            int num_elements)
{
    return doTest(device, context, queue, CL_MEM_OBJECT_IMAGE3D);
}
int test_1Darray(cl_device_id device, cl_context context,
                 cl_command_queue queue, int num_elements)
{
    return doTest(device, context, queue, CL_MEM_OBJECT_IMAGE1D_ARRAY);
}
int test_2Darray(cl_device_id device, cl_context context,
                 cl_command_queue queue, int num_elements)
{
...
test_definition test_list[] = {
    ...
    ADD_TEST(1Darray), ADD_TEST(2Darray),
};
...
int main(int argc, const char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "CL_FILTER_NEAREST") == 0
                 || strcmp(argv[i], "NEAREST") == 0)
            gFilterModeToUse = CL_FILTER_NEAREST;
        else if (strcmp(argv[i], "CL_FILTER_LINEAR") == 0
                 || strcmp(argv[i], "LINEAR") == 0)
        ...
        {
            // 2.0 Spec does not allow using mem flags, unnormalized coordinates
            // with mipmapped images
            gTestMipmaps = true;
            gMemFlagsToUse = 0;
            gNormalizedModeToUse = true;
        }
        ...
        else if (strcmp(argv[i], "CL_MEM_COPY_HOST_PTR") == 0
                 || strcmp(argv[i], "COPY_HOST_PTR") == 0)
            gMemFlagsToUse = CL_MEM_COPY_HOST_PTR;
        else if (strcmp(argv[i], "CL_MEM_USE_HOST_PTR") == 0
                 || strcmp(argv[i], "USE_HOST_PTR") == 0)
            gMemFlagsToUse = CL_MEM_USE_HOST_PTR;
        else if (strcmp(argv[i], "CL_MEM_ALLOC_HOST_PTR") == 0
                 || strcmp(argv[i], "ALLOC_HOST_PTR") == 0)
    ...
    if (testTypesToRun == 0) testTypesToRun = kAllTests;
    if (gTypesToTest == 0) gTypesToTest = kTestAllTypes;

    if (gTestSmallImages) log_info("Note: Using small test images\n");

    // On most platforms which support denorm, default is FTZ off. However,
    // on some hardware where the reference is computed, default might be flush
    // denorms to zero e.g. arm. This creates issues in result verification.
    // Since spec allows the implementation to either flush or not flush denorms
    // to zero, an implementation may choose not to flush i.e. return denorm
    // result whereas reference result may be zero (flushed denorm). Hence we
    // need to disable denorm flushing on host side where reference is being
    // computed to make sure we get non-flushed reference result. If
    // implementation returns flushed result, we correctly take care of that in
    // verification code.
    ...
    int ret =
    runTestHarness(argCount, argList, test_num, test_list, true, false, 0);
...
static void printUsage(const char *execName)
{
    ...
    log_info("Options:\n");
    log_info("\n");
    log_info("\tThe following flags specify what kinds of operations to test. "
             "They can be combined; if none are specified, all are tested:\n");
    log_info("\t\tread - Tests reading from an image\n");
    log_info("\t\twrite - Tests writing to an image (can be specified with "
             "read to run both; default is both)\n");
    log_info("\n");
    log_info("\tThe following flags specify the types to test. They can be "
             "combined; if none are specified, all are tested:\n");
    ...
    log_info("\n");
    log_info("\tCL_FILTER_LINEAR - Only tests formats with CL_FILTER_LINEAR "
             "filtering\n");
    log_info("\tCL_FILTER_NEAREST - Only tests formats with CL_FILTER_NEAREST "
             "filtering\n");
    log_info("\n");
    ...
    log_info("\n");
    log_info("\tCL_ADDRESS_CLAMP - Only tests formats with CL_ADDRESS_CLAMP "
             "addressing\n");
    log_info("\tCL_ADDRESS_CLAMP_TO_EDGE - Only tests formats with "
             "CL_ADDRESS_CLAMP_TO_EDGE addressing\n");
    log_info("\tCL_ADDRESS_REPEAT - Only tests formats with CL_ADDRESS_REPEAT "
             "addressing\n");
    log_info("\tCL_ADDRESS_MIRRORED_REPEAT - Only tests formats with "
             "CL_ADDRESS_MIRRORED_REPEAT addressing\n");
    log_info("\n");
    log_info(
    "You may also use appropriate CL_ channel type and ordering constants.\n");
    log_info("\n");
    log_info("\tlocal_samplers - Use samplers declared in the kernel functions "
             "instead of passed in as arguments\n");
    log_info("\n");
    log_info("\tThe following specify to use the specific flag to allocate "
             "images to use in the tests:\n");
    ...
    log_info("\t\tsmall_images - Runs every format through a loop of widths "
             "1-13 and heights 1-9, instead of random sizes\n");
    log_info(
    "\t\tmax_images - Runs every format through a set of size combinations "
    "with the max values, max values - 1, and max values / 128\n");
    log_info("\t\trounding - Runs every format through a single image filled "
             "with every possible value for that image format, to verify "
             "rounding works properly\n");
    log_info("\n");
    log_info("\tno_offsets - Disables offsets when testing reads (can be good "
             "for diagnosing address repeating/clamping problems)\n");
    log_info("\tdebug_trace - Enables additional debug info logging\n");
    log_info("\textra_validate - Enables additional validation failure debug "
             "information\n");
