...
"*xOffsets, __global float *yOffsets, __global float *zOffsets,  __global %s4 "
"*results %s)\n"
"{\n"
"%s"
...
"   int height_lod = (get_image_height(input) >> lod_int) "
        ...
        if (true) // gExtraValidateInfo )
        {
            ...
            if (imageSampler->filter_mode != CL_FILTER_LINEAR)
            {
                if (found)
                    ...
                    actualY, actualZ, (found > 1) ? "NOT unique!!" : "unique");
                else
            ...
            for (size_t z = 0; z < imageInfo->depth; z++)
            {
                for (size_t y = 0; y < imageInfo->height; y++)
                {
                    for (size_t x = 0; x < imageInfo->width; x++, i++)
                    {
                        ...
                        zOffsets[i] =
                        (float)((double)zOffsets[i] / (double)imageInfo->depth);
                    }
                }
            }
        }
                       ...
                       ExplicitType outputType, MTdata d)
{
    ...
    if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
    {
        ...
        else
        {
            ...
            context, (cl_mem_flags)(image_read_write_flags), imageInfo->format,
    ...
    if (gMemFlagsToUse != CL_MEM_COPY_HOST_PTR)
    {
        ...
        if (!gTestMipmaps)
        {
            ...
            {
                log_error(
                "ERROR: Unable to write to 3D image of size %d x %d x %d \n",
                (int)imageInfo->width, (int)imageInfo->height,
                (int)imageInfo->depth);
                return error;
            }
                           ...
                           ExplicitType outputType)
{
    ...
    samplerVar, gTestMipmaps ? offset3DLodKernelSource : offset3DKernelSource,
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                ...
                {
                    if (gTestMipmaps)
                        imageInfo.num_mip_levels = (cl_uint)(
                        2
                        + rand()
                    ...
                    if (gDebugTrace)
                        log_info("   at size %d,%d,%d\n", (int)imageInfo.width,
                                 (int)imageInfo.height, (int)imageInfo.depth);
                    int retCode = test_read_image_3D(
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            if (gDebugTrace)
                log_info("   at max size %d,%d,%d\n", (int)sizes[idx][0],
                         (int)sizes[idx][1], (int)sizes[idx][2]);
            int retCode =
            test_read_image_3D(context, queue, kernel, &imageInfo, imageSampler,
                               floatCoords, outputType, seed);
            if (retCode) return retCode;
        }
    }
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                {
                    // imageInfo.num_mip_levels = (cl_uint)
                    // random_log_in_range(2,
                    // (int)compute_max_mip_levels(imageInfo.width,
                    // imageInfo.depth, imageInfo.depth), seed);
                    imageInfo.num_mip_levels = (cl_uint)(
                    2
                    + rand()
