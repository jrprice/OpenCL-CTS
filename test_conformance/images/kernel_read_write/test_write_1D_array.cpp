...
"__kernel void sample_kernel( __global %s4 *input, write_only image1d_array_t "
"output %s)\n"
"{\n"
"   int tidX = get_global_id(0), tidY = get_global_id(1);\n"
"%s"
         ...
         mem_flag_index++)
    {
        ...
        if (!gTestMipmaps)
        {
                ...
                && imageInfo->format->image_channel_data_type != CL_HALF_FLOAT)
            {
                ...
                if (is_sRGBA_order(imageInfo->format->image_channel_order))
                {
                    ...
                    {
                        float *inputValues =
                        (float *)(char *)imageValues + y * imageInfo->width * 4;
        ...
        if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
        {
            ...
            if (gTestMaxImages)
            {
                ...
                imageInfo->format, imageInfo->width, imageInfo->arraySize, 0, 0,
                maxImageUseHostPtrBackingStore, &error);
            }
            else
            {
                error = protImage.Create(
                context, (cl_mem_object_type)CL_MEM_OBJECT_IMAGE1D_ARRAY,
                mem_flag_types[mem_flag_index], imageInfo->format,
                imageInfo->width, 1, 1, imageInfo->arraySize);
            }
            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to create 1D image array of size %ld "
                          "x %ld pitch %ld (%s, %s)\n",
        ...
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
                 ...
                 imageInfo.arraySize++)
            {
                if (gTestMipmaps)
                    ...
                    2, (compute_max_mip_levels(imageInfo.width, 0, 0) - 1), d);
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                imageInfo.arraySize =
                (size_t)random_log_in_range(16, (int)maxArraySize / 32, d);
