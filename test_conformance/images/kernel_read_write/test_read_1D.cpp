...
gEnablePitch, gTestMaxImages, gTestMipmaps;
...
template <class T>
int determine_validation_error_1D(void *imagePtr, image_descriptor *imageInfo,
                                  image_sampler_data *imageSampler,
                                  T *resultPtr, T *expected, float error,
                                  float x, float xAddressOffset, size_t j,
                                  ...
                                  bool printAsFloat, int lod)
{
    ...
    x, 0.0f, 0.0f, xAddressOffset, 0.0f, 0.0f, imageInfo->width, 0, 0,
    imageSampler, imageInfo, clampedX, ignoreMe, ignoreMe);
        ...
        if (clamped && clampedX == actualX)
        {
            if ((--numClamped) == 0)
            {
                log_error("ERROR: TEST FAILED: Read is erroneously clamping "
                          "coordinates for image size %ld!\n",
                          imageInfo->width);
                if (printAsFloat)
                {
                    log_error("Sample %d: coord {%f(%a)} did not "
                              "validate!\n\tExpected (%g,%g,%g,%g),\n\tgot     "
                              " (%g,%g,%g,%g),\n\terror of %g\n",
                              (int)j, x, x, (float)expected[0],
                ...
                {
                    log_error(
                    "Sample %d: coord {%f(%a)} did not validate!\n\tExpected "
                    "(%x,%x,%x,%x),\n\tgot      (%x,%x,%x,%x)\n",
                    (int)j, x, x, (int)expected[0], (int)expected[1],
            ...
            && resultPtr[3] == 0)
        {
            if ((--numClamped) == 0)
            {
                log_error("ERROR: TEST FAILED: Clamping is erroneously "
                          "returning border color for image size %ld!\n",
                          imageInfo->width);
                if (printAsFloat)
                {
                    log_error("Sample %d: coord {%f(%a)} did not "
    ...
    if (!clampingErr)
    {
        if (printAsFloat)
        {
            log_error("Sample %d: coord {%f(%a)} did not validate!\n\tExpected "
                      ...
                      (int)j, x, x, (float)expected[0], (float)expected[1],
                      (float)expected[2], (float)expected[3],
                      (float)resultPtr[0], (float)resultPtr[1],
                      (float)resultPtr[2], (float)resultPtr[3], error);
        }
        else
        {
            log_error("Sample %d: coord {%f(%a)} did not validate!\n\tExpected "
        ...
        log_error("img size %ld (pitch %ld)", imageInfo->width,
                  imageInfo->rowPitch);
        if (clamped)
        {
        ...
        if (printAsFloat && gExtraValidateInfo)
        {
                      ...
                      clampedX, clampedX + 1);
            {
                ...
                read_image_pixel_float(imagePtr, imageInfo, clampedX - 2, 0, 0,
                                       top);
                read_image_pixel_float(imagePtr, imageInfo, clampedX - 1, 0, 0,
                                       real);
                read_image_pixel_float(imagePtr, imageInfo, clampedX, 0, 0,
                                       bot);
                read_image_pixel_float(imagePtr, imageInfo, clampedX + 1, 0, 0,
                                       bot2);
...
static void InitFloatCoords(image_descriptor *imageInfo,
                            ...
                            float xfract, int normalized_coords, MTdata d,
                            int lod)
{
    size_t i = 0;
    ...
    else
    {
        ...
        {
            xOffsets[i] =
            (float)(xfract + (double)((int)x + random_in_range(-10, 10, d)));
        }
    }
                       ...
                       ExplicitType outputType, MTdata d)
{
    ...
    malloc(sizeof(cl_float) * imageInfo->width));
    ...
    if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
    {
        ...
        {
            log_error(
            "ERROR: Unable to create 1D image of size %d pitch %d (%s)\n",
    ...
    else if (gMemFlagsToUse == CL_MEM_COPY_HOST_PTR)
    {
        ...
        imageInfo->format, imageInfo->width,
        (gEnablePitch ? imageInfo->rowPitch : 0), imageValues, NULL, &error);
        if (error != CL_SUCCESS)
        {
            log_error(
            "ERROR: Unable to create 1D image of size %d pitch %d (%s)\n",
    ...
    else // Either CL_MEM_ALLOC_HOST_PTR or none
    {
        ...
        if (gTestMipmaps)
        {
            ...
            unprotImage =
            clCreateImage(context, image_read_write_flags, imageInfo->format,
                          &image_desc, NULL, &error);
            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to create %d level mipmapped 1D image "
                          "of size %d (pitch %d, %d ) (%s)",
    ...
    if (gMemFlagsToUse != CL_MEM_COPY_HOST_PTR)
    {
        ...
        if (gTestMipmaps)
        {
            ...
            for (int i = 0; i < imageInfo->num_mip_levels; i++)
            {
                ...
                nextLevelOffset +=
                region[0] * get_pixel_size(imageInfo->format);
    ...
    clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
                   sizeof(cl_float) * imageInfo->width, xOffsetValues, &error);
    test_error(error, "Unable to create x offset buffer");
    results = clCreateBuffer(
    context, (cl_mem_flags)(CL_MEM_READ_WRITE),
    get_explicit_type_size(outputType) * 4 * imageInfo->width, NULL, &error);
         ...
         lod++)
    {
        ...
        width_lod * get_explicit_type_size(outputType) * 4;
        ...
        for (int q = 0; q < loopCount; q++)
        {
            ...
            if (gDebugTrace)
                log_info(
                "    reading results, %ld kbytes\n",
                (unsigned long)(width_lod * get_explicit_type_size(outputType)
                                * 4 / 1024));

            error = clEnqueueReadBuffer(
            queue, results, CL_TRUE, 0,
            width_lod * get_explicit_type_size(outputType) * 4, resultValues, 0,
            NULL, NULL);
                ...
                && (outputType == kFloat))
            {
                ...
                imageInfo->format, imageSampler, 0 /*not 3D*/,
                CL_FILTER_LINEAR == imageSampler->filter_mode);
                {
                    for (size_t x = 0, j = 0; x < width_lod; x++, j++)
                    {
                        // Step 1: go through and see if the results verify for
                        // the pixel For the normalized case on a GPU we put in
                        // offsets to the X and Y to see if we land on the right
                        // pixel. This addresses the significant inaccuracy in
                        // GPU normalization in OpenCL 1.0.
                             ...
                             norm_offset_x += NORM_OFFSET)
                        {
                            ...
                            // Try sampling the pixel, without flushing
                            // denormals.
                            int containsDenormals = 0;
                            FloatPixel maxPixel =
                            sample_image_pixel_float_offset(
                            imagePtr, imageInfo, xOffsetValues[j], 0.0f, 0.0f,
                            norm_offset_x, 0.0f, 0.0f, imageSampler, expected,
                            0, &containsDenormals, lod);
                                ...
                                || !(err3 <= maxErr) || !(err4 <= maxErr))
                            {
                                ...
                                if (containsDenormals)
                                {
                                    ...
                                    imagePtr, imageInfo, xOffsetValues[j], 0.0f,
                                    0.0f, norm_offset_x, 0.0f, 0.0f,
                            ...
                            // If the final result DOES match, then we've found
                            // a valid result and we're done with this pixel.
                            found_pixel = (err1 <= maxErr) && (err2 <= maxErr)
                            && (err3 <= maxErr) && (err4 <= maxErr);
                        } // norm_offset_x


                        // Step 2: If we did not find a match, then print out
                        // debugging info.
                        if (!found_pixel)
                        {
                            // For the normalized case on a GPU we put in
                            // offsets to the X and Y to see if we land on the
                            // right pixel. This addresses the significant
                            // inaccuracy in GPU normalization in OpenCL 1.0.
                            checkOnlyOnePixel = 0;
                            int shouldReturn = 0;
                            for (float norm_offset_x = -offset;
                                 ...
                                 norm_offset_x += NORM_OFFSET)
                            {
                                ...
                                // normalized, then only test with offsets (0.0,
                                // 0.0) E.g., test one pixel.
                                if (!imageSampler->normalized_coords
                                    || gDeviceType != CL_DEVICE_TYPE_GPU
                                    || NORM_OFFSET == 0)
                                {
                                ...
                                sample_image_pixel_float_offset(
                                imagePtr, imageInfo, xOffsetValues[j], 0.0f,
                                0.0f, norm_offset_x, 0.0f, 0.0f, imageSampler,
                                expected, 0, &containsDenormals, lod);

                                float err1 = fabsf(sRGBmap(resultPtr[0])
                                                   - sRGBmap(expected[0]));
                                float err2 = fabsf(sRGBmap(resultPtr[1])
                                                   - sRGBmap(expected[1]));
                                float err3 = fabsf(sRGBmap(resultPtr[2])
                                                   - sRGBmap(expected[2]));
                                float err4 = fabsf(resultPtr[3] - expected[3]);
                                    ...
                                    || !(err3 <= maxErr) || !(err4 <= maxErr))
                                {
                                    ...
                                    {
                                        // If implementation decide to flush
                                        // subnormals to zero, max error needs
                                        // to be adjusted
                                        maxErr += 4 * FLT_MIN;
                                        ...
                                        0.0f, 0.0f, norm_offset_x, 0.0f, 0.0f,
                                        imageSampler, expected, 0, NULL, lod);
                                    ...
                                    || !(err3 <= maxErr) || !(err4 <= maxErr))
                                {
                                    ...
                                    float tempOut[4];
                                    shouldReturn |=
                                    determine_validation_error_1D<float>(
                                    imagePtr, imageInfo, imageSampler,
                                    resultPtr, expected, error,
                                    xOffsetValues[j], norm_offset_x, j,
                                    ...
                                    Ulp_Error(MAKE_HEX_FLOAT(0x1.000002p0f,
                                                             0x1000002L, -24)
                                              + maxErr,
                                              MAKE_HEX_FLOAT(0x1.000002p0f,
                                                             0x1000002L, -24)));
                                }
                                else
                                {
                                    log_error("Test error: we should have "
                                              "detected this passing above.\n");
                                }
                ...
                CL_FILTER_LINEAR == imageSampler->filter_mode);
                {
                    for (size_t x = 0, j = 0; x < width_lod; x++, j++)
                    {
                             ...
                             norm_offset_x += NORM_OFFSET)
                        {
                            ...
                            float err4 = fabsf(resultPtr[3] - expected[3]);
                            // Clamp to the minimum absolute error for the
                            // format
                                 ...
                                 norm_offset_x += NORM_OFFSET)
                            {
                                    ...
                                    || !(err3 <= maxErr3) || !(err4 <= maxErr4))
                                {
                                    ...
                                    if (containsDenormals)
                                    {
                                        ...
                                        err1 =
                                        fabsf(resultPtr[0] - expected[0]);
                                        err2 =
                                        fabsf(resultPtr[1] - expected[1]);
                                        err3 =
                                        fabsf(resultPtr[2] - expected[2]);
                                        err4 =
                                        fabsf(resultPtr[3] - expected[3]);
                                    }
                                }
            ...
            else if (outputType == kUInt)
            {
                ...
                for (size_t x = 0, j = 0; x < width_lod; x++, j++)
                {
                    ...
                    // pixel For the normalized case on a GPU we put in offsets
                    // to the X and Y to see if we land on the right pixel. This
                    ...
                    if (!found_pixel)
                    {
                        // For the normalized case on a GPU we put in offsets to
                        // the X and Y to see if we land on the right pixel.
                        // This addresses the significant inaccuracy in GPU
                        // normalization in OpenCL 1.0.
                        checkOnlyOnePixel = 0;
                        int shouldReturn = 0;
                        for (float norm_offset_x = -NORM_OFFSET;
                             ...
                             norm_offset_x += NORM_OFFSET)
                        {
                            ...
                            if (error > MAX_ERR)
                            {
                                ...
                                shouldReturn |=
                                determine_validation_error_1D<unsigned int>(
                                imagePtr, imageInfo, imageSampler, resultPtr,
                                expected, error, xOffsetValues[j],
                                norm_offset_x, j, numTries, numClamped, false,
                                lod);
                            }
                            else
                            {
                    ...
                    if (!found_pixel)
                    {
                             ...
                             norm_offset_x += NORM_OFFSET)
                        {
                            ...
                            if (error > MAX_ERR)
                            {
                                ...
                                shouldReturn |=
                                determine_validation_error_1D<int>(
                                imagePtr, imageInfo, imageSampler, resultPtr,
                                expected, error, xOffsetValues[j],
                                norm_offset_x, j, numTries, numClamped, false,
                                lod);
                           ...
                           ExplicitType outputType)
{
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            if (gTestMipmaps)
                ...
                2, (compute_max_mip_levels(imageInfo.width, 0, 0) - 1), seed);

            if (gDebugTrace) log_info("   at size %d\n", (int)imageInfo.width);

            int retCode =
            test_read_image_1D(context, queue, kernel, &imageInfo, imageSampler,
    ...
    else if (gTestRounding)
    {
        ...
        typeRange /= get_pixel_size(imageInfo.format)
        ...
        do
        {
            if (gDebugTrace)
                log_info(
                "   at size %d, starting round ramp at %llu for range %llu\n",
            ...
            gRoundingStartValue += imageInfo.width * pixelSize
            / get_format_type_size(imageInfo.format);
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                {
                    imageInfo.num_mip_levels = (size_t)random_in_range(
                    2, (compute_max_mip_levels(imageInfo.width, 0, 0) - 1),
                    seed);
                    size =
                    (cl_ulong)compute_mipmapped_image_size(imageInfo) * 4;
                }
                else
                {
                    if (gEnablePitch)
