...
"__global float *yOffsets, __global float *zOffsets,  __global %s%s *results "
"%s )\n"
"{\n"
"%s"
...
const char *offset2DarraySource =
...
"   width_lod = (get_image_width(input) >> lod_int ) ? (get_image_width(input) "
">> lod_int ) : 1;\n"
"   height_lod = (get_image_height(input) >> lod_int ) ? "
"(get_image_height(input) >> lod_int ) : 1;\n"
...
"   float4 coords = (float4)( xOffsets[offset], yOffsets[offset], "
"zOffsets[offset], 0.0f );\n";
...
template <class T>
int determine_validation_error_offset_2D_array(
void *imagePtr, image_descriptor *imageInfo, image_sampler_data *imageSampler,
T *resultPtr, T *expected, float error, float x, float y, float z,
float xAddressOffset, float yAddressOffset, float zAddressOffset, size_t j,
int &numTries, int &numClamped, bool printAsFloat, int lod)
{
    ...
    size_t imageWidth = imageInfo->width, imageHeight = imageInfo->height,
           imageDepth = imageInfo->arraySize;
    ...
    if (found)
    {
            ...
            && clampedZ == actualZ)
        {
            if ((--numClamped) == 0)
            {
                log_error("\nERROR: TEST FAILED! Read is erroneously clamping "
                          "coordinates!\n");
                if (printAsFloat)
                {
    ...
    if (clamped && !otherClampingBug)
    {
            ...
            && resultPtr[3] == 0)
        {
            if ((--numClamped) == 0)
            {
                log_error("\nERROR: TEST FAILED: Clamping is erroneously "
    ...
    if (!clampingErr)
    {
        if (true) // gExtraValidateInfo )
        {
            ...
            {
                log_error(
                "Sample %ld: coord {%f(%a),%f(%a),%f(%a)} did not validate!\n"
                "   Expected (%g,%g,%g,%g)\n"
                "   Observed (%g,%g,%g,%g)\n"
                "   error of %g\n",
                j, x, x, y, y, z, z, (float)expected[0], (float)expected[1],
            ...
            log_error(
            "Integer coords resolve to %d,%d,%d, image size = %d,%d,%d\n",
        ...
        for (size_t z = 0; z < imageInfo->arraySize; z++)
        {
            for (size_t y = 0; y < height_lod; y++)
            {
                for (size_t x = 0; x < width_lod; x++, i++)
                {
                    ...
                    zOffsets[i] =
                    (float)(zfract
                            + (double)((int)z + random_in_range(-10, 10, d)));
                }
            }
        }
    }
                             ...
                             MTdata d)
{
    ...
    if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create 2D image array of size %d x %d "
                      "x %d (pitch %d, %d ) (%s)",
    ...
    else // Either CL_MEM_ALLOC_HOST_PTR or none
    {
        if (gTestMipmaps)
        {
            ...
            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to create %d level mipmapped 2D image "
                          "array of size %d x %d x %d (pitch %d, %d ) (%s)",
    ...
    if (gMemFlagsToUse != CL_MEM_COPY_HOST_PTR)
    {
        ...
        if (gTestMipmaps)
        {
            for (int level = 0; level < imageInfo->num_mip_levels; level++)
            {
                ...
                if (error != CL_SUCCESS)
                {
                    log_error("ERROR: Unable to write to level %d of 2D image "
                              "array of size %d x %d x %d\n",
                ...
                tmpNextLevelOffset += region[0] * region[1] * region[2]
        ...
        else
        {
            ...
            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to write to 2D image array of size %d "
                          "x %d x %d\n",
                          (int)imageInfo->width, (int)imageInfo->height,
                          (int)imageInfo->arraySize);
                return error;
    ...
    zOffsets = clCreateBuffer(context, (cl_mem_flags)(CL_MEM_COPY_HOST_PTR),
                              sizeof(cl_float) * imageInfo->width
                              * imageInfo->height * imageInfo->arraySize,
                              zOffsetValues, &error);
    test_error(error, "Unable to create y offset buffer");
    ...
    for (size_t lod = 0; (gTestMipmaps && (lod < imageInfo->num_mip_levels))
         ...
         lod++)
    {
        size_t resultValuesSize = width_lod * height_lod * imageInfo->arraySize
        * get_explicit_type_size(outputType) * 4;
        ...
        for (int q = 0; q < loopCount; q++)
        {
            ...
            clEnqueueReadBuffer(queue, results, CL_TRUE, 0,
                                width_lod * height_lod * imageInfo->arraySize
                                * get_explicit_type_size(outputType) * 4,
                                resultValues, 0, NULL, NULL);
                                 ...
                                 norm_offset_x += NORM_OFFSET)
                            {
                                     ...
                                     norm_offset_y += NORM_OFFSET)
                                {
                                         ...
                                         norm_offset_z += NORM_OFFSET)
                                    {
                                        ...
                                        norm_offset_z, imageSampler, expected,
                                        0, &hasDenormals, lod);
                                         ...
                                         norm_offset_y += NORM_OFFSET)
                                    {
                                             ...
                                             norm_offset_z += NORM_OFFSET)
                                        {
                                            ...
                                            if (!(err1 <= maxErr1))
                                            {
                                                ...
                                                float tempOut[4];
                                                shouldReturn |=
                                                determine_validation_error_offset_2D_array<
                                                float>(
                                                imagePtr, imageInfo,
                                                ...
                                                1 /*verbose*/, &hasDenormals,
                                                lod);
                                                log_error(
                                                "\tulps: %2.2f  (max allowed: "
                                                "%2.2f)\n\n",
                                            ...
                                            {
                                                log_error(
                                                "Test error: we should have "
                                                "detected this passing "
                                                "above.\n");
                                            }
                                        } // norm_offset_z
                                    } // norm_offset_y
                                         ...
                                         norm_offset_z += NORM_OFFSET)
                                    {
                                            ...
                                            || !(err4 <= maxErr4))
                                        {
                                            ...
                                            if (hasDenormals)
                                            {
                                                ...
                                                err1 = fabsf(resultPtr[0]
                                                             - expected[0]);
                                                err2 = fabsf(resultPtr[1]
                                                             - expected[1]);
                                                err3 = fabsf(resultPtr[2]
                                                             - expected[2]);
                                                err4 = fabsf(resultPtr[3]
                                                             - expected[3]);
                                            }
                                         ...
                                         norm_offset_z += NORM_OFFSET)
                                    {
                                        ...
                                        errMax(abs_diff_uint(expected[0],
                                               ...
                                               abs_diff_uint(expected[1],
                                                             resultPtr[1])),
                                        errMax(abs_diff_uint(expected[2],
                                                             resultPtr[2]),
                                             ...
                                             norm_offset_z += NORM_OFFSET)
                                        {
                                            ...
                                            if (error > MAX_ERR)
                                            {
                                                ...
                                                int>(imagePtr, imageInfo,
                                                     ...
                                                     norm_offset_x,
                                                     norm_offset_y,
                                                     norm_offset_z, j, numTries,
                                                     numClamped, false, lod);
                                            }
                                            else
                                            {
                                                log_error(
        ...
        {
            nextLevelOffset += width_lod * height_lod * imageInfo->arraySize
            * get_pixel_size(imageInfo->format);
    ...
    else if (gTestMaxImages)
    {
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            if (gTestMipmaps)
                ...
                - 1,
                seed);
            cl_ulong size = (cl_ulong)imageInfo.slicePitch
    ...
    else
    {
        ...
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                imageInfo.height =
                (size_t)random_log_in_range(16, maxHeighthRange, seed);
                imageInfo.arraySize =
                (size_t)random_log_in_range(8, maxArraySizeRange, seed);
                ...
                if (gTestMipmaps)
                {
                    imageInfo.num_mip_levels = random_in_range(
                    2,
                    compute_max_mip_levels(imageInfo.width, imageInfo.height, 0)
                    - 1,
                    seed);
                    // Need to take into account the output buffer size,
                    // otherwise we will end up with input buffer that is
                    // exceeding MaxAlloc
                    size = (cl_ulong)4 * compute_mipmapped_image_size(imageInfo)
            ...
            if (gDebugTrace)
            {
                log_info("   at size %d,%d,%d (pitch %d,%d) out of %d,%d,%d\n",
                         (int)imageInfo.width, (int)imageInfo.height,
                         (int)imageInfo.arraySize, (int)imageInfo.rowPitch,
                         (int)imageInfo.slicePitch, (int)maxWidth,
