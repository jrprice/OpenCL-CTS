...
static const char *convert_image_type_to_string(cl_mem_object_type imageType)
{
    ...
    switch (imageType)
    {
        case CL_MEM_OBJECT_IMAGE1D: p = str_1d_image; break;
        case CL_MEM_OBJECT_IMAGE2D: p = str_2d_image; break;
        case CL_MEM_OBJECT_IMAGE3D: p = str_3d_image; break;
        case CL_MEM_OBJECT_IMAGE1D_ARRAY: p = str_1d_image_array; break;
        case CL_MEM_OBJECT_IMAGE2D_ARRAY: p = str_2d_image_array; break;
    }
    return p;
}
                    ...
                    unsigned int &outFormatCount, cl_mem_flags flags)
{
    ...
    error = clGetSupportedImageFormats(context, flags, imageType, 128, tempList,
                                       &outFormatCount);
         ...
         adMode++)
    {
        ...
        switch (imageType)
        {
            case CL_MEM_OBJECT_IMAGE1D:
                retCode =
                test_read_image_set_1D(device, context, queue, format,
            ...
            case CL_MEM_OBJECT_IMAGE2D:
                retCode =
                test_read_image_set_2D(device, context, queue, format,
                                       imageSampler, floatCoords, outputType);
                break;
            ...
            case CL_MEM_OBJECT_IMAGE3D:
                retCode =
                test_read_image_set_3D(device, context, queue, format,
                                       imageSampler, floatCoords, outputType);
                break;
        }
        if (retCode != 0)
    ...
    for (normalizedIdx = 0; normalizedIdx < 2; normalizedIdx++)
    {
        ...
        for (floatCoordIdx = 0; floatCoordIdx < 2; floatCoordIdx++)
        {
            ...
            log_info("read_image (%s coords, %s results) "
                     "*****************************\n",
                     flipFlop[floatCoordIdx]
                     ? (imageSampler->normalized_coords ? "normalized float"
                                                        : "unnormalized float")
                     : "integer",
            ...
            for (unsigned int i = 0; i < numFormats; i++)
            {
                ...
                ret |= test_read_image_type(
                device, context, queue, &imageFormat, flipFlop[floatCoordIdx],
                imageSampler, outputType, imageType);
            }
        }
    }
    return ret;
                   ...
                   cl_mem_object_type imageType)
{
        ...
        && (formatTestFn == test_write_image_formats))
    {
        ...
        log_error("This device does not support the mandated extension "
    ...
    {
        cl_channel_type floatFormats[] = { CL_UNORM_SHORT_565,
                                           CL_UNORM_SHORT_555,
                                           CL_UNORM_INT_101010,
        ...
        else
        {
            ...
            ret += formatTestFn(device, context, queue, formatList, filterFlags,
                                numFormats, &imageSampler, kFloat, imageType);
    ...
    {
        cl_channel_type intFormats[] = { CL_SIGNED_INT8, CL_SIGNED_INT16,
                                         CL_SIGNED_INT32, (cl_channel_type)-1 };
        ...
        else
        {
            ...
            imageSampler.filter_mode = CL_FILTER_NEAREST;
            ret += formatTestFn(device, context, queue, formatList, filterFlags,
                                numFormats, &imageSampler, kInt, imageType);
        }
    }
    ...
    {
        cl_channel_type uintFormats[] = { CL_UNSIGNED_INT8, CL_UNSIGNED_INT16,
                                          CL_UNSIGNED_INT32,
                                          (cl_channel_type)-1 };
        ...
        else
        {
            ...
            imageSampler.filter_mode = CL_FILTER_NEAREST;
            ret += formatTestFn(device, context, queue, formatList, filterFlags,
                                numFormats, &imageSampler, kUInt, imageType);
        }
    }
    return ret;
}
