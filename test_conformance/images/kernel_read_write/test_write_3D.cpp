...
"%s"
"__kernel void sample_kernel( __global %s4 *input, read_write image3d_t output "
"%s )\n"
"{\n"
"   int tidX = get_global_id(0), tidY = get_global_id(1), tidZ = "
"get_global_id(2);\n"
                        ...
                        MTdata d)
{
         ...
         mem_flag_index++)
    {
        ...
        if (gMemFlagsToUse == CL_MEM_USE_HOST_PTR)
        {
            ...
            imageInfo->depth, 0, 0, maxImageUseHostPtrBackingStore, &error);

            if (error != CL_SUCCESS)
            {
                log_error("ERROR: Unable to create 3D image of size %ld x %ld "
                          "x %ld pitch %ld (%s)\n",
        ...
        else // Either CL_MEM_ALLOC_HOST_PTR, CL_MEM_COPY_HOST_PTR or none
        {
            ...
            else
            {
                ...
                if (error != CL_SUCCESS)
                {
                    log_error("ERROR: Unable to create 3D image of size %ld x "
                              "%ld x %ld pitch %ld (%s)\n",
        ...
        for (int lod = 0; lod < num_lod_loops; lod++)
        {
                    ...
                    for (size_t x = 0; x < width_lod; x++, i++)
                    {
                            ...
                            imageInfo->format->image_channel_order))
                        {
                            ...
                            if ((err[0] > max_err) || (err[1] > max_err)
                                || (err[2] > max_err)
                                || (err[3]
                                    > FLT_EPSILON)) // there is no conversion
                                                    // for alpha
                            {
            ...
            {
                nextLevelOffset +=
                width_lod * height_lod * depth_lod * pixelSize;
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                ...
                for (imageInfo.depth = 2; imageInfo.depth < 7;
                     imageInfo.depth++)
                {
                    if (gTestMipmaps)
                        imageInfo.num_mip_levels = (size_t)random_in_range(
                        2,
                        (compute_max_mip_levels(
                         imageInfo.width, imageInfo.height, imageInfo.depth)
                         - 1),
                        d);
                    ...
                    int retCode = test_write_image_3D(
    ...
    else if (gTestMaxImages)
    {
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            if (gTestMipmaps)
                imageInfo.num_mip_levels = (size_t)random_in_range(
                2,
                (compute_max_mip_levels(imageInfo.width, imageInfo.height,
                                        imageInfo.depth)
                 - 1),
                d);
        ...
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                if (gTestMipmaps)
                {
                    ...
                    * get_explicit_type_size(inputType);
                }
                else
                {
                    imageInfo.rowPitch =
                    ...
                    size =
                    (size_t)imageInfo.slicePitch * (size_t)imageInfo.depth * 4;
                }
