                       ...
                       image_descriptor *imageInfo, MTdata d)
{
    ...
    if (!gTestMipmaps)
    {
        image = create_image_1d(context, (cl_mem_flags)(CL_MEM_READ_ONLY),
                                imageInfo->format, imageInfo->width, 0, NULL,
                                NULL, &error);
        if (image == NULL)
        {
                           ...
                           cl_command_queue queue, cl_image_format *format)
{
    ...
    else if (gTestMaxImages)
    {
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            if (test_read_image_1D(context, queue, &imageInfo, seed)) return -1;
        }
    }
    else
    {
