                       ...
                       image_descriptor *imageInfo, MTdata d)
{
    ...
    {
        image =
        create_image_3d(context, (cl_mem_flags)(CL_MEM_READ_ONLY),
                        imageInfo->format, imageInfo->width, imageInfo->height,
                        imageInfo->depth, 0, 0, NULL, &error);
        if (image == NULL)
        {
            log_error(
            "ERROR: Unable to create 2D image of size %d x %d x %d (%s)",
         ...
         lod++)
    {
        ...
        if (error != CL_SUCCESS)
        {
            ...
            gTestMipmaps ? "mipmapped" : "", (int)width_lod, (int)height_lod,
            (int)depth_lod);
            return -1;
        }
            ...
            for (size_t y = 0; y < height_lod; y++)
            {
                if (memcmp(sourcePtr, destPtr, scanlineSize) != 0)
                {
                    ...
                    log_error("ERROR: Scanline %d,%d did not verify for image "
                              ...
                              (int)depth_lod, (int)row_pitch_lod,
                              (int)slice_pitch_lod);
                    return -1;
                }
        ...
        width_lod * height_lod * depth_lod * get_pixel_size(imageInfo->format);
    }
    return 0;
}
                     ...
                     imageInfo.depth++)
                {
                    ...
                    if (gDebugTrace)
                        log_info("   at size %d,%d,%d\n", (int)imageInfo.width,
                                 (int)imageInfo.height, (int)imageInfo.depth);
                    int ret =
                    test_read_image_3D(context, queue, &imageInfo, seed);
                    if (ret) return -1;
                }
            }
        }
    ...
    else if (gTestMaxImages)
    {
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            log_info("Testing %d x %d x %d\n", (int)imageInfo.width,
                     (int)imageInfo.height, (int)imageInfo.depth);
            if (test_read_image_3D(context, queue, &imageInfo, seed)) return -1;
        }
    }
    else
    {
