                             ...
                             image_descriptor *imageInfo, MTdata d)
{
    ...
    if (!gTestMipmaps)
    {
        image = create_image_2d_array(context, (cl_mem_flags)(CL_MEM_READ_ONLY),
        ...
        {
            log_error(
            "ERROR: Unable to create 2D image array of size %d x %d x %d (%s)",
    ...
    else
    {
        ...
        image = clCreateImage(context, CL_MEM_READ_ONLY, imageInfo->format,
                              &image_desc, NULL, &error);
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create %d level mipmapped 3D image of "
         ...
         lod++)
    {
        ...
        : imageInfo->rowPitch;
        slice_pitch_lod =
        gTestMipmaps ? (row_pitch_lod * height_lod) : imageInfo->slicePitch;
        region[0] = width_lod;
        region[1] = height_lod;
        ...
        error = clEnqueueWriteImage(queue, image, CL_FALSE, origin, region,
                                    (gEnablePitch ? row_pitch_lod : 0),
                                    (gEnablePitch ? slice_pitch_lod : 0),
                                    (char *)imageValues + imgValMipLevelOffset,
                                    0, NULL, NULL);
        if (error != CL_SUCCESS)
        {
            log_error(
            "ERROR: Unable to write to 2D image array of size %d x %d x %d\n",
        ...
        // match the input
        if (gDebugTrace) log_info(" - Initing result array...\n");
        ...
        for (size_t z = 0; z < imageInfo->arraySize; z++)
        {
            for (size_t y = 0; y < height_lod; y++)
            {
                ...
                {
                    log_error("ERROR: Scanline %d,%d did not verify for image "
                              "size %d,%d,%d pitch %d,%d\n",
                              (int)y, (int)z, (int)width_lod, (int)height_lod,
                              (int)imageInfo->arraySize, (int)row_pitch_lod,
                              (int)slice_pitch_lod);
                    return -1;
                }
        ...
        imgValMipLevelOffset += width_lod * height_lod * imageInfo->arraySize
        ...
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                     ...
                     imageInfo.arraySize++)
                {
                    ...
                    int ret =
                    test_read_image_2D_array(context, queue, &imageInfo, seed);
                    if (ret) return -1;
                }
            }
        }
            ...
            do
            {
                ...
                else
                {
                    ...
                    if (gEnablePitch)
                    {
                        ...
                        size_t extraHeight =
                        (int)random_log_in_range(0, 8, seed);
                        imageInfo.slicePitch =
