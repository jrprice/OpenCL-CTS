    ...
    for (unsigned int i = 0; i < numFormats; i++)
    {
        ...
        switch (imageType)
        {
            case CL_MEM_OBJECT_IMAGE1D:
                test_return =
                test_read_image_set_1D(device, context, queue, &formatList[i]);
                break;
            case CL_MEM_OBJECT_IMAGE2D:
                test_return =
                test_read_image_set_2D(device, context, queue, &formatList[i]);
                break;
            case CL_MEM_OBJECT_IMAGE3D:
                test_return =
                test_read_image_set_3D(device, context, queue, &formatList[i]);
                break;
                   ...
                   cl_command_queue queue, cl_mem_object_type imageType)
{
    ...
    ret +=
    test_image_type(device, context, queue, imageType, CL_MEM_WRITE_ONLY);
