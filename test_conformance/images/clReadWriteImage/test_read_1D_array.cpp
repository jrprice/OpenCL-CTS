                             ...
                             image_descriptor *imageInfo, MTdata d)
{
    ...
    else
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create %d level mipmapped 1D image of "
                      "width %d and array size %d (pitch %d ) (%s)",
         ...
         lod++)
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to write to 1D image array  of width %d "
                      "and size %d\n",
        ...
        for (size_t y = 0; y < imageInfo->arraySize; y++)
        {
            ...
            {
                log_error("ERROR: Image array index %d did not verify for "
                          "image size %d,%d pitch %d (extra %d bytes)\n",
                          (int)y, (int)width_lod, (int)imageInfo->arraySize,
                          (int)row_pitch_lod,
                          (int)row_pitch_lod
                          - (int)width_lod
                ...
                imageValues + imgValMipLevelOffset, imageInfo, destPtr + offset,
                get_pixel_size(imageInfo->format), &outX, &outY, NULL);
                if (foundCount > 0)
                {
                    int returnedOffset =
                    ((int)y * (int)width_lod
                     + offset / (int)get_pixel_size(imageInfo->format))
                    - (outY * (int)width_lod + outX);
        ...
        width_lod * imageInfo->arraySize * get_pixel_size(imageInfo->format);
    }
    return 0;
}
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
                 ...
                 imageInfo.arraySize++)
            {
                ...
                int ret =
                test_read_image_1D_array(context, queue, &imageInfo, seed);
                if (ret) return -1;
            }
        }
    }
