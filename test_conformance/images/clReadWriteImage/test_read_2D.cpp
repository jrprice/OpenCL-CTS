                       ...
                       image_descriptor *imageInfo, MTdata d)
{
    ...
    if (!gTestMipmaps)
    {
        ...
        if (image == NULL)
        {
            log_error("ERROR: Unable to create 2D image of size %d x %d (%s)",
    ...
    else
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create %d level mipmapped 2D image of "
         ...
         lod++)
    {
        ...
        for (size_t y = 0; y < height_lod; y++)
        {
            if (memcmp(sourcePtr, destPtr, scanlineSize) != 0)
            {
                ...
                log_error(
                "ERROR: Scanline %d did not verify for image size %d,%d pitch "
                "%d (extra %d bytes)\n",
                (int)y, (int)width_lod, (int)height_lod, (int)row_pitch_lod,
                (int)row_pitch_lod
                - (int)width_lod * (int)get_pixel_size(imageInfo->format));
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            if (gDebugTrace)
                log_info("   at max size %d,%d\n", (int)maxWidth,
                         (int)maxHeight);
            if (test_read_image_2D(context, queue, &imageInfo, seed)) return -1;
        }
    }
    else
    {
