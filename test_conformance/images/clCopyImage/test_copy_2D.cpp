                            ...
                            image_descriptor *imageInfo, MTdata d)
{
    ...
    retCode = test_copy_image_generic(context, queue, imageInfo, imageInfo,
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        : width_lod;
        regionSize[1] = (height_lod > 8)
        ? (size_t)random_in_range(8, (int)height_lod - 1, d)
        : height_lod;

        // Now pick positions within valid ranges
        sourcePos[0] = (width_lod > regionSize[0])
        ? (size_t)random_in_range(0, (int)(width_lod - regionSize[0] - 1), d)
        : 0;
        sourcePos[1] = (height_lod > regionSize[1])
        ? (size_t)random_in_range(0, (int)(height_lod - regionSize[1] - 1), d)
        : 0;

        destPos[0] = (width_lod > regionSize[0])
        ? (size_t)random_in_range(0, (int)(width_lod - regionSize[0] - 1), d)
        : 0;
        destPos[1] = (height_lod > regionSize[1])
        ? (size_t)random_in_range(0, (int)(height_lod - regionSize[1] - 1), d)
        : 0;
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                ...
                int ret =
                test_copy_image_size_2D(context, queue, &imageInfo, seed);
                if (ret) return -1;
            }
        }
    }
