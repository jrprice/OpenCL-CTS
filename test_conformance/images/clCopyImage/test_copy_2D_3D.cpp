                               ...
                               image_descriptor *dstImageInfo, MTdata d)
{
    ...
    size_t twoImage_lod = 0, twoImage_width_lod = twoImage->width,
           twoImage_row_pitch_lod;
    size_t twoImage_height_lod = twoImage->height;
    size_t threeImage_lod = 0, threeImage_width_lod = threeImage->width,
           threeImage_row_pitch_lod, threeImage_slice_pitch_lod;
    size_t threeImage_height_lod = threeImage->height,
           depth_lod = threeImage->depth;
    ...
    if (gTestMipmaps)
    {
        ...
        : 1;
        twoImage_row_pitch_lod =
        twoImage_width_lod * get_pixel_size(twoImage->format);
        threeImage_row_pitch_lod =
        threeImage_width_lod * get_pixel_size(threeImage->format);
        threeImage_slice_pitch_lod =
        threeImage_height_lod * threeImage_row_pitch_lod;
    }
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        {
            regionSize[0] =
            (width_lod > 8) ? random_in_range(8, width_lod, d) : width_lod;
            regionSize[1] =
            (height_lod > 8) ? random_in_range(8, height_lod, d) : height_lod;
        }
        ...
        : 0;
        sourcePos[2] = (srcImageInfo->depth > 0)
        ? (size_t)random_in_range(0, (int)(srcImageInfo->depth - 1), d)
        : 0;
        ...
        if (gTestMipmaps)
            if (srcImageInfo->depth > 0)
            {
                ...
                : 0;
                sourcePos[2] =
                (size_t)random_in_range(0, (int)(depth_lod - 1), d);
                sourcePos[3] = threeImage_lod;
            }
            else
            {
        ...
        : 0;
        destPos[2] = (dstImageInfo->depth > 0)
        ? (size_t)random_in_range(0, (int)(dstImageInfo->depth - 1), d)
        : 0;
                              ...
                              bool reverse = false)
{
    ...
    error |= clGetDeviceInfo(device, CL_DEVICE_IMAGE3D_MAX_DEPTH,
                             sizeof(max3DDepth), &max3DDepth, NULL);
    ...
    if (gTestSmallImages)
    {
             ...
             dstImageInfo.width++)
        {
            for (dstImageInfo.height = 4; dstImageInfo.height < 13;
                 dstImageInfo.height++)
            {
                for (dstImageInfo.depth = 4; dstImageInfo.depth < 9;
                     dstImageInfo.depth++)
                {
                    ...
                    &dstImageInfo, dstImageInfo.width, dstImageInfo.height,
                    dstImageInfo.depth, rowPadding, slicePadding);
                    set_image_dimensions(&srcImageInfo, dstImageInfo.width,
                                         dstImageInfo.height, 0, rowPadding,
                                         slicePadding);
    ...
    else if (gTestMaxImages)
    {
        ...
        // Try to allocate a bit smaller images because we need the 2D ones as
        // well for the copy.
        ...
        for (size_t i = 0; i < numberOfSizes2D; i++)
            for (size_t j = 0; j < numberOfSizes3D; j++)
            {
                ...
                set_image_dimensions(&dstImageInfo, sizes3D[j][0],
                                     sizes3D[j][1], sizes3D[j][2], rowPadding,
                                     slicePadding);
                set_image_dimensions(&srcImageInfo, sizes2D[i][0],
                                     sizes2D[i][1], 0, rowPadding,
                                     slicePadding);
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                {
                    set_image_dimensions(&srcImageInfo, srcImageInfo.width,
                                         srcImageInfo.height, 0, rowPadding,
                                         slicePadding);
                    set_image_dimensions(
                    &dstImageInfo, dstImageInfo.width, dstImageInfo.height,
                    dstImageInfo.depth, rowPadding, slicePadding);

                    srcSize = (cl_ulong)srcImageInfo.rowPitch
                    * (cl_ulong)srcImageInfo.height * 4;
                    dstSize = (cl_ulong)dstImageInfo.slicePitch
                    * (cl_ulong)dstImageInfo.depth * 4;
                }
            ...
            int ret;
            if (reverse)
                ret = test_copy_image_size_2D_3D(context, queue, &dstImageInfo,
                                                 &srcImageInfo, seed);
            else
                ret = test_copy_image_size_2D_3D(context, queue, &srcImageInfo,
                                                 &dstImageInfo, seed);
            if (ret) return -1;
        }
    }
