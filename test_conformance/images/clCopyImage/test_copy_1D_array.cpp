                                  ...
                                  image_descriptor *imageInfo, MTdata d)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        {
            // Work at a random mip level
            src_lod = (size_t)(max_mip_level > 1)
            ? random_in_range(0, max_mip_level - 1, d)
            : 0;
            dst_lod = (size_t)(max_mip_level > 1)
            ? random_in_range(0, max_mip_level - 1, d)
            : 0;
            src_width_lod =
            (imageInfo->width >> src_lod) ? (imageInfo->width >> src_lod) : 1;
        ...
        : (int)width_lod;
        regionSize[1] = (imageInfo->arraySize > 8)
        ? (size_t)random_in_range(8, (int)imageInfo->arraySize - 1, d)
        : imageInfo->arraySize;
        ...
        : 0;
        sourcePos[1] = (imageInfo->arraySize > regionSize[1])
        ? (size_t)random_in_range(
        0, (int)(imageInfo->arraySize - regionSize[1] - 1), d)
        : 0;
        ...
        ? (size_t)random_in_range(0, (int)(width_lod - regionSize[0] - 1), d)
        : 0;
        destPos[1] = (imageInfo->arraySize > regionSize[1])
        ? (size_t)random_in_range(
        0, (int)(imageInfo->arraySize - regionSize[1] - 1), d)
        : 0;
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
                 ...
                 imageInfo.arraySize++)
            {
                ...
                int ret =
                test_copy_image_size_1D_array(context, queue, &imageInfo, seed);
                if (ret) return -1;
            }
        }
    }
