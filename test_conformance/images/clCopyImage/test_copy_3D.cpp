                       ...
                       image_descriptor *imageInfo, MTdata d)
{
    ...
    if (gTestMipmaps)
    {
        size_t lod = (imageInfo->num_mip_levels > 1)
        ? (size_t)random_in_range(0, imageInfo->num_mip_levels - 1, d)
        : 0;
        origin[3] = lod;
    ...
    return test_copy_image_generic(context, queue, imageInfo, imageInfo, origin,
                                   origin, region, d);
}
                           ...
                           cl_command_queue queue, cl_image_format *format)
{
        ...
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                     ...
                     imageInfo.depth++)
                {
                    ...
                    int ret =
                    test_copy_image_3D(context, queue, &imageInfo, seed);
                    if (ret) return -1;
                }
            }
        }
    ...
    else if (gTestMaxImages)
    {
        ...
        get_max_sizes(&numbeOfSizes, 100, sizes, maxWidth, maxHeight, maxDepth,
                      1, maxAllocSize, memSize, imageInfo.type,
                      imageInfo.format);
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            if (gDebugTrace)
                log_info("   at max size %d,%d,%d\n", (int)sizes[idx][0],
                         (int)sizes[idx][1], (int)sizes[idx][2]);
            if (test_copy_image_3D(context, queue, &imageInfo, seed)) return -1;
        }
    }
    else
    {
