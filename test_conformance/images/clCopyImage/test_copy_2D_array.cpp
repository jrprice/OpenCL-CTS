                             ...
                             image_descriptor *imageInfo, MTdata d)
{
    ...
    size_t region[] = { imageInfo->width, imageInfo->height,
                        imageInfo->arraySize };

    size_t src_lod = 0, src_width_lod = imageInfo->width,
           src_height_lod = imageInfo->height;
    size_t dst_lod = 0, dst_width_lod = imageInfo->width,
           dst_height_lod = imageInfo->height;
    ...
    if (gTestMipmaps)
    {
        ...
        (size_t)random_in_range(0, max_mip_level ? max_mip_level - 1 : 0, d);
        src_width_lod =
        (imageInfo->width >> src_lod) ? (imageInfo->width >> src_lod) : 1;
        dst_width_lod =
        (imageInfo->width >> dst_lod) ? (imageInfo->width >> dst_lod) : 1;
        src_height_lod =
        (imageInfo->height >> src_lod) ? (imageInfo->height >> src_lod) : 1;
        dst_height_lod =
        (imageInfo->height >> dst_lod) ? (imageInfo->height >> dst_lod) : 1;
        width_lod =
        (src_width_lod > dst_width_lod) ? dst_width_lod : src_width_lod;
        height_lod =
        (src_height_lod > dst_height_lod) ? dst_height_lod : src_height_lod;
    ...
    return test_copy_image_generic(context, queue, imageInfo, imageInfo, srcPos,
                                   dstPos, region, d);
}
                                 ...
                                 cl_image_format *format)
{
        ...
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                     ...
                     imageInfo.arraySize++)
                {
                    ...
                    int ret =
                    test_copy_image_2D_array(context, queue, &imageInfo, seed);
                    if (ret) return -1;
                }
            }
        }
    ...
    else if (gTestMaxImages)
    {
        ...
        get_max_sizes(&numbeOfSizes, 100, sizes, maxWidth, maxHeight, 1,
                      maxArraySize, maxAllocSize, memSize, imageInfo.type,
                      imageInfo.format);
