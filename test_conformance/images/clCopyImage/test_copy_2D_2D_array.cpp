...
int test_copy_image_set_2D_2D_array(cl_device_id device, cl_context context,
                                    ...
                                    bool reverse = false)
{
    ...
    else if (gTestMaxImages)
    {
        ...
        get_max_sizes(&numberOfSizes2DArray, 100, sizes2DArray, maxWidth,
                      ...
                      CL_MEM_OBJECT_IMAGE2D_ARRAY, dstImageInfo.format);
        ...
        for (size_t i = 0; i < numberOfSizes2D; i++)
        {
            for (size_t j = 0; j < numberOfSizes2DArray; j++)
            {
                ...
                set_image_dimensions(&dstImageInfo, sizes2DArray[j][0],
                ...
                else
                {
                    if (reverse)
                        log_info(
                        "Not testing max size %d x %d x %d to %d x %d due to "
                        "memory constraints.\n",
                        (int)dstImageInfo.width, (int)dstImageInfo.height,
                        (int)dstImageInfo.arraySize, (int)srcImageInfo.width,
                        (int)srcImageInfo.height);
                        ...
                        log_info(
                        "Not testing max size %d x %d to %d x %d x %d due to "
                        "memory constraints.\n",
                        (int)srcImageInfo.width, (int)srcImageInfo.height,
                        (int)dstImageInfo.width, (int)dstImageInfo.height,
                        (int)dstImageInfo.arraySize);
