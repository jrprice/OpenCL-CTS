...
int test_2Dto3D(cl_device_id device, cl_context context, cl_command_queue queue,
                int num_elements)
{
    return test_image_set(device, context, queue, k2DTo3D);
}
int test_3Dto2D(cl_device_id device, cl_context context, cl_command_queue queue,
                int num_elements)
{
    return test_image_set(device, context, queue, k3DTo2D);
}
int test_2Darrayto2D(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    return test_image_set(device, context, queue, k2DArrayTo2D);
}
int test_2Dto2Darray(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    return test_image_set(device, context, queue, k2DTo2DArray);
}
int test_2Darrayto3D(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements)
{
    return test_image_set(device, context, queue, k2DArrayTo3D);
}
int test_3Dto2Darray(cl_device_id device, cl_context context,
                     cl_command_queue queue, int num_elements)
{
...
test_definition test_list[] = {
    ...
    ADD_TEST(2Darrayto3D), ADD_TEST(3Dto2Darray),
};
...
static void printUsage(const char *execName)
{
    ...
    log_info("\tsmall_images - Runs every format through a loop of widths 1-13 "
             "and heights 1-9, instead of random sizes\n");
    log_info(
    "\tmax_images - Runs every format through a set of size combinations with "
    "the max values, max values - 1, and max values / 128\n");
    log_info("\trandomize - Use random seed\n");
    log_info("\tuse_pitches - Enables row and slice pitches\n");
    log_info("\tuse_ramp - Instead of random data, uses images filled with "
             "ramps (and 0xff on any padding pixels) to ease debugging\n");
    log_info("\n");
    log_info("Test names:\n");
