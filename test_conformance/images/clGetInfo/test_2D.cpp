                               ...
                               size_t slice_pitch)
{
    ...
    switch (imageInfo->type)
    {
        ...
        case CL_MEM_OBJECT_IMAGE2D_ARRAY:
            ...
            if (gDebugTrace)
                log_info(
                " - Creating 2D image array %d by %d by %d with flags=0x%lx "
                "row_pitch=%d slice_pitch=%d host_ptr=%p...\n",
                (int)imageInfo->width, (int)imageInfo->height,
                (int)imageInfo->arraySize, (unsigned long)flags, (int)row_pitch,
                (int)slice_pitch, host_ptr);
            break;
    }
    ...
    error =
    clGetImageInfo(image, CL_IMAGE_FORMAT, sizeof(outFormat), &outFormat, NULL);
    ...
    {
        log_error(
        "ERROR: image format returned is invalid! (expected %s:%s, got %s:%s "
        "(%d:%d))\n",
    ...
    size_t outWidth;
    error =
    clGetImageInfo(image, CL_IMAGE_WIDTH, sizeof(outWidth), &outWidth, NULL);
    ...
    size_t outHeight;
    error =
    clGetImageInfo(image, CL_IMAGE_HEIGHT, sizeof(outHeight), &outHeight, NULL);
    ...
    size_t outDepth;
    error =
    clGetImageInfo(image, CL_IMAGE_DEPTH, sizeof(outDepth), &outDepth, NULL);
    ...
    {
        log_error(
        "ERROR: image array size returned is invalid! (expected %d, got %d)\n",
    ...
    cl_mem outBuffer;
    error =
    clGetImageInfo(image, CL_IMAGE_BUFFER, sizeof(outBuffer), &outBuffer, NULL);
    ...
    {
        log_error(
        "ERROR: image num_samples returned is invalid! (expected %d, got %d)\n",
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                size =
                (cl_ulong)imageInfo.rowPitch * (cl_ulong)imageInfo.height * 4;
                 ...
                 j < sizeof(all_host_ptr_flags) / sizeof(cl_mem_flags); j++)
            {
                if (gDebugTrace)
                    log_info(
                    "   at size %d,%d (flags[%u] 0x%x pitch %d) out of %d,%d\n",
                    (int)imageInfo.width, (int)imageInfo.height, j,
                    (unsigned int)all_host_ptr_flags[j],
                    (int)imageInfo.rowPitch, (int)maxWidth, (int)maxHeight);
                if (test_get_image_info_single(context, &imageInfo, seed,
                                               all_host_ptr_flags[j], 0, 0))
                    return -1;
                if (all_host_ptr_flags[j]
