...
static bool check_minimum_supported(cl_image_format *formatList,
                                    unsigned int numFormats, cl_mem_flags flags,
                                    ...
                                    cl_device_id device)
{
    ...
    for (auto &format : formatsToSupport)
    {
        ...
        {
            log_error("ERROR: Format required by OpenCL %s is not supported: ",
...
int get_format_list(cl_context context, cl_mem_object_type image_type,
                    ...
                    unsigned int &outFormatCount, cl_mem_flags flags)
{
    ...
    error = clGetSupportedImageFormats(context, (cl_mem_flags)flags, image_type,
                    ...
                    cl_mem_object_type image_type, cl_mem_flags flags)
{
    ...
    {
        log_info(
        "No requirement for 3D write in OpenCL 1.2. Not checking formats.\n");
    }
    else
    {
        log_info("Checking for required OpenCL 1.2 formats.\n");
        if (check_minimum_supported(formatList, numFormats, flags, image_type,
                                    device)
            == false)
        {
            ret++;
    ...
    for (unsigned int i = 0; i < numFormats; i++)
    {
        ...
        switch (image_type)
        {
            case CL_MEM_OBJECT_IMAGE1D:
                test_return =
                test_get_image_info_1D(device, context, &formatList[i], flags);
                break;
            case CL_MEM_OBJECT_IMAGE2D:
                test_return =
                test_get_image_info_2D(device, context, &formatList[i], flags);
                break;
            case CL_MEM_OBJECT_IMAGE3D:
                test_return =
                test_get_image_info_3D(device, context, &formatList[i], flags);
                break;
