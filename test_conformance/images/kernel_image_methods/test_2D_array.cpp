...
" } image_kernel_data;\n"
"__kernel void sample_kernel( read_only %s input, __global image_kernel_data "
"*outData )\n"
"{\n"
                                       ...
                                       image_descriptor *imageInfo, MTdata d)
{
    ...
    image = create_image_2d_array(context, (cl_mem_flags)(CL_MEM_READ_ONLY),
                                  ...
                                  imageInfo->height, imageInfo->arraySize, 0, 0,
                                  NULL, &error);
    if (image == NULL)
    {
        log_error(
                                 ...
                                 cl_image_format *format)
{
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            int ret = test_get_2Dimage_array_info_single(context, queue,
                                                         &imageInfo, seed);
            if (ret) return -1;
        }
    }
