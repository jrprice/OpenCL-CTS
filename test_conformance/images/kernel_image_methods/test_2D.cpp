...
" } image_kernel_data;\n"
"__kernel void sample_kernel( read_only image%dd%s_t input, __global "
...
static const char *depthKernelLine =
                               ...
                               image_descriptor *imageInfo, MTdata d)
{
    ...
    if (imageInfo->depth != 0)
        ...
        image = create_image_2d(context, (cl_mem_flags)(CL_MEM_READ_ONLY),
                                imageInfo->format, imageInfo->width,
                                imageInfo->height, 0, NULL, &error);
    if (image == NULL)
    {
        log_error("ERROR: Unable to create image of size %d x %d x %d (%s)",
    ...
    sprintf(
    programSrc, methodTestKernelPattern, (imageInfo->depth != 0) ? 3 : 2,
    ...
    {
        log_error(
        "ERROR: Returned height did not validate (expected %d, got %d)\n",
        (int)imageInfo->height, (int)outKernelData.height);
        error = -1;
    }
    if ((imageInfo->depth != 0)
        && (outKernelData.depth != (cl_int)imageInfo->depth))
    {
        log_error(
        "ERROR: Returned depth did not validate (expected %d, got %d)\n",
        (int)imageInfo->depth, (int)outKernelData.depth);
        error = -1;
    }
    if (outKernelData.widthDim != (cl_int)imageInfo->width)
    {
        log_error("ERROR: Returned width from get_image_dim did not validate "
                  "(expected %d, got %d)\n",
    ...
    {
        log_error("ERROR: Returned height from get_image_dim did not validate "
                  "(expected %d, got %d)\n",
                  (int)imageInfo->height, (int)outKernelData.heightDim);
        error = -1;
    }
    if ((imageInfo->depth != 0)
        && (outKernelData.depthDim != (cl_int)imageInfo->depth))
    {
        log_error("ERROR: Returned depth from get_image_dim did not validate "
                  "(expected %d, got %d)\n",
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                ...
                int ret =
                test_get_image_info_single(context, queue, &imageInfo, seed);
                if (ret) return -1;
            }
        }
    }
