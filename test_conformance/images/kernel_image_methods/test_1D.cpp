...
" } image_kernel_data;\n"
"__kernel void sample_kernel( read_only image1d_t input, __global "
                                        ...
                                        image_descriptor *imageInfo, MTdata d)
{
    ...
    image =
    create_image_1d(context, (cl_mem_flags)(CL_MEM_READ_ONLY),
                    imageInfo->format, imageInfo->width, 0, NULL, NULL, &error);
    if (image == NULL)
    {
                           ...
                           cl_command_queue queue, cl_image_format *format)
{
    ...
    if (gTestSmallImages)
    {
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            int ret =
            test_get_1Dimage_info_single(context, queue, &imageInfo, seed);
            if (ret) return -1;
        }
    }
