                    ...
                    int *error)
{
    ...
    if (*error != CL_SUCCESS)
    {
        ...
        switch (imageInfo->type)
        {
            case CL_MEM_OBJECT_IMAGE1D:
                log_error(
                "ERROR: Unable to create 1D image of size %d (%llu MB): %s\n",
    ...
    void *mapped =
    (char *)clEnqueueMapImage(queue, img, CL_TRUE, CL_MAP_WRITE, origin, region,
    ...
    if ((mappedRow == scanlineSize)
        && ((mappedSlice == imageInfo->slicePitch)
    ...
    else
    {
        ...
        for (size_t z = 0; z < depth; z++)
        {
            ...
            // mappedSlicePad is incorrect for 2D images here, but we will exit
            // the z loop before this is a problem.
            dst += mappedSlicePad;
            src += sliceSize;
        }
    }
...
static void fill_region_with_value(image_descriptor *imageInfo,
                                   void *imageValues, void *value,
                            ...
                            MTdata d)
{
    ...
    switch (imageInfo->type)
    {
        case CL_MEM_OBJECT_IMAGE1D: dataBytes = imageInfo->rowPitch; break;
        case CL_MEM_OBJECT_IMAGE2D:
    ...
    if (gDebugTrace) log_info(" - Creating image...\n");

    image = create_image(context, queue, imgData, imageInfo, &error);
    if (image == NULL) return error;

    // Now fill the region defined by origin, region with the pixel value found
    // at origin.
    if (gDebugTrace)
        log_info(" - Filling at %d,%d,%d size %d,%d,%d\n", (int)origin[0],
                 (int)origin[1], (int)origin[2], (int)region[0], (int)region[1],
                 (int)region[2]);
    ...
    if (outputType == kFloat)
    {
        ...
        read_image_pixel_float(imgHost, imageInfo, origin[0], origin[1],
                               origin[2], fillColor);
        if (gDebugTrace)
            log_info(" - with value %g, %g, %g, %g\n", fillColor[0],
                     fillColor[1], fillColor[2], fillColor[3]);
        error = clEnqueueFillImage(queue, image, fillColor, origin, region, 0,
                                   NULL, NULL);
        if (error != CL_SUCCESS)
        {
            log_error(
            "ERROR: Unable to fill image at %d,%d,%d size %d,%d,%d! (%s)\n",
            (int)origin[0], (int)origin[1], (int)origin[2], (int)region[0],
            (int)region[1], (int)region[2], IGetErrorString(error));
            return error;
        }
        ...
        fill_region_with_value(imageInfo, imgHost, verificationValue, origin,
                               region);
    ...
    {
        cl_int fillColor[4];
        read_image_pixel<cl_int>(imgHost, imageInfo, origin[0], origin[1],
                                 origin[2], fillColor);
        if (gDebugTrace)
    ...
    {
        cl_uint fillColor[4];
        read_image_pixel<cl_uint>(imgHost, imageInfo, origin[0], origin[1],
                                  origin[2], fillColor);
        if (gDebugTrace)
    ...
    switch (imageInfo->type)
    {
        ...
        case CL_MEM_OBJECT_IMAGE2D: imageRegion[1] = imageInfo->height; break;
        case CL_MEM_OBJECT_IMAGE3D:
    ...
    queue, image, CL_TRUE, CL_MAP_READ, imageOrigin, imageRegion, &mappedRow,
    ...
    for (size_t z = 0; z < thirdDim; z++)
    {
        ...
        {
            // If the data type is 101010 ignore bits 31 and 32 when comparing
            // the row
        ...
        sourcePtr += imageInfo->slicePitch
        - (imageInfo->rowPitch
           * (imageInfo->height > 0 ? imageInfo->height : 1));
        destPtr += mappedSlice
        - (mappedRow * (imageInfo->height > 0 ? imageInfo->height : 1));
    }
