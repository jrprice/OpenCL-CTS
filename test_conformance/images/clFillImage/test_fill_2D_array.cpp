                                    ...
                                    ExplicitType outputType, MTdata d)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        : imageInfo->width;
        region[1] = (imageInfo->height > 8)
        ? (size_t)random_in_range(8, (int)imageInfo->height - 1, d)
        : imageInfo->height;
        region[2] = (imageInfo->arraySize > 8)
        ...
        ? (size_t)random_in_range(0, (int)(imageInfo->width - region[0] - 1), d)
        : 0;
        origin[1] = (imageInfo->height > region[1]) ? (size_t)random_in_range(
                    0, (int)(imageInfo->height - region[1] - 1), d)
                                                    : 0;
        origin[2] = (imageInfo->arraySize > region[2])
        ? (size_t)random_in_range(
        0, (int)(imageInfo->arraySize - region[2] - 1), d)
        : 0;
                                 ...
                                 ExplicitType outputType)
{
        ...
        for (imageInfo.width = 1; imageInfo.width < 13; imageInfo.width++)
        {
            ...
            for (imageInfo.height = 1; imageInfo.height < 9; imageInfo.height++)
            {
                     ...
                     imageInfo.arraySize++)
                {
                    ...
                    context, queue, &imageInfo, outputType, seed);
                    if (ret) return -1;
                }
            }
        }
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            {
                imageInfo.width =
                (size_t)random_log_in_range(16, (int)maxWidth / 64, seed);
                imageInfo.height =
                (size_t)random_log_in_range(16, (int)maxHeight / 64, seed);
                imageInfo.arraySize =
