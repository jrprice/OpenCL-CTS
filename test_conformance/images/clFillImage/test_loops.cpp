    ...
    if (gTypesToTest & kTestFloat)
    {
        ...
        else
        {
            ...
            for (unsigned int i = 0; i < numFormats; i++)
            {
                ...
                if (testMethod == k1D)
                    test_return = test_fill_image_set_1D(
                    device, context, queue, &formatList[i], kFloat);
                else if (testMethod == k2D)
                    test_return = test_fill_image_set_2D(
                    device, context, queue, &formatList[i], kFloat);
                else if (testMethod == k1DArray)
                    test_return = test_fill_image_set_1D_array(
                    device, context, queue, &formatList[i], kFloat);
                else if (testMethod == k2DArray)
                    test_return = test_fill_image_set_2D_array(
                    device, context, queue, &formatList[i], kFloat);
                else if (testMethod == k3D)
                    test_return = test_fill_image_set_3D(
                    device, context, queue, &formatList[i], kFloat);
    ...
    if (gTypesToTest & kTestInt)
    {
        ...
        else
        {
            ...
            for (unsigned int i = 0; i < numFormats; i++)
            {
                ...
                if (testMethod == k1D)
                    test_return = test_fill_image_set_1D(device, context, queue,
                                                         &formatList[i], kInt);
                else if (testMethod == k2D)
                    test_return = test_fill_image_set_2D(device, context, queue,
                                                         &formatList[i], kInt);
                else if (testMethod == k1DArray)
                    test_return = test_fill_image_set_1D_array(
                    ...
                    test_return = test_fill_image_set_3D(device, context, queue,
                                                         &formatList[i], kInt);
