...
"__kernel void sample_kernel( read_only %s input, sampler_t sampler, __global "
"int *results )\n"
"{\n"
...
"__kernel void sample_kernel( read_only %s read_only_image, read_write %s "
"read_write_image, sampler_t sampler, __global int *results )\n"
"{\n"
"   int tidX = get_global_id(0), tidY = get_global_id(1), tidZ = "
"get_global_id(2);\n"
"   int offset = "
                             ...
                             ExplicitType outputType, MTdata d)
{
    ...
    if (error != CL_SUCCESS)
    {
        log_error("ERROR: Unable to create read_only 2D image array of size %d "
                  "x %d x %d (pitch %d, %d ) (%s)",
    ...
    if (gTestReadWrite)
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create read_write 2D image array of "
                      "size %d x %d x %d (pitch %d, %d ) (%s)",
                      (int)imageInfo->width, (int)imageInfo->height,
                      (int)imageInfo->arraySize, (int)imageInfo->rowPitch,
                      (int)imageInfo->slicePitch, IGetErrorString(error));
            return error;
    ...
    cl_mem results = clCreateBuffer(context, 0,
                                    ...
                                    * imageInfo->arraySize * sizeof(cl_int),
                                    NULL, &error);
    test_error(error, "Unable to create results buffer");

    size_t resultValuesSize = imageInfo->width * imageInfo->height
    * imageInfo->arraySize * sizeof(cl_int);
    ...
    bool allZeroes = true;
    for (size_t ic = 0;
         ic < imageInfo->width * imageInfo->height * imageInfo->arraySize; ++ic)
    {
        if (resultValues[ic])
        {
            allZeroes = false;
                                 ...
                                 ExplicitType outputType)
{
    ...
    else // kFloat
    {
        readFormat = "f";
        dataType =
        (format->image_channel_order == CL_DEPTH) ? "float" : "float4";
    }
    ...
    {
        sprintf(
        programSrc, read_write2DArrayKernelSourcePattern,
        (format->image_channel_order == CL_DEPTH) ? "image2d_array_depth_t"
                                                  : "image2d_array_t",
        (format->image_channel_order == CL_DEPTH) ? "image2d_array_depth_t"
                                                  : "image2d_array_t",
        dataType, readFormat, readFormat, readFormat);
    }
    else
    {
        sprintf(programSrc, read2DArrayKernelSourcePattern,
                ...
                : "image2d_array_t",
                dataType, readFormat, readFormat);
    }
    ...
    else if (gTestMaxImages)
    {
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            context, queue, kernel, &imageInfo, imageSampler, outputType, seed);
            if (retCode) return retCode;
        }
    }
    else
