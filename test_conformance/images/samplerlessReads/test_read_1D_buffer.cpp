...
"__kernel void sample_kernel( read_only image1d_buffer_t inputA, read_only "
"image1d_t inputB, sampler_t sampler, __global int *results )\n"
"{\n"
                              ...
                              ExplicitType outputType, MTdata d)
{
    ...
    clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                   imageInfo->rowPitch, imageValues, &error);
    if (error != CL_SUCCESS)
    {
    ...
    {
        log_error(
        "ERROR: Unable to create IMAGE1D_BUFFER of size %d pitch %d (%s)\n",
    ...
    cl_mem ret = NULL;
    error = clGetMemObjectInfo(image[0], CL_MEM_ASSOCIATED_MEMOBJECT,
                               sizeof(ret), &ret, NULL);
    if (error != CL_SUCCESS)
    {
    ...
    {
        log_error(
        "ERROR: clGetImageInfo for CL_IMAGE_BUFFER returned wrong value\n");
        return -1;
    }
                                  ...
                                  ExplicitType outputType)
{
    ...
    error |= clGetDeviceInfo(device, CL_DEVICE_IMAGE2D_MAX_WIDTH,
                             sizeof(maxWidth), &maxWidth1D, NULL);
    ...
    // So the test can't use the biggest possible size for image_buffer if it's
    // bigger than the max image1D size
    ...
    sprintf(programSrc, read1DBufferKernelSourcePattern, dataType, readFormat,
            readFormat);
