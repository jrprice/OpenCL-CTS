...
"__kernel void sample_kernel( read_only image1d_array_t input, sampler_t "
...
"__kernel void sample_kernel( read_only image1d_array_t read_only_image, "
"read_write image1d_array_t read_write_image, sampler_t sampler, __global int "
"*results )\n"
"{\n"
"   int tidX = get_global_id(0), tidY = get_global_id(1);\n"
                             ...
                             ExplicitType outputType, MTdata d)
{
    ...
    if (error != CL_SUCCESS)
    {
        log_error("ERROR: Unable to create read_only 1D image array of size %d "
                  "x %d pitch %d (%s)\n",
    ...
    if (gTestReadWrite)
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create read_write 1D image array of "
    ...
    context, 0, imageInfo->width * imageInfo->arraySize * sizeof(cl_int), NULL,
    &error);
    test_error(error, "Unable to create results buffer");

    size_t resultValuesSize =
    imageInfo->width * imageInfo->arraySize * sizeof(cl_int);
    ...
    if (gDebugTrace)
        log_info("    reading results, %ld kbytes\n",
                 (unsigned long)(imageInfo->width * imageInfo->arraySize
                                 * sizeof(cl_int) / 1024));
                                 ...
                                 ExplicitType outputType)
{
    ...
    if (gTestReadWrite)
    {
        sprintf(programSrc, read_write1DArrayKernelSourcePattern, dataType,
    ...
    else
    {
        for (int i = 0; i < NUM_IMAGE_ITERATIONS; i++)
        {
            ...
            do
            {
                ...
                size =
                (size_t)imageInfo.rowPitch * (size_t)imageInfo.arraySize * 4;
