                       ...
                       ExplicitType outputType, MTdata d)
{
    ...
    if (error != CL_SUCCESS)
    {
        log_error("ERROR: Unable to create 2D read_only image of size %d x %d "
                  "pitch %d (%s)\n",
    ...
    if (gTestReadWrite)
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create 2D read_write image of size %d "
                      "x %d pitch %d (%s)\n",
                      (int)imageInfo->width, (int)imageInfo->height,
                      (int)imageInfo->rowPitch, IGetErrorString(error));
            return error;
    ...
    context, 0, imageInfo->width * imageInfo->height * sizeof(cl_int), NULL,
    &error);
    test_error(error, "Unable to create results buffer");

    size_t resultValuesSize =
    imageInfo->width * imageInfo->height * sizeof(cl_int);
                           ...
                           ExplicitType outputType)
{
    ...
    if (gTestReadWrite)
    {
        sprintf(programSrc, read_write2DKernelSourcePattern,
    ...
    {
        sprintf(programSrc, read2DKernelSourcePattern,
                (format->image_channel_order == CL_DEPTH) ? "image2d_depth_t"
                                                          : "image2d_t",
