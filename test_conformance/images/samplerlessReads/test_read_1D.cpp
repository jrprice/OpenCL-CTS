...
"__kernel void sample_kernel( read_only image1d_t read_only_image, read_write "
"image1d_t read_write_image, sampler_t sampler, __global int *results )\n"
"{\n"
"   int tidX = get_global_id(0);\n"
"   int offset = tidX;\n"
                       ...
                       ExplicitType outputType, MTdata d)
{
    ...
    {
        log_error(
        "ERROR: Unable to create read_only 1D image of size %d pitch %d (%s)\n",
    ...
    if (gTestReadWrite)
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create read_write 1D image of size %d "
                      "pitch %d (%s)\n",
                      (int)imageInfo->width, (int)imageInfo->rowPitch,
                      IGetErrorString(error));
            return error;
    ...
    cl_mem results =
    clCreateBuffer(context, 0, imageInfo->width * sizeof(cl_int), NULL, &error);
                           ...
                           ExplicitType outputType)
{
    ...
    if (gTestReadWrite)
    {
        sprintf(programSrc, read_write1DKernelSourcePattern, dataType,
                readFormat, readFormat, readFormat);
    }
    else
    {
        sprintf(programSrc, read1DKernelSourcePattern, dataType, readFormat,
                readFormat);
    }
    ...
    else if (gTestMaxImages)
    {
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            int retCode = test_read_image_1D(context, queue, kernel, &imageInfo,
                                             imageSampler, outputType, seed);
            if (retCode) return retCode;
        }
    }
