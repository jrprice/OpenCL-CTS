...
extern bool gDebugTrace, gTestSmallImages, gEnablePitch, gTestMaxImages,
gDeviceLt20;
...
"__kernel void sample_kernel( read_only image3d_t read_only_image, read_write "
"image3d_t read_write_image, sampler_t sampler, __global int *results )\n"
"{\n"
"   int tidX = get_global_id(0), tidY = get_global_id(1), tidZ = "
"get_global_id(2);\n"
"   int offset = "
"tidZ*get_image_width(read_only_image)*get_image_height(read_only_image) + "
"tidY*get_image_width(read_only_image) + tidX;\n"
"   int4 coords = (int4)( tidX, tidY, tidZ, 0 );\n"
"   %s clr = read_image%s( read_only_image, sampler, coords );\n"
"   write_image%s(read_write_image, coords, clr);\n"
"   atomic_work_item_fence(CLK_IMAGE_MEM_FENCE, memory_order_acq_rel, "
                       ...
                       ExplicitType outputType, MTdata d)
{
    ...
    clCreateImage(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                  imageInfo->format, &image_desc, imageValues, &error);
    if (error != CL_SUCCESS)
    {
        log_error("ERROR: Unable to create read_only 3D image of size %d x %d "
                  "x %d (pitch %d, %d ) (%s)",
    ...
    if (gTestReadWrite)
    {
        ...
        if (error != CL_SUCCESS)
        {
            log_error("ERROR: Unable to create read_write 3D image of size %d "
                      "x %d x %d (pitch %d, %d ) (%s)",
                      (int)imageInfo->width, (int)imageInfo->height,
                      (int)imageInfo->depth, (int)imageInfo->rowPitch,
                      (int)imageInfo->slicePitch, IGetErrorString(error));
    ...
    actualSampler = clCreateSampler(context, CL_FALSE, CL_ADDRESS_NONE,
                                    CL_FILTER_NEAREST, &error);
    test_error(error, "Unable to create image sampler");

    // Create results buffer
    cl_mem results = clCreateBuffer(context, 0,
                                    imageInfo->width * imageInfo->height
                                    * imageInfo->depth * sizeof(cl_int),
                                    NULL, &error);
    test_error(error, "Unable to create results buffer");

    size_t resultValuesSize =
    imageInfo->width * imageInfo->height * imageInfo->depth * sizeof(cl_int);
    ...
    {
        error =
        clSetKernelArg(kernel, idx++, sizeof(cl_mem), &read_write_image);
        test_error(error, "Unable to set kernel arguments");
    }
    ...
    // Run the kernel
    error = clEnqueueNDRangeKernel(queue, kernel, 3, NULL, threads, NULL, 0,
                                   NULL, NULL);
    test_error(error, "Unable to run kernel");

    if (gDebugTrace)
        log_info("    reading results, %ld kbytes\n",
                 (unsigned long)(imageInfo->width * imageInfo->height
                                 * imageInfo->depth * sizeof(cl_int) / 1024));

    // Get results
    error = clEnqueueReadBuffer(queue, results, CL_TRUE, 0, resultValuesSize,
                                resultValues, 0, NULL, NULL);
    ...
    bool allZeroes = true;
    size_t ic;
    for (ic = 0; ic < imageInfo->width * imageInfo->height * imageInfo->depth;
         ++ic)
    {
        if (resultValues[ic])
        {
    ...
    {
        log_error(
        " Sampler-less reads differ from reads with sampler at index %lu.\n",
        ic);
        return -1;
    }
                           ...
                           ExplicitType outputType)
{
    ...
    if (gTestReadWrite)
    {
        sprintf(programSrc, read_write3DKernelSourcePattern, dataType,
                readFormat, readFormat, readFormat);
    }
    else
    {
        sprintf(programSrc, read3DKernelSourcePattern, dataType, readFormat,
                readFormat);
    }
        ...
        for (size_t idx = 0; idx < numbeOfSizes; idx++)
        {
            ...
            if (gDebugTrace)
                log_info("   at max size %d,%d,%d\n", (int)sizes[idx][0],
                         (int)sizes[idx][1], (int)sizes[idx][2]);
            int retCode = test_read_image_3D(context, queue, kernel, &imageInfo,
                                             imageSampler, outputType, seed);
            if (retCode) return retCode;
        }
    }
