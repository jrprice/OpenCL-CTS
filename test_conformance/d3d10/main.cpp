...
int main(int argc, const char* argv[])
{
    ...
    NonTestRequire(num_devices_tested,
                   "No D3D10 compatible cl_device_ids were found.");

    HarnessD3D10_TestStats();
}

void TestAdapterEnumeration(cl_platform_id platform, IDXGIAdapter* pAdapter,
                            ID3D10Device* pDevice, cl_uint* num_devices)
{
    ...
    // get the cl_device_ids for the adapter
    {
        ...
        else
        {
            ...
            result = clGetDeviceIDsFromD3D10KHR(
            platform, CL_D3D10_DXGI_ADAPTER_KHR, pAdapter,
            CL_ALL_DEVICES_FOR_D3D10_KHR, num_adapter_devices, adapter_devices,
            NULL);
    ...
    {
        result = clGetDeviceIDsFromD3D10KHR(
        platform, CL_D3D10_DEVICE_KHR, pDevice, CL_ALL_DEVICES_FOR_D3D10_KHR, 0,
        NULL, &num_device_devices);
        TestRequire((result == CL_SUCCESS || result == CL_DEVICE_NOT_FOUND),
                    "clGetDeviceIDsFromD3D10KHR failed.");
        ...
        else
        {
            ...
            result = clGetDeviceIDsFromD3D10KHR(
            platform, CL_D3D10_DEVICE_KHR, pDevice,
            CL_ALL_DEVICES_FOR_D3D10_KHR, num_device_devices, device_devices,
            NULL);
            TestRequire((result == CL_SUCCESS),
                        "clGetDeviceIDsFromD3D10KHR failed.");
        }

        // require that each cl_device_id returned for the ID3D10Device was
        // among the devices listed for the adapter
        for (cl_uint device_device = 0; device_device < num_device_devices;
             ++device_device)
        {
            cl_uint adapter_device;
            for (adapter_device = 0; adapter_device < num_adapter_devices;
                 ++adapter_device)
            {
                if (device_devices[device_device]
                    == adapter_devices[adapter_device])
                {
                    break;
                }
            }
            TestRequire((adapter_device != num_adapter_devices),
                        "CL_D3D10_DEVICE_KHR devices not a subset of "
                        "CL_D3D10_DXGI_ADAPTER_KHR devices");
        }
    }
...
void TestAdapterDevices(cl_platform_id platform, IDXGIAdapter* pAdapter,
                        ID3D10Device* pDevice, cl_uint num_devices_expected)
{
    cl_int result;
    cl_uint num_devices = 0;
    ...
    NonTestRequire(devices, "Memory allocation failure.");

    result = clGetDeviceIDsFromD3D10KHR(
    platform, CL_D3D10_DEVICE_KHR, pDevice, CL_ALL_DEVICES_FOR_D3D10_KHR,
    num_devices_expected, devices, &num_devices);
    ...
    for (cl_uint i = 0; i < num_devices; ++i)
    {
        ...
        {
            TestPrint(
            "Device does not support cl_khr_d3d10_sharing extension\n");
            continue;
        }
...
void TestDevice(cl_device_id device, ID3D10Device* pDevice)
{
    ...
    result = clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(device_name),
                             device_name, NULL);
    NonTestRequire(CL_SUCCESS == result,
                   "clGetDeviceInfo with CL_DEVICE_NAME failed");
    TestPrint("--------------------\n");
    ...
    context, CL_CONTEXT_D3D10_PREFER_SHARED_RESOURCES_KHR,
    sizeof(prefer_shared_resources), &prefer_shared_resources, NULL);
    NonTestRequire(CL_SUCCESS == result,
                   "clGetContextInfo with "
                   "CL_CONTEXT_D3D10_PREFER_SHARED_RESOURCES_KHR failed");

    // run buffer tests
    TestDeviceBuffer(context, command_queue, pDevice);

    // run 2D texture tests
    TestDeviceTexture2D(device, context, command_queue, pDevice);

    // run 3D texture tests
    TestDeviceTexture3D(device, context, command_queue, pDevice);

    // run misc tests
    TestDeviceMisc(device, context, command_queue, pDevice);

    clReleaseContext(context);
    clReleaseCommandQueue(command_queue);
}

bool TestDeviceContextCreate(cl_device_id device, ID3D10Device* pDevice,
