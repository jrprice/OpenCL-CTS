...
const char* texture3DPatterns[2][2][2] = {
    {
...
void SubTestTexture3D(cl_context context, cl_command_queue command_queue,
                      ID3D10Device* pDevice, const TextureFormat* format,
                      const Texture3DSize* size)
{
            ...
            for (UINT y = 0; y < 2; ++y)
                for (UINT z = 0; z < 2; ++z)
                {
                    ...
                    ID3D10Texture3D* pStagingBuffer = NULL;
                    {
                        ...
                        desc.MiscFlags = 0;
                        hr =
                        pDevice->CreateTexture3D(&desc, NULL, &pStagingBuffer);
                        TestRequire(SUCCEEDED(hr), "CreateTexture3D failed.");
                    }
                    ...
                    // write the data to the staging buffer
                    {
                        ...
                        hr = pStagingBuffer->Map(0, D3D10_MAP_READ_WRITE, 0,
                                                 &mappedTexture);
                        memcpy(mappedTexture.pData, texture3DPatterns[x][y][z],
                    ...
                    // copy the data to to the texture
                    {
                        ...
                        z ? subResourceInfo[i].depth - 1 : 0, pStagingBuffer, 0,
                        &box);
                    }
    ...
    for (UINT i = 0; i < size->SubResourceCount; ++i)
    {
        ...
        TestRequire(result == CL_SUCCESS,
                    "clCreateFromD3D10Texture3DKHR failed");
        ...
        // query depth
        size_t depth;
        result = clGetImageInfo(subResourceInfo[i].mem, CL_IMAGE_DEPTH,
                                sizeof(depth), &depth, NULL);
        TestRequire(result == CL_SUCCESS,
                    "clGetImageInfo for CL_IMAGE_DEPTH failed");
        TestRequire(
        depth == subResourceInfo[i].depth,
        "clGetImageInfo for CL_IMAGE_DEPTH returned incorrect value.");
    }
    ...
    // acquire the resources for OpenCL
    {
        ...
        command_queue, size->SubResourceCount, memToAcquire, 0, NULL, NULL);
        TestRequire(result == CL_SUCCESS,
        ...
        for (UINT x = 0; x < 2; ++x)
            for (UINT y = 0; y < 2; ++y)
                for (UINT z = 0; z < 2; ++z)
                {
                    ...
                    command_queue, subResourceInfo[i].mem,
                    subResourceInfo[i].mem, src, dst, region, 0, NULL, NULL);
                    TestRequire(result == CL_SUCCESS,
                                "clEnqueueCopyImage failed.");
                }
        ...
        for (UINT x = 0; x < 2; ++x)
            for (UINT y = 0; y < 2; ++y)
                for (UINT z = 0; z < 2; ++z)
                {
                    ...
                    // wipe out the staging buffer to make sure we don't get
                    // stale values
                    {
                        D3D10_MAPPED_TEXTURE3D mappedTexture;
                        hr = pStagingBuffer->Map(0, D3D10_MAP_READ_WRITE, 0,
                    ...
                    // copy the pixel to the staging buffer
                    {
                        ...
                        box.front = z ? subResourceInfo[i].depth - 2 : 1;
...
void TestDeviceTexture3D(cl_device_id device, cl_context context,
                         cl_command_queue command_queue, ID3D10Device* pDevice)
{
    ...
    {
        SubTestTexture3D(context, command_queue, pDevice, &formats[format],
